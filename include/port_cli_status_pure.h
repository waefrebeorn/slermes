 ...
 * snapshot_json: JSON object with optional keys goal_active / goal_turns_used
 * / goal_max_turns. Returns malloc'd string; caller frees. */
char *cli_status_goal_segment(const char *snapshot_json);

/* PoP: _fmt_stash_age @ cli.py:_fmt_stash_age */
/* Human-readable age string for a stash entry ("just now", "42s ago",
