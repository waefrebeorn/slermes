# Battleship v49 — Active Gap Map (DA-Corrected)

**Last sweep: Phase 582 — S13 Fuzz corpus expansion**
**Total active gaps: ~69** (Fuzz corpus expanded v582)

**FIXED this phase:**
- S13 Fuzz corpus (P2): **expanded from 179→180 assertions**. Added `test_json_unicode_edge()` (+8 unicode edge cases), +15 JSON edge cases, +4 JSON5 edge cases in `tests/test_fuzz.c`. Fuzz 180/0 passing.
...
**FIXED:** C01 (ASan), C04 (stringop truncation), C06 (credential audit), ~~C13 #2 Null safety~~ ✅ (v572), ~~C14 #3 Thread safety~~ ✅ (v580), ~~FD leaks~~ ✅ STALE (v581), ~~Stack buf audit~~ ✅ (v581), ~~Fuzz corpus~~ ✅ (v582).
**VERIFIED STALE:** signed vs unsigned — 0 warnings across codebase. FD leaks — 0 real leaks across 137 source files.
**REMAINING:** Error path coverage (P2).
