"""AUTO-GENERATED integration oracle for port_tools_url_safety (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['tools.url_safety'] = _load('tools/url_safety.py')

DISPATCH = {
    'cli_tools_url_safety__is_blocked_ip': ('tools.url_safety', '_is_blocked_ip'),
    'cli_tools_url_safety_is_always_blocked_url': ('tools.url_safety', 'is_always_blocked_url'),
    'cli_tools_url_safety_is_safe_url': ('tools.url_safety', 'is_safe_url'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_tools_url_safety.py <cases.json>\n"); return 2
