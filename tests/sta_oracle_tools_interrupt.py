"""AUTO-GENERATED oracle for tools_interrupt (gen_oracle.py)."""
import sys, json, os
sys.path.insert(0, os.path.expanduser("~/hermes-agent-dev"))
from tools.interrupt import (_ThreadAwareEventProxy)
...
for line in sys.stdin:
    ...
    fn = rec['func']
    ARGS = {
        'tools_interrupt__ThreadAwareEventProxy_is_set': ('_ThreadAwareEventProxy', []),
    }
    if fn not in ARGS:
        continue
    pyf, pargs = ARGS[fn]
