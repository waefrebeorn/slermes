...
static json_t *emit_file_ops_has_bom(const json_t *c){
    ...
    bool v = (bool)file_ops_has_bom(value);
...
static json_t *emit_file_ops_is_line_oriented_newline_error(const json_t *c){
    const char *value = json_get_str(c, "value", "");
    bool v = (bool)file_ops_is_line_oriented_newline_error(value);
...
static json_t *emit_file_ops_pattern_has_regex_newline(const json_t *c){
    const char *value = json_get_str(c, "value", "");
    bool v = (bool)file_ops_pattern_has_regex_newline(value);
...
int main(int argc, char **argv){
    ...
    for (int i = 0; i < n; i++){
        ...
        else if (strcmp(op, "file_ops_is_line_oriented_newline_error") == 0) o = emit_file_ops_is_line_oriented_newline_error(c);
        else if (strcmp(op, "file_ops_pattern_has_regex_newline") == 0) o = emit_file_ops_pattern_has_regex_newline(c);
