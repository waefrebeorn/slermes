"""AUTO-GENERATED integration oracle for port_lifecycle_guard (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['cron.lifecycle_guard'] = _load('cron/lifecycle_guard.py')

DISPATCH = {
    'cron_lifecycle_contains_gateway_lifecycle_command': ('cron.lifecycle_guard', 'check_gateway_lifecycle'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_lifecycle_guard.py <cases.json>\n"); return 2
