"""AUTO-GENERATED integration oracle for port_agent_secret_scope (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['agent.secret_scope'] = _load('agent/secret_scope.py')

DISPATCH = {
    'secret_scope_is_global_env_fn': ('agent.secret_scope', '_is_global_env'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_agent_secret_scope.py <cases.json>\n"); return 2
