"""AUTO-GENERATED integration oracle for port_tools_managed_tool_gateway (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['tools.managed_tool_gateway'] = _load('tools/managed_tool_gateway.py')

DISPATCH = {
    'cli_tools_managed_tool_gateway_is_managed_tool_gateway_ready': ('tools.managed_tool_gateway', 'is_managed_tool_gateway_ready'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_tools_managed_tool_gateway.py <cases.json>\n"); return 2
