"""AUTO-GENERATED integration oracle for port_context_breakdown_helpers (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['agent.context_breakdown'] = _load('agent/context_breakdown.py')

DISPATCH = {
    'context_breakdown_chars_to_tokens': ('agent.context_breakdown', '_chars_to_tokens'),
    'context_breakdown_json_tokens': ('agent.context_breakdown', '_json_tokens'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_context_breakdown_helpers.py <cases.json>\n"); return 2
