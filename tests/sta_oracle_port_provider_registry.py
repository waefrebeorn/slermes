"""AUTO-GENERATED integration oracle for port_provider_registry (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['hermes_cli.runtime_provider'] = _load('hermes_cli/runtime_provider.py')

DISPATCH = {
    'provider_loopback_hostname': ('hermes_cli.runtime_provider', '_loopback_hostname'),
    'provider_anthropic_base_url_override_ok': ('hermes_cli.runtime_provider', '_anthropic_base_url_override_ok'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_provider_registry.py <cases.json>\n"); return 2
