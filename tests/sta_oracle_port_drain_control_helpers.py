"""AUTO-GENERATED integration oracle for port_drain_control_helpers (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['gateway.drain_control'] = _load('gateway/drain_control.py')

DISPATCH = {
    'drain_control_notification_suppressed': ('gateway.drain_control', 'drain_notification_suppressed'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_drain_control_helpers.py <cases.json>\n"); return 2
