"""AUTO-GENERATED integration oracle for port_web_server_managed_files (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['hermes_cli.web_server'] = _load('hermes_cli/web_server.py')

DISPATCH = {
    'ws_is_sensitive_filename': ('hermes_cli.web_server', '_is_sensitive_filename'),
    'ws_is_sensitive_path': ('hermes_cli.web_server', '_is_sensitive_path'),
    'ws_chat_image_extension_allowed': ('hermes_cli.web_server', '_is_sensitive_path'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_web_server_managed_files.py <cases.json>\n"); return 2
