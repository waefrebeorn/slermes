"""AUTO-GENERATED integration oracle for port_tools_browser_camofox (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['tools.browser_camofox'] = _load('tools/browser_camofox.py')

DISPATCH = {
    '_is_loopback_hostname': ('tools.browser_camofox', '_is_loopback_hostname'),
    '_camofox_private_page_block': ('tools.browser_camofox', '_camofox_private_page_block'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_tools_browser_camofox.py <cases.json>\n"); return 2
