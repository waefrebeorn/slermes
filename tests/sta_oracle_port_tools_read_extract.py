"""AUTO-GENERATED integration oracle for port_tools_read_extract (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['tools.read_extract'] = _load('tools/read_extract.py')

DISPATCH = {
    'read_extract_is_extractable_document': ('tools.read_extract', 'is_extractable_document'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_tools_read_extract.py <cases.json>\n"); return 2
