"""AUTO-GENERATED integration oracle for port_agent_replay_cleanup (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['agent.replay_cleanup'] = _load('agent/replay_cleanup.py')

DISPATCH = {
    'agent_replay_cleanup_is_interrupted_tool_result': ('agent.replay_cleanup', 'is_interrupted_tool_result'),
    'agent_replay_cleanup_is_dangerous_confirmation': ('agent.replay_cleanup', 'is_dangerous_confirmation'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_agent_replay_cleanup.py <cases.json>\n"); return 2
