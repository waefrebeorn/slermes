...
static json_t *emit_config_coerce_ssl_verify(const json_t *c){
    ...
    long v = (long)config_coerce_ssl_verify(value);
...
static json_t *emit_config_is_env_config_key(const json_t *c){
    const char *value = json_get_str(c, "value", "");
    int v = (int)config_is_env_config_key(value);
...
int main(int argc, char **argv){
    ...
    for (int i = 0; i < n; i++){
        ...
        if (strcmp(op, "config_coerce_config_version") == 0) o = emit_config_coerce_config_version(c);
        if (strcmp(op, "config_is_env_config_key") == 0) o = emit_config_is_env_config_key(c);
