"""AUTO-GENERATED integration oracle for port_verification_stop_helpers (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['agent.verification_stop'] = _load('agent/verification_stop.py')

DISPATCH = {
    'verification_stop_is_non_code_path': ('agent.verification_stop', '_is_non_code_path'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_verification_stop_helpers.py <cases.json>\n"); return 2
