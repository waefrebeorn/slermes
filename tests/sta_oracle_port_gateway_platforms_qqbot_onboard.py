"""AUTO-GENERATED integration oracle for port_gateway_platforms_qqbot_onboard (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['gateway.platforms.qqbot.onboard'] = _load('gateway/platforms/qqbot/onboard.py')

DISPATCH = {
    'qqbot_onboard__render_qr': ('gateway.platforms.qqbot.onboard', '_render_qr'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_gateway_platforms_qqbot_onboard.py <cases.json>\n"); return 2
