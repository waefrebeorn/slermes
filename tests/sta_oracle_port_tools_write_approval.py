"""AUTO-GENERATED integration oracle for port_tools_write_approval (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['tools.write_approval'] = _load('tools/write_approval.py')

DISPATCH = {
    'cli_tools_write_approval_pending_count': ('tools.write_approval', 'pending_count'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_tools_write_approval.py <cases.json>\n"); return 2
