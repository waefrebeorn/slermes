"""AUTO-GENERATED integration oracle for port_tools_env_passthrough (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['tools.env_passthrough'] = _load('tools/env_passthrough.py')

DISPATCH = {
    'cli_tools_env_passthrough_is_env_passthrough': ('tools.env_passthrough', 'is_env_passthrough'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_tools_env_passthrough.py <cases.json>\n"); return 2
