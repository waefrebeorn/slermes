"""AUTO-GENERATED integration oracle for port_scale_to_zero_helpers (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['gateway.scale_to_zero'] = _load('gateway/scale_to_zero.py')

DISPATCH = {
    'scale_to_zero_enabled': ('gateway.scale_to_zero', 'scale_to_zero_enabled'),
    'scale_to_zero_messaging_is_relay_only_or_absent': ('gateway.scale_to_zero', 'messaging_is_relay_only_or_absent'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_scale_to_zero_helpers.py <cases.json>\n"); return 2
