"""AUTO-GENERATED integration oracle for port_skill_usage_wrappers (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['tools.skill_usage'] = _load('tools/skill_usage.py')

DISPATCH = {
    'su_is_protected_builtin': ('tools.skill_usage', 'is_protected_builtin'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_skill_usage_wrappers.py <cases.json>\n"); return 2
