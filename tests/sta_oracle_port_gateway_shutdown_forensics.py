"""AUTO-GENERATED integration oracle for port_gateway_shutdown_forensics (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['gateway.shutdown_forensics'] = _load('gateway/shutdown_forensics.py')

DISPATCH = {
    'cli_gateway_shutdown_forensics__parse_systemd_duration_to_us': ('gateway.shutdown_forensics', '_parse_systemd_duration_to_us'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_gateway_shutdown_forensics.py <cases.json>\n"); return 2
