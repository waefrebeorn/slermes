"""AUTO-GENERATED integration oracle for port_transcription_wrappers (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['tools.transcription_tools'] = _load('tools/transcription_tools.py')

DISPATCH = {
    'tsc_try_lazy_install_stt': ('tools.transcription_tools', '_try_lazy_install_stt'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_transcription_wrappers.py <cases.json>\n"); return 2
