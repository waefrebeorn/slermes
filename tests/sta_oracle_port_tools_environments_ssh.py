"""AUTO-GENERATED integration oracle for port_tools_environments_ssh (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['tools.environments.ssh'] = _load('tools/environments/ssh.py')

DISPATCH = {
    'cli_tools_environments_ssh__ensure_remote_dirs': ('tools.environments.ssh', '_ensure_remote_dirs'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_tools_environments_ssh.py <cases.json>\n"); return 2
