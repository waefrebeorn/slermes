"""AUTO-GENERATED integration oracle for port_agent_verify_hooks (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['agent.verify_hooks'] = _load('agent/verify_hooks.py')

DISPATCH = {
    'agent_verify_hooks_max_verify_nudges': ('agent.verify_hooks', 'max_verify_nudges'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_agent_verify_hooks.py <cases.json>\n"); return 2
