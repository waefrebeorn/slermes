"""AUTO-GENERATED oracle for tools_skill_usage (gen_oracle.py)."""
import sys, json, os
sys.path.insert(0, os.path.expanduser("~/hermes-agent-dev"))
from tools.skills_sync import (is_bundled_skills_opt_out)
...
for line in sys.stdin:
    ...
    fn = rec['func']
    ARGS = {
        'is_bundled_skills_opt_out': ('is_bundled_skills_opt_out', []),
    }
    if fn not in ARGS:
        continue
    pyf, pargs = ARGS[fn]
