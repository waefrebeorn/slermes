"""AUTO-GENERATED integration oracle for port_gateway_platforms_qqbot_keyboards (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['gateway.platforms.qqbot.keyboards'] = _load('gateway/platforms/qqbot/keyboards.py')

DISPATCH = {
    'cli_gateway_platforms_qqbot_keyboards_parse_update_prompt_button_data': ('gateway.platforms.qqbot.keyboards', 'parse_update_prompt_button_data'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_gateway_platforms_qqbot_keyboards.py <cases.json>\n"); return 2
