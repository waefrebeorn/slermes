"""AUTO-GENERATED integration oracle for port_agent_browser_provider_methods (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['agent.browser_provider'] = _load('agent/browser_provider.py')

DISPATCH = {
    'browser_provider_close_session': ('agent.browser_provider', 'close_session'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_agent_browser_provider_methods.py <cases.json>\n"); return 2
