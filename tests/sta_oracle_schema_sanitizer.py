#!/usr/bin/env python3
"""Oracle: tools/schema_sanitizer strip_* + sanitize_tool_schemas vs LIVE Python.

Usage: python3 sta_oracle_schema_sanitizer.py <tools.json> <pf|se|san>
Prints the same shape the C harness (t_port_schema_sanitizer) emits so the
two can be diffed directly. Key order is preserved (both Python and libjson
keep insertion order), so the byte diff is meaningful.
"""
import sys
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("usage: sta_oracle_schema_sanitizer.py <tools.json> <pf|se|san>", file=sys.stderr)
        sys.exit(2)
