"""AUTO-GENERATED integration oracle for port_webhook_helpers (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['gateway.platforms.webhook'] = _load('gateway/platforms/webhook.py')

DISPATCH = {
    'webhook_is_loopback_host': ('gateway.platforms.webhook', '_is_loopback_host'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_webhook_helpers.py <cases.json>\n"); return 2
