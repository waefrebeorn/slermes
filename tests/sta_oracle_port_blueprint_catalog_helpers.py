"""AUTO-GENERATED integration oracle for port_blueprint_catalog_helpers (gen_integration_oracle.py)."""
...
def _load(rel):
    ...
    for base in sys.path:
        ...
        try:
            ...
            continue
    return None
MODS['cron.blueprint_catalog'] = _load('cron/blueprint_catalog.py')

DISPATCH = {
    'blueprint_catalog_validate_slot_type': ('cron.blueprint_catalog', '__post_init__'),
}

def main():
    if len(sys.argv) < 2: sys.stderr.write("usage: sta_oracle_port_blueprint_catalog_helpers.py <cases.json>\n"); return 2
