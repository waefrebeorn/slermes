...
test("normalizeActiveWork drops junk and keeps the count at least the title count", () => {
  ...
  assert.deepEqual(normalizeActiveWork({ count: "many", titles: "nope" }), {
    count: 0,
    titles: [],
  });
  assert.deepEqual(
    normalizeActiveWork({ count: -3, titles: ["  Fix login  ", "", 7] }),
    {
      count: 1,
      titles: ["Fix login"],
    },
...
test("normalizeActiveWork keeps untitled sessions in the count", () => {
  assert.deepEqual(normalizeActiveWork({ count: 3, titles: ["Fix login"] }), {
    count: 3,
    titles: ["Fix login"],
  });
});
