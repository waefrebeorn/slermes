...
test("resolveReadinessProbeAuth reuses the oauth bearer-vs-cookie choice", () => {
  assert.deepEqual(resolveReadinessProbeAuth("oauth", "native-at"), {
    kind: "bearer",
    token: "native-at",
  });
  assert.deepEqual(resolveReadinessProbeAuth("oauth", null), {
    kind: "cookie",
  });
...
test("resolveReadinessProbeAuth sends the session token for a token gateway", () => {
  ...
  assert.deepEqual(resolveReadinessProbeAuth("token", null, null), {
    kind: "token",
    token: null,
  });
});
...
test("resolveReadinessProbeAuth stays public for local and unknown modes", () => {
  ...
  assert.deepEqual(
    resolveReadinessProbeAuth("local", "native-at", "session-token"),
    { kind: "public" },
  );
  assert.deepEqual(
    resolveReadinessProbeAuth(undefined, "native-at", "session-token"),
    { kind: "public" },
  );
  assert.deepEqual(resolveReadinessProbeAuth("something-new", null, null), {
    kind: "public",
  });
});
...
test("oauthGuardMayHardFail is false only when EVERY provider is password-based", () => {
  assert.equal(
    oauthGuardMayHardFail([{ name: "basic", supportsPassword: true }]),
    false,
  );
  assert.equal(
    oauthGuardMayHardFail([
...
test("oauthGuardMayHardFail keeps the strict guard for oauth and mixed deployments", () => {
  assert.equal(
    oauthGuardMayHardFail([{ name: "nous", supportsPassword: false }]),
    true,
  );
  assert.equal(
    oauthGuardMayHardFail([
...
test("resolveGatedDownloadAuth matches oauth REST: bearer first, then cookie", () => {
  assert.deepEqual(resolveGatedDownloadAuth("oauth", "native-at"), {
    kind: "bearer",
    token: "native-at",
  });
...
test("resolveGatedDownloadAuth uses the session token for token and local modes", () => {
  assert.deepEqual(
    resolveGatedDownloadAuth("token", "native-at", "session-token"),
    {
      kind: "token",
      token: "session-token",
    },
  );
  assert.deepEqual(resolveGatedDownloadAuth("local", null, "sess"), {
    kind: "token",
    token: "sess",
  });
  assert.deepEqual(resolveGatedDownloadAuth(undefined, null, null), {
    kind: "token",
    token: null,
  });
});
