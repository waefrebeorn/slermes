...
test("screen cursor becomes a point relative to the window", () => {
  assert.deepEqual(cursorPointInWindow({ x: 300, y: 200 }, BOUNDS, 1), {
    x: 0,
    y: 0,
  });
  assert.deepEqual(cursorPointInWindow({ x: 460, y: 240 }, BOUNDS, 1), {
    x: 160,
    y: 40,
  });
});
...
test("the far edges belong to the window on entry, not on exit", () => {
  assert.deepEqual(cursorPointInWindow({ x: 919, y: 519 }, BOUNDS, 1), {
    x: 619,
    y: 319,
  });
...
test("a nonsense zoom factor is treated as unzoomed rather than poisoning the point", () => {
  for (const bad of [0, -1, Number.NaN, Number.POSITIVE_INFINITY]) {
    assert.deepEqual(cursorPointInWindow({ x: 460, y: 240 }, BOUNDS, bad), {
      x: 160,
      y: 40,
    });
  }
