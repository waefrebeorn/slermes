...
): SecretStoragePolicyIo & { fileText: () => string | null } {
  let text = initial;
...
test("corrupt or non-object policy file reads as the default", () => {
  for (const bad of ["not-json", "[]", '"on"', "null", "123"]) {
    assert.deepEqual(readSecretStoragePolicy(fakeIo(bad)), {
      on: false,
      migrated: false,
    });
  }
...
test("safeStorage blob with encryption ON is keep", () => {
  assert.equal(
    classifyStoredSecret(SAFE_BLOB, { on: true, migrated: true }),
    "keep",
  );
});
...
test("safeStorage blob, encryption OFF, pre-migration is migrate", () => {
  assert.equal(
    classifyStoredSecret(SAFE_BLOB, { on: false, migrated: false }),
    "migrate",
  );
});
...
test("safeStorage blob, encryption OFF, post-migration is drop — never touch the keychain again", () => {
  assert.equal(
    classifyStoredSecret(SAFE_BLOB, { on: false, migrated: true }),
    "drop",
  );
});
