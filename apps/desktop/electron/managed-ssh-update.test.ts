...
test("ManagedConnectionUpdateGate blocks new dials but admits the exact restoring transaction", () => {
  ...
  assert.equal(
    gate.claim("homelab", "22345678-1234-4678-9234-567812345678"),
    false,
  );
...
test("durable recovery claim survives memory release and fences ordinary dials, edits, and removal", () => {
  ...
  const gate = new ManagedConnectionUpdateGate((id) =>
    id === "homelab" ? durable : null,
  );
...
test("inactive SSH crash recovery keeps ordinary dials fenced until positive clearance", async () => {
  ...
  const relaunchedGate = new ManagedConnectionUpdateGate((id) =>
    id === "homelab" ? durableOwner : null,
  );
...
test("managed update joins a pre-claim bootstrap until its final gate check rolls back the serve", async () => {
  ...
  startPromise.catch(() => {});
  const barrier = waitForManagedSshBootstrapFence(
    coordinator.active,
    "homelab",
  );
...
test("registry-qualified primary token adoption stays reusable across consecutive launches", () => {
  ...
  for (const servedToken of [
    "served-on-first-launch",
    "served-on-second-launch",
  ]) {
...
test("actual primary and matching bare/composite pools keep distinct managed scope roles", () => {
  ...
  assert.equal(
    managedSshScopeRole({ ...base, key: "conn:homelab::research" }),
    "pool",
  );
});
...
test("POSIX managed launcher is detached, correlation-scoped, and never publishes handoff exit 75", () => {
  ...
  assert.match(
    command,
    new RegExp(`HERMES_UPDATE_CORRELATION_ID=.*${CORRELATION}`),
  );
...
test("Windows managed launcher starts a hidden child and leaves exit 75 to the external coordinator", () => {
  ...
  const outer = Buffer.from(command.split(" ").at(-1) || "", "base64").toString(
    "utf16le",
  );
  ...
  assert.ok(
    wrapperBase64,
    "outer launcher carries a separately encoded detached wrapper",
  );
  ...
  assert.ok(
    wrapper.indexOf("update_launch_intent") < wrapper.indexOf("update --yes"),
  );
    ...
    () =>
      parseRemoteUpdateObservation(
        observation({
          exitCode: 0,
          receipt: {
            correlationId: "22345678-1234-4678-9234-567812345678",
            outcome: "success",
          },
        }),
        CORRELATION,
...
test("managed observer unwraps a named profile home for the install-wide marker", async () => {
  const root = await mkdtemp(
    path.join(os.tmpdir(), "hermes-managed-profile-marker-"),
  );
...
test("live or malformed remote markers fail actionably at bounded update and recovery deadlines", async () => {
  for (const marker of ["live", "malformed"] as const) {
    ...
    await assert.rejects(
      waitForManagedRemoteUpdate(target, CORRELATION, clock),
      /services remain stopped/,
    );
    now = 0;
    await assert.rejects(
      waitForManagedRemoteClearance(target, CORRELATION, clock),
...
test("remote launch intent fences crash recovery even before the local journal records launch proof", async () => {
  ...
  const target = {
    ...
    ssh: {
      exec: async () =>
        observation({ marker: "absent", launchIntent: "present" }),
    },
  };
  ...
  target.ssh.exec = async () =>
    observation({ marker: "absent", launchIntent: "absent" });
  await assert.doesNotReject(
    waitForManagedRemoteClearance(target, CORRELATION, { timeoutMs: 0 }),
  );
  target.ssh.exec = async () =>
    observation({ marker: "absent", launchIntent: "dead" });
  await assert.doesNotReject(
    waitForManagedRemoteClearance(target, CORRELATION, {
      requireTerminal: true,
      timeoutMs: 0,
