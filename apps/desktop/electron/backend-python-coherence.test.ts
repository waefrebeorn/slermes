...
test("createPythonBackend derives venvRoot from the selected interpreter, not a hardcoded venv path", () => {
  ...
  const derivesFromPython =
    /findPythonForRoot|python/.test(fn) && !hardcodesVenv;
...
export function coherentVenvRootForPython(
  pythonPath: string,
  root: string,
): string | null {
...
test("coherentVenvRootForPython maps a selected interpreter back to ITS venv root", () => {
  assert.equal(
    coherentVenvRootForPython("/repo/.venv/bin/python", "/repo"),
    "/repo/.venv",
  );
  assert.equal(
    coherentVenvRootForPython("/repo/venv/bin/python", "/repo"),
