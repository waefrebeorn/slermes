...
function fakeGlobalShortcut(
  options: { register?: boolean; taken?: string[] } = {},
) {
...
describe("parseQuickEntryShortcut", () => {
  it("normalizes casing, aliases, and modifier order", () => {
    ...
    expect(parseQuickEntryShortcut("  Shift + CTRL + k ")).toEqual({
      accelerator: "Control+Shift+K",
      ok: true,
    });
    expect(parseQuickEntryShortcut("Alt+f5")).toEqual({
      accelerator: "Alt+F5",
      ok: true,
    });
    expect(parseQuickEntryShortcut("Meta+/")).toEqual({
      accelerator: "Super+/",
      ok: true,
    });
  });
  ...
  it("collapses duplicate modifiers", () => {
    expect(parseQuickEntryShortcut("Ctrl+Control+Shift+J")).toEqual({
  ...
  it("requires a modifier so a global bind cannot swallow a bare key", () => {
    expect(parseQuickEntryShortcut("K")).toEqual({
      ok: false,
      reason: "no-modifier",
    });
    expect(parseQuickEntryShortcut("Space")).toEqual({
      ok: false,
      reason: "no-modifier",
    });
  });
  ...
  it("requires exactly one non-modifier key", () => {
    expect(parseQuickEntryShortcut("Shift+Control")).toEqual({
      ok: false,
      reason: "no-key",
    });
    expect(parseQuickEntryShortcut("Shift+A+B")).toEqual({
      ok: false,
      reason: "invalid-key",
    });
    expect(parseQuickEntryShortcut("A+Shift")).toEqual({
      ok: false,
  ...
  it("rejects empty, junk, and the reserved Escape key", () => {
    ...
    expect(parseQuickEntryShortcut("   ")).toEqual({
      ok: false,
      reason: "empty",
    });
    expect(parseQuickEntryShortcut(null)).toEqual({
      ok: false,
      reason: "empty",
    });
    expect(parseQuickEntryShortcut("Ctrl+NotAKey")).toEqual({
      ok: false,
      reason: "invalid-key",
    });
    // Escape hides the window; binding it globally would make it un-toggleable.
    expect(parseQuickEntryShortcut("Ctrl+Escape")).toEqual({
      ok: false,
      reason: "reserved",
    });
  });
...
describe("sanitizeQuickEntrySettings", () => {
  it("defaults to enabled with the default shortcut", () => {
    expect(sanitizeQuickEntrySettings(undefined)).toEqual({
      enabled: true,
  ...
  it("keeps an explicit disable and normalizes a stored shortcut", () => {
    expect(
      sanitizeQuickEntrySettings({ enabled: false, shortcut: "alt+j" }),
    ).toEqual({
      enabled: false,
      shortcut: "Alt+J",
    });
  ...
  it("falls back to the default when the stored shortcut is unusable", () => {
    expect(
      sanitizeQuickEntrySettings({ enabled: true, shortcut: "Q" }),
    ).toEqual({
      enabled: true,
      shortcut: DEFAULT_QUICK_ENTRY_SHORTCUT,
    });
...
describe("createQuickEntryShortcut", () => {
  ...
  it("survives a throwing globalShortcut", () => {
    ...
    expect(controller.apply({ enabled: true, shortcut: "Alt+J" }).error).toBe(
      "taken",
    );
  });
...
describe("quickEntryWindowBounds", () => {
  ...
  it("falls back to the origin without a work area", () => {
    expect(quickEntryWindowBounds()).toEqual({
      height: 168,
      width: 640,
      x: 0,
      y: 0,
