...
const glass = (
  intensity: number,
  material: GlassMaterial = DEFAULT_GLASS_MATERIAL,
  fade = 0,
): TranslucencyState => ({
  intensity,
  fade,
...
describe("windowOpacityFor", () => {
  ...
  it("is fully opaque at 0 and exactly the floor at 100", () => {
    ...
    expect(windowOpacityFor(clear(100))).toBe(
      1 - (1 - TRANSLUCENCY_OPACITY_FLOOR),
    );
  });
  ...
  it("decreases monotonically and never sinks below the floor", () => {
    ...
    for (
      let intensity = TRANSLUCENCY_MIN + 1;
      intensity <= TRANSLUCENCY_MAX;
      intensity += 1
    ) {
      ...
      expect(
        opacity,
        `${intensity} should be more see-through than ${intensity - 1}`,
      ).toBeLessThan(previous);
      expect(
        opacity,
        `${intensity} should stay at or above the floor`,
      ).toBeGreaterThanOrEqual(TRANSLUCENCY_OPACITY_FLOOR);
  ...
  it("spends a useful stretch of the lever on readable settings, not its first few percent", () => {
    ...
    for (
      let intensity = TRANSLUCENCY_MIN;
      intensity <= TRANSLUCENCY_MAX;
      intensity += 1
    ) {
    ...
    expect(
      readable.length,
      `only ${readable.length} readable settings`,
    ).toBeGreaterThan(20);
  });
  ...
  it("clamps before mapping so corrupt state cannot escape the range", () => {
    ...
    expect(windowOpacityFor(clear(240))).toBe(
      windowOpacityFor(clear(TRANSLUCENCY_MAX)),
    );
  });
  ...
  it("fades a glass window only through its own lever, on the ramp clear uses", () => {
    ...
    expect(windowOpacityFor(glass(60, DEFAULT_GLASS_MATERIAL, 40))).toBe(
      windowOpacityFor(clear(40)),
    );
    expect(windowOpacityFor(glass(100, DEFAULT_GLASS_MATERIAL, 100))).toBe(
      windowOpacityFor(clear(100)),
    );
  });

  it("leaves fade inert under clear, where the intensity lever already is the opacity", () => {
    expect(windowOpacityFor({ ...clear(40), fade: 100 })).toBe(
      windowOpacityFor(clear(40)),
    );
  });
});
...
describe("hudFrostFor", () => {
  it("wears the chosen frost on both platforms while the band is showing", () => {
    expect(hudFrostFor(glass(60, "header"), true)).toEqual({
      vibrancy: "header",
  ...
  it("is off whenever the band is not covering the window", () => {
    expect(hudFrostFor(glass(60, "header"), false)).toEqual({
      vibrancy: null,
  ...
  it("is off whenever glass itself is off", () => {
    expect(hudFrostFor(clear(60), true)).toEqual({
      vibrancy: null,
      backgroundMaterial: "none",
    });
    expect(hudFrostFor(glass(0, "header"), true)).toEqual({
      vibrancy: null,
      backgroundMaterial: "none",
    });
  });
...
describe("glassSupportedOn", () => {
  ...
  it("is on for Windows 11 22H2 and newer, off for everything older", () => {
    expect(glassSupportedOn("win32", `10.0.${WINDOWS_GLASS_MIN_BUILD}`)).toBe(
      true,
    );
    expect(glassSupportedOn("win32", `10.0.${WINDOWS_GLASS_MIN_BUILD}.1`)).toBe(
      true,
    );
    expect(
      glassSupportedOn("win32", `10.0.${WINDOWS_GLASS_MIN_BUILD - 1}`),
    ).toBe(false);
...
describe("backgroundMaterialFor", () => {
  ...
  it("collapses Glare onto mica with Bright", () => {
    ...
    expect(backgroundMaterialFor(glass(60, "header"))).toBe(
      backgroundMaterialFor(glass(60, "titlebar")),
    );
  });

  it("resolves every shipped rung to a real system material", () => {
    for (const material of GLASS_MATERIALS) {
      expect(WINDOWS_BACKGROUND_MATERIALS, material).toContain(
        backgroundMaterialFor(glass(60, material)),
      );
    }
  });
});
...
describe("the frost rungs a platform offers", () => {
  ...
  it("never offers two rungs that render the same Windows backdrop", () => {
    const backdrops = glassMaterialsFor(true).map((material) =>
      backgroundMaterialFor(glass(60, material)),
    );
  ...
  it("keeps every distinct Windows backdrop reachable from the picker", () => {
    const backdrops = new Set(
      glassMaterialsFor(true).map((material) =>
        backgroundMaterialFor(glass(60, material)),
      ),
    );
  ...
  it("folds a dropped rung onto the button that looks the same", () => {
    ...
    expect(glassMaterialsFor(true)).toContain(
      glassMaterialForPicker("header", true),
    );
    expect(
      backgroundMaterialFor(glass(60, glassMaterialForPicker("header", true))),
    ).toBe(backgroundMaterialFor(glass(60, "header")));
  });
...
describe("a fresh profile", () => {
  ...
  it("leaves the window exactly as it is today", () => {
    ...
    expect(windowBackingOptions(fresh, "#101014")).toEqual({
...
describe("windowBackingOptions", () => {
  ...
  it("omits backgroundColor entirely while glass is active", () => {
    ...
    expect(
      "backgroundColor" in windowBackingOptions(glass(60), "#111111"),
    ).toBe(false);
  });

  it("keeps the themed anti-flash backing in every other state", () => {
    expect(windowBackingOptions(glass(0), "#111111")).toEqual({
      backgroundColor: "#111111",
    });
    expect(windowBackingOptions(clear(60), "#111111")).toEqual({
      backgroundColor: "#111111",
    });
    expect(windowBackingOptions(clear(0), "#f7f7f7")).toEqual({
...
describe("windowOpacityOptions", () => {
  ...
  it("passes it the moment the state actually fades", () => {
    for (const state of [
      clear(1),
      clear(100),
      glass(60, DEFAULT_GLASS_MATERIAL, 1),
    ]) {
      expect(windowOpacityOptions(state)).toStrictEqual({
  ...
  it("leaves every untouched Windows window unlayered", () => {
    for (const appearance of ["light", "dark"] as const) {
      const state = {
        ...defaultTranslucencyValues(appearance, true),
        mode: "glass" as const,
      };
...
describe("what an update actually changes natively", () => {
  ...
  it("is nothing at all while dragging intensity under glass", () => {
    for (let intensity = 41; intensity <= 100; intensity += 1) {
      expect(
        nativeDiff(glass(intensity - 1), glass(intensity)),
        `at ${intensity}`,
      ).toEqual({
        backing: false,
        material: false,
  ...
  it("is only the opacity while dragging intensity under clear", () => {
    expect(nativeDiff(clear(40), clear(41))).toEqual({
      backing: false,
      material: false,
      opacity: true,
    });
  ...
  it("is only the opacity while dragging fade under glass", () => {
    expect(
      nativeDiff(
        glass(60, DEFAULT_GLASS_MATERIAL, 40),
        glass(60, DEFAULT_GLASS_MATERIAL, 41),
      ),
    ).toEqual({
      backing: false,
      material: false,
  ...
  it("is the material alone when the frost level changes", () => {
    ...
    expect(backgroundMaterialFor(glass(60, "under-window"))).not.toBe(
  ...
  it("is the backing and material when glass crosses zero", () => {
    expect(nativeDiff(glass(0), glass(1))).toEqual({
      backing: true,
      material: true,
      opacity: false,
    });
    expect(nativeDiff(glass(1), glass(0))).toEqual({
      backing: true,
      material: true,
      opacity: false,
    });
  ...
  it("is everything when switching between the two modes", () => {
    expect(nativeDiff(clear(60), glass(60))).toEqual({
      backing: true,
      material: true,
      opacity: true,
    });
...
describe("the defaults a fresh profile lands on", () => {
  const mac = (appearance: "dark" | "light") =>
    defaultTranslucencyValues(appearance, false);
  const win = (appearance: "dark" | "light") =>
    defaultTranslucencyValues(appearance, true);
  ...
  it("ships glass on, not a lever resting at zero", () => {
    ...
    for (const appearance of ["light", "dark"] as const) {
      expect(defaultTranslucencyState(appearance, true, false).mode).toBe(
        "glass",
      );
      expect(defaultTranslucencyState(appearance, true, true).mode).toBe(
        "glass",
      );
    }
  });
...
describe("resolving the book for the painted appearance", () => {
  ...
  it("falls all the way through to the platform default", () => {
    expect(resolveTranslucency(empty, "dark", false).intensity).toBe(
      defaultTranslucencyValues("dark", false).intensity,
    );
    expect(resolveTranslucency(empty, "dark", true).intensity).toBe(
      defaultTranslucencyValues("dark", true).intensity,
    );
  });
  ...
  it("scopes an edit to the appearance it was made in", () => {
    ...
    expect(resolveTranslucency(book, "dark", false).intensity).toBe(
      defaultTranslucencyValues("dark", false).intensity,
    );
  });
  ...
  it("lets an appearance override base without disturbing the other", () => {
    const tuned = setTranslucencyValues(
      normalizeBook({ intensity: 40, mode: "glass" }, true),
      "dark",
      {
        intensity: 10,
      },
  ...
  it("inherits per KEY, not per appearance", () => {
    ...
    const book = setTranslucencyValues(
      normalizeBook({ material: "popover", mode: "glass" }, true),
      "dark",
      {
        intensity: 33,
      },
  ...
  it("keeps mode global — clear vs glass is about the window, not the palette", () => {
    const book = setTranslucencyValues({ ...empty, mode: "clear" }, "light", {
      intensity: 50,
    });
