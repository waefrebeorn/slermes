...
contextBridge.exposeInMainWorld("hermesDesktop", {
  ...
  getConnectionFor: (payload) =>
    ipcRenderer.invoke("hermes:connection:for", payload),
  getProfileRoutes: (profiles) =>
    ipcRenderer.invoke("hermes:plugin-profile-routes", profiles),
  revalidateConnection: () =>
    ipcRenderer.invoke("hermes:connection:revalidate"),
  touchBackend: (profile) =>
    ipcRenderer.invoke("hermes:backend:touch", profile),
  ...
  getGatewayWsUrl: (profile) =>
    ipcRenderer.invoke("hermes:gateway:ws-url", profile),
  // Registry-scoped fresh WS URL: { connectionId, profile } → result shape of
  // getGatewayWsUrl, minted against that connection's backend.
  getGatewayWsUrlFor: (payload) =>
    ipcRenderer.invoke("hermes:gateway:ws-url-for", payload),
  // Union agent roster across every registered connection.
  getAgentRoster: () => ipcRenderer.invoke("hermes:agents:roster"),
  openSessionWindow: (sessionId, opts) =>
    ipcRenderer.invoke("hermes:window:openSession", sessionId, opts),
  openSessionInTerminal: (sessionId, opts) =>
    ipcRenderer.invoke("hermes:window:openInTerminal", sessionId, opts),
  openWindow: () => ipcRenderer.invoke("hermes:window:openInstance"),
  openBrowserWindow: (tabId) =>
    ipcRenderer.invoke("hermes:window:openBrowser", tabId),
  onBrowserPopoutClosed: (callback) => {
    const listener = (_event, tabId) => callback(tabId);
    ipcRenderer.on("hermes:browser-popout:closed", listener);

    return () =>
      ipcRenderer.removeListener("hermes:browser-popout:closed", listener);
  },
  ...
  wakeIndicator: {
    ...
    onState: (callback) => {
      ...
      return () =>
        ipcRenderer.removeListener("hermes:wake-indicator:state", listener);
    },
  },
  petOverlay: {
    ...
    setBounds: (bounds) =>
      ipcRenderer.send("hermes:pet-overlay:set-bounds", bounds),
    setIgnoreMouse: (ignore) =>
      ipcRenderer.send("hermes:pet-overlay:ignore-mouse", ignore),
    // Flip the overlay focusable (and focus it) while the composer needs keys.
    setFocusable: (focusable) =>
      ipcRenderer.send("hermes:pet-overlay:set-focusable", focusable),
    // Main renderer → overlay (forwarded by main): push the latest pet state.
    pushState: (payload) =>
      ipcRenderer.send("hermes:pet-overlay:state", payload),
    // Overlay → main renderer (forwarded by main): pop back in / composer submit.
    control: (payload) =>
      ipcRenderer.send("hermes:pet-overlay:control", payload),
    ...
    onState: (callback) => {
      ...
      return () =>
        ipcRenderer.removeListener("hermes:pet-overlay:state", listener);
    },
    ...
    onControl: (callback) => {
      ...
      return () =>
        ipcRenderer.removeListener("hermes:pet-overlay:control", listener);
    },
  },
  ...
  hud: {
    ...
    setIgnoreMouse: (ignore) =>
      ipcRenderer.send("hermes:hud:ignore-mouse", ignore),
    ...
    setWorkspaceTransfer: (transferring) =>
      ipcRenderer.send("hermes:hud:workspace-transfer", transferring),
    ...
    setSession: (sessionId) =>
      ipcRenderer.send("hermes:hud:session", sessionId),
    onGoto: (callback) => {
    ...
    onGameOverlay: (callback) => {
      ...
      return () =>
        ipcRenderer.removeListener("hermes:hud:game-overlay", listener);
    },
  },
  ...
  quickEntry: {
    ...
    setSettings: (patch) =>
      ipcRenderer.invoke("hermes:quick-entry:settings:set", patch),
    ...
    pushState: (payload) =>
      ipcRenderer.send("hermes:quick-entry:state", payload),
    ...
    onState: (callback) => {
      ...
      return () =>
        ipcRenderer.removeListener("hermes:quick-entry:state", listener);
    },
    ...
    onSubmit: (callback) => {
      ...
      return () =>
        ipcRenderer.removeListener("hermes:quick-entry:submit", listener);
    },
    ...
    onShown: (callback) => {
      ...
      return () =>
        ipcRenderer.removeListener("hermes:quick-entry:shown", listener);
    },
  },
  getBootProgress: () => ipcRenderer.invoke("hermes:boot-progress:get"),
  getConnectionConfig: (profile) =>
    ipcRenderer.invoke("hermes:connection-config:get", profile),
  saveConnectionConfig: (payload) =>
    ipcRenderer.invoke("hermes:connection-config:save", payload),
  applyConnectionConfig: (payload) =>
    ipcRenderer.invoke("hermes:connection-config:apply", payload),
  testConnectionConfig: (payload) =>
    ipcRenderer.invoke("hermes:connection-config:test", payload),
  ...
  getSecretStorageEncryption: () =>
    ipcRenderer.invoke("hermes:secret-storage:get"),
  setSecretStorageEncryption: (on: boolean) =>
    ipcRenderer.invoke("hermes:secret-storage:set", on),
  ...
  connections: {
    ...
    setPrimary: (id) =>
      ipcRenderer.invoke("hermes:connections:set-primary", id),
    setLaunchMode: (mode) =>
      ipcRenderer.invoke("hermes:connections:set-launch-mode", mode),
    setLastUsed: (id) =>
      ipcRenderer.invoke("hermes:connections:set-last-used", id),
    test: (id) => ipcRenderer.invoke("hermes:connections:test", id),
    updateManaged: (id) =>
      ipcRenderer.invoke("hermes:connections:update-managed", id),
    // Fan out `hermes update` to every eligible registered connection.
    // Optional excludeIds skips rows the caller updates through another path.
    updateAll: (options) =>
      ipcRenderer.invoke("hermes:connections:update-all", options),
    ...
    onChanged: (callback) => {
      ...
      return () =>
        ipcRenderer.removeListener("hermes:connections:changed", listener);
    },
  },
  sshConfigHosts: () => ipcRenderer.invoke("hermes:ssh-config:hosts"),
  sshResolveHost: (host) =>
    ipcRenderer.invoke("hermes:ssh-config:resolve", host),
  probeConnectionConfig: (remoteUrl) =>
    ipcRenderer.invoke("hermes:connection-config:probe", remoteUrl),
  oauthLoginConnectionConfig: (remoteUrl) =>
    ipcRenderer.invoke("hermes:connection-config:oauth-login", remoteUrl),
  oauthLogoutConnectionConfig: (remoteUrl) =>
    ipcRenderer.invoke("hermes:connection-config:oauth-logout", remoteUrl),
  ...
  cloud: {
    ...
    agentSignIn: (dashboardUrl) =>
      ipcRenderer.invoke("hermes:cloud:agent-sign-in", dashboardUrl),
  },
  profile: {
  ...
  requestMicrophoneAccess: () =>
    ipcRenderer.invoke("hermes:requestMicrophoneAccess"),
  readWindowBelow: () => ipcRenderer.invoke("hermes:window:readBelow"),
  readFileDataUrl: (filePath) =>
    ipcRenderer.invoke("hermes:readFileDataUrl", filePath),
  readFileDataUrlForAttach: (filePath) =>
  ...
  readFileText: (filePath) =>
    ipcRenderer.invoke("hermes:readFileText", filePath),
  readPluginSource: (filePath: string) =>
    ipcRenderer.invoke("hermes:readPluginSource", filePath),
  selectPaths: (options) => ipcRenderer.invoke("hermes:selectPaths", options),
  selectSavePath: (options) =>
    ipcRenderer.invoke("hermes:selectSavePath", options),
  writeClipboard: (text) => ipcRenderer.invoke("hermes:writeClipboard", text),
  readClipboard: () => ipcRenderer.invoke("hermes:readClipboard"),
  saveGatewayFile: (payload) =>
    ipcRenderer.invoke("hermes:saveGatewayFile", payload),
  saveImageFromUrl: (url) => ipcRenderer.invoke("hermes:saveImageFromUrl", url),
  contextMenuEdit: (command) =>
    ipcRenderer.invoke("hermes:context-menu:edit", command),
  contextMenuCopyImage: () =>
    ipcRenderer.invoke("hermes:context-menu:copy-image"),
  contextMenuSpellcheck: (action) =>
    ipcRenderer.invoke("hermes:context-menu:spellcheck", action),
  contextMenuGuestAddWord: (payload) =>
    ipcRenderer.invoke("hermes:context-menu:guest-add-word", payload),
  onContextMenuSpellcheck: (callback) => {
    const listener = (_event, payload) => callback(payload);
    ipcRenderer.on("hermes:context-menu-spellcheck", listener);

    return () =>
      ipcRenderer.removeListener("hermes:context-menu-spellcheck", listener);
  },
  saveImageBuffer: (data, ext, name) =>
    ipcRenderer.invoke("hermes:saveImageBuffer", { data, ext, name }),
  capturePreview: (payload) =>
    ipcRenderer.invoke("hermes:capturePreview", payload),
  ...
  setDisableF12: (blocked) =>
    ipcRenderer.send("hermes:devtools:disable-f12", blocked),
  setPreviewShortcutActive: (active) =>
    ipcRenderer.send("hermes:previewShortcutActive", Boolean(active)),
  ...
  mcpOauth: {
    ...
    wait: (id, timeoutMs) =>
      ipcRenderer.invoke("hermes:mcp-oauth:wait", id, timeoutMs),
    cancel: (id) => ipcRenderer.invoke("hermes:mcp-oauth:cancel", id),
  },
  openPreviewInBrowser: (url) =>
    ipcRenderer.invoke("hermes:openPreviewInBrowser", url),
  reachPreviewUrl: (url) => ipcRenderer.invoke("hermes:preview:reach", url),
  setActiveConnectionRoute: (route) =>
    ipcRenderer.send("hermes:connection:active-route", route),
  fetchLinkTitle: (url) => ipcRenderer.invoke("hermes:fetchLinkTitle", url),
  resolveFavicon: (url) => ipcRenderer.invoke("hermes:resolveFavicon", url),
  sanitizeWorkspaceCwd: (cwd) =>
    ipcRenderer.invoke("hermes:workspace:sanitize", cwd),
  settings: {
    getDefaultProjectDir: () =>
      ipcRenderer.invoke("hermes:setting:defaultProjectDir:get"),
    setDefaultProjectDir: (dir) =>
      ipcRenderer.invoke("hermes:setting:defaultProjectDir:set", dir),
    pickDefaultProjectDir: () =>
      ipcRenderer.invoke("hermes:setting:defaultProjectDir:pick"),
  },
  zoom: {
    ...
    setPercent: (percent) =>
      ipcRenderer.send("hermes:zoom:set-percent", percent),
  ...
  reportRendererError: (report) =>
    ipcRenderer.send("hermes:logs:renderer-error", report),
  readDir: (dirPath) => ipcRenderer.invoke("hermes:fs:readDir", dirPath),
  gitRoot: (startPath) => ipcRenderer.invoke("hermes:fs:gitRoot", startPath),
  revealPath: (targetPath) =>
    ipcRenderer.invoke("hermes:fs:reveal", targetPath),
  ...
  renamePath: (targetPath, newName) =>
    ipcRenderer.invoke("hermes:fs:rename", targetPath, newName),
  writeTextFile: (filePath, content) =>
    ipcRenderer.invoke("hermes:fs:writeText", filePath, content),
  trashPath: (targetPath) => ipcRenderer.invoke("hermes:fs:trash", targetPath),
  git: {
    worktreeList: (repoPath) =>
      ipcRenderer.invoke("hermes:git:worktreeList", repoPath),
    worktreeAdd: (repoPath, options) =>
      ipcRenderer.invoke("hermes:git:worktreeAdd", repoPath, options),
    ...
    branchSwitch: (repoPath, branch) =>
      ipcRenderer.invoke("hermes:git:branchSwitch", repoPath, branch),
    branchList: (repoPath) =>
      ipcRenderer.invoke("hermes:git:branchList", repoPath),
    baseBranchList: (repoPath) =>
      ipcRenderer.invoke("hermes:git:baseBranchList", repoPath),
    repoStatus: (repoPath) =>
      ipcRenderer.invoke("hermes:git:repoStatus", repoPath),
    fileDiff: (repoPath, filePath) =>
      ipcRenderer.invoke("hermes:git:fileDiff", repoPath, filePath),
    scanRepos: (roots, options) =>
      ipcRenderer.invoke("hermes:git:scanRepos", roots, options),
    review: {
      list: (repoPath, scope, baseRef) =>
        ipcRenderer.invoke("hermes:git:review:list", repoPath, scope, baseRef),
      ...
      stage: (repoPath, filePath) =>
        ipcRenderer.invoke("hermes:git:review:stage", repoPath, filePath),
      unstage: (repoPath, filePath) =>
        ipcRenderer.invoke("hermes:git:review:unstage", repoPath, filePath),
      revert: (repoPath, filePath) =>
        ipcRenderer.invoke("hermes:git:review:revert", repoPath, filePath),
      revParse: (repoPath, ref) =>
        ipcRenderer.invoke("hermes:git:review:revParse", repoPath, ref),
      commit: (repoPath, message, push) =>
        ipcRenderer.invoke("hermes:git:review:commit", repoPath, message, push),
      commitContext: (repoPath) =>
        ipcRenderer.invoke("hermes:git:review:commitContext", repoPath),
      push: (repoPath) =>
        ipcRenderer.invoke("hermes:git:review:push", repoPath),
      shipInfo: (repoPath) =>
        ipcRenderer.invoke("hermes:git:review:shipInfo", repoPath),
      ...
      fetchPrComment: (repoPath, url) =>
        ipcRenderer.invoke("hermes:git:review:fetchPrComment", repoPath, url),
      createPr: (repoPath) =>
        ipcRenderer.invoke("hermes:git:review:createPr", repoPath),
    },
  },
  terminal: {
    ...
    resize: (id, size) =>
      ipcRenderer.invoke("hermes:terminal:resize", id, size),
  ...
  onClosePreviewRequested: (callback) => {
    ...
    return () =>
      ipcRenderer.removeListener("hermes:close-preview-requested", listener);
  },
  ...
  onOpenFolderRequested: (callback) => {
    ...
    return () =>
      ipcRenderer.removeListener("hermes:open-folder-requested", listener);
  },
  ...
  probePluginRepo: (payload) =>
    ipcRenderer.invoke("hermes:plugin:probe", payload),
  installDesktopPlugin: (payload) =>
    ipcRenderer.invoke("hermes:plugin:installDesktop", payload),
  onWindowStateChanged: (callback) => {
    const listener = (_event, payload) => callback(payload);
    ipcRenderer.on("hermes:window-state-changed", listener);

    return () =>
      ipcRenderer.removeListener("hermes:window-state-changed", listener);
  },
  ...
  onNotificationAction: (callback) => {
    ...
    return () =>
      ipcRenderer.removeListener("hermes:notification-action", listener);
  },
  onNotificationActivate: (callback) => {
    ...
    return () =>
      ipcRenderer.removeListener("hermes:notification-activate", listener);
  },
  onPreviewFileChanged: (callback) => {
    ...
    return () =>
      ipcRenderer.removeListener("hermes:preview-file-changed", listener);
  },
  ...
  onConnectionApplied: (callback) => {
    ...
    return () =>
      ipcRenderer.removeListener("hermes:connection:applied", listener);
  },
  ...
  continueBootstrapLocal: () =>
    ipcRenderer.invoke("hermes:bootstrap:continue-local"),
  recycleBackend: (profile) =>
    ipcRenderer.invoke("hermes:backend:recycle", profile),
  ...
  getRemoteDisplayReason: () =>
    ipcRenderer.invoke("hermes:get-remote-display-reason"),
  uninstall: {
  ...
  updates: {
    ...
    onProgress: (callback) => {
      ...
      return () =>
        ipcRenderer.removeListener("hermes:updates:progress", listener);
    },
  },
  themes: {
    fetchMarketplace: (id) =>
      ipcRenderer.invoke("hermes:vscode-theme:fetch", id),
    searchMarketplace: (query) =>
      ipcRenderer.invoke("hermes:vscode-theme:search", query),
  },
  ...
  findInPage: (query, options) =>
    ipcRenderer.invoke("hermes:find-in-page", query, options),
