...
function dependencies(overrides: Partial<MediaProtocolDependencies> = {}) {
  return {
    ...
    fetchLocal: vi.fn(
      async (_resolvedPath: string, _headers: Headers) =>
        new Response("local", { status: 206 }),
    ),
    fetchRemote: vi.fn(
      async (_url: string, _headers: Headers) =>
        new Response("remote", { status: 206 }),
    ),
    fetchRemoteWithCookies: vi.fn(
      async (_url: string, _headers: Headers) =>
        new Response("cookie", { status: 206 }),
    ),
    resolveLocalFile: vi.fn(async (filePath: string) => filePath),
    resolveRemoteConnection: vi.fn(
      async (_scope: { connectionId?: string; profile?: string }) => ({
...
describe("media protocol helpers", () => {
  ...
  it("preserves a configured gateway path prefix", () => {
    const endpoint = new URL(
      remoteMediaEndpoint("https://gateway.test/hermes/", "/tmp/a b.mp4"),
    );
...
describe("createMediaProtocolHandler", () => {
  ...
  it("proxies token-auth remote media without placing the token in the URL", async () => {
    ...
    const response = await createMediaProtocolHandler(deps)(
      request(
        "hermes-media://remote/%2Froot%2Foutputs%2Frender.mp4?connectionId=work-ssh&profile=reviewer",
        {
          Range: "bytes=0-1023",
        },
      ),
  ...
  it("adds profile scope when one registry backend serves multiple profiles", async () => {
    ...
    await createMediaProtocolHandler(deps)(
      request(
        "hermes-media://remote/%2Froot%2Foutputs%2Frender.mp4?connectionId=cloud&profile=research",
      ),
    );
  ...
  it("preserves explicit HEAD requests through the token-auth remote proxy", async () => {
    ...
    const response = await createMediaProtocolHandler(deps)(
      request(
        "hermes-media://remote/%2Froot%2Foutputs%2Frender.mp4",
        {},
        "HEAD",
      ),
    );
  ...
  it("preserves explicit HEAD requests through the native-bearer remote fetch", async () => {
    ...
    expect(fetchRemote).toHaveBeenCalledOnce();
    expect(
      (fetchRemote.mock.calls[0]?.[1] as Headers).get("authorization"),
    ).toBe("Bearer native-access-token");
    expect(fetchRemote.mock.calls[0]?.[2]).toBe("HEAD");
    expect(deps.fetchRemoteWithCookies).not.toHaveBeenCalled();
  });
  ...
  it("preserves explicit HEAD requests through the isolated-cookie remote fetch", async () => {
    const fetchRemoteWithCookies = vi.fn(
      async (..._args: unknown[]) => new Response(null, { status: 200 }),
    );

    const deps = dependencies({
      fetchRemoteWithCookies:
        fetchRemoteWithCookies as MediaProtocolDependencies["fetchRemoteWithCookies"],
      resolveRemoteConnection: vi.fn(async () => ({
        authMode: "oauth" as const,
        baseUrl: "https://gateway.test",
        mode: "remote" as const,
    ...
    expect(
      (fetchRemoteWithCookies.mock.calls[0]?.[1] as Headers).get(
        "authorization",
      ),
    ).toBeNull();
  ...
  it("fails closed for unsupported extensions and missing remote auth", async () => {
    ...
    expect(
      (await handler(request("hermes-media://remote/%2Ftmp%2Fsecret.txt")))
        .status,
    ).toBe(415);
    expect(
      (await handler(request("hermes-media://remote/%2Ftmp%2Fclip.mp4")))
        .status,
    ).toBe(401);
    expect(deps.fetchRemote).not.toHaveBeenCalled();
  });
