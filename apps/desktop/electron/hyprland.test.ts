...
const payload = (...clients: Array<Record<string, unknown>>) =>
...
describe("hyprlandSocketPath", () => {
  it("is null when Hyprland is not the compositor", () => {
    ...
    expect(
      hyprlandSocketPath({ XDG_RUNTIME_DIR: "/run/user/1000" }, 1000),
    ).toBeNull();
  });

  it("addresses the running instance, not just the user", () => {
    const path = hyprlandSocketPath(
      {
...
describe("parseHyprlandClients", () => {
  ...
  it("maps position and size into bounds", () => {
    const [window] = parseHyprlandClients(
      payload(client({ at: [120, 80], pid: 2, size: [640, 320] })),
      SELF_PID,
    );
