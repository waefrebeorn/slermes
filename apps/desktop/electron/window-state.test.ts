...
test("sanitizeWindowState treats isMaximized strictly", () => {
  assert.equal(
    sanitizeWindowState({ width: 1400, height: 900, isMaximized: "yes" })
      .isMaximized,
    false,
  );
});
...
test("onScreen accepts a window on the primary or a secondary display", () => {
  ...
  assert.equal(
    onScreen({ x: 100, y: 100, width: 1220, height: 800 }, PRIMARY),
    true,
  );
  assert.equal(
    onScreen({ x: 2200, y: 200, width: 1220, height: 800 }, dual),
    true,
  );
});
...
test("onScreen rejects off-screen, slivers, and bad input", () => {
  assert.equal(
    onScreen({ x: 3000, y: 100, width: 1220, height: 800 }, PRIMARY),
    false,
  ); // past right edge
  assert.equal(
    onScreen({ x: 100, y: -900, width: 1220, height: 800 }, PRIMARY),
    false,
  ); // above top
  assert.equal(
    onScreen({ x: 1910, y: 100, width: 1220, height: 800 }, PRIMARY),
    false,
  ); // ~10px sliver
...
test("computeWindowOptions clamps a size larger than the only display", () => {
  ...
  assert.deepEqual(computeWindowOptions(saved, LAPTOP), {
    width: 1366,
    height: 728,
  });
});
...
test("computeWindowOptions keeps the MIN floor on a sub-minimum display", () => {
  ...
  assert.deepEqual(computeWindowOptions(saved, tiny), {
    width: MIN_WIDTH,
    height: MIN_HEIGHT,
  });
});
...
test("computeWindowOptions does not clamp when displays are unknown", () => {
  const saved = sanitizeWindowState({ width: 2560, height: 1440 });
  assert.deepEqual(computeWindowOptions(saved, []), {
    width: 2560,
    height: 1440,
  });
});
