...
describe("primary-remote descriptor reuse keeps profile scope", () => {
  it("scopes a shared-remote request with ?profile=<profile>", () => {
    expect(
      pathForRegistryBackendRequest("/api/skills", "acme", {
        sharedRemote: true,
      }),
    ).toBe("/api/skills?profile=acme");
  });
  ...
  it("marks the reused primary-remote descriptor sharedRemote so the router scopes it", () => {
    const branchStart = mainSource.indexOf(
      "if (id === registry.primary && source.kind !== 'local'",
    );
    ...
    expect(branch).toContain(
      "registrySourceOwnsPrimaryBackend(registry, id, primaryDescriptor)",
    );
