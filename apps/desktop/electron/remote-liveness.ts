...
export interface RevalidateRemoteConnectionOptions<
...
export class RemoteRevalidationCoordinator {
  ...
  run<T>(connection: object, task: () => Promise<T>): Promise<T> {
    const existing = this.#inflightByConnection.get(connection) as
      Promise<T> | undefined;
...
export class RemoteLivenessTracker {
  ...
  readonly #failuresByBaseUrl = new Map<
    string,
  ...
  constructor(
    ...
    if (!Number.isInteger(failureLimit) || failureLimit < 1) {
      throw new Error(
        "Remote liveness failure limit must be a positive integer.",
      );
    }
...
export interface PooledRemoteEntry<
  TConnection extends RemoteConnectionDescriptor = RemoteConnectionDescriptor,
> {
...
>({
  ...
  stopBackend,
  tracker,
}: RevalidatePooledRemoteBackendsOptions<TConnection>): Promise<{
  dropped: string[];
}> {
  const remotes = [...entries].filter(
    ([, entry]) => !entry.process && entry.remoteBaseUrl,
  );
  ...
  await Promise.all(
    remotes.map(async ([profile, entry]) => {
      ...
      try {
        ...
        log(
          `Pooled remote backend for profile "${profile}" failed liveness probe; dropping stale descriptor.`,
        );
        stopBackend(profile);
...
export interface RevalidateSuspectPooledRemoteBackendsOptions<
  TConnection extends RemoteConnectionDescriptor,
> {
  entries: Iterable<[string, PooledRemoteEntry<TConnection>]>;
  log: (message: string) => void;
 ...
 */
export async function revalidateSuspectPooledRemoteBackends<
  TConnection extends RemoteConnectionDescriptor,
>({
  entries,
  log,
  ...
  retire,
  tracker,
}: RevalidateSuspectPooledRemoteBackendsOptions<TConnection>): Promise<{
  rebuilt: string[];
  retired: string[];
}> {
  const remotes = [...entries].filter(
...
export function attachPowerResumeRemoteRevalidation({
  ...
  const trigger = async (): Promise<void> => {
    ...
    if (
      lastKickAt !== null &&
      at - lastKickAt < POWER_RESUME_REVALIDATION_HOLDOFF_MS
    ) {
      return;
    }
 ...
 */
export async function revalidateRemoteConnection<
  TConnection extends RemoteConnectionDescriptor,
>({
  connectionPromise,
  currentConnectionPromise,
