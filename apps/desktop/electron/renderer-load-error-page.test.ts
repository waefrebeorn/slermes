...
test("error page names the failure and carries a Reload button", () => {
  const html = buildRendererLoadErrorPage({
    errorCode: -6,
    errorDescription:
      "The desktop renderer bundle is incomplete after the last update (2 missing file(s)).",
  ...
  assert.match(
    html,
    /incomplete after the last update \(2 missing file\(s\)\)/,
  );
  assert.match(html, /-6/);
...
test("error page reload button targets the real renderer URL when provided", () => {
  ...
  assert.match(
    html,
    /location\.replace\("file:\/\/\/C:\/Hermes%20Agent\/dist\/index\.html"\)/,
  );
...
test("reloadUrl cannot break out of the inline script block", () => {
  ...
  const html = buildRendererLoadErrorPage({
    errorDescription: "load failed",
    reloadUrl:
      'file:///C:/x/index.html</script><script>alert("pwned")</script>',
  });
