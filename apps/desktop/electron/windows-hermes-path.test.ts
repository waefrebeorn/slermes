...
test("buildPathExtCandidates: Windows tries PATHEXT extensions before the empty extension", () => {
  ...
  assert.equal(
    ...
    "",
    "empty extension must be last, not first",
  );
  assert.notEqual(
    extensions[0],
    "",
    "the buggy empty-extension-first order must not return",
  );
});

test("buildPathExtCandidates: defaults to .COM;.EXE;.BAT;.CMD when PATHEXT is unset on Windows", () => {
  assert.deepEqual(buildPathExtCandidates(undefined, true), [
    ".COM",
    ".EXE",
    ".BAT",
    ".CMD",
...
test("buildPathExtCandidates: respects a custom PATHEXT, still empty-last", () => {
  assert.deepEqual(buildPathExtCandidates(".EXE;.PS1", true), [
    ".EXE",
    ".PS1",
    "",
  ]);
...
test("chooseUpdaterArgs: gentle --update when both updater runtime files exist", () => {
  assert.deepEqual(
    chooseUpdaterArgs(
      { hasBootstrapMarker: true, hasVenvHermes: true, hasVenvPython: true },
      "main",
    ),
    ["--update", "--branch", "main"],
  );
...
test("chooseUpdaterArgs: marker-only install uses --repair when the venv is gone", () => {
  assert.deepEqual(
    chooseUpdaterArgs(
      { hasBootstrapMarker: true, hasVenvHermes: false, hasVenvPython: false },
      "main",
    ),
    ["--repair", "--branch", "main"],
  );
...
test("chooseUpdaterArgs: partial updater runtimes use --repair", () => {
  assert.deepEqual(
    chooseUpdaterArgs(
      { hasBootstrapMarker: true, hasVenvHermes: false, hasVenvPython: true },
      "main",
    ),
    ["--repair", "--branch", "main"],
  );
  assert.deepEqual(
    chooseUpdaterArgs(
      { hasBootstrapMarker: true, hasVenvHermes: true, hasVenvPython: false },
      "main",
    ),
    ["--repair", "--branch", "main"],
  );
...
test("chooseUpdaterArgs: passes the branch through unchanged in both modes", () => {
  assert.deepEqual(
    chooseUpdaterArgs(
      { hasBootstrapMarker: false, hasVenvHermes: true, hasVenvPython: true },
      "release/1.2",
    ),
  ...
  assert.deepEqual(
    chooseUpdaterArgs(
      { hasBootstrapMarker: false, hasVenvHermes: false, hasVenvPython: false },
      "release/1.2",
    ),
...
function makeDeps(
  overrides: Partial<Parameters<typeof resolveVenvHermesCommand>[2]> = {},
) {
  return {
    isWindows: true,
...
test("resolveVenvHermesCommand: returns null for a .cmd/.bat script command", () => {
  ...
  assert.equal(
    resolveVenvHermesCommand("/root/venv/Scripts/hermes.cmd", [], deps),
    null,
  );
});
...
test("resolveVenvHermesCommand: returns null when the basename is not hermes/hermes.exe", () => {
  ...
  assert.equal(
    resolveVenvHermesCommand("/root/venv/Scripts/python.exe", [], deps),
    null,
  );
});
...
test("resolveVenvHermesCommand: returns null when the parent dir is not Scripts", () => {
  ...
  assert.equal(
    resolveVenvHermesCommand("/root/venv/bin/hermes.exe", [], deps),
    null,
  );
});
...
test("resolveVenvHermesCommand: probes the venv python before trusting it (returns null on failed probe)", () => {
  ...
  const result = resolveVenvHermesCommand(
  ...
  assert.equal(
    probed,
    true,
    "must probe the venv interpreter; a broken venv must not be re-selected forever",
  );
  assert.equal(
    result,
    null,
    "a failed probe must fall through (return null) so the resolver reaches bootstrap",
  );
});
...
test("resolveVenvHermesCommand: returns the resolved python backend descriptor when the probe passes", () => {
  ...
  assert.ok(
    result,
    "a passing probe must return a backend descriptor, not null",
  );
...
test("getVenvSitePackagesEntries: returns empty for a falsy venvRoot", () => {
  ...
  assert.deepEqual(
    getVenvSitePackagesEntries(null, {
      isWindows: true,
      directoryExists: () => true,
    }),
    [],
  ...
  assert.deepEqual(
    getVenvSitePackagesEntries(undefined, {
      isWindows: true,
      directoryExists: () => true,
    }),
    [],
