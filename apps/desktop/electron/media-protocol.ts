...
const FORWARDED_MEDIA_REQUEST_HEADERS = [
  "accept",
  "if-modified-since",
  "if-none-match",
  "if-range",
...
export interface MediaProtocolDependencies {
  ...
  resolveRemoteConnection: (
    scope: MediaRemoteScope,
  ) => Promise<MediaRemoteConnection>;
}
...
function parseMediaProtocolTarget(rawUrl: string): MediaProtocolTarget {
  ...
  const connectionId =
    url.searchParams.get("connectionId")?.trim() || undefined;
...
export function isStreamableMediaPath(filePath: string): boolean {
  ...
  return STREAMABLE_MEDIA_EXTENSIONS.some((extension) =>
...
export function remoteMediaEndpoint(
  baseUrl: string,
  filePath: string,
  profile?: string,
): string {
...
export function createMediaProtocolHandler(
  ...
  return async (
    ...
    if (target.mode === "stream") {
      try {
        const resolvedPath = await dependencies.resolveLocalFile(
          target.filePath,
        );
    ...
    try {
      ...
      if (connection.mode !== "remote") {
        return new Response("Remote media backend unavailable", {
          status: 404,
        });
      }
      ...
      if (connection.authMode === "oauth") {
        const bearer = await dependencies.ensureRemoteBearer(
          connection.baseUrl,
        );
        ...
        return await dependencies.fetchRemoteWithCookies(
          endpoint,
          headers,
          method,
        );
      ...
      if (!connection.token) {
        return new Response("Remote media authentication unavailable", {
          status: 401,
        });
      }
