...
describe("formatRendererConsoleLine", () => {
  it("formats the canonical Electron 36+ details shape at error level", () => {
    ...
    expect(line).toBe(
      "[renderer console:hud] Minified React error #310 (file:///app/index.js:13)",
    );
  });
  ...
  it("drops non-error levels in both shapes", () => {
    expect(
      formatRendererConsoleLine("main", {
        level: 1,
        message: "x",
        sourceUrl: "s",
        lineNumber: 1,
...
describe("attachRendererConsoleCapture", () => {
  it("logs error-level messages and skips the rest", () => {
    ...
    handler?.(
      {},
      { level: 3, message: "crash", sourceUrl: "src", lineNumber: 2 },
    );
    handler?.(
      {},
      { level: 0, message: "debug", sourceUrl: "src", lineNumber: 3 },
    );

    expect(log).toHaveBeenCalledTimes(1);
    expect(log).toHaveBeenCalledWith(
      "[renderer console:quick-entry] crash (src:2)",
    );
  });
});
...
describe("formatRendererBoundaryReport", () => {
  it("carries window label, boundary label, message, and component stack", () => {
    ...
    expect(report).toContain(
      "[renderer crash:main] [error-boundary:root] Minified React error #310",
    );
  ...
  it("survives a malformed payload and clamps oversized fields", () => {
    ...
    expect(report).toContain(
      "[renderer crash:unknown] [error-boundary:unknown]",
    );
