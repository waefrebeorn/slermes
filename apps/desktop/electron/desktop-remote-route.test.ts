...
test("environment route wins over global but never claims a registry id", () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      mode: "remote",
      remote: { url: "https://global.test", token: tokenB },
    },
...
test("global route fails closed when primary differs, even if another row matches", () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      mode: "remote",
      remote: { url: "https://gateway.test", authMode: "token", token: tokenA },
    },
...
test("profile SSH route fails closed when any dial field differs", () => {
  ...
  for (const [index, variant] of variants.entries()) {
    const route = resolveDesktopRemoteRoute({
      ...
      profile: "worker",
      registry: registry("local", [
        { id: `ssh-${index}`, kind: "ssh", label: `SSH ${index}`, ...variant },
      ]),
    });
...
test("global SSH treats an omitted port as 22 and checks the primary route", () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      mode: "ssh",
      remote: { mode: "ssh", host: "box.test", user: "hermes" },
    },
  ...
  const cases = [
    {
      config: {
        mode: "remote",
        remote: { url: "https://gateway.test", token: tokenA },
      },
    ...
    },
    {
      config: {
        mode: "cloud",
        remote: { url: "https://gateway.test", authMode: "oauth", org: "a" },
      },
      primary: {
        kind: "cloud",
        url: "https://gateway.test",
  ...
  for (const [index, item] of cases.entries()) {
    const route = resolveDesktopRemoteRoute({
      ...
      registry: registry("primary", [
        { id: "primary", label: `Primary ${index}`, ...item.primary },
      ]),
    });
...
test("the v1 global remote still outranks the registry primary", () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      mode: "remote",
      remote: { url: "https://global.test", authMode: "token", token: tokenA },
    },
    profile: null,
    registry: registry("gw-b", [
      {
