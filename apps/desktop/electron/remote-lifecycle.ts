...
async function locateHermes(ssh, remoteHermesPath) {
  ...
  const isExecutable = async (candidate: string) => {
    try {
      ...
      const ok = (
        await ssh.exec(
          `[ -x ${expandRemotePath(candidate)} ] && echo OK || true`,
        )
      ).trim();
  ...
  try {
    const found = (
      await ssh.exec(`bash -lc ${shq("command -v hermes")}`)
    ).trim();
...
async function probeHermesVersion(ssh, hermesPath) {
  try {
    const out = (
      await ssh.exec(`${expandRemotePath(hermesPath)} --version 2>&1`)
    ).trim();
...
async function probeRemoteHermesHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${HERMES_HOME:-$HOME/.hermes}"'))
      .trim()
      .split("\n")
      .pop();
...
async function assertRemoteInstallUpdateClear(ssh, hermesHome) {
  ...
  try {
    ...
    const error: any = new Error(
      "Could not prove that the remote Hermes install is clear for SSH startup.",
    );
...
async function readLockfile(ssh, ownershipId) {
  ...
  try {
    raw = await ssh.exec(
      `if [ ! -e ${expandRemotePath(lpath)} ]; then exit 0; fi; cat ${expandRemotePath(lpath)}`,
    );
  } catch (cause) {
    const error: any = new Error(
      "Could not read the SSH backend ownership record.",
    );
    error.kind = "transient-transport-error";
    error.cause = cause;
    throw error;
  ...
  if (parsed.schemaVersion !== LOCKFILE_SCHEMA_VERSION) {
    return lockfileSkew(
      `schema-version ${JSON.stringify(parsed.schemaVersion ?? null)}`,
    );
  }
  ...
  if (
    parsed.ownershipId !== ownershipId ||
    !/^[0-9a-f]{16}$/.test(parsed.spawnNonce || "")
  ) {
  ...
  if (
    ...
    (typeof parsed.creationTime !== "string" ||
      !/^(?:linux:[0-9]+|darwin:[A-Za-z0-9 :+-]+)$/.test(parsed.creationTime))
  ) {
    return null;
  }
...
async function remoteProcessCreationTime(ssh, pid) {
  ...
  try {
    ...
    return /^(?:linux:[0-9]+|darwin:[A-Za-z0-9 :+-]+)$/.test(value)
      ? value
      : "";
  } catch {
    return "";
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  ...
  const expectedLogPath = lock?.spawnNonce
    ? spawnLogPath(ownershipId, lock.spawnNonce)
    : "";
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedHome = lock.hermesHome
    ? expandRemotePath(lock.hermesHome)
    : "''";
  const expectedToken = expandRemotePath(
    spawnTokenPath(ownershipId, lock.spawnNonce),
  );
...
async function terminateOwnedDashboardForUpdate(ssh, expected) {
  ...
  if (!expected?.creationTime) {
    const error: any = new Error(
      "The remote POSIX serve has no process creation-time proof.",
    );
  ...
  if (!lock || !lockMatchesManagedUpdateScope(lock, expected)) {
    const error: any = new Error(
      "The remote POSIX ownership record changed before the managed update.",
    );
    error.kind = "ownership-changed";
    throw error;
  }
  ...
  if ((await remoteProcessCreationTime(ssh, lock.pid)) !== lock.creationTime) {
    const error: any = new Error(
      "The remote POSIX PID creation time no longer matches its ownership record.",
    );
    error.kind = "ownership-changed";
    throw error;
  }
  ...
  if (
    ...
    const error: any = new Error(
      "Refusing to terminate a remote process whose Desktop ownership is unproven.",
    );
  ...
  if (!lock || !lockMatchesManagedUpdateScope(lock, expected)) {
    const error: any = new Error(
      "The remote POSIX ownership record changed during process verification.",
    );
    error.kind = "ownership-changed";
    throw error;
  }
  ...
  if (
    ...
    const error: any = new Error(
      "The remote POSIX process identity changed during managed update drain.",
    );
    error.kind = "ownership-changed";
    throw error;
  }
  ...
  try {
    const result = String(
      await ssh.exec(buildOwnedTerminationCommand(lock, ownershipId)),
    ).trim();
    ...
    if (result !== "TERMINATED") {
      ...
      error.kind =
        result === "REFUSED" || result === "DARWIN_UNAVAILABLE"
          ? "ownership-changed"
    ...
    const error: any = new Error(
      "Could not terminate the Desktop-owned remote serve for update.",
    );
    error.kind = "transient-transport-error";
    error.cause = cause;
    throw error;
...
function buildSpawnCommand(hermesPath, profile, opts: any = {}) {
  ...
  const tokenArg = tokenFilePath
    ? ` --ssh-session-token-file ${expandRemotePath(tokenFilePath)}`
    : "";
  const ownerArg = opts.spawnNonce
    ? ` --ssh-owner-nonce ${validateSpawnNonce(opts.spawnNonce)}`
    : "";
  const subCmd = `serve --isolated --host 127.0.0.1 --port 0${tokenArg}${ownerArg}`;
  const marker = expandRemotePath(
    `${remoteInstallRoot(opts.hermesHome || "~/.hermes")}/.hermes-update-in-progress`,
  );
  ...
  const reservation = expandRemotePath(
    connectReservationPath(opts.ownershipId),
  );
  ...
  const reservationNonce = validateSpawnNonce(
    opts.reservationNonce || crypto.randomBytes(8).toString("hex"),
  );
...
async function scrapeReadyPort(
  ssh,
  logPath,
  { timeoutMs = DEFAULT_READY_TIMEOUT_MS, isAlive, signal }: any = {},
) {
  ...
  while (Date.now() < deadline) {
    ...
    if (isAlive && !(await isAlive())) {
      const err: any = new Error(
        "Remote dashboard process exited before announcing its port.",
      );
  ...
  const err: any = new Error(
    `Timed out waiting for the remote dashboard to announce its port (${timeoutMs}ms).`,
  );
...
async function spawnRemoteDashboard(
  ...
  if (!Number.isInteger(pid) || pid <= 0) {
    ...
    const err: any = new Error(
      "Failed to launch the remote dashboard (no pid returned).",
    );
    err.kind = "spawn-failed";
    throw err;
  }
...
function isForwardBindCollision(error) {
  return /address already in use|cannot listen to port|bind.*failed/i.test(
 ...
 */
async function adoptOwnedServedToken(
  adoptServedToken,
  baseUrl,
  expectedToken,
  ssh,
  ...
  if (!(await remotePidAlive(ssh, pid))) {
    const error: any = new Error(
      `${label} exited while its served token was being resolved.`,
    );
...
async function waitForRemoteSpawnCompletion(ssh, ownershipId, timeoutMs) {
  ...
  const error: any = new Error(
    "Timed out waiting for the concurrent SSH connection to publish its backend.",
  );
...
async function connect(deps) {
  ...
  if (lock) {
    ...
    if (reusable) {
      const creationTime =
        lock.creationTime || (await remoteProcessCreationTime(ssh, lock.pid));
      ...
      try {
        ...
        try {
          reuseClassification = await probeReuseProof(
            baseUrl,
            reuseToken,
            lock.spawnNonce,
          );
          ...
          const error: any = new Error(
            "Could not verify the existing SSH backend.",
          );
          error.kind = "transient-transport-error";
          error.cause = cause;
          throw error;
        ...
        if (reuseClassification === "authenticated-stale") {
          ...
          const error: any = new Error(
            "SSH reuse proof returned an invalid classification.",
          );
          error.kind = "transient-transport-error";
          throw error;
        }
  ...
  if (spawned.existing) {
    ...
    const published = await waitForRemoteSpawnCompletion(
      ssh,
      ownershipId,
      readyTimeoutMs,
    );
  ...
  try {
    ...
    await writeLockfile(ssh, ownershipId, {
      ...ownedSpawn,
      port: remotePort,
      tokenFingerprint,
    });
