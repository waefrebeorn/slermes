...
beforeEach(async () => {
  dir = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "hermes-download-fs-"),
  );
});
...
test("a download that fails mid-stream leaves the pre-existing destination byte-for-byte and no temp file", async () => {
  ...
  await assert.rejects(
    pumpStreamToFile(
      body(["partial"], new Error("socket hang up")),
      dest,
      fsPumpDeps(),
    ),
    /socket hang up/,
  ...
  assert.ok(
    original.equals(await fs.promises.readFile(dest)),
    "destination bytes must be unchanged",
  );
...
test("a download into a name with no existing file that fails leaves nothing behind", async () => {
  ...
  await assert.rejects(
    pumpStreamToFile(body(["partial"], new Error("reset")), dest, fsPumpDeps()),
    /reset/,
  );
...
test("a pre-open EEXIST collision leaves the seeded temp file and the destination untouched", async () => {
  ...
  await assert.rejects(
    pumpStreamToFile(body(["new content"]), dest, deps),
  ...
  assert.ok(
    seeded.equals(await fs.promises.readFile(pinnedTemp)),
    "the colliding file must not be unlinked",
  );
  assert.ok(
    original.equals(await fs.promises.readFile(dest)),
    "destination must not be renamed over",
  );
  assert.deepEqual(await listing(), [
    ".hermes-download-pinned.part",
    "report.bin",
  ]);
});
...
test("a failed final rename removes the owned temp file and leaves the destination as it was", async () => {
  ...
  await assert.rejects(
    pumpStreamToFile(body(["new content"]), dest, fsPumpDeps()),
  );

  assert.ok(
    (await fs.promises.stat(dest)).isDirectory(),
    "destination directory must survive",
  );
  assert.equal(
    await fs.promises.readFile(path.join(dest, "keep.txt"), "utf8"),
    "inside",
  );
  assert.deepEqual(
    await listing(),
    ["report.bin"],
    "the owned temp file must be cleaned up",
  );
});
...
test("writeBufferToFile into a missing directory fails without creating anything", async () => {
  ...
  await assert.rejects(
    writeBufferToFile(Buffer.from("payload"), dest, fsPumpDeps()),
    (err: NodeJS.ErrnoException) => {
