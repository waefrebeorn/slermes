...
export interface ReloadPolicyDecision {
  ...
  suppressedReason?:
    "crash-loop" | "expected-teardown" | "unrecoverable-reason";
  /**
...
export function pruneReloadTimes(
  times: number[],
  now: number,
  windowMs: number,
): number[] {
 ...
 */
export function describeRendererLifecycleEvent(
  ...
  const teardown =
    event.isDestroyed && reason === "killed" ? " (expected teardown)" : "";
...
export function installWindowRendererLifecycle(
  ...
  const onRendererGone = (
    _event: unknown,
    details?: RendererLifecycleDetails,
  ) => {
    ...
    setImmediate(() => {
      ...
      try {
        ...
        log(
          `[renderer:${kind}] reload after crash failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    });
  };
  ...
  const onDidFailLoad = (
    ...
    const code =
      typeof errorCode === "number" ? errorCode : String(errorCode ?? "");
    ...
    setImmediate(() => {
      ...
      try {
        ...
        log(
          `[renderer:${kind}] reload after failed load: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    });
  };
