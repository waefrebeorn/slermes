...
const HTML_BYTES = new Uint8Array([
  ...Buffer.from("<!doctype html><html>nope</html>"),
...
describe("ranking, so the best mark is fetched first", () => {
  ...
  test("an apple-touch link with no sizes still beats a guessed path", () => {
    const declared = iconCandidatesFromHtml(
      '<link rel="apple-touch-icon" href="/touch.png">',
      "https://acme.test",
    );
    const ranked = rankCandidates([
      ...fallbackIconCandidates("https://acme.test"),
      ...declared,
    ]);
  ...
  test("unparseable manifest JSON is not an error, just no candidates", () => {
    assert.deepEqual(
      iconCandidatesFromManifest("<!doctype html>", "https://acme.test/m.json"),
      [],
    );
  });
  ...
  test("the apex is tried as well as the subdomain", () => {
    ...
    // api.example.com.
    const urls = fallbackIconCandidates("https://mcp.acme.test/sse").map(
...
describe("deciding whether bytes are actually an image", () => {
  test.each([
    ...
    assert.equal(
      sniffImageMime(new Uint8Array([...signature, ...new Array(60).fill(0)])),
      expected,
    );
  });
  ...
  test("webp needs both its RIFF header and its WEBP tag", () => {
    ...
    assert.equal(
      sniffImageMime(new Uint8Array([...riff, ...new Array(60).fill(0)])),
      "",
    );
  });
...
describe("walking the ladder", () => {
  ...
  test("a declared icon is preferred over the well-known path", async () => {
    const { io } = fakeIo({
      ...
      text: {
        "https://acme.test":
          '<link rel="icon" sizes="180x180" href="/declared.png">',
      },
    });
  ...
  test("a site that declares nothing still gets its guessed favicon", async () => {
    const { io } = fakeIo({
      images: { "https://acme.test/favicon.ico": { bytes: PNG, mime: "" } },
    });

    assert.ok(
      (await resolveFavicon("https://acme.test", io)).startsWith(
  ...
  test("a page that throws on read falls through to the guessed paths", async () => {
    const io: FaviconIo = {
      fetchImage: async (url) =>
        url.endsWith("/favicon.ico") ? { bytes: PNG, mime: "" } : null,
      fetchText: async () => {
