...
test("shallow checkout with a merge-base does not trust an inflated rev-list count", () => {
  ...
  try {
    ...
    const targetSha = execFileSync(
      "git",
      ["commit-tree", tree, "-p", currentSha, "-p", redundantParent],
      {
        cwd,
        encoding: "utf8",
        input: "remote merge\n",
    ...
    assert.ok(
      Number.parseInt(shallowCount, 10) > Number.parseInt(completeCount, 10),
    );
    assert.equal(
      resolveBehindCount({
...
test("shallow commit logs select only the fetched remote tip", () => {
  assert.deepEqual(
    resolveCommitLogSelection({ branch: "main", isShallow: true }),
    {
      limit: 1,
...
test("full-clone commit logs keep the complete behind range", () => {
  assert.deepEqual(
    resolveCommitLogSelection({ branch: "release", isShallow: false }),
    {
      limit: 40,
...
test("compareApiUrl refuses non-GitHub remotes and partial SHAs", () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: "https://gitlab.com/x/y.git",
      targetSha: SHA_B,
    }),
    null,
  );
