...
const LIBRARY_PACKAGE_SUFFIXES = [
  ".photoslibrary",
  ".musiclibrary",
  ".tvlibrary",
  ".aplibrary",
...
export function normalizeRepoScanPath(
  rawPath: string,
  options: RepoScanPathOptions = {},
): NormalizedScanPath | null {
  ...
  const absolute = pathApi.isAbsolute(expanded)
    ? expanded
    : pathApi.resolve(homeDir, expanded);
...
export function repoScanPathIsWithin(
  candidate: string,
  parent: string,
  options: RepoScanPathOptions = {},
): boolean {
...
export async function scanGitRepos(
  ...
  const maxDepth =
    Number.isFinite(maxDepthValue) && maxDepthValue >= 0
      ? maxDepthValue
      : DEFAULT_MAX_DEPTH;
  const pathOptions: RepoScanPathOptions = options.platform
  ...
  function isExcluded(candidate: string): boolean {
    return exclusions.some((excluded) =>
      repoScanPathIsWithin(candidate, excluded.value, pathOptions),
    );
  }
  ...
  async function walk(dir: string, depth: number): Promise<void> {
    ...
    const skipTccProtectedPaths =
      (pathOptions.platform ?? process.platform) === "darwin";
    ...
    await mapLimit(subdirs, MAX_CONCURRENCY, (subdir) =>
      walk(subdir, depth + 1),
    );
  }
