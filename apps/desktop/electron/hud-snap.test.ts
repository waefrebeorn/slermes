...
test("anchor under cursor at zoom 1", () => {
  assert.deepEqual(
    windowOriginForCursorAnchor({ x: 500, y: 400 }, { x: 310, y: 48 }, 1),
    { x: 190, y: 352 },
  );
});
...
test("anchor scales with page zoom", () => {
  assert.deepEqual(
    windowOriginForCursorAnchor({ x: 500, y: 400 }, { x: 100, y: 50 }, 0.9),
    {
...
test("clamp keeps a sliver visible when the anchor would park off-screen", () => {
  const origin = windowOriginForCursorAnchor(
    { x: 10, y: 30 },
    { x: 310, y: 48 },
    1,
  );
