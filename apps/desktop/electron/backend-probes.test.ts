...
test("canImportHermesCli returns false when binary does not exist", () => {
  const ghost = path.join(
    os.tmpdir(),
    "hermes-probes-ghost-" + Date.now() + ".exe",
  );
...
test("verifyHermesCli returns true when --version exits 0", () => {
  ...
  const scriptPath = path.join(
    os.tmpdir(),
    `hermes-probes-ok-${Date.now()}-${process.pid}.cjs`,
  );
...
test("verifyHermesCli swallows timeouts (does not throw)", () => {
  ...
  assert.equal(
    verifyHermesCli("/definitely/not/a/real/binary/anywhere"),
    false,
  );
});
...
test("resolveProbeTimeoutMs honours HERMES_PROBE_TIMEOUT_MS", () => {
  ...
  assert.equal(
    resolveProbeTimeoutMs({ HERMES_PROBE_TIMEOUT_MS: "30000" }),
    30_000,
  );
  assert.equal(
    resolveProbeTimeoutMs({ HERMES_PROBE_TIMEOUT_MS: "0" }),
    DEFAULT_PROBE_TIMEOUT_MS,
  );
  assert.equal(
    resolveProbeTimeoutMs({ HERMES_PROBE_TIMEOUT_MS: "nope" }),
    DEFAULT_PROBE_TIMEOUT_MS,
  );
  // Cap runaway values
  assert.equal(
    resolveProbeTimeoutMs({ HERMES_PROBE_TIMEOUT_MS: "999999" }),
    120_000,
  );
});
