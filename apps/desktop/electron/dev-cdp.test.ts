...
test("a dev-server run opens the default port with no opt-in", () => {
  assert.deepEqual(resolveDevCdpPort(devRun), {
    port: DEFAULT_PORT,
    reason: null,
  });
});
...
test("packaged is checked before every other gate", () => {
  ...
  for (const value of ["9222", "", "off", "garbage"]) {
    ...
    assert.equal(
      decision.port,
      null,
      `expected packaged to refuse ${JSON.stringify(value)}`,
    );
...
test("an unpackaged dist run (no dev server) does not qualify", () => {
  ...
  assert.deepEqual(resolveDevCdpPort({ ...devRun, devServer: undefined }), {
    port: null,
...
test("the port is overridable", () => {
  assert.equal(
    resolveDevCdpPort({ ...devRun, env: { HERMES_DESKTOP_CDP_PORT: "9333" } })
      .port,
    9333,
  );
});
...
test("can be switched off on a dev run", () => {
  for (const value of ["0", "off", "OFF", "false", "no"]) {
    ...
    assert.equal(
      decision.port,
      null,
      `expected ${JSON.stringify(value)} to close the port`,
    );
...
test("explains itself when an explicit setting was not honoured", () => {
  ...
  for (const value of ["garbage", "off"]) {
    ...
    assert.ok(
      describeDevCdpDecision(decision),
      `expected an explanation for ${JSON.stringify(value)}`,
    );
  }
});
...
test("stays quiet when the port opened, or is closed by design", () => {
  ...
  assert.equal(
    describeDevCdpDecision(resolveDevCdpPort({ ...devRun, isPackaged: true })),
    null,
  );
  assert.equal(
    describeDevCdpDecision(
