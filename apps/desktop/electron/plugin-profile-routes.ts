...
interface BuildOpaqueProfileRoutesOptions {
  getProfileConfig: (
    profile: string,
  ) => ProfileRouteConfig | Promise<ProfileRouteConfig>;
...
async function connectionScope(
  ...
  if (config.mode === "remote") {
    return {
      key: `remote\0${normalizeRemoteUrl(config.remoteUrl)}`,
      mode: "remote",
    };
  }
...
function opaqueConnectionId(scope: string, installationId: string): string {
  const digest = crypto
    .createHmac("sha256", installationId)
    .update(scope)
    .digest("hex");
...
function backendTargetProfile(
  scoped: ProfileRouteConfig,
  globalConfig: ProfileRouteConfig,
  profile: string,
): string {
  if (scoped.mode === "ssh") {
  ...
  return Promise.all(
    names.map(async (profile) => {
      ...
      const scope =
        scoped.mode === "local"
          ? globalScope
          : await connectionScope(scoped, resolveSsh);
...
export function buildRegistryProfileRoutes({
  ...
  for (const agent of agents) {
    ...
    routes.push({
      ...
      profile,
      targetProfile:
        source.kind === "ssh" && source.remoteProfile
          ? normalizeProfile(source.remoteProfile)
          : profile,
    });
  }
