...
test("redactSecrets scrubs the spawn-time session token env var", () => {
  const line =
    "setsid env HERMES_DASHBOARD_SESSION_TOKEN=abc123deadbeef HERMES_DESKTOP=1 hermes dashboard";
...
test("redactSecrets scrubs ?token= and ?ticket= URL params", () => {
  assert.match(
    redactSecrets("ws://127.0.0.1:5000/api/ws?token=supersecret"),
    /\?token=<redacted>/,
  );
  assert.match(
    redactSecrets("ws://127.0.0.1:5000/api/ws?ticket=onetimeticket"),
    /\?ticket=<redacted>/,
  );
...
test("redactSecrets scrubs Authorization and X-Hermes-Session-Token headers", () => {
  assert.match(
    redactSecrets("Authorization: Bearer tok_9999"),
    /Authorization: Bearer <redacted>/,
  );
  assert.ok(
    !redactSecrets("Authorization: Bearer tok_9999").includes("tok_9999"),
  );
  assert.match(
    redactSecrets("X-Hermes-Session-Token: hdr_888"),
    /X-Hermes-Session-Token: ?<redacted>/,
  );
  assert.ok(
    !redactSecrets("X-Hermes-Session-Token: hdr_888").includes("hdr_888"),
  );
});
...
test("redactSecrets masks a credential glued into an ssh target string", () => {
  ...
  const mangled = redactSecrets(
    "connecting (no-mux) to root@100,84,204,123:Luisclawy2026:22",
  );
...
test("controlSocketPath default base stays under sun_path even with the temp-listener suffix", () => {
  ...
  assert.ok(
    ...
    "default base must not be os.tmpdir() on macOS",
  );
});
...
test("baseSshOptions carries the house ControlMaster/BatchMode/accept-new policy", () => {
  ...
  assert.ok(
    !joined.includes("StrictHostKeyChecking=no"),
...
test("hostArgs adds -p only for non-default port and -i only with a key", () => {
  ...
  assert.deepEqual(hostArgs({ port: 2200, keyPath: "/k" }), [
    "-p",
    "2200",
    "-i",
    "/k",
...
test("buildInteractiveSshArgs requests a PTY, reuses the control master, execs a login shell", () => {
  ...
  assert.equal(
    args[0],
    "-tt",
    "forces a PTY so the remote sees a real terminal",
  );
  assert.ok(
    args.join(" ").includes("ControlPath=/tmp/x.sock"),
    "reuses the existing master (no new auth)",
  );
  assert.equal(args[args.length - 2], "me@box");
...
test("buildInteractiveSshArgs cds into the remote cwd (best-effort) before the shell", () => {
  ...
  assert.match(
    remoteCmd,
    /^cd '\/home\/me\/project' 2>\/dev\/null; exec "\$SHELL" -l$/,
  );
});
...
test("classifySshError detects a changed host key (fail-closed)", () => {
  ...
  assert.equal(
    classifySshError("Host key verification failed."),
    SSH_ERROR.HOST_KEY_CHANGED,
  );
  assert.equal(
    classifySshError("Offending ECDSA key in /home/u/.ssh/known_hosts:5"),
    SSH_ERROR.HOST_KEY_CHANGED,
  );
});
...
test("classifySshError detects auth failure", () => {
  assert.equal(
    classifySshError("Permission denied (publickey)."),
    SSH_ERROR.AUTH_FAILED,
  );
  assert.equal(
    classifySshError("Too many authentication failures"),
    SSH_ERROR.AUTH_FAILED,
  );
});
...
test("classifySshError detects unreachable", () => {
  assert.equal(
    classifySshError("ssh: Could not resolve hostname nope"),
    SSH_ERROR.UNREACHABLE,
  );
  assert.equal(
    classifySshError("connect to host x port 22: Connection refused"),
    SSH_ERROR.UNREACHABLE,
  );
});
...
test("sshErrorMessage gives actionable guidance for auth and host-key-change", () => {
  ...
  assert.match(
    sshErrorMessage(SSH_ERROR.AUTH_FAILED, conn, "Permission denied"),
    /ssh-agent|ssh-add|IdentityFile/,
  );
  assert.match(
    sshErrorMessage(SSH_ERROR.HOST_KEY_CHANGED, conn, "CHANGED"),
    /ssh-keygen -R box/,
  );
});
...
test("open() establishes the master when not already alive", async () => {
  ...
  assert.deepEqual(
    ops,
    ["check", "master"],
    "probes liveness first, then opens the master",
  );
});
...
test("open() is a no-op when the master is already alive and execs verify", async () => {
  ...
  assert.deepEqual(
    ops,
    ["check", "verify"],
    "alive master is exec-verified, then trusted without reopening",
  );
});
...
test("close() removes the control socket when -O exit fails", async () => {
  const dir = path.join(
    os.tmpdir(),
    `hermes-ssh-close-${process.pid}-${Date.now()}`,
  );
  ...
  const conn = new SshConnection(
    { host: "box", user: "me" },
    { spawnFn, controlDir: dir },
  );
  await conn.open();
  fs.writeFileSync(conn.controlPath, ""); // simulate the lingering socket file
  await conn.close();
  assert.ok(
    !fs.existsSync(conn.controlPath),
    "failed -O exit drops the socket so the next open dials fresh",
  );
...
test("open() creates the control-socket directory if it does not exist", async () => {
  const dir = path.join(
    os.tmpdir(),
    `hermes-ssh-test-${process.pid}-${Date.now()}`,
  );
  assert.ok(!fs.existsSync(dir), "precondition: control dir absent");
  const spawnFn = scriptedSpawn((args) =>
    args.includes("check") ? { code: 255 } : { code: 0 },
  );
  const conn = new SshConnection(
    { host: "box", user: "me" },
    { spawnFn, controlDir: dir },
  ...
  try {
    ...
    assert.ok(
      fs.existsSync(dir),
      "open() created the control-socket directory before spawning ssh",
    );
  } finally {
    try {
      fs.rmSync(dir, { recursive: true, force: true });
...
test("exec() returns stdout on success and rejects (classified) on failure", async () => {
  ...
  const conn = new SshConnection(
    { host: "box", user: "me" },
    { spawnFn: okSpawn, controlDir: "/tmp/d" },
  );
  assert.equal((await conn.exec("uname -s")).trim(), "Linux");

  const failSpawn = scriptedSpawn([
    { code: 1, stderr: "ssh: Could not resolve hostname box" },
  ]);
  const conn2 = new SshConnection(
    { host: "box", user: "me" },
    { spawnFn: failSpawn, controlDir: "/tmp/d" },
  );
  await assert.rejects(
    () => conn2.exec("uname -s"),
    (err: any) => {
...
test("no-mux: ssh args carry no ControlMaster/ControlPath options", async () => {
  ...
  for (const args of spawnFn.calls) {
    assert.ok(
      !args.some((a) => /ControlMaster|ControlPath|ControlPersist/.test(a)),
      `mux option leaked: ${args}`,
    );
  }
});
...
test("SSH probe never creates or closes a ControlMaster", async () => {
  const spawnFn = scriptedSpawn({ code: 0 });
  const conn = createSshProbeConnection(
    { host: "box", user: "me" },
    { spawnFn },
  );
  await conn.open();
  ...
  assert.ok(
    !args.some((value) => /Control(?:Master|Path|Persist)/.test(value)),
  );
});

test("no-mux: open() classifies auth failure", async () => {
  const spawnFn = scriptedSpawn([
    { code: 255, stderr: "me@box: Permission denied (publickey)." },
  ]);
  const conn = new SshConnection(
    { host: "box", user: "me" },
    { spawnFn, mux: false },
...
test("no-mux: forward fails fast when the tunnel child dies (bad spec/auth)", async () => {
  const spawnFn: any = (_cmd, args) => {
    ...
    if (args.includes("-N")) {
      process.nextTick(() => {
        child.stderr.emit(
          "data",
          Buffer.from("Permission denied (publickey)."),
        );
        child.exitCode = 255;
      });
    }
  ...
  const conn = new SshConnection(
    { host: "box", user: "me" },
    { spawnFn, mux: false, forwardTimeoutMs: 2000 },
  );
  await assert.rejects(
    conn.forward(1, 9119),
    (err: any) => err.kind === "auth-failed",
  );
});
...
test("no-mux: an unrelated listener cannot mask a delayed bind failure", async () => {
  ...
  const spawnFn: any = (_cmd, args) => {
    ...
    if (args.includes("-N")) {
      setTimeout(() => {
        child.stderr.emit(
          "data",
          Buffer.from(`bind [127.0.0.1]:${localPort}: Address already in use`),
        );
        child.exitCode = 255;
        child.emit("exit", 255);
      }, 20);
  ...
  const conn = new SshConnection(
    { host: "box" },
    { spawnFn, mux: false, forwardTimeoutMs: 1000 },
  );
  await assert.rejects(
    conn.forward(localPort, 9119),
...
test("no-mux: tunnel death after readiness triggers a bounded restart, then unhealthy", async () => {
  ...
  assert.equal(
    await conn.isAlive(),
    true,
    "flap within the restart budget must not poison isAlive",
  );
  ...
  assert.equal(
    await conn.isAlive(),
    false,
    "exhausted budget reports the connection dead",
  );
  srv.close();
});
...
test("no-mux: cancelForward during a pending tunnel restart cancels the restart", async () => {
  ...
  assert.equal(
    tunnels.length,
    1,
    "cancelled forward must not restart its tunnel",
  );
  srv.close();
});
...
test("no-mux: close() during a pending tunnel restart cancels the restart", async () => {
  ...
  assert.equal(
    tunnels.length,
    1,
    "closed connection must not restart its tunnels",
  );
  srv.close();
});
...
test("validateSshTarget rejects a host starting with a dash (option injection)", () => {
  assert.throws(
    () => validateSshTarget("-oProxyCommand=evil", "", 22),
    /unsafe/i,
  );
...
test("validateSshTarget rejects commas in host (mistyped IP)", () => {
  assert.throws(
    () => validateSshTarget("100,84,204,123", "root", 22),
    /commas/i,
  );
});
...
test("validateSshTarget rejects a password glued into the host field", () => {
  ...
  try {
    ...
    assert.ok(
      !String(err.message).includes("hunter2"),
      "error message must not leak the credential",
    );
  }
});
...
test("validateSshTarget rejects garbage hostnames", () => {
  assert.throws(
    () => validateSshTarget("host!bad", "", 22),
    /not a valid hostname/i,
  );
  assert.throws(
    () => validateSshTarget("host/path", "", 22),
    /not a valid hostname/i,
  );
});
...
test("validateSshTarget accepts valid targets", () => {
  assert.doesNotThrow(() =>
    validateSshTarget("my-host.example.com", "alice", 22),
  );
...
test("SshConnection constructor rejects hostile host/user/port", () => {
  ...
  assert.throws(
    () => new SshConnection({ host: "box", user: "-oForward" }),
    /unsafe/i,
  );
...
test("buildExecArgs inserts -- before the destination", () => {
  ...
  assert.equal(
    args[ddIdx + 1],
    "me@box",
    "-- immediately precedes the destination",
  );
  assert.equal(
    args[ddIdx + 2],
    "uname -s",
    "remote command follows destination",
  );
});
...
test("hostArgs accepts valid key paths", () => {
  assert.deepEqual(hostArgs({ keyPath: "/home/user/.ssh/id_ed25519" }), [
    "-i",
    "/home/user/.ssh/id_ed25519",
  ]);
  assert.deepEqual(hostArgs({ keyPath: "~/.ssh/id_rsa" }), [
    "-i",
    "~/.ssh/id_rsa",
  ]);
});
...
test("runSsh delivers stdinData to the child and does not log it", async () => {
  ...
  const spawnFn: any = (_cmd, _args, opts) => {
    ...
    assert.equal(
      opts.stdio[0],
      "pipe",
      "stdin must be pipe when stdinData is provided",
    );
    process.nextTick(() => child.emit("close", 0));
  ...
  await runSsh(["host", "cat"], {
    timeoutMs: 5000,
    spawnFn,
    stdinData: "secret-token-value",
  });
  assert.equal(
    stdinWritten,
    "secret-token-value",
    "stdinData must be written to child.stdin",
  );
});
...
test("open() enforces 0700 on an existing control dir with lax permissions", async () => {
  ...
  assert.equal(
    stat.mode & 0o777,
    0o700,
    "control dir must be tightened to 0700",
  );
  fs.rmSync(tmp, { recursive: true, force: true });
});
