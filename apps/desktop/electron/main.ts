...
const DEV_CDP = resolveDevCdpPort({
  env: process.env,
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync("/dev/dxg")) {
  ...
  console.log(
    "[hermes] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration",
  );
}
...
if (IS_WINDOWS) {
  ...
  if (shouldAttemptAclRepair(priorMarker)) {
    ...
    if (acl.ok) {
      console.log(
        `[hermes] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`,
      );
    } else if (acl.error && acl.error !== "missing-target-or-exec") {
      console.warn(
        `[hermes] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`,
      );
    }
  }
  ...
  windowsSandboxFallbackSticky =
    sandboxDecision.nextMarker.state === "fallback";

  if (
    sandboxDecision.nextMarker.state === "fallback" &&
    sandboxDecision.nextMarker.reason
  ) {
  ...
  app.on("child-process-gone", (_event, details) => {
    if (
      !shouldRelaunchForGpuSandboxCrash({
        details,
        alreadyNoSandbox:
          windowsSandboxFallbackActive ||
          alreadyHasNoSandbox(process.argv, process.env),
    ...
    try {
      writeSandboxMarker(
        ...
        fallbackMarker("gpu-breakpoint", app.getVersion()),
      );
    } catch {
      void 0;
    }
    ...
    try {
      ...
      console.error(
        `[hermes] --no-sandbox relaunch failed: ${error?.message || error}`,
      );
    }
  });
}
...
function loadInstallStamp() {
  ...
  for (const p of candidates) {
    try {
      ...
      console.warn(
        `[hermes] install-stamp.json found at ${p} , but parsing failed with ${e}`,
      );
...
function pathWithHermesManagedNode(...entries) {
  const managed =
    hermesManagedNodePathEntries(HERMES_HOME).filter(directoryExists);

  return [...managed, ...entries, process.env.PATH]
    .filter(Boolean)
    .join(path.delimiter);
}
...
const DESKTOP_CONNECTIONS_REGISTRY_PATH = path.join(
  app.getPath("userData"),
  "connections.json",
);
const DESKTOP_INSTALLATION_PATH = path.join(
  app.getPath("userData"),
...
const DESKTOP_WINDOW_STATE_PATH = path.join(
  app.getPath("userData"),
  "window-state.json",
);
const DESKTOP_BACKEND_OWNERSHIP_PATH = path.join(
  app.getPath("userData"),
  "backend-ownership.json",
);
const DESKTOP_MANAGED_SSH_RECOVERY_PATH = path.join(
  app.getPath("userData"),
  "managed-ssh-update-recovery.json",
);
...
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(
    String(process.env.HERMES_DESKTOP_BOOT_FAKE_STEP_MS || ""),
    10,
  );
...
const NATIVE_THEME_CONFIG_PATH = path.join(
  app.getPath("userData"),
  "native-theme.json",
);
...
function readPersistedThemeSource() {
  try {
    const parsed = JSON.parse(
      fs.readFileSync(NATIVE_THEME_CONFIG_PATH, "utf8"),
    );
...
function writePersistedThemeSource(mode) {
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ themeSource: mode }, null, 2),
      "utf8",
    );
  } catch (error) {
...
const TRANSLUCENCY_CONFIG_PATH = path.join(
  app.getPath("userData"),
  "translucency.json",
);
...
function readPersistedTranslucency() {
  try {
    return normalizeTranslucency(
      JSON.parse(fs.readFileSync(TRANSLUCENCY_CONFIG_PATH, "utf8")),
      GLASS_SUPPORTED,
    );
  } catch {
...
function applyWindowOpacity(win) {
  ...
  if (
    ...
    opacityNeedsSetting(opacity, win.getOpacity?.() ?? 1)
  ) {
...
function applyWindowTranslucency(
  win,
  changed = { backing: true, material: true, opacity: true },
) {
    ...
    if (translucencyBackedWindows.has(win)) {
      ...
      if (changed.material) {
        ...
        if (IS_MAC && typeof win.setVibrancy === "function") {
          win.setVibrancy(vibrancyForTranslucency(translucencyState), {
...
function registerMediaProtocol() {
  const handler = createMediaProtocolHandler({
    ensureRemoteBearer: (baseUrl) =>
      ensureNativeAccessToken(baseUrl).catch(() => null),
...
function readPersistedPoolLimits() {
  try {
    ...
    if (
      fromEnv.maxBackends !== POOL_LIMITS_DEFAULTS.maxBackends ||
      fromEnv.idleMs !== POOL_LIMITS_DEFAULTS.idleMs
    ) {
      rememberLog(
...
const localBackendSpawnCoordinator = new LocalBackendSpawnCoordinator(
  poolLimits.maxBackends,
);
...
function planDesktopLogRotation(size) {
  ...
  const backups = (n) =>
    Array.from({ length: n }, (_, i) => desktopLogBackupPath(i + 1));

  // Pathological boot-loop log: reclaim live + every backup outright.
  if (size > DESKTOP_LOG_DISCARD_BYTES) {
    return [DESKTOP_LOG_PATH, ...backups(DESKTOP_LOG_BACKUP_COUNT)].map((p) => [
      "rm",
      p,
    ]);
  }
...
function openExternalUrl(rawUrl) {
  ...
  if (parsed.protocol === "file:") {
    ...
    try {
      localPath = resolveRequestedPathForIpc(parsed.toString(), {
...
function relaunchIntoSwappedBundle() {
  ...
  rememberLog(
    "[updates] app bundle was swapped during the update; relaunching into the new build",
  );

  try {
    app.relaunch({
      args: [
        ...buildNoSandboxRelaunchArgs(process.argv.slice(1)),
        BUNDLE_SWAP_RELAUNCH_FLAG,
      ],
    });
  } catch (err) {
    rememberLog(
      `[updates] bundle-swap relaunch failed: ${err?.message || err}; continuing with the current build`,
    );
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async (reason) => {
      if (!announced) {
        announced = true;
        rememberLog(
          `[updates] update in progress (${reason}); deferring backend start until it finishes`,
        );
      }
  ...
  try {
    ...
    if (result && result.ok && result.manual) {
      ...
      rememberLog(
        `[updates] detached update finished OK (branch ${result.branch})`,
      );
    } else if (result) {
      rememberLog(
  ...
  if (outcome === "timeout") {
    rememberLog(
      "[updates] update still in progress after wait timeout; starting backend anyway",
    );
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, BUNDLE_SWAP_RELAUNCH_FAILSAFE_MS),
    );
    rememberLog(
...
function backendSupportsServe(backend) {
  ...
  if (supported === null) {
    try {
      const prefix =
        backend.args && backend.args[0] === "-m"
          ? backend.args.slice(0, 2)
          : [];
...
function getBackendArgsForRuntime(backend) {
  return backendSupportsServe(backend)
    ? backend.args
    : dashboardFallbackArgs(backend.args);
}
...
function makeDashboardReadyFile() {
  ...
  return path.join(
    dir,
    `dashboard-${process.pid}-${Date.now()}-${crypto.randomBytes(6).toString("hex")}.json`,
  );
}
...
function readWindowState() {
  try {
    return sanitizeWindowState(
      JSON.parse(fs.readFileSync(DESKTOP_WINDOW_STATE_PATH, "utf8")),
    );
  } catch {
    return null;
  }
...
function persistWindowState() {
  ...
  try {
    ...
    writeFileAtomic(
      ...
      JSON.stringify(
        { x, y, width, height, isMaximized: mainWindow.isMaximized() },
        null,
        2,
      ),
    );
...
const DESKTOP_ZOOM_STATE_PATH = path.join(
  app.getPath("userData"),
  "zoom-state.json",
);
...
function writeZoomState(zoomLevel) {
  try {
    ...
    writeFileAtomic(
      ...
      JSON.stringify({ zoomLevel }, null, 2),
    );
  } catch (error) {
...
async function resolveHealedBranch(updateRoot, branch) {
  ...
  const remote = isOfficialSshRemote(originUrl)
...
async function checkUpdates() {
  ...
  const [currentSha, targetSha, dirtyStr, currentBranch, shallowStr] =
    await Promise.all([
      git(["rev-parse", "HEAD"]),
  ...
  if (behind === null) {
    behind = await fetchCompareBehindCount({
      currentSha,
      originUrl,
      targetSha,
    });
  ...
  const commits =
    behind !== 0 ? await readCommitLog(updateRoot, branch, isShallow) : [];
...
function killHermesOwnedVenvDaemons(updateRoot) {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(
        `[updates] stopping Hermes-owned venv daemon (hindsight) PID ${pid} before hand-off`,
      );
...
function writeBackendOwnership(contents) {
  fs.mkdirSync(path.dirname(DESKTOP_BACKEND_OWNERSHIP_PATH), {
    recursive: true,
  });
...
async function processIdentityMatches(identity, timeoutMs: number = 30_000) {
  ...
  try {
    return (
      (await processStartMarker(identity.pid, timeoutMs)) ===
...
async function backendIdentityMatches(identity) {
  const processMatches = await processIdentityMatches(
    identity,
    REAP_PROBE_TIMEOUT_MS,
  );
...
async function backendParentMatches(entry) {
  ...
  try {
    return (
      (await processStartMarker(entry.parentPid, REAP_PROBE_TIMEOUT_MS)) ===
      entry.parentStartMarker
    );
  } catch (error) {
    return error?.code === "ENOENT" || error?.code === "ESRCH"
...
async function stopOwnedBackend(identity) {
  ...
  if (matches !== true) {
    ...
    throw new Error(
      `Could not verify backend PID ${identity.pid} before stopping it.`,
    );
  }
  ...
  if (IS_WINDOWS) {
    ...
    while (Date.now() < deadline) {
      if (
        (await processIdentityMatches(identity, REAP_PROBE_TIMEOUT_MS)) !== true
      ) {
        return;
      }
  ...
  await new Promise((resolve) => setTimeout(resolve, 50));
  const remaining = await processIdentityMatches(
    identity,
    REAP_PROBE_TIMEOUT_MS,
  );
  ...
  store: {
    ...
    quarantine: () => {
      ...
      try {
        ...
        rememberLog(
          `Backend ownership file was unreadable; moved to ${parked}`,
        );
      } catch {
...
function releaseBackendChild(child) {
  ...
  try {
    ...
    rememberLog(
      `Could not release backend ownership for PID ${identity.pid}: ${error.message}`,
    );
  }
}
  ...
  if (!backendOrphanReapPromise) {
    backendOrphanReapPromise = backendOwnership
      ...
      .then((pids) => {
        if (pids.length) {
          rememberLog(
            `Reaped orphaned desktop backend PID(s): ${pids.join(", ")}`,
          );
        }
      })
      .catch((error) => {
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        if (
          currentHermesProcess &&
          Number.isInteger(currentHermesProcess.pid)
        ) {
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater) {
      ...
      rememberLog(
        "[updates] no staged updater; using repo hand-off script for CLI install",
      );
    }
    ...
    if (IS_WINDOWS) {
      ...
      if (scanOutcome.kind === "blocked" && opts.stopSafeBlockers) {
        const stopResult = await stopSafeVenvBlockers(
          updateRoot,
          scanOutcome.result,
        );
        rememberLog(
      ...
      for (
        let attempt = 0;
        scanOutcome.kind === "blocked" && attempt < 2;
        attempt++
      ) {
        rememberLog(
      ...
      if (scanOutcome.kind === "blocked") {
        ...
        rememberLog(
          `[updates] venv-blocked: ${scanOutcome.result.processes.length} process(es) hold the install`,
        );
        emitUpdateProgress({ stage: "error", message, percent: null });
        startHermes().catch(() => {});
        ...
        return {
          ...
          error: "venv-blocked",
          message,
          blockers: scanOutcome.result.processes,
        };
      }
      ...
      if (scanOutcome.kind === "probe-failure") {
        ...
        rememberLog(
          `[updates] venv-blocker probe failed: ${scanOutcome.error}`,
        );
        emitUpdateProgress({ stage: "error", message, percent: null });
        startHermes().catch(() => {});
    ...
    if (scriptHandoff) {
      ...
      if (
        ...
        stagedUpdaterSupportsPrewrittenMarker(updater)
      ) {
        writeUpdateMarker(HERMES_HOME, child.pid);
    ...
    const handoffOutcome = await observeUpdaterHandoff(
      child,
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  if (handoffConflict) {
    ...
    rememberLog(
      `[bootstrap] refusing recovery hand-off: ${handoffConflict.message}`,
    );
    isQuittingForHandoff = true;
    setTimeout(() => {
      app.quit();
  ...
  if (!handoffOutcome.ok) {
    rememberLog(
      `[bootstrap] recovery hand-off not viable, staying alive: ${handoffOutcome.message}`,
    );
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (handoffConflict) {
    ...
    rememberLog(
      `[updates] refusing posix hand-off: ${handoffConflict.message}`,
    );
    emitUpdateProgress({
      stage: "error",
      message: handoffConflict.message,
  ...
  if (!handoffOutcome.ok) {
    ...
    rememberLog(
      `[updates] posix hand-off not viable, aborting quit: ${handoffOutcome.message}`,
    );
    emitUpdateProgress({ stage: "error", message, percent: null });
...
function isActiveRuntimeUsable() {
  ...
  return (
    ...
    canImportHermesCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_HERMES_ROOT, process.env.PYTHONPATH]
          .filter(Boolean)
          .join(path.delimiter),
      },
    })
...
function resolveWebDist() {
  ...
  if (
    IS_PACKAGED &&
    /app\.asar(?=$|[\\/])/.test(fallback) &&
    !directoryExists(fallback)
  ) {
    rememberLog(
...
function resolveRendererIndex() {
  ...
  const candidates = IS_PACKAGED
    ? [webDistIndex, asarIndex]
    : [asarIndex, webDistIndex];
...
function createActiveBackend(backendArgs) {
  ...
  return {
    ...
    env: buildDesktopBackendEnv({
      ...
      pythonPathEntries: [
        ACTIVE_HERMES_ROOT,
        ...getVenvSitePackagesEntries(VENV_ROOT),
      ],
      venvRoot: VENV_ROOT,
    }),
...
function resolveHermesBackend(backendArgs) {
  ...
  if (bootstrapRepairRequested) {
    rememberLog(
      "[bootstrap] repair requested; bypassing the usable active runtime to re-run the installer",
    );
  }
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== "1") {
    ...
    if (hermesCommand) {
      const unwrapped = unwrapWindowsVenvHermesCommand(
        hermesCommand,
        backendArgs,
      );
      ...
      if (
        ...
        verifyHermesCli(hermesCommand, { shell: shellForProbe })
      ) {
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
          reject(
            new Error(
              `Unsupported Hermes backend URL protocol: ${parsed.protocol}`,
            ),
          );
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition("hermes:link-titles", {
    cache: false,
  });
...
const FAVICON_CACHE_PATH = path.join(
  app.getPath("userData"),
  "favicon-cache.json",
);
...
function loadFaviconCache(): Map<string, { at: number; icon: string }> {
  ...
  try {
    ...
    for (const [host, entry] of Object.entries(raw?.icons ?? {})) {
      ...
      if (
        Number.isFinite(at) &&
        Date.now() - at < (icon ? FAVICON_TTL_MS : FAVICON_MISS_TTL_MS)
      ) {
...
const faviconIo: FaviconIo = {
  ...
  fetchText: async (url) => {
    ...
    return response.ok
      ? (await response.text()).slice(0, TITLE_BYTE_BUDGET * 2)
      : "";
  },
};
...
function resolveFaviconCached(rawUrl: string): Promise<string> {
  ...
  if (
    hit &&
    Date.now() - hit.at < (hit.icon ? FAVICON_TTL_MS : FAVICON_MISS_TTL_MS)
  ) {
...
async function saveImageFromUrl(rawUrl) {
  ...
  const fallbackName = path.extname(baseName)
    ? baseName
    : `${baseName}${extension}`;
...
async function writeComposerImage(buffer, ext = ".png", name = "") {
  ...
  const fileName = safeName
    ? `${safeName}_${random}${safeExt}`
    : `composer_${stamp}_${random}${safeExt}`;
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  let resolved = resolveRequestedPathForIpc(
    /^file:/i.test(raw) ? raw : expandUserPath(raw),
    {
      baseDir: base,
  ...
  ({ resolvedPath: resolved } = await resolveReadableFileForIpc(resolved, {
    purpose: "Preview target",
  }));
  ...
  const isPdf =
    PREVIEW_PDF_EXTENSIONS.has(ext) || mimeType === "application/pdf";
...
async function filePathFromPreviewUrl(rawUrl) {
  const { resolvedPath } = await resolveReadableFileForIpc(
    String(rawUrl || ""),
    { purpose: "Preview file" },
  );
  ...
  const watcher = fs.watch(watchDir, () => {
    ...
    timer = setTimeout(() => {
      ...
      sendPreviewFileChanged({
        id,
        path: watchDir,
        url: pathToFileURL(watchDir).toString(),
      });
    }, PREVIEW_WATCH_DEBOUNCE_MS);
  });
...
async function buildReadinessHealthProbe(baseUrl, authMode, token) {
  const nativeAt =
    authMode === "oauth"
      ? await ensureNativeAccessToken(baseUrl).catch(() => null)
      : null;
  ...
  if (probeAuth.kind === "bearer") {
    return {
      ...
      probeHealth: (url, options: any = {}) =>
        fetchJson(url, null, { ...options, bearer: probeAuth.token }),
  ...
  if (probeAuth.kind === "token" && probeAuth.token) {
    return {
      probeHealth: (url, options: any = {}) =>
        fetchJson(url, probeAuth.token, options),
      probeIsCredentialed: true,
    };
  }
...
async function waitForHermes(baseUrl, token, signal?, authMode?, headers = {}) {
  ...
  return waitForHermesReady(baseUrl, {
    ...
    fetchJson: probeIsCredentialed
      ? (url, _token, options = {}) =>
          probeHealth(url, requestOptionsWithHeaders(options, headers))
      : fetchJson,
    probeHealth: (url, options = {}) =>
      probeHealth(url, requestOptionsWithHeaders(options, headers)),
    probeIsCredentialed,
  });
}
...
function sendWindowStateChanged(
    ...
    submenu: [
      ...
      { role: "forceReload" },
      {
        ...
        click: (_menuItem, browserWindow) =>
          toggleDevTools(browserWindow || mainWindow),
      },
      { type: "separator" },
      {
        label: "Actual Size",
        ...
        click: () => {
          if (mainWindow && !mainWindow.isDestroyed()) {
            setAndPersistZoomLevel(
              mainWindow,
              mainWindow.webContents.getZoomLevel() + ZOOM_STEP,
            );
          }
        },
      },
...
function setAndPersistZoomLevel(window, zoomLevel) {
  ...
  window.webContents
    ...
    .catch((error) =>
      rememberLog(`[zoom] persist failed: ${error?.message || error}`),
    );
}
...
function restorePersistedZoomLevel(window) {
  ...
  window.webContents
    ...
    .catch((error) =>
      rememberLog(`[zoom] restore failed: ${error?.message || error}`),
    );
}
...
function installZoomShortcuts(window) {
  ...
  window.webContents.on("before-input-event", (event, input) => {
    ...
    if (key === "0") {
      ...
      setAndPersistZoomLevel(
        window,
        window.webContents.getZoomLevel() + ZOOM_STEP,
      );
    } else if (key === "-") {
      if (input.shift) {
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on("context-menu", (_event, params) => {
    lastContextMenuPoint.set(window.webContents.id, {
      x: params.x,
      y: params.y,
    });
...
function installDownloadHandling() {
  session.defaultSession.on("will-download", (_event, item) => {
    ...
    const extension = hasExtension
      ? ""
      : extensionForMimeType(item.getMimeType());
...
async function gatedFileAuth(connection: GatewayFileConnection) {
  const nativeAt =
    connection.authMode === "oauth"
      ? await ensureNativeAccessToken(connection.baseUrl).catch(() => null)
      : null;

  return resolveGatedDownloadAuth(
    connection.authMode,
    nativeAt,
    connection.token,
  );
...
async function saveGatewayFile(payload: GatewayFileSavePayload = {}) {
  ...
  const { connection, connectionId, profile } =
  ...
  try {
    ...
    if (isNotFoundError(error)) {
      return await saveGatewayFileViaDataUrl(
        connection,
        requestPaths.dataUrl,
        ctx,
      );
    }
...
async function mintGatewayWsTicket(baseUrl, headers = {}) {
  return withTransientRetries(async () => {
    ...
    const body = (await fetchJsonViaOauthSession(
...
function resolvePortalBaseUrl() {
  const raw =
    process.env.HERMES_PORTAL_BASE_URL ||
    process.env.NOUS_PORTAL_BASE_URL ||
  ...
  portalAccessRenewal = (async () => {
    ...
    return await new Promise<boolean>((resolve) => {
      ...
      const finish = (ok: boolean) => {
        ...
        rememberLog(
          `[cloud] silent portal access renewal ${ok ? "succeeded" : "did not complete"}`,
        );
        resolve(ok);
      };
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(
        new Error("Desktop is not ready to start a Hermes Cloud sign-in."),
      );
    ...
    win.on("closed", () => {
      if (!settled) {
        finish(
          new Error("Sign-in window closed before authentication completed."),
        );
      }
    });
...
function parseOrgSelectionError(error) {
  ...
  if (
    parsed?.error !== "org_selection_required" ||
  ...
  return agents
    ...
    .map((a) => ({
      ...
      dashboardGatewayState:
        typeof a.dashboardGatewayState === "string"
          ? a.dashboardGatewayState
          : "unknown",
    }));
}
...
// ---------------------------------------------------------------------------
const SECRET_STORAGE_POLICY_PATH = path.join(
  app.getPath("userData"),
  SECRET_STORAGE_POLICY_FILE,
);
...
const _secretStoragePolicyIo = {
  ...
  writeText: (text: string) =>
    writeSecretFileAtomic(SECRET_STORAGE_POLICY_PATH, text, {
      encoding: "utf8",
    }),
};
 ...
 */
function rewriteAllStoredSecrets(
  ...
  const rewriteBlock = (block: any) => {
    ...
    const next = {
      ...block,
      ...(block.token ? { token: reencode(block.token) } : {}),
    };

    if (block.headers && typeof block.headers === "object") {
      next.headers = Object.fromEntries(
        Object.entries(block.headers).map(([k, v]) => [k, reencode(v)]),
      );
    }
  ...
  if (
    blockNeedsRewrite(config.remote) ||
    Object.values(config.profiles || {}).some(blockNeedsRewrite)
  ) {
    touched = true;
    writeDesktopConnectionConfig({
      ...
      profiles: Object.fromEntries(
        Object.entries(config.profiles || {}).map(([k, v]) => [
          k,
          rewriteBlock(v),
        ]),
      ),
  ...
  if (registry.connections?.some(blockNeedsRewrite)) {
    ...
    writeDesktopConnectionsRegistry({
      ...registry,
      connections: registry.connections.map(rewriteBlock),
    });
  }
  ...
  try {
    ...
    if (store && typeof store === "object" && !Array.isArray(store)) {
      ...
      if (entries.some(([, v]) => shouldRewrite(v))) {
        ...
        io.writeStoreText(
          JSON.stringify(
            Object.fromEntries(entries.map(([k, v]) => [k, reencode(v)])),
          ),
        );
      }
    }
...
function migrateLegacyEncryptedSecretsOnce() {
  ...
  const needsMigration = (secret: any) =>
    classifyStoredSecret(secret, policy) === "migrate";
  ...
  if (touchedKeychain) {
    rememberLog(
      "[secret-storage] migrated legacy keychain-encrypted secrets to opt-out storage (one-shot pass)",
    );
  }
}
...
function applySecretStorageEncryption(on: boolean) {
  ...
  if (enable) {
    const needsEncrypt = (secret: any) =>
      secret?.encoding === "plain" && Boolean(secret.value);
...
function encryptIncomingRemoteHeaders(
  ...
  for (const [name, value] of Object.entries(raw || {})) {
    ...
    if (typeof value === "string") {
      ...
      if (trimmed) {
        out[key] = encryptDesktopSecret(trimmed, {
...
function headersForRemoteRequest(requestUrl) {
  ...
  if (modeIsRemoteLike(config.mode) && config.remote?.url) {
    ...
    if (
      ...
      remoteRequestMatchesBaseUrl(requestUrl, config.remote.url)
    ) {
      return headers;
    }
  }
...
function readDesktopConnectionsRegistry() {
  ...
  try {
    ...
    registry = normalizeRegistry(
      JSON.parse(fs.readFileSync(DESKTOP_CONNECTIONS_REGISTRY_PATH, "utf8")),
    );
  } catch {
  ...
  const reconciled = reconcileRegistryDrift(
    registry,
...
function writeDesktopConnectionsRegistry(registry) {
  fs.mkdirSync(path.dirname(DESKTOP_CONNECTIONS_REGISTRY_PATH), {
    recursive: true,
  });
  ...
  connectionRegistryCache = registry;
  connectionRegistryCacheMtime = fs.statSync(
    DESKTOP_CONNECTIONS_REGISTRY_PATH,
  ).mtimeMs;
}
...
function sanitizeConnectionsRegistry(
  registry = readDesktopConnectionsRegistry(),
) {
  ...
  return {
    ...
    quarantined: (registry.quarantined || []).map((q) => ({
      ...
      label:
        q &&
        q.entry &&
        typeof q.entry === "object" &&
        typeof (q.entry as any).label === "string"
...
async function saveRegistryConnection(input: any = {}) {
  ...
  const existing = input.id
    ? registry.connections.find((c) => c.id === input.id)
    : null;
  const incomingToken =
  ...
  if (
    ...
    !decryptDesktopSecret(entry.token)
  ) {
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    ...
    readdirSync: (p, opts) =>
      fs.readdirSync(p, opts as { withFileTypes: true }),
    isHermesProcess,
    now: () => Date.now(),
...
async function sanitizeDesktopConnectionConfig(
  ...
  const envOverride = key
    ? false
    : Boolean(process.env.HERMES_DESKTOP_REMOTE_URL);
  ...
  const remoteTokenPlainText =
    !envOverride && block.token?.encoding === "plain";
...
function coerceDesktopConnectionConfig(
  ...
  const leavingSsh =
    rawExistingBlock.mode === "ssh" && mode !== "ssh" && mode !== "local";
  ...
  const cloudOrg =
    mode === "cloud"
      ? String(input.cloudOrg ?? existingBlock.org ?? "").trim()
      : "";
  const incomingToken =
  ...
  if (mode === "ssh") {
    const sshBlock = buildSshBlock(
      input,
      savedProfileSsh(existing, key) || rawExistingBlock,
    );
    ...
    if (key) {
      ...
      return {
        mode:
          existing.mode === "ssh" || modeIsRemoteLike(existing.mode)
            ? existing.mode
            : "local",
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteHermesPath:
      input.sshRemoteHermesPath ?? existingBlock.remoteHermesPath,
  ...
  if (authMode === "oauth") {
    ...
    if (
      !oauthSessionIsLive(
        ...
        await hasLiveOauthSession(baseUrl),
      ) &&
...
const desktopInstallationId = loadOrCreateInstallationId(
...
const managedConnectionUpdateGate = new ManagedConnectionUpdateGate(
  (connectionId) =>
    readManagedSshRecoveryRecords().find(
...
const managedPrimaryRestoreOwners = new Map<
  string,
  { correlationId: string; profile: string; source: any }
>();
...
function assertCanMutateManagedPrimaryRouting() {
  const durableIds = readManagedSshRecoveryRecords().map(
...
function readManagedSshRecoveryRecords(): any[] {
  try {
    ...
    if (
      ...
      throw new Error(
        "Managed SSH recovery journal is not a safe owner-only file.",
      );
    }

    const payload = JSON.parse(
      fs.readFileSync(DESKTOP_MANAGED_SSH_RECOVERY_PATH, "utf8"),
    );
    ...
    const valid = payload.records.every((record) => {
      ...
      const identities = record.scopes.map(
        (scope) => `${scope.kind}\0${scope.key}\0${scope.profile}`,
      );
    ...
    if (!valid) {
      throw new Error(
        "Managed SSH recovery journal contains an invalid record.",
      );
    }
...
function writeManagedSshRecoveryRecords(records) {
  fs.mkdirSync(path.dirname(DESKTOP_MANAGED_SSH_RECOVERY_PATH), {
    recursive: true,
  });
  writeSecretFileAtomic(
    DESKTOP_MANAGED_SSH_RECOVERY_PATH,
    JSON.stringify(
      { version: 1, records, updatedAt: new Date().toISOString() },
      null,
      2,
    ),
  );
...
function persistManagedSshRecovery(source, correlationId, scopes) {
  ...
  const records = readManagedSshRecoveryRecords().filter(
    (record) => record.connectionId !== source.id,
  );
  records.push({
...
function markManagedSshRecoveryLaunching(connectionId, correlationId) {
  ...
  if (index < 0) {
    throw new Error(
      "Managed SSH recovery record disappeared before remote update launch.",
    );
  }
...
function activeSshTerminalTarget(webContentsId?: number) {
  const windowRoute =
    typeof webContentsId === "number"
      ? windowConnectionRoutes.get(webContentsId)
      : null;
...
async function ensureTerminalBackend(webContentsId: number) {
  ...
  // here avoids bootstrapping a second SSH tunnel / remote dashboard.
  if (windowRoute?.registryScoped && windowRoute.connectionId) {
    return backendDialClaims.run(
      backendScopeKey(windowRoute.connectionId, windowRoute.profile),
      () =>
  ...
  return backendDialClaims.run(backendScopeKey(null, profile), () =>
    ensureBackend(profile),
  );
}
 ...
 */
async function reachablePreviewUrl(
  ...
  try {
    const rewritten = await reach.registry.resolve(rawUrl, {
      cancel: (localPort, remotePort) =>
        ssh.cancelForward(localPort, remotePort),
      forward: (localPort, remotePort, remoteHost) =>
        ssh.forward(localPort, remotePort, remoteHost),
    ...
    sshRememberLog(
      `preview reach failed for ${rawUrl}: ${error?.message || error}`,
    );
...
async function effectiveSshConfigFingerprint(sshConfig) {
  ...
  args.push(
    "--",
    sshConfig.user ? `${sshConfig.user}@${sshConfig.host}` : sshConfig.host,
  );
...
async function bootstrapSshConnection(
  ...
  const scope = sshScopeKey(profile);
  const effectiveConfigFingerprint =
    resolvedEffectiveFingerprint ||
    (await effectiveSshConfigFingerprint(sshConfig));
...
async function rollbackSshBootstrapResult(
  ssh,
  result,
  profile,
  sshConfig,
  ...
  try {
    const expected = {
      ...
      profile: resolveRemoteSshDashboardProfile(
    ...
    if (result.platform?.os === "Windows") {
      ...
      result.platform?.os === "Darwin"
    ) {
      await remoteLifecycle.terminateOwnedDashboardForUpdate(ssh, expected);
    } else {
      cleanupErrors.push(
        `unsupported remote platform ${result.platform?.os || "unknown"}`,
      );
    }
  } catch (error: any) {
...
async function bootstrapSshConnectionInner(
  ...
  const scope = sshScopeKey(profile);
  const hostLabel = sshConfig.user
  ...
  try {
    ...
    result = await lifecycle({
      ...
      waitForHermes: (baseUrl, token) =>
        waitForHermes(baseUrl, token, lease.signal, "token"),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        remoteProfile: resolveRemoteSshDashboardProfile(
          sshConfig.remoteProfile,
          profile,
        ),
    ...
    rollback: (error) =>
      rollbackSshBootstrapResult(ssh, result, profile, sshConfig, error),
  });
...
function persistSshConnectionToken(
  profile,
  source,
  token,
  registryConnectionId = "",
  ...
  try {
    const persistence = managedSshTokenPersistencePlan(
      source,
      registryConnectionId,
    );
    ...
    if (id) {
      ...
      if (entry && entry.kind === "ssh") {
        writeDesktopConnectionsRegistry(
          upsertConnection(registry, { ...entry, token: encrypted }),
        );
      }
    }
...
async function resolveRemoteBackend(
  profile,
  options: { poolKey?: string; primary?: boolean } = {},
) {
  const profileKey = String(profile || "").trim() || "default";

  const managedPrimary = options.primary
    ? [...managedPrimaryRestoreOwners.values()].find(
        (owner) => owner.profile === profileKey,
      )
    : null;
  ...
  if (managedPrimary) {
    ...
    if (!sshConfig) {
      throw new Error(
        `SSH connection "${source.label}" has no host configured.`,
      );
    }

    managedConnectionUpdateGate.assertCanDial(
      source.id,
      managedPrimary.correlationId,
    );
  ...
  if (route.kind === "ssh") {
    ...
    connection = await bootstrapSshConnection(
      ...
      undefined,
      {
        managedScope: options.primary
          ? "primary"
          : options.poolKey
            ? "pool"
            : "transient",
        poolKey: options.poolKey || "",
        primaryRegistryScope:
          options.primary === true && Boolean(route.connectionId),
        registryConnectionId: route.connectionId || "",
      },
    );
  } else {
    const token =
      route.authMode === "oauth"
        ? null
        : route.source === "env"
          ? route.token
          : decryptDesktopSecret(route.token);
...
function configuredRemoteProfileNames() {
  ...
  return Object.keys(config.profiles || {}).filter((name) =>
    profileHasRemoteConnection(config, name),
  );
}
...
function primaryBackendIsRemote() {
  return (
    Boolean(profileHasRemoteOverride(primaryProfileKey())) ||
    globalRemoteActive()
  );
}
...
async function requestJsonForProfile(
  ...
  if (conn.authMode === "oauth") {
    ...
    const nativeAt = await ensureNativeAccessToken(conn.baseUrl).catch(
      () => null,
    );
      ...
      for (;;) {
        try {
          ...
          if (platform.os === "Windows") {
            ...
            const inspection = await helper(ssh, runtime, "inspect", [
              runtime.hermesPath,
            ]);
            ...
            hermesVersion = await remoteLifecycle.probeHermesVersion(
              ssh,
              hermesPath,
            );
            supported = await remoteLifecycle.remoteSupportsSshOwnership(
              ssh,
              hermesPath,
            );
          }
          ...
          if (!supported) {
            return {
              ...
              error:
                "Update Hermes on the remote host before connecting with Desktop SSH.",
            };
          }
...
function profileRouteOptions(profile, request?) {
  ...
  return {
    ...
    profileRemoteOverride: Boolean(
      profileRemoteOverride(config, profile) || sshOverride,
    ),
...
async function ensureBackend(profile) {
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(
    async (error) => {
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteProfile:
          source.remoteProfile || (profileKey === "default" ? "" : profileKey),
      });
    }
    ...
    return registryDispatchRevalidation.run(connectionPromise, () =>
      ensureHealthyPooledRemoteBackendForDispatch({
        ...
        currentConnectionPromise: () =>
          backendPool.get(key)?.connectionPromise || null,
        probe: (connection, requestPath, options) =>
          fetchJsonForBackend(connection, requestPath, options),
...
async function connectRegistryBackend(
  ...
  if (source.kind === "ssh") {
    ...
    const connection = await bootstrapSshConnection(
      ...
      resolvedEffectiveFingerprint
        ? await resolvedEffectiveFingerprint
        : undefined,
      {
        managedScope: "pool",
        managedUpdateCorrelation,
...
async function ensureManagedSshBackend(source, profile, correlationId) {
  return ensureManagedSshBackendAtKey(
    source,
    profile,
...
async function ensureManagedSshBackendAtKey(
  source,
  profile,
  key,
  correlationId,
...
async function restoreManagedPrimarySshBackend(source, profile, correlationId) {
  ...
  if (
    managedPrimaryRestoreOwners.size > 0 &&
    !managedPrimaryRestoreOwners.has(source.id)
  ) {
    throw new Error(
  ...
  managedPrimaryRestoreOwners.set(source.id, {
    correlationId,
    profile: profileKey,
    source,
  });
  ...
  try {
    ...
    if (
      managedPrimaryRestoreOwners.get(source.id)?.correlationId ===
      correlationId
    ) {
...
async function captureManagedSshScopes(source) {
  ...
  const pooled = [...backendPool.entries()]
    ...
    .map(([key, entry]) => ({
      ...
      key,
      profile: String(key).startsWith(prefix)
        ? String(key).slice(prefix.length) || "default"
        : String(key),
  ...
  if (
    ...
    primary.push({
      ...
      forwardRestored: false,
      key:
        primaryRoute.source === "profile"
          ? sshScopeKey(primaryProfile)
          : sshScopeKey(null),
      primary: true,
  ...
  if (primary.length > 1) {
    throw new Error(
      "Managed SSH update found multiple primary scopes; refusing an ambiguous drain.",
    );
  }
  ...
  await waitForManagedSshBootstrapFence(
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.hermesPath || !state?.hermesHome) {
    throw new Error(
      "The managed SSH scope does not carry a complete remote runtime identity.",
    );
  }

  if (!["Darwin", "Linux", "Windows"].includes(state.remotePlatform)) {
    throw new Error(
      `Unsupported managed SSH update platform: ${state.remotePlatform || "unknown"}.`,
    );
  }
  ...
  try {
    ...
    if (platform.os === "Windows") {
      const runtime = platform.hermesPath
        ? platform
        : await probeWindowsRemote(ssh, config.remoteHermesPath || "");
    ...
    if (state.remotePlatform === "Windows") {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        {
          hermesPath: state.hermesPath,
          hermesHome: state.hermesHome,
          python: state.pythonPath,
        },
        expected,
      );
    } else if (
      state.remotePlatform === "Linux" ||
      state.remotePlatform === "Darwin"
    ) {
      await remoteLifecycle.terminateOwnedDashboardForUpdate(
        state.ssh,
        expected,
      );
    } else {
...
async function updateManagedSshConnection(source, correlationId) {
  ...
  return runManagedSshUpdate({
    ...
    scopes,
    preflightRemote: () =>
      assertManagedUpdatePreflightClear(target, correlationId),
    ...
    awaitRestoreClearance: () =>
      waitForManagedRemoteClearance(target, correlationId, {
        requireTerminal: launchAttempted,
      }),
    ...
    restoreScope: (scope) => {
      if (scope.unsafeDrainFailure) {
        if (!scope.forwardRestored) {
          throw new Error(
            `The original ${scope.profile} SSH forward could not be restored safely.`,
          );
        }
    ...
    prepareRecovery: async () =>
      persistManagedSshRecovery(sourceSnapshot, correlationId, scopes),
    completeRecovery: async () =>
      clearManagedSshRecovery(source.id, correlationId),
    releaseGate: () =>
      managedConnectionUpdateGate.release(source.id, correlationId),
  });
}
...
async function recoverManagedSshUpdate(record) {
  ...
  if (
    managedConnectionRecoveries.has(connectionId) ||
    managedConnectionUpdates.has(connectionId)
  ) {
    return;
  }
  ...
  const operation = (async () => {
    ...
    try {
      ...
      const results = await recoverManagedSshScopes<any>({
        ...
        restoreScope: (scope) =>
          scope.kind === "primary"
            ? restoreManagedPrimarySshBackend(
                record.source,
                scope.profile,
                recoveryCorrelation,
              )
            : scope.kind === "legacy"
              ...
              : ensureManagedSshBackend(
                  record.source,
                  scope.profile,
                  recoveryCorrelation,
                ),
        completeRecovery: async () =>
          clearManagedSshRecovery(connectionId, record.correlationId),
      });
...
async function resumeManagedSshRecoveries() {
  await Promise.allSettled(
    readManagedSshRecoveryRecords().map((record) =>
      recoverManagedSshUpdate(record),
    ),
  );
}
...
async function stopRegistryConnectionBackends(connectionId) {
  ...
  const sshScopes = new Set([
    ...[...sshConnections.keys()].filter((scope) =>
...
function evictLruPoolBackends(keep) {
  const evictions = selectPoolEvictions(
    backendPool.entries(),
    Math.max(0, keep),
    Date.now(),
  ...
  poolIdleReaper = setInterval(() => {
    ...
    for (const [profile, entry] of [...backendPool.entries()]) {
      if (now - (entry.lastActiveAt || 0) > poolIdleMs()) {
        rememberLog(
          `Reaping idle profile backend "${profile}" (idle > ${Math.round(poolIdleMs() / 1000)}s)`,
        );
        stopPoolBackend(profile);
      }
    }
...
function releaseLocalBackendSlot(entry: any) {
  ...
  const request =
    entry.localBackendSpawnRequest as LocalBackendSpawnRequest | null;
...
function assertPoolEntryStillOwned(poolKey: string, entry: any) {
  if (backendPool.get(poolKey) !== entry) {
    ...
    throw new Error(
      `Profile backend start for "${poolKey}" was cancelled before spawn.`,
    );
  }
}
...
function teardownFailedLocalBackend(
  poolKey: string,
  entry: any,
): Promise<void> {
...
async function spawnPoolBackend(
  ...
  const remote = opts.forceLocal
    ? null
    : await resolveRemoteBackend(profile, { poolKey });
  profileDeletionGate.assertCanStart(profile);
  ...
  rememberLog(
    `Starting Hermes backend for profile "${profile}" via ${backend.label}`,
  );

  const parentStartMarker = await desktopParentStartMarker();
  const backendNonce = crypto.randomBytes(16).toString("hex");
  const parentIdentityEnv = parentWatchdogEnv(
    process.pid,
    parentStartMarker,
    backendNonce,
  );
  ...
  child.once("error", (error) => {
    rememberLog(
      `Hermes backend for profile "${profile}" failed to start: ${error.message}`,
    );
  ...
  child.once("exit", (code, signal) => {
    rememberLog(
      `Hermes backend for profile "${profile}" exited (${signal || code})`,
    );
    releaseLocalBackendSlot(entry);
    releaseBackendChild(child);
...
async function teardownPoolBackendAndWait(profile) {
  await Promise.all(
    localProfilePoolKeys(profile).map((key) => stopPoolBackend(key)),
  );
}
...
async function prepareProfileDeleteRequest(request) {
  ...
  if (decision.action === "teardown-primary") {
    ...
    await Promise.all([
      ...
      teardownPoolBackendAndWait(decision.profile),
    ]);
...
async function startHermes() {
  ...
  let attemptedRemote =
    managedPrimaryRestoreOwners.size > 0 || primaryBackendIsRemote();
  ...
  const connectionPromise = (async () => {
    const connectRemote = async (remote) => {
      ...
      return createPrimaryRemoteConnection(
        remote,
        hermesLog.slice(-80),
        getWindowState(),
      );
    ...
    if (loginShellPath.applied) {
      rememberLog(
        "[env] merged login-shell PATH into process.env for backend spawn",
      );
    } else if (
      loginShellPath.reason &&
      !["win32", "unchanged"].includes(loginShellPath.reason)
    ) {
      rememberLog(
    ...
    await advanceBootProgress(
      "backend.spawn",
      `Starting Hermes backend via ${backend.label}`,
      84,
    );
    ...
    await advanceBootProgress(
      "backend.port",
      "Waiting for Hermes backend to launch",
      86,
    );
    ...
    await advanceBootProgress(
      "backend.wait",
      "Waiting for Hermes backend to become ready",
      90,
    );
    ...
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () =>
        hermesProcess.exitCode === null && !hermesProcess.killed,
      rememberLog,
    });
    ...
    const isCloudBackendDown = Boolean(
      error &&
      typeof error === "object" &&
      (error as any).isCloudBackendDown === true,
    );
...
function wireWindowReveal(
  win,
  { show, onRevealed }: { show?: () => void; onRevealed?: () => void } = {},
) {
...
function createSessionWindow(
  sessionId,
  { profile = null, watch = false } = {},
) {
  return sessionWindows.openOrFocus(sessionId, () =>
    spawnSecondaryWindow({ sessionId, profile, watch }),
  );
}
...
function spawnPetOverlayWindow(bounds) {
  ...
  win.on("closed", () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send("hermes:pet-overlay:control", {
        type: "pop-in",
      });
    }
  });
...
function readHudState() {
  try {
    ...
    if (
      [raw?.x, raw?.y, raw?.width, raw?.height].every((v) =>
        Number.isFinite(v),
      ) &&
      raw.width >= 380 &&
      raw.height >= 160
...
function persistHudState() {
  ...
  try {
    ...
    writeFileAtomic(
      HUD_STATE_PATH,
      JSON.stringify({ x, y, width, height }, null, 2),
    );
  } catch (err) {
...
const hudSnapShortcut = createHudSnapShortcut(
  globalShortcut,
...
function registerHudSnapShortcut() {
  if (!hudSnapShortcut.register()) {
    rememberLog(
      "[hud] snap shortcut unavailable — CommandOrControl+Shift+G may be owned by another app",
    );
  }
}
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const enumerate = async () => {
    const windows = await enumerateWindowsFrontToBack(
      process.pid,
      titlesAvailable,
    );
...
const QUICK_ENTRY_CONFIG_PATH = path.join(
  app.getPath("userData"),
  "quick-entry.json",
);
...
function readQuickEntrySettings() {
  try {
    return sanitizeQuickEntrySettings(
      JSON.parse(fs.readFileSync(QUICK_ENTRY_CONFIG_PATH, "utf8")),
    );
  } catch {
...
const quickEntryShortcut = createQuickEntryShortcut(
  globalShortcut,
...
function applyQuickEntrySettings(settings) {
  ...
  if (state.error === "taken") {
    rememberLog(
      `[quick-entry] shortcut ${state.shortcut} is already taken by another application`,
    );
  } else if (state.error === "invalid") {
    rememberLog(
      `[quick-entry] shortcut ${state.shortcut} is not a valid accelerator`,
    );
  }
...
function createWindow() {
    ...
    onRevealed: () => {
      ...
      if (IS_WINDOWS) {
        try {
          ...
          rememberLog(
            `[sandbox] marker update after main-window reveal failed: ${error?.message || error}`,
          );
        }
      }
    },
  ...
  installWindowRendererLifecycle(mainWindow, {
    ...
    callbacks: {
      ...
      onCrashLoopSuppressed: (details) => {
        ...
        try {
          app.relaunch({
            args: buildNoSandboxRelaunchArgs(process.argv.slice(1)),
          });
          void exitAfterBackendShutdown(0);
        } catch (err) {
          rememberLog(
            `[renderer] --no-sandbox relaunch failed: ${err?.message || err}`,
          );
        }
      },
      ...
      onFailedLoadBudgetExhausted: (details) => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          url: details?.url,
          errorDescription:
            "The desktop renderer failed to load repeatedly after the update.",
  ...
  if (!DEV_SERVER && rendererIndex && tornAssets.length > 0) {
    ...
    loadWindowUrl(
      mainWindow,
      DEV_SERVER ||
        pathToFileURL(rendererIndex || resolveRendererIndex()).toString(),
      "Renderer",
    );
  }
...
ipcMain.handle("hermes:connection", async (_event, profile) => {
  ...
  const profileKey =
    profile && String(profile).trim()
      ? String(profile).trim()
      : primaryProfileKey();
  const connection = await backendDialClaims.run(
    backendScopeKey(null, profileKey),
    () => ensureBackend(profile),
  );
    ...
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        probe: (connection, path, options) =>
          fetchJsonForBackend(connection, path, options),
...
ipcMain.handle(
  ...
  async (_event, sessionId, opts) => {
    ...
    try {
      ...
      const backend = resolveHermesBackend(
        tuiResumeArgs(sessionId.trim(), profile || undefined),
      );
      ...
      rememberLog(
        `[terminal] opening session ${sessionId} via ${launch.command}`,
      );
...
ipcMain.handle("hermes:wake-indicator:get", () =>
  wakeIndicatorController.getState(),
);
...
ipcMain.handle("hermes:bootstrap:continue-local", async () => {
  rememberLog(
    "[bootstrap] local install selected by renderer; continuing first-launch bootstrap",
  );
  ...
  async (_event, rawProfileNames) => {
    ...
    if (sshSeeds.length > 0) {
      agents = [
        ...agents,
        ...sshSeeds.map((seed) => {
          const source = registry.connections.find(
            (connection) => connection.id === seed.connectionId,
          )!;
    ...
    const localSource = registry.connections.find(
...
ipcMain.handle("hermes:ssh-config:resolve", async (_event, host) => {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      ssh,
      ["-G", "--", value],
      hiddenWindowsChildOptions({ stdio: ["ignore", "pipe", "pipe"] }),
    );
    let stdout = "";
    let stderr = "";
...
ipcMain.handle("hermes:connections:save", async (_event, payload) => {
  ...
  return {
    ok: true,
    connection: saved,
    registry: sanitizeConnectionsRegistry(),
  };
});
...
ipcMain.handle("hermes:connections:set-primary", async (_event, id) => {
  ...
  const registry = setPrimaryConnection(
    readDesktopConnectionsRegistry(),
    String(id || ""),
  );
  writeDesktopConnectionsRegistry(registry);
...
ipcMain.handle("hermes:connections:set-launch-mode", async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting();
  const registry = setConnectionLaunchMode(
    readDesktopConnectionsRegistry(),
    String(mode || ""),
  );
  writeDesktopConnectionsRegistry(registry);
...
ipcMain.handle("hermes:connections:set-last-used", async (_event, id) => {
  const registry = setLastUsedConnection(
    readDesktopConnectionsRegistry(),
    String(id || ""),
  );
  writeDesktopConnectionsRegistry(registry);
...
async function probeConnectionInstallId(
  connectionId: string,
  descriptor: any,
  ...
  if (
    cached &&
    Date.now() - cached.ts <
      (cached.id ? INSTALL_ID_TTL_MS : INSTALL_ID_NEGATIVE_TTL_MS)
  ) {
    return cached.id;
  }

  try {
    const status: any = await getJsonForBackend(descriptor, "/api/status", {
      timeoutMs: 8_000,
    });
...
async function probeSshProfileInventory(connection) {
  ...
  try {
    ...
    sshRememberLog(
      `[ssh] profile inventory failed for ${connection.id}: ${error?.message || error}`,
    );
  } finally {
    try {
      await ssh.close();
...
async function enumerateRegistryAgentSources(
  registry = readDesktopConnectionsRegistry(),
) {
    ...
    try {
      return await Promise.race([
        ...
        new Promise<never>((_resolve, reject) => {
          timer = setTimeout(
            () => reject(new Error("roster enumeration timed out")),
            perSourceTimeoutMs,
          );
        }),
      ]);
  ...
  return Promise.all(
    registry.connections.map(async (connection) => {
        ...
        if (connection.kind === "ssh") {
          ...
          if (connection.kind === "local") {
            const localRoute = resolveRegistryLocalRoute("default", {
              globalRemote: globalRemoteActive(),
              profileRemoteOverride: Boolean(
                profileHasRemoteOverride(primaryProfileKey()),
              ),
            });
          ...
          const body: any = await getJsonForBackend(
            descriptor,
            "/api/profiles",
            { timeoutMs: 8_000 },
          );
          ...
          const installId = await probeConnectionInstallId(
            connection.id,
            descriptor,
          );
            ...
            ? Object.fromEntries(
                body.profiles
                  ...
                  .filter(
                    (
                      entry,
                    ): entry is readonly [string, RosterProfileMetadata] =>
                      Boolean(entry),
                  ),
              )
            : undefined;
      ...
      const remembered = rememberSshEnumeration(
        raw,
        sshRosterCache.get(connection.id),
        connection.kind,
      );
...
async function requestManagedSshUpdate(rawId) {
  ...
  if (!managedConnectionUpdateGate.claim(connectionId, correlationId)) {
    return refusedManagedSshUpdate(
      connectionId,
      correlationId,
      "A managed update is already in progress.",
    );
  }
...
ipcMain.handle("hermes:connections:update-all", async (_event, payload) => {
  ...
  const excludeIds = new Set<string>(
    Array.isArray((payload as any)?.excludeIds)
      ? (payload as any).excludeIds.map((id: unknown) => String(id))
      : [],
  );
        ...
        try {
          if (connection.kind === "local") {
            ...
            return {
              ...base,
              ok: result?.ok !== false,
              detail: result?.message || "update started",
            };
          }
          ...
          const descriptor: any = await backendDialClaims.run(
          ...
          return {
            ...base,
            ok: true,
            detail: body?.message || "update started",
          };
        } catch (error: any) {
...
async function postJsonForBackend(descriptor, path, body, opts: any = {}) {
  return fetchJsonForBackend(descriptor, path, {
    ...opts,
    body: body ?? {},
    method: "POST",
  });
...
async function fetchJsonForBackend(
  ...
  if (descriptor.authMode === "oauth") {
    ...
    const nativeAt = await ensureNativeAccessToken(descriptor.baseUrl).catch(
      () => null,
    );
...
ipcMain.handle(
  ...
  async (_event, rawUrl) => {
    ...
    try {
      statusBody = await fetchPublicJson(`${baseUrl}/api/status`, {
        timeoutMs: 8_000,
      });
    } catch {
    ...
    const authRequired =
      statusBody && authModeFromStatus(statusBody) === "oauth";
    ...
    if (strategy === "native") {
      try {
        ...
        rememberLog(
          `[native-oauth] native login failed (${error instanceof Error ? error.message : String(error)})`,
        );
...
ipcMain.handle("hermes:connection-config:apply", async (_event, payload) => {
  ...
  const nextRegistry = key
    ? previousRegistry
    : reconcileAppliedGlobalConnection(previousRegistry, config);
    ...
    if (profileHasRemoteOverride(profile)) {
      if (method === "GET") {
        return fetchJsonForProfile(
          profile,
          passthroughQuery ? `${pathname}?${passthroughQuery}` : pathname,
        );
      }
...
const remoteOwnerBySessionId = new Map<
  string,
  { at: number; profile: null | string }
>();
...
async function remoteOwnerProfileForSession(sessionId: string) {
  ...
  const owner = await findRemoteOwnerProfileForSession(
    sessionId,
    remoteProfiles,
    (profile, params) => remoteSessionList(profile, params),
  ).catch(() => null);
...
// carries local rows.
async function pooledRegistrySessionSources(): Promise<
  RegistrySessionSource[]
> {
  const registry = readDesktopConnectionsRegistry();
  ...
  for (const connection of registry.connections) {
    ...
    const backends: Array<{
      descriptor: unknown;
      profileLabel: null | string;
    }> = [];

    for (const [key, entry] of connection.kind === "ssh"
      ? pooled
      : pooled.slice(0, 1)) {
      try {
        ...
        const descriptor = await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(() => reject(new Error("pending")), 2_000),
          ),
        ]);
        ...
        backends.push({
          ...
          profileLabel:
            connection.kind === "ssh"
              ? key.slice(prefix.length) || "default"
              : null,
        });
      } catch {
...
async function dispatchRegistryApiRequest(
  ...
  const connection: any = await backendDialClaims.run(
...
async function teardownConnectionScopedProfileBackend(connectionId, profile) {
  ...
  await Promise.all([
    ...
    sshBootstrapCoordinator
      .cancelAndWait(key)
      .then(() => teardownSshConnection(key)),
  ]);
}
...
async function handleHermesApiRequest(request) {
  ...
  const apiRoute = resolveProfileApiRequest(
    profile,
    request.path,
    profileRouteOptions(profile, request),
  );
  ...
  try {
    ...
    if (connection.authMode === "oauth") {
      ...
      const nativeAt = await ensureNativeAccessToken(connection.baseUrl).catch(
        () => null,
      );
    ...
    if (profileRename) {
      try {
        ...
        rememberLog(
          `Failed to restore primary profile after rename error: ${String(rollbackError)}`,
        );
      }
    }
...
ipcMain.handle("hermes:api", async (_event, request) => {
  ...
  const mutatingProfile =
    deletingProfile || profileRenameFromRequest(request)?.oldName || null;
  const registryConnectionId = apiRequestRegistryConnectionId(request);
  ...
  if (deletingProfile && registryConnectionId) {
    return dispatchConnectionScopedProfileDelete(request, {
      ...
      prepareLocal: (localRequest) =>
        prepareProfileDeleteRequest(localRequest).then(() => undefined),
...
ipcMain.handle(
  ...
  (_event, key) => !claimedAmbientCue(String(key ?? "")),
);
...
ipcMain.handle("hermes:notify", (_event, payload) => {
  ...
  if (
    isDuplicateNotification(
      `${payload?.kind ?? ""}:${payload?.sessionId ?? payload?.tag ?? ""}`,
    )
  ) {
    return true;
  }
...
const DATA_URL_READ_MAX_CONFIG_PATH = path.join(
  app.getPath("userData"),
...
function readPersistedDataUrlReadMaxMb() {
  try {
    return clampDataUrlReadMaxMb(
      JSON.parse(fs.readFileSync(DATA_URL_READ_MAX_CONFIG_PATH, "utf8")).maxMb,
    );
  } catch {
...
function persistDataUrlReadMaxMb(maxMb) {
  ...
  try {
    fs.mkdirSync(path.dirname(DATA_URL_READ_MAX_CONFIG_PATH), {
      recursive: true,
    });
    fs.writeFileSync(
      DATA_URL_READ_MAX_CONFIG_PATH,
      JSON.stringify({ maxMb: next }, null, 2),
      "utf8",
    );
  } catch (error) {
...
ipcMain.handle("hermes:readFileDataUrl", async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
    ...
    mimeType: mimeTypeForPath(
      resolveRequestedPathForIpc(filePath, { purpose: "File preview" }),
    ),
...
ipcMain.handle("hermes:readFileDataUrlForAttach", async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
    ...
    mimeType: mimeTypeForPath(
      resolveRequestedPathForIpc(filePath, { purpose: "Attachment upload" }),
    ),
...
ipcMain.handle("hermes:watchPreviewFile", (_event, url) =>
  watchPreviewFile(String(url || "")),
);

ipcMain.handle("hermes:watchDirectory", (_event, dir) =>
  watchDirectory(String(dir || "")),
);

ipcMain.handle("hermes:stopPreviewFileWatch", (_event, id) =>
...
ipcMain.on("hermes:translucency:support", (event) => {
  event.returnValue = {
    glass: GLASS_SUPPORTED,
    translucency: TRANSLUCENCY_SUPPORTED,
  };
});
...
ipcMain.on("hermes:translucency", (_event, payload) => {
  ...
  const changed = {
    ...
    material:
      vibrancyForTranslucency(previous) !== vibrancyForTranslucency(next),
...
const KEEP_AWAKE_CONFIG_PATH = path.join(
  app.getPath("userData"),
  "keep-awake.json",
);
...
function readPersistedKeepAwake() {
  try {
    return (
      JSON.parse(fs.readFileSync(KEEP_AWAKE_CONFIG_PATH, "utf8")).on === true
    );
  } catch {
    return false;
  }
...
ipcMain.on("hermes:keep-awake", (_event, on) => {
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ on: enabled }, null, 2),
      "utf8",
    );
  } catch (error) {
...
ipcMain.handle("hermes:quick-entry:settings:set", async (_event, patch) => {
  ...
  const next = sanitizeQuickEntrySettings({
    ...
    shortcut:
      typeof patch?.shortcut === "string" && patch.shortcut.trim()
        ? patch.shortcut
        : current.shortcut,
  });
...
ipcMain.on("hermes:quick-entry:submit", (_event, payload) => {
  ...
  if (!mainWindow || mainWindow.isDestroyed()) {
    rememberLog(
      "[quick-entry] dropped a submit: no primary window to route it to",
    );
...
const DISABLE_F12_CONFIG_PATH = path.join(
  app.getPath("userData"),
  "disable-f12.json",
);
...
function readPersistedDisableF12() {
  try {
    return (
      JSON.parse(fs.readFileSync(DISABLE_F12_CONFIG_PATH, "utf8")).on === true
    );
  } catch {
    return false;
  }
...
ipcMain.on("hermes:devtools:disable-f12", (_event, on) => {
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ on: f12Blocked }, null, 2),
      "utf8",
    );
  } catch (error) {
...
ipcMain.handle("hermes:workspace:sanitize", async (_event, cwd) =>
...
ipcMain.handle("hermes:logs:recent", async () => ({
...
ipcMain.on("hermes:logs:renderer-error", (_event, report) => {
  const { label, boundary, message, componentStack } =
    report && typeof report === "object" ? report : {};
  rememberLog(
    formatRendererBoundaryReport(label, boundary, message, componentStack),
  );
...
ipcMain.handle("hermes:version", async () => {
  ...
  return {
    ...
    bundleSwapPending:
      IS_PACKAGED && detectBundleSwap(INSTALL_STAMP, loadInstallStamp()),
  };
});
...
ipcMain.handle("hermes:app:relaunch", async () => {
  rememberLog(
    "[updates] renderer requested an app relaunch (swapped bundle pending)",
  );
  app.relaunch({ args: buildNoSandboxRelaunchArgs(process.argv.slice(1)) });
  void exitAfterBackendShutdown(0);
});
    ...
    try {
      ...
      child.on("exit", (code) => {
        ...
        try {
          ...
          parsed.running_app_path = resolveRemovableAppPath(
            process.execPath,
            process.platform,
            process.env,
          );
...
async function runDesktopUninstall(mode) {
  ...
  const appPath = resolveRemovableAppPath(
    process.execPath,
    process.platform,
    process.env,
  );
  const removeBundle = shouldRemoveAppBundle(IS_PACKAGED, appPath)
    ? appPath
    : null;
  ...
  try {
    ...
    rememberLog(
      `[uninstall] backend teardown errored (continuing): ${error.message}`,
    );
  }
  ...
  try {
    if (IS_WINDOWS) {
      ...
      scriptPath = path.join(
        app.getPath("temp"),
        `hermes-uninstall-${Date.now()}.sh`,
      );
      fs.writeFileSync(scriptPath, buildPosixCleanupScript(scriptArgs), {
        mode: 0o755,
      });
      runner = "/bin/bash";
  ...
  return {
    ok: true,
    mode,
    willRemoveAppBundle: Boolean(removeBundle),
    scriptPath,
  };
}
...
ipcMain.handle("hermes:vscode-theme:fetch", async (_event, id) =>
  fetchMarketplaceThemes(String(id || "")),
);
...
function _extractDeepLink(argv) {
  ...
  return (
    argv.find(
      (a) =>
        typeof a === "string" &&
        DEEPLINK_SCHEMES.some((s) => a.startsWith(`${s}://`)),
    ) || null
  );
}
...
function handleDeepLink(url) {
  ...
  if (!DEEPLINK_SCHEMES.includes(scheme)) {
    rememberLog(
      `[deeplink] ignoring scheme ${scheme} (expected ${DEEPLINK_SCHEMES.join(" or ")})`,
    );
...
app.whenReady().then(() => {
  ...
  if (systemCa.applied) {
    ...
    rememberLog(
      `[tls] could not load Windows system CA certificates: ${systemCa.error}`,
    );
  }
...
app.on("before-quit", (event) => {
  ...
  if (IS_WINDOWS && !windowsSandboxFallbackSticky) {
    try {
      writeSandboxMarker(
        app.getPath("userData"),
        markerAfterSuccessfulBoot({ fallbackActive: false }),
      );
    } catch {
      void 0;
    }
