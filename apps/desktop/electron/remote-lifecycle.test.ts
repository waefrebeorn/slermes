...
test("SSH reuse proof remains compatible when runtime state is absent", () => {
  assert.equal(
    classifySshReuseProof(
      { ok: true, sshOwnerNonce: SPAWN_NONCE, protocolVersion: 1 },
      SPAWN_NONCE,
    ),
    "authenticated-ok",
  );
  ...
  return {
    ...
    async exec(cmd) {
      ...
      for (const [matcher, resp] of applicableRules) {
        const hit =
          typeof matcher === "function" ? matcher(cmd) : matcher.test(cmd);
...
test("POSIX relaunch gate refuses live and uncertain install markers without executing Hermes", async () => {
  for (const observation of ["LIVE:4242", "UNCERTAIN"]) {
    ...
    assert.equal(
      calls.some((command) =>
        /\[ -x |--version|lock\.json|serve --help|setsid/.test(command),
      ),
      false,
    );
  }
...
test("POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide", async () => {
  ...
  await assertRemoteInstallUpdateClear(
    ssh,
    "/home/alice/.hermes/profiles/research",
  );
...
test("listRemoteHermesProfiles inventories Mini-style profile dirs without spawning a dashboard", async () => {
  ...
  assert.deepEqual(await listRemoteHermesProfiles(ssh), [
    "default",
    "bob",
    "dixie",
    "goose",
...
test("locateHermes returns an explicit remoteHermesPath unchanged", async () => {
  ...
  assert.equal(
    await locateHermes(ssh, "~/.local/bin/hermes"),
    "~/.local/bin/hermes",
  );
  assert.ok(
    !ssh.calls.some((cmd) => cmd.includes("python3 -c")),
...
test("probeRemotePlatform accepts Linux and macOS", async () => {
  assert.deepEqual(
    await probeRemotePlatform(fakeSsh([[/uname/, "Linux\nx86_64"]])),
    {
      os: "Linux",
      arch: "x86_64",
    },
  ...
  assert.deepEqual(
    await probeRemotePlatform(fakeSsh([[/uname/, "Darwin\narm64"]])),
    {
      os: "Darwin",
      arch: "arm64",
    },
...
test("ownership paths are isolated by ownership ID and spawn nonce", () => {
  assert.equal(
    ownershipDirectory(OWNERSHIP_ID),
    `~/.hermes/desktop-ssh/${OWNERSHIP_ID}`,
  );
  assert.equal(
    lockfilePath(OWNERSHIP_ID),
    `~/.hermes/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`,
  );
  assert.equal(
    spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    `~/.hermes/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`,
  );
});
...
test("readLockfile returns null ONLY for a missing/empty lockfile", async () => {
  ...
  assert.deepEqual(
    await readLockfile(fakeSsh([[/cat/, JSON.stringify(good)]]), OWNERSHIP_ID),
    good,
  );
});
...
test("readLockfile classifies existing-but-foreign lockfiles as skew, never null", async () => {
  ...
  const foreign = await readLockfile(
    fakeSsh([[/cat/, "not json at all"]]),
    OWNERSHIP_ID,
  );
  ...
  const future = await readLockfile(
    fakeSsh([
      [
        /cat/,
        JSON.stringify(
          ownedLock({ schemaVersion: LOCKFILE_SCHEMA_VERSION + 1 }),
        ),
      ],
    ]),
    OWNERSHIP_ID,
  ...
  const unknown = await readLockfile(
    fakeSsh([[/cat/, JSON.stringify({ schemaVersion: 999 })]]),
    OWNERSHIP_ID,
  );
  ...
  assert.equal(
    isLockfileSkew(await readLockfile(fakeSsh([[/cat/, ""]]), OWNERSHIP_ID)),
    false,
  );
  assert.equal(
    isLockfileSkew(
      await readLockfile(
        fakeSsh([[/cat/, JSON.stringify(ownedLock())]]),
        OWNERSHIP_ID,
      ),
    ),
    false,
...
test("connect() fails closed on lockfile schema/ownership skew: skips reap, touches nothing", async () => {
  const skewShapes: Array<[string, string]> = [
    [
      "foreign-schema lockfile",
      '{"pid":333,"owner":"some-fork-desktop","version":"9.9.9"}',
    ],
  ...
  for (const [label, raw] of skewShapes) {
    ...
    assert.ok(
      !ssh.calls.some(
        (c) => /(^|[^-\d])kill -?9? ?\d/.test(c) && !/kill -0/.test(c),
      ),
      `${label}: must not kill any pid`,
    );
    assert.ok(
      !ssh.calls.some((c) => /rm -f/.test(c)),
      `${label}: must not remove any remote file`,
    );
    assert.ok(
      !ssh.calls.some((c) => /setsid|nohup/.test(c)),
      `${label}: must not spawn on top of foreign state`,
    );
    assert.ok(
      !ssh.calls.some((c) => /printf '%s' '.*schemaVersion/.test(c)),
      `${label}: must not overwrite the lockfile`,
    );
  }
});
...
test("disconnect() fails closed on lockfile skew: never reaps, never drops the foreign lockfile", async () => {
  ...
  assert.ok(
    ...
    "must not kill any pid",
  );
  assert.ok(
    !ssh.calls.some((c) => /rm -f/.test(c)),
    "must not remove the foreign lockfile or logs",
  );
});
...
test("cleanupStale is inert when handed a skew sentinel (defense in depth)", async () => {
  ...
  assert.equal(
    ssh.calls.length,
    0,
    "skew must short-circuit before any remote command",
  );
});
...
test("remotePidAlive maps kill -0 ALIVE/DEAD", async () => {
  assert.equal(
    await remotePidAlive(fakeSsh([[/kill -0/, "ALIVE"]]), 123),
    true,
  );
  assert.equal(
    await remotePidAlive(fakeSsh([[/kill -0/, "DEAD"]]), 123),
    false,
  );
    ...
    await pidIsOurDashboard(
      fakeSsh([
        [
          /print\("OWNED"/,
          (command) =>
            command.includes("fedcba9876543210") ? "FOREIGN\n" : "OWNED\n",
        ],
      ]),
      5,
      "fedcba9876543210",
...
test.skipIf(process.platform === "win32")(
  ...
  async () => {
    const temp = await mkdtemp(
      path.join(os.tmpdir(), "hermes wrapper ownership "),
    );
    ...
    const tokenPath = path.join(
      os.homedir(),
      spawnTokenPath(OWNERSHIP_ID, SPAWN_NONCE).replace(/^~\//, ""),
    );
    ...
    const ssh = {
      exec: async (command: string) =>
        (await exec(command, { shell: "/bin/bash" })).stdout,
    };

    const waitForEntrypoint = async (process: ReturnType<typeof spawn>) => {
      for (let attempt = 0; attempt < 40; attempt += 1) {
        const command = (await exec(`ps -ww -o command= -p ${process.pid}`))
          .stdout;
    ...
    try {
      assert.equal(
        await waitForEntrypoint(child),
        true,
        "wrapper must exec into the fake installer entrypoint",
      );
      assert.equal(
        await pidIsOurDashboard(
          ssh,
      ...
      const misplacedIsolated = spawnInstaller([
        "--profile",
        "ops",
        "--isolated",
        "serve",
...
test("buildSpawnCommand is headless serve, detached, token not in argv", () => {
  ...
  assert.ok(
    !cmd.includes("tok_secret_value"),
    "token must not appear in spawn command",
  );
  assert.ok(
    !cmd.includes("HERMES_DASHBOARD_SESSION_TOKEN"),
    "token env var must not appear",
  );
});
...
test.skipIf(process.platform === "win32")(
  ...
  async () => {
    const directory = await mkdtemp(
      path.join(os.tmpdir(), "hermes-update-mutex-"),
    );
      ...
      for (let attempt = 0; attempt < 40; attempt += 1) {
        try {
          ...
          assert.equal(
            report,
            "",
            "the backend process must not retain the update mutex descriptor",
          );
...
test("connect() spawns fresh when there is no lockfile, adopts the served token", async () => {
  ...
  const result = await connect(
    connectDeps(ssh, { adoptServedToken: async () => "the-served-token" }),
  );
...
test("managed SSH maps a local scope to a different non-default remote profile", async () => {
  ...
  assert.match(
    spawn,
    /\.hermes\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/,
  );
  assert.ok(
    !spawn.includes(" work"),
    "the local Desktop scope must not become the remote profile",
  );
});
...
test("connect() reuses a healthy dashboard when fingerprint + probe pass", async () => {
  ...
  assert.ok(
    ...
    "reuse path must not spawn a new dashboard",
  );
});
...
test("connect() respawns when the lockfile hermesPath differs from the resolved path", async () => {
  ...
  assert.equal(
    result.reused,
    false,
    "must respawn, not reuse the old-path dashboard",
  );
  assert.ok(
    ssh.calls.some((c) => /setsid/.test(c)),
...
test("connect() respawns when the lockfile protocolVersion is incompatible", async () => {
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, adoptServedToken: async () => "fresh" }),
  );
  assert.equal(
    result.reused,
    false,
    "incompatible protocol must force a fresh spawn, not a reattach",
  );
...
test("connect() respawns when the lockfile pid is dead (killed dashboard)", async () => {
  ...
  assert.ok(
    !ssh.calls.some(
      (command) =>
        command.includes("pid=333") && command.includes('print("OWNED"'),
    ),
...
test("managed update drain preserves a live foreign POSIX owner and its lock bytes", async () => {
  ...
  await assert.rejects(
    terminateOwnedDashboardForUpdate(ssh, lock),
...
test("managed update drain rechecks the POSIX ownership record before signalling", async () => {
  ...
  await assert.rejects(
    terminateOwnedDashboardForUpdate(ssh, lock),
    /changed during process verification/,
  );
  assert.equal(
    ssh.calls.some((command) => /kill 333 &&/.test(command)),
    false,
...
test("managed update drain refuses Darwin termination because PID signals cannot be atomically bound", async () => {
  ...
  await assert.rejects(
    terminateOwnedDashboardForUpdate(ssh, lock),
    /identity changed at the signal boundary/,
  );
  assert.equal(
  ...
  const termination = ssh.calls.find((command) =>
    command.includes("identity_before_signal"),
  );
...
test("validateRemotePath accepts absolute POSIX paths", () => {
  ...
  assert.doesNotThrow(() =>
    validateRemotePath("/home/user/.hermes/hermes-agent/venv/bin/hermes"),
  );
});
...
test("validateRemotePath accepts ~/ prefix paths", () => {
  ...
  assert.doesNotThrow(() =>
    validateRemotePath("~/.hermes/logs/desktop-ssh.log"),
  );
...
test("validateRemotePath accepts paths with spaces and quotes", () => {
  ...
  assert.doesNotThrow(() =>
    validateRemotePath('/path with "double quotes"/file'),
  );
});
...
test("expandRemotePath preserves spaces as data", () => {
  ...
  assert.ok(
    result.includes("my project"),
    "spaces must be preserved, not split",
  );
});
...
test("buildSpawnCommand does not embed the token in the command string", () => {
  ...
  assert.ok(
    !cmd.includes("super_secret_token_value"),
    "token must not appear in the spawn command",
  );
  assert.ok(
    !cmd.includes("HERMES_DASHBOARD_SESSION_TOKEN"),
    "env var name must not appear",
  );
});
...
test("buildSpawnCommand raises the SSH child file limit before execing Hermes", () => {
  ...
  assert.match(
    cmd,
    /ulimit -n 65536 2>\/dev\/null \|\| true; exec env HERMES_DESKTOP=1/,
  );
...
test("buildSpawnCommand payload variables keep $HOME expandable (no double quoting)", () => {
  ...
  for (const name of ["reservation", "lock", "owner_file"]) {
    assert.match(
      cmd,
      new RegExp(`${name}="\\$HOME"`),
      `${name}= must start with an expandable "$HOME"`,
    );
    assert.doesNotMatch(
      cmd,
      new RegExp(`${name}='`),
      `${name}= must not be re-quoted`,
    );
  }
});
...
test("buildSpawnCommand lockfile publication is POSIX sh (no bash substitution)", () => {
  ...
  assert.doesNotMatch(
    cmd,
    /\$\{lock_json\/\//,
    "must not use ${var//} substitution under sh",
  );
  assert.ok(
    cmd.includes('sed "s/__PID__/${child}/"'),
    "pid substitution must use sed",
  );
});
...
test("spawnRemoteDashboard removes a token file when upload reporting fails", async () => {
  ...
  const ssh = fakeSsh([
    [/grep -q ssh-session-token-file/, "YES\n"],
    [
      (command) => /python3 -c/.test(command) && !/rm -f/.test(command),
      failure,
    ],
    [/rm -f/, ""],
  ]);
...
test("spawnRemoteDashboard streams the token over stdin, not argv/env", async () => {
  ...
  for (const cmd of calls) {
    assert.ok(
      !cmd.includes("secret_token_val"),
      `token leaked into command: ${cmd}`,
    );
  }
});
...
test("spawnRemoteDashboard upload uses exclusive-create and O_NOFOLLOW", async () => {
  ...
  const uploadCmd = calls.find(
    (c) => /python3 -c/.test(c) && !/fcntl\.flock/.test(c),
  );
  assert.ok(uploadCmd, "must use python3 -c for token upload");
  assert.match(
    uploadCmd,
    /O_EXCL/,
    "upload must use O_EXCL to reject existing files",
  );
  assert.match(
    uploadCmd,
    /O_NOFOLLOW/,
    "upload must use O_NOFOLLOW to reject symlinks",
  );
  assert.match(uploadCmd, /O_WRONLY/, "upload must open write-only");
  assert.match(
    uploadCmd,
    /dir_fd=dd/,
    "upload must create relative to the opened parent directory",
  );
  assert.match(
    uploadCmd,
    /os\.fstat\(dd\)/,
    "upload must validate the opened parent directory",
  );
  assert.ok(
    !uploadCmd.includes("tk"),
    "token must not appear in the upload command",
  );
});
...
test("readLockfile treats a lock with non-integer pid as skew", async () => {
  ...
  assert.equal(
    isLockfileSkew(
      await readLockfile(
        fakeSsh([[/cat/, JSON.stringify(lock)]]),
        OWNERSHIP_ID,
      ),
    ),
    true,
...
test("readLockfile treats a lock with port out of range as skew", async () => {
  ...
  assert.equal(
    isLockfileSkew(
      await readLockfile(
        fakeSsh([[/cat/, JSON.stringify(lock2)]]),
        OWNERSHIP_ID,
      ),
    ),
    true,
...
test("connect() reuse path does not write a token file", async () => {
  ...
  assert.equal(result.reused, true);
  assert.ok(
    !ssh.calls.some((c) => /sys\.stdin\.buffer\.read/.test(c)),
...
test("cleanupStale never deletes a lock-supplied unexpected log path", async () => {
  ...
  await cleanupStale(
    ssh,
    OWNERSHIP_ID,
    ownedLock({ logPath: "~/.hermes/unrelated.log" }),
  );
...
test("remote SSH ownership capability requires both secure bootstrap flags", async () => {
  ...
  assert.equal(
    await remoteSupportsSshOwnership(unsupported, "/x/hermes"),
    false,
  );
});
...
test("cleanupStale escalates to SIGKILL when the backend survives the graceful wait (#91668 quit-during-active-turn)", async () => {
  ...
  const ssh = fakeSsh([
    [/print\("OWNED"/, "OWNED\n"],
    [
      (cmd: string) => /kill 9 &&/.test(cmd),
      new Error("exit 1: pid alive after graceful wait"),
    ],
  ]);
...
test("cleanupStale keeps the lockfile when even SIGKILL cannot confirm the pid died", async () => {
  const ssh = fakeSsh([
    ...
    [
      (cmd: string) => /kill -9 9\b/.test(cmd),
      new Error("exit 1: unkillable (D-state)"),
    ],
  ]);
