...
const WAYLAND_SESSION = {
  WAYLAND_DISPLAY: "wayland-0",
  XDG_SESSION_TYPE: "wayland",
};
const XWAYLAND_SESSION = {
  DISPLAY: ":0",
  WAYLAND_DISPLAY: "wayland-0",
  XDG_SESSION_TYPE: "wayland",
};
...
test("macOS and Windows share the click-through + renderer-drag profile", () => {
  for (const platform of ["darwin", "win32"] as const) {
    const windowing = resolveHudWindowing(platform, WAYLAND_SESSION, [
...
test("an auto hint follows the session", () => {
  assert.equal(
    input("linux", WAYLAND_SESSION, ["--ozone-platform-hint=auto"]),
    "click-through",
  );
  assert.equal(
    input("linux", X11_SESSION, ["--ozone-platform-hint=auto"]),
    "solid",
  );
  assert.equal(
    input("linux", XWAYLAND_SESSION, ["--ozone-platform-hint=auto"]),
    "click-through",
  );
});
...
test("a backend nobody recognises follows the session, not a silent X11 default", () => {
  assert.equal(
    input("linux", X11_SESSION, ["--ozone-platform=headless"]),
    "solid",
  );
  assert.equal(
    input("linux", WAYLAND_SESSION, ["--ozone-platform=headless"]),
    "click-through",
  );
...
test("linuxOzoneBackend is the session/argv normalizer", () => {
  ...
  assert.equal(
    linuxOzoneBackend(WAYLAND_SESSION, ["--ozone-platform=x11"]),
    "x11",
  );
  assert.equal(
    linuxOzoneBackend(X11_SESSION, ["--ozone-platform-hint=auto"]),
    "x11",
  );
});

test("the renderer view is a boolean slice of the profile", () => {
  const wayland = hudWindowingView(
    resolveHudWindowing("linux", WAYLAND_SESSION, []),
  );
