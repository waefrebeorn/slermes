...
type RemoteMarkerState =
  "absent" | "dead" | "live" | "malformed" | "unavailable";
...
function managedSshTokenPersistencePlan(
  ...
  const sourceRegistryId = value.startsWith("registry:")
    ? value.slice("registry:".length)
    : "";

  return {
    legacySource: sourceRegistryId
      ? null
      : value === "profile"
        ? "profile"
        : "global",
    registryConnectionId: String(
      registryConnectionId || sourceRegistryId || "",
    ).trim(),
  };
}
...
function managedSshScopeRole(input: {
  ...
  if (
    input.key.startsWith(input.prefix) ||
    input.routeConnectionId === input.connectionId
  ) {
    return "pool";
  }
...
function buildPosixManagedUpdateLaunch(
  ...
  const intentPath = posixChildPath(
    home,
    `.update_launch_intent.${correlation}`,
  );
  const outputPath = posixChildPath(
    home,
    `logs/desktop-update-${correlation}.log`,
  );
...
function buildWindowsManagedUpdateLaunch(
  ...
  const readyPath = windowsChildPath(
    home,
    `.update_coordinator_ready.${correlation}`,
  );
  const intentPath = windowsChildPath(
    home,
    `.update_launch_intent.${correlation}`,
  );
  const outputPath = windowsChildPath(
    home,
    `logs\\desktop-update-${correlation}.log`,
  );
...
function buildRemoteUpdateObservationCommand(
  target: RemoteUpdateTarget,
  correlationId: string,
): string {
  const correlation = validateCorrelationId(correlationId);
...
function parseRemoteUpdateObservation(
  raw: string,
  correlationId: string,
): RemoteUpdateObservation {
  const correlation = validateCorrelationId(correlationId);
  let parsed: any;
  ...
  if (
    !parsed ||
    !["absent", "dead", "live", "malformed", "unavailable"].includes(
      parsed.marker,
    )
  ) {
  ...
  if (
    !["absent", "dead", "present", "malformed", "unavailable"].includes(
      parsed.launchIntent,
    )
  ) {
  ...
  if (parsed.coordinatorReady != null) {
    ...
    if (
      ...
      throw new Error(
        "Remote update coordinator readiness proof was malformed.",
      );
    }
  ...
  const markerPid =
    parsed.markerPid == null ? undefined : Number(parsed.markerPid);

  if (
    markerPid !== undefined &&
    (!Number.isSafeInteger(markerPid) ||
      markerPid <= 0 ||
      markerPid > 4_294_967_295)
  ) {
...
async function assertManagedUpdatePreflightClear(
  target: RemoteUpdateTarget,
  correlationId: string,
): Promise<void> {
  ...
  if (!markerIsClear(observation)) {
    const owner = observation.markerPid
      ? ` (PID ${observation.markerPid})`
      : "";
    throw new Error(
      `The remote install update marker is ${observation.marker}${owner}; refusing to drain its serves.`,
    );
  }
}
...
async function launchManagedRemoteUpdate(
  ...
  if (target.platform === "Windows") {
    ...
    if (
      ...
      throw new Error(
        "Remote Windows update launcher did not acknowledge its detached child.",
      );
    }
  } else if (!String(output || "").includes("MANAGED_UPDATE_STARTED")) {
    throw new Error(
      "Remote update launcher did not acknowledge its detached child.",
    );
  }
}
...
async function waitForManagedRemoteUpdate(
  ...
  while (true) {
    ...
    if (
      observation.marker === "malformed" ||
      observation.marker === "unavailable"
    ) {
      if (now() >= deadline) {
        throw new Error(
    ...
    if (observation.exitCode !== null) {
      ...
      if (
        ...
        now() - terminalSeenAt >= RECEIPT_GRACE_MS
      ) {
        throw new Error(
          "Remote update finished without a correlated durable receipt.",
        );
      }
    }
...
async function waitForManagedRemoteClearance(
  ...
  while (true) {
    try {
      const observation = await observeManagedRemoteUpdate(
        target,
        correlationId,
      );
      ...
      if (
        observation.launchIntent === "malformed" ||
        observation.launchIntent === "unavailable"
      ) {
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  try {
    ...
    if (drainErrors.length) {
      throw new Error(
        `Could not safely drain every managed SSH scope (${drainErrors.join("; ")}).`,
      );
    }
    ...
    if (restoreNeedsMarkerFence) {
      try {
        ...
        updateError = [updateError, restorationBlocked]
          .filter(Boolean)
          .join(" ");
      }
    }
    ...
    try {
      if (
        recoveryPrepared &&
        !restorationBlocked &&
        restoreResults.every((result) => result.restored)
      ) {
  ...
  const updateOk = Boolean(
    proof && proof.exitCode === 0 && proof.receipt.outcome === "success",
  );
  const restoreOk =
    recoveryComplete && restoreResults.every((result) => result.restored);
...
function refusedManagedSshUpdate(
  connectionId: string,
  correlationId: string,
  error: string,
) {
  const message = String(
    error || "This connection is not managed by Desktop SSH.",
  );
...
async function recoverManagedSshScopes<TScope>(deps: {
  ...
  const results = await Promise.allSettled(
    deps.scopes.map((scope) => deps.restoreScope(scope)),
  );
...
async function waitForManagedSshBootstrapFence(
  entries: Iterable<{
    metadata?: { registryConnectionId?: string };
    promise: Promise<unknown>;
  }>,
  connectionId: string,
): Promise<void> {
  const pending = [...entries].filter(
    (entry) => entry.metadata?.registryConnectionId === connectionId,
  );
  const results = await Promise.allSettled(
  ...
  const unsafe = results.find(
    (result) =>
      ...
      (result.reason as any)?.unsafeManagedBootstrap === true,
  );
  ...
  assertCanDial(connectionId: string, correlationId = ""): void {
    ...
    if (owner && owner !== correlationId) {
      const error: any = new Error(
        `SSH connection "${id}" is paused while its managed update is in progress.`,
      );
