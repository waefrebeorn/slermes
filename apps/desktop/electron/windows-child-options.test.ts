...
test("hiddenWindowsChildOptions preserves an existing windowsHide:false on Windows", () => {
  assert.deepEqual(hiddenWindowsChildOptions({ windowsHide: false }, true), {
    windowsHide: false,
  });
});

test("hiddenWindowsChildOptions preserves an existing windowsHide:true on Windows", () => {
  assert.deepEqual(hiddenWindowsChildOptions({ windowsHide: true }, true), {
    windowsHide: true,
  });
});
...
test("hiddenWindowsChildOptions leaves options unchanged off Windows", () => {
  ...
  assert.deepEqual(hiddenWindowsChildOptions({ stdio: "ignore" }, false), {
    stdio: "ignore",
  });
});
...
function makeChild(
  overrides: Partial<{ pid: number | null; killed: boolean }> = {},
) {
  const calls: string[] = [];
...
test("stopBackendChild tree-kills on Windows when the child has a pid", () => {
  ...
  assert.deepEqual(
    calls,
    [],
    "SIGTERM must not be sent when the Windows tree-kill path is taken",
  );
});
...
test("stopBackendChild group-SIGTERMs on POSIX (negative pgid) when the child has a pid", () => {
  ...
  assert.deepEqual(
    groupKills,
    [[-4242, "SIGTERM"]],
    "must signal the whole process group",
  );
  assert.deepEqual(
    calls,
    [],
    "direct child.kill must not run when the group send succeeds",
  );
...
test("stopBackendChild falls back to direct SIGTERM on POSIX when the group send throws", () => {
  ...
  assert.deepEqual(
    calls,
    ["SIGTERM"],
    "must fall back to signalling the direct child",
  );
});
...
test("Windows update tree-kills captured roots without pre-signalling the primary backend", () => {
  ...
  assert.deepEqual(
    primary.calls,
    [],
    "the primary root must not be signalled before taskkill /T sees it",
  );
