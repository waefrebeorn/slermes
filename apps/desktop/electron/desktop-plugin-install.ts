  ...
  if (/^(https?:\/\/|git@|ssh:\/\/|file:\/\/)/.test(trimmed)) {
    if (trimmed.startsWith("https://github.com/")) {
      ...
      if (
        parts.length >= 3 &&
        parts[2] &&
        GITHUB_BROWSER_SEGMENTS.has(parts[2])
      ) {
    ...
    if (trimmed.includes("#")) {
      ...
      const subdir =
        trimmed.slice(hashIdx + 1).replace(/^\/+|\/+$/g, "") || null;
...
export function desktopPluginFolderName(
  gitUrl: string,
  subdir: string | null,
): string {
  if (subdir) {
...
export function findDesktopEntry(
  pluginRoot: string,
): { entryFile: string; sourceSubdir: string } | null {
...
async function cloneToTemp(gitBin: string, gitUrl: string): Promise<string> {
  ...
  try {
    const { code, stderr } = await runGit(gitBin, [
      "clone",
      "--depth",
      "1",
      gitUrl,
...
function insecureSchemeWarnings(gitUrl: string): {
  warnings: string[];
  insecure: boolean;
} {
...
async function copyDesktopTree(
  sourceDir: string,
  targetDir: string,
): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      if (!detected.desktop || !detected.desktopSourceSubdir) {
        return {
          ok: false,
          error: "No desktop plugin.js found in this repository.",
        };
      }
      ...
      if (
        (await pathIsDirectory(targetDir)) ||
        (await pathIsFile(targetPlugin))
      ) {
        if (!force) {
          return {
            ok: false,
      ...
      if (!(await pathIsFile(targetPlugin))) {
        return {
          ok: false,
          error: `Install completed but ${targetPlugin} is missing.`,
        };
      }
