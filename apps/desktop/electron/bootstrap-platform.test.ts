...
test("isWslEnvironment detects WSL2 env vars on linux", () => {
  ...
  assert.equal(
    isWslEnvironment({ WSL_INTEROP: "/run/WSL/123_interop" }, "linux"),
    true,
  );
  assert.equal(
    isWslEnvironment({}, "linux", "6.6.87.2-microsoft-standard-WSL2"),
    true,
  );
  assert.equal(isWslEnvironment({}, "linux", "6.6.87-generic"), false);
  assert.equal(
    isWslEnvironment({ WSL_DISTRO_NAME: "Ubuntu" }, "darwin"),
    false,
  );
});
...
test("isWindowsBinaryPathInWsl blocks Windows binary types on WSL", () => {
  assert.equal(
    isWindowsBinaryPathInWsl("/mnt/c/Tools/hermes.exe", { isWsl: true }),
    true,
  );
  assert.equal(
    isWindowsBinaryPathInWsl("/mnt/c/Tools/hermes.cmd", { isWsl: true }),
    true,
  );
  assert.equal(
    isWindowsBinaryPathInWsl("/mnt/c/Tools/hermes.bat", { isWsl: true }),
    true,
  );
  assert.equal(
    isWindowsBinaryPathInWsl("/mnt/c/Tools/install.ps1", { isWsl: true }),
    true,
  );
  assert.equal(
    isWindowsBinaryPathInWsl("/usr/local/bin/hermes", { isWsl: true }),
    false,
  );
  assert.equal(
    isWindowsBinaryPathInWsl("/mnt/c/Tools/hermes.exe", { isWsl: false }),
    false,
  );
});
...
test("bundledRuntimeImportCheck selects platform-specific import checks", () => {
  assert.equal(
    bundledRuntimeImportCheck("win32"),
    "import fastapi, uvicorn, winpty",
  );
  assert.equal(
    bundledRuntimeImportCheck("darwin"),
    "import fastapi, uvicorn, ptyprocess",
  );
  assert.equal(
    bundledRuntimeImportCheck("linux"),
    "import fastapi, uvicorn, ptyprocess",
  );
});
...
test("detectRemoteDisplay keeps GPU on for local sessions", () => {
  ...
  assert.equal(
    detectRemoteDisplay({ env: { DISPLAY: ":0" }, platform: "linux" }),
    null,
  );
  assert.equal(
    detectRemoteDisplay({
  ...
  assert.equal(
    detectRemoteDisplay({ env: { SESSIONNAME: "Console" }, platform: "win32" }),
    null,
  );
...
test("detectRemoteDisplay flags SSH sessions on any platform", () => {
  ...
  assert.equal(
    detectRemoteDisplay({
      env: { SSH_CLIENT: "1.2.3.4 5 22" },
      platform: "darwin",
    }),
    "ssh-session",
  );
  assert.equal(
    detectRemoteDisplay({ env: { SSH_TTY: "/dev/pts/0" }, platform: "win32" }),
    "ssh-session",
  );
});
...
test("detectRemoteDisplay flags forwarded X11 displays but not local ones", () => {
  ...
  assert.equal(
    detectRemoteDisplay({ env: { DISPLAY: ":1" }, platform: "linux" }),
    null,
  );
});
...
test("detectRemoteDisplay honors the HERMES_DESKTOP_DISABLE_GPU override both ways", () => {
  ...
  assert.equal(
    detectRemoteDisplay({
      env: {
        HERMES_DESKTOP_DISABLE_GPU: "false",
...
test("resolveLinuxPasswordStore applies known backends on linux", () => {
  for (const store of [
    ...
    assert.deepEqual(
      resolveLinuxPasswordStore({
        env: { HERMES_DESKTOP_PASSWORD_STORE: store },
        platform: "linux",
      }),
      {
        store,
...
test("resolveLinuxPasswordStore is a no-op when the env var is unset", () => {
  assert.deepEqual(resolveLinuxPasswordStore({ env: {}, platform: "linux" }), {
    store: null,
    warning: null,
  });
  assert.deepEqual(
    resolveLinuxPasswordStore({
      env: { HERMES_DESKTOP_PASSWORD_STORE: "  " },
      platform: "linux",
    }),
    {
      store: null,
...
test("resolveLinuxPasswordStore ignores the env var off linux", () => {
  assert.deepEqual(
    resolveLinuxPasswordStore({
      env: { HERMES_DESKTOP_PASSWORD_STORE: "gnome-libsecret" },
      platform: "darwin",
    }),
  ...
  assert.deepEqual(
    resolveLinuxPasswordStore({
      env: { HERMES_DESKTOP_PASSWORD_STORE: "kwallet6" },
      platform: "win32",
    }),
    { store: null, warning: null },
  );
