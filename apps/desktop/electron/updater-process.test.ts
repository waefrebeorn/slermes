...
test("resolveStagedUpdaterBinary returns null on Windows when nothing is staged", () => {
  const resolved = resolveStagedUpdaterBinary(
    "C:\\Users\\hermes\\AppData\\Local\\hermes",
    {
      fileExists: () => false,
      isWindows: true,
    },
...
test("resolveUpdateScriptHandoff returns null when the checkout predates the script", () => {
  const handoff = resolveUpdateScriptHandoff(
    String.raw`C:\Users\hermes\AppData\Local\hermes\hermes-agent`,
    {
      isWindows: true,
      fileExists: () => false,
    },
...
test("resolvePosixScriptHandoff is null when the checkout predates the script", () => {
  const handoff = resolvePosixScriptHandoff(
    "/home/hermes/.hermes/hermes-agent",
    {
      isWindows: false,
      fileExists: () => false,
...
test("sandboxFallbackFromEnv: ELECTRON_DISABLE_SANDBOX / --no-sandbox opt out", () => {
  assert.equal(
    sandboxFallbackFromEnv({ ELECTRON_DISABLE_SANDBOX: "1" }, []),
    true,
  );
  assert.equal(
    sandboxFallbackFromEnv({ ELECTRON_DISABLE_SANDBOX: "true" }, []),
    true,
  );
  assert.equal(sandboxFallbackFromEnv({}, ["--no-sandbox"]), true);
  assert.equal(
    sandboxFallbackFromEnv({ ELECTRON_DISABLE_SANDBOX: "0" }, []),
    false,
  );
...
test("observeUpdaterHandoff settles ok for children without an event interface", async () => {
  const timer = manualTimer();
  const outcomePromise = observeUpdaterHandoff(
    { pid: 1, unref: () => {} },
    2500,
    timer.deps,
  );
