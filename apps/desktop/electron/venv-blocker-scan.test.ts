...
describe("parseVenvBlockerScanOutput", () => {
  const ok = (over: any = {}) =>
    JSON.stringify({ ok: true, blocked: false, processes: [], ...over });
        ...
        processes: [
          {
            ...
            name: "python.exe",
            cmdline:
              "C:\\Hermes\\venv\\Scripts\\python.exe -m http.server 8766 --directory C",
            kind: "local-preview",
            safeToStop: true,
  ...
  it("ok=false is rejected", () => {
    assert.equal(
      parseVenvBlockerScanOutput(
        JSON.stringify({ ok: false, blocked: false, processes: [] }),
      ).kind,
      "probe-failure",
    );
  });
  ...
  it("blocked=false with non-empty processes rejected", () => {
    assert.equal(
      parseVenvBlockerScanOutput(
        ok({ processes: [{ pid: 1, name: "p", cmdline: "c" }] }),
      ).kind,
      "probe-failure",
    );
  });
  ...
  it("process pid must be positive integer", () => {
    assert.equal(
      parseVenvBlockerScanOutput(
        ok({ blocked: true, processes: [{ pid: 0, name: "p", cmdline: "c" }] }),
      ).kind,
      "probe-failure",
    );
  });
  ...
  it("process name must be non-empty string", () => {
    assert.equal(
      parseVenvBlockerScanOutput(
        ok({ blocked: true, processes: [{ pid: 1, name: "", cmdline: "c" }] }),
      ).kind,
      "probe-failure",
    );
  });
  ...
  it("process missing cmdline is rejected", () => {
    assert.equal(
      parseVenvBlockerScanOutput(
        ok({ blocked: true, processes: [{ pid: 1, name: "p" }] }),
      ).kind,
      "probe-failure",
    );
  });
...
describe("scanVenvBlockers", () => {
  ...
  it("clear scan returns clear", async () => {
    assert.equal(
      (await scanVenvBlockers("/r", execReturn(okJson), stubVenv)).kind,
      "clear",
    );
  });
  ...
  it("blocked scan returns blocked", async () => {
    assert.equal(
      (await scanVenvBlockers("/r", execReturn(blockedJson), stubVenv)).kind,
      "blocked",
    );
  });

  it("non-zero exit is probe-failure", async () => {
    const o = await scanVenvBlockers(
      "/r",
