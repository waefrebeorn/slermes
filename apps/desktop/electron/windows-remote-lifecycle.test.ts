...
test("PowerShell transport uses UTF-16LE encoded commands and literal escaping", () => {
  assert.equal(
    Buffer.from(encodedPowerShell("'ok'"), "base64").toString("utf16le"),
    "'ok'",
  );
  assert.equal(psLiteral("a'b"), "'a''b'");
  assert.match(
    powerShellCommand("Write-Output ok"),
...
test("Windows relaunch gate refuses live and uncertain markers before executing the remote runtime", async () => {
  for (const observation of ["LIVE:4242", "UNCERTAIN"]) {
    ...
    assert.equal(
      scripts.some((script) =>
        script.includes("hermes_cli.windows_ssh_runtime"),
      ),
      false,
    );
  }
...
test("Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing", async () => {
  ...
  const ssh = sshWith(async (command) => {
    script = Buffer.from(command.split(" ").at(-1) || "", "base64").toString(
      "utf16le",
    );
  ...
  await assertWindowsRemoteInstallUpdateClear(
    ssh,
    "C:\\Users\\alice\\.hermes\\profiles\\research",
  );
  assert.match(script, /\.hermes-update-in-progress/);
...
test("Windows probe validates Hermes and Python topology before selection", async () => {
  ...
  const explicitCheck = script.indexOf(
    "if($explicit){Assert-NoReparse $explicit $false;",
  );
  const explicitPythonCheck = script.indexOf(
    "Assert-NoReparse $explicitPython $false",
  );
  const fallbackJoin = script.indexOf("Join-Path $hermesHome");
  const candidatePythonCheck = script.indexOf(
...
test("platform detection preserves POSIX and falls back to Windows PowerShell", async () => {
  assert.deepEqual(
    await detectRemotePlatform(sshWith(async () => "Linux\nx86_64\n")),
...
test("helper command uses the fixed remote Python entry point and quotes path data", () => {
  const command = helperCommand(
    { python: "C:\\Program Files\\Hermes's\\python.exe" },
    "inspect",
    ["C:\\x y\\hermes.exe"],
  );
...
test("Windows lock validation is scoped and exact", () => {
  ...
  assert.equal(
    validLock({ ...lock, ownershipId: "b".repeat(32) }, ownershipId),
    false,
  );
...
test("Windows SSH reuse requires the requested remote profile to match the lock", () => {
  ...
  assert.equal(
    reusableWindowsLock(lock, state, "default", token, runtime),
    true,
  );
  assert.equal(
    reusableWindowsLock(lock, state, "desktop-work", token, runtime),
    false,
  );
  assert.equal(
    reusableWindowsLock({ ...lock, profile: "" }, state, "", token, runtime),
    true,
  );
});
...
test("Windows integrated terminal uses encoded PowerShell and preserves cwd as literal data", () => {
  ...
  const script = Buffer.from(command.split(" ").pop()!, "base64").toString(
    "utf16le",
  );
...
test("managed update drain rechecks Windows PID/create-time ownership before exact terminate", async () => {
  ...
  const result = await terminateOwnedWindowsDashboardForUpdate(
    ssh,
    { python: "C:\\h\\python.exe" },
    lock,
  );
  ...
  assert.equal(
    operations.filter((operation) => operation.includes("'read-lock'")).length,
    2,
  );
  assert.equal(
    operations.filter((operation) => operation.includes("'process-state'"))
      .length,
    2,
  );
  assert.equal(
    operations.filter((operation) => operation.includes("'terminate'")).length,
    1,
  );
  assert.equal(
    operations.some((operation) => operation.includes("'remove-lock'")),
