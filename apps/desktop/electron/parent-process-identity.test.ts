...
test("electronProcessStartMarker uses Electron creation time only for its own PID", () => {
  assert.equal(
    electronProcessStartMarker(42, 42, 1_723_456_789_123.75),
    "winms:1723456789123",
  );
...
test("parentWatchdogEnv atomically falls back to PID-only identity", () => {
  ...
  assert.throws(
    () => parentWatchdogEnv(42, "", "nonce-1"),
    /marker and nonce must be non-empty/,
  );
  assert.throws(
    () => parentWatchdogEnv(42, "winms:1723456789123", ""),
    /marker and nonce must be non-empty/,
  );
});
...
test("spawnTag derives seconds from a winms marker and dashes without one", () => {
  assert.equal(
    spawnTag(42, "winms:1723456789123"),
    "v1:-:serve:42:1723456789.123",
  );
