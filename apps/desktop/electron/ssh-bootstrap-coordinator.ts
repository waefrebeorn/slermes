  ...
  function start(scope, fingerprint, run, metadata = null) {
    if (shutdownRequested) {
      const error: any = new Error(
        "SSH bootstrap was cancelled because Desktop is quitting.",
      );
    ...
    const lease = {
      ...
      isCurrent: () =>
        !controller.signal.aborted && generations.get(scope) === generation,
      assertCurrent() {
        if (!this.isCurrent()) {
          const error: any = new Error(
    ...
    const predecessor = current
      ? Promise.allSettled([current.promise, drain])
      : drain;
    const entry: any = {
      controller,
      fingerprint,
  ...
  async function cancelAndWait(scope) {
    ...
    try {
      ...
      await Promise.allSettled(
        entries
          .flatMap((entry) => [...entry.forceCleanups])
          .map((cleanup) => cleanup()),
      );
