...
describe("isTransientTransportError", () => {
  ...
  it("rejects non-transport errors", () => {
    ...
    expect(
      isTransientTransportError(new Error("Invalid JSON from http://x")),
    ).toBe(false);
...
describe("shouldRetryRequest truth table", () => {
  ...
  const refused = () =>
    errWithCode("ECONNREFUSED", "connect ECONNREFUSED 127.0.0.1:1");
  ...
  it("GET: never retries non-transport errors (HTTP 4xx/5xx surfaced as Error)", () => {
    expect(
      shouldRetryRequest(new Error("500: boom"), "GET", { bodySent: true }),
    ).toBe(false);
  });

  it("POST: retries when the connection provably never happened", () => {
    expect(shouldRetryRequest(refused(), "POST", { bodySent: false })).toBe(
      true,
    );
    expect(shouldRetryRequest(refused(), "POST", { bodySent: true })).toBe(
      true,
    ); // refused == nothing sent
    expect(
      shouldRetryRequest(errWithCode("ENOTFOUND"), "PUT", { bodySent: false }),
    ).toBe(true);
  });
  ...
  it("POST: retries transient errors thrown before the body was flushed", () => {
    ...
    expect(shouldRetryRequest(hangUp(), "DELETE", { bodySent: false })).toBe(
      true,
    );
  });
  ...
  it("POST: does NOT retry ambiguous resets after the body went out", () => {
    ...
    expect(shouldRetryRequest(hangUp(), "POST", { bodySent: true })).toBe(
      false,
    );
    expect(
      shouldRetryRequest(errWithCode("EPIPE"), "PUT", { bodySent: true }),
    ).toBe(false);
    expect(
      shouldRetryRequest(errWithCode("ETIMEDOUT"), "DELETE", {
        bodySent: true,
      }),
    ).toBe(false);
  });
...
function bareJsonGet(url: string): Promise<any> {
  return new Promise((resolve, reject) => {
    const req = http.request(
      new URL(url),
      { agent: jsonAgentFor("http:"), method: "GET" },
      (res) => {
        const chunks: Buffer[] = [];
        res.on("error", reject);
...
function retriedJsonGet(url: string): Promise<any> {
  return withRetry(() => bareJsonGet(url), {
    method: "GET",
...
describe("live: POST reset after server-side processing", () => {
  it("does not double-submit: server hit count stays 1, error surfaces", async () => {
    ...
    try {
      await expect(postOnce()).rejects.toSatisfy((error: any) =>
