...
test("resolveRemoteSshDashboardProfile never sends a conn: pool key to the remote", () => {
  ...
  assert.equal(
    resolveRemoteSshDashboardProfile("", "conn:mac-mini::default"),
    "",
  );
  assert.equal(
    resolveRemoteSshDashboardProfile(undefined, "conn:spark::default"),
    "",
  );
  assert.equal(
    resolveRemoteSshDashboardProfile("", "conn:mac-mini::dixie"),
    "dixie",
  );
  assert.equal(resolveRemoteSshDashboardProfile("", "bob"), "bob");
  assert.equal(resolveRemoteSshDashboardProfile("", "default"), "");
  assert.equal(
    resolveRemoteSshDashboardProfile("writer", "conn:mac-mini::default"),
    "writer",
  );
});
...
test("remoteRequestMatchesBaseUrl treats HTTPS and WSS as the same gateway origin", () => {
  ...
  assert.equal(
    remoteRequestMatchesBaseUrl(
      "wss://hermes.example.com/other/api/ws",
      "https://hermes.example.com/gateway",
    ),
    false,
  );
  assert.equal(
    remoteRequestMatchesBaseUrl(
      "wss://other.example.com/gateway/api/ws",
      "https://hermes.example.com/gateway",
    ),
    false,
  );
...
test("profileRemoteOverride ignores local or url-less profile entries", () => {
  assert.equal(
    profileRemoteOverride(
      { profiles: { p: { mode: "local", url: "https://x" } } },
      "p",
    ),
    null,
  );
  assert.equal(
    profileRemoteOverride(
      { profiles: { p: { mode: "remote", url: "" } } },
      "p",
    ),
    null,
  );
  assert.equal(
    profileRemoteOverride({ profiles: { p: { mode: "remote" } } }, "p"),
    null,
  );
});
...
test("profileRemoteOverride preserves an explicit oauth auth mode", () => {
  const config = {
    profiles: {
      coder: { mode: "remote", url: "https://x", authMode: "oauth" },
    },
  };
...
test("normalizeSshConfig rejects unsafe remote profile mappings", () => {
  assert.deepEqual(
    normalizeSshConfig({ mode: "ssh", host: "box", remoteProfile: "writer_2" }),
    {
      mode: "ssh",
      host: "box",
  ...
  assert.deepEqual(
    normalizeSshConfig({ mode: "ssh", host: "box", remoteProfile: "" }),
    {
      mode: "ssh",
      host: "box",
    },
  ...
  assert.deepEqual(
    normalizeSshConfig({ mode: "ssh", host: "box", remoteProfile: "root" }),
    {
      mode: "ssh",
      host: "box",
    },
  ...
  assert.deepEqual(
    normalizeSshConfig({ mode: "ssh", host: "box", remoteProfile: "default" }),
    {
      mode: "ssh",
      host: "box",
      remoteProfile: "default",
...
test("normalizeSshConfig handles IPv6 and strict port bounds", () => {
  ...
  assert.deepEqual(
    normalizeSshConfig({ mode: "ssh", host: "box", port: "2222junk" }),
    {
      mode: "ssh",
      host: "box",
    },
  ...
  assert.deepEqual(
    normalizeSshConfig({ mode: "ssh", host: "box", port: 65536 }),
    {
      mode: "ssh",
      host: "box",
    },
...
test('normalizeSshConfig strips a pasted "ssh " command prefix', () => {
  ...
  assert.deepEqual(
    normalizeSshConfig({ mode: "ssh", host: "SSH root@box:2222" }),
    {
      mode: "ssh",
      host: "box",
      user: "root",
...
test("localProfileEntry preserves inactive SSH drafts but drops Cloud state", () => {
  ...
  assert.equal(
    localProfileEntry({ mode: "cloud", url: "https://agent" }),
    null,
  );
});
...
test("saved SSH drafts are inactive and explicit overrides take precedence", () => {
  ...
  const config: any = {
    profiles: { coder: { mode: "local", savedSsh: saved } },
  };
...
for (const route of ROUTES) {
  test(`resolveProfileBackendRoute: ${route.name}`, () => {
    assert.deepEqual(
      resolveProfileBackendRoute(route.profile, route.opts),
      route.expected,
    );
  });
}
...
test("apiRequestRegistryConnectionId extracts a genuinely non-local connection id", () => {
  ...
  assert.equal(
    apiRequestRegistryConnectionId({ connectionId: "  gw-1  ", path: "/x" }),
    "gw-1",
  );
});
...
test("apiRequestRegistryConnectionId preserves an explicit local registry route", () => {
  assert.equal(
    apiRequestRegistryConnectionId({ connectionId: "local", path: "/x" }),
    "local",
  );
});
...
test("apiRequestRegistryConnectionId resolves null for unscoped legacy routes", () => {
  assert.equal(
    apiRequestRegistryConnectionId({ path: "/api/cron/jobs" }),
    null,
  );
  assert.equal(
    apiRequestRegistryConnectionId({ connectionId: "", path: "/x" }),
    null,
  );
  assert.equal(
    apiRequestRegistryConnectionId({ connectionId: null, path: "/x" }),
    null,
  );
...
test("pathWithProfileScope keeps an explicit profile query and no-ops on empty profile", () => {
  assert.equal(
    pathWithProfileScope("/api/cron/jobs?profile=all", "research"),
...
test("pathForRegistryBackendRequest uses the resolved registry backend scope", () => {
  assert.equal(
    pathForRegistryBackendRequest(
      "/api/fs/read-data-url?path=%2Fsrv%2Fimage.png",
      "research",
      {
        sharedRemote: true,
      },
  ...
  assert.equal(
    pathForRegistryBackendRequest(
      "/api/fs/download?path=%2Fsrv%2Freport.pdf&profile=mara",
      "mara",
      {
        remoteProfile: "default",
      },
...
test("pathWithGlobalRemoteProfile preserves cross-profile selectors when translating an SSH alias", () => {
  ...
  assert.equal(
    pathWithGlobalRemoteProfile("/api/cron/jobs?profile=all", "mara", opts),
    "/api/cron/jobs?profile=all",
  );
  assert.equal(
...
test("translateSelfProfileQuery rewrites the self-profile filter into the backend namespace", () => {
  ...
  assert.equal(
    translateSelfProfileQuery(
      "/api/cron/blueprints/instantiate?profile=mara",
      "mara",
      "default",
    ),
...
test("translateSelfProfileQuery leaves cross-profile and unfiltered paths untouched", () => {
  assert.equal(
    translateSelfProfileQuery("/api/cron/jobs?profile=all", "mara", "default"),
    "/api/cron/jobs?profile=all",
  );
  assert.equal(
    translateSelfProfileQuery(
  ...
  assert.equal(
    translateSelfProfileQuery("/api/cron/jobs", "mara", "default"),
    "/api/cron/jobs",
  );
});
...
test("translateSelfProfileQuery no-ops when alias and backend profile agree or are missing", () => {
  assert.equal(
    translateSelfProfileQuery("/api/cron/jobs?profile=mara", "mara", "mara"),
    "/api/cron/jobs?profile=mara",
  );
  assert.equal(
    translateSelfProfileQuery("/api/cron/jobs?profile=mara", "mara", ""),
    "/api/cron/jobs?profile=mara",
  );
  assert.equal(
    translateSelfProfileQuery("/api/cron/jobs?profile=mara", "", "default"),
    "/api/cron/jobs?profile=mara",
  );
});
...
test("resolveProfileApiRequest uses exact method and path eligibility for mixed families", () => {
  ...
  assert.deepEqual(
    resolveProfileApiRequest(
      "iris",
      "/api/model/recommended-default?provider=nous",
      {
        requestMethod: "GET",
      },
    ),
...
test("resolveProfileApiRequest routes action-status polls with the action-spawning routes", () => {
  ...
  assert.deepEqual(
    resolveProfileApiRequest(
      "iris",
      "/api/actions/skills-install-ascii-art-dd7bccf1/status?lines=200",
      {
        requestMethod: "GET",
      },
    ),
    {
      backendProfile: null,
      requestPath:
        "/api/actions/skills-install-ascii-art-dd7bccf1/status?lines=200&profile=iris",
    },
  );
  ...
  assert.deepEqual(
    ...
    {
      backendProfile: null,
      requestPath: "/api/skills/hub/install?profile=iris",
    },
  );
  ...
  assert.deepEqual(
    ...
    {
      backendProfile: null,
      requestPath: "/api/mcp/catalog/install?profile=iris",
    },
  );
});
...
test("normalizeRemoteBaseUrl strips trailing slashes, hash, and query", () => {
  assert.equal(
    normalizeRemoteBaseUrl("https://gw.example.com/"),
    "https://gw.example.com",
  );
  assert.equal(
    normalizeRemoteBaseUrl("https://gw.example.com/hermes/"),
    "https://gw.example.com/hermes",
  );
  assert.equal(
    normalizeRemoteBaseUrl("https://gw.example.com/hermes?x=1#frag"),
    "https://gw.example.com/hermes",
  );
});
...
test("normalizeRemoteBaseUrl preserves a path prefix", () => {
  assert.equal(
    normalizeRemoteBaseUrl("https://host/hermes"),
    "https://host/hermes",
  );
});
...
test("normalizeRemoteBaseUrl rejects non-http(s) protocols", () => {
  assert.throws(
    () => normalizeRemoteBaseUrl("ftp://host"),
    /http:\/\/ or https:\/\//,
  );
  assert.throws(
    () => normalizeRemoteBaseUrl("file:///etc/passwd"),
    /http:\/\/ or https:\/\//,
  );
});
...
test("normalizeRemoteBaseUrl auto-prepends http:// for scheme-less host:port input", () => {
  assert.equal(
    normalizeRemoteBaseUrl("100.64.0.1:9119"),
    "http://100.64.0.1:9119",
  );
  assert.equal(
    normalizeRemoteBaseUrl("mini.tailnet-1234.ts.net:9119"),
    "http://mini.tailnet-1234.ts.net:9119",
  );
  assert.equal(
    normalizeRemoteBaseUrl("localhost:9119"),
    "http://localhost:9119",
  );
  assert.equal(
    normalizeRemoteBaseUrl("gw.example.com"),
    "http://gw.example.com",
  );
  assert.equal(
    normalizeRemoteBaseUrl("gw.example.com/hermes/"),
    "http://gw.example.com/hermes",
  );
});

test("normalizeRemoteBaseUrl still rejects explicit non-http(s) schemes after scheme-less handling", () => {
  assert.throws(
    () => normalizeRemoteBaseUrl("ws://host:9119"),
    /http:\/\/ or https:\/\//,
  );
  assert.throws(
    () => normalizeRemoteBaseUrl("ftp://host:21"),
    /http:\/\/ or https:\/\//,
  );
});
...
test("buildGatewayWsUrlWithTicket uses ?ticket= not ?token=", () => {
  const url = buildGatewayWsUrlWithTicket(
    "https://gw.example.com/hermes",
    "tkt-9",
  );
...
test("authModeFromStatus returns oauth when auth_required is true", () => {
  assert.equal(
    authModeFromStatus({ auth_required: true, auth_providers: ["nous"] }),
    "oauth",
  );
});
...
test("cookiesHaveSession detects the bare access-token cookie", () => {
  assert.equal(
    cookiesHaveSession([{ name: "hermes_session_at", value: "x" }]),
    true,
  );
});
...
test("cookiesHaveSession detects the __Host- and __Secure- prefixed variants", () => {
  assert.equal(
    cookiesHaveSession([{ name: "__Host-hermes_session_at", value: "x" }]),
    true,
  );
  assert.equal(
    cookiesHaveSession([{ name: "__Secure-hermes_session_at", value: "x" }]),
    true,
  );
});
...
test("cookiesHaveSession is false for an empty value", () => {
  assert.equal(
    cookiesHaveSession([{ name: "hermes_session_at", value: "" }]),
    false,
  );
});
...
test("cookiesHaveSession ignores unrelated cookies (AT-only by design)", () => {
  ...
  assert.equal(
    cookiesHaveSession([{ name: "hermes_session_rt", value: "x" }]),
    false,
  );
...
test("cookiesHaveLiveSession is true for a live access-token cookie", () => {
  assert.equal(
    cookiesHaveLiveSession([{ name: "hermes_session_at", value: "x" }]),
    true,
  );
  assert.equal(
    cookiesHaveLiveSession([{ name: "__Host-hermes_session_at", value: "x" }]),
    true,
  );
  assert.equal(
    cookiesHaveLiveSession([
      { name: "__Secure-hermes_session_at", value: "x" },
    ]),
    true,
  );
});
...
test("cookiesHaveLiveSession is true for an RT cookie even with NO access-token cookie", () => {
  ...
  assert.equal(
    cookiesHaveLiveSession([{ name: "hermes_session_rt", value: "x" }]),
    true,
  );
  assert.equal(
    cookiesHaveLiveSession([{ name: "__Host-hermes_session_rt", value: "x" }]),
    true,
  );
  assert.equal(
    cookiesHaveLiveSession([
      { name: "__Secure-hermes_session_rt", value: "x" },
    ]),
    true,
  );
});
...
test("cookiesHaveLiveSession is false for empty values", () => {
  assert.equal(
    cookiesHaveLiveSession([{ name: "hermes_session_at", value: "" }]),
    false,
  );
  assert.equal(
    cookiesHaveLiveSession([{ name: "hermes_session_rt", value: "" }]),
    false,
  );
  assert.equal(
    cookiesHaveLiveSession([
...
test("cookiesHavePrivySession detects the privy-token access cookie", () => {
  assert.equal(
    cookiesHavePrivySession([{ name: "privy-token", value: "jwt" }]),
    true,
  );
});
...
test("cookiesHavePrivySession detects __Host-/__Secure- prefixes and the legacy privy-session name", () => {
  assert.equal(
    cookiesHavePrivySession([{ name: "__Host-privy-token", value: "x" }]),
    true,
  );
  assert.equal(
    cookiesHavePrivySession([{ name: "__Secure-privy-token", value: "x" }]),
    true,
  );
  assert.equal(
    cookiesHavePrivySession([{ name: "privy-session", value: "x" }]),
    true,
  );
});
...
test("cookiesHavePrivySession is false for an empty value", () => {
  assert.equal(
    cookiesHavePrivySession([{ name: "privy-token", value: "" }]),
    false,
  );
});
...
test("cookiesHavePrivySession does NOT treat hermes gateway cookies as a portal session", () => {
  ...
  assert.equal(
    cookiesHavePrivySession([{ name: "hermes_session_at", value: "x" }]),
    false,
  );
  assert.equal(
    cookiesHavePrivySession([{ name: "__Host-hermes_session_rt", value: "x" }]),
    false,
  );
});
...
test("cookiesHavePrivySession treats refresh-token material as a (renewable) session", () => {
  ...
  assert.equal(
    cookiesHavePrivySession([{ name: "privy-refresh-token", value: "x" }]),
    true,
  );
});
...
test("cookiesHavePrivyAccessToken detects privy-token and its secured prefixes", () => {
  assert.equal(
    cookiesHavePrivyAccessToken([{ name: "privy-token", value: "jwt" }]),
    true,
  );
  assert.equal(
    cookiesHavePrivyAccessToken([{ name: "__Host-privy-token", value: "x" }]),
    true,
  );
  assert.equal(
    cookiesHavePrivyAccessToken([{ name: "__Secure-privy-token", value: "x" }]),
    true,
  );
});
...
test("cookiesHavePrivyAccessToken is false for empty values, gateway cookies, and non-arrays", () => {
  assert.equal(
    cookiesHavePrivyAccessToken([{ name: "privy-token", value: "" }]),
    false,
  );
  assert.equal(
    cookiesHavePrivyAccessToken([{ name: "hermes_session_at", value: "x" }]),
    false,
  );
...
test("resolveTestWsUrl (token mode) builds a ?token= URL the WS probe can use", async () => {
  const url = await resolveTestWsUrl(
    "https://gw.example.com",
    "token",
    "tok123",
  );
...
test("resolveTestWsUrl (token mode, no token) returns null — genuine skip", async () => {
  assert.equal(
    await resolveTestWsUrl("https://gw.example.com", "token", null),
    null,
  );
});
...
test("resolveTestWsUrl (oauth, auth rejected) requests sign-in and does not skip WS validation", async () => {
  const cause = Object.assign(new Error("ticket mint failed"), {
    statusCode: 401,
  });
...
test("gateway ticket failures classify only explicit auth rejection statuses as reauth", () => {
  ...
  const serverFailure = gatewayTicketFailure(
...
test("gateway WS URL IPC result serializes success and the auth-vs-transport matrix", async () => {
  assert.deepEqual(
    await gatewayWsUrlIpcResult(
      async () => "wss://gateway.example.com/api/ws?ticket=fresh",
    ),
    {
      ok: true,
  ...
  for (const statusCode of [401, 403]) {
    const error = Object.assign(new Error(`${statusCode}: rejected`), {
      statusCode,
    });

    assert.deepEqual(
      await gatewayWsUrlIpcResult(async () => Promise.reject(error)),
      {
...
test("OAuth ticket-mint 503 surfaces the Cloud-down error (startup boundary)", () => {
  ...
  if (cloudError !== null) {
    ...
    assert.ok(
      cloudError.message.includes(
        "Nous Cloud agent ares-3009.agents.nousresearch.com is down",
      ),
    );
  ...
  assert.fail(
    `expected Cloud-down classification, got wrapper: ${wrapped.message}`,
  );
});
