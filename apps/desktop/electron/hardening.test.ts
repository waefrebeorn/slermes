...
test("encryptDesktopSecret stores safeStorage base64 payload", () => {
  ...
  assert.doesNotMatch(
    String(secret?.value),
    /token-123/,
    "the plaintext is not recoverable from the payload",
  );
});
...
test("writeSecretFileAtomic creates the file owner-only, not at the 0644 umask default", () => {
  withTempDir((dir) => {
    ...
    const payload = JSON.stringify({
      remote: { token: { encoding: SAFE_STORAGE_ENCODING, value: "BLOB" } },
    });
...
test("enableBasicPasswordStoreEncryption ignores non-basic password-store values on linux", () => {
  for (const passwordStoreSwitch of ["gnome-libsecret", "", undefined]) {
    ...
    assert.equal(
      result,
      false,
      `value ${JSON.stringify(passwordStoreSwitch)} must not enable plain text`,
    );
...
test("enableBasicPasswordStoreEncryption never enables plain text off linux even with --password-store=basic", () => {
  for (const platform of ["win32", "darwin"]) {
    ...
    assert.equal(
      result,
      false,
      `platform ${platform} must not enable plain text`,
    );
    assert.deepEqual(calls, []);
  }
});
...
test("resolvePersistedRemoteToken stores plain text end-to-end only with the explicit opt-in", () => {
  ...
  const encryptSecret = (value: string, options: any) =>
    encryptDesktopSecret(value, unavailableSafeStorage, options);
...
test("resolvePersistedRemoteToken keeps encrypting when the keyring is available even with the opt-in", () => {
  ...
  const encryptSecret = (value: string, options: any) =>
    encryptDesktopSecret(value, availableSafeStorage, options);
...
test("resolvePersistedRemoteToken passes the token through untouched on the transient path", () => {
  ...
  assert.equal(
    called,
    false,
    "the transient test-connection path must not touch secure storage",
  );
});
...
test("resolvePersistedRemoteToken keeps the existing token when no new token is supplied", () => {
  ...
  assert.equal(
    called,
    false,
    "an empty incoming token must not re-encrypt anything",
  );
});
  ...
  withTempDir((dir) => {
    ...
    try {
      ...
      assert.notEqual(
        modeOf(witness),
        SECRET_FILE_MODE,
        "the ambient default is NOT already owner-only",
      );

      writeSecretFileAtomic(target, "tok", {
        fs: fsWith({ chmodSync: () => void 0 }),
      });
    } finally {
    ...
    assert.equal(
      modeOf(target),
      SECRET_FILE_MODE,
      "created owner-only, not tightened after the fact",
    );
  });
});
...
test("the written file is owner-only even when a stale temp cannot be removed", () => {
  ...
  withTempDir((dir) => {
    ...
    writeSecretFileAtomic(target, "tok", {
      fs: fsWith({ rmSync: () => void 0 }),
    });

    assert.equal(
      modeOf(target),
      SECRET_FILE_MODE,
      "tightened before the rename handed the bits over",
    );
  ...
  withTempDir((dir) => {
    ...
    const legacyPlain = JSON.stringify({
      ...
      remote: {
        url: "https://gw.example.com",
        authMode: "token",
        token: { encoding: "plain", value: "tok-live-42" },
      },
    });
    ...
    assert.equal(modeOf(target), SECRET_FILE_MODE);
    assert.equal(
      JSON.parse(fs.readFileSync(target, "utf8")).remote.token.value,
      "tok-live-42",
    );
  });
});
...
test("tightenSecretFileMode refuses to chmod a symlink instead of following it to its target", () => {
  ...
  withTempDir((dir) => {
    ...
    assert.equal(
      tightenSecretFileMode(target),
      false,
      'reports "not tightened" rather than acting on the link',
    );
    assert.equal(
      modeOf(victim),
      0o644,
      "the symlink target keeps its own mode",
    );
  });
});
...
test("tightenSecretFileMode leaves Windows alone rather than flipping the read-only bit", () => {
  ...
  // suppressed it above.
  assert.equal(
    tightenSecretFileMode("/home/me/connection.json", {
      fs: fakeFs,
      platform: "linux",
    }),
    true,
  ...
  assert.ok(
    chmods.includes("/home/me/connection.json"),
    "the POSIX path was tightened",
  );
});
  ...
  assert.throws(
    ...
    (error: unknown) => {
      assert.ok(
        ...
        "aborts instead of returning a payload",
      );
      assert.match(
      ...
      assert.doesNotMatch(
        String((error as Error).message),
        /tok-live-42/,
        "the secret is not echoed in the error",
      );
...
test("sensitiveFileBlockReason blocks obvious secret file patterns", () => {
  ...
  assert.match(
    String(sensitiveFileBlockReason("/Users/me/.ssh/id_ed25519")),
    /SSH/,
  );
  assert.match(
    String(sensitiveFileBlockReason("/tmp/server-cert.pem")),
    /\.pem/,
  );
});
...
test("path helpers reject blank non-string NUL and Windows device syntax", async () => {
  await rejectsWithCode(
    resolveReadableFileForIpc("", { purpose: "File preview" }),
    "invalid-path",
  );
  await rejectsWithCode(
    resolveReadableFileForIpc("   ", { purpose: "File preview" }),
  ...
  await rejectsWithCode(
    resolveReadableFileForIpc(null, { purpose: "File preview" }),
    "invalid-path",
  );
  await rejectsWithCode(
    resolveReadableFileForIpc(`safe${String.fromCharCode(0)}name.txt`),
    "invalid-path",
  );
  ...
  for (const devicePath of devicePaths) {
    ...
    await rejectsWithCode(
      resolveReadableFileForIpc(devicePath, { purpose: "File preview" }),
      "device-path",
    );
  }
  ...
  assert.throws(
    () =>
      resolveRequestedPathForIpc("file:///%E0%A4%A", {
        purpose: "File preview",
      }),
    (error: any) => {
  ...
  await rejectsWithCode(
    resolveReadableFileForIpc("file:///%E0%A4%A", { purpose: "File preview" }),
    "invalid-path",
  );
});
...
test("resolveRequestedPathForIpc expands ~ to the home directory", () => {
  assert.equal(
    resolveRequestedPathForIpc("~", { purpose: "Directory read" }),
  ...
  assert.equal(
    resolveRequestedPathForIpc("~other/secret", {
      baseDir: os.tmpdir(),
...
test("resolveReadableFileForIpc validates existence type size and sensitivity", async () => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "hermes-desktop-hardening-"),
  );
  ...
  try {
    ...
    const fromFileUrl = await resolveReadableFileForIpc(
      pathToFileURL(textPath).toString(),
      {
        purpose: "File preview",
      },
    );
    ...
    const fromSpacedFileUrl = await resolveReadableFileForIpc(
      pathToFileURL(spacedPath).toString(),
      {
        purpose: "File preview",
      },
    );
...
test("resolveReadableFileForIpc blocks common sensitive files", async () => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "hermes-desktop-sensitive-"),
  );
  ...
  try {
    ...
    for (const filePath of blockedFiles) {
      ...
      await rejectsWithCode(
        resolveReadableFileForIpc(filePath, { purpose: "File preview" }),
        "sensitive-file",
      );
    }
    ...
    assert.equal(
      (await resolveReadableFileForIpc(allowed, { purpose: "File preview" }))
        .resolvedPath,
      allowed,
    );
  } finally {
...
test("resolveReadableFileForIpc blocks symlinks whose realpath is sensitive", async () => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "hermes-desktop-realpath-"),
  );
  ...
  try {
    ...
    await rejectsWithCode(
      resolveReadableFileForIpc(linkPath, { purpose: "File preview" }),
      "sensitive-file",
    );
  } finally {
    fs.rmSync(tempDir, { recursive: true, force: true });
...
test("resolveDirectoryForIpc accepts directories and rejects invalid directory targets", async () => {
  ...
  try {
    ...
    await rejectsWithCode(
      resolveDirectoryForIpc(path.join(tempDir, "missing")),
      "ENOENT",
    );
    await rejectsWithCode(
      resolveDirectoryForIpc("\\\\?\\C:\\secret"),
      "device-path",
    );
  } finally {
    fs.rmSync(tempDir, { recursive: true, force: true });
...
test("resolveDirectoryForIpc accepts directory symlinks or junctions", async () => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "hermes-desktop-dir-link-"),
  );
...
function readMain() {
  return fs
    .readFileSync(path.join(__dirname, "main.ts"), "utf8")
    .replace(/\r\n/g, "\n");
}
...
test("coerceDesktopConnectionConfig routes token persistence through resolvePersistedRemoteToken", () => {
  ...
  assert.notEqual(
    fnStart,
    -1,
    "coerceDesktopConnectionConfig must exist in main.ts",
  );
  ...
  assert.match(
    body,
    /encryptSecret: encryptDesktopSecret\b/,
    "the helper must encrypt via encryptDesktopSecret",
  );
});
...
test("whenReady enables basic password-store encryption before createWindow", () => {
  ...
  assert.notEqual(
    enableIndex,
    -1,
    "whenReady must call enableBasicPasswordStoreEncryption",
  );
  ...
  assert.match(
    ...
    "the real platform must be forwarded",
  );
  assert.match(
    call,
  ...
  assert.match(
    ...
    "the real safeStorage must be forwarded",
  );
  ...
  assert.notEqual(
    createWindowIndex,
    -1,
    "whenReady must call createWindow after enabling encryption",
  );
  assert.ok(
...
test("sanitizeDesktopConnectionConfig exposes secureTokenStorage and remoteTokenPlainText", () => {
  ...
  assert.notEqual(
    fnStart,
    -1,
    "sanitizeDesktopConnectionConfig must exist in main.ts",
  );
  const fnEnd = source.indexOf("\nfunction ", fnStart + 1);
  const body = source.slice(fnStart, fnEnd === -1 ? undefined : fnEnd);
  ...
  assert.notEqual(
    returnIndex,
    -1,
    "sanitizeDesktopConnectionConfig must return a sanitized object",
  );
  const returned = body.slice(returnIndex);
  assert.match(
    returned,
    /\bsecureTokenStorage\b/,
    "the renderer needs the secure-storage availability signal",
  );
  assert.match(
    returned,
    /\bremoteTokenPlainText\b/,
    "the renderer needs the plain-text token signal",
  );
});
