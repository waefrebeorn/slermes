...
function createFakeDisk(
  ...
  overrides: Partial<NativeTokenStoreIo> = {},
): FakeDisk {
...
test("a fresh load restores both tokens and preserves expiry, provider and user", () => {
  ...
  const loaded = loadNativeTokenSet(
    GATEWAY,
    createFakeDisk(first.fileText()).io,
  )!;
...
test("the full login-to-restart sequence keeps the two parser boundaries apart", () => {
  ...
  // it back unchanged.
  assert.deepEqual(
    loadNativeTokenSet(GATEWAY, createFakeDisk(first.fileText()).io),
    fromGateway,
  );
});
...
test("persisting one gateway leaves other gateways intact", () => {
  ...
  persistNativeTokenSet(
    other,
    { ...TOKENS, accessToken: "AT-other", userId: "u-99" },
    disk.io,
  );
  ...
  assert.equal(
    loadNativeTokenSet(GATEWAY, restarted.io)!.accessToken,
    "AT-live-abc123",
  );
  assert.equal(
    loadNativeTokenSet(other, restarted.io)!.accessToken,
    "AT-other",
  );
});
...
test("a locked keychain keeps the stored entry for a later retry", () => {
  ...
  assert.match(
    locked.logs[0],
    /failed to decrypt stored tokens for https:\/\/gw\.example\.com/,
  );
...
test("an array store file is replaced by a real map rather than swallowing the write", () => {
  ...
  assert.deepEqual(
    loadNativeTokenSet(GATEWAY, createFakeDisk(disk.fileText()).io),
    TOKENS,
  );
});
...
test("a corrupt decrypted blob is reported and loads as signed out", () => {
  const disk = createFakeDisk(
    JSON.stringify({
      [GATEWAY]: { encoding: "safeStorage", value: "bm90LWpzb24=" },
    }),
  );
  ...
  assert.match(
    disk.logs[0],
    /failed to load stored tokens for https:\/\/gw\.example\.com/,
  );
});
...
test("a non-Error decryption failure keeps its detail in the log", () => {
  const disk = createFakeDisk(
    JSON.stringify({ [GATEWAY]: { encoding: "safeStorage", value: "AAAA" } }),
    {
      decrypt: () => {
...
test("a non-Error write failure keeps its detail in the log", () => {
  ...
  assert.equal(
    disk.logs[0],
    "[native-oauth] failed to persist tokens: disk went away",
  );
});
...
test("an encrypt that returns null is refused rather than blanking the stored entry", () => {
  ...
  assert.deepEqual(
    loadNativeTokenSet(GATEWAY, createFakeDisk(before).io),
    TOKENS,
  );
});
...
test("a decryption failure logs the gateway host and path but not its credentials", () => {
  ...
  assert.match(
    locked.logs[0],
    /failed to decrypt stored tokens for https:\/\/gw\.example\.com\/hermes/,
  );
  assert.match(locked.logs[0], /keeping stored entry for retry/);
...
test("a parsing failure logs the gateway host and path but not its credentials", () => {
  const disk = createFakeDisk(
    JSON.stringify({
      [CRED_GATEWAY]: { encoding: "safeStorage", value: "bm90LWpzb24=" },
    }),
  );
  ...
  assert.match(
    disk.logs[0],
    /failed to load stored tokens for https:\/\/gw\.example\.com\/hermes/,
  );
...
test("the credential-bearing base URL stays the exact store key", () => {
  ...
  assert.deepEqual(
    loadNativeTokenSet(CRED_GATEWAY, createFakeDisk(first.fileText()).io),
    TOKENS,
  );
...
test("an unparseable gateway URL logs a fixed placeholder rather than the raw value", () => {
  ...
  const disk = createFakeDisk(
    JSON.stringify({ [invalid]: { encoding: "safeStorage", value: "AAAA" } }),
    {
      decrypt: () => "",
    },
  );
