...
async function helper(ssh, runtime, operation, args = [], stdinData?) {
  const output = await ssh.exec(
    helperCommand(runtime, operation, args),
...
async function atomicWindowsSpawn(
  ssh,
  runtime,
  stdinData,
  reservation: any = {},
  ...
  const output = await ssh.exec(
    atomicWindowsSpawnCommand(runtime, reservation),
    { stdinData },
  );
...
async function cleanupOwned(ssh, runtime, ownershipId, lock) {
  ...
  if (lock) {
    ...
    if (lock.spawnNonce) {
      await attempt(() =>
        helper(ssh, runtime, "remove-token", [ownershipId, lock.spawnNonce]),
      );
      await attempt(() =>
        helper(ssh, runtime, "remove-log", [ownershipId, lock.spawnNonce]),
      );
    }
  }
...
async function terminateOwnedWindowsDashboardForUpdate(ssh, runtime, expected) {
  let lock = await helper(ssh, runtime, "read-lock", [
    expected?.ownershipId || "",
  ]);

  if (
    !validLock(lock, expected?.ownershipId) ||
    !windowsLockMatchesManagedUpdateScope(lock, expected)
  ) {
    const error: any = new Error(
      "The remote Windows ownership record changed before the managed update.",
    );
  ...
  if (state.indeterminate) {
    const error: any = new Error(
      "Could not prove the remote Windows process identity for the managed update.",
    );
  ...
  if (!state.owned) {
    const error: any = new Error(
      "Refusing to terminate a remote Windows process whose ownership is unproven.",
    );
  ...
  if (
    ...
    const error: any = new Error(
      "The remote Windows ownership record changed during process verification.",
    );
    error.kind = "ownership-changed";
    throw error;
  }
  ...
  if (state.indeterminate || !state.alive || !state.owned) {
    const error: any = new Error(
      "The remote Windows process identity changed during managed update drain.",
    );
    error.kind = state.indeterminate
...
async function waitReady(ssh, runtime, ownershipId, lock, timeoutMs, signal) {
  ...
  while (Date.now() < deadline) {
    ...
    if (!state.indeterminate && (!state.alive || !state.owned)) {
      ...
      try {
        detail =
          (
            await helper(ssh, runtime, "read-log", [
              ownershipId,
              lock.spawnNonce,
            ])
          )?.content || "";
    ...
    try {
      content =
        (await helper(ssh, runtime, "read-log", [ownershipId, lock.spawnNonce]))
          ?.content || "";
    } catch {
      void 0;
    }
  ...
  const error: any = new Error(
    `Timed out waiting for the remote Windows backend (${timeoutMs}ms).`,
  );
  error.kind = "ready-timeout";
  throw error;
}

async function waitForWindowsSpawnCompletion(
  ssh,
  runtime,
  ownershipId,
  timeoutMs,
  ...
  const error: any = new Error(
    "Timed out waiting for the concurrent Windows SSH connection to publish its backend.",
  );
  error.kind = "spawn-failed";
  throw error;
}
...
async function connectWindowsRemote(deps) {
  ...
  if (!inspection.supported) {
    const error: any = new Error(
      "Update Hermes on the remote Windows host before connecting with Desktop SSH.",
    );
  ...
  if (validLock(lock, ownershipId)) {
    ...
    const reusable = reusableWindowsLock(
      lock,
      state,
      profile,
      reuseToken,
    ...
    if (reusable) {
      ...
      try {
        ...
        const classification = await probeReuseProof(
          baseUrl,
          reuseToken,
          lock.spawnNonce,
        );
  ...
  if (spawned.existing) {
    ...
    const published = await waitForWindowsSpawnCompletion(
      ssh,
      runtime,
      ownershipId,
      readyTimeoutMs,
  ...
  try {
    ...
    await helper(
      ...
      "write-lock",
      [ownershipId],
      JSON.stringify({ ...owned, port: remotePort }),
    );
...
function buildWindowsInteractiveCommand(remoteCwd = "") {
  ...
  script.push(
    '$host.UI.RawUI.WindowTitle="Hermes SSH"',
