...
function createHarness(connection: RegistryGatewayWsConnection) {
  ...
  const handler = createRegistryGatewayWsUrlHandler({
    ensureBackend,
    mintTicket,
    buildTicketUrl: (baseUrl) =>
      `${baseUrl.replace(/^https:/, "wss:")}/api/ws?region=us&ticket=fresh-ticket&profile=old`,
...
function expectRequestHeaders(
  ...
  applyRemoteRequestHeaders(
    { url, requestHeaders: { Origin: "app://hermes" } },
    callback,
    store.headersFor,
  );
  ...
  expect(callback).toHaveBeenCalledWith(
    expected ? { requestHeaders: { Origin: "app://hermes", ...expected } } : {},
  );
}

function expectNoHeadersForNearbyUrls(
  store: ReturnType<typeof createRemoteWsHeaderStore>,
  exactUrl: string,
) {
...
describe("registry gateway WebSocket headers", () => {
  it("evicts the least recently accessed exact URL", () => {
    ...
    const firstUrl =
      "wss://gateway.example/api/ws?token=first&profile=research";
    const secondUrl =
      "wss://gateway.example/api/ws?token=second&profile=research";
    const thirdUrl =
      "wss://gateway.example/api/ws?token=third&profile=research";

    store.remember(firstUrl, accessHeaders);
    store.remember(secondUrl, accessHeaders);
    expect(
      store.headersFor(
        "wss://gateway.example/api/ws?token=missing&profile=research",
      ),
    ).toEqual({});
  ...
  it("token path binds headers to the exact profile scoped URL", async () => {
    ...
    const expectedUrl =
      "wss://gateway.example/api/ws?token=secret&trace=one&profile=research";
  ...
  it("OAuth path binds headers to the exact fresh profile scoped URL", async () => {
    ...
    const result = await handler({
      connectionId: "cloud-one",
      profile: "research",
    });
    const expectedUrl =
      "wss://gateway.example/api/ws?region=us&ticket=fresh-ticket&profile=research";

    expect(result).toBe(expectedUrl);
    expect(mintTicket).toHaveBeenCalledOnce();
    expect(mintTicket).toHaveBeenCalledWith(
  ...
  it("sharedRemote false preserves the original URL and exact header behavior", async () => {
    ...
    expectRequestHeaders(store, result, accessHeaders);
    expect(
      store.headersFor("wss://gateway.example/api/ws?token=secret&trace=one"),
    ).toEqual({});
  });
});
