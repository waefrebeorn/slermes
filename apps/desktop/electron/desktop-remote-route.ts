...
function withConnectionId<T extends object>(
  route: T,
  connectionId?: string,
    ...
    return withConnectionId(
      {
        ...
        org:
          kind === "cloud"
            ? String(profileConfig?.org || "").trim() || undefined
            : undefined,
        source: "profile" as const,
  ...
  return withConnectionId(
    {
      ...
      org:
        kind === "cloud"
          ? String(config.remote?.org || "").trim() || undefined
          : undefined,
      source: "settings" as const,
      token: config.remote?.token,
 ...
 */
function resolveRegistryPrimaryRoute(
  registry: ConnectionRegistry,
): DesktopRemoteRoute | null {
  ...
  const entry = (registry.connections || []).find(
    (connection) => connection.id === primaryId,
  );
  ...
  return {
    ...
    org:
      entry.kind === "cloud"
        ? String(entry.org || "").trim() || undefined
        : undefined,
    source: "registry",
    token: entry.token,
    url,
