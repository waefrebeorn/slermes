...
function isPinned(row: unknown): boolean {
  return Boolean(
    row && typeof row === "object" && "pinned" in row && row.pinned,
  );
}
...
export function mergeProfileSessionWindow(
  rows: unknown[],
  offset: number,
  limit: number,
): unknown[] {
  ...
  const seenIds = new Set(
    window.map(profileSessionId).filter((id): id is string => id !== null),
  );
...
export function buildSidebarSessionSliceParams(
  searchParams: URLSearchParams,
): SidebarSessionSliceParams {
  const profile =
    (searchParams.get("recents_profile") || "all").trim() || "all";
...
export async function fetchPrimaryProfileSessions(
  ...
  try {
    return (await fetchJsonForProfile(
      null,
      `/api/profiles/sessions?${searchParams}`,
    )) as ProfileSessionsResponse;
  } catch {
  ...
  await Promise.all(
    sources.map(async (source) => {
      ...
      if (!data) {
        ...
        flat.delete("profile");
        data = await getJson(shared.descriptor, `/api/sessions?${flat}`).catch(
          () => null,
        );
      }
...
export function spliceRegistrySessionRows(
  ...
  const seen = new Set(
    merged.map(sessionId).filter((id): id is string => id !== null),
  );
  let added = 0;
...
export async function fetchRemoteProfileSessions(
  ...
  while (pageOffset < targetOffset) {
    ...
    const page = (await fetchJsonForProfile(
      profile,
      `/api/sessions?${pageParams}`,
    )) as SessionListResponse;
    firstPage ??= page;
    ...
    const windowedCount =
      total !== null
        ? Math.min(pageLimit, Math.max(0, total - pageOffset))
        : Math.min(pageLimit, pageRows.length);
...
export async function findRemoteOwnerProfileForSession(
  ...
  const matches = await Promise.all(
    remoteProfiles.map(async (profile) => {
      ...
      const rows = Array.isArray(list?.sessions)
        ? (list.sessions as Array<Record<string, unknown>>)
        : [];

      return rows.some(
        (row) => row?.id === sessionId || row?._lineage_root_id === sessionId,
      )
        ? profile
        : null;
    }),
