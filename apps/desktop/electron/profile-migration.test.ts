...
const isValidProfileName = (n: string) =>
  n !== "default" && PROFILE_NAME_RE.test(n);
...
test("scoreStateDb floors recency weight at 0.1 for ancient files", () => {
  const ancient = scoreStateDb("/p", NOW, () => ({
    size: 10 * 1024 * 1024,
    mtimeMs: 0,
  }));
  const fresh = scoreStateDb("/p", NOW, () => ({
    size: 10 * 1024 * 1024,
...
test("scoreStateDb prefers larger DB at similar recency", () => {
  ...
  const big = scoreStateDb("/big", NOW, () => ({
    size: 409 * 1024 * 1024,
    mtimeMs: NOW - 86_400_000,
  }));
  const small = scoreStateDb("/small", NOW, () => ({
    size: 28 * 1024 * 1024,
    mtimeMs: NOW - 60_000,
  }));
...
test("scoreStateDb floors size weight at log10(MIN_SIZE) for tiny files", () => {
  ...
  const tiny = scoreStateDb("/tiny", NOW, () => ({
    size: 100,
    mtimeMs: NOW - 86_400_000,
  }));
  const min = scoreStateDb("/min", NOW, () => ({
    size: PROFILE_SCORE_MIN_SIZE_BYTES,
    mtimeMs: NOW - 86_400_000,
  }));
...
test("decideMigration falls through to scoring when multiple gateways run", () => {
  ...
  const d = decideMigration(
    ...
    ["coder", "writer"],
    deps,
    (p) => (p.endsWith("/writer/state.db") ? 50 : 10),
  );
...
test("decideMigration suppresses write when best is default (single-profile fallback)", () => {
  ...
  const d = decideMigration(null, [], ["default", "coder"], deps, (p) =>
    p.endsWith("/.hermes/state.db") ? 99 : 50,
  );
...
test("migrateActiveProfileIfMissing is a no-op when a user-selected preference file exists", () => {
  ...
  const fs = makeFs({
    ...
    "/home/u/.hermes/profiles/coder": { dir: true },
    "/home/u/.hermes/profiles/writer": { dir: true },
    "/home/u/.hermes/profiles/writer/state.db": {
      size: 400 * 1024 * 1024,
  ...
  assert.equal(
    migrateActiveProfileIfMissing("/cfg/active-profile.json", deps),
    false,
  );
...
test("migrateActiveProfileIfMissing writes heuristic choice with _migrated=true", () => {
  ...
  const fs = makeFs({
    "/home/u/.hermes/profiles/coder/state.db": {
      size: 50 * 1024 * 1024,
      mtime: NOW - 86_400_000,
    },
    "/home/u/.hermes/profiles/writer/state.db": {
...
test("migrateActiveProfileIfMissing still pins a named profile that actually beats default", () => {
  ...
  const fs = makeFs({
    ...
    "/home/u/.hermes/profiles/work/state.db": {
      size: 200 * 1024 * 1024,
      mtime: NOW - 86_400_000,
    },
  });
...
test("readExistingPreference treats _migrated as heuristic-owned", () => {
  const fs = makeFs({
    "/cfg/active-profile.json": {
      content: '{"profile":"conduit","_migrated":true}',
    },
  });

  assert.deepEqual(
    readExistingPreference("/cfg/active-profile.json", fs.readFileSync),
    {
      profile: "conduit",
      migrated: true,
    },
...
test("migrateActiveProfileIfMissing leaves a still-correct heuristic pin alone", () => {
  ...
  const fs = makeFs({
    "/cfg/active-profile.json": {
      content: '{"profile":"work","_migrated":true}',
    },
    "/home/u/.hermes/profiles/work": { dir: true },
    "/home/u/.hermes/state.db": {
      size: 5 * 1024 * 1024,
