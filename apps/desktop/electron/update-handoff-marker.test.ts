...
function sandbox(tag: string) {
  const home = fs.mkdtempSync(
    path.join(os.tmpdir(), `hermes-handoff-marker-${tag}-`),
  );
...
function markerStartedAt(home: string): number {
  const [, startedAt] = fs
    .readFileSync(path.join(home, ".hermes-update-in-progress"), "utf8")
    .split("\n");
...
function assertScriptHandoff(
  ...
  assert.equal(
    ...
    0,
    String(preservedResult.stderr || preservedResult.stdout),
  );
  assert.equal(
    markerStartedAt(preserved.home),
    acquiredAt,
    "the script must preserve the Desktop acquisition time",
  );

  const refreshed = sandbox("refreshed");
  fs.writeFileSync(
    path.join(refreshed.home, ".hermes-update-in-progress"),
    "999999\n1\n",
  );
  ...
  assert.equal(
    refreshedResult.status,
    0,
    String(refreshedResult.stderr || refreshedResult.stdout),
  );
  assert.ok(
    markerStartedAt(refreshed.home) >= before &&
      markerStartedAt(refreshed.home) <= after,
  ...
  assert.equal(
    ...
    0,
    String(oversizedResult.stderr || oversizedResult.stdout),
  );
  assert.ok(
    markerStartedAt(oversized.home) >= oversizedBefore &&
      markerStartedAt(oversized.home) <= oversizedAfter,
