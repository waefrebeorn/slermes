...
test("latches a LOCAL backend failure so the install-retry loop is broken", () => {
  assert.equal(
    shouldLatchBackendStartFailure({ attemptedRemote: false }),
    true,
  );
});
...
test("never latches a REMOTE failure so recovery stays retryable without a restart", () => {
  ...
  assert.equal(
    shouldLatchBackendStartFailure({ attemptedRemote: true }),
    false,
  );
});
...
test("latches a CONFIRMED remote reauth failure so the overlay stays clickable", () => {
  ...
  assert.equal(
    shouldLatchRemoteReauthFailure({ attemptedRemote: true, isReauth: true }),
    true,
  );
});
...
test("does not latch a transient remote failure as reauth", () => {
  ...
  assert.equal(
    shouldLatchRemoteReauthFailure({ attemptedRemote: true, isReauth: false }),
    false,
  );
});
...
test("never latches a LOCAL failure as reauth (that is backendStartFailure job)", () => {
  assert.equal(
    shouldLatchRemoteReauthFailure({ attemptedRemote: false, isReauth: true }),
    false,
  );
  assert.equal(
    shouldLatchRemoteReauthFailure({ attemptedRemote: false, isReauth: false }),
    false,
  );
});
...
test("the two latches never fire for the same failure", () => {
  ...
  for (const attemptedRemote of [true, false]) {
    for (const isReauth of [true, false]) {
      ...
      const reauth = shouldLatchRemoteReauthFailure({
        attemptedRemote,
        isReauth,
      });
      assert.ok(
        !(start && reauth),
        `both latched for remote=${attemptedRemote} reauth=${isReauth}`,
      );
    }
  }
});
...
test("FIX #82679: a transient remote failure is retryable so a dropped SSH/HTTP connection self-heals", () => {
  ...
  assert.equal(
    isRetryableRemoteBootFailure({ attemptedRemote: true, isReauth: false }),
    true,
  );
});
...
test("a CONFIRMED reauth rejection is never auto-retried (missing capability, not transient failure)", () => {
  assert.equal(
    isRetryableRemoteBootFailure({ attemptedRemote: true, isReauth: true }),
    false,
  );
});
...
test("unsigned OAuth latches and is never auto-retried; needsOauthLogin alone still retries", () => {
  ...
  assert.equal(
    isRetryableRemoteBootFailure({ attemptedRemote: true, isReauth: unsigned }),
    false,
  );
...
test("local failures are never auto-retried by the remote self-heal loop", () => {
  assert.equal(
    isRetryableRemoteBootFailure({ attemptedRemote: false, isReauth: false }),
    false,
  );
  assert.equal(
    isRetryableRemoteBootFailure({ attemptedRemote: false, isReauth: true }),
    false,
  );
});
...
test("retryable and reauth-latch are mutually exclusive for remote failures", () => {
  ...
  for (const isReauth of [true, false]) {
    const retry = isRetryableRemoteBootFailure({
      attemptedRemote: true,
      isReauth,
    });
    const latch = shouldLatchRemoteReauthFailure({
      attemptedRemote: true,
      isReauth,
    });
    assert.equal(
      retry !== latch,
      true,
      `remote failure with reauth=${isReauth} must pick exactly one path`,
    );
  }
});
...
test("FIX host-key change: classified from the kind tag and from stringified ssh banners", () => {
  ...
  const tagged = Object.assign(new Error("SSH refused to connect."), {
  ...
  assert.equal(
    isHostKeyChangedBootFailure(new Error("Host key verification failed.")),
    true,
  );
  assert.equal(
    isHostKeyChangedBootFailure(
      new Error(
        "The host key for root@203.0.113.7 has CHANGED since you last connected.",
      ),
    ),
    true,
  );
  assert.equal(
    isHostKeyChangedBootFailure(new Error("Connection refused")),
    false,
  );
...
test("every remote failure picks exactly one path: retry, reauth latch, or host-key latch", () => {
  for (const isReauth of [true, false]) {
    for (const isHostKeyChanged of [true, false]) {
      ...
      const hostKey = shouldLatchHostKeyChangedFailure({
        attemptedRemote: true,
        isReauth,
        isHostKeyChanged,
      });
      ...
      assert.ok(
        picked >= 1,
        `remote failure reauth=${isReauth} hostKey=${isHostKeyChanged} fell through every path`,
      );
    }
  }
});
