...
test("remote sidebar slices preserve the explicit all-profiles scope", () => {
  const slices = buildSidebarSessionSliceParams(
    new URLSearchParams({ recents_profile: "all" }),
  );
...
test("remote sidebar slices fall back to the all-profiles scope and default limits", () => {
  for (const searchParams of [
    new URLSearchParams(),
    new URLSearchParams({ recents_profile: "   " }),
  ]) {
...
test("primary session reads use the profile-aware request path", async () => {
  ...
  assert.deepEqual(calls, [
    { profile: null, path: "/api/profiles/sessions?profile=default&limit=20" },
  ]);
...
test("remote session reads split oversized sidebar windows into API-safe pages", async () => {
  ...
  assert.deepEqual(calls, [
    {
      profile: "remote-work",
      path: "/api/sessions?limit=100&offset=0&order=updated",
    },
    {
      profile: "remote-work",
      path: "/api/sessions?limit=100&offset=100&order=updated",
    },
    {
      profile: "remote-work",
      path: "/api/sessions?limit=50&offset=200&order=updated",
    },
  ]);
...
test("remote paging preserves offsets and deduplicates pinned backfill rows", async () => {
  ...
  const result = await fetchRemoteProfileSessions(
    ...
    async (_profile, path) => {
      ...
      return {
        sessions: [
          ...window,
          ...pinned.filter((row) => !windowIds.has(row.id)),
        ],
        total: rows.length,
        limit,
        offset,
  ...
  assert.deepEqual(calls, [
    "/api/sessions?limit=100&offset=80",
    "/api/sessions?limit=50&offset=180",
  ]);
  assert.deepEqual(
    result.sessions.map((row) => (row as { id: string }).id),
...
test("remote session reads keep small requests on one call", async () => {
  ...
  assert.deepEqual(calls, [
    { profile: "remote-work", path: "/api/sessions?limit=20&offset=0" },
  ]);
  assert.equal(result, expected);
});
...
test("registry sources: ssh backends are read natively and rows tagged with connection + profile", async () => {
  ...
  const rows = await fetchRegistrySessionRows(
    ...
    async (descriptor, path) => {
      ...
      return {
        sessions: [{ id: `s-${descriptor}`, message_count: 3 }],
        total: 1,
      };
    },
  );
  ...
  assert.ok(
    calls.every(
      ({ path }) =>
        path.startsWith("/api/sessions?") && !path.includes("profile="),
    ),
  );
...
test("registry response ownership tagging ignores non-session payloads and transcript messages", () => {
  ...
  assert.equal(
    tagRegistrySessionResponse("/api/status", status, "test-amnezia"),
    status,
  );
  assert.equal(
    tagRegistrySessionResponse(
      "/api/sessions/remote-chat/messages",
      messages,
      "test-amnezia",
    ),
    messages,
    ...
    [
      {
        connectionId: "gw-old",
        kind: "remote",
        backends: [{ descriptor: "old-desc", profileLabel: null }],
      },
    ],
...
test("registry sources: a dead gateway contributes nothing instead of breaking the list", async () => {
  const rows = await fetchRegistrySessionRows(
    [
      {
        connectionId: "gw-dead",
        kind: "ssh",
        backends: [{ descriptor: "dead", profileLabel: "x" }],
      },
      {
        connectionId: "gw-live",
        kind: "ssh",
        backends: [{ descriptor: "live", profileLabel: "y" }],
      },
    ],
...
test("remote owner lookup matches a compression lineage root id too", async () => {
  const owner = await findRemoteOwnerProfileForSession(
    ...
    async () => {
      return {
        sessions: [{ id: "tip-2", _lineage_root_id: "root-1" }],
      } as never;
    },
  );
...
test("remote owner lookup returns null when no remote lists the id or remotes fail", async () => {
  const missing = await findRemoteOwnerProfileForSession(
    "sess-x",
    ["vps-a"],
    async () => {
      return { sessions: [{ id: "other" }] } as never;
  ...
  const dead = await findRemoteOwnerProfileForSession(
    "sess-x",
    ["vps-a"],
    async () => {
      throw new Error("remote unavailable");
  ...
  const noRemotes = await findRemoteOwnerProfileForSession(
    "sess-x",
    [],
    async () => {
