...
test("ensureGitRepo: inits a plain dir with a root commit so worktrees branch", async () => {
  ...
  try {
    ...
    execFileSync(
      "git",
      ["worktree", "add", "-b", "wt", path.join(dir, ".worktrees", "wt")],
      { cwd: dir },
    );
...
test("listBranches: lists locals and flags the checked-out branch", async () => {
  ...
  try {
    ...
    assert.equal(
      fs.realpathSync(branches.find((b) => b.name === current).worktreePath),
      fs.realpathSync(dir),
    );
...
test("listBranches: flags a free default branch as default, not checked out", async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), "hermes-branches-default-"),
  );
  const git = (...args) =>
    execFileSync("git", args, { cwd: dir }).toString().trim();
...
test("addWorktree: existingBranch checks the branch out without a new branch", async () => {
  ...
  try {
    ...
    assert.equal(
      execFileSync("git", ["branch", "--show-current"], { cwd: result.path })
        .toString()
        .trim(),
      "cool/feature",
    );
...
test("addWorktree: base param branches off a specified local branch", async () => {
  ...
  try {
    ...
    assert.equal(
      git("-C", result.path, "merge-base", "HEAD", "staging").length > 0,
      true,
    );
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
...
test("addWorktree: base origin/main does not set up upstream tracking", async () => {
  ...
  const git = (...args) =>
    execFileSync("git", args, { cwd: cloneDir }).toString().trim();
...
function seedRemoteAndClone(label, branches) {
  const remoteDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `hermes-${label}-remote-`),
  );
  const cloneDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `hermes-${label}-clone-`),
  );
...
test("listBranches: offers remote branches that have no local counterpart", async () => {
  const { cloneDir, remoteDir } = seedRemoteAndClone("branches-remote", [
    "teammate-work",
  ]);
...
test("addWorktree: a remote branch becomes a local branch tracking it", async () => {
  const { cloneDir, remoteDir } = seedRemoteAndClone("convert-remote", [
    "teammate-work",
  ]);
  ...
  try {
    const result = await addWorktree(
      cloneDir,
      { existingBranch: "origin/teammate-work" },
      "git",
    );
    ...
    // setup.
    assert.equal(
      inTree("rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}"),
      "origin/teammate-work",
    );
  } finally {
    fs.rmSync(remoteDir, { recursive: true, force: true });
...
test("addWorktree: a remote default branch gets its own worktree, not a home switch", async () => {
  const { cloneDir, remoteDir } = seedRemoteAndClone(
    "convert-remote-default",
    [],
  );
