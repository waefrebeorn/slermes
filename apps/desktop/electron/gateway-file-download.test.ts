...
test("downloadTempPath stays beside the destination with a short, random per-call name", () => {
  ...
  assert.notEqual(
    a,
    b,
    "two concurrent saves into the same directory must not share a temp file",
  );
  ...
  assert.equal(
    path.normalize(path.dirname(downloadTempPath(longName))),
    path.normalize(path.dirname(longName)),
  );
...
test("pumpStreamToFile applies backpressure: pauses on a full buffer and resumes on drain", async () => {
  ...
  assert.equal(
    res.paused,
    true,
    "source should be paused when write() returns false",
  );
  ...
  assert.equal(
    res.resumed,
    true,
    "source should resume after the write stream drains",
  );
...
test("pumpStreamToFile removes only the temp file and rejects on a write error", async () => {
  ...
  assert.deepEqual(
    renamed,
    [],
    "a failed body must never be moved onto the destination",
  );
  assert.equal(
    res.destroyed,
    true,
    "source should be torn down on write failure",
  );
});
...
test("pumpStreamToFile never unlinks a temp path it did not create when the exclusive open fails", async () => {
  ...
  const eexist: any = new Error(
    "EEXIST: file already exists, open '/tmp/.hermes-download-deadbeef.part'",
  );
...
test("writeBufferToFile leaves the destination untouched when the write fails after open", async () => {
  ...
  await assert.rejects(
    writeBufferToFile(Buffer.from("whole body"), "/tmp/out.bin", deps),
    /disk full/,
  );
  ...
  assert.deepEqual(
    unlinked,
    [opened[0]],
    "only the owned temp file is removed",
  );
  assert.deepEqual(renamed, []);
});
...
test("pumpStreamToFile leaves a pre-existing destination untouched when the response fails mid-stream", async () => {
  ...
  await assert.rejects(promise, /socket hang up/);
  assert.ok(
    !opened.includes("/tmp/out.bin"),
    "destination must not be opened (and truncated) before the body lands",
  );
  assert.ok(
    !unlinked.includes("/tmp/out.bin"),
    "destination must not be removed on failure",
  );
  assert.deepEqual(unlinked, [opened[0]]);
  assert.deepEqual(renamed, []);
});
...
test("pumpStreamToFile removes the temp file and rejects when the final rename fails", async () => {
  ...
  const ws = new FakeWriteStream();
  const { deps, opened, unlinked } = recordingDeps(ws, {
    renameError: new Error("EPERM: destination locked"),
  });
...
test("parseDataUrlToBuffer throws on a malformed data URL", () => {
  assert.throws(
    () => parseDataUrlToBuffer("not-a-data-url"),
    /Malformed data URL/,
  );
});
...
test("filenameFromContentDisposition prefers filename* and reduces to a basename", () => {
  assert.equal(
    filenameFromContentDisposition(
      "attachment; filename*=UTF-8''report%20with%20spaces.pdf",
    ),
...
test("gatewayFilePath normalizes bare paths and file:// URLs", () => {
  ...
  assert.equal(
    gatewayFilePath("file:///Users/me/a%20b.md"),
    "/Users/me/a b.md",
  );
...
test("gatewayFileRequestPaths keeps streaming and fallback requests on the same registered backend", () => {
  const paths = gatewayFileRequestPaths(
    ...
    (requestPath) =>
      pathForRegistryBackendRequest(requestPath, "research", {
        sharedRemote: true,
      }),
  );
  ...
  assert.deepEqual(paths, {
    dataUrl:
      "/api/fs/read-data-url?path=%2Fsrv%2Foutput%2Fimage+one.png&profile=research",
    download:
      "/api/fs/download?path=%2Fsrv%2Foutput%2Fimage+one.png&profile=research",
  });
});
