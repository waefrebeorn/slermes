...
function createOwnership(
  store = memoryStore(),
  overrides: Partial<Parameters<typeof createBackendOwnership>[0]> = {},
) {
...
test("incomplete claims and persisted records are rejected", async () => {
  ...
  await assert.rejects(
    ownership.claim({ ...ownershipEntry(), startMarker: "" }),
    /complete process identity/,
  );
...
test("startup reap still reaps a backend whose parent is gone or reused", async () => {
  const gone = {
    ...ownershipEntry({ pid: 55 }),
    parentPid: 200,
    parentStartMarker: "os-start-dead",
  };
  const reused = {
    ...ownershipEntry({ pid: 56 }),
    parentPid: 201,
    parentStartMarker: "os-start-old",
  };
...
test("claim persists the parent identity so a later reap can see it", async () => {
  ...
  assert.deepEqual(
    parseBackendOwnership(store.value())[0].parentStartMarker,
    "os-start-parent",
  );
});
...
test("backend identity check matches only serve and dashboard invocation shapes", () => {
  ...
  assert.equal(
    backendCommandMatches("python -m hermes_cli.main dashboard --no-open"),
    true,
  );
  assert.equal(
    backendCommandMatches("/venv/bin/hermes --profile work serve --port 0"),
    true,
  );
  assert.equal(
    backendCommandMatches(
      '"C:\\Hermes Runtime\\hermes.exe" dashboard --no-open',
    ),
    true,
  );
