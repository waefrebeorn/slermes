...
test("consumes and returns a fresh failure result", () => {
  ...
  assert.equal(
    fs.existsSync(handoffResultPath(home)),
    false,
    "result file must be consumed",
  );
});
...
test("manual flag survives the round trip and defaults false", () => {
  ...
  write(home, {
    ...
    manual: true,
    message:
      "Update complete. Reopen Hermes to finish (it could not restart itself).",
    branch: "main",
    finished_at: Math.floor(Date.now() / 1000),
  });
...
test("an old manual result survives the freshness window but an old ordinary one does not", () => {
  ...
  assert.equal(
    readAndConsumeHandoffResult(ordinary),
    null,
    "a stale ordinary result is discarded",
  );
  assert.equal(
    fs.existsSync(handoffResultPath(ordinary)),
    false,
    "and still consumed",
  );
  ...
  assert.ok(
    result,
    "a stale manual result is still surfaced — it is the last-resort channel",
  );
  assert.equal(result.manual, true);
