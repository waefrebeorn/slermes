...
test("cancelling an old same-key request never rejects a newer waiter", async () => {
  ...
  assert.equal(
    ...
    1,
    "the newer same-key waiter must remain queued",
  );
...
test("a queued start times out with a clear error and frees its queue position", async () => {
  ...
  await assert.rejects(
    queued.acquired,
    /timed out while waiting for a free slot/,
  );
  assert.equal(coordinator.activeCount, 1);
  assert.equal(coordinator.queuedCount, 0);
...
test("lowering the limit never revokes granted slots; new requests queue until under cap", async () => {
  ...
  const releases = await Promise.all(
    ["a", "b", "c"].map((key) => coordinator.acquire(key)),
  );
...
// fast with a reason.
{
  ...
  const mainSource = fs
    .readFileSync(path.join(here, "main.ts"), "utf8")
    .replace(/\r\n/g, "\n");

  const withTimeoutSource = fs
    .readFileSync(
      path.join(here, "..", "src", "lib", "with-timeout.ts"),
      "utf8",
    )
    .replace(/\r\n/g, "\n");

  test("main.ts bounds the slot wait below the renderer backend-boot budget", () => {
    const slotWait = Number(
      /const POOL_SLOT_WAIT_MS = ([\d_]+)/
        .exec(mainSource)?.[1]
        ?.replace(/_/g, ""),
    );

    const bootBudget = Number(
      /export const BACKEND_BOOT_WAIT_TIMEOUT_MS = ([\d_]+)/
        .exec(withTimeoutSource)?.[1]
        ?.replace(/_/g, ""),
    );

    assert.ok(
      Number.isFinite(slotWait) && slotWait > 0,
      "POOL_SLOT_WAIT_MS must be a literal in main.ts",
    );
    assert.ok(
      Number.isFinite(bootBudget),
      "BACKEND_BOOT_WAIT_TIMEOUT_MS must be a literal",
    );
    assert.ok(
      slotWait < bootBudget,
      `slot wait ${slotWait}ms must be below the boot budget ${bootBudget}ms`,
    );
    assert.match(
      mainSource,
      /localBackendSpawnCoordinator\.request\(poolKey, \{ timeoutMs: POOL_SLOT_WAIT_MS \}\)/,
    );
    assert.doesNotMatch(
      mainSource,
      /request\(poolKey, \{ timeoutMs: POOL_IDLE_MS \}\)/,
    );
  });
  ...
  test("main.ts pushes the live pool max into the coordinator when the preference changes", () => {
    ...
    assert.match(
      mainSource,
      /new LocalBackendSpawnCoordinator\(poolLimits\.maxBackends\)/,
    );
    assert.match(
      mainSource,
