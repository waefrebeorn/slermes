...
test("registry SSH fingerprint failures name the connection and ssh -G step", async () => {
  ...
  const source = registry.connections.find(
    (connection) => connection.id === registry.primary,
  )!;
...
test("registry primary reuses a matching primary backend descriptor", () => {
  ...
  assert.equal(
    registrySourceOwnsPrimaryBackend(registry, "hermes-vps", descriptor),
    true,
  );
  assert.equal(
    registrySourceOwnsPrimaryBackend(registry, LOCAL_CONNECTION_ID, descriptor),
    false,
  );
});
...
test("resolvedConnectionId identifies local and migrated remote descriptors", () => {
  const registry = migrateV1ToRegistry({
    ...
    profiles: {
      personal: {
        mode: "remote",
        url: "https://personal.example:9443/",
        authMode: "token",
      },
  ...
  const personal = registry.connections.find(
    (connection) => connection.kind === "remote",
  );
  const work = registry.connections.find(
    (connection) => connection.kind === "ssh",
  );

  assert.equal(
    resolvedConnectionId(registry, { mode: "local" }),
    LOCAL_CONNECTION_ID,
  );
  assert.equal(
...
test("agentHandle bare when unique, @name-device shape when duplicated", () => {
  ...
  assert.equal(
    agentHandle("research", "Work Laptop", true),
    "research-work-laptop",
  );
...
test("resolvedConnectionId accepts only a current exact descriptor id and never falls back", () => {
  const registry: ConnectionRegistry = {
    ...
    connections: [
      { id: LOCAL_CONNECTION_ID, kind: "local", label: "This device" },
      {
        ...
        headers: {
          "CF-Access-Client-Id": { encoding: "safeStorage", value: "header-a" },
        },
      },
      {
        id: "remote-b",
        ...
        authMode: "oauth",
        headers: {
          "CF-Access-Client-Id": { encoding: "safeStorage", value: "header-b" },
        },
      },
    ],
  };
  ...
  for (const connectionId of [
    ...
    assert.equal(
      resolvedConnectionId(registry, { ...inferableRemoteA, connectionId }),
      null,
    );
  }
  ...
  const reordered = {
    ...registry,
    connections: [...registry.connections].reverse(),
  };
    ...
    connections: [
      ...registry.connections,
      {
        ...registry.connections.find(
          (connection) => connection.id === "remote-token",
        )!,
        id: "remote-token-copy",
      },
    ],
  };
...
test("resolvedConnectionId keeps same-host SSH routes distinct by port, key, path, and profile", () => {
  ...
  const resolve = (
    ssh: NonNullable<Parameters<typeof resolvedConnectionId>[1]["ssh"]>,
  ) =>
  ...
  assert.equal(
    resolve({ ...base, remoteHermesPath: "/opt/hermes" }),
    "ssh-path",
  );
...
test('connectionIdForLabel suffixes on collision and never mints "local"', () => {
  ...
  assert.equal(
    connectionIdForLabel("Homelab", ["homelab", "homelab-2"]),
    "homelab-3",
  );
...
test("backendScopeKey: electron and shared implementations agree everywhere", async () => {
  ...
  for (const [conn, profile] of cases) {
    assert.equal(
      backendScopeKey(conn, profile),
      shared.backendScopeKey(conn, profile),
    );
  }

  assert.equal(
    backendScopePrefix("homelab"),
    shared.backendScopePrefix("homelab"),
  );
...
test("backendScopeKey: non-local connections get an unambiguous composite", () => {
  assert.equal(
    backendScopeKey("homelab", "research"),
  ...
  assert.ok(
    backendScopeKey("homelab", "research").startsWith(
      backendScopePrefix("homelab"),
    ),
  );
  assert.ok(
    !backendScopeKey("homelab-2", "research").startsWith(
      backendScopePrefix("homelab"),
    ),
  );
...
test("registry local route: delegates to the legacy path when v1 is local (single-source users byte-identical)", () => {
  assert.deepEqual(resolveRegistryLocalRoute("research", {}), {
    delegate: true,
    poolKey: "research",
  });
  assert.deepEqual(resolveRegistryLocalRoute("", {}), {
    delegate: true,
    poolKey: "default",
  });
  assert.deepEqual(resolveRegistryLocalRoute(null, { globalRemote: false }), {
    delegate: true,
    poolKey: "default",
  });
});
...
test("registry local route: v1 REMOTE global mode forces a genuinely-local backend (migration scenario)", () => {
  ...
  assert.notEqual(
    route.poolKey,
    backendScopeKey(LOCAL_CONNECTION_ID, "default"),
  );
});
...
test("local enumeration: delegate route (local-primary desktop) always enumerates", () => {
  ...
  assert.equal(
    shouldDeferLocalEnumeration(route, ["conn:local::default"]),
    false,
  );
});
...
test("local enumeration: forced-local route defers until a local child exists (remote-primary desktop)", () => {
  ...
  assert.equal(
    shouldDeferLocalEnumeration(route, ["default", "research"]),
    true,
  );
...
test("roster: unique profiles keep bare handles; duplicates get @name-device", () => {
  ...
  const byKey = new Map(
    roster.map((a) => [`${a.connectionId}/${a.profile}`, a.handle]),
  );
...
test("roster: source profile metadata follows the connection-qualified row", () => {
  ...
  const roster = buildAgentRoster([
    { connection: local, profiles: ["default"] },
    {
      connection: vps,
      profiles: ["default"],
      profileMetadata: { default: vpsMeta },
    },
  ]);

  assert.deepEqual(
    roster.find((agent) => agent.connectionId === "vps")?.profileMetadata,
    vpsMeta,
  );
  assert.equal(
    roster.find((agent) => agent.connectionId === "local")?.profileMetadata,
    undefined,
  );
});
...
test("rememberSshEnumeration: live list wins, cache then seed default", () => {
  assert.deepEqual(
    rememberSshEnumeration({ profiles: ["bob", "kai"] }, ["stale"], "ssh"),
    {
      profiles: ["bob", "kai"],
    },
  );
...
test("rememberSshEnumeration: a bounced remote source keeps its last-known roster (4-bots-show-as-2)", () => {
  ...
  assert.deepEqual(
    rememberSshEnumeration(
      { profiles: null, error: "unreachable" },
      ["default", "ceo", "accounter"],
      "remote",
    ),
...
test("roster: duplicate profiles from one connection remain one routable agent", () => {
  ...
  assert.equal(
    roster.find(
      (agent) => agent.connectionId === "local" && agent.profile === "research",
    )?.handle,
  ...
  assert.equal(
    roster.find(
      (agent) =>
        agent.connectionId === "homelab" && agent.profile === "research",
    )?.handle,
    "research-homelab",
  );
});
...
test("roster: two connections with the same install_id collapse to one row per profile", () => {
  ...
  assert.deepEqual(roster.map((agent) => agent.handle).sort(), [
    "default",
    "research",
  ]);
});
...
test("roster: missing install_id bypasses the collapse (older backends keep current behavior)", () => {
  ...
  assert.deepEqual(roster.map((a) => a.handle).sort(), [
    "default-spark",
    "default-spark-ts",
  ]);
...
test("update fan-out: cloud is platform-managed, everything else eligible", () => {
  assert.deepEqual(
    updateEligibility({ id: "c", kind: "cloud", label: "Cloud" }),
    {
      eligible: false,
  ...
  assert.equal(
    updateEligibility({ id: "local", kind: "local", label: "x" }).eligible,
    true,
  );
  assert.equal(
    updateEligibility({ id: "r", kind: "remote", label: "x" }).eligible,
    true,
  );
  assert.equal(
    updateEligibility({ id: "s", kind: "ssh", label: "x" }).eligible,
    true,
  );
});
...
test('save rejects the reserved "local" id on non-local kinds', () => {
  assert.throws(
    () =>
      normalizeConnectionInput(
        { id: "local", kind: "remote", label: "Sneaky", url: "http://x:1" },
        emptyRegistry(),
      ),
    /reserved/,
  );
...
test("merge preserves fields the editor does not carry (org, ssh extras)", () => {
  ...
  const renamed = mergeConnectionInput(
    { id: "c", kind: "cloud", label: "Renamed", url: "https://a.cloud" },
    cloud,
  );
  ...
  const labelOnly = mergeConnectionInput(
    { id: "s", kind: "ssh", label: "Renamed box" },
    ssh,
  );
...
test("merge: a supplied ssh host string beats stored user/port", () => {
  ...
  const merged = mergeConnectionInput(
    { host: "admin@newbox:2200", id: "s", kind: "ssh", label: "Spark" },
    ssh,
  );
...
test("save rejects a missing label with a device-name message", () => {
  assert.throws(
    () =>
      normalizeConnectionInput(
        { kind: "remote", label: "  ", url: "http://10.0.0.5:9119" },
        emptyRegistry(),
      ),
    /device name/,
  );
...
test("save rejects a duplicate label case-insensitively", () => {
  ...
  registry = upsertConnection(
    registry,
    normalizeConnectionInput(
      { kind: "remote", label: "Homelab", url: "http://10.0.0.5:9119" },
      registry,
    ),
  );
  ...
  assert.throws(
    () =>
      normalizeConnectionInput(
        { kind: "remote", label: " homelab ", url: "http://10.0.0.9:9119" },
        registry,
      ),
    /must be unique/,
  );
...
test("duplicate gateway URLs are rejected across remote and cloud kinds", () => {
  ...
  assert.throws(
    () =>
      normalizeConnectionInput(
        { kind: "remote", label: "Twin", url: "http://10.0.0.5:9119/" },
        registry,
      ),
    /already exists/,
  );
  assert.throws(
    () =>
      normalizeConnectionInput(
        { kind: "cloud", label: "Cloud twin", url: "http://10.0.0.5:9119" },
        registry,
      ),
    /already exists/,
  );
...
test("ssh input requires a host; local input only carries the label", () => {
  ...
  const ssh = normalizeConnectionInput(
    { kind: "ssh", label: "Spark", host: "tek@spark1:2222" },
    registry,
  );
  ...
  const local = normalizeConnectionInput(
    { kind: "local", label: "My MacBook" },
    registry,
  );
...
test("normalizeRegistry guarantees local, dedupes labels, fixes dangling primary", () => {
  ...
  assert.equal(
    registry.connections.filter((c) => c.kind === "remote").length,
    2,
  );
});
...
test("migrate: per-profile overrides become extra sources, deduped by URL", () => {
  const registry = migrateV1ToRegistry({
    ...
    profiles: {
      ...
      plain: {
        mode: "local",
        savedSsh: { mode: "ssh", host: "spark1", user: "tek" },
      },
    },
  });
...
test("removeConnection: local refuses, primary and last-used retarget safely", () => {
  ...
  assert.throws(
    () => removeConnection(registry, LOCAL_CONNECTION_ID),
    /cannot be removed/,
  );
...
test("setPrimaryConnection validates the target id", () => {
  ...
  assert.equal(
    setPrimaryConnection(registry, LOCAL_CONNECTION_ID).primary,
    LOCAL_CONNECTION_ID,
  );
});
...
test("last-used source and launch mode validate their persisted values", () => {
  ...
  assert.throws(
    () => setLastUsedConnection(registry, "ghost"),
    /No connection/,
  );
  assert.equal(setLastUsedConnection(registry, entry.id).lastUsed, entry.id);
  assert.equal(
    setConnectionLaunchMode(registry, "last-used").launchMode,
    "last-used",
  );
  assert.throws(
    () => setConnectionLaunchMode(registry, "sometimes"),
    /Unknown connection launch mode/,
  );
});

test("upsertConnection replaces by id and appends new ids", () => {
  let registry = emptyRegistry();
  const a = normalizeConnectionInput(
    { kind: "remote", label: "A", url: "http://a:1" },
    registry,
  );
  ...
  assert.equal(
    registry.connections.find((c) => c.id === a.id)?.url,
    "http://a:2",
  );
});
...
test("Apply remote preserves an existing URL identity and label without duplicates", () => {
  ...
  const matches = applied.connections.filter(
    (connection) => connection.url === "https://gateway.example.com",
  );
...
test("Apply local moves primary/current to This device without deleting registered remotes", () => {
  ...
  const localRegistry = reconcileAppliedGlobalConnection(remoteRegistry, {
    mode: "local",
    remote: {},
  });
  ...
  assert.equal(
    localRegistry.connections.filter(
      (connection) => connection.kind === "remote",
    ).length,
    1,
  );
  assert.equal(
    resolvedConnectionId(localRegistry, { mode: "local" }),
    LOCAL_CONNECTION_ID,
  );
});
...
test("Apply between two remotes keeps each real registration once and activates the latest", () => {
  ...
  const remotes = second.connections.filter(
    (connection) => connection.kind === "remote",
  );
  ...
  assert.equal(
    second.primary,
    remotes.find((connection) => connection.url === "https://two.example.com")
      ?.id,
  );
...
test("drift heal registers a v1 remote the registry never learned about and makes it primary", () => {
  ...
  const remote = drifted.registry.connections.find(
    (connection) => connection.kind === "remote",
  );
...
test("drift heal ignores local and unparseable v1 routes", () => {
  ...
  for (const v1 of [
    ...
    assert.equal(
      drifted.changed,
      false,
      `expected no heal for ${JSON.stringify(v1)}`,
    );
...
test("drift heal registers a v1 SSH route the registry never learned about and makes it primary", () => {
  ...
  const ssh = drifted.registry.connections.find(
    (connection) => connection.kind === "ssh",
  );
...
test("drift heal adds the missing SSH source without disturbing other registered sources", () => {
  ...
  assert.equal(drifted.changed, true);
  assert.ok(
    drifted.registry.connections.some(
      (connection) => connection.id === "homelab",
    ),
  );
  assert.ok(
    drifted.registry.connections.some(
...
test("drift heal adds the missing remote without disturbing other registered sources", () => {
  ...
  assert.equal(drifted.changed, true);
  assert.equal(
    drifted.registry.connections.filter(
      (connection) => connection.kind === "remote",
    ).length,
    2,
  );
...
test("connectionDialFieldsChanged: url / auth / token changes recycle", () => {
  ...
  assert.equal(
    connectionDialFieldsChanged(before, {
      ...before,
      token: { encoding: "safeStorage", value: "NEW" },
    }),
    true,
  );
});
...
test("connectionDialFieldsChanged: ssh routing fields recycle, kind change recycles", () => {
  ...
  assert.equal(
    connectionDialFieldsChanged(before, { ...before, label: "Box 2" }),
    false,
  );
  assert.equal(
    connectionDialFieldsChanged(before, { ...before, host: "other.lan" }),
    true,
  );
  assert.equal(
    connectionDialFieldsChanged(before, { ...before, port: 2222 }),
    true,
  );
  assert.equal(
    connectionDialFieldsChanged(before, { ...before, remoteProfile: "work" }),
    true,
  );
  assert.equal(
    connectionDialFieldsChanged(before, {
...
test("mergeConnectionInput inherits stored headers when the editor payload omits them", () => {
  ...
  const renamed = mergeConnectionInput(
    { id: "cf", kind: "remote", label: "Renamed" },
    stored,
  );
  ...
  const cleared = mergeConnectionInput(
    { id: "cf", kind: "remote", label: "CF box", headers: {} },
    stored,
  );
...
test("connectionDialFieldsChanged: a header change recycles live backends", () => {
  ...
  assert.equal(
    connectionDialFieldsChanged(before, {
      ...before,
      headers: {
        "CF-Access-Client-Id": { encoding: "safeStorage", value: "OTHER" },
      },
    }),
    true,
  );
  assert.equal(
    connectionDialFieldsChanged(before, { ...before, headers: undefined }),
    true,
  );
});
...
test("normalizeRegistry quarantines malformed entries instead of silently dropping them", () => {
  ...
  assert.deepEqual(reasons, [
    "entry-missing-ssh-host",
    "entry-missing-url",
    "entry-unrecognized-kind",
  ]);

  const mystery = registry.quarantined!.find(
    (q) => q.reason === "entry-unrecognized-kind",
  );

  assert.deepEqual(mystery!.entry, {
    kind: "nonsense",
    label: "Mystery box",
    extra: "still my data",
  });
...
test("normalizeRegistry preserves previously quarantined entries across round trips", () => {
  ...
  assert.deepEqual(second.quarantined![0].entry, {
    id: "c",
    kind: "remote",
    label: "No URL entry",
  });
...
test("normalizeRegistry quarantines non-object junk items that could still be user data", () => {
  const registry = normalizeRegistry({
    version: 2,
    connections: [
      "{ mangled json fragment }",
      null,
      false,
      { id: "a", kind: "remote", label: "A", url: "http://x:1" },
    ],
  });
