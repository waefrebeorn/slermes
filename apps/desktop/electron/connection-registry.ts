 ...
 */
export function backendScopeKey(
 ...
 * would use.
 */
export function parseBackendScopeKey(key: string): {
...
export function resolvedConnectionId(
  ...
  if (Object.prototype.hasOwnProperty.call(descriptor, "connectionId")) {
    ...
    return registry.connections.some(
  ...
  if (descriptor.mode === "local") {
    const localConnections = registry.connections.filter(
  ...
  if (descriptor.remoteKind === "ssh") {
    ...
    const coarseMatches = registry.connections.filter(
      (connection) =>
        connection.kind === "ssh" && normalizedSshTarget(connection) === target,
    );
    ...
    return (
      matchingConnectionId(registry, { kind: "ssh", ...ssh }, "unique") ?? null
    );
  }

  const kind =
    descriptor.remoteKind === "cloud"
      ? "cloud"
  ...
  const hasExactEnvelope =
    ...
    (authMode === "oauth" ||
      Object.prototype.hasOwnProperty.call(descriptor, "token")) &&
    (kind === "remote" ||
      Object.prototype.hasOwnProperty.call(descriptor, "org"));
...
export async function reuseMatchingPrimarySshBackend({
  ...
  const activeSsh =
    descriptor.mode === "remote" && descriptor.remoteKind === "ssh"
      ? descriptor.ssh
      : null;
  const rootProfile = (value: unknown) =>
  ...
  if (
    ...
    !activeSsh ||
    sourceFingerprint !==
      String(activeSsh.effectiveConfigFingerprint || "").trim() ||
    String(source.remoteHermesPath || "").trim() !==
      String(activeSsh.remoteHermesPath || "").trim() ||
...
export function registrySourceOwnsPrimaryBackend(
  ...
  return (
    Boolean(id) &&
    id === registry.primary &&
    resolvedConnectionId(registry, descriptor) === id
  );
}
...
export function resolveRegistryLocalRoute(
  ...
  if (opts.globalRemote) {
    return {
      delegate: false,
      poolKey: `${backendScopePrefix(LOCAL_CONNECTION_ID)}${profileKey}`,
    };
  }
...
export function buildAgentRoster(
  ...
  for (const {
    ...
    if (group) {
      ...
      backends.set(key, [
        { connection, order: rank, profile, profileMetadata },
      ]);
    }
  }
...
function pickCanonicalConnection<
  T extends { connection: RegistryConnection; order: number },
...
export function connectionIdForLabel(
  label: string,
  taken: Iterable<string>,
): string {
...
export function normalizeConnectionInput(
  ...
  if (label.length > LABEL_MAX) {
    throw new Error(
      `Connection name is too long (max ${LABEL_MAX} characters).`,
    );
  }

  const key = labelKey(label);
  const collision = registry.connections.find(
    (c) => labelKey(c.label) === key && c.id !== input.id,
  );

  if (collision) {
    throw new Error(
      `A connection named "${collision.label}" already exists. Connection names must be unique.`,
    );
  }
  ...
  if (kind === "ssh") {
    ...
    const sshDupe = registry.connections.find(
      (c) => c.kind === "ssh" && c.id !== id && sshKey(c) === sshKey(sshFields),
    );

    if (sshDupe) {
      throw new Error(
        `A connection to this SSH host already exists ("${sshDupe.label}").`,
      );
    }
  ...
  if (kind === "remote" || kind === "cloud") {
    ...
    const urlDupe = registry.connections.find(
      (c) =>
        ...
        c.id !== id &&
        urlKey(c.url || "") === urlKey(url),
    );

    if (urlDupe) {
      throw new Error(
        `A connection to this gateway URL already exists ("${urlDupe.label}").`,
      );
    }
...
export function mergeConnectionInput(
  input: ConnectionInput,
  existing?: null | RegistryConnection,
): ConnectionInput {
 ...
 */
export function connectionDialFieldsChanged(
  before: RegistryConnection,
  after: RegistryConnection,
): boolean {
  ...
  if (
    JSON.stringify(before.token ?? null) !== JSON.stringify(after.token ?? null)
  ) {
    return true;
  }
  ...
  return (
    JSON.stringify(before.headers ?? null) !==
    JSON.stringify(after.headers ?? null)
  );
}
...
export function normalizeRegistry(raw: unknown): ConnectionRegistry {
  ...
  if (Array.isArray(parsed.quarantined)) {
    for (const item of parsed.quarantined) {
      if (
        item &&
        typeof item === "object" &&
        "entry" in (item as Record<string, unknown>)
      ) {
        quarantine(
  ...
  for (const item of rawConnections) {
    ...
    try {
      ...
      let id =
        kind === "local" ? LOCAL_CONNECTION_ID : String(entry.id || "").trim();
  ...
  const primary = connections.some((c) => c.id === storedPrimary)
    ? storedPrimary
    : LOCAL_CONNECTION_ID;
  ...
  const normalized: ConnectionRegistry = {
    ...
    lastUsed: connections.some((c) => c.id === storedLastUsed)
      ? storedLastUsed
      : primary,
    connections,
  };
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  const config =
    v1 && typeof v1 === "object" ? (v1 as Record<string, any>) : {};
  ...
  const addRemoteLike = (
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) ||
        (kind === "cloud" ? "Hermes Cloud" : "Remote gateway"),
  ...
  if (modeIsRemoteLike(globalMode)) {
    const entry = addRemoteLike(
      config.remote || {},
      globalMode === "cloud" ? "cloud" : "remote",
    );
...
export function removeConnection(
  ...
  const primary =
    registry.primary === id ? LOCAL_CONNECTION_ID : registry.primary;
...
export function setPrimaryConnection(
  registry: ConnectionRegistry,
  id: string,
): ConnectionRegistry {
...
export function setLastUsedConnection(
  registry: ConnectionRegistry,
  id: string,
): ConnectionRegistry {
  if (!registry.connections.some((c) => c.id === id)) {
...
export function reconcileRegistryDrift(
  ...
  return {
    changed: true,
    registry: reconcileAppliedGlobalConnection(registry, config),
  };
}

/** Choose whether launch restores the explicit primary or the last-used source. */
export function setConnectionLaunchMode(
  registry: ConnectionRegistry,
  launchMode: string,
): ConnectionRegistry {
