...
interface RegistryGatewayWsUrlDependencies {
  ensureBackend: (
    ...
    profile: unknown,
  ) => Promise<RegistryGatewayWsConnection>;
  mintTicket: (
    baseUrl: string,
    headers?: Record<string, string>,
  ) => Promise<string>;
...
type RemoteRequestCallback = (result: {
  requestHeaders?: Record<string, string>;
}) => void;
...
export function createRegistryGatewayWsUrlHandler(
  dependencies: RegistryGatewayWsUrlDependencies,
) {
  return async (payload: unknown): Promise<string> => {
    ...
    if (connection.authMode === "oauth") {
      const ticket = await dependencies.mintTicket(
        connection.baseUrl,
        connection.headers,
      );
