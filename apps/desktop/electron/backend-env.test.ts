...
test("desktop backend PATH adds Hermes-managed bins and missing POSIX sane entries", () => {
  ...
  assert.ok(
    entries.includes("/opt/homebrew/bin"),
    "Apple Silicon Homebrew bin is added",
  );
  assert.ok(
    entries.includes("/opt/homebrew/sbin"),
    "Apple Silicon Homebrew sbin is added",
  );
  assert.ok(
    entries.includes("/usr/local/sbin"),
    "missing standard sbin is added",
  );
...
test("managed Node dirs lead with the platform-native layout but always offer both", () => {
  ...
  const windows = hermesManagedNodePathEntries(
...
test("managed Node dirs are empty without a Hermes home", () => {
  assert.deepEqual(
    hermesManagedNodePathEntries(undefined, {
      platform: "darwin",
      pathModule: path.posix,
    }),
    [],
...
test("every managed Node dir outranks the inherited PATH on both platforms", () => {
  for (const [platform, pathModule, home, inherited, delimiter] of [
    ...
    const managed = hermesManagedNodePathEntries(home, {
      platform,
      pathModule,
    });
    const firstInherited = Math.min(
      ...inherited.split(delimiter).map((entry) => entries.indexOf(entry)),
    );
...
test("desktop backend PATH preserves first occurrence and avoids duplicates", () => {
  ...
  const entries = result.split(":");
  assert.equal(
    entries.filter((entry) => entry === "/opt/homebrew/bin").length,
    1,
  );
  assert.ok(
    entries.indexOf("/opt/homebrew/bin") <
      entries.indexOf("/opt/homebrew/sbin"),
...
test("normalizeHermesHomeRoot maps profile homes back to the global Hermes root", () => {
  assert.equal(
    normalizeHermesHomeRoot("/Users/test/.hermes/profiles/oracle", {
      pathModule: path.posix,
    }),
    "/Users/test/.hermes",
  );
  assert.equal(
    normalizeHermesHomeRoot(
      "C:\\Users\\test\\AppData\\Local\\hermes\\profiles\\oracle",
      { pathModule: path.win32 },
    ),
    "C:\\Users\\test\\AppData\\Local\\hermes",
  );
