...
export async function resolveGatewayFileBackend<T>(
  ...
  const connection = connectionId
    ? await deps.ensureRegistry(connectionId, profile)
    : await deps.ensureLegacy(profile);
...
export function downloadTempPath(destPath: string): string {
  return path.join(
    ...
    `.hermes-download-${crypto.randomBytes(4).toString("hex")}.part`,
  );
}
...
export function pumpStreamToFile(
  res: ReadableLike,
  destPath: string,
  deps: PumpDeps,
): Promise<void> {
...
export function isNotFoundError(error: unknown): boolean {
  return (
    Boolean(error) && (error as { statusCode?: number }).statusCode === 404
  );
}
