...
test("dead pid => no live update and marker is pruned", () => {
  ...
  assert.ok(
    ...
    "a dead-pid marker self-heals (deleted)",
  );
});
...
test("expired marker (past age ceiling) => no live update and pruned", () => {
  ...
  assert.equal(
    readLiveUpdateMarker(home, { kill: ALIVE, now: () => now }),
    null,
  );
  assert.ok(
    !fs.existsSync(markerPath(home)),
    "an expired marker self-heals (deleted)",
  );
});
...
test("writeUpdateMarker writes a marker that readLiveUpdateMarker accepts", () => {
  ...
  assert.ok(
    res,
    "marker written by writeUpdateMarker should be detected as live",
  );
  assert.equal(res.pid, 4242);
  assert.ok(
    fs.existsSync(markerPath(home)),
...
test("writeUpdateMarker preserves a live holder age across pid hand-off", () => {
  ...
  const [pidLine, startedLine] = fs
    .readFileSync(markerPath(home), "utf8")
    .split("\n");
  assert.equal(
    Number.parseInt(pidLine, 10),
    2020,
    "the hand-off records the new owner",
  );
  assert.equal(
    Number.parseInt(startedLine, 10),
    startedAt,
    "the holder age must not restart during hand-off",
  );
});
...
test("writeUpdateMarker is best-effort (no throw on bad path)", () => {
  ...
  const badHome = path.join(
    os.tmpdir(),
    "hermes-marker-nonexistent-" + Date.now(),
  );
...
test("writeUpdateMarker + dead pid => self-heals on read", () => {
  ...
  assert.equal(
    res,
    null,
    "a dead-pid marker from writeUpdateMarker self-heals",
  );
...
test("an expired marker does not block a hand-off (self-heals)", () => {
  ...
  assert.equal(
    updateHandoffConflict(home, { kill: ALIVE, now: () => now }),
    null,
  );
});
