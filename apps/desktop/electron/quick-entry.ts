...
export type QuickEntryShortcutParse =
  | { ok: false; reason: QuickEntryShortcutError }
...
function isAcceleratorKey(token: string): boolean {
  ...
  return (
    token.length === 1 &&
    (/^[a-z0-9]$/.test(token) || ACCELERATOR_PUNCTUATION.has(token))
  );
}
...
export function parseQuickEntryShortcut(raw: unknown): QuickEntryShortcutParse {
  ...
  const normalizedModifiers = modifiers
    ...
    .filter((modifier) =>
      seen.has(modifier) ? false : (seen.add(modifier), true),
    )
  ...
  return {
    accelerator: [...normalizedModifiers, canonicalKey(key)].join("+"),
    ok: true,
  };
}
...
export function createQuickEntryShortcut(
  ...
  let state: QuickEntryRegistration = {
    error: null,
    registered: false,
 ...
 */
export function quickEntryWindowBounds(workArea?: {
  height: number;
  width: number;
  x: number;
  y: number;
  ...
  const width = Math.min(
    ...
    workArea?.width ?? QUICK_ENTRY_WINDOW_WIDTH,
  );
  const height = Math.min(
    QUICK_ENTRY_WINDOW_HEIGHT,
    workArea?.height ?? QUICK_ENTRY_WINDOW_HEIGHT,
  );
  ...
  const y = Math.round(
    Math.min(
      Math.max(
        workArea.y,
        workArea.y + workArea.height * QUICK_ENTRY_TOP_FRACTION,
      ),
      maxY,
    ),
  );
