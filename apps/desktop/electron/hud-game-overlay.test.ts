...
const hudWindow = win({
  app: "Hermes",
  bounds: { x: 970, y: 1048, width: 620, height: 320 },
  id: 2,
  pid: SELF_PID,
});
const desktopShell = win({
...
test("DPI rounding and the 1px-oversize trick stay within the epsilon", () => {
  assert.equal(
    coversDisplay({ x: -1, y: -1, width: 2562, height: 1442 }, DISPLAY),
    true,
  );
  assert.equal(
    coversDisplay({ x: 1, y: 1, width: 2558, height: 1438 }, DISPLAY),
    true,
  );
});
...
test("a maximized window stopping at the taskbar does not cover", () => {
  assert.equal(
    coversDisplay({ x: 0, y: 0, width: 2560, height: 1392 }, DISPLAY),
    false,
  );
});
...
test("a fullscreen window on ANOTHER display does not cover this one", () => {
  assert.equal(
    coversDisplay({ x: 2560, y: 0, width: 2560, height: 1440 }, DISPLAY),
    false,
  );
});
...
test("fullscreen game under the HUD is detected", () => {
  const found = detectFullscreenApp(
    [hudWindow, fullscreenGame, desktopShell],
    SELF_PID,
    DISPLAY,
  );
...
test("own windows never count, wherever they sit in the z-order", () => {
  ...
  assert.equal(
    detectFullscreenApp([selfFullscreen, desktopShell], SELF_PID, DISPLAY),
    null,
  );
});
...
test("the desktop shell reporting display bounds is not a game", () => {
  assert.equal(
    detectFullscreenApp([hudWindow, desktopShell], SELF_PID, DISPLAY),
    null,
  );
});
...
test("a windowed app in FRONT of the game vetoes the overlay", () => {
  const editor = win({
    app: "VS Code",
    bounds: { x: 200, y: 100, width: 1400, height: 900 },
    id: 9,
    pid: 3000,
  });

  assert.equal(
    detectFullscreenApp([hudWindow, editor, fullscreenGame], SELF_PID, DISPLAY),
    null,
  );
});
...
test("zero-area rows (minimized) neither decide nor veto", () => {
  ...
  assert.equal(
    detectFullscreenApp([minimized, fullscreenGame], SELF_PID, DISPLAY)?.app,
    "Balatro",
  );
});

test("a window intersecting only another display does not veto", () => {
  const otherDisplayWin = win({
    app: "Slack",
    bounds: { x: 3000, y: 0, width: 800, height: 600 },
    id: 12,
    pid: 3200,
  });

  assert.equal(
    detectFullscreenApp([otherDisplayWin, fullscreenGame], SELF_PID, DISPLAY)
      ?.app,
    "Balatro",
  );
});
...
test("gameOverlayStateFor maps detection to the pushed state shape", () => {
  assert.deepEqual(gameOverlayStateFor([fullscreenGame], SELF_PID, DISPLAY), {
    active: true,
    app: "Balatro",
  });
  assert.deepEqual(
    gameOverlayStateFor([hudWindow], SELF_PID, DISPLAY),
    INACTIVE_GAME_OVERLAY,
  );
});
...
test("a windowed app on top vetoes ENTERING but not STAYING", () => {
  ...
  assert.deepEqual(
    gameOverlayStateFor(stack, SELF_PID, DISPLAY, false),
    INACTIVE_GAME_OVERLAY,
  );
  assert.deepEqual(gameOverlayStateFor(stack, SELF_PID, DISPLAY, true), {
    active: true,
    app: "Balatro",
  });
});
...
test("the game actually closing ends overlay mode even when it was active", () => {
  assert.deepEqual(
    gameOverlayStateFor([hudWindow, desktopShell], SELF_PID, DISPLAY, true),
    INACTIVE_GAME_OVERLAY,
  );
});
...
test("findFullscreenAppAnywhere ignores z-order but keeps every other guard", () => {
  ...
  assert.equal(
    findFullscreenAppAnywhere([selfFullscreen], SELF_PID, DISPLAY),
    null,
  );
  assert.equal(
    findFullscreenAppAnywhere([desktopShell], SELF_PID, DISPLAY),
    null,
  );
  assert.equal(
    findFullscreenAppAnywhere([hudWindow, fullscreenGame], SELF_PID, DISPLAY)
      ?.app,
    "Balatro",
  );
});
...
test("a single transient failure does not give up the watch", async () => {
  ...
  startHudGameOverlayWatch({
    enumerate: () =>
      fail ? Promise.resolve(null) : Promise.resolve([fullscreenGame]),
    displayBounds: () => DISPLAY,
    selfPid: SELF_PID,
    send: (state) => pushed.push(state),
    setIntervalFn: clock.setIntervalFn,
