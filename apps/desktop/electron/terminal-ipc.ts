...
export interface TerminalIpcDeps {
  ...
  getSshConnectionState: (
    scope: string,
  ) => undefined | { remotePlatform?: string };
}
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    ...
    const override = (
      ...
      (isWindows ? "" : process.env.SHELL) ||
      ""
    ).trim();

    if (override) {
      const resolved = isExecutableFile(override)
        ? override
        : findOnPath(override);
    ...
    const shellPath = ["/bin/zsh", "/bin/bash", "/bin/sh"].find((candidate) =>
  ...
  function ensureNodePtySpawnHelper() {
    ...
    try {
      ...
      for (const helperPath of fixed) {
        rememberLog(
          `[terminal] restored +x on node-pty spawn-helper: ${helperPath}`,
        );
      }

      for (const failure of errors) {
        rememberLog(
          `[terminal] could not chmod spawn-helper ${failure.path}: ${failure.error}`,
        );
      }
    } catch (error) {
      rememberLog(
  ...
  ipcMain.handle("hermes:terminal:start", async (event, payload = {}) => {
    ...
    terminalSessions.set(id, {
      ...
      ...(remote
        ? { sshScope: sshTarget.scope, remoteCwd: String(payload?.cwd || "") }
        : {}),
    });
  ...
  ipcMain.handle("hermes:terminal:resize", (_event, id, size = {}) => {
    ...
    const cols = Math.max(
      2,
      Number.parseInt(String(size?.cols || 80), 10) || 80,
    );
    const rows = Math.max(
      2,
      Number.parseInt(String(size?.rows || 24), 10) || 24,
    );
  ...
  ipcMain.handle("hermes:terminal:cwd", async (_event, id) => {
    ...
    return sessionInfo.sshScope !== undefined
      ? null
