...
function classifyVenvBlocker(
  ...
  const moduleMatch = process.cmdline.match(
    /(?:^|\s)-m\s+http\.server(?:\s+(\d{1,5}))?(?:\s|$)/i,
  );
  const isPython = /^python(?:w)?(?:\.exe)?$/i.test(process.name);
  const hintedCreateTime =
    typeof hints?.createTime === "number" ? hints.createTime : undefined;
  ...
  const hintedPort =
    trustedScannerIdentity && typeof hints?.port === "number"
      ? hints.port
      : undefined;
  ...
  const directoryMatch = process.cmdline.match(
    /(?:^|\s)--directory\s+(?:"([^"]+)"|'([^']+)'|(.+))$/i,
  );
  const directory = (
    directoryMatch?.[1] ||
  ...
  const parsedLabel = directory
    ? path.win32.basename(directory.replace(/["']$/, ""))
    : undefined;
  const hintedLabel =
    trustedScannerIdentity && typeof hints?.label === "string"
      ? hints.label.trim()
      : "";
...
export function parseVenvBlockerScanOutput(raw: string): ScanOutcome {
  ...
  for (const entry of parsed.processes) {
    if (!entry || typeof entry !== "object") {
      return {
        kind: "probe-failure",
        error: "process entry must be an object",
      };
    }
    ...
    if (!Number.isInteger(pid) || pid <= 0) {
      return {
        kind: "probe-failure",
        error: "process pid must be a positive integer",
      };
    }
    ...
    if (typeof cmdline !== "string") {
      return {
        kind: "probe-failure",
        error: "process cmdline must be a string",
      };
    }
  ...
  if (parsed.blocked && processes.length === 0) {
    return {
      kind: "probe-failure",
      error: "blocked is true but process list is empty",
    };
  }
  ...
  if (!parsed.blocked && processes.length > 0) {
    return {
      kind: "probe-failure",
      error: "blocked is false but process list is non-empty",
    };
  }
