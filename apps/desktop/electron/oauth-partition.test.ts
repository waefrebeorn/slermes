...
const registry = (primary: string, connections: any[]) => ({
  primary,
  connections,
});
...
describe("resolveOauthPartition (#92183 per-connection cookie jars)", () => {
  it("gives two same-host different-port registry gateways DISTINCT partitions (eviction fix)", () => {
    ...
    const a = resolveOauthPartition("https://10.27.27.7:9119", {
      registry: reg,
    });
    const b = resolveOauthPartition("https://10.27.27.7:9220", {
      registry: reg,
    });
  ...
  it("scopes a full request URL (REST/ws-ticket path) to its connection jar via longest base-url prefix", () => {
    ...
    const a = resolveOauthPartition(
      "https://gw.example.com/api/auth/ws-ticket",
      { registry: reg },
    );
    const b = resolveOauthPartition(
      "https://gw.example.com/team-b/api/auth/ws-ticket",
      { registry: reg },
    );
  ...
  it("keeps the v1 primary remote on the LEGACY partition so upgrades do not sign the user out", () => {
    const reg = registry("mig-1", [
      remote("mig-1", "https://gw-a.example.com"),
    ]);
  ...
  it("keeps the registry PRIMARY connection on the legacy partition", () => {
    ...
    expect(
      resolveOauthPartition("https://gw-a.example.com/api/status", {
        registry: reg,
      }),
    ).toBe(LEGACY_OAUTH_PARTITION);
    expect(
      resolveOauthPartition("https://gw-b.example.com/api/status", {
        registry: reg,
      }),
    ).not.toBe(LEGACY_OAUTH_PARTITION);
  });
    ...
    const reg = registry("local", [
      {
        id: "cloud-1",
        kind: "cloud",
        url: "https://agent.nousresearch.com",
        authMode: "oauth",
      },
    ]);

    expect(
      resolveOauthPartition("https://agent.nousresearch.com/api/status", {
        registry: reg,
      }),
    ).toBe(LEGACY_OAUTH_PARTITION);
  });
  ...
  it("keeps token-auth registry remotes on the legacy partition (no cookies involved)", () => {
    ...
    expect(
      resolveOauthPartition("https://gw-t.example.com", { registry: reg }),
    ).toBe(LEGACY_OAUTH_PARTITION);
  });
  ...
  it("falls back to the legacy partition for unmatched, portal, and malformed inputs", () => {
    ...
    expect(
      resolveOauthPartition("https://portal.nousresearch.com/api/agents", {
        registry: reg,
      }),
    ).toBe(LEGACY_OAUTH_PARTITION);
    expect(resolveOauthPartition("not a url", { registry: reg })).toBe(
      LEGACY_OAUTH_PARTITION,
    );
    ...
    expect(
      resolveOauthPartition("https://gw-a.example.com", {
        registry: { primary: "x", connections: "junk" } as any,
      }),
    ).toBe(LEGACY_OAUTH_PARTITION);
  });
  ...
  it("does not treat a hostname PREFIX as a base-url match", () => {
    ...
    expect(
      resolveOauthPartition("https://gw.example.com.evil.tld/login", {
        registry: reg,
      }),
    ).toBe(LEGACY_OAUTH_PARTITION);
  });
  ...
  it("normalizes trailing slashes and default ports when matching entry URLs", () => {
    ...
    const got = resolveOauthPartition(
      "https://gw-a.example.com/api/auth/ws-ticket",
      { registry: reg },
    );
  ...
  it("produces a deterministic, partition-safe name from hostile connection ids", () => {
    ...
    expect(
      resolveOauthPartition("https://gw-a.example.com", { registry: reg }),
    ).toBe(got);
  });
