...
test("does not fall back to heavyweight /api/status for transient health failures", async () => {
  ...
  assert.ok(
    calls.every(
      (call) => call[0] === "public" && call[1].endsWith("/api/health"),
    ),
  );
});
...
test("recognizes missing-route shapes only", () => {
  assert.equal(
    isMissingHealthEndpointError(new Error('404: {"detail":"Not Found"}')),
    true,
  );
  assert.equal(
    isMissingHealthEndpointError(
      new Error(
        "Expected JSON from /api/health but got HTML. The endpoint is likely missing on the Hermes backend.",
      ),
    ),
    true,
  );
  assert.equal(
    isMissingHealthEndpointError(
      new Error("Timed out connecting to Hermes backend after 15000ms"),
    ),
    false,
  );
...
test("unsigned OAuth is a terminal reauth failure; needsOauthLogin alone is not", () => {
  ...
  assert.equal(
    isReauthRequiredError(
      new Error("Could not reach the remote Hermes gateway"),
    ),
    false,
  );
});
...
test("credentialed 5xx and 429 keep polling — only 401/403 are terminal", async () => {
  for (const transient of [
    ...
    assert.ok(
      attempts > 1,
      `${transient} should have retried, got ${attempts} attempt(s)`,
    );
  }
});
...
test("error-shape predicates", () => {
  ...
  assert.equal(
    isGatedMissingHealthError(new Error('401: {"detail":"Unauthorized"}')),
    false,
  );
  assert.equal(
    isGatedMissingHealthError(new Error('404: {"detail":"Not Found"}')),
    false,
  );

  assert.equal(isAuthRejectionError(new Error(GATE_401)), true);
  assert.equal(
    isAuthRejectionError(new Error('403: {"detail":"Forbidden"}')),
    true,
  );
  assert.equal(
    isAuthRejectionError(new Error('404: {"detail":"Not Found"}')),
    false,
  );
...
test("isServerSideHttpError detects 502/503/504", () => {
  ...
  const result503 = isServerSideHttpError(
  ...
  const result500 = isServerSideHttpError(
  ...
  assert.equal(
    isServerSideHttpError(new Error("429: Too Many Requests")),
    null,
  );
...
test("isNousCloudAgentUrl detects cloud agent hosts", () => {
  // Positive cases
  assert.equal(
    isNousCloudAgentUrl("https://ares-3009.agents.nousresearch.com"),
    true,
  );
  assert.equal(
    isNousCloudAgentUrl("https://ares-3009.agents.nousresearch.com/api/health"),
    true,
  );
  assert.equal(
    isNousCloudAgentUrl("http://test.agents.nousresearch.com"),
    true,
  );
...
test("waitForHermesReady surfaces actionable error for cloud agent 503", async () => {
  ...
  try {
    ...
    assert.ok(
      error.message.includes("Nous Cloud agent"),
      `unexpected message: ${error.message}`,
    );
    assert.ok(
      error.message.includes("503"),
      `should mention status code: ${error.message}`,
    );
    assert.ok(
      error.message.includes("portal.nousresearch.com"),
      `should mention portal: ${error.message}`,
    );
    assert.ok(
      error.message.includes("discord.gg/NousResearch"),
      `should mention Discord: ${error.message}`,
    );
...
test("isServerSideHttpError structured path excludes 500/401/403/404/429 even when statusCode is attached", () => {
  for (const code of [500, 401, 403, 404, 429]) {
    ...
    err.statusCode = code;
    assert.equal(
      isServerSideHttpError(err),
      null,
      `should reject statusCode ${code}`,
    );
  }
});
...
test("makeNousCloudBackendDownError returns null for a non-Cloud 503 (generic remote failure)", () => {
  ...
  err.statusCode = 503;
  assert.equal(
    makeNousCloudBackendDownError("https://gateway.example.com", err),
    null,
  );
  assert.equal(
    makeNousCloudBackendDownError("http://127.0.0.1:9000", err),
    null,
  );
});
