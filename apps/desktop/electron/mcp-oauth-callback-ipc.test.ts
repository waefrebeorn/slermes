...
const { registerMcpOauthCallbackIpc } =
  await import("./mcp-oauth-callback-ipc");
...
test("listen binds a loopback listener and wait resolves with the redirect params", async () => {
  const { id, redirectUri } = (await invoke("hermes:mcp-oauth:listen")) as {
    id: string;
    redirectUri: string;
  };
...
test("cancel tears the listener down and wait reports listener not found afterwards", async () => {
  ...
  const result = (await invoke("hermes:mcp-oauth:wait", id, 100)) as {
    error: null | string;
  };
...
test("wait times out when no callback arrives", async () => {
  ...
  const result = (await invoke("hermes:mcp-oauth:wait", id, 1000)) as {
    code: null | string;
    error: null | string;
  };
