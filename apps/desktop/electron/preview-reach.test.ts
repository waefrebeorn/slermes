...
describe("rewriteToLocalPort", () => {
  it("keeps path, query and hash — they are what make the URL useful", () => {
    expect(rewriteToLocalPort("http://localhost:5173/a/b?q=1#f", 42)).toBe(
      "http://127.0.0.1:42/a/b?q=1#f",
    );
  });
  ...
  it("forces http", () => {
    expect(rewriteToLocalPort("https://localhost:5173/", 42)).toBe(
      "http://127.0.0.1:42/",
    );
  });
});
...
describe("openPreviewReach", () => {
  it("forwards the remote port and returns the local URL", async () => {
    ...
    expect(lease?.expiresAt).toBeLessThanOrEqual(
      Date.now() + PREVIEW_REACH_LEASE_MS,
    );
  });
  ...
  it("surfaces a failed forward instead of pretending it worked", async () => {
    ...
    await expect(openPreviewReach("http://localhost:5173/", d)).rejects.toThrow(
      "ssh exited 255",
    );
  });
