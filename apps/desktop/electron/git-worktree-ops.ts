...
async function listWorktrees(repoPath, gitBin) {
  ...
  try {
    resolved = resolveRequestedPathForIpc(repoPath, {
      purpose: "Worktree list",
    });
  } catch {
    return [];
  }
...
async function remoteOfRef(gitBin, cwd, name) {
  ...
  if (
    !(await gitOk(
      gitBin,
      ["show-ref", "--verify", "--quiet", `refs/remotes/${name}`],
      cwd,
    ))
  ) {
    return "";
...
async function defaultBranch(gitBin, cwd) {
  ...
  const configured = await gitLine(
    gitBin,
...
async function ensureGitRepo(gitBin, dir) {
  ...
  try {
    const inside = (
      await runGit(gitBin, ["rev-parse", "--is-inside-work-tree"], dir)
    ).trim();
...
async function addExistingBranchWorktree(gitBin, root, name) {
  ...
  if (remote) {
    ...
    await runGit(
      gitBin,
      ["worktree", "add", "--track", "-b", branch, dir, requested],
      root,
    );
...
async function addWorktree(repoPath, options, gitBin) {
  const resolved = resolveRequestedPathForIpc(repoPath, {
    purpose: "Worktree add",
  });
...
async function removeWorktree(repoPath, worktreePath, options, gitBin) {
  const resolvedRepo = resolveRequestedPathForIpc(repoPath, {
    purpose: "Worktree remove (repo)",
  });
  const resolvedTree = resolveRequestedPathForIpc(worktreePath, {
    purpose: "Worktree remove (tree)",
  });
...
async function switchBranch(repoPath, branch, gitBin) {
  ...
  try {
    inside = (
      await runGit(gitBin, ["rev-parse", "--is-inside-work-tree"], resolved)
    ).trim();
  } catch {
    ...
    return out
      ...
      .map((line) => {
        ...
        return {
          ...
          // its trunk.
          isDefault: Boolean(
            (remoteDefault && name === remoteDefault) ||
            (!remoteDefault && localDefault && name === localDefault),
          ),
        };
      });
  } catch {
