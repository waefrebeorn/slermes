...
function makeRepo() {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), "hermes-desktop-git-status-"),
  );

  tempDirs.push(dir);
  execFileSync("git", ["init", "-q"], { cwd: dir });
  execFileSync("git", ["config", "user.email", "hermes-test@example.com"], {
    cwd: dir,
  });
...
test("resolveRenamePath: brace rename resolves to the new path", () => {
  assert.equal(
    resolveRenamePath("src/{old => new}/file.ts"),
    "src/new/file.ts",
  );
});
...
test("reviewList caps the file payload returned to the renderer", async () => {
  ...
  for (let i = 0; i < REVIEW_FILE_CAP + 10; i++) {
    fs.writeFileSync(
      path.join(dir, `untracked-${String(i).padStart(4, "0")}.txt`),
      "generated\n",
    );
  }
