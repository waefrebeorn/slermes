...
test("hasValidBootstrapMarker rejects missing, wrong-schema, and too-short markers", () => {
  ...
  assert.equal(
    hasValidBootstrapMarker(
      { schemaVersion: 2, pinnedCommit: VALID_MARKER.pinnedCommit },
      1,
    ),
    false,
  );
  assert.equal(
    hasValidBootstrapMarker({ schemaVersion: 1, pinnedCommit: "abc123" }, 1),
    false,
  );
});
...
test("classifyActiveRuntime uses a healthy active runtime even when the marker is stale or malformed", () => {
  assert.deepEqual(
    classifyActiveRuntime(
      { schemaVersion: 999, pinnedCommit: "abc1234" },
      1,
      true,
    ),
    {
...
test("a repair that deleted the marker does not strand a healthy install", () => {
  ...
  assert.equal(
    classifyActiveRuntime(null, 1, true).shouldUseActiveRuntime,
    true,
  );
});
