...
test("statusSupportsNativeFlow reads the auth_flows array", () => {
  assert.equal(
    statusSupportsNativeFlow({ auth_flows: ["cookie", NATIVE_FLOW_ID] }),
    true,
  );
...
test("resolveLoginStrategy picks native only when advertised and not forced", () => {
  ...
  assert.equal(
    resolveLoginStrategy(gated, { forceEmbedded: true }),
    "embedded",
  );
});
...
test("nativeTokenUrl / nativeRefreshUrl build the right endpoints", () => {
  assert.equal(
    nativeTokenUrl("https://gw.example.com"),
    "https://gw.example.com/auth/native/token",
  );
  assert.equal(
    nativeRefreshUrl("https://gw.example.com/hermes"),
    "https://gw.example.com/hermes/auth/native/refresh",
  );
});
...
test("parseLoopbackCallback throws when the code is absent", () => {
  assert.throws(
    () => parseLoopbackCallback("/callback?state=xyz", "xyz"),
