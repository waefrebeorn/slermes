...
export function registerFsIpc({
  ...
  ipcMain.handle("hermes:fs:desktopPluginsRoot", async () =>
    localPluginsRoot("desktop-plugins"),
  );
  ...
  ipcMain.handle("hermes:fs:agentPluginsRoot", async () =>
    localPluginsRoot("plugins"),
  );

  ipcMain.handle("hermes:plugin:probe", async (_event, payload) => {
    const identifier = String(
      payload?.identifier || payload?.repo || "",
    ).trim();
  ...
  ipcMain.handle("hermes:fs:writeText", async (_event, filePath, content) => {
    ...
    const resolved = resolveRequestedPathForIpc(expandUserPath(raw), {
