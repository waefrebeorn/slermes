...
export function isServerSideHttpError(error: unknown): {
  ...
  if ("statusCode" in error) {
    ...
    if (
      ...
      (structured === 502 || structured === 503 || structured === 504)
    ) {
 ...
 */
export function makeNousCloudBackendDownError(
  baseUrl: string,
  error: unknown,
): Error | null {
...
export function isMissingHealthEndpointError(error: unknown): boolean {
  ...
  return (
    /^404:/.test(message) || message.includes("endpoint is likely missing")
  );
}
...
function supersededError() {
  const error: any = new Error(
    "SSH bootstrap was superseded by newer connection settings.",
  );
...
export async function waitForHermesReady(
  ...
  const healthProbeTimeoutMs =
    options.healthProbeTimeoutMs ?? DEFAULT_HEALTH_PROBE_TIMEOUT_MS;
