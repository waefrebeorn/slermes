...
test("keeps terminal routing isolated by renderer sender id", async () => {
  ...
  const getTarget = (webContentsId: number) =>
    targets.get(webContentsId) ?? null;
  const ensureBackend = async (_webContentsId: number) => undefined;

  const windowB = await resolveTerminalConnectionForSender(
    11,
    getTarget,
    ensureBackend,
  );
  const windowC = await resolveTerminalConnectionForSender(
    22,
    getTarget,
    ensureBackend,
  );
