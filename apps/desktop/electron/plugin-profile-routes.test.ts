...
function config(
  overrides: Partial<ProfileRouteConfig> = {},
): ProfileRouteConfig {
  return {
    cloudOrg: "",
    mode: "local",
...
describe("buildOpaqueProfileRoutes", () => {
  ...
  it("keys IDs to the Desktop installation", async () => {
    const options = {
      getProfileConfig: () =>
        config({ mode: "remote", remoteUrl: "https://gateway.example" }),
      globalConfig: config(),
      primaryProfile: "default",
      profileNames: ["default", "worker"],
      resolveSsh: vi.fn(),
    ...
    const first = await buildOpaqueProfileRoutes({
      ...options,
      installationId: "install-a-secret",
    });
    const second = await buildOpaqueProfileRoutes({
      ...options,
      installationId: "install-b-secret",
    });
  ...
  it("isolates an SSH resolution failure to its configured route", async () => {
    ...
    expect(routes.find((route) => route.profile === "broken")).toMatchObject({
      mode: "remote",
      profile: "broken",
    });
  });
  ...
  it("inherits the global remote gateway and deduplicates profile names", async () => {
    ...
    expect(routes.map((route) => route.profile)).toEqual([
      "default",
      "alpha",
      "beta",
    ]);
    expect(routes.map((route) => route.targetProfile)).toEqual([
      "default",
      "alpha",
      "beta",
    ]);
  ...
  it("keeps cloud organizations on one service URL in distinct groups", async () => {
    ...
    expect(
      JSON.stringify(
        routes.map(({ connectionId, mode }) => ({ connectionId, mode })),
      ),
    ).not.toContain("org-a");
  });
});
...
describe("buildRegistryProfileRoutes", () => {
  it("keeps duplicate profile names distinct by registry connection without exposing source details", () => {
    ...
    expect(
      new Set(routes.map((route) => `${route.connectionId}/${route.profile}`)),
    ).toHaveLength(2);
  });
...
describe("localRouteFallbackProfiles", () => {
  it("restores failed local profiles when another source returned agents", () => {
    ...
    expect(
      localRouteFallbackProfiles(agents, "local", ["default", "venture"], true),
    ).toEqual(["default", "venture"]);
  });
  ...
  it("does not synthesize local routes for a deferred connect-on-demand enumeration", () => {
    expect(
      localRouteFallbackProfiles(
        ...
        "local",
        ["default"],
        isLocalEnumerationFailure("connect-on-demand"),
      ),
    ).toEqual([]);
  });
  ...
  it("synthesizes local routes for a genuine local enumeration error", () => {
    expect(
      localRouteFallbackProfiles(
        ...
        "local",
        ["default"],
        isLocalEnumerationFailure("ECONNREFUSED"),
      ),
    ).toEqual(["default"]);
  });
});
