...
test("installZoomReassertOnWindowEvents re-verifies Linux zoom after the debounced re-assert so a dropped re-apply is retried", () => {
  ...
  try {
    ...
    assert.equal(
      calls,
      1 + ZOOM_REASSERT_MAX_SETTLE_CHECKS,
      "settle chain is bounded and stops",
    );
  } finally {
    vi.useRealTimers();
  }
  ...
  try {
    ...
    for (let i = 1; i <= ZOOM_REASSERT_MAX_SETTLE_CHECKS; i++) {
      ...
      assert.equal(
        calls,
        3 + i,
        `restarted chain settle check ${i} re-asserts`,
      );
    }
    ...
    assert.equal(
      calls,
      3 + ZOOM_REASSERT_MAX_SETTLE_CHECKS,
      "restarted chain is bounded too",
    );
  } finally {
    vi.useRealTimers();
  }
...
test("installZoomReassertOnNavigation covers full loads and main-frame in-page routes", () => {
  ...
  assert.deepEqual(
    [...handlers.keys()],
    ["did-finish-load", "did-navigate-in-page"],
  );
  ...
  handlers.get("did-navigate-in-page")(
    {},
    "file:///app/index.html#/session/next",
    true,
  );
  assert.equal(calls, 2);
});
...
test("applyZoomLevel applies the level then notifies the renderer", () => {
  ...
  assert.deepEqual(wc.calls, [
    ...
    [
      "send",
      "hermes:zoom:changed",
      { level: 3, percent: zoomLevelToPercent(3) },
    ],
  ]);
});
...
test("applyZoomLevel clamps garbage before applying and notifying", () => {
  ...
  assert.deepEqual(wc.calls, [
    ...
    [
      "send",
      "hermes:zoom:changed",
      { level: clamped, percent: zoomLevelToPercent(clamped) },
    ],
  ]);
});
