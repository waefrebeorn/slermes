...
export const WAKE_INDICATOR_STATES = [
  "hidden",
  "detected",
  "capturing",
] as const;
...
export function normalizeWakeIndicatorState(
  value: unknown,
): WakeIndicatorState {
  return WAKE_INDICATOR_STATES.includes(value as WakeIndicatorState)
    ? (value as WakeIndicatorState)
    : "hidden";
}

export function selectWakeIndicatorDisplay<T extends DisplayLike>(
  displays: T[],
  primary: T,
): T {
...
export function wakeIndicatorWindowBounds(display: DisplayLike) {
  return {
    ...
    x: Math.round(
      display.bounds.x +
        (display.bounds.width - WAKE_INDICATOR_WINDOW_WIDTH) / 2,
    ),
