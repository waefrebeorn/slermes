...
test("tuiResumeArgs resumes the session in the TUI", () => {
  assert.deepEqual(tuiResumeArgs("20260814_101010_abc123"), [
    "--tui",
    "--resume",
    "20260814_101010_abc123",
  ]);
});

test("tuiResumeArgs pins the profile ahead of the mode flag", () => {
  assert.deepEqual(tuiResumeArgs("sess", "work"), [
    "--profile",
    "work",
    "--tui",
    "--resume",
...
test("Linux falls down the emulator ladder and omits a flagless terminal", () => {
  const onlyKitty = (command: string) =>
    command === "kitty" ? "/usr/bin/kitty" : null;

  assert.deepEqual(
    resolveTerminalLaunch({
      findOnPath: onlyKitty,
      platform: "linux",
      scriptPath: "/tmp/x.sh",
    }),
    {
