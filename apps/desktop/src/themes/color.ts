...
export function readableOn(hex: string): string {
  return contrastRatio(hex, "#ffffff") >= contrastRatio(hex, "#161616")
    ? "#ffffff"
    : "#161616";
}
...
const linearize01 = (c: number): number =>
  c <= 0.04045 ? c / 12.92 : ((c + 0.055) / 1.055) ** 2.4;
const delinearize01 = (c: number): number =>
...
function rgbToOklab([r255, g255, b255]: readonly number[]): [
  number,
  number,
  number,
] {
...
const inSrgbGamut = (rgb: readonly number[]): boolean =>
...
export function oklchToHex(color: Oklch): string {
  if (inSrgbGamut(oklchToRgbRaw(color))) {
    return rgbToHex(
      oklchToRgbRaw(color).map((c) => delinearize01(c) * 255) as [
        number,
        number,
        number,
      ],
  ...
  return rgbToHex(
    oklchToRgbRaw({ ...color, c: lo }).map((c) => delinearize01(c) * 255) as [
      number,
      number,
      number,
    ],
...
export function oklchToSrgb255(color: Oklch): [number, number, number] | null {
  ...
  return raw.map((c) =>
    Math.round(Math.min(1, Math.max(0, delinearize01(c))) * 255),
...
export function harmonize(
  ...
  const h =
    (base.h +
      hueDelta(base.h, target.h) * Math.min(1, Math.max(0, strength)) +
      360) %
    360;

  return oklchToHex({
    ...base,
    c: Math.min(Math.max(base.c, target.c * 0.85), maxChroma(base.l, h)),
    h,
  });
}
