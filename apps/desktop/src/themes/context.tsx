...
const normalizeSkin = (name: string | null): string =>
  name && resolveTheme(name) && !RETIRED_SKINS.has(name)
    ? name
    : DEFAULT_SKIN_NAME;
...
const profilePref = <T extends string>(
  ...
  resolve: (profile: string): T =>
    normalize(storedStringRecord(record)[profile] ?? storedString(legacy)),
  assign: (profile: string, value: T): void => {
    if (profile === "default") {
      persistString(legacy, value);
    } else {
      persistStringRecord(record, {
        ...storedStringRecord(record),
        [profile]: value,
      });
    }
  },
...
const readBootProfileKey = () =>
  normalizeProfileKey(storedString(LAST_PROFILE_KEY));
const rememberActiveProfileKey = (profile: string) =>
  persistString(LAST_PROFILE_KEY, profile);
...
function applyTheme(theme: DesktopTheme, mode: "light" | "dark") {
  ...
  const typo = {
    ...DEFAULT_TYPOGRAPHY,
    ...nousTheme.typography,
    ...theme.typography,
  };
  const rendered = renderedModeFor(c, mode);
  const isDark = rendered === "dark";
  const midground = c.midground ?? c.ring;
  const skinName = theme.name.endsWith(`-${mode}`)
    ? theme.name.slice(0, -mode.length - 1)
    : theme.name;
  ...
  const palette: Record<string, string> = {
    ...
    "--dt-primary-solid": ensureContrast(
      c.primary,
      PRIMARY_SOLID_FOREGROUND,
      4.5,
    ),
...
const syncNativeTheme = (pref: ThemeMode, rendered: "light" | "dark") =>
  window.hermesDesktop?.setNativeTheme?.(
    pref === "system" ? "system" : rendered,
  );
...
const SKIN_LIST = BUILTIN_THEME_LIST.map(({ name, label, description }) => ({
  name,
  label,
  description,
}));
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [themeName, setThemeNameState] = useState(() =>
    typeof window === "undefined"
      ? DEFAULT_SKIN_NAME
      : skinPref.resolve(readBootProfileKey()),
  );

  const [mode, setModeState] = useState<ThemeMode>(() =>
    typeof window === "undefined"
      ? "system"
      : modePref.resolve(readBootProfileKey()),
  );
  ...
  const paintedTheme = useMemo(
    () =>
      accentOverride === null
        ? activeTheme
        : retintTheme(activeTheme, accentOverride),
    [activeTheme, accentOverride],
  );

  // What actually gets painted (matches the `.dark` class applyTheme toggles).
  const renderedMode = useMemo(
    () => renderedModeFor(paintedTheme.colors, paintedMode),
  ...
  const previewTheme = useCallback(
    (name: string, previewMode: "light" | "dark") => {
