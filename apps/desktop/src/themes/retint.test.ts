...
describe("themeHue", () => {
  ...
  it("reads the upstream GitHub green from the unforked theme", () => {
    ...
    expect(Math.round(hexToOklch(githubTheme.darkColors!.primary)!.h)).toBe(
      148,
    );
  });
});
...
describe("the shipped nous accents", () => {
  ...
  it.each(cases)(
    "$appearance seeds every accent slot from $seed",
    ({ colors, seed }) => {
      for (const key of [
        "primary",
        "ring",
  ...
  it.each(cases)(
    "$appearance clears AA on its own sidebar",
    ({ colors, seed }) => {
      expect(
        contrastRatio(seed, colors.sidebarBackground!),
      ).toBeGreaterThanOrEqual(4.5);
    },
  );

  it.each(cases)(
    "$appearance keeps text on the accent readable",
    ({ colors, seed }) => {
      expect(
        contrastRatio(seed, colors.primaryForeground),
      ).toBeGreaterThanOrEqual(4.5);
    },
  );
...
describe("retintTheme", () => {
  ...
  it("leaves the chrome alone", () => {
    ...
    for (const key of [
      ...
      expect(violet.darkColors![key], `dark ${key}`).toBe(
        nousTheme.darkColors![key],
      );
    }
  });
  ...
  it("holds perceived lightness and chroma while only the hue moves", () => {
    ...
    for (const hue of HUES) {
      const seed = hexToOklch(
        retintTheme(nousTheme, seedAt(hue)).colors.primary,
      )!;
  ...
  it("re-picks the foreground that sits on the accent", () => {
    for (const hue of HUES) {
      ...
      expect(
        contrastRatio(c.primary, c.primaryForeground),
        `on-accent @ ${hue}°`,
      ).toBeGreaterThanOrEqual(4.5);
    }
  });
  ...
  it("accepts any hex form and ignores junk", () => {
    expect(retintTheme(nousTheme, "#0053FD").colors.primary).toBe(
      retintTheme(nousTheme, "0053fd").colors.primary,
    );
  ...
  describe("a seed that only works in one mode", () => {
    ...
    it("lightens it for the mode where it does not", () => {
      ...
      expect(
        contrastRatio(dark, blue.darkColors!.sidebarBackground!),
      ).toBeGreaterThanOrEqual(4.5);
    });
  ...
  describe("a theme whose accent slots are shades of each other", () => {
    ...
    it("moves every slot in the family", () => {
      ...
      expect(
        Math.abs(hexToOklch(teal.colors.ring)!.h - hexToOklch("#0f9b8e")!.h),
      ).toBeLessThan(3);
    });
