...
describe("capability helpers are connection-scoped", () => {
  const api = vi.fn(
    async (_req: { connectionId?: string; path: string; profile?: string }) =>
      ({}) as never,
  );
  ...
  const last = () =>
    api.mock.calls.at(-1)?.[0] as { connectionId?: string; profile?: string };
  ...
  it("carries the ambient registry connection on the legacy string/ambient path", () => {
    ...
    for (const call of api.mock.calls) {
      expect((call[0] as { connectionId?: string }).connectionId).toBe(
        "gw-tailscale",
      );
  ...
  it("object scopes pin every read and write to the named connection", () => {
    ...
    void getSkillContent("arxiv", {
      connectionId: "homelab",
      profile: "inbox-bot",
    });
    void setSkillEnabled("arxiv", false, {
      connectionId: "homelab",
      profile: "inbox-bot",
    });
    ...
    void installSkillFromHub("official/research/arxiv", {
      connectionId: "homelab",
      profile: "inbox-bot",
    });
  ...
  it("profileScopeKey keeps legacy keys byte-identical and namespaces every explicit pin", () => {
    ...
    expect(profileScopeKey({ connectionId: "local", profile: "coder" })).toBe(
      "local::coder",
    );
    expect(profileScopeKey({ connectionId: "homelab", profile: "coder" })).toBe(
      "homelab::coder",
    );
    expect(profileScopeKey({ connectionId: "homelab" })).toBe(
      "homelab::default",
    );
  });
});
