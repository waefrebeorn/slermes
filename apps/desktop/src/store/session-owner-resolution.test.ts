...
describe("session owner topology", () => {
  it("fails closed while the modern registry bridge is present but its async cache is not loaded", () => {
    (window as unknown as { hermesDesktop?: unknown }).hermesDesktop = {
      connections: {
        list: vi.fn(async () => Promise.reject(new Error("ipc unavailable"))),
      },
    };
    $connectionsRegistry.set(null);
    ...
    expect(() =>
      assertSessionOwnerResolved("default", {
        method: "session.resume",
        sessionId: "registry-loading",
      }),
    ).not.toThrow();
    expect(() =>
      assertSessionOwnerResolved(null, {
        method: "session.resume",
        sessionId: "registry-loading",
      }),
  ...
  it("fails closed on an unknown owner in registry topology while preserving legacy profile routes", () => {
    ...
    expect(() =>
      assertSessionOwnerResolved(null, {
        method: "session.resume",
        sessionId: "legacy-single-profile",
      }),
    ).not.toThrow();
    ...
    expect(ambientGatewayOwnsEverySession()).toBe(false);
    expect(() =>
      assertSessionOwnerResolved("loki", {
        method: "session.resume",
        sessionId: "legacy-profile-owner",
      }),
    ).not.toThrow();
  });
});
