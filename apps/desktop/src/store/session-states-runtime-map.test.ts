...
describe("storedSessionIdForRuntimeId", () => {
  ...
  it("maps a runtime id to the stored id of the tile bound to it", () => {
    ...
    expect(storedSessionIdForRuntimeId("rt-developer")).toBe(
      "stored-developer",
    );
  ...
  it("ignores tiles with no runtime binding instead of matching undefined ids", () => {
    ...
    $sessionTiles.set([
      { storedSessionId: "stored-unbound" },
      { runtimeId: "rt-b", storedSessionId: "stored-b" },
    ]);
...
describe("knownOwnerForSession / requestForOwnedSession", () => {
  ...
  it("resolves a main-pane runtime id to its EXACT owner via the mirror + the hint, then the tagged row", () => {
    publishSessionState("rt-main", createClientSessionState("stored-main"));
    setSessionOwnerHint("stored-main", {
      connectionId: "local",
      profile: "omar",
    });

    expect(knownOwnerForSession("rt-main")).toEqual({
      connectionId: "local",
      profile: "omar",
    });
  ...
  it("fails closed with an explicit owner-resolution error instead of the ambient socket", async () => {
    ...
    expect(ambient).toHaveBeenCalledWith("approval.respond", {
      session_id: "rt-orphan",
    });
  });
  ...
  it("routes a connection-tagged orphan runtime through the owner its inbound event recorded (#97511)", () => {
    ...
    recordSessionEventScope({
      ...
      profile: "omar",
      session_id: "rt-unbound",
    });

    expect(knownOwnerForSession("rt-unbound")).toEqual({
      connectionId: "homelab",
      profile: "omar",
    });
    ...
    recordSessionEventScope({
      connectionId: "homelab",
      session_id: "rt-unprofiled",
    });
    expect(knownOwnerForSession("rt-unprofiled")).toEqual({
      connectionId: "homelab",
      profile: "default",
    });
  });
  ...
  it("still prefers the durable stored owner when a stale runtime ledger entry collides with a stored id (#97511)", () => {
    ...
    setSessionOwnerHint("stored-live", {
      connectionId: "local",
      profile: "omar",
    });
    recordSessionEventScope({
    ...
    expect(knownOwnerForSession("stored-live")).toEqual({
      connectionId: "local",
      profile: "omar",
    });
  });
  ...
  it("drops the recorded event owner together with the runtime state (#97511)", () => {
    recordSessionEventScope({
      connectionId: "homelab",
      profile: "omar",
      session_id: "rt-dropped",
    });
    expect(knownOwnerForSession("rt-dropped")).toEqual({
      connectionId: "homelab",
      profile: "omar",
    });
  ...
  it("answers an approval on a sole-local registry install through the primary socket (#96394)", async () => {
    ...
    const primaryRequest = vi.fn(async (method: string, params: unknown) => ({
      method,
      params,
      via: "primary",
    }));
