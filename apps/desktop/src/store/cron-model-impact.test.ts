...
async function waitForConfirmToast() {
  return vi.waitFor(() => {
    const toast = $notifications
      .get()
      .find((item) => item.id.startsWith("model-warning-confirm-"));
...
): ModelAssignmentResponse["cron_model_impact"] {
  return {
    available: true,
    guard_enabled: true,
    affected_count: 1,
...
describe("setMainModelAssignment", () => {
  it("shows one consumer warning and routes via a read-only review action", async () => {
    ...
    const notification = $notifications
      .get()
      .find((item) => item.id === CRON_MODEL_IMPACT_NOTIFICATION_ID);
  ...
  it("prompts and retries with confirm_expensive_model when the user accepts", async () => {
    ...
    setModelAssignment.mockResolvedValueOnce(
      response(positive("Confirmed job")),
    );

    const pending = setMainModelAssignment({
      provider: "openrouter",
      model: "openai/gpt-5.5-pro",
    });
    ...
    expect(
      $notifications
        .get()
        .some((item) => item.detail?.includes("Confirmed job")),
    ).toBe(true);
  });
  ...
  it("publishes only the latest same-profile assignment when responses reverse", async () => {
    ...
    const firstCall = setMainModelAssignment({
      provider: "nous",
      model: "first",
    });
    const secondCall = setMainModelAssignment({
      provider: "nous",
      model: "second",
    });
