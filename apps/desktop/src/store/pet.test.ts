...
describe("derivePetState", () => {
  ...
  it("waits (blocked on the user) above the in-flight signals", () => {
    expect(
      derivePetState({ awaitingInput: true, toolRunning: true, busy: true }),
    ).toBe("waiting");
    // but a finish beat still wins over waiting
    expect(derivePetState({ justCompleted: true, awaitingInput: true })).toBe(
      "wave",
    );
  });

  it("honors the full priority chain: error > celebrate > complete > tool", () => {
    expect(derivePetState({ error: true, celebrate: true, busy: true })).toBe(
      "failed",
    );
    expect(
      derivePetState({
    ...
    expect(derivePetState({ justCompleted: true, toolRunning: true })).toBe(
      "wave",
    );
  });
});
