...
const running = (id: string, command = `cmd ${id}`) => ({
  command,
  session_id: id,
  status: "running",
});
...
describe("reconcileBackgroundProcesses", () => {
  ...
  it("self-clears a failed task too, but only after a longer linger", () => {
    ...
    vi.advanceTimersByTime(5_000);
    expect(itemsOf("sess-fail").map((i) => [i.id, i.state])).toEqual([
      ["a", "failed"],
    ]);
...
describe("refreshBackgroundProcesses dead-session guard", () => {
  ...
  it("classifies a 4001 session-not-found as gone, and a timeout as transient", () => {
    expect(
      isSessionGoneForBackgroundPolling(new Error("session not found")),
    ).toBe(true);
    expect(
      isSessionGoneForBackgroundPolling(new Error("4001 session not found")),
    ).toBe(true);
    expect(
      isSessionGoneForBackgroundPolling(new Error("Session Not Found")),
    ).toBe(true);
    ...
    expect(
      isSessionGoneForBackgroundPolling(
        new Error("request timed out after 30s: process.list"),
      ),
    ).toBe(false);
    expect(isSessionGoneForBackgroundPolling(new Error("not connected"))).toBe(
      false,
    );
  });
  ...
  it("does not suppress a different, healthy session", async () => {
    ...
    const targets = request.mock.calls.map(
      (c) => (c[1] as { session_id?: string } | undefined)?.session_id,
    );
  ...
  it("keeps the row and reports failure when the gateway is disconnected", async () => {
    ...
    expect(items()).toEqual([
      expect.objectContaining({ id: "unreachable", state: "running" }),
    ]);
...
describe("refreshBackgroundProcesses dead-session guard hardenings", () => {
  ...
  it("matches the structured 4001 code, not a message substring, when a code is present", () => {
    ...
    expect(
      isSessionGoneForBackgroundPolling(
        new JsonRpcGatewayError("gone", { code: 4001 }),
      ),
    ).toBe(true);
    ...
    expect(
      isSessionGoneForBackgroundPolling(
        new JsonRpcGatewayError(
          "tool failed: upstream said session not found",
          { code: 5007 },
        ),
      ),
    ).toBe(false);
    ...
    expect(
      isSessionGoneForBackgroundPolling(
        new JsonRpcGatewayError("session not found"),
      ),
    ).toBe(true);
  });
