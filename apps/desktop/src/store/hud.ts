...
export const canUseHud = (): boolean =>
  ...
  typeof window.hermesDesktop?.hud?.open === "function";
...
export const toggleHud = (sessionId?: null | string) =>
  $hudActive.get() ? closeHud() : openHud(sessionId);
...
export const reportHudSession = (sessionId: null | string): void =>
  window.hermesDesktop?.hud?.setSession?.(sessionId);
...
export function watchHudState(
  onClosed?: (sessionId: null | string) => void,
): () => void {
