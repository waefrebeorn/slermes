...
const normalizeCwd = (cwd?: null | string): null | string =>
  cwd?.trim() || null;
...
export const $repoWorktrees: ReadableAtom<HermesGitWorktree[]> = computed(
  ...
  (byCwd, cwd) =>
    workspaceCwdBelongsToSelectedSession()
      ? (byCwd[normalizeCwd(cwd) ?? ""] ?? EMPTY_WORKTREES)
      : EMPTY_WORKTREES,
);
...
const statusAtomByCwd = new Map<
  string,
  ReadableAtom<HermesRepoStatus | null>
>();
...
export function repoStatusForCwd(
  cwd?: null | string,
): ReadableAtom<HermesRepoStatus | null> {
...
export function repoWorktreesForCwd(
  cwd?: null | string,
): ReadableAtom<HermesGitWorktree[]> {
  const key = normalizeCwd(cwd);
  ...
  if (!$slice) {
    $slice = computed(
      $repoWorktreesByCwd,
      (byCwd) => byCwd[key] ?? EMPTY_WORKTREES,
    );
...
export const $repoChangeByPath = computed(
  [$repoStatus, $currentCwd],
  (status, cwd) => {
    ...
    for (const file of status.files) {
      const kind: RepoChangeKind = file.conflicted
        ? "conflicted"
        : file.untracked
          ? "added"
          : "modified";
 ...
 */
export function repoChangeKindForPath(
  path: string,
): ReadableAtom<RepoChangeKind | undefined> {
...
export function registerRepoStatusCwd(
  cwd?: null | string,
): (() => void) | undefined {
  const key = normalizeCwd(cwd);
...
function setRepoWorktreesEntry(
  target: string,
...
const isCurrentSeq = (target: string, seq: number): boolean =>
  seqByCwd.get(target) === seq;
...
async function drainRepoStatusRefreshes(): Promise<void> {
  while (pendingByCwd.size > 0) {
    const [target, request] = pendingByCwd.entries().next().value as [
      string,
      RepoStatusRefreshRequest,
    ];
...
export async function openWorktreeDialog(options?: {
  ...
  const repoPath =
    options?.repoPath?.trim() || (await resolveWorktreeRepoPath());
