...
const updateManaged =
  vi.fn<(id: string) => Promise<DesktopManagedConnectionUpdateResult>>();

function managedResult(
  over: Partial<DesktopManagedConnectionUpdateResult> = {},
...
describe("runManagedUpdate", () => {
  it("routes through the managed drain/update/restore bridge and lands on updated with the receipt", async () => {
    ...
    expect($managedUpdates.get()["linux-ssh"]).toMatchObject({
      status: "updating",
    });
...
describe("isManagedUpdateBusyMessage", () => {
  it("recognizes every busy-gate phrasing and rejects unrelated failures", () => {
    expect(
      isManagedUpdateBusyMessage("A managed update is already in progress."),
    ).toBe(true);
    expect(isManagedUpdateBusyMessage("code managed-update-in-progress")).toBe(
      true,
    );
