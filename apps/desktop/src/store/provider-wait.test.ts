...
describe("providerWaitText", () => {
  ...
  it("still accepts classic wait frames and rejects spinner noise", () => {
    expect(
      providerWaitText("⏳ waiting on local-model — 30s with no output yet"),
    ).not.toBe("");
...
describe("parseModelLoadWait", () => {
  ...
  it("returns null for every other wait frame", () => {
    expect(
      parseModelLoadWait("⏳ waiting on qwen — 30s with no output yet"),
    ).toBeNull();
    expect(
      parseModelLoadWait(
  ...
  it("clamps out-of-range percents", () => {
    expect(parseModelLoadWait("⏳ loading m into memory — 999%")?.percent).toBe(
      100,
    );
  });
});
