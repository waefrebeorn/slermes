...
vi.mock("@/hermes", () => ({
  ...
  setSessionUnreadRemote: (
    ...
    unread: boolean,
    profile?: null | string,
  ) => setUnreadRemote(id, unread, profile),
}));
...
describe("composer model persistence scope", () => {
  ...
  it("keeps inferred local-primary connections on the historical bare keys", () => {
    ...
    expect(window.localStorage.getItem("hermes.desktop.composer.model")).toBe(
      "next-model",
    );
    expect(
      window.localStorage.getItem(
        "hermes.desktop.composer.model.registry.local.default",
      ),
    ).toBeNull();
  });
...
describe("session owner hints", () => {
  ...
  it("preserves the exact registry owner for session-scoped RPC routing", () => {
    ...
    expect(
      knownSessionOwner(
        [session({ id: "remote-session", profile: "default" })],
        "remote-session",
      ),
    ).toEqual(route);
  });
  ...
  it("bounds owner hints and evicts the oldest scoped identity", () => {
    ...
    expect(getSessionOwnerHint("bounded-256", scope)).toMatchObject({
      connectionId: "bounded-source",
    });
  });
  ...
  it("survives a relaunch: hints are persisted and rehydrated in LRU order", () => {
    ...
    for (let index = 0; index < 255; index += 1) {
      setSessionOwnerHint(`filler-${index}`, {
        connectionId: "filler",
        profile: "p",
      });
    }
  ...
  it("ignores malformed persisted entries and never throws on hydrate", () => {
    ...
    expect(getSessionOwnerHint("good")).toEqual({
      connectionId: "local",
      profile: "omar",
    });
  ...
  it("forgets every hint naming a removed connection, in memory and on disk", () => {
    ...
    expect(getSessionOwnerHint("stored-c")).toEqual({
      connectionId: "local",
      profile: "omar",
    });
  ...
  it("forgets every route for one session without disturbing other sessions", () => {
    ...
    expect(getSessionOwnerHint("healthy")).toMatchObject({
      connectionId: "remote-a",
    });
  ...
  it("pins only connection-tagged rows and leaves primary SSH rows ambient", () => {
    expect(
      sessionOwnerRouteFromRow(
        session({ connection_id: "source-a", profile: "worker" }),
      ),
    ).toEqual({
      connectionId: "source-a",
      profile: "worker",
    ...
    expect(
      sessionOwnerRouteFromRow(session({ profile: "default" })),
    ).toBeUndefined();
    expect(
      sessionOwnerRouteFromRow(
        session({ connection_id: "  ", profile: "default" }),
      ),
    ).toBeUndefined();
  });
});
...
describe("knownSessionOwner", () => {
  ...
  it("returns the EXACT route for a connection-tagged row, the bare profile otherwise", () => {
    ...
    expect(knownSessionOwner(rows, "tagged")).toEqual({
      connectionId: "local",
      profile: "omar",
    });
...
describe("computed $attentionSessionIds", () => {
  ...
  it("reflects sessions with needsInput=true and a storedSessionId", () => {
    ...
    publishSessionState("rt2", {
      ...createClientSessionState("s2"),
      needsInput: false,
    });
...
describe("resolveComposerSessionKey", () => {
  it("keeps the lineage root across compression tip rotation", () => {
    ...
    expect(
      resolveComposerSessionKey(tipBefore, [session({ id: tipBefore })]),
    ).toBe(tipBefore);
...
describe("shouldMigrateComposerScope", () => {
  ...
  it("blocks cross-session migrate when route flipped but store selection lags", () => {
    ...
    expect(shouldMigrateComposerScope("root-a", "root-b", sessions)).toBe(
      false,
    );
  ...
  it("is a no-op for identical or missing keys", () => {
    ...
    expect(shouldMigrateComposerScope("root-a", "root-a", sessions)).toBe(
      false,
    );
...
describe("mergeSessionPage", () => {
  it("carries the owning connection onto a row that comes back untagged (local registry source)", () => {
    ...
    const incoming = [
      session({ id: "omar-1", last_active: 5, profile: "omar" }),
    ];
  ...
  it("drops the carried tag when the refreshed row names a different profile, and never overrides an incoming tag", () => {
    ...
    expect(merged.find((s) => s.id === "foreign")?.connection_id).toBe(
      "homelab",
    );
  });
  ...
  it("never stitches one profile twin onto another: same id in two profiles stays two sessions (#92454)", () => {
    ...
    expect(merged.map((s) => `${s.profile}:${s.id}`).sort()).toEqual([
      "quietbot:twin",
      "testbot:twin",
    ]);
  ...
  it("a kept twin in another profile survives the incoming page dedupe (#92454)", () => {
    ...
    const incoming = [
      session({ id: "twin", message_count: 2, profile: "testbot" }),
    ];
  ...
  it("never resurrects a session the server dropped that is not in the keep set", () => {
    ...
    expect(
      mergeSessionPage(previous, incoming, ["b"]).map((s) => s.id),
    ).toEqual(["b"]);
  });
  ...
  it("never regresses last_active behind an optimistic user-send bump", () => {
    ...
    const incoming = [
      session({ id: "old", last_active: 100, message_count: 4 }),
    ];
  ...
  it("carries an optimistic last_active across a compression tip rotation", () => {
    const previous = [
      session({ id: "tip-4", _lineage_root_id: "root", last_active: 9_000 }),
    ] as SessionInfo[];
    const incoming = [
      session({ id: "tip-5", _lineage_root_id: "root", last_active: 50 }),
    ] as SessionInfo[];
  ...
  it("keeps a survivor whose optimistic last_active outranks the whole page on top", () => {
    ...
    const previous = [
      session({ id: "typing", last_active: 900 }),
      session({ id: "settled", last_active: 100 }),
    ];

    const incoming = [
      session({ id: "settled", last_active: 100, message_count: 3 }),
    ];
  ...
  it("falls back to started_at for survivors that have no last_active yet", () => {
    ...
    const incoming = [
      session({ id: "other", last_active: 400, message_count: 2 }),
    ];
  ...
  it("interleaves against the title-preserving merged rows, not the raw incoming page", () => {
    ...
    const previous = [
      session({ id: "survivor", last_active: 200 }),
      session({ id: "bumped", last_active: 300 }),
    ];

    const incoming = [
      session({ id: "bumped", last_active: 100, message_count: 2 }),
    ];
...
describe("carryForwardFailedProfileSessions", () => {
  it("is a no-op when the backend reported no profile errors", () => {
    ...
    expect(
      carryForwardFailedProfileSessions(previous, incoming, undefined),
    ).toBe(incoming);
    expect(carryForwardFailedProfileSessions(previous, incoming, [])).toBe(
      incoming,
    );
  });
  ...
  it("re-attaches idle rows for a profile whose slice failed (empty 200 + errors)", () => {
    ...
    const carried = carryForwardFailedProfileSessions(
      previous,
      [],
  ...
  it("does not resurrect a successful profile’s omitted rows, and does not duplicate", () => {
    ...
    const incoming = [
      session({ id: "home-fresh", message_count: 4, profile: "default" }),
    ];

    const carried = carryForwardFailedProfileSessions(previous, incoming, [
      { profile: "work" },
    ]);

    expect(carried.map((s) => `${s.profile}:${s.id}`)).toEqual([
      "default:home-fresh",
      "work:work-old",
    ]);
  });
  ...
  it("re-ranks carried rows by recency instead of parking them at the tail", () => {
    ...
    const incoming = [
      session({ id: "home", last_active: 100, profile: "default" }),
    ];

    expect(
      carryForwardFailedProfileSessions(previous, incoming, [
        { profile: "work" },
      ]).map((s) => s.id),
...
describe("keepFailedProfileMeta", () => {
  ...
  it("restores previous usage/truncated flags for profiles whose slice failed", () => {
    const previous = {
      ...
      work: { cost_usd: 1, tokens: 10 },
    };
    const incoming = { work: { cost_usd: 2, tokens: 20 } };

    expect(
      keepFailedProfileMeta(previous, incoming, [{ profile: "default" }]),
    ).toEqual({
      default: { cost_usd: 4, tokens: 40 },
...
describe("workspaceCwdForNewSession", () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem(
      "hermes.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default",
    );
  ...
  it("does not publish a delayed sanitized cwd after ownership is lost", async () => {
    ...
    const seeding = ensureDefaultWorkspaceCwd(() => ownsSwitch);
    await vi.waitFor(() =>
      expect(getConfiguredDefaultProjectDir()).toBe("/configured"),
    );
    ownsSwitch = false;
  ...
  it("uses encodeURIComponent so profile names with reserved chars are isolated", () => {
    ...
    expect(
      localStorage.getItem(
        "hermes.desktop.lastSessionId.profile.research%2Fops",
      ),
    ).toBe("ops-session");
...
describe("remembered route (per profile)", () => {
  ...
  it("uses encodeURIComponent so profile names with reserved chars are isolated", () => {
    ...
    expect(
      localStorage.getItem("hermes.desktop.lastRoute.profile.research%2Fops"),
    ).toBe("/cron");
    expect(getRememberedRoute("research")).toBeNull();
  });
  ...
  it("route and session id agree on the owner, so restore cannot cross profiles", () => {
    ...
    const owner = rememberedSessionProfile(
      [session({ id: "stored-1", profile: "ai-engineer" })],
      "stored-1",
      "default",
    );
...
describe("sessionBelongsToProfile", () => {
  it("validates that a session row matches a stored id and target profile", () => {
    ...
    // Missing session.
    expect(sessionBelongsToProfile(sessions, "s-missing", "ai-engineer")).toBe(
      false,
    );
  });
  ...
  it("matches on lineage root so compressed tips validate their owner", () => {
    const sessions = [
      session({ id: "tip-2", _lineage_root_id: "root-1", profile: "work" }),
    ];
  ...
  it("normalizes blank/empty profiles to default", () => {
    const sessions = [
      session({ id: "s1", profile: "" }),
      session({ id: "s2", profile: null as unknown as string }),
    ];
...
describe("rememberedSessionProfile", () => {
  it("keys by the session row owning profile, not the active one", () => {
    ...
    expect(rememberedSessionProfile(sessions, "stored-1", "default")).toBe(
      "ai-engineer",
    );
  });
  ...
  it("matches on the lineage root so a compressed tip resolves its owner", () => {
    const sessions = [
      session({ _lineage_root_id: "root-1", id: "tip-2", profile: "work" }),
    ];

    expect(rememberedSessionProfile(sessions, "root-1", "default")).toBe(
      "work",
    );
  });

  it("falls back to the active profile for a session not yet in the list", () => {
    expect(rememberedSessionProfile([], "uncached", "research")).toBe(
      "research",
    );
  });
  ...
  it("consults the owner hint for a hidden session absent from the list (Bot Chat 4001 class)", () => {
    ...
    setSessionOwnerHint("hidden-bot-chat", {
      connectionId: "local",
      mode: "local",
      profile: "developer",
    });

    expect(rememberedSessionProfile([], "hidden-bot-chat", "default")).toBe(
      "developer",
    );
  });
  ...
  it("prefers the routed targetProfile over the route profile in the hint fallback", () => {
    ...
    expect(rememberedSessionProfile([], "hidden-remote-chat", "default")).toBe(
      "clippy",
    );
  });

  it("still prefers a real session row over the hint", () => {
    setSessionOwnerHint("rowed-session", {
      connectionId: "local",
      profile: "wrong-hint",
    });
    const sessions = [session({ id: "rowed-session", profile: "right-owner" })];

    expect(rememberedSessionProfile(sessions, "rowed-session", "default")).toBe(
      "right-owner",
    );
  });
