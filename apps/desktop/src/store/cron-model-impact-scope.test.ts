...
describe("cron model impact backend identity", () => {
  it("survives disconnects and reminted websocket tickets but invalidates for another backend", () => {
    syncCronModelImpactConnection(
      connection("https://one.example", "wss://one.example?ticket=first"),
    );
    ...
    syncCronModelImpactConnection(
      connection("https://one.example", "wss://one.example?ticket=second"),
    );
    expect(getCronModelImpactScope()).toEqual(first);

    syncCronModelImpactConnection(
      connection("https://two.example", "wss://two.example?ticket=third"),
    );
  ...
  it("treats reminted SSH tunnel ports as the same remote backend", () => {
    ...
    syncCronModelImpactConnection(
      connection("http://127.0.0.1:41001", "ws://127.0.0.1:41001", ssh),
    );
    const first = getCronModelImpactScope();

    syncCronModelImpactConnection(
      connection("http://127.0.0.1:52002", "ws://127.0.0.1:52002", ssh),
    );
