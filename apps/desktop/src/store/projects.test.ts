...
describe("project scope", () => {
  ...
  it("persists the scope to localStorage", () => {
    enterProject("p_abc");
    expect(window.localStorage.getItem("hermes.desktop.projectScope")).toBe(
      "p_abc",
    );
  });
});
...
describe("projectNameForCwd", () => {
  const treeNode = (
    over: Partial<SidebarProjectTree> &
      Pick<SidebarProjectTree, "id" | "label">,
  ): SidebarProjectTree => ({
    path: null,
    repos: [],
    sessionCount: 0,
  ...
  it("matches nested Windows paths across separator and case differences", () => {
    $projectTree.set([
      treeNode({ id: "p_win", label: "Windows app", path: "C:\\Repos\\App" }),
    ]);
...
describe("pickProjectFolder", () => {
  ...
  it("seeds the picker with the backend cwd on a remote gateway", async () => {
    isDesktopFsRemoteMode.mockReturnValue(true);
    desktopDefaultCwd.mockResolvedValue({
      branch: "main",
      cwd: "/backend/work",
    });
...
describe("createProject", () => {
  ...
  it("marks the backend stale and surfaces a friendly error when projects.create is missing", async () => {
    activeGateway.mockReturnValue({
      ...
      request: vi
        .fn()
        .mockRejectedValue(new Error("unknown method: projects.create")),
    } as never);

    await expect(
      createProject({ folders: ["/srv/demo"], name: "Demo" }),
...
describe("projects RPC capability", () => {
  ...
  it("marks the backend stale when projects.list is missing", async () => {
    activeGateway.mockReturnValue({
      ...
      request: vi
        .fn()
        .mockRejectedValue(new Error("unknown method: projects.list")),
    } as never);
  ...
  it("does not publish a late project list from the previous source", async () => {
    ...
    const gatewayB = {
      ...
      request: vi
        ...
        .mockResolvedValue({
          active_id: null,
          projects: [{ id: "source-b", name: "Source B" }],
        }),
    };
    ...
    resolveA?.({
      active_id: null,
      projects: [{ id: "source-a", name: "Source A" }],
    });
    await pendingA;
  ...
  it("blocks opening the create dialog once the backend is known stale", () => {
    ...
    expect(notify).toHaveBeenCalledWith(
      expect.objectContaining({
        kind: "warning",
        message: "sidebar.projects.staleBackend",
      }),
    );
  });
});
...
describe("repository discovery policy", () => {
  ...
  it("passes custom roots and exclusions to Electron and records on the origin gateway", async () => {
    ...
    const scanRepos = vi
      .fn()
      .mockResolvedValue([{ label: "repo", root: "/work/repo" }]);
    desktopGit.mockReturnValue({ scanRepos } as never);
    getHermesConfig.mockResolvedValue({
      desktop: {
  ...
  it("does not scan the local filesystem for remote connections but still refreshes the project tree", async () => {
    ...
    // refresh again.
    expect(request).toHaveBeenCalledWith("projects.discover_repos", {
      profile: "default",
      scan: true,
    });
    expect(request).toHaveBeenCalledWith(
  ...
  it("records repos under the profile the scan started with, not one focused mid-scan", async () => {
    const { promise: scanResult, resolve: resolveScan } =
      deferred<Array<{ label: string; root: string }>>();
    ...
    expect(request).not.toHaveBeenCalledWith(
      ...
      expect.objectContaining({ profile: "coder" }),
    );
    expect($projectTree.get()).toEqual([]);
  });
});
...
describe("project tree profile isolation", () => {
  ...
  it("retries a dropped projects.tree request once on the active gateway", async () => {
    const request = vi
      ...
      .mockRejectedValueOnce(
        new Error("request timed out after 30s: projects.tree"),
      )
  ...
  it("does not publish a late projects.list response from the previous profile", async () => {
    const { promise: defaultResponse, resolve: resolveDefault } =
      deferred<unknown>();
