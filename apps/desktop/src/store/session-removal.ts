...
export function untombstoneSessions(
...
export const beginSessionMutation = (
  ids: Array<null | string | undefined>,
): void => mutateInFlight(ids, true);
export const endSessionMutation = (
  ids: Array<null | string | undefined>,
): void => mutateInFlight(ids, false);
...
export function isSessionRemovalPending(
  ...
  return (
    $removedSessionIds.get().has(id) || $sessionMutationsInFlight.get().has(id)
  );
}
