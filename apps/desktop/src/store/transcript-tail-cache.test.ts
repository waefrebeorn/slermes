...
const msg = (id: string, chars = 10): ChatMessage =>
  ({
    id,
    parts: [{ text: "x".repeat(chars), type: "text" }],
    role: "assistant",
  }) as never;
...
describe("transcript tail cache", () => {
  ...
  it("self-evicts a corrupt entry instead of returning garbage", () => {
    window.localStorage.setItem(
      "hermes.transcript-tail.v2:sess-bad",
      "{not json",
    );

    expect(loadTranscriptTail("sess-bad")).toBeNull();
    expect(
      window.localStorage.getItem("hermes.transcript-tail.v2:sess-bad"),
    ).toBeNull();
  });
  ...
  it("repairs a poisoned persisted tail carrying a duplicate toolCallId (#87857)", () => {
    ...
    window.localStorage.setItem(
      "hermes.transcript-tail.v2:sess-poisoned",
  ...
  describe("profile scoping (#94828)", () => {
    it("never returns a tail cached under another profile", () => {
      ...
      expect(
        loadTranscriptTail("sess-x", { profile: "ai-energy" }),
      ).toHaveLength(1);
    ...
    it("keeps same-id tails on different connections distinct", () => {
      saveTranscriptTail("sess-y", [msg("local-row")], {
        connectionId: "local",
        profile: "default",
      });
      saveTranscriptTail("sess-y", [msg("remote-row")], {
        connectionId: "conn:mimir",
        profile: "default",
      });

      expect(
        loadTranscriptTail("sess-y", {
          connectionId: "local",
          profile: "default",
        })?.[0].id,
      ).toBe("local-row");
    ...
    it("delete-path everywhere-drop clears every scope of the id even when scopes drifted", () => {
      ...
      saveTranscriptTail("sess-gone", [msg("routed-row")], {
      ...
      expect(
        loadTranscriptTail("sess-kept", { profile: "ops" }),
      ).not.toBeNull();
    });
    ...
    it("purges pre-scoping v1 entries so a stale tail can never paint again", async () => {
      ...
      window.localStorage.setItem(
        "hermes.transcript-tail.v1:sess-legacy",
        JSON.stringify(stale),
      );
      window.localStorage.setItem(
        "hermes.transcript-tail.v1-index",
        JSON.stringify(["sess-legacy"]),
      );
      ...
      try {
        ...
        expect(
          fresh.loadTranscriptTail("sess-legacy", { profile: "ai-energy" }),
        ).toBeNull();
        expect(
          window.localStorage.getItem("hermes.transcript-tail.v1:sess-legacy"),
        ).toBeNull();
        expect(
          window.localStorage.getItem("hermes.transcript-tail.v1-index"),
        ).toBeNull();
      } finally {
        vi.resetModules();
      }
