...
describe("mcpServerFromToolName", () => {
  it("extracts the server from a namespaced MCP tool", () => {
    expect(mcpServerFromToolName("mcp__figma__get_design_context")).toBe(
      "figma",
    );
    expect(mcpServerFromToolName("mcp__browsermcp__browser_navigate")).toBe(
      "browsermcp",
    );
  });

  it("handles multi-underscore server names", () => {
    expect(mcpServerFromToolName("mcp__comfy_cloud__generate")).toBe(
      "comfy_cloud",
    );
  });
