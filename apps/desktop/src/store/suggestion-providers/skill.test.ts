...
const hits = (name: string, draft: string) =>
  skillHit(skillPattern(name), draft.toLowerCase());
...
describe("collidesWithWorkspace", () => {
  it("suppresses a skill named exactly like the cwd folder", () => {
    expect(
      collidesWithWorkspace("hermes-agent", "/Users/b/www/hermes-agent"),
    ).toBe(true);
  });
  ...
  it("suppresses inside worktree-suffixed folders too", () => {
    expect(
      collidesWithWorkspace(
        "hermes-agent",
        "/Users/b/www/hermes-agent-suggest",
      ),
    ).toBe(true);
  });
  ...
  it("does not suppress on substring-only overlap", () => {
    ...
    expect(collidesWithWorkspace("perf", "/Users/b/www/perfect-app")).toBe(
      false,
    );
    expect(collidesWithWorkspace("clean", "/Users/b/www/hermes-agent")).toBe(
      false,
    );
  });
...
describe("skillTouchedInMessages", () => {
  it("detects a skill_view load of the skill", () => {
    expect(
      skillTouchedInMessages("pr-ready", [
        toolCall("skill_view", { name: "pr-ready" }),
      ]),
    ).toBe(true);
  });
  ...
  it("detects a skill_manage touch of the skill", () => {
    expect(
      skillTouchedInMessages("pr-ready", [
        toolCall("skill_manage", { action: "patch", name: "pr-ready" }),
      ]),
    ).toBe(true);
  });
  ...
  it("matches qualified skill names (category/name, plugin:name)", () => {
    expect(
      skillTouchedInMessages("hermes-agent-dev", [
        toolCall("skill_view", { name: "github/hermes-agent-dev" }),
      ]),
    ).toBe(true);
    expect(
      skillTouchedInMessages("writing-plans", [
        toolCall("skill_view", { name: "superpowers:writing-plans" }),
      ]),
    ).toBe(true);
  });
  ...
  it("falls back to argsText when args were not parsed", () => {
    expect(
      skillTouchedInMessages("pr-ready", [
        toolCall("skill_view", undefined, '{"name":"pr-ready"}'),
      ]),
    ).toBe(true);
  });
  ...
  it("detects the user loading the skill via its slash command", () => {
    expect(
      skillTouchedInMessages("pr-ready", [
        userText("/pr-ready check this branch"),
      ]),
    ).toBe(true);
    expect(skillTouchedInMessages("pr-ready", [userText("/pr-ready")])).toBe(
      true,
    );
  });
  ...
  it("ignores touches of OTHER skills and non-skill tools", () => {
    expect(
      skillTouchedInMessages("pr-ready", [
        toolCall("skill_view", { name: "clean" }),
      ]),
    ).toBe(false);
    expect(
      skillTouchedInMessages("pr-ready", [
        toolCall("read_file", { path: "pr-ready" }),
      ]),
    ).toBe(false);
    // Slash prefix must be exact — /pr-ready-extra is a different command.
    expect(
      skillTouchedInMessages("pr-ready", [userText("/pr-ready-extra go")]),
    ).toBe(false);
    // Merely mentioning the name in prose is not a load.
    expect(
      skillTouchedInMessages("pr-ready", [userText("is pr-ready any good?")]),
    ).toBe(false);
  });
