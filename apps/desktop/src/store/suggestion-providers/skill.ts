...
export function collidesWithWorkspace(name: string, cwd: string): boolean {
  return new RegExp(
    `(?<![\\p{L}\\p{N}])${escape(name.toLowerCase())}(?![\\p{L}\\p{N}])`,
    "u",
  ).test(cwd.toLowerCase());
}
 ...
 *  Exported for tests. */
export function skillTouchedInMessages(
  skillName: string,
...
function toSuggestion(name: string): ComposerSuggestion {
  const copy = (key: string, ...args: unknown[]) =>
    translateNow(`composer.skillSuggestions.${key}`, ...args);
...
registerDraftProvider("skill", async ({ sessionId, text }) => {
  ...
  const matched = skills.filter(
    (skill) =>
      skillHit(skill.pattern, haystack) &&
      !collidesWithWorkspace(skill.name, cwd),
  );
  ...
  return matched
    .filter((skill) => !skillTouchedInMessages(skill.name, transcript))
