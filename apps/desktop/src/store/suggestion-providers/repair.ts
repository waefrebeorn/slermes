...
async function reconnect(
  ...
  try {
    ...
    if (!(error instanceof McpOAuthCancelled)) {
      notifyError(
        error,
        translateNow("composer.repairSuggestions.failed", prettyName(server)),
      );
    }
...
function toSuggestion(
  ...
  const copy = (key: string, ...args: unknown[]) =>
    translateNow(`composer.repairSuggestions.${key}`, ...args);
  ...
  return {
    ...
    id: server,
    invoke: (context) =>
      reconnect(server, context.sessionId ?? sessionId, context.cancelled),
...
export function clearMcpFailure(
  sessionId: string | null | undefined,
  server: string,
): void {
