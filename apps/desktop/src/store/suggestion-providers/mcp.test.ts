...
describe("matchSuggestions", () => {
  ...
  it("is case-insensitive against the draft", () => {
    expect(matchSuggestions("open FIGMA please", INDEX)).toEqual([
      { keyword: "figma", server: "figma" },
    ]);
  });

  it("caps the number of suggestions", () => {
    const matches = matchSuggestions(
      "linear ticket for the figma design in unreal",
      INDEX,
    );
  ...
  it("one suggestion per server even when several keywords hit", () => {
    expect(matchSuggestions("a linear ticket", INDEX)).toEqual([
      { keyword: "linear", server: "linear" },
    ]);
  });
  ...
  it("handles regex metacharacters in keywords safely", () => {
    ...
    expect(matchSuggestions("help with c++ code", index)).toEqual([
      { keyword: "c++", server: "cpp-tools" },
    ]);
  });

  it("matches a pasted vendor URL by host suffix", () => {
    const index = [
      { hosts: ["atlassian.net"], keywords: ["jira"], server: "atlassian" },
    ];

    expect(
      matchSuggestions(
        "look at https://yourco.atlassian.net/browse/ENG-123 pls",
        index,
      ),
    ).toEqual([{ keyword: "atlassian.net", server: "atlassian" }]);
  });
  ...
  it("host hit wins over keyword hit as the reported trigger", () => {
    ...
    expect(
      matchSuggestions(
        "linear ticket: https://linear.app/team/issue/ABC-1",
        index,
      ),
    ).toEqual([{ keyword: "linear.app", server: "linear" }]);
  });
  ...
  it("does not match a host suffix embedded in another domain", () => {
    ...
    expect(
      matchSuggestions("see https://linear.app.example.com/x", index),
    ).toEqual([]);
  ...
  it("strips port and credentials before host comparison", () => {
    ...
    expect(
      matchSuggestions(
        "logs at https://user@myorg.sentry.io:443/issues",
        index,
      ),
    ).toEqual([{ keyword: "sentry.io", server: "sentry" }]);
  });
  ...
  it("a pasted URL fires even as the last thing in the draft", () => {
    ...
    expect(
      matchSuggestions("look at https://linear.app/team/issue/ABC-1", index),
    ).toEqual([{ keyword: "linear.app", server: "linear" }]);
  });
