...
const clearAttr = () =>
  document.documentElement.hasAttribute("data-hermes-clear");
const keep = () =>
  document.documentElement.style.getPropertyValue("--translucency-glass-keep");
...
describe("window translucency lever", () => {
  ...
  it("mirrors every tick to the desktop bridge so the clear-mode fade tracks the drag", () => {
    ...
    window.hermesDesktop = {
      setTranslucency: (payload: unknown) => calls.push(payload),
    } as never;
...
describe("frost and area", () => {
  ...
  it("publishes the area as an attribute only while glass is on", () => {
    ...
    if (!GLASS_SUPPORTED) {
      expect(
        document.documentElement.hasAttribute("data-hermes-glass-scope"),
      ).toBe(false);
    ...
    setTranslucencyMode("glass");
    expect(
      document.documentElement.getAttribute("data-hermes-glass-scope"),
    ).toBe("sidebar");
...
describe("translucency peek", () => {
  it("stays open until every overlapping hold has ended", () => {
    ...
    beginTranslucencyPeek();
    expect(
      document.documentElement.hasAttribute("data-hermes-translucency-peek"),
    ).toBe(true);
...
describe("cross-window sync", () => {
  ...
  it("adopts a sibling window's persisted state", () => {
    ...
    window.dispatchEvent(
      new StorageEvent("storage", { key: KEY, newValue: "x" }),
    );
...
describe("glass is confined to chat windows", () => {
  ...
  it("does not thin surfaces in a special-purpose window", () => {
    ...
    expect($translucency.get().mode).toBe("glass");
    expect(document.documentElement.hasAttribute("data-hermes-glass")).toBe(
      false,
    );
  });
  ...
  it("still publishes the live setting and the tint to a special-purpose window", () => {
    ...
    expect(document.documentElement.hasAttribute("data-hermes-glass-on")).toBe(
      true,
    );
    expect(
...
describe("per-appearance settings", () => {
  ...
  it("carries an unset key over from the shared base", () => {
    $translucencyBook.set({
      mode: "glass",
      base: { intensity: 44, scope: "sidebar" },
      light: {},
      dark: {},
    });
...
describe("v1 → v2 migration", () => {
  ...
  it("prefers a v2 book over a stale v1 state", async () => {
    ...
    window.localStorage.setItem(
      LEGACY_KEY,
      JSON.stringify({ intensity: 25, mode: "clear" }),
    );
    window.localStorage.setItem(
      KEY,
      JSON.stringify({
