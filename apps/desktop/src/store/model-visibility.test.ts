...
describe("model visibility", () => {
  it("keeps newly configured providers visible when stored choices are stale", () => {
    ...
    expect(
      visible.has(modelVisibilityKey("copilot", "claude-sonnet-4.6")),
    ).toBe(true);
    expect(
      visible.has(modelVisibilityKey("local-ollama", "qwen3:latest")),
    ).toBe(true);
    expect(
      visible.has(modelVisibilityKey("local-ollama", "llama3.2:latest")),
    ).toBe(true);
  });
  ...
  it("preserves hidden-provider sentinel without re-adding defaults", () => {
    ...
    expect(
      visible.has(modelVisibilityKey("nous", "hermes-3-llama-3.1-70b")),
    ).toBe(false);
    expect(
      visible.has(modelVisibilityKey("nous", "hermes-3-llama-3.1-8b")),
    ).toBe(false);
    ...
    expect(visible.has(modelVisibilityKey("ollama", "qwen3:latest"))).toBe(
      true,
    );
  });
  ...
  it("resolveVisibleKeys preserves sentinels that effectiveVisibleKeys strips", () => {
    ...
    expect(resolved.has(modelVisibilityKey("ollama", "qwen3:latest"))).toBe(
      true,
    );

    // Display variant drops the sentinel.
    expect(
      effectiveVisibleKeys(stored, providers).has(
        emptyProviderSentinelKey("nous"),
      ),
    ).toBe(false);
  });
...
describe("toggleModelVisibility", () => {
  ...
  it("records a hide-all sentinel when the last model of a provider is toggled off", () => {
    ...
    expect(
      effectiveVisibleKeys(stored, providers).has(
        modelVisibilityKey("openai", "gpt-a"),
      ),
    ).toBe(false);
    expect(
      effectiveVisibleKeys(stored, providers).has(
        modelVisibilityKey("openai", "gpt-b"),
      ),
    ).toBe(false);
  });
  ...
  it("tolerates a provider with zero models (defensive — dialog filters these out)", () => {
    ...
    const next = toggleModelVisibility(
      new Set([modelVisibilityKey("openai", "gpt-a")]),
      ps,
      "empty",
      "ghost",
    );
    ...
    expect(
      [...next].some((k) => k.startsWith("empty::") && !isProviderSentinel(k)),
    ).toBe(true);
    expect(next.has(modelVisibilityKey("openai", "gpt-a"))).toBe(true);
  });
});
...
describe("resolveVisibleKeys", () => {
  ...
  it("returns the curated defaults verbatim for null stored", () => {
    expect(resolveVisibleKeys(null, providers)).toEqual(
      defaultVisibleKeys(providers),
    );
  });
...
describe("featured defaults", () => {
  ...
  it("defaults to the featured shortlist when a provider publishes one", () => {
    ...
    expect(visible.has(modelVisibilityKey("nous", "anthropic/opus"))).toBe(
      true,
    );
    ...
    expect(visible.has(modelVisibilityKey("nous", "anthropic/haiku"))).toBe(
      false,
    );
  });
  ...
  it("falls back to top-N when a provider ships no featured list", () => {
    ...
    expect(visible.has(modelVisibilityKey("ollama", "llama3.2:latest"))).toBe(
      true,
    );
  });

  it("ignores an empty featured list and falls back to top-N", () => {
    const plain = featuredProvider(
      "ollama",
...
describe("setProviderVisibility", () => {
  ...
  it("round-trips: enable then disable returns to a clean hidden-all", () => {
    ...
    expect(
      [...disabled].some(
        (k) => k.startsWith("openai::") && !isProviderSentinel(k),
      ),
    ).toBe(false);
  });
