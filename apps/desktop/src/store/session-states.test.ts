...
describe("resetTileRuntimeBindings", () => {
  ...
  it("invalidates the delegate wiring cache AND drops tile runtime ids (sleep/wake reconnect)", () => {
    ...
    $sessionTiles.set([
      { runtimeId: "runtime-dead", storedSessionId: "stored-a" },
    ]);
    resetTileRuntimeBindings();
  ...
  it("rebinds only the restarted connection while preserving other Bot gateways", () => {
    ...
    const [barryBot, barrySibling, workBot, ordinarySession] =
      $sessionTiles.get();
    ...
    expect(workBot).toMatchObject({
      runtimeId: "runtime-work-live",
      storedSessionId: "stored-work-bot",
    });
    expect(ordinarySession).toMatchObject({
      storedSessionId: "stored-session",
    });
    expect(ordinarySession).not.toHaveProperty("runtimeId");
    expect(invalidateRuntimeBindings).toHaveBeenCalledWith(
      new Set(["stored-barry-sibling-bot", "stored-work-bot"]),
    );
  });
  ...
  it("keeps an owner-routed SESSIONS tile (branch child) bound across an unrelated reconnect", () => {
    ...
    $sessionTiles.set([
      {
        ownerRoute: {
          connectionId: "100-125-133-71-9119",
          mode: "remote",
          profile: "default",
        },
...
describe("SessionTile workspace scope", () => {
  ...
  it("persists a sessions-mode owner route so a branch child tile pins its owning socket", () => {
    ...
    openSessionTile("branch-child", "center", undefined, null, {
      ownerRoute,
      workspaceMode: "sessions",
    });
  ...
  it("keeps an existing sessions-mode owner route on a route-less re-scope", () => {
    ...
    expect($sessionTiles.get()).toEqual([
      expect.objectContaining({ ownerRoute, storedSessionId: "branch-child" }),
    ]);
  });
  ...
  it("allows a Bot-scoped tab when the same stored session is hidden in Sessions main", () => {
    ...
    $layoutTree.set(
      group(["workspace", tilePane("bot-chat")], {
        active: "workspace",
        id: "main",
      }),
    );
  ...
  it("fronts the existing tab when compaction rotated the tip id — never a duplicate", () => {
    ...
    setSessions([
      {
        _lineage_ids: ["seg-1", "seg-2", "seg-3"],
        _lineage_root_id: "seg-1",
        id: "seg-3",
      } as never,
    ]);
    openSessionTile("seg-2");
    $layoutTree.set(
      group(["workspace", tilePane("seg-2")], {
        active: "workspace",
        id: "main",
      }),
    );
  ...
  it("preserves an existing Bot tile scope when moving it without an explicit scope", () => {
    ...
    openSessionTile("bot-chat", "right", undefined, undefined, scope);
    $layoutTree.set(
      group(["workspace", "session-tile:bot-chat"], { id: "workspace-group" }),
    );
    ...
    expect($sessionTiles.get()[0]).not.toHaveProperty(
      "workspaceOwnerKey",
...
describe("focusWorkspaceOwnerSessionTile", () => {
  ...
  it("fronts the tab the owner last had active and reports its stored id", () => {
    ...
    $layoutTree.set(
      group(["workspace", tilePane("older-thread"), tilePane("newer-thread")], {
        active: "workspace",
        id: "main",
      }),
    );
    rememberActivePane(
      workspaceScopeKey("bots", "bot:a"),
      tilePane("older-thread"),
    );

    expect(focusWorkspaceOwnerSessionTile("bot:a")).toBe("older-thread");
    expect(
      findGroupOfPane($layoutTree.get()!, tilePane("older-thread"))?.active,
    ).toBe(tilePane("older-thread"));
  });
  ...
  it("falls back to the most recently opened tab when nothing is remembered", () => {
    ...
    expect(
      findGroupOfPane($layoutTree.get()!, tilePane("newer-thread"))?.active,
    ).toBe(tilePane("newer-thread"));
  });
  ...
  it("ignores a remembered tab that has since been closed", () => {
    ...
    rememberActivePane(
      workspaceScopeKey("bots", "bot:a"),
      tilePane("closed-bot-chat"),
    );
    $sessionTiles.set(
      $sessionTiles
        .get()
        .filter((t) => t.storedSessionId !== "closed-bot-chat"),
    );
    $layoutTree.set(
      group(["workspace", tilePane("thread")], {
        active: "workspace",
        id: "main",
      }),
    );
  ...
  describe("staleness probe (#90102): the tile bucket reconciles with backend truth before it wins", () => {
    it("discards a stale tile and reports null so the caller runs its authoritative open", () => {
      ...
      expect(
        focusWorkspaceOwnerSessionTile(
          "bot:a",
          (tile) => tile.storedSessionId === "stale-bot-chat",
        ),
      ).toBeNull();
    ...
    it("fronts the surviving fresh tile after discarding the stale one", () => {
      ...
      $layoutTree.set(
        group(
          ["workspace", tilePane("stale-bot-chat"), tilePane("live-thread")],
...
describe("closeAllOpenSessionTiles persists Bot Mode Close All (#94137)", () => {
  ...
  it("drops persisted bot tiles so a roster/profile rehydrate cannot restore them", () => {
    ...
    $layoutTree.set(
      group(["workspace", tilePane("chat-a"), tilePane("chat-b")], {
        active: "workspace",
        id: "main",
      }),
    );
  ...
  it("leaves session tiles stacked in a different zone open", () => {
    ...
    $layoutTree.set(
      split("row", [
        group(["workspace", tilePane("close-me")], {
          active: "workspace",
          id: "main",
        }),
...
describe("dropTilesForProfile", () => {
  ...
  it("drops Bot Mode tiles owned by a locally-deleted profile and keeps the other bots' tiles", () => {
    ...
    expect(
      (storedTiles()[BOTS_BUCKET] as Array<{ storedSessionId: string }>).map(
        (tile) => tile.storedSessionId,
      ),
    ).toEqual(["bot-chat-2"]);
  });
  ...
  it("drops a legacy Bot tile whose ownerRoute predates the connectionId field", () => {
    ...
    mod.openSessionTile("bot-legacy", "right", undefined, undefined, {
      ownerRoute: {
        mode: "local" as const,
        profile: "press-bot",
      } as unknown as SessionProfileRoute,
      workspaceMode: "bots" as const,
      workspaceOwnerKey: "press-bot",
    });
...
describe("orderTilesByTree", () => {
  ...
  it("reorders tiles to layout-tree encounter order across a split", () => {
    const tree = split("row", [
      group(["workspace", tilePane("b")]),
      group([tilePane("a")]),
    ]);

    expect(orderTilesByTree(tree, [tile("a"), tile("b")])).toEqual([
      tile("b"),
      tile("a"),
    ]);
  });
  ...
  it("sorts not-yet-adopted tiles after placed ones, stably", () => {
    ...
    expect(orderTilesByTree(tree, [tile("a"), tile("b"), tile("c")])).toEqual([
      tile("b"),
      tile("a"),
      tile("c"),
    ]);
...
describe("nextSessionTileForWorkspace (⌘W promotion source)", () => {
  ...
  it("prefers a tile stacked WITH the workspace tab (nearest-out)", () => {
    $layoutTree.set(
      group(["workspace", tilePane("a"), tilePane("b")], {
        active: "workspace",
        id: "main",
      }),
    );
...
describe("boot-restore selection homing (⌘R tab persistence)", () => {
  const mainGroup = () =>
    group(["workspace", tilePane("t")], { active: tilePane("t"), id: "main" });
...
describe("$focusedStoredSessionId in Bot Mode (#96062)", () => {
  ...
  it("a Bots-pane click keeps the main-zone bot tile focused instead of collapsing to a null selection edge", () => {
    ...
    $layoutTree.set(
      split("row", [
        group(["sessions", "hermes-bots:pane"], {
          active: "hermes-bots:pane",
          id: "grp-sessions",
        }),
        group(["workspace", tilePane("chat-b")], {
          active: tilePane("chat-b"),
          id: "grp-main",
        }),
      ]),
    );
  ...
  it("sessions mode keeps collapsing to the primary selection (derivation gated to Bot Mode)", () => {
    ...
    $layoutTree.set(
      split("row", [
        ...
        group(["workspace", tilePane("stacked")], {
          active: tilePane("stacked"),
          id: "grp-main",
        }),
      ]),
    );
...
describe("blankDraftTile", () => {
  ...
  it("leaves a blank-but-busy tab alone — its first turn is already in flight", () => {
    expect(
      blankDraftTile([bound("a", "run-a")], { "run-a": state(0, true) }),
    ).toBeNull();
  });
  ...
  it("is null when every open tab holds a conversation", () => {
    expect(
      blankDraftTile([bound("a", "run-a")], { "run-a": state(2) }),
    ).toBeNull();
...
describe("reopenLastClosedTile focuses the restored tab", () => {
  ...
  async function setup() {
    ...
    const syncTiles = () => {
      const wanted = new Set(
        states.$sessionTiles.get().map((t) => t.storedSessionId),
      );
    ...
    expect(
      findGroupOfPane(tree.$layoutTree.get()!, tilePane("closed"))?.active,
    ).toBe(tilePane("closed"));
  ...
  it("fronts the restored tab after ⌘⇧T", async () => {
    ...
    expect(
      states.$sessionTiles.get().some((t) => t.storedSessionId === "closed"),
    ).toBe(false);
    expect(findGroupOfPane(tree.$layoutTree.get()!, "workspace")?.active).toBe(
      "workspace",
    );
...
describe("knownOwnerForSession / requestForOwnedSession (#91684 client half)", () => {
  ...
  it("resolves the tile owner route first, translating a runtime id to its stored id", () => {
    ...
    expect(knownOwnerForSession("rt-1")).toEqual({
      connectionId: "conn-a",
      profile: "work",
    });
    expect(knownOwnerForSession("stored-1")).toEqual({
      connectionId: "conn-a",
      profile: "work",
    });
  });
  ...
  it("keeps a session row connection owner when profiles share the same name", () => {
    ...
    expect(knownOwnerForSession("stored-shared")).toEqual({
      connectionId: "source-b",
      profile: "default",
    });
  });
  ...
  it("requestForOwnedSession dispatches ambient with the exact 2-arg shape when no owner is known", async () => {
    ...
    expect(ambient.mock.calls[0]).toEqual([
      "approval.respond",
      { choice: "once", session_id: "unknown-session" },
    ]);
  });
});
