...
export const DEFAULT_MANUAL_ONBOARDING_REASON =
...
const sessionIdFor = (flow: OnboardingFlow) =>
  "start" in flow && flow.start ? flow.start.session_id : undefined;
...
async function fetchProviderDefaultModel(
  ...
  const matched =
    providers.find((p: ModelOptionProvider) =>
      lower.includes(String(p.slug).toLowerCase()),
    ) ?? providers[0];
    ...
    try {
      ...
      onFail(
        error instanceof Error
          ? error.message
          : "Hermes could not save the selected model.",
      );
...
export function requestDesktopOnboardingForCredentialWarning(
...
export async function startProviderOAuth(
  ...
  try {
    ...
    const browserUrl =
      start.flow === "device_code" ? start.verification_url : start.auth_url;
    ...
    pollTimer = window.setInterval(
      () => void pollSession(provider, start, ctx),
      POLL_MS,
    );
  } catch (error) {
    setFlow({
      status: "error",
      provider,
      message: `Could not start sign-in: ${errMessage(error)}`,
    });
  }
}
  ...
  try {
    ...
    if (status === "approved") {
      ...
      setFlow({
        ...
        provider,
        start,
        message: error_message || `Sign-in ${status}.`,
      });
    }
  } catch (error) {
    clearPoll();
    setFlow({
      ...
      provider,
      start,
      message: `Polling failed: ${errMessage(error)}`,
    });
  }
}
  ...
  try {
    ...
    if (resp.ok && resp.status === "approved") {
      ...
      setFlow({
        ...
        provider,
        start,
        message: resp.message || "Token exchange failed.",
      });
    }
  } catch (error) {
...
async function copyAndFlash(
  text: string,
  predicate: (flow: OnboardingFlow) => boolean,
) {
  try {
...
export async function copyDeviceCode() {
  ...
  await copyAndFlash(
    ...
    (f) => f.status === "polling" && f.start.session_id === sid,
  );
}
...
export async function copyExternalCommand() {
  ...
  await copyAndFlash(
    ...
    (f) => f.status === "external_pending" && f.provider.id === id,
  );
}
...
export async function saveOnboardingApiKey(
  ...
  if (envKey === "OPENAI_BASE_URL") {
    return saveOnboardingLocalEndpoint(
      trimmed,
      endpointApiKey?.trim() ?? "",
      ctx,
    );
...
export async function saveOnboardingLocalEndpoint(
  ...
  try {
    ...
    if (!probe.ok && probe.reachable) {
      return {
        ok: false,
        message: probe.message || "Could not reach that endpoint.",
      };
    }
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return {
        ok: false,
        message: detail || `Saved, but Hermes still cannot reach ${url}.`,
      };
    }
