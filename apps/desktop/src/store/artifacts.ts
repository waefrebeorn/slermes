...
function pruneRegistry(registry: ArtifactRegistry): ArtifactRegistry {
  const entries = Object.entries(registry)
    ...
    .sort(([, a], [, b]) => {
      const latest = (records: readonly ArtifactRecord[]) =>
        Math.max(...records.map((record) => record.updatedAt));
...
export function upsertArtifact(
  ...
  if (existing) {
    ...
    const versions = [
      ...existing.versions,
      { content: trimmed, createdAt: now, hash },
    ...
    $artifactRegistry.set(
      pruneRegistry({
        ...
        [id]: records.map((record) =>
          record.id === existing.id ? next : record,
        ),
      }),
    );
  ...
  $artifactRegistry.set(
    pruneRegistry({ ...registry, [id]: [...records, record] }),
  );
...
export function selectArtifactVersion(
  ...
  const clamped = Math.max(
    0,
    Math.min(record.versions.length - 1, versionIndex),
  );
