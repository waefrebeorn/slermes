...
const SIDEBAR_PROJECT_FILTER_STORAGE_KEY =
  "hermes.desktop.sidebarProjectFilter";
const SIDEBAR_PROFILE_FILTER_STORAGE_KEY =
...
const SIDEBAR_WORKSPACE_PARENT_ORDER_STORAGE_KEY =
  "hermes.desktop.workspaceParentOrder";
const SIDEBAR_PROJECT_ORDER_STORAGE_KEY = "hermes.desktop.projectOrder";
const SIDEBAR_WORKSPACE_COLLAPSED_STORAGE_KEY =
  "hermes.desktop.workspaceCollapsed";
const SIDEBAR_WORKSPACE_NODE_OPEN_STORAGE_KEY =
  "hermes.desktop.workspaceNodeOpen";
const SIDEBAR_DISMISSED_AUTO_PROJECTS_STORAGE_KEY =
  "hermes.desktop.dismissedAutoProjects";
const SIDEBAR_DISMISSED_WORKTREES_STORAGE_KEY =
...
export type RightRailTabId =
  `artifact:${string}` | `file:${string}` | `url:${string}`;
...
export const $rightRailActiveTabId = persistentAtom<RightRailTabId | null>(
...
export const $sidebarWidth: ReadableAtom<number> = computed(
  $paneStates,
  (states) => {
...
export const $pinnedSessionIds = connectionScopedAtom(
...
export const $sidebarWorkspaceNodeOpen = persistentAtom<
...
export const $sidebarCronOpen = persistentAtom(
...
const $sidebarFlatAgentsGrouped = persistentAtom(
  SIDEBAR_AGENTS_GROUPED_STORAGE_KEY,
  false,
  Codecs.bool,
);
...
export const $sidebarAgentsGrouped: ReadableAtom<boolean> = computed(
  [
    $showAllProfiles,
    $sidebarFlatAgentsGrouped,
    $sidebarAllProfilesAgentsGrouped,
  ],
...
export type SidebarOrdering =
  "cost" | "created" | "manual" | "status" | "tokens" | "updated";
...
export type SidebarRowMeta =
  "cost" | "pr" | "preview" | "profile" | "tokens" | "updated";
...
const ROW_META: readonly SidebarRowMeta[] = [
  "cost",
  "pr",
  "preview",
  "profile",
...
const STATUS_FILTERS: readonly SessionStatusBucket[] = [
  "needs-input",
  "working",
  "unread",
  "draft",
...
const PR_FILTERS: readonly PullRequestBucket[] = [
  "open",
  "draft",
  "merged",
  "closed",
...
export const SIDEBAR_SORT_KEYS: readonly SidebarSortKey[] = [
  "updated",
  "created",
  "status",
  "tokens",
...
export const $sidebarCardRows = persistentAtom(
  SIDEBAR_CARD_ROWS_STORAGE_KEY,
  false,
  Codecs.bool,
);
...
export const $sidebarShowArchived = persistentAtom(
  SIDEBAR_SHOW_ARCHIVED_STORAGE_KEY,
  false,
  Codecs.bool,
);
...
export const $sidebarGrouping: ReadableAtom<SidebarGrouping> = computed(
  [
    ...
    $sidebarFlatGrouping,
    $sidebarAllProfilesGrouping,
    $showAllProfiles,
  ],
  (grouped, flat, allProfiles, showAll) =>
    grouped ? "project" : showAll ? allProfiles : flat,
);
...
export const $panesFlipped = persistentAtom(
  PANES_FLIPPED_STORAGE_KEY,
  false,
  Codecs.bool,
);
...
export function toggleWorkspaceNodeCollapsed(
  id: string,
  defaultOpen = true,
): void {
...
export function setWorkspaceNodesOpen(
  ids: readonly string[],
  open: boolean,
): void {
...
export function filterVisibleProjects<
  T extends { id: string; isAuto?: boolean },
>(
  projects: readonly T[],
  ...
  return projects.filter(
    (project) => !(project.isAuto && dismissed.has(project.id)),
  );
}
...
export function restoreWorktree(id: string): void {
  ...
  if (current.includes(id)) {
    $dismissedWorktreeIds.set(
      current.filter((worktreeId) => worktreeId !== id),
    );
  }
}
...
function revealNarrowPane(
  id: string,
  mode: "close" | "open" | "toggle",
): boolean {
  if (
    typeof window === "undefined" ||
    !matchesQuery(SIDEBAR_COLLAPSE_MEDIA_QUERY)
  ) {
    return false;
  }

  window.dispatchEvent(
    new CustomEvent(PANE_TOGGLE_REVEAL_EVENT, { detail: { id, mode } }),
  );
...
export function requestSessionSearchFocus() {
  ...
  if (typeof window !== "undefined") {
    window.setTimeout(
      () => window.dispatchEvent(new CustomEvent(SESSION_SEARCH_FOCUS_EVENT)),
      0,
    );
  }
}
...
export function pinSession(sessionId: string, index?: number) {
  ...
  setOrderIds(
    ...
    insertUniqueId(
      prev,
      sessionId,
      index ?? prev.filter((id) => id !== sessionId).length,
    ),
  );
}
