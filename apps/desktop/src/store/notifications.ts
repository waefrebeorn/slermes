...
const ERROR_SUMMARIES: {
  ...
  {
    test: (msg) =>
      /incorrect api key provided/i.test(msg) ||
      /['"]code['"]\s*:\s*['"]invalid_api_key['"]/i.test(msg),
    summarize: (msg) => {
      const status = msg.match(
        /(?:error code|status(?:Code)?)[^\d]*(\d{3})/i,
      )?.[1];

      return status
        ? translateNow(
            "notifications.errors.openaiRejectedApiKeyWithStatus",
            status,
          )
  ...
  {
    test: (msg) =>
      /neither voice_tools_openai_key nor openai_api_key is set/i.test(msg),
    summarize: () => translateNow("notifications.errors.openaiTtsNeedsKey"),
  },
  {
    test: (msg) =>
      /ELEVENLABS_API_KEY not set/i.test(msg) ||
      /ElevenLabs STT API error \(HTTP 401\)/i.test(msg),
    summarize: (msg) =>
      /ELEVENLABS_API_KEY not set/i.test(msg)
  ...
  },
  {
    ...
    summarize: () =>
      translateNow("notifications.errors.codeSkewRestartRequired"),
  },
];
