...
export const $reviewCommitDefault = persistentAtom<CommitAction>(
  COMMIT_DEFAULT_KEY,
  "commit",
  {
...
export const $reviewTreeMode = persistentAtom<ReviewTreeMode>(
  TREE_MODE_KEY,
  "tree",
  {
...
export const $reviewSelectedPath = persistentAtom<null | string>(
  SELECTED_KEY,
  null,
  Codecs.nullableText,
);
...
export const $reviewShipInfo = atom<HermesReviewShipInfo>({
  ghReady: false,
  pr: null,
});
...
export const reviewRepoCwd = (): null | string =>
  $reviewScopeCwd.get()?.trim() || $currentCwd.get()?.trim() || null;

const repoCwd = reviewRepoCwd;

type ReviewBridge = NonNullable<
  NonNullable<NonNullable<Window["hermesDesktop"]>["git"]>["review"]
>;
...
function matchReviewFile(
  files: readonly HermesReviewFile[],
  path: string,
): HermesReviewFile | undefined {
...
async function afterMutation(): Promise<void> {
  ...
  const selected = $reviewSelectedPath.get();
  const file = selected
    ? $reviewFiles.get().find((f) => f.path === selected)
    : null;
...
export async function createOrOpenPr(): Promise<void> {
  ...
  await runShip(async () => {
    ...
    const session = $sessions
      .get()
      .find((s) => s.id === $selectedStoredSessionId.get());
