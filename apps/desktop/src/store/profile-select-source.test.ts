...
const ensureGatewayForAgent = vi.fn(
  async (_connectionId: null | string, _profile: string) => true,
);
...
const { $activeGatewayProfile, newSessionInProfile, selectProfile } =
...
describe("selectProfile", () => {
  it("activates the pick on the live registry source, not the primary", async () => {
    ...
    await vi.waitFor(() =>
      expect(ensureGatewayForAgent).toHaveBeenCalledWith("mini", "researcher"),
    );
  ...
  it("keeps the legacy profile-only path when the primary is live", async () => {
    ...
    await vi.waitFor(() =>
      expect(ensureGatewayForProfile).toHaveBeenCalledWith("ops"),
    );
  ...
  it("keeps the legacy profile-only path when the explicit local source is live", async () => {
    ...
    await vi.waitFor(() =>
      expect(ensureGatewayForProfile).toHaveBeenCalledWith("override-profile"),
    );
    expect(ensureGatewayForAgent).not.toHaveBeenCalled();
  });
...
describe("newSessionInProfile", () => {
  it("opens the new chat on the live registry source", async () => {
    ...
    await vi.waitFor(() =>
      expect(ensureGatewayForAgent).toHaveBeenCalledWith("mini", "designer"),
    );
    expect(ensureGatewayForProfile).not.toHaveBeenCalled();
  });
...
describe("selectProfile startup preference (#79886)", () => {
  const rememberProfile = vi.fn(async (name: null | string) => ({
    profile: name,
  }));
  ...
  it("remembers the selected workspace for the next Desktop launch", async () => {
    ...
    await vi.waitFor(() =>
      expect(rememberProfile).toHaveBeenCalledWith("tilly"),
    );
  ...
  it("does not replace the local startup preference for a profile SSH override", async () => {
    ...
    await vi.waitFor(() =>
      expect(ensureGatewayForProfile).toHaveBeenCalledWith("macmini-hermes"),
    );
    await vi.waitFor(() =>
      expect(getConnection).toHaveBeenCalledWith("macmini-hermes"),
    );
