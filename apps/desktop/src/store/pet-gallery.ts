...
const petRpc = <T>(
  ...
  method: string,
  params: Record<string, unknown> = {},
): Promise<T> => request<T>(method, { ...params, profile: petProfile() });
...
export function loadPetThumb(
  ...
  if (!pending) {
    pending = petRpc<{ ok: boolean; dataUri?: string }>(request, "pet.thumb", {
      slug,
      url: url ?? "",
    })
...
export async function applyAdoptedPet(
  ...
  patchGallery((gallery) => ({
    ...
    pets: gallery.pets.some((p) => p.slug === slug)
      ? gallery.pets.map((p) =>
          p.slug === slug ? { ...p, installed: true, displayName } : p,
        )
      : [
          ...gallery.pets,
          { slug, displayName, installed: true, spritesheetUrl: "" },
        ],
  }));
 ...
 */
export function rankedGalleryPets(
  gallery: PetGallery | null,
  query = "",
): GalleryPet[] {
  if (!gallery) {
    return [];
...
export function adoptPet(
  ...
  return mutate(slug, fallback, request, async () => {
    ...
    patchGallery((g) => ({
      ...
      active: slug,
      pets: g.pets.map((p) =>
        p.slug === slug ? { ...p, installed: true } : p,
      ),
    }));
  });
}
...
export const clampPetScale = (n: number) =>
  Math.max(PET_SCALE_MIN, Math.min(PET_SCALE_MAX, n));
...
export function setPetScale(request: GatewayRequest, scale: number): void {
  ...
  scalePersist = setTimeout(() => {
    petRpc<{ ok: boolean; scale?: number }>(request, "pet.scale", {
      scale: next,
    })
      .then((result) => {
        ...
        if (
          ...
          result.scale !== $petInfo.get().scale
        ) {
...
export async function exportPet(
  ...
  try {
    const res = await petRpc<{
      ok: boolean;
      filename: string;
      zipBase64: string;
    }>(request, "pet.export", { slug });
...
export function renamePet(
  ...
  const prev =
    $petGallery.get()?.pets.find((p) => p.slug === slug)?.displayName ?? "";

  // Optimistic: paint the new name now (slug reconciles when the RPC returns).
  patchGallery((g) => ({
    ...g,
    pets: g.pets.map((p) =>
      p.slug === slug ? { ...p, displayName: trimmed } : p,
    ),
  }));
  $petGalleryError.set(null);
  ...
  return (async () => {
    try {
      ...
      patchGallery((g) => ({
        ...g,
        pets: g.pets.map((p) =>
          p.slug === slug ? { ...p, displayName: prev } : p,
        ),
      }));
      $petGalleryError.set(e instanceof Error ? e.message : fallback);
  ...
  return mutate(slug, fallback, request, async () => {
    ...
    patchGallery((g) => ({
      ...
      pets: g.pets.flatMap((p) => {
        ...
        return p.generated || !p.spritesheetUrl
          ? []
          : [{ ...p, installed: false }];
      }),
    }));
