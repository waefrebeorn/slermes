...
export type ManagedUpdateStatus =
  "failed" | "idle" | "partial" | "refused" | "updated" | "updating";
...
const IN_PROGRESS_PATTERN =
  /managed[\s-]update[\s\w]*\bin[\s-]progress|managed-update-in-progress|already in progress/i;

function managedUpdater():
  ((id: string) => Promise<DesktopManagedConnectionUpdateResult>) | null {
...
export function isManagedUpdateBusyMessage(
...
function terminalState(
  connectionId: string,
  result: DesktopManagedConnectionUpdateResult,
): ManagedUpdateState {
  const message =
    result.message || result.error || result.receipt?.stopReason || null;
  const alreadyRunning =
    result.outcome === "refused" && isManagedUpdateBusyMessage(message);
...
export function runManagedUpdate(
  connectionId: string,
): Promise<ManagedUpdateState> {
    ...
    try {
      ...
      const alreadyRunning =
        (error as { code?: unknown } | null)?.code ===
          "managed-update-in-progress" || isManagedUpdateBusyMessage(message);
