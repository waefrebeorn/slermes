...
export function isCronJobsScopeCurrent(token: CronJobsScopeToken): boolean {
  return (
    token.scope === cronJobsRequestScope &&
...
export function isCronJobsRequestCurrent(request: CronJobsRequest): boolean {
  return (
    request.scope === cronJobsRequestScope &&
    request.generation === cronJobsRequestGeneration
  );
}
...
export function commitCronJobsRequest(
...
export const requestCronReview = () =>
  $cronReviewRequest.set($cronReviewRequest.get() + 1);
