...
function attachment(
  overrides: Partial<ComposerAttachment> & Pick<ComposerAttachment, "id">,
): ComposerAttachment {
...
describe("updateComposerAttachment", () => {
  ...
  it("replaces an existing attachment in place", () => {
    addComposerAttachment(
      attachment({ id: "file:a", uploadState: "uploading" }),
    );

    const updated = updateComposerAttachment(
      attachment({ id: "file:a", attachedSessionId: "sess-1" }),
    );
  ...
  it("updates only the exact attachment occurrence captured before an async operation", () => {
    ...
    expect(
      scope.updateIfCurrent(first, {
        thumbnailUrl: "data:image/png;base64,stale",
      }),
    ).toBe(false);
    expect(scope.$attachments.get()).toEqual([replacement]);
    expect(
      scope.updateIfCurrent(replacement, {
        thumbnailUrl: "data:image/png;base64,current",
      }),
    ).toBe(true);
    expect(scope.$attachments.get()[0]?.thumbnailUrl).toBe(
...
describe("session drafts", () => {
  ...
  it("keeps drafts isolated per session scope", () => {
    ...
    stashSessionDraft("session-b", "draft b", [
      attachment({ id: "image:b", kind: "image" }),
    ]);
    ...
    expect(takeSessionDraft("session-b").attachments.map((a) => a.id)).toEqual([
      "image:b",
    ]);
  });
  ...
  it("persists draft text (not attachments) to localStorage", () => {
    stashSessionDraft("session-a", "survives reload", [
      attachment({ id: "file:a" }),
    ]);

    const persisted = JSON.parse(
      window.localStorage.getItem(SESSION_DRAFTS_STORAGE_KEY) ?? "{}",
  ...
  it("clears a stashed draft after an accepted submit", () => {
    stashSessionDraft("session-a", "sent prompt", [
      attachment({ id: "file:a" }),
    ]);
    clearSessionDraft("session-a");
