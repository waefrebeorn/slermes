...
export type SubagentStatus =
  "completed" | "failed" | "interrupted" | "queued" | "running";
...
const toolLabel = (name: string) =>
  name.split("_").filter(Boolean).map(capitalize).join(" ") || name;
...
const timeoutSummary = (payload: SubagentPayload): string => {
  ...
  return str(payload.status) === "timeout"
...
function streamFromPayload(
  ...
  const summary = compact(
    str(payload.summary) || str(payload.text) || timeoutSummary(payload),
  );
...
export const failedSubagentCount = (items: readonly SubagentProgress[]) =>
  items.filter(
    (item) => item.status === "failed" || item.status === "interrupted",
  ).length;
