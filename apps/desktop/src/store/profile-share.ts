...
export async function exportProfileBundle(
  ...
  const { archive } = await exportProfileArchive(profile, {
    extraFiles: {
      [DESKTOP_OVERLAY_FILENAME]: JSON.stringify(overlay, null, 2),
    },
    output,
  });
...
export function applyDesktopOverlay(
  profile: string,
  overlay: null | ProfileDesktopOverlay | undefined,
): void {
...
export async function runExportProfileFlow(
  ...
  try {
    ...
    notify({
      kind: "success",
      title: translateNow("profiles.exported"),
      message: archive,
    });
...
export async function runImportProfileFlow(): Promise<null | string> {
  ...
  try {
    ...
    notify({
      kind: "success",
      title: translateNow("profiles.imported"),
      message: name,
    });
