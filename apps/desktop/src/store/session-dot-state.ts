...
export const hasLiveTurn = (state: SessionDotState): boolean =>
  showsRunningArc(state) || state === "needs-input";
...
export type SessionStatusBucket =
  "draft" | "idle" | "needs-input" | "unread" | "working";

export const sessionStatusBucket = (
  state: SessionDotState = "idle",
): SessionStatusBucket =>
...
export const sessionStatusRank = (state?: SessionDotState): number =>
  STATUS_RANK[sessionStatusBucket(state)];
