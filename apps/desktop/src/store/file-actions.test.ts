...
const { downloadRemoteFile, shouldOfferRemoteFileDownload } =
...
describe("downloadRemoteFile", () => {
  ...
  it("saves a remote gateway file through the native download bridge", async () => {
    downloadGatewayMediaFile.mockResolvedValue({
      path: "/Users/me/Downloads/notes.md",
      saved: true,
    });

    await downloadRemoteFile("/home/linux/project/notes.md");

    expect(downloadGatewayMediaFile).toHaveBeenCalledWith(
      "/home/linux/project/notes.md",
    );
  ...
  it("toasts when the gateway download fails", async () => {
    downloadGatewayMediaFile.mockRejectedValue(
      new Error("Desktop file download bridge is unavailable"),
    );
