...
async function runHubAction(
  ...
  try {
    ...
    if (exitCode !== null && exitCode !== 0) {
      const detail = ($hubActions.get()[key]?.lines ?? [])
        .slice(-3)
        .join("\n")
        .trim();
...
export function installHubSkill(
  ...
  return runHubAction(
    identifier,
    "install",
    () => installSkillFromHub(identifier, profile),
    profile,
  );
}
...
export function uninstallHubSkill(
  ...
  return runHubAction(
    identifier,
    "uninstall",
    () => uninstallSkillFromHub(name, profile),
    profile,
  );
}
