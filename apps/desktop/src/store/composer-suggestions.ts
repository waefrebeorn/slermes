...
export const suggestionKey = (
  suggestion: Pick<ComposerSuggestion, "id" | "provider">,
): string => `${suggestion.provider}:${suggestion.id}`;
...
export const $composerSuggestionsBySession = atom<
  Record<string, ComposerSuggestion[]>
>({});
...
const sameSuggestions = (
  a: readonly ComposerSuggestion[],
  b: readonly ComposerSuggestion[],
) =>
  a.length === b.length &&
  a.every((x, i) => {
    const y = b[i]!;
    ...
    return (
      suggestionKey(x) === suggestionKey(y) &&
      RENDERED.every((field) => x[field] === y[field])
    );
  });
...
export function markSuggestionInvoked(
  sessionId: string | null | undefined,
  key: string,
): void {
...
function recordWithdrawals(
  sessionKey: string,
  next: readonly ComposerSuggestion[],
): void {
  ...
  if (previous) {
    ...
    for (const key of previous) {
      if (!surviving.has(key)) {
        const counts =
          ignoredCounts.get(sessionKey) ?? new Map<string, number>();
 ...
 */
export function sampleComposerDraft(
  sessionId: string | null | undefined,
  text: string,
): void {
  const key = keyFor(sessionId);
    ...
    window.setTimeout(() => {
      void Promise.all(
        [...draftProviders.values()].map((provider) =>
          provider({ sessionId: sessionId ?? null, text }).catch(
            (): ComposerSuggestion[] => [],
          ),
        ),
      ).then((results) => {
...
export function clearDraftSuggestions(
  sessionId: string | null | undefined,
): void {
  const key = keyFor(sessionId);
