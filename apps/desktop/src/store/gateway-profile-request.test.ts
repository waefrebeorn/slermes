...
describe("requestGatewayForProfile", () => {
  it("requests through a pooled profile gateway without changing the active gateway", async () => {
    ...
    const result = await requestGatewayForProfile("worker", "profiles.list", {
    ...
    expect(secondaryGateways[0].request).toHaveBeenCalledWith("profiles.list", {
      include_sessions: true,
    });
  ...
  it("uses the primary socket and adds profile scope for a shared global remote route", async () => {
    ...
    const result = await requestGatewayForProfile("venture", "session.list", {
      limit: 20,
      profile: "wrong",
    });
    ...
    expect(result).toEqual({
      method: "session.list",
      params: { limit: 20, profile: "venture" },
    });
    expect(primary.request).toHaveBeenCalledWith("session.list", {
      limit: 20,
      profile: "venture",
    });
...
describe("requestGatewayForAgent", () => {
  it("reuses the active primary socket when its registry connection owns the session", async () => {
    ...
    (window as unknown as { hermesDesktop: unknown }).hermesDesktop = {
      getConnection: vi.fn(),
      getConnectionFor,
      getGatewayWsUrlFor: vi.fn(async () => ({
        ok: true as const,
    ...
    const result = await requestGatewayForAgent(
      "remote-primary",
      "default",
      "session.resume",
      {
    ...
    expect(primary.request).toHaveBeenCalledWith("session.resume", {
      session_id: "stored-session",
    });
  ...
  it("leases separate registry sockets for duplicate profile names without changing the active gateway", async () => {
    ...
    const [fromA, fromB] = await Promise.all([
      requestGatewayForAgent("source-a", "research", "session.list", {
        limit: 1,
      }),
      requestGatewayForAgent("source-b", "research", "session.list", {
        limit: 2,
      }),
    ]);
  ...
  it("routes an explicit local registry descriptor through getConnectionFor", async () => {
    ...
    await expect(
      requestGatewayForAgent("local", "worker", "profiles.list"),
    ).resolves.toEqual({
      method: "profiles.list",
      params: {},
    });
  ...
  it("invalidates an active pinned source with its window profile and a route epoch", async () => {
    ...
    expect(onActiveConnectionInvalidated).toHaveBeenCalledWith(
      "pinned",
      invalidationEpoch,
    );
    expect($gateway.get()).toBe(primary);
    ...
    await vi.waitFor(() =>
      expect(secondaryGateways[1]?.connect).toHaveBeenCalledOnce(),
    );
    expect(gatewayActivationEpoch()).toBeGreaterThan(invalidationEpoch);
    releaseConnect();
    await sourceBActivation;
    expect(onActiveConnectionChanged).toHaveBeenLastCalledWith(
  ...
  it("does not activate or publish a source whose activation owner aborted while dialing", async () => {
    ...
    const abandoned = ensureGatewayForAgent("source-b", "default", {
...
describe("retainGatewayForAgent (#93602)", () => {
  ...
  it("holds the registry socket across multiple leased requests — no mid-turn disposal", async () => {
    ...
    await requestGatewayForAgent("mini", "helper", "session.create", {
      title: "g",
    });
    await requestGatewayForAgent("mini", "helper", "image.attach_bytes", {
      session_id: "rt-1",
    });
    await requestGatewayForAgent("mini", "helper", "prompt.submit", {
      session_id: "rt-1",
      text: "hi",
    });
  ...
  it("plain-profile retain leases the pooled profile socket and releases it", async () => {
    ...
    await requestGatewayForProfile("worker", "prompt.submit", {
      session_id: "rt-1",
      text: "hi",
    });
...
describe("attached shared-remote group turns (#96493)", () => {
  ...
  it("reuses the primary socket for a named profile on the attached shared remote", async () => {
    ...
    await requestGatewayForAgent("homelab", "voter", "session.create", {
      title: "Group: room",
    });
    await requestGatewayForAgent("homelab", "voter", "prompt.submit", {
      session_id: "rt-1",
      text: "hi",
    });
