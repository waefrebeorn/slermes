...
function recordResult(
  ...
  if (
    !shouldNotifyOnTransition(previous, status) ||
    notifiedThisSession.has(key)
  ) {
    return;
  }
  ...
  notify({
    action: {
      label: translateNow(
        needsAuth ? "notifications.mcp.signIn" : "notifications.mcp.view",
      ),
    ...
    message: translateNow(
      needsAuth
        ? "notifications.mcp.needsAuthMessage"
        : "notifications.mcp.errorMessage",
      name,
    ),
    title: translateNow(
      needsAuth
        ? "notifications.mcp.needsAuthTitle"
        : "notifications.mcp.errorTitle",
    ),
  });
}
