...
function composerScopeForConnection(
...
function composerSelectionKey(base: string): string | null {
  return composerSelectionScope === null
    ? null
    : `${base}${composerSelectionScope}`;
}
...
export function knownSessionProfile(
  ...
  return typeof owner === "string"
    ? owner
    : (owner?.targetProfile ?? owner?.profile)?.trim() || undefined;
}
...
export function knownSessionOwner(
  ...
  const session = sessions.find((candidate) =>
    sessionMatchesStoredId(candidate, sessionId),
  );
  ...
  const hintProfiles = new Set([
    hint?.profile.trim() || "default",
    hint?.targetProfile?.trim() || "default",
  ]);
...
export function rememberedSessionProfile(
  ...
  return (
    knownSessionProfile(sessions, sessionId) ??
...
export const getRememberedWorkspaceCwd = (): string =>
  storedString(workspaceCwdKey())?.trim() || "";
export type NewChatWorkspaceTarget = null | string | undefined;

export const getConfiguredDefaultProjectDir = (): string =>
...
export function applyConfiguredDefaultProjectDir(
...
export const sessionPinId = (
  session: Pick<SessionInfo, "_lineage_root_id" | "id">,
): string => session._lineage_root_id ?? session.id;
...
const lineageIndexBySessions = new WeakMap<
  readonly LineageRow[],
  Map<string, string[]>
>();
...
export function idsShareLineage(
  ...
  return sessions.some(
    (session) =>
      sessionMatchesStoredId(session, a) && sessionMatchesStoredId(session, b),
  );
}
 ...
 *  resurrected here. */
const profileKeyOf = (profile: null | string | undefined): string =>
  (profile ?? "").trim() || "default";

function carriedConnectionId(
  ...
  return !incoming.profile?.trim() ||
    profileKeyOf(incoming.profile) === profileKeyOf(prev?.profile)
    ? carried
    : undefined;
}
...
export function mergeSessionPage(
  ...
  const profileKeyOf = (session: SessionInfo) =>
    (session.profile ?? "").trim() || "default";
  const identity = (session: SessionInfo) =>
    `${profileKeyOf(session)}::${session.id}`;
  ...
  const prevById = new Map(
    previous.map((session) => [identity(session), session]),
  );
  ...
  const prevByLineage = new Map(
    previous.map((session) => [lineageIdentity(session), session]),
  );
  ...
  const merged = incoming.map((session) => {
    ...
    return last_active === session.last_active &&
      title === session.title &&
  ...
  const recency = (session: SessionInfo): number =>
    Math.max(session.last_active || 0, session.started_at || 0);

  const sortedSurvivors = [...survivors].sort(
  ...
  while (
    survivorIndex < sortedSurvivors.length &&
    mergedIndex < merged.length
  ) {
    if (
      recency(sortedSurvivors[survivorIndex]) >= recency(merged[mergedIndex])
    ) {
...
export function carryForwardFailedProfileSessions(
  ...
  const failed = new Set(
    errors.map((error) => (error.profile ?? "").trim() || "default"),
  );
...
// foreground route.
export const $activeSessionStoredIdRotation =
  atom<ActiveSessionStoredIdRotation | null>(null);
...
export const $messagesEmpty = computed(
  $messages,
  (messages) => messages.length === 0,
);
export const $lastVisibleMessageIsUser = computed(
  $messages,
...
const sessionOwnerHints = new Map<
  string,
  { id: string; route: SessionOwnerRoute }
>();

function sessionOwnerHintKey(
  sessionId: string,
  route: Pick<SessionOwnerRoute, "connectionId" | "profile">,
): string {
  return JSON.stringify([
    route.connectionId.trim(),
    route.profile.trim() || "default",
    sessionId,
  ]);
}
...
function normalizeOwnerRoute(route: SessionOwnerRoute): SessionOwnerRoute {
  return {
    ...
    ...(route.targetProfile
      ? { targetProfile: route.targetProfile.trim() || "default" }
      : {}),
  };
}
...
function persistSessionOwnerHints(): void {
  writeJson(
    ...
    sessionOwnerHints.size === 0
      ? null
      : [...sessionOwnerHints.values()].map((entry) => [entry.id, entry.route]),
  );
}

function rememberSessionOwnerHint(
  sessionId: string,
...
export function hydrateSessionOwnerHints(): void {
  ...
  for (const entry of raw) {
    ...
    rememberSessionOwnerHint(id, {
      ...
      ...(typeof candidate.targetProfile === "string"
...
export function setSessionOwnerHint(
  sessionId: string,
  route: SessionOwnerRoute,
): void {
...
export function forgetSessionOwnerHintsForConnection(
  connectionId: string,
): void {
...
export function _resetSessionOwnerHintsForTests({
  storage = false,
}: { storage?: boolean } = {}): void {
...
export function getSessionOwnerHint(
  ...
  const matches = [...sessionOwnerHints.values()].filter(
    (entry) => entry.id === id,
  );
...
export const $currentModel = atom(
  storedComposerString(COMPOSER_MODEL_KEY) ?? "",
);
export const $currentProvider = atom(
  storedComposerString(COMPOSER_PROVIDER_KEY) ?? "",
);
export const $currentReasoningEffort = atom(
  storedString(COMPOSER_EFFORT_KEY) ?? "",
);
...
export function setComposerSelectionOwner(
  connectionId: string,
  profile: string,
): void {
...
export const setGatewayState = (next: Updater<ConnectionState>) =>
  updateAtom($gatewayState, next);
export const setSessions = (next: Updater<SessionInfo[]>) =>
  updateAtom($sessions, next);
export const setCronSessions = (next: Updater<SessionInfo[]>) =>
  updateAtom($cronSessions, next);
export const setMessagingSessions = (next: Updater<SessionInfo[]>) =>
  updateAtom($messagingSessions, next);
export const setMessagingPlatformTotals = (
  next: Updater<Record<string, number>>,
) => updateAtom($messagingPlatformTotals, next);
export const setMessagingTruncated = (next: Updater<boolean>) =>
  updateAtom($messagingTruncated, next);
export const setSessionProfilesTruncated = (
  next: Updater<Record<string, boolean>>,
) => updateAtom($sessionProfilesTruncated, next);
export const setSessionProfilesUsage = (
  next: Updater<Record<string, ProfileUsage>>,
) => updateAtom($sessionProfilesUsage, next);
export const setSessionsLoading = (next: Updater<boolean>) =>
  updateAtom($sessionsLoading, next);
export const setActiveSessionId = (next: Updater<string | null>) =>
  updateAtom($activeSessionId, next);
export const setActiveSessionStoredIdRotation = (
  next: Updater<ActiveSessionStoredIdRotation | null>,
...
export const markSessionRead = (storedSessionId: string | null | undefined) => {
  ...
  $unreadFinishedSessionIds.set(
    $unreadFinishedSessionIds.get().filter((id) => !familyIds.has(id)),
  );
};

export const setMessages = (next: Updater<ChatMessage[]>) =>
  updateAtom($messages, next);
export const setFreshDraftReady = (next: Updater<boolean>) =>
  updateAtom($freshDraftReady, next);
...
export const $currentModelSource = atom<ComposerModelSource>(
...
export const getComposerSelectionGeneration = (): number =>
  composerSelectionGeneration;
...
export const setDefaultReasoningEffort = (next: string) =>
  updateAtom($defaultReasoningEffort, next);
...
export const setTerminalBackend = (next: Updater<string>) =>
  updateAtom($terminalBackend, next);
 ...
 */
export const setWorkspaceCwdOwner = (storedSessionId: null | string) =>
  updateAtom($workspaceCwdOwner, storedSessionId);
 ...
 */
export const releaseWorkspaceCwdOwner = () =>
  updateAtom($workspaceCwdOwner, WORKSPACE_CWD_UNOWNED);
...
export const workspaceCwdBelongsToSelectedSession = (): boolean =>
  ($workspaceCwdOwner.get() ?? null) ===
  ($selectedStoredSessionId.get() ?? null);

export const setNewChatWorkspaceTarget = (
  next: NewChatWorkspaceTarget,
): number => {
...
export const setCurrentUsage = (next: Updater<UsageStats>) =>
  updateAtom($currentUsage, next);
export const setSessionStartedAt = (next: Updater<number | null>) =>
  updateAtom($sessionStartedAt, next);
export const setTurnStartedAt = (next: Updater<number | null>) =>
  updateAtom($turnStartedAt, next);
export const setIntroPersonality = (next: Updater<string>) =>
  updateAtom($introPersonality, next);
export const setCurrentPersonality = (next: Updater<string>) =>
  updateAtom($currentPersonality, next);
export const setAvailablePersonalities = (next: Updater<string[]>) =>
  updateAtom($availablePersonalities, next);
export const setIntroSeed = (next: Updater<number>) =>
  updateAtom($introSeed, next);
export const setContextSuggestions = (next: Updater<ContextSuggestion[]>) =>
  updateAtom($contextSuggestions, next);
export const setModelPickerOpen = (next: Updater<boolean>) =>
  updateAtom($modelPickerOpen, next);
export const setSessionPickerOpen = (next: Updater<boolean>) =>
