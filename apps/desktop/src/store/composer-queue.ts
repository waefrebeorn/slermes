...
const cloneAttachments = (attachments: ComposerAttachment[]) =>
  attachments.map((a) => ({ ...a }));

export const getQueuedPrompts = (
...
export const dequeueQueuedPrompt = (
  key: string | null | undefined,
): null | QueuedPromptEntry => {
  const sid = sidOf(key);
...
export const updateQueuedPromptText = (
  key: string | null | undefined,
  id: string,
  text: string,
): boolean => updateQueuedPrompt(key, id, { text });
 ...
 * this same gate.
 */
export const shouldAutoDrain = ({
  isBusy,
  parked,
  queueLength,
}: AutoDrainInput): boolean => !isBusy && !parked && queueLength > 0;
