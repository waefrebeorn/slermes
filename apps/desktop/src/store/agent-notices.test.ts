...
function usage(
  overrides: Partial<AgentNoticePayload> = {},
): AgentNoticePayload {
  return {
    key: "credits.usage",
    kind: "sticky",
...
test("ttl notice without a usable ttl_ms defers to notify()’s default", () => {
  ...
  expect(
    noticeToToast({ text: "x", kind: "ttl", ttl_ms: 0 })?.durationMs,
  ).toBeUndefined();
});

test("the notice key is the toast id, falling back to id", () => {
  expect(noticeToToast(usage({ key: "credits.usage" }))?.id).toBe(
    "credits.usage",
  );
...
test("the leading severity glyph is stripped from the toast message", () => {
  ...
  expect(noticeToToast(usage())?.message).toBe(
    "You've used $110.00 of your $220.00 cap",
  );
  expect(
    noticeToToast(usage({ level: "error", text: "✕ Credit access paused" }))
      ?.message,
  ).toBe("Credit access paused");
  expect(
    noticeToToast(usage({ level: "success", text: "✓ Credit access restored" }))
      ?.message,
  ).toBe("Credit access restored");
});
...
test('the trailing "· detail" is split off as a secondary meta line, not inlined', () => {
  ...
  const grant = noticeToToast({
    key: "credits.grant_spent",
    level: "info",
    text: "• Grant spent · $12.00 top-up left",
  });
...
test("splitMeta splits on the first space-middot-space only", () => {
  ...
  expect(splitMeta("Grant spent · $12.00 top-up left")).toEqual([
    "Grant spent",
    "$12.00 top-up left",
  ]);
  expect(splitMeta("Credit access restored")).toEqual([
...
test("stripGlyph removes only a single leading severity glyph", () => {
  ...
  expect(stripGlyph("Credits 50% used")).toBe("Credits 50% used");
  expect(stripGlyph("spent · $12.00 • top-up left")).toBe(
    "spent · $12.00 • top-up left",
  );
});
...
test("usageFraction derives $used / $cap from the notice text", () => {
  expect(usageFraction("You've used $15.00 of your $20.00 cap")).toBeCloseTo(
    0.75,
  );
  expect(usageFraction("You've used $198.00 of your $220.00 cap")).toBeCloseTo(
    0.9,
  );
...
test("usage accent stays muted below 75%, then ramps orange → red", () => {
  expect(
    noticeAccent(usage({ text: "• You've used $10.00 of your $20.00 cap" })),
  ).toBeUndefined(); // 50%
  expect(
    noticeAccent(usage({ text: "• You've used $14.80 of your $20.00 cap" })),
  ).toBeUndefined(); // 74%
  expect(
    noticeAccent(
      usage({ level: "warn", text: "⚠ You've used $15.00 of your $20.00 cap" }),
    ),
  ).toBe("var(--ui-orange)"); // 75%
  expect(
    noticeAccent(
      usage({ level: "warn", text: "⚠ You've used $17.80 of your $20.00 cap" }),
    ),
  ).toBe("var(--ui-orange)"); // 89%
  expect(
    noticeAccent(
      usage({ level: "warn", text: "⚠ You've used $18.00 of your $20.00 cap" }),
    ),
  ).toBe("var(--ui-red)"); // 90%
  expect(
    noticeAccent(
      usage({ level: "warn", text: "⚠ You've used $20.00 of your $20.00 cap" }),
    ),
  ).toBe("var(--ui-red)"); // 100%
});

test("terminal credit states carry their own accent; others stay default", () => {
  expect(noticeAccent({ key: "credits.depleted", text: "✕ paused" })).toBe(
    "var(--ui-red)",
  );
  expect(noticeAccent({ key: "credits.restored", text: "✓ restored" })).toBe(
    "var(--ui-green)",
  );
  expect(
    noticeAccent({ key: "credits.grant_spent", text: "• Grant spent" }),
  ).toBeUndefined();
...
test("noticeToToast attaches the band accent to the toast", () => {
  ...
  expect(
    noticeToToast(usage({ text: "• You've used $10.00 of your $20.00 cap" }))
      ?.accentColor,
  ).toBeUndefined();
});
...
test("re-emitting the same key replaces the toast instead of stacking (50→75→90)", () => {
  showAgentNotice(
    usage({ level: "info", text: "• You've used $10.00 of your $20.00 cap" }),
  );
  showAgentNotice(
    usage({ level: "warn", text: "⚠ You've used $15.00 of your $20.00 cap" }),
  );
  ...
  const toasts = $notifications
    .get()
    .filter((item) => item.id === "credits.usage");
...
test("only credits.depleted and credits.restored map to a native notification", () => {
  expect(
    nativeNoticeInput(usage({ key: "credits.usage" }), "Credits"),
  ).toBeNull();
  expect(
    nativeNoticeInput(usage({ key: "credits.grant_spent" }), "Credits"),
  ).toBeNull();
  expect(
    nativeNoticeInput({ text: "x", key: undefined }, "Credits"),
  ).toBeNull();
  expect(
    nativeNoticeInput({ text: "", key: "credits.depleted" }, "Credits"),
  ).toBeNull();
});
