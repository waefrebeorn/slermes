    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt === MAX_RETRIES || epoch !== profileListEpoch) {
          ...
          console.error(
            `[profiles] refreshProfiles failed after ${attempt + 1} attempt(s):`,
            error,
          );
...
export const $profileOrder = atom<string[]>(
  storedStringArray(PROFILE_ORDER_STORAGE_KEY),
);

$profileOrder.subscribe((value) =>
  persistStringArray(PROFILE_ORDER_STORAGE_KEY, [...value]),
);
...
export function sortByProfileOrder<T extends { name: string }>(
  items: T[],
  order: string[],
): T[] {
...
export const $profileColors = atom<Record<string, string>>(
  storedStringRecord(PROFILE_COLORS_STORAGE_KEY),
);

$profileColors.subscribe((value) =>
  persistStringRecord(PROFILE_COLORS_STORAGE_KEY, value),
);
...
export function captureNewChatSource(
  connectionId: null | string = activeGatewayConnectionId(),
): void {
...
async function resolveConnectionForProfile(
  profile: string,
  ...
  try {
    ...
    console.warn(
      `[profile] descriptor lookup for "${profile}" failed; keeping the previous connection`,
      err,
    );
...
// previous descriptor.
async function resolveConnectionForAgent(
  connectionId: string,
  profile: string,
): Promise<HermesConnection | null> {
...
export async function ensureGatewayAgent(
  ...
  const activationWork = (async () => {
    ...
    if (!activated) {
      ...
      console.warn(
        `[profile] agent gateway activation for "${connection}:${target}" did not land`,
      );
...
export const messagingTotalsKey = (
  messagingProfile: string,
  sourceId: string,
): string => `${messagingProfile}:${sourceId}`;
...
export const $showAllProfiles = atom<boolean>(
  storedBoolean(SHOW_ALL_PROFILES_STORAGE_KEY, false),
);

$showAllProfiles.subscribe((value) =>
  persistBoolean(SHOW_ALL_PROFILES_STORAGE_KEY, value),
);
...
export const $profileScope = computed(
  ...
  (showAll, gateway) => (showAll ? ALL_PROFILES : normalizeProfileKey(gateway)),
);
...
export function selectProfile(name: string): void {
  ...
  const shouldRememberStartupProfile = onPrimary
    ? isLocalDesktopProfile(target)
    : Promise.resolve(false);

  void Promise.all([
    activateOnCurrentSource(target),
    shouldRememberStartupProfile,
  ])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
...
export function newSessionInAgent(route: AgentProfileRoute): void {
  const captured = {
    ...
    ...(route.targetProfile
      ? { targetProfile: normalizeProfileKey(route.targetProfile) }
      : {}),
  };
  ...
  requestFreshSession();
  // #81094: surface the failed dial instead of failing silently.
  void ensureGatewayAgent(captured.connectionId, captured.profile).catch(
    (error: unknown) => {
...
export function cycleProfile(direction: 1 | -1): void {
  ...
  const current = $showAllProfiles.get()
    ? -1
    : keys.indexOf(normalizeProfileKey($activeGatewayProfile.get()));
