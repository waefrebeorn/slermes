...
function publishUnconfirmed(): void {
  ...
  if (
    published.size === unconfirmed.size &&
    [...unconfirmed.keys()].every((id) => published.has(id))
  ) {
    return;
  }
...
function loadedRowFor(pinId: string): SessionInfo | undefined {
  const rows = loadedSessionRows().filter((row) =>
    sessionMatchesStoredId(row, pinId),
  );
  const gateway = normalizeProfileKey($activeGatewayProfile.get());

  return (
    rows.find((row) => normalizeProfileKey(row.profile) === gateway) ?? rows[0]
  );
}
...
function rowsByPinId(rows: readonly SessionInfo[]): Map<string, SessionInfo> {
  ...
  for (const row of rows) {
    ...
    if (
      normalizeProfileKey(row.profile) === gateway &&
      normalizeProfileKey(existing.profile) !== gateway
    ) {
      byId.set(pinId, row);
    }
  }
...
function pullRemotePins(): void {
  ...
  for (const row of rowsByPinId(loadedSessionRows()).values()) {
    ...
    const guardKey = unconfirmed.has(pinId)
      ? pinId
