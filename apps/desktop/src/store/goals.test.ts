...
describe("goal store", () => {
  ...
  it("keeps the current title for continuation and pause messages", () => {
    ...
    applyGoalStatusText(
      "s1",
      "↻ Continuing toward goal (1/20): next step is tests",
    );
  ...
  it("clears on no-goal output", () => {
    applyGoalStatusText(
      "s1",
      "⊙ Goal set (20-turn budget): ship another feature",
    );
  ...
  it("does not resurrect a done goal on hydration", () => {
    ...
    applyGoalStatusText("s1", "✓ Goal done (12/20 turns): ship the feature", {
      hydrate: true,
    });
  ...
  it("hydration drops a lingering done chip already on screen", () => {
    ...
    applyGoalStatusText("s1", "✓ Goal achieved: tests pass");
    applyGoalStatusText("s1", "✓ Goal done (12/20 turns): tests pass", {
      hydrate: true,
    });
  ...
  it("hydration still surfaces non-terminal goals", () => {
    applyGoalStatusText("s1", "⊙ Goal (active, 3/20 turns): ship the feature", {
      hydrate: true,
    });
    ...
    applyGoalStatusText("s2", "⏸ Goal (paused, 20/20 turns): other work", {
      hydrate: true,
    });

    expect($goalsBySession.get().s2).toMatchObject({
      status: "paused",
      title: "other work",
    });
  });
