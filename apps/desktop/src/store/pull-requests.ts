...
export const $pullRequestsByBranch = atom<
  Record<string, HermesBranchPullRequest>
>({});
...
const $prScannedSessions = persistentAtom<string[]>(
...
export const branchPrKey = (repoRoot: string, branch: string): string =>
  `${repoRoot}\n${branch}`;
...
export const numberPrKey = (repoRoot: string, number: number): string =>
  `${repoRoot}\n#${number}`;
...
export function sessionPrKey(session: SessionInfo): null | string {
  ...
  return root && branch && !TRUNK_BRANCHES.has(branch.toLowerCase())
    ? branchPrKey(root, branch)
    : null;
}

/** Bind a session to the branch it just opened a PR from. */
export function stampSessionPrBranch(
  sessionId: string,
  repoRoot: string,
  branch: string,
): void {
  ...
  $prBranchBySession.set({
    ...$prBranchBySession.get(),
    [sessionId]: branchPrKey(repoRoot, branch),
  });
}
...
export async function recoverSessionPullRequests(
  ...
  for (const session of sessions) {
    if (
      ...
      !scanned.has(session.id) &&
      !sessionPrKey(session)
    ) {
  ...
  try {
    const { pull_requests: found, scanned: asked } =
      await scanSessionPullRequests([...roots.keys()]);
...
export function pullRequestBucket(
  pr: HermesBranchPullRequest | undefined,
): PullRequestBucket {
  if (!pr) {
    return "none";
  }
...
export async function refreshPullRequests(
  ...
  const stale = Object.keys(lookupsByRepo).filter(
    (root) =>
      !inFlight.has(root) &&
      (force || now - (fetchedAt.get(root) ?? 0) > PR_STALE_MS),
  );
    ...
    stale.map(async (root) => {
      ...
      try {
        ...
        const next = Object.fromEntries(
          Object.entries($pullRequestsByBranch.get()).filter(
            ([key]) => !key.startsWith(`${root}\n`),
          ),
        );
