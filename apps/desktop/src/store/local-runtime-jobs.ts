...
function jobsEqual(
  a: readonly LocalRuntimeJob[],
  b: readonly LocalRuntimeJob[],
) {
...
function notifySettled(
  previous: readonly LocalRuntimeJob[],
  next: readonly LocalRuntimeJob[],
) {
  const wasRunning = new Set(
    previous.filter((j) => j.status === "running").map((j) => j.job_id),
  );

  for (const job of next) {
    if (
      job.status === "running" ||
      !wasRunning.has(job.job_id) ||
        ...
        message:
          job.kind === "model-download"
            ...
            : job.kind === "model-activate"
              ? translateNow(
                  "settings.localModels.activateDoneToast",
                  job.target,
                )
              : job.kind === "quickstart"
                ? translateNow(
                    "settings.localModels.quickstartDoneToast",
                    job.target,
                  )
...
async function poll() {
  ...
  const anyRunning = $localRuntimeJobs
    .get()
...
export function runningDownloadFor(
  jobs: readonly LocalRuntimeJob[],
  modelId: string,
): LocalRuntimeJob | null {
  return (
    jobs.find(
      (j) =>
...
export function runningModelDownloads(
  jobs: readonly LocalRuntimeJob[],
): LocalRuntimeJob[] {
  return jobs.filter(
    (j) =>
      j.status === "running" &&
      (j.kind === "model-download" ||
        (j.kind === "quickstart" && DOWNLOAD_PHASES.has(j.phase))),
  );
}

export function runningRuntimeInstall(
  jobs: readonly LocalRuntimeJob[],
): LocalRuntimeJob | null {
  return (
    jobs.find((j) => j.kind === "runtime-install" && j.status === "running") ??
    null
  );
}
    ...
    if (status.enabled && status.update_available) {
      ...
      notify({
        ...
        message: translateNow(
          "settings.localModels.updateToast",
          status.configured_tag,
        ),
      });
    }
  } catch {
