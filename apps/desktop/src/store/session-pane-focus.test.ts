...
describe("focusing a saved Bot Chat requires a visible pane", () => {
  ...
  it("re-adopts a saved tab after a profile overlay replaces the layout", async () => {
    ...
    applyDesktopOverlay("imported-profile", {
      version: 1,
      layoutTree: model.group(["workspace"], {
        active: "workspace",
        id: "imported-main",
      }),
    });
    ...
    expect(
      states.focusWorkspaceOwnerSessionTile(
    ...
    expect(states.sessionTileOwnerRoute("canonical-chat")).toEqual(
      scope.ownerRoute,
    );
  });
