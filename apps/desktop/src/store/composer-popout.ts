...
const POPOUT_GESTURES_ENABLED_STORAGE_KEY =
  "hermes.desktop.composerPopout.gesturesEnabled";
...
const gesturesEnabledAtLoad = storedBoolean(
...
const DEFAULT_ZONE: PopoutZoneState = {
  poppedOut: false,
...
function legacyPosition(): PopoutPosition {
  try {
    const parsed = JSON.parse(
      storedString(LEGACY_POSITION_KEY) || "null",
    ) as unknown;

    return isPosition(parsed)
...
function load(): Record<string, PopoutZoneState> {
  ...
  try {
    const parsed = JSON.parse(
      storedString(POPOUT_STORAGE_KEY) || "null",
    ) as unknown;
    ...
    for (const [id, value] of Object.entries(
      ...
      if (typeof zone?.poppedOut === "boolean" && isPosition(zone.position)) {
        out[id] = {
          poppedOut: gesturesEnabledAtLoad && zone.poppedOut,
...
export const $composerPopoutZones =
  atom<Record<string, PopoutZoneState>>(load());
...
const zoneState = (
  ...
  groupId: string,
): PopoutZoneState => zones[groupId] ?? legacySeed ?? DEFAULT_ZONE;
...
export function $composerPopoutZone(
  groupId: string,
): ReadableAtom<PopoutZoneState> {
  let cached = zoneCache.get(groupId);

  if (!cached) {
    cached = computed($composerPopoutZones, (zones) =>
      zoneState(zones, groupId),
    );
...
export const popoutBoundsElement = (composer: Element | null): Element | null =>
  (composer?.closest("[data-chat-surface]") ?? document).querySelector(
...
export function readPopoutBounds(
  composer: Element | null,
): PopoutBounds | undefined {
...
export function clampPopoutPosition(
  ...
  return {
    bottom: clampRange(
      bottom,
      vh - a.bottom + EDGE_MARGIN,
      vh - a.top - height - EDGE_MARGIN,
    ),
    right: clampRange(
      right,
      vw - a.right + EDGE_MARGIN,
      vw - a.left - width - EDGE_MARGIN,
    ),
  };
}
...
export function setComposerPopoutGesturesEnabled(value: boolean) {
  ...
  const docked = Object.fromEntries(
    Object.entries(zones).map(([groupId, zone]) => [
      groupId,
      { ...zone, poppedOut: false },
    ]),
  );
...
export function pruneComposerPopoutZones(liveGroupIds: Iterable<string>) {
  ...
  const next = Object.fromEntries(
    Object.entries(zones).filter(([id]) => live.has(id)),
  );
