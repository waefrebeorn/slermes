...
vi.mock("@/hermes", () => ({
  HermesGateway: class {
    ...
    request = vi.fn(async (method: string, params: Record<string, unknown>) => {
      ...
      return method === "prompt.submit" && promptAckStatus
        ? { status: promptAckStatus }
        : { method, params };
    });
    close = vi.fn();
    emit = (event: {
...
const { requestForSessionProfile, sessionRpcNeedsProfileRoute } =
...
describe("sessionRpcNeedsProfileRoute", () => {
  ...
  it("pins a route owner with a connectionId", () => {
    ...
    expect(
      sessionRpcNeedsProfileRoute({ connectionId: "", profile: "developer" }),
    ).toBe(false);
  });
});
...
describe("requestForSessionProfile", () => {
  it("clears a dead-runtime latch only after a successful resume or activate", async () => {
    ...
    await requestForSessionProfile(null, ambient as never, "session.activate", {
  ...
  it("keeps routing a bare profile owner through its legacy profile pool when a connection registry exists", async () => {
    ...
    await expect(
      requestForSessionProfile("loki", ambient as never, "session.resume", {
        session_id: "stored-a",
      }),
    ).resolves.toEqual({
      method: "session.resume",
      params: { session_id: "stored-a" },
    });
  ...
  it("keeps concurrent same-name requests pinned while foreground activation changes", async () => {
    ...
    const fromA = requestForSessionProfile(
      routeA,
      ambient as never,
      "session.resume",
      {
    ...
    const fromB = requestForSessionProfile(
      routeB,
      ambient as never,
      "session.resume",
      {
    ...
    expect(secondaryGateways[1].request).toHaveBeenCalledWith(
      "session.resume",
      {
        profile: "backend-a",
        session_id: "stored-a",
    ...
    expect(secondaryGateways[2].request).toHaveBeenCalledWith(
      "session.resume",
      {
        profile: "backend-b",
        session_id: "stored-b",
  ...
  it("dispatches on the owning profile's own socket when the active route moved off it (#89206)", async () => {
    ...
    }>("loki", ambient as never, "session.resume", {
    ...
    expect(result).toEqual({
      method: "session.resume",
      params: { session_id: "stored-loki-chat" },
    });
    expect(secondaryGateways).toHaveLength(1);
  ...
  it.each(["complete", "completed", "error"])(
    "releases a routed socket for terminal ACK status %s",
    async (status) => {
      const primary = makePrimary();
      setPrimaryGateway(primary as never, "default");
      installDesktop();
  ...
  it("releases turn leases when their route is pruned and creates a fresh route next time", async () => {
    ...
    await requestForSessionProfile(route, ambient as never, "session.resume", {
      session_id: "rt-pruned",
    });
  ...
  it("routes an owner that IS the primary profile onto the primary socket (no active comparison)", async () => {
    ...
    const result = await requestForSessionProfile<{
      method: string;
      params: Record<string, unknown>;
    }>("loki", ambient as never, "session.activate", { session_id: "rt-1" });

    expect(secondaryGateways).toHaveLength(0);
    expect(primary.request).toHaveBeenCalledWith("session.activate", {
      session_id: "rt-1",
    });
    expect(result).toEqual({
  ...
  it("keeps the ambient dispatcher for sessions with no owning profile", async () => {
    ...
    const ambient = vi.fn(async () => ({ ambient: true }));
    await requestForSessionProfile(null, ambient as never, "session.usage", {
      session_id: "rt-2",
    });
  ...
  it("preserves ambient request arity as optional controls are supplied", async () => {
    ...
    expect(ambient).toHaveBeenNthCalledWith(
      2,
      "session.usage",
      params,
      1_800_000,
