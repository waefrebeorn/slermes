...
function nextGoalFromText(
  ...
  const now = Date.now();
  const fromSet = goalTitleFromLine(
    line,
    /^⊙ Goal set(?:\s*\([^)]*\))?:\s*(.+)$/,
  );
  const fromActive = goalTitleFromLine(
    line,
    /^⊙ Goal\s*\([^)]*active[^)]*\):\s*(.+)$/,
  );
  ...
  if (fromSet || fromActive || fromResume) {
    return {
      status: "active",
      title: fromSet || fromActive || fromResume,
      updatedAt: now,
    };
  }

  const fromWaiting = goalTitleFromLine(
    line,
    /^⏳ Goal\s*\([^)]*(?:parked|active)[^)]*\):\s*(.+)$/,
  );
  ...
  const fromPaused = goalTitleFromLine(
    line,
    /^⏸ Goal(?:\s*\([^)]*\)| paused)?:\s*(.+)$/,
  );
...
export function applyGoalStatusText(
  sid: string,
  text: string,
