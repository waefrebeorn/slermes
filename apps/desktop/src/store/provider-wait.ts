...
export function sessionProviderWait(sessionId: null | string) {
  return computed(
    ...
    (sessions) => sessions[keyFor(sessionId)] ?? "",
  );
}

export function setSessionProviderWait(
...
export function clearSessionProviderWait(
  sessionId: string | null | undefined,
): void {
...
export function providerWaitText(text: string): string {
  ...
  return /^(?:⏳|⚠|↻|⚙)\s*(?:waiting on|loading|processing prompt|no (?:output|response)|model returned)/i.test(
    value,
  )
    ? value
    : "";
...
export function parseModelLoadWait(
  ...
  const value = text.trim();
  const load = /^⏳\s*loading\s+(.+?)\s+into memory\s+—\s+(\d{1,3})%/i.exec(
    value,
  );
  ...
  if (prefill) {
    return {
      ...
      percent:
        prefill[1] === undefined
          ? null
          : Math.max(0, Math.min(100, Number(prefill[1]))),
    };
  }
