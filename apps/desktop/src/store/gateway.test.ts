...
describe("connection-scoped dial failure identity (#95421)", () => {
  it("logs the route scope while preserving the original dial error", async () => {
    ...
    try {
      await expect(openGatewayForAgent("work", "default")).rejects.toBe(
        dialError,
      );
      await expect(openGatewayForAgent("homelab", "default")).rejects.toBe(
        dialError,
      );
      ...
      expect(
        messages.every((message) => message.includes('profile="default"')),
      ).toBe(true);
...
describe("secondary connection timeout (#93454)", () => {
  it("rejects instead of hanging forever when openSecondary's getConnection() wedges", async () => {
    ...
    const pending = expect(ensureGatewayForProfile("work")).rejects.toThrow(
      'Timed out connecting to profile "work"',
    );
