...
const pillsFor = (sessionId: string) =>
  ($composerSuggestionsBySession.get()[sessionId] ?? []).map((s) => s.id);
...
describe("composer suggestion bus", () => {
  ...
  it("replaces an offer whose rendered copy changed under the same key", () => {
    offerSuggestions("s7", "test", [
      { ...suggestion("linear"), tip: "because you mentioned “linear”" },
    ]);
    offerSuggestions("s7", "test", [
      { ...suggestion("linear"), tip: "because you pasted linear.app" },
    ]);

    // Same key, new trigger — the strip must paint the new reason, not the
    // first one it ever saw.
    expect(
      ($composerSuggestionsBySession.get().s7 ?? []).map((s) => s.tip),
    ).toEqual(["because you pasted linear.app"]);
  ...
  it("re-offering the same key swaps in the fresh invoke closure", async () => {
    ...
    const offer = (tag: string) =>
      offerSuggestions("s8", "test", [
        {
          ...suggestion("linear"),
          invoke: async () => void calls.push(tag),
          label: `Add linear ${tag}`,
        },
      ]);
