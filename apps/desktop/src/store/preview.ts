...
export type PreviewRecordSource =
  "explicit-link" | "file-browser" | "manual" | "tool-result";
...
function isPdfFileTarget(target: PreviewTarget): boolean {
  ...
  if (
    [target.path, target.source].some((value) =>
      value ? /\.pdf$/i.test(value) : false,
    )
  ) {
    return true;
  }
...
export function decodePreviewTabs(raw: string): PreviewTab[] {
  ...
  return (Array.isArray(parsed) ? parsed.filter(isPreviewTab) : []).map(
    (tab) =>
...
function resolveActiveTab(
  tabs: PreviewTab[],
  activeTabId: RightRailTabId | null,
): PreviewTab | null {
...
export function commitBrowserTabLocation(
  tabId: string,
  url: string,
  title?: string,
) {
  ...
  if (
    tab.target.kind !== "url" ||
    (tab.target.url === nextUrl &&
      (!nextTitle || tab.target.label === nextTitle))
  ) {
    return;
  }
...
export const $dockedPreviewTabs = computed(
  [$previewTabs, $poppedBrowserTabIds],
  (tabs, popped) =>
...
export const $previewServerRestartStatus = computed(
  $previewServerRestart,
  (restart) => restart?.status ?? "idle",
);
...
export function openPreview(
  ...
  const id =
    resolved.kind === "url" ? browserTabId(current) : previewTabId(resolved);
