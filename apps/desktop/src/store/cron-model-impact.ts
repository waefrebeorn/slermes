...
export function parseCronModelImpact(value: unknown): CronModelImpact | null {
  ...
  if (
    ...
    (impact.truncated &&
      (impact.jobs.length !== MAX_JOBS || count <= impact.jobs.length))
  ) {
    return null;
  }
...
function currentActionScope(profile: string, connection: string): boolean {
  return (
    profileIdentity() === profile &&
    getCronModelImpactScope().connection === connection
  );
}
    ...
    if (request.confirm_expensive_model || options?.skipConfirmPrompt) {
      ...
      throw new Error(
        result.confirm_message?.trim() ||
          translateNow("cron.modelImpact.saveFailed"),
      );
    }

    const accepted = await confirmModelWarning(
      result.confirm_message?.trim() ?? "",
    );
...
export function invalidateCronModelImpactScope(
  options: { clearNotification?: boolean } = {},
): void {
