...
test("disk-full / ENOSPC errors toast a free-space message", () => {
  expect(
    isDiskFullErrorMessage("OSError: [Errno 28] No space left on device"),
  ).toBe(true);
  expect(
    isDiskFullErrorMessage(
  ...
  expect(
    isDiskFullErrorMessage(
      "disk full: session storage could not be written — free some disk space",
    ),
  ).toBe(true);
  expect(
    isDiskFullErrorMessage("This is often a full disk — free some space"),
  ).toBe(true);
  expect(
    isDiskFullErrorMessage(
      "session storage could not be written: permission denied",
    ),
  ).toBe(false);
  expect(isDiskFullErrorMessage("network timeout")).toBe(false);

  notifyError(
    new Error("OSError: [Errno 28] No space left on device: state.db"),
    "Prompt failed",
  );
