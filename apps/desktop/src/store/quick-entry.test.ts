...
function run(
  events: QuickComposerEvent[],
  from: QuickComposerState = initialQuickComposerState,
) {
  let state = from;
...
describe("quickComposerReducer", () => {
  ...
  it("submit is DISABLED while disconnected — the draft survives for the reconnect", () => {
    ...
    expect(after.sent).toEqual([
      { target: QUICK_TARGET_CURRENT, text: "hello?" },
    ]);
  });
  ...
  it("a full summon → type → submit → summon cycle sends exactly once per round", () => {
    ...
    const second = run(
      [{ type: "shown" }, { draft: "two", type: "edit" }, { type: "submit" }],
      first.state,
    );

    expect(first.sent).toEqual([{ target: QUICK_TARGET_CURRENT, text: "one" }]);
    expect(second.sent).toEqual([
      { target: QUICK_TARGET_CURRENT, text: "two" },
    ]);
  });
});
