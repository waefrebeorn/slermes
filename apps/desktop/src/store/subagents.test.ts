...
describe("subagent store", () => {
  ...
  it("keeps root nodes in spawn order, not task index order", () => {
    ...
    expect(buildSubagentTree(listFor("s1")).map((n) => n.id)).toEqual([
      "a",
      "b",
    ]);
  });
  ...
  it("captures live thinking/progress/tool stream lines", () => {
    ...
    const item = listFor("s1")[0];
    expect(item?.stream.map((e) => e.kind)).toEqual([
      "tool",
      "thinking",
      "progress",
      "summary",
    ]);
    expect(item?.stream.find((e) => e.kind === "tool")?.text).toContain(
      "Search Files",
    );
    expect(item?.stream.find((e) => e.kind === "thinking")?.text).toBe(
      "plan the search order",
    );
    expect(item?.stream.find((e) => e.kind === "summary")?.text).toBe(
      "search complete",
    );
  });
  ...
  it("counts running/failed across every session, matching the aggregated tree", () => {
    ...
    const indicatorRunning = Object.values($subagentsBySession.get()).reduce(
      (n, l) => n + activeSubagentCount(l),
      0,
    );
    const indicatorFailed = Object.values($subagentsBySession.get()).reduce(
      (n, l) => n + failedSubagentCount(l),
      0,
    );
  ...
  it("normalises backend terminal statuses to recognised SubagentStatus values", () => {
    ...
    upsertSubagent(
      "s1",
      { status: "cancelled", subagent_id: "c", task_index: 2 },
      false,
      "subagent.complete",
    );
    upsertSubagent(
      "s1",
      { status: "canceled", subagent_id: "d", task_index: 3 },
      false,
      "subagent.complete",
    );
  ...
  it("falls back to a placeholder when timeout duration is missing", () => {
    ...
    upsertSubagent(
      "s1",
      { status: "timeout", subagent_id: "t2", task_index: 0 },
      false,
      "subagent.complete",
    );
  ...
  it("fails closed on unrecognized completion statuses but stays lenient for live events", () => {
    ...
    upsertSubagent(
      "s1",
      {
        status: "some_future_terminal_status",
        subagent_id: "u1",
        task_index: 0,
      },
      false,
  ...
  it.each(["running", "queued"] as const)(
    ...
    (status) => {
      ...
      upsertSubagent(
        "s1",
        { status, subagent_id: "ic1", task_index: 0 },
        false,
        "subagent.complete",
      );
