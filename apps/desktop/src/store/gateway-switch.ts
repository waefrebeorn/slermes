...
export function registerGatewaySwitchLifecycle(
  lifecycle: GatewaySwitchLifecycle,
): () => void {
 ...
 */
export function recoverActiveSourceAfterFailedGatewaySwitch(
  token: GatewaySwitchToken,
): void {
  ...
  void Promise.resolve()
    .then(() =>
      isCurrentGatewaySwitch(token)
        ? lifecycle.refreshSessions(() => isCurrentGatewaySwitch(token))
        : undefined,
    )
    .catch(() => undefined)
    .finally(() => {
