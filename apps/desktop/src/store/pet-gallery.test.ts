...
describe("pet gallery pet.info sync", () => {
  ...
  it("uses pet.info.meta and keeps the cached spritesheet when the revision is current", async () => {
    ...
    const requestMock = vi.fn(async (method: string) => {
      ...
      if (method === "pet.info") {
        throw new Error(
          "full pet.info should not be called for an unchanged sprite",
        );
      }
  ...
  it("keeps mutation sync on metadata when the selected pet sprite is unchanged", async () => {
    ...
    const requestMock = vi.fn(async (method: string) => {
      ...
      if (method === "pet.info") {
        throw new Error(
          "full pet.info should not be called after an unchanged select",
        );
      }
    ...
    await expect(
      adoptPet(request, "boba", "Could not adopt pet."),
    ).resolves.toBe(true);
