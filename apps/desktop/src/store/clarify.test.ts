...
describe("normalizeChoices", () => {
  ...
  it("filters out non-string items", () => {
    expect(normalizeChoices(["a", 42, "b", null, "c"])).toEqual([
      "a",
      "b",
      "c",
    ]);
  ...
  it("drops blank and whitespace-only strings", () => {
    expect(normalizeChoices(["a", "", "b", "   ", "c"])).toEqual([
      "a",
      "b",
      "c",
    ]);
  ...
  it("drops empty items and keeps valid ones", () => {
    expect(normalizeChoices(["valid", "  ", "", "also valid"])).toEqual([
      "valid",
      "also valid",
    ]);
  });
