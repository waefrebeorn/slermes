...
vi.mock("@/lib/oneshot", () => ({
  requestOneShot: (args: unknown) => requestOneShot(args),
}));
...
describe("refreshReview", () => {
  ...
  it("populates the changed-file list from the bridge", async () => {
    stubReview({
      list: vi.fn(async () => ({ files: [file("a.ts"), file("b.ts")] })),
    });
    $reviewOpen.set(true);
  ...
  it("filters excluded paths (node_modules et al.) out of the list", async () => {
    stubReview({
      list: vi.fn(async () => ({
        files: [file("src/a.ts"), file("node_modules/x/index.js")],
      })),
    });
    $reviewOpen.set(true);
...
describe("$reviewMaxChurn", () => {
  it("is the largest added+removed across files", () => {
    $reviewFiles.set([
      file("a", { added: 3, removed: 2 }),
      file("b", { added: 10, removed: 1 }),
      file("c"),
    ]);
...
describe("selectReviewFile / clearReviewSelection", () => {
  it("sets the selected path and fetches its diff", async () => {
    ...
    expect(review.diff).toHaveBeenCalledWith(
      "/repo",
      "a.ts",
      "uncommitted",
      null,
...
describe("view state", () => {
  ...
  it("openReview pins the pane to a tile worktree when scoped", async () => {
    ...
    await Promise.resolve();
    await Promise.resolve();
    expect(review.list).toHaveBeenCalledWith(
      "/tile-worktree",
      "uncommitted",
      null,
    );
  ...
  it("scoped pane ignores main-pane cwd changes", async () => {
    const review = stubReview({
      list: vi.fn(async (cwd: string) => ({
        files: [file(cwd === "/tile" ? "tile.ts" : "main.ts")],
      })),
    });
...
describe("ship flow", () => {
  ...
  it("createOrOpenPr creates a PR when none exists, then opens it", async () => {
    const review = stubReview({
      createPr: vi.fn(async () => ({ url: "https://example.com/pr/new" })),
    });
...
describe("generateCommitMessage", () => {
  ...
  it("returns empty (no model call) when the diff is blank", async () => {
    stubReview({
      commitContext: vi.fn(async () => ({ diff: "   ", recent: "" })),
    });
