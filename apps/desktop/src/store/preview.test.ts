...
describe("preview store", () => {
  ...
  it("does not notify status subscribers for restart progress text", () => {
    ...
    const unsubscribe = $previewServerRestartStatus.subscribe((status) =>
  ...
  it("gives every kind of target its own tab, side by side", () => {
    ...
    expect($previewTabs.get().map((tab) => tab.target.kind)).toEqual([
      "file",
      "url",
      "artifact",
    ]);
  ...
  it("commits the live page onto a Browser tab without changing its id", () => {
    ...
    expect($previewTabs.get()[0].target.url).toBe(
      "https://news.ycombinator.com/item?id=1",
    );
  ...
  it("navigates the Browser you are looking at", () => {
    ...
    expect($previewTabs.get().find((tab) => tab.id === first)?.target.url).toBe(
      "https://www.reddit.com",
    );
  ...
  it("re-fronts an existing tab instead of duplicating it, refreshing its target", () => {
    openPreview(
      { ...fileTarget("/work/demo.html"), label: "old" },
      "file-browser",
    );
    openPreview(
      { ...fileTarget("/work/demo.html"), label: "new" },
      "file-browser",
    );
  ...
  it("closes a tab whose url or label matches even when source differs", () => {
    ...
    openPreview(
      { ...fileTarget("/work/demo.html"), label: "Demo" },
      "tool-result",
    );
  ...
  it("persists file and url tabs but never artifacts, whose content is memory-only", () => {
    ...
    const stored =
      window.localStorage.getItem("hermes.desktop.previewTabs.v2") ?? "";
  ...
  it("does not persist remote HTML without its in-memory document", () => {
    openPreview({
      ...fileTarget("/remote/report.html"),
      dataUrl: "data:text/html;base64,PGgxPnJlbW90ZTwvaDE+",
    });
  ...
  it("preserves an explicit HTML source fallback", () => {
    openPreview(
      { ...fileTarget("/remote/report.html"), renderMode: "source" },
      "tool-result",
    );
