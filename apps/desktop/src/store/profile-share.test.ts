...
vi.mock("@/hermes", () => ({
  exportProfileArchive: vi.fn(async () => ({
    archive: "/tmp/out.tar.gz",
    ok: true,
  })),
  getProfiles: vi.fn(async () => ({ profiles: [] })),
  importProfileArchive: vi.fn(async () => ({
    desktop: null,
    name: "imported",
    ok: true,
    path: "/tmp/p",
...
const { applyDesktopOverlay, buildDesktopOverlay, exportProfileBundle } =
...
describe("applyDesktopOverlay", () => {
  ...
  it("ignores a skin that resolves to nothing and junk layout trees", () => {
    ...
    expect(skinPref.resolve("glam-copy")).toBe(
      skinPref.resolve("some-unassigned"),
    );
...
describe("exportProfileBundle", () => {
  it("stages desktop.json into the archive through extra_files", async () => {
    ...
    const overlay = JSON.parse(
      call[1]?.extraFiles?.["desktop.json"] ?? "{}",
    ) as ProfileDesktopOverlay;
    expect(overlay.skin).toBe("mono");
