...
describe("composer queue store", () => {
  ...
  it("clones attachments when queueing", () => {
    ...
    expect(getQueuedPrompts(SESSION_KEY)[0]?.attachments[0]).not.toBe(
      source[0],
    );
  });
  ...
  it("updates and removes queued entries by id", () => {
    ...
    expect(
      updateQueuedPromptText(SESSION_KEY, first!.id, "draft one edited"),
    ).toBe(true);
  ...
  it("clears queue state for a session", () => {
    enqueueQueuedPrompt(SESSION_KEY, {
      attachments: [attachment("img-1", "image")],
      text: "queued",
    });
...
describe("migrateQueuedPrompts", () => {
  ...
  it("appends after existing target entries (FIFO preserved)", () => {
    ...
    expect(getQueuedPrompts("rt-new").map((e) => e.text)).toEqual([
      "already here",
      "migrated",
    ]);
  });
  ...
  it("must not be used across sessions without a same-lineage guard (documents the leak)", () => {
    ...
    expect(getQueuedPrompts("root-b").map((e) => e.text)).toEqual([
      "belongs to A",
    ]);
  });
});
...
describe("shouldAutoDrain", () => {
  ...
  it("does not drain a parked queue, even when idle", () => {
    ...
    expect(
      shouldAutoDrain({ isBusy: false, parked: true, queueLength: 1 }),
    ).toBe(false);
  });

  it("drains again once the park is lifted", () => {
    expect(
      shouldAutoDrain({ isBusy: false, parked: false, queueLength: 1 }),
    ).toBe(true);
  });
});
