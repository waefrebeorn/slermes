...
export async function replayPendingApproval(
  ...
  const result =
    rawResult && typeof rawResult === "object"
      ? (rawResult as { approvals?: PendingApprovalPayload[] })
      : {};

  const pending = Array.isArray(result?.approvals)
    ? result.approvals[0]
    : undefined;
...
export const sessionApprovalInlineVisible = (sessionId: string | null) =>
  computed(
    $approvalInlineAnchors,
    (anchors) => (anchors[keyFor(sessionId)] ?? 0) > 0,
  );
...
export const $activeSessionAwaitingInput = computed(
  ...
  (clarify, approval, sudo, secret) =>
    Boolean(clarify || approval || sudo || secret),
);
 ...
 *  not on every render. */
export const hasBlockingPromptRequest = (
  sessionId: string | null | undefined,
): boolean => {
  const key = keyFor(sessionId);

  return Boolean(
    approval.$all.get()[key] || sudo.$all.get()[key] || secret.$all.get()[key],
  );
};
...
export function sessionAwaitingInput(sessionId: string | null) {
  return computed(
    ...
    (clarify, approvals, sudos, secrets) => {
      ...
      return Boolean(
        clarify[key] || approvals[key] || sudos[key] || secrets[key],
      );
    },
  );
}
