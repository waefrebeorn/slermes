...
const cachedState = (storedSessionId: string) =>
  ({ messages: [], storedSessionId }) as never;

const tile = (storedSessionId: string, runtimeId?: string) =>
  ({ runtimeId, storedSessionId }) as never;
...
describe("markRuntimeGone", () => {
  it("unbinds the tile holding the dead runtime so its resume effect refires", () => {
    ...
    expect(
      tiles.find((t) => t.storedSessionId === STORED)?.runtimeId,
    ).toBeUndefined();
    // A healthy neighbour must not be disturbed.
    expect(tiles.find((t) => t.storedSessionId === "stored-2")?.runtimeId).toBe(
      "runtime-live",
    );
  });
...
describe("gone-latch classifier and rebind seam", () => {
  it("recognizes structured 4001 and bare legacy text without misclassifying coded errors", () => {
    ...
    expect(
      isSessionGoneForBackgroundPolling(
        new JsonRpcGatewayError("session not found", { code: 5007 }),
      ),
    ).toBe(false);
    expect(
      isSessionGoneForBackgroundPolling(
    ...
    expect(
      isSessionGoneForBackgroundPolling(
        new Error("Error invoking remote method 'x': Error: session not found"),
      ),
    ).toBe(true);
    expect(
      isSessionGoneForBackgroundPolling(
        new Error("tool failed: upstream said session not found"),
      ),
    ).toBe(false);
  });
