...
describe("clampPopoutPosition", () => {
  it("keeps the whole box inside its surface, not just its corner", () => {
    const { bottom, right } = clampPopoutPosition(
      { bottom: 24, right: 24 },
      BOX,
      RIGHT_HALF,
    );
  ...
  it("pulls an out-of-bounds intent back into the surface", () => {
    ...
    const { right } = clampPopoutPosition(
      { bottom: 24, right: 900 },
      BOX,
      RIGHT_HALF,
    );
  ...
  it("assumes the compact float width when the box is unmeasured", () => {
    ...
    const { right } = clampPopoutPosition(
      { bottom: 24, right: 5000 },
      undefined,
      RIGHT_HALF,
    );

    expect(VW - right - POPOUT_WIDTH_REM * 16).toBeGreaterThanOrEqual(
      RIGHT_HALF.left,
    );
  });
});
...
describe("pop-out state is scoped to a layout zone", () => {
  ...
  it("keeps each zone position independent", () => {
    ...
    expect(getComposerPopoutZone(LEFT_ZONE).position).toEqual({
      bottom: 100,
      right: 100,
    });
    expect(getComposerPopoutZone(RIGHT_ZONE).position).toEqual({
      bottom: 24,
      right: 24,
    });
  });
  ...
  it("reports the default for a zone nobody has touched", () => {
    ...
    expect(getComposerPopoutZone("g-unseen")).toEqual({
      poppedOut: false,
