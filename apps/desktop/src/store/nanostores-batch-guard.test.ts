...
describe("nanostores batch()", () => {
  it("is not annotated @__NO_SIDE_EFFECTS__ — bundlers would erase its calls", () => {
    ...
    const atomSource = readFileSync(
      path.join(path.dirname(nanostoresEntry), "atom/index.js"),
      "utf8",
    );

    // Capture any block comment immediately preceding the batch export.
    const match = atomSource.match(
      /(\/\*(?:[^*]|\*(?!\/))*\*\/\s*)?export const batch\b/,
    );

    expect(
      match,
      "nanostores atom/index.js should export const batch",
    ).toBeTruthy();
