...
describe("remoteHostLabel", () => {
  it("keeps a non-default port and drops default ones", () => {
    expect(remoteHostLabel("https://hermes.example.com:8443/x")).toBe(
      "hermes.example.com:8443",
    );
    expect(remoteHostLabel("https://hermes.example.com:443")).toBe(
      "hermes.example.com",
    );
    expect(remoteHostLabel("http://hermes.example.com:80")).toBe(
      "hermes.example.com",
    );
  });
...
describe("refreshProfileRemoteOverrides", () => {
  ...
  it("drops a stale override entry when the profile went back to local", async () => {
    $profileRemoteOverrides.set({
      work: { host: "old.example.com", url: "https://old.example.com" },
    });
...
describe("notifyRemoteOverrideAuthFailure", () => {
  ...
  it("surfaces a re-enter-token toast whose action reopens the dialog on a 401", () => {
    const handled = notifyRemoteOverrideAuthFailure(
      "work",
      new Error("WebSocket handshake failed: 401 Unauthorized"),
    );
  ...
  it("ignores connectivity failures — a down host is not a rotated token", () => {
    ...
    expect(
      notifyRemoteOverrideAuthFailure(
        "work",
        new Error("timeout waiting for backend"),
      ),
    ).toBe(false);
  ...
  it("ignores profiles without an override entirely", () => {
    expect(
      notifyRemoteOverrideAuthFailure("home", new Error("401 Unauthorized")),
    ).toBe(false);
    expect($notifications.get()).toEqual([]);
  });
});
