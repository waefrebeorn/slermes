...
describe("Hermes REST parity helpers (hub / mcp / maintenance)", () => {
  ...
  it("loads the built-in optional-skills catalog", async () => {
    ...
    expect(api).toHaveBeenCalledWith(
      expect.objectContaining({ path: "/api/skills/hub/official" }),
    );
  });
  ...
  it("encodes hub search params", async () => {
    ...
    expect(api).toHaveBeenCalledWith(
      expect.objectContaining({
        path: "/api/skills/hub/search?q=gif+search&source=official&limit=5",
      }),
    );
  });
  ...
  it("reads the MCP catalog", async () => {
    ...
    expect(api).toHaveBeenCalledWith(
      expect.objectContaining({ path: "/api/mcp/catalog" }),
    );
  });
  ...
  it("manages the curator", async () => {
    ...
    expect(api).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ path: "/api/curator" }),
    );
    expect(api).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
  ...
  it("reads a backend model catalog scoped to a provider row", async () => {
    ...
    expect(api).toHaveBeenCalledWith(
      expect.objectContaining({
        path: "/api/tools/toolsets/image_gen/models?provider=FAL.ai",
      }),
    );
  });
