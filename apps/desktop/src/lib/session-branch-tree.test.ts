...
describe("flattenSessionsWithBranches", () => {
  ...
  it("re-sorts roots by group recency by default (pinned-style jumps without preserveOrder)", () => {
    ...
    expect(
      flattenSessionsWithBranches([important, background]).map(
        (e) => e.session.id,
      ),
    ).toEqual(["background", "important"]);
  });
  ...
  it("preserveOrder keeps the caller's root order even when activity is newer lower down", () => {
    ...
    const branch = session("branch", {
      last_active: 50,
      parent_session_id: "important",
    });

    expect(
      flattenSessionsWithBranches([important, background, branch], {
        preserveOrder: true,
      }).map((e) => ({
        id: e.session.id,
        stem: e.branchStem,
