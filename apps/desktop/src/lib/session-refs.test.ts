...
describe("parseSessionRefValue", () => {
  ...
  it("treats bare values as session ids", () => {
    expect(parseSessionRefValue("20260101_abc123")).toEqual({
      sessionId: "20260101_abc123",
    });
  });
});
...
describe("sessionRefFallbackLabel", () => {
  it("truncates long ids", () => {
    expect(sessionRefFallbackLabel("default/20260610_120000_abcdef")).toBe(
      "20260610…",
    );
  });
...
describe("sessionRefCacheKey", () => {
  it("separates the same id across profiles", () => {
    expect(sessionRefCacheKey("work/abc")).not.toBe(
      sessionRefCacheKey("home/abc"),
    );
  });
...
describe("splitSessionRefValue", () => {
  it("peels prose punctuation off a bare value", () => {
    expect(splitSessionRefValue("default/abc123.")).toEqual({
      trailing: ".",
      value: "default/abc123",
    });
  });

  it("leaves a quoted value fenced", () => {
    expect(splitSessionRefValue("`my session`")).toEqual({
      trailing: "",
      value: "my session",
    });
  });
...
describe("linkifySessionRefs", () => {
  ...
  it("keeps trailing prose punctuation outside the link", () => {
    expect(linkifySessionRefs("see @session:default/abc123.")).toBe(
      "see [abc123](#session/default%2Fabc123).",
    );
  });
...
describe("preprocessMarkdown session refs", () => {
  it("linkifies a ref in prose", () => {
    expect(
      preprocessMarkdown("Context is in @session:work/20260101_abc123 there."),
    ).toContain("[20260101…](#session/work%2F20260101_abc123)");
  });

  it("leaves refs inside inline code alone", () => {
    const out = preprocessMarkdown(
      "Type `@session:work/20260101_abc123` to link a chat.",
    );
  ...
  it("leaves refs inside a fenced block alone", () => {
    const out = preprocessMarkdown(
      ["```text", "@session:work/20260101_abc123", "```"].join("\n"),
    );
