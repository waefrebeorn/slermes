...
describe("gateway event routing", () => {
  it("rehydrates pending approvals on reconnect ready and resumed session info", () => {
    expect(approvalReplaySessionId("gateway.ready", "active-1", null)).toBe(
      "active-1",
    );
    expect(
      approvalReplaySessionId("session.info", "active-1", "routed-1"),
    ).toBe("routed-1");
    expect(
      approvalReplaySessionId("message.delta", "active-1", "routed-1"),
    ).toBeNull();
  });
  ...
  it("does not replay against an active runtime the gateway already reported gone", () => {
    ...
    expect(
      approvalReplaySessionId("session.info", "dead-1", "dead-1", {
        explicit: false,
        isGone,
      }),
    ).toBeNull();
    expect(
      approvalReplaySessionId("gateway.ready", "dead-1", null, {
        explicit: false,
        isGone,
      }),
    ).toBeNull();
    ...
    expect(
      approvalReplaySessionId("session.info", "live-1", "live-1", {
        explicit: false,
        isGone,
      }),
    ).toBe("live-1");
  ...
  it("allows global events to remain unscoped", () => {
    ...
    expect(gatewayEventRequiresSessionId("preview.restart.progress")).toBe(
      false,
    );
