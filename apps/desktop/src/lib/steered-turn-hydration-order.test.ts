...
const steeredTurnRows: SessionMessage[] = [
  ...
  {
    content: "Build is green. Now writing the docs section",
    role: "assistant",
    row_id: 731381,
    timestamp: 103,
  },
  ...
  {
    content: "Now the ladder in the resolver:",
    role: "assistant",
    row_id: 731382,
    timestamp: 104,
  },
...
describe("toChatMessages on a persisted steered turn", () => {
  it("keeps the correction between the output it followed and the output it redirected", () => {
    ...
    expect(
      preCorrection.some((m) => chatMessageText(m).includes("Build is green")),
    ).toBe(true);
    expect(
      preCorrection.some((m) =>
        m.parts.some(
    ...
    expect(
      preCorrection.some((m) => chatMessageText(m).includes("tests first")),
    ).toBe(false);
    expect(
      preCorrection.some((m) =>
        m.parts.some(
    ...
    expect(
      postCorrection.some((m) => chatMessageText(m).includes("tests first")),
    ).toBe(true);
    expect(
      postCorrection.some((m) =>
        m.parts.some(
          (p) => p.type === "tool-call" && p.toolCallId === "call-2",
        ),
      ),
  ...
  it("a tool-result row arriving after the correction cannot pull its call below the user bubble", () => {
    ...
    const rows: SessionMessage[] = [
      { content: "go", role: "user", timestamp: 100 },
      {
        ...
        tool_calls: [
          {
            function: { arguments: '{"command":"sleep 60"}', name: "terminal" },
            id: "call-9",
            type: "function",
          },
        ],
    ...
    const correctionIndex = messages.findIndex(
      (m) => m.role === "user" && chatMessageText(m).includes("never mind"),
    );
    const callRowIndex = messages.findIndex((m) =>
      m.parts.some((p) => p.type === "tool-call" && p.toolCallId === "call-9"),
    );

    expect(correctionIndex).toBeGreaterThan(0);
    expect(callRowIndex).toBeGreaterThanOrEqual(0);
    expect(callRowIndex, messages.map((m) => m.role).join(" | ")).toBeLessThan(
      correctionIndex,
    );
    ...
    expect(
      messages
        .flatMap((m) => m.parts)
        .filter((p) => p.type === "tool-call" && p.toolCallId === "call-9"),
    ).toHaveLength(1);
  });
});
