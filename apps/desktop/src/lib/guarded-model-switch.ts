...
export interface SurfaceModelSwitchConfirmOptions<
  T extends GuardedModelSwitchResult,
> {
  ...
  const applyConfirmedSwitch = async () => {
    ...
    try {
      ...
      if (result?.confirm_required) {
        throw new Error(
          result.confirm_message?.trim() || options.failureMessage,
        );
      }
