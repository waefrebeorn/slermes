...
const PUNCTUATION_KEYS = new Set(
  Object.values(CODE_TO_KEY).filter((token) => token.length === 1),
);
...
export function comboFromEvent(event: KeyboardEvent): string | null {
  ...
  const base =
    baseKeyFromEventKey(event.key, event.shiftKey) ??
    baseKeyFromCode(event.code);
