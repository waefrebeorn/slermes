...
describe("contributed keybind actions", () => {
  it("dispatches, resolves its default combo, and disappears on unload", () => {
    ...
    expect(
      allKeybindActions().find((a) => a.id === "demo.newThing")?.label,
    ).toBe("Demo: New thing");

    dispose();

    expect(contributedKeybindHandler("demo.newThing")).toBeUndefined();
    expect(allKeybindActions().some((a) => a.id === "demo.newThing")).toBe(
      false,
    );
  });
  ...
  it("cannot shadow a built-in action id", () => {
    ...
    expect(
      allKeybindActions().filter((a) => a.id === "session.new"),
    ).toHaveLength(1);
  ...
  it("leaves no registry residue between plugin loads", () => {
    expect(
      registry.getArea(KEYBINDS_AREA).filter((c) => c.source === "plugin:demo"),
    ).toHaveLength(0);
  });
});
