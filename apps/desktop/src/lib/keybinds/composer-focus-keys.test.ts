...
describe("typeToFocusChar", () => {
  it("returns printables (case/symbols via event.key)", () => {
    ...
    expect(
      typeToFocusChar(keydown({ key: "A", code: "KeyA", shiftKey: true })),
    ).toBe("A");
    expect(
      typeToFocusChar(keydown({ key: "?", code: "Slash", shiftKey: true })),
    ).toBe("?");
  ...
  it("rejects non-printables and modified chords", () => {
    expect(
      typeToFocusChar(keydown({ key: "Enter", code: "Enter" })),
    ).toBeNull();
    expect(
      typeToFocusChar(keydown({ key: "a", code: "KeyA", metaKey: true })),
    ).toBeNull();
    expect(
      typeToFocusChar(keydown({ key: "a", code: "KeyA", isComposing: true })),
    ).toBeNull();
  });
});
...
describe("composerFocusKeysAllowed", () => {
  ...
  it("passes rebound chords; allows soft keys on the transcript", () => {
    expect(
      composerFocusKeysAllowed(
        keydown({ key: "i", code: "KeyI", metaKey: true }),
        "mod+i",
      ),
    ).toBe(true);
    expect(
      composerFocusKeysAllowed(
        keydown({ key: "/", code: "Slash", target: document.body }),
        "/",
      ),
    ).toBe(true);
    expect(
      composerFocusKeysAllowed(
        keydown({ key: "Enter", code: "Enter", target: document.body }),
        "enter",
      ),
    ).toBe(true);
    expect(
      composerFocusKeysAllowed(
        keydown({ key: "h", code: "KeyH", target: document.body }),
        "type",
      ),
    ).toBe(true);
  });
  ...
  it("refuses editables; refuses Enter on buttons but allows / and typing", () => {
    ...
    expect(
      composerFocusKeysAllowed(
        keydown({ key: "a", code: "KeyA", target: input }),
        "type",
      ),
    ).toBe(false);
    expect(
      composerFocusKeysAllowed(
        keydown({ key: "Enter", code: "Enter", target: button }),
        "enter",
      ),
    ).toBe(false);
    expect(
      composerFocusKeysAllowed(
        keydown({ key: "/", code: "Slash", target: button }),
        "/",
      ),
    ).toBe(true);
    expect(
      composerFocusKeysAllowed(
        keydown({ key: "a", code: "KeyA", target: button }),
        "type",
      ),
    ).toBe(true);
  });
  ...
  it("refuses when a dialog is open", () => {
    ...
    expect(
      composerFocusKeysAllowed(
        keydown({ key: "a", code: "KeyA", target: document.body }),
        "type",
      ),
    ).toBe(false);
  });
  ...
  it("leaves every key alone for a clarify card in a background tab", () => {
    ...
    expect(
      composerFocusKeysAllowed(
        keydown({ key: "a", target: document.body }),
        "type",
      ),
    ).toBe(true);
    expect(
      composerFocusKeysAllowed(
        keydown({ key: "Enter", target: document.body }),
        "enter",
      ),
    ).toBe(true);
  });
