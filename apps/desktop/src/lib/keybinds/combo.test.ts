...
describe("comboFromEvent — ctrl as a distinct modifier on macOS", () => {
  it('reports Control+Tab as "ctrl+tab" on macOS (not Cmd)', async () => {
    ...
    expect(comboFromEvent(keydown({ code: "Tab", ctrlKey: true }))).toBe(
      "ctrl+tab",
    );
    expect(
      comboFromEvent(keydown({ code: "Tab", ctrlKey: true, shiftKey: true })),
    ).toBe("ctrl+shift+tab");
  });
  ...
  it('keeps Cmd as "mod" and distinct from Control on macOS', async () => {
    ...
    expect(comboFromEvent(keydown({ code: "KeyK", metaKey: true }))).toBe(
      "mod+k",
    );
    expect(comboFromEvent(keydown({ code: "KeyK", ctrlKey: true }))).toBe(
      "ctrl+k",
    );
  });
  ...
  it("uses layout-aware letters for Cmd shortcuts on non-QWERTY layouts", async () => {
    ...
    expect(
      comboFromEvent(keydown({ code: "KeyI", key: "c", metaKey: true })),
    ).toBe("mod+c");
    expect(
      comboFromEvent(
        keydown({ code: "KeyI", key: "C", metaKey: true, shiftKey: true }),
      ),
    ).toBe("mod+shift+c");
  });
  ...
  it("uses layout-aware punctuation for Cmd shortcuts on non-QWERTY layouts", async () => {
    ...
    expect(
      comboFromEvent(keydown({ code: "KeyV", key: ".", metaKey: true })),
    ).toBe("mod+.");
    expect(
      comboFromEvent(keydown({ code: "KeyW", key: ",", metaKey: true })),
    ).toBe("mod+,");
    expect(
      comboFromEvent(keydown({ code: "BracketLeft", key: "/", metaKey: true })),
    ).toBe("mod+/");
    // AZERTY reaches "," from the physical QWERTY M key.
    expect(
      comboFromEvent(keydown({ code: "KeyM", key: ",", metaKey: true })),
    ).toBe("mod+,");
  });
  ...
  it("keeps digits physical so AZERTY's shifted number row still binds", async () => {
    ...
    expect(
      comboFromEvent(keydown({ code: "Digit1", key: "&", metaKey: true })),
    ).toBe("mod+1");
    expect(
      comboFromEvent(keydown({ code: "Digit1", key: "1", metaKey: true })),
    ).toBe("mod+1");
  });
  ...
  it("falls back to the physical key for glyphs we do not ship as tokens", async () => {
    ...
    expect(
      comboFromEvent(
        keydown({ code: "KeyK", key: "˚", metaKey: true, altKey: true }),
      ),
    ).toBe("mod+alt+k");
    expect(
      comboFromEvent(
        keydown({ code: "KeyN", key: "Dead", metaKey: true, altKey: true }),
      ),
    ).toBe("mod+alt+n");
    expect(
      comboFromEvent(keydown({ code: "KeyK", key: "л", metaKey: true })),
    ).toBe("mod+k");
  });
  ...
  it("recognizes PageUp and PageDown chords", async () => {
    ...
    expect(comboFromEvent(keydown({ code: "PageUp", ctrlKey: true }))).toBe(
      "ctrl+pageup",
    );
    expect(comboFromEvent(keydown({ code: "PageDown", ctrlKey: true }))).toBe(
      "ctrl+pagedown",
    );
  });
});
...
describe("formatCombo — honest Control labels", () => {
  ...
  it.each(["Linux x86_64", "Win32"])(
    'renders "Ctrl+…" off macOS on %s (base key keeps its glyph)',
    async (platform) => {
...
describe("actionAllowedInInput", () => {
  ...
  it("leaves text navigation chords with the focused input even when rebound to an allowed action", async () => {
    ...
    expect(actionAllowedInInput("nav.commandPalette", "mod+pageup")).toBe(
      false,
    );
...
describe("comboFromEvent — IME composition keydowns never resolve to combos (#84957)", () => {
  it("returns null while a composition is in progress (isComposing)", async () => {
    ...
    expect(
      comboFromEvent(keydown({ code: "KeyN", isComposing: true, key: "n" })),
    ).toBeNull();
    expect(
      comboFromEvent(
        keydown({ code: "Enter", isComposing: true, key: "Enter" }),
      ),
    ).toBeNull();
    expect(
      comboFromEvent(keydown({ code: "Space", isComposing: true, key: " " })),
    ).toBeNull();
  });
  ...
  it('returns null for the legacy key="Process" (VK_PROCESSKEY) keydown', async () => {
    ...
    expect(
      comboFromEvent(keydown({ code: "KeyW", key: "Process" })),
    ).toBeNull();
  });
  ...
  it("ignores IME-synthesized modifier-name keys on non-modifier codes", async () => {
    ...
    expect(
      comboFromEvent(keydown({ code: "KeyW", key: "Control" })),
    ).toBeNull();
  ...
  it("still resolves real combos after composition ends", async () => {
    ...
    expect(
      comboFromEvent(
        keydown({ code: "KeyN", isComposing: false, key: "n", metaKey: true }),
      ),
    ).toBe("mod+n");
  });
});
