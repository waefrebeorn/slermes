...
const PROFILE_SWITCH_ACTIONS: KeybindActionMeta[] = Array.from(
  { length: PROFILE_SLOT_COUNT },
  (_, i) => ({
...
const SESSION_SLOT_ACTIONS: KeybindActionMeta[] = Array.from(
...
export const KEYBIND_ACTION_IDS: readonly string[] = KEYBIND_ACTIONS.map(
  (action) => action.id,
);
...
export function keybindAction(id: string): KeybindActionMeta | undefined {
  return (
    ACTION_BY_ID.get(id) ??
    allKeybindActions().find((action) => action.id === id)
  );
}

/** The contributed handler for an action id (built-ins wire theirs in use-keybinds). */
export function contributedKeybindHandler(
  id: string,
): (() => void) | undefined {
...
export function defaultBindings(): KeybindBindings {
  return Object.fromEntries(
    allKeybindActions().map((action) => [action.id, [...action.defaults]]),
  );
}
...
export const KEYBIND_READONLY: readonly KeybindReadonly[] = [
  ...
  {
    id: "view.terminalCopy",
    category: "view",
    keys: IS_MAC ? ["mod+c"] : ["mod+shift+c"],
  },
  {
    id: "view.terminalPaste",
    category: "view",
    keys: IS_MAC ? ["mod+v"] : ["mod+shift+v"],
  },
