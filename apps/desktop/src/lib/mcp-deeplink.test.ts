...
import {
  MCP_DEEPLINK_MAX_CONFIG_BYTES,
  parseMcpInstallDeepLink,
} from "./mcp-deeplink";
...
describe("parseMcpInstallDeepLink", () => {
  ...
  it("accepts a command-shaped config (standard base64 with padding)", () => {
    ...
    expect(result).toEqual({
      ok: true,
      request: { name: "fs.local-1", transport: "stdio", config },
    });
  });
  ...
  it("rejects names outside ^[A-Za-z0-9._-]{1,64}$", () => {
    ...
    for (const name of ["", "has space", "näme", "a/b", "a".repeat(65)]) {
      expect(parseMcpInstallDeepLink({ name, config })).toEqual({
        ok: false,
        error: "invalid_name",
      });
    }
  ...
  it("rejects a missing or undecodable config param", () => {
    expect(parseMcpInstallDeepLink({ name: "x" })).toEqual({
      ok: false,
      error: "missing_config",
    });
    expect(
      parseMcpInstallDeepLink({ name: "x", config: "!!!not-base64!!!" }),
    ).toEqual({
      ok: false,
      error: "bad_encoding",
    });
    ...
    expect(
      parseMcpInstallDeepLink({ name: "x", config: btoa("not json") }),
    ).toEqual({
      ok: false,
      error: "bad_encoding",
    });
  ...
  it("rejects non-object configs", () => {
    for (const value of ["a string", 42, null, true, ["array"]]) {
      expect(
        parseMcpInstallDeepLink({ name: "x", config: b64url(value) }),
      ).toEqual({
        ok: false,
        error: "not_an_object",
      });
  ...
  it("rejects non-http(s) url schemes", () => {
    for (const url of [
      ...
      expect(
        parseMcpInstallDeepLink({ name: "x", config: b64url({ url }) }),
      ).toEqual({
        ok: false,
        error: "invalid_url",
      });
  ...
  it("rejects ambiguous configs carrying both url and command", () => {
    expect(
      parseMcpInstallDeepLink({
        name: "x",
        config: b64url({ url: "https://example.com", command: "rm" }),
      }),
  ...
  it("rejects oversized payloads", () => {
    const big = {
      ...
      padding: "x".repeat(MCP_DEEPLINK_MAX_CONFIG_BYTES),
    };
