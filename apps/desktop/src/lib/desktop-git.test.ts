...
const worktreeList = vi.fn(async () => [
  { branch: "main", detached: false, isMain: true, locked: false, path: "/r" },
]);
...
describe("desktop git facade", () => {
  ...
  it("uses Electron git locally", async () => {
    ...
    await expect(desktopGit()?.repoStatus("/work")).resolves.toEqual({
      branch: "main",
    });
  ...
  it("routes reads through the backend REST mirror on a remote gateway", async () => {
    ...
    await expect(desktopGit()?.repoStatus("/srv/work")).resolves.toEqual({
      branch: "remote-main",
    });
  ...
  it("routes the convert-a-branch worktree flow through the backend on a remote gateway", async () => {
    ...
    await desktopGit()?.worktreeAdd("/srv/work", {
      existingBranch: "origin/feature",
    });
