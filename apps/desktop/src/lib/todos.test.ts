...
describe("todoTree", () => {
  ...
  it("preserves parent through parseTodos", () => {
    expect(
      parseTodos([{ content: "x", id: "c", parent: "p", status: "pending" }]),
    ).toEqual([{ content: "x", id: "c", parent: "p", status: "pending" }]);
  });
});
...
describe("parseTodos", () => {
  ...
  it("parses nested todo payloads from wrapped objects and JSON strings", () => {
    expect(
      parseTodos({
        todos: [{ content: "Plate", id: "plate", status: "pending" }],
      }),
    ).toEqual([{ content: "Plate", id: "plate", status: "pending" }]);

    expect(
      parseTodos(
        '{"todos":[{"id":"plate","content":"Plate","status":"pending"}]}',
      ),
    ).toEqual([{ content: "Plate", id: "plate", status: "pending" }]);
  });
...
describe("latestSessionTodos", () => {
  ...
  it("returns the last todo list across the transcript (result beats args)", () => {
      ...
      { parts: [{ type: "text", text: "hi" }] },
      {
        parts: [
          todoPart([{ content: "Stale", id: "a", status: "pending" }], {
            result: {
              todos: [{ content: "Fresh", id: "a", status: "completed" }],
            },
          }),
        ],
      },
    ...
    expect(latestSessionTodos(messages)).toEqual([
      { content: "Fresh", id: "a", status: "completed" },
    ]);
  });
  ...
  it("prefers the live carried `todos` field over args", () => {
    ...
    expect(latestSessionTodos(messages)).toEqual([
      { content: "Live", id: "a", status: "in_progress" },
    ]);
  });

  it("returns null when no todo tool calls exist", () => {
    expect(
      latestSessionTodos([{ parts: [{ type: "text", text: "hi" }] }]),
    ).toBeNull();
...
describe("nextTodosFromToolEvent", () => {
  ...
  it("does not wipe the list when a merge payload has no usable items", () => {
    expect(
      nextTodosFromToolEvent(current, { args: { merge: true, todos: [] } }),
    ).toBeNull();
  });
  ...
  it("still replaces when merge is off", () => {
    expect(
      nextTodosFromToolEvent(current, {
        args: {
          todos: [{ content: "Only this", id: "c", status: "completed" }],
        },
      }),
...
describe("parseTodoPatch", () => {
  it("keeps status-only items that parseTodos would drop", () => {
    ...
    expect(parseTodoPatch([{ id: "c", status: "completed" }])).toEqual([
