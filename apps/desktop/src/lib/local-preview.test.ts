...
const { readDesktopFileDataUrl } = vi.hoisted(() => ({
...
describe("remote HTML previews", () => {
  ...
  it("loads bounded authenticated bytes while retaining the canonical file URL", async () => {
    ...
    await expect(
      normalizeOrLocalPreviewTarget("/srv/report.html"),
    ).resolves.toEqual({
      ...remoteTarget,
      dataUrl,
    });
  ...
  it("rejects malformed base64", () => {
    expect(validatedRemoteHtmlDataUrl("data:text/html;base64,SGVsbG8=")).toBe(
      "data:text/html;base64,SGVsbG8=",
    );
    expect(
      validatedRemoteHtmlDataUrl("data:text/html;base64,SGVsbG8"),
    ).toBeNull();
    expect(
      validatedRemoteHtmlDataUrl("data:text/html;base64,SGVsbG8%3D"),
    ).toBeNull();
  });
  ...
  it("wraps remote HTML in a deny-by-default content policy", () => {
    ...
    const document = remoteHtmlPreviewDocument(
  ...
  it("does not create scripts when sanitized HTML is reparsed", () => {
    const html =
      "<form><math><mtext></form><form><mglyph><style></math><script>MUTATION-SCRIPT</script>";
  ...
  it("decodes remote HTML as UTF-8", () => {
    ...
    expect(
      remoteHtmlPreviewDocument(`data:text/html;base64,${payload}`),
    ).toContain(html);
  });
  ...
  it("stages remote HTML before opening it in the system browser", async () => {
    ...
    const saveImageBuffer = vi.fn(
      async (_data: ArrayBuffer | Uint8Array, _ext: string) =>
        "/tmp/report #1?.html",
    );
    ...
    expect(new TextDecoder().decode(saveImageBuffer.mock.calls[0]?.[0])).toBe(
      "<h1>remote</h1>",
    );
    ...
    expect(openPreviewInBrowser).toHaveBeenCalledWith(
      "file:///tmp/report%20%231%3F.html",
    );
  });
  ...
  it("serializes UNC staging paths as file URLs", async () => {
    const dataUrl = `data:text/html;base64,${btoa("<h1>remote</h1>")}`;
    const saveImageBuffer = vi.fn(
      async () => "\\\\server\\share\\report #1.html",
    );
    const openPreviewInBrowser = vi.fn(async () => undefined);
    window.hermesDesktop = { openPreviewInBrowser, saveImageBuffer } as never;
    ...
    expect(openPreviewInBrowser).toHaveBeenCalledWith(
      "file://server/share/report%20%231.html",
    );
  });

  it("preserves backslashes in POSIX local file paths", () => {
    expect(localPreviewTarget("/tmp/report\\draft.html")?.url).toBe(
      "file:///tmp/report%5Cdraft.html",
    );
  });

  it("preserves POSIX double-slash file paths", () => {
    expect(localPreviewTarget("//srv/share/report #1?.html")?.url).toBe(
      "file:////srv/share/report%20%231%3F.html",
    );
  });
...
describe("PDF previews", () => {
  ...
  it("does not UTF-8-enrich remote PDFs before loading their bytes", async () => {
    ...
    await expect(
      normalizeOrLocalPreviewTarget("/remote/spec.pdf"),
    ).resolves.toMatchObject({
