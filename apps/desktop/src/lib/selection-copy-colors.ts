...
const SRGB_FN_RE =
  /^color\(\s*srgb\s+([\d.]+%?)\s+([\d.]+%?)\s+([\d.]+%?)(?:\s*[/\s]+\s*([\d.%]+))?\s*\)$/i;
...
function expandHex(hex: string): [number, number, number, number] | null {
  ...
  const pick = (i: number) =>
    Number.parseInt(wide ? h.slice(i, i + 2) : h[i]! + h[i]!, 16);
...
export function textColorLuma(cssColor: string): number | null {
  ...
  const composite = (channel: number) =>
    channel * parsed.alpha + 128 * (1 - parsed.alpha);

  return (
    0.2126 * composite(parsed.r) +
    0.7152 * composite(parsed.g) +
    0.0722 * composite(parsed.b)
  );
}

function isOppositeScheme(textLuma: number, mode: RenderedMode): boolean {
  return mode === "dark"
    ? textLuma >= LIGHT_TEXT_LUMA_MIN
    : textLuma <= DARK_TEXT_LUMA_MAX;
}
 ...
 */
export function serializeSelectionStructure(
  sel: Selection,
  doc: Document,
): string {
 ...
 */
export function installSelectionCopyColorGuard(
  doc: Document = document,
): () => void {
  const trace = (entry: Record<string, unknown>) => {
    if (import.meta.env.DEV) {
      const view = doc.defaultView as
        (Window & { __copyGuardLog?: unknown[] }) | null;
  ...
  const onCopy = (event: ClipboardEvent) => {
    ...
    if (inkLuma === null || !isOppositeScheme(inkLuma, mode)) {
      trace({
        step: inkLuma === null ? "no-ink" : "same-scheme",
        inkLuma,
        mode,
      });
