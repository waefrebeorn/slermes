...
export function cachedSlashCompletion<T>(
  key: string,
  fetcher: () => Promise<T>,
): Promise<T> {
...
export function hasCachedSlashCompletion(key: string): boolean {
  ...
  return (
    ...
    Date.now() - state.dataUpdatedAt < SLASH_COMPLETIONS_TTL_MS
  );
}
...
export function peekCachedSlashCompletion<T>(key: string): T | undefined {
  return hasCachedSlashCompletion(key)
    ? queryClient.getQueryData<T>([SLASH_COMPLETIONS_KEY, key])
    : undefined;
}
...
export function cachedPathCompletion<T>(
  key: string,
  fetcher: () => Promise<T>,
): Promise<T> {
  return queryClient.fetchQuery({
...
export function hasCachedPathCompletion(key: string): boolean {
  ...
  return (
    state?.data !== undefined &&
    Date.now() - state.dataUpdatedAt < PATH_COMPLETIONS_TTL_MS
  );
}
