...
describe("isCodeSkewRestartRequired", () => {
  ...
  it("detects a bare FastAPI detail string", () => {
    expect(
      isCodeSkewRestartRequired("Restart required: checkout drifted"),
    ).toBe(true);
  });

  it("ignores unrelated 503s and load failures", () => {
    expect(
      isCodeSkewRestartRequired(new Error("503: Service Unavailable")),
    ).toBe(false);
    expect(isCodeSkewRestartRequired(new Error("Failed to load models"))).toBe(
      false,
    );
