...
import {
  isToolCallPart,
  type ToolCallLike,
} from "@/components/assistant-ui/tool/run-summary";
...
function replayLive(steps: TurnStep[]): ChatMessage[] {
  ...
  const openBubble = (): ChatMessage => {
    ...
    streamIndex += 1;
    open = {
      id: `assistant-stream-${streamIndex}`,
      role: "assistant",
      parts: [],
      pending: true,
    };
  ...
  for (const step of steps) {
    switch (step.kind) {
      ...
      case "tool": {
        ...
        bubble.parts = upsertToolPart(
          bubble.parts,
          { tool_id: step.id, name: step.name },
          "complete",
        );
...
function replayStored(steps: TurnStep[]): ChatMessage[] {
  ...
  return coalesceToolOnlyAssistants(
    toChatMessages(rows),
    createToolMergeCache(),
  );
}
...
describe("tool run segmentation survives rehydration", () => {
  for (const [name, steps] of Object.entries(TURNS)) {
    it(name, () => {
      expect(runsAcross(replayLive(steps))).toEqual(
        runsAcross(replayStored(steps)),
      );
    });
  }
});
...
describe("run identity", () => {
  ...
  it("breaks a run on any non-tool part", () => {
    const runs = toolRuns([
      tool("a", "read_file"),
      assistantTextPart("Now editing."),
      tool("b", "write_file"),
    ]);

    expect(runs.map((run) => run.map((t) => t.toolCallId))).toEqual([
      ["a"],
      ["b"],
    ]);
  });
