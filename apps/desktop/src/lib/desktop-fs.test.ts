...
const api = vi.fn(async ({ path }: { path: string }) => {
  if (path.startsWith("/api/fs/list?")) {
    return {
      entries: [{ name: "remote", path: "/remote", isDirectory: true }],
    };
  }
...
describe("desktop filesystem facade", () => {
  ...
  it("uses local Electron filesystem methods in local mode", async () => {
    ...
    await expect(readDesktopFileText("/work/file.txt")).resolves.toMatchObject({
      text: "local",
    });
    await expect(readDesktopFileDataUrl("/work/file.txt")).resolves.toBe(
  ...
  it("routes filesystem reads through authenticated backend REST in remote mode", async () => {
    ...
    await expect(readDesktopDir("/home/user/project")).resolves.toMatchObject({
      entries: [{ name: "remote" }],
    });
    await expect(
      readDesktopFileText("/home/user/project/a b.txt"),
    ).resolves.toMatchObject({ text: "remote" });
    await expect(
      readDesktopFileDataUrl("/home/user/project/a b.txt"),
    ).resolves.toBe("data:text/plain;base64,cmVtb3Rl");
    await expect(desktopGitRoot("/home/user/project")).resolves.toBe("/remote");
    await expect(desktopDefaultCwd()).resolves.toEqual({
      cwd: "/backend/project",
      branch: "main",
    });

    expect(api).toHaveBeenCalledWith({
      path: "/api/fs/list?path=%2Fhome%2Fuser%2Fproject",
    });
    expect(api).toHaveBeenCalledWith({
    ...
    expect(api).toHaveBeenCalledWith({
      path: "/api/fs/git-root?path=%2Fhome%2Fuser%2Fproject",
    });
  ...
  it("does not retry the same unreadable path through the local facade", async () => {
    ...
    await expect(readDesktopFileDataUrlLocalFirst("/missing.png")).rejects.toBe(
      error,
    );
  ...
  it("falls back from local disk to the active gateway in remote mode", async () => {
    ...
    await expect(
      readDesktopFileDataUrlLocalFirst("/remote/image.png"),
    ).resolves.toBe("data:text/plain;base64,cmVtb3Rl");
    expect(readFileDataUrl).toHaveBeenCalledOnce();
    expect(api).toHaveBeenCalledWith({
  ...
  it("pins remote filesystem requests to the active registry connection", async () => {
    ...
    await readDesktopFileDataUrl(
      "/home/doug/default-profile-workspace/IDEA.md",
    );
  ...
  it("routes file diffs through backend git in remote mode", async () => {
    ...
    await expect(desktopFileDiff("/repo", "src/a b.ts")).resolves.toBe(
      "remote diff",
    );
    expect(api).toHaveBeenCalledWith({
      path: "/api/git/file-diff?path=%2Frepo&file=src%2Fa%20b.ts",
    });
  });
  ...
  it("uses the local Electron picker for remote file selection", async () => {
    ...
    await expect(
      selectDesktopPaths({ directories: false, multiple: false }),
