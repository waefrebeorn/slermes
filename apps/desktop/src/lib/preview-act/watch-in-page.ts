...
export function watchInPage(
  ...
  const win = doc.defaultView as
    (Window & { __hermesWatch?: Record<string, unknown> }) | null;
  ...
  const build = () => {
    ...
    if (was) {
      style(pointer, {
        transform: "translate3d(" + was.x + "px," + was.y + "px,0)",
      });
    }
  ...
  const fit = (mark: WatchMark) => {
    ...
    style(cell, {
      ...
      transform:
        "translate3d(" + (from.left - gap) + "px," + (from.top - gap) + "px,0)",
    ...
    if (mark.tag) {
      ...
      const row =
        from.top - gap >= tall
          ? "up"
          : from.top + from.height + gap + tall <= win.innerHeight
            ? "down"
            : "in";
  ...
  const enter = (mark: WatchMark) => {
    ...
    const frames = blip
      ...
      : // A box snapping shut on a target, not a material-design tap. Scale is
        ...
        [
          {
            offset: 0,
            transform: spec.kind === "hit" ? "scale(1.35)" : "scale(1.06)",
          },
          {
            offset: spec.kind === "hit" ? 0.1 : 90 / life,
            transform: "scale(1)",
          },
    ...
    const beam =
      spec.kind === "lock"
        ? (skin.firstElementChild as HTMLElement | null)
        : null;
  ...
  const outline = (
    all: Element[],
    kind: WatchKind,
    beat: (at: number, seen: WatchSpot) => number,
    life?: number,
  ) => {
    for (let i = 0; i < all.length; i++) {
      ...
      add({
        ...
        beat: beat(i, seen),
        kind,
        label: named ? sniff(all[i]) : undefined,
        life,
      });
    }
  };
  ...
  const prose = () => {
    ...
    for (let i = 0; i < all.length && found.length < page; i++) {
      ...
      const look = (
        el as Element & { checkVisibility?: (opts?: object) => boolean }
      ).checkVisibility;

      if (
        typeof look === "function" &&
        !look.call(el, { opacityProperty: true, visibilityProperty: true })
      ) {
        continue;
      }
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __hermesWatchTag?: number })
    .__hermesWatchTag;
  ...
  if (stage === "think") {
    ...
    state.pulse = () => {
      const live = (
        holder.field && holder.field.length ? holder.field : holder.nodes || []
      ) as Element[];
      ...
      if (seen.width >= 1 && seen.height >= 1) {
        add({
          ...
          kind: "flash",
          label: sniff(el),
          life: ponder + Math.random() * ponder,
        });
        pump();
      }
    };
  ...
  if (stage === "pin") {
    ...
    add({
      a: target as Element,
      kind: "held",
      label: label || sniff(target as Element),
    });
    holder.aimed = null;
  } else if (stage === "hold") {
    ...
    outline(
      ...
      (_at, seen) =>
        Math.max(
          0,
          Math.min(1, (seen.top + seen.height / 2) / win.innerHeight),
        ) * wipe,
      glance,
    );
