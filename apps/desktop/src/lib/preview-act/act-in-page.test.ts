...
function layOutTheDocument() {
  vi.spyOn(Element.prototype, "getBoundingClientRect").mockImplementation(
    function (this: Element) {
      ...
      const width =
        style.display === "none" ? 0 : parseFloat(style.width) || 40;
      const height =
        style.display === "none" ? 0 : parseFloat(style.height) || 40;
...
describe("self-containment", () => {
  ...
  it("runs after being stringified and eval’d with no module scope", () => {
    ...
    expect(
      injected(document, holder, { kind: "click", ref: save.ref }).success,
    ).toBe(true);
...
describe("elements", () => {
  ...
  it("tells two elements with the same name apart", () => {
    ...
    expect(inventory(holder).elements?.map((e) => e.ref)).toEqual([
      "btn-edit",
      "btn-edit-1",
      "btn-edit-2",
    ]);
  ...
  it("skips a control buried under another layer, which would take the click instead", () => {
    ...
    document.elementFromPoint = (x: number) =>
      x === 120 ? wall : document.getElementById("real");
  ...
  it("keeps a control whose own child is what the hit test lands on", () => {
    const holder = page(
      '<a href="/home" id="real"><svg id="icon"></svg> Home</a>',
    );
  ...
  it("honours the cap", () => {
    const holder = page(
      Array.from({ length: 10 }, (_, i) => `<button>B${i}</button>`).join(""),
    );

    expect(inventory(holder).elements).toHaveLength(10);
    expect(
      actInPage(document, holder, { full: true, kind: "elements", max: 3 })
        .elements,
    ).toHaveLength(3);
  });
});
...
describe("delta", () => {
  ...
  it("reports only what moved on every look after that", () => {
    ...
    document
      .getElementById("host")!
      .insertAdjacentHTML("beforeend", '<button id="quit">Quit</button>');
  ...
  it("reports a relabelled control as changed, keeping its handle", () => {
    ...
    expect(next.delta?.changed?.map((e) => [e.ref, e.label])).toEqual([
      [ref, "Added"],
    ]);
    expect(actInPage(document, holder, { kind: "click", ref }).success).toBe(
      true,
    );
  });
  ...
  it("reports a control becoming available on its own", () => {
    ...
    expect(inventory(holder).delta?.changed).toEqual([
      { disabled: false, ref: "btn-continue" },
    ]);
  });
  ...
  it("falls back to the whole inventory when most of the page is new", () => {
    ...
    expect(next.elements?.map((e) => e.ref)).toEqual([
      "lnk-a",
      "lnk-b",
      "lnk-c",
    ]);
  ...
  it("does not report an element as removed just because it fell past the cap", () => {
    const holder = page(
      Array.from({ length: 4 }, (_, i) => `<button>B${i}</button>`).join(""),
    );
    inventory(holder);
    ...
    expect(
      actInPage(document, holder, { kind: "click", ref: "btn-b3" }).success,
    ).toBe(true);
  });
...
describe("rebind", () => {
  ...
  it("keeps the handle when a re-render replaces the node", () => {
    ...
    const ref = inventory(holder).elements!.find(
      (e) => e.label === "Sign in",
    )!.ref;
...
describe("click", () => {
  ...
  it("takes a raw CSS selector when no ref fits", () => {
    ...
    expect(
      actInPage(document, holder, { kind: "click", selector: "#save" }).success,
    ).toBe(true);
    expect(clicked).toHaveBeenCalledOnce();
  });
  ...
  it("reports the live url so a navigation is visible to the agent", () => {
    ...
    expect(
      actInPage(document, holder, { kind: "click", ref: firstRef(holder) }).url,
    ).toBe(document.location.href);
  });
});
...
describe("stale refs", () => {
  ...
  it("catches a node that was removed after the snapshot", () => {
    ...
    expect(actInPage(document, holder, { kind: "click", ref }).error).toContain(
      "removed",
    );
  });
  ...
  it("asks for a target when given neither", () => {
    expect(
      actInPage(document, page("<button>Save</button>"), { kind: "click" })
        .error,
    ).toContain("selector");
  });
  ...
  it("reports a selector that matches nothing", () => {
    expect(
      actInPage(document, page(""), { kind: "click", selector: "#nope" }).error,
...
describe("type", () => {
  ...
  it("writes into a contenteditable host", () => {
    const holder = page(
      '<div id="editor" contenteditable="true" aria-label="Body"></div>',
    );

    actInPage(document, holder, {
      kind: "type",
      ref: firstRef(holder),
      text: "hello",
    });
  ...
  it("submits the owning form when asked", () => {
    const holder = page(
      '<form id="f"><input id="q" placeholder="Search" /></form>',
    );
    const ref = firstRef(holder);
...
describe("press", () => {
  it("sends the key to the target", () => {
    ...
    document
      .getElementById("q")!
      .addEventListener("keydown", (e) => keys.push((e as KeyboardEvent).key));

    expect(
      actInPage(document, holder, { key: "Enter", kind: "press", ref }).success,
    ).toBe(true);
  ...
  it("needs a key", () => {
    ...
    expect(
      actInPage(document, holder, { kind: "press", ref: firstRef(holder) })
        .error,
    ).toContain("key");
  });
});
...
describe("scroll", () => {
  ...
  it("scrolls a ref’d container instead of the page", () => {
    const holder = page(
      '<div id="list" aria-label="Results" tabindex="0" style="overflow: auto"></div>',
    );
    const ref = firstRef(holder);
