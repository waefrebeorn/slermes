...
describe("labelOf", () => {
  it("prefers what the page tells assistive clients", () => {
    expect(
      naming().labelOf(el('<button aria-label="Close dialog">×</button>')),
    ).toBe("Close dialog");
  });

  it("follows aria-labelledby to the element that names it", () => {
    document.body.innerHTML =
      '<h2 id="t">Billing</h2><button aria-labelledby="t">→</button>';
  ...
  it("falls back through placeholder and title before giving up", () => {
    expect(naming().labelOf(el('<input placeholder="Your email" />'))).toBe(
      "Your email",
    );
...
describe("selectorFor", () => {
  ...
  it("names the element only when the page gave it a durable name", () => {
    ...
    expect(selectorFor(el('<button data-testid="save">S</button>'))).toBe(
      '[data-testid="save"]',
    );
...
describe("anchorOf", () => {
  ...
  it("places an element by the landmark it sits in", () => {
    document.body.innerHTML =
      "<nav><div><div><button>Home</button></div></div></nav>";
  ...
  it("tells two same-tag landmarks apart by their label", () => {
    document.body.innerHTML =
      '<nav aria-label="Primary nav"><button>Home</button></nav>';

    expect(naming().anchorOf(document.querySelector("button")!)).toBe(
      "nav#primary-nav",
    );
  });
...
describe("affinity", () => {
  ...
  it("is certain when both sides carry the same stable attribute", () => {
    expect(affinity(bound({ stable: "save" }), bound({ stable: "save" }))).toBe(
      1,
    );
  });
  ...
  it("refuses outright when two stable attributes disagree", () => {
    expect(
      affinity(bound({ stable: "save" }), bound({ stable: "cancel" })),
    ).toBe(0);
  });
  ...
  it("clears the bar on name and place together, and not on place alone", () => {
    const named = affinity(
      bound({ name: "Save", path: "main" }),
      bound({ name: "Save", path: "main" }),
    );
    const placed = affinity(
      bound({ name: "Save", path: "main" }),
      bound({ name: "Publish", path: "main" }),
    );
