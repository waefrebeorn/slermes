...
export function visibilityKit(
  ...
  const onScreen = (el: Element): boolean => {
    ...
    return !(
      box.width >= win.innerWidth * 0.95 && box.height >= win.innerHeight * 0.9
    );
  };
  ...
  const shown = (el: Element): boolean => {
    ...
    if (
      typeof seen === "function" &&
      !seen.call(el, { checkOpacity: true, checkVisibilityCSS: true })
    ) {
      return false;
    }
    ...
    if (style) {
      ...
      if (
        ...
        (style.clipPath || "").indexOf("inset(50%") !== -1
      ) {
        return false;
      }
    }
