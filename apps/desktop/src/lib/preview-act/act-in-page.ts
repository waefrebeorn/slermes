...
export function buildActKit(doc: Document): ActKit {
  ...
  return {
    identity: identityKit(naming),
    naming,
    visibility: visibilityKit(doc, doc.defaultView),
  };
}

/** Run one action against `doc`, resolving refs through `holder`. */
export function actInPage(
  doc: Document,
...
export function actInPageCore(
  ...
  const survey = (max: number): PreviewActResult => {
    ...
    const pool = book.filter(
      (bound) => !claimed[bound.ref] && !doc.contains(bound.el),
    );
  ...
  const resolve = (): { el?: Element; error?: string } => {
    ...
    if (ref) {
      if (holder.url !== here) {
        return {
          error:
            "The page navigated since the last snapshot, so " +
            ref +
            " no longer points anywhere. Call elements again.",
        };
      }
      ...
      if (!bound) {
        return {
          error:
            "Unknown element " + ref + ". Call elements to get current refs.",
        };
      }
      ...
      if (!doc.contains(bound.el)) {
        return {
          error:
            ref +
            " has been removed from the page since the last snapshot. Call elements again.",
        };
      }
  ...
  if (action.kind === "elements") {
    const looked = survey(
      Math.max(1, Math.min(action.max || maxElements, maxElements)),
    );
  ...
  if (action.kind === "scroll") {
    ...
    return answer({
      acted: scroller ? "scrolled " + describe(scroller) : "scrolled the page",
      success: true,
    });
  }
  ...
  if (action.kind === "locate") {
    ...
    return answer({
      ...
      typable:
        tag === "TEXTAREA" || tag === "INPUT" || el.isContentEditable === true,
    });
  }
  ...
  if (action.kind === "type") {
    ...
    if (editable) {
      ...
      return fail(
        describe(el) +
          " is a dropdown — click it and click the option you want.",
      );
    } else {
    ...
    return answer({
      acted:
        "typed into " + describe(el) + (action.submit ? " and submitted" : ""),
      success: true,
    });
  }
  ...
  if (action.kind === "press") {
    ...
    return answer({
      acted: "pressed " + key + " on " + describe(el),
      success: true,
    });
  }
