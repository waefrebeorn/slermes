...
const drawn = () =>
  (
    window as unknown as {
      __hermesWatch: {
        parts: Record<string, HTMLElement> & { shadow: ShadowRoot };
      };
    }
  ).__hermesWatch.parts;
...
const marks = () =>
  [...drawn().shadow.children].filter((child) => child !== drawn().pointer);
...
describe("watchInPage", () => {
  ...
  it("flips the label under the box when there is no headroom above it", () => {
    ...
    const tags = marks().map(
      (mark) => mark.querySelector("div:last-child") as HTMLElement,
    );
  ...
  it("runs after being stringified and eval’d with no module scope", () => {
    ...
    const injected = new Function(
      "return (" + watchInPage.toString() + ")",
    )() as typeof watchInPage;

    expect(() =>
      injected(document, { aimed: target }, "aim", "Clicking Save"),
    ).not.toThrow();
