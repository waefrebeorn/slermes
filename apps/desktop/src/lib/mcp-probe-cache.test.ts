...
const result = (over: Partial<McpTestResult> = {}): McpTestResult => ({
  ok: true,
  tools: [],
  ...over,
});
...
describe("classifyProbe", () => {
  ...
  it("classifies other failures as error", () => {
    expect(
      classifyProbe(
        result({ ok: false, error: "ECONNREFUSED 127.0.0.1:3845" }),
      ),
    ).toBe("error");
  });
...
describe("probeKey", () => {
  it("scopes by profile, name, and connection-relevant config", () => {
    ...
    expect(probeKey("github", server, "default")).not.toBe(
      probeKey("github", server, "work"),
    );
    expect(probeKey("github", server, "default")).not.toBe(
      probeKey("gh2", server, "default"),
    );
    expect(probeKey("github", server, "default")).not.toBe(
  ...
  it("ignores non-connection fields so cosmetic edits still hit the cache", () => {
    ...
    expect(probeKey("s", server, "default")).toBe(
      probeKey("s", { ...server, description: "hi" }, "default"),
    );
  });
});
...
describe("NEEDS_AUTH_RE", () => {
  it("does not match unrelated failure text", () => {
    expect(NEEDS_AUTH_RE.test("connection timed out after 60000ms")).toBe(
      false,
    );
  });
});
