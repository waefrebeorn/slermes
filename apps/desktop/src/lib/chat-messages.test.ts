...
describe("toChatMessages", () => {
  it("rebuilds the full command from a gateway tool row carrying args", () => {
    ...
    expect((toolPart as { args: { command?: string } }).args.command).toBe(
      longCommand,
    );
  });
  ...
  it("keeps assistant tool-call iterations in one loaded assistant bubble", () => {
    const messages = toChatMessages([
      { role: "user", content: "check this repo", timestamp: 1 },
      {
        ...
        tool_calls: [
          {
            id: "tc-1",
            function: { name: "search_files", arguments: '{"path":".github"}' },
          },
        ],
      },
      {
      ...
      },
      {
        ...
        tool_name: "terminal",
        content:
          '{"output":"M src/ui/components/image-distortion.tsx\\n","exit_code":0}',
        timestamp: 5,
      },
      {
        role: "assistant",
    ...
    expect(assistantMessages[0].parts.map((part) => part.timestamp)).toEqual([
      2, 2, 4, 4, 6,
    ]);
    expect(
    ...
    expect(
      assistantMessages[0].parts.filter((part) => part.type === "tool-call"),
    ).toHaveLength(2);
    expect(chatMessageText(assistantMessages[0])).toContain(
    ...
    const [message] = toChatMessages([
      {
        role: "assistant",
        content:
          "MEDIA:/Users/brooklyn/.hermes/cache/audio/tts_20260501_222725.mp3\n\nhow's that sound?",
        timestamp: 1,
      },
    ]);
  ...
  it("renders a native-vision turn as caption plus thumbnail, not raw placeholder text", () => {
    ...
    const ref =
      "@image:`/Users/me/Library/Application Support/Hermes/composer-images/a.png`";
  ...
  it("leaves a plain user prompt without attachment refs untouched", () => {
    ...
    expect(
      (message as { attachmentRefs?: string[] }).attachmentRefs,
    ).toBeUndefined();
  });
  ...
  it.each([
    [
      "an object",
      { delegation_id: "deleg_1", task_count: 2 },
      "2 background agents finished",
    ],
    [
      "JSON text",
      JSON.stringify({ delegation_id: "deleg_1", task_count: 1 }),
    ...
    [
      "a missing task count",
      { delegation_id: "deleg_1" },
      "background agent work finished",
    ],
  ])(
    "labels a delegation event given %s",
    (_case, displayMetadata, expected) => {
      const read = () =>
        toChatMessages([
          {
            role: "user",
            ...
            display_kind: "async_delegation_complete",
            display_metadata:
              displayMetadata as SessionMessage["display_metadata"],
            timestamp: 1,
          },
        ]);
...
describe("renderMediaTags", () => {
  it("renders standalone and inline MEDIA tags as links", () => {
    ...
    expect(renderMediaTags("MEDIA:/tmp/demo.mp4")).toBe(
      "[Video: demo.mp4](#media:%2Ftmp%2Fdemo.mp4)",
    );
  });

  it("renders streamed assistant media once the tag is complete", () => {
    const parts = appendAssistantTextPart(
      appendAssistantTextPart([], "ok\nMEDIA:"),
      "/tmp/voice.mp3",
    );
...
describe("interleaved reasoning/text boundaries", () => {
  it("preserves text → reasoning → text as three ordered activities", () => {
    ...
    parts = appendAssistantTextPart(
      parts,
      "verify the full file is correct:",
      3,
    );
  ...
  it("starts a fresh text part after a tool call (segment boundary)", () => {
    let parts: ChatMessagePart[] = appendAssistantTextPart(
      [],
      "Let me check.",
      10.125,
    );
...
describe("preserveLocalAssistantErrors", () => {
  ...
  it("keeps local assistant error when hydrated message reuses same id", () => {
    ...
    const assistant = merged.find(
      (message) => message.id === "assistant-stream-1",
    );
...
describe("upsertToolPart", () => {
  ...
  it("closes active commentary when a tool starts", () => {
    ...
    const withTool = upsertToolPart(
      text,
      { name: "read_file", tool_id: "call-2" },
      "running",
      21,
    );
  ...
  it("keeps live todo rows stable across sparse progress payloads", () => {
    ...
    const args =
      part && "args" in part ? (part.args as Record<string, unknown>) : {};
  ...
  it("archives todo state on completion and accepts explicit empty clears", () => {
    ...
    const completedResult =
      completed[0] && "result" in completed[0]
        ? (completed[0].result as Record<string, unknown>)
        : {};

    const clearedResult =
      cleared[0] && "result" in cleared[0]
        ? (cleared[0].result as Record<string, unknown>)
        : {};

    expect(completedResult.todos).toEqual([
      { content: "Boil water", id: "boil", status: "in_progress" },
    ]);
  ...
  it("keeps parallel same-name tools distinct without explicit ids", () => {
    ...
    const contexts = webParts.map((part) =>
      String((part.args as Record<string, unknown>)?.context || ""),
    );
  ...
  it("pairs a terminal completion with its context-only start when event IDs differ", () => {
    const started = upsertToolPart(
      [],
      {
        context: 'echo "Hello from the terminal"',
        name: "terminal",
    ...
    expect(
      terminalParts[0] && "result" in terminalParts[0]
        ? terminalParts[0].result
        : undefined,
    ).toMatchObject({
      exit_code: 0,
  ...
  it("preserves query args when completion payload omits context", () => {
    ...
    expect(part?.type).toBe("tool-call");
    expect(
      (part as Extract<ChatMessagePart, { type: "tool-call" }>).args,
    ).toMatchObject({
      context: "auckland weather today and tomorrow forecast",
    });
    expect(
      (part as Extract<ChatMessagePart, { type: "tool-call" }>).result,
    ).toMatchObject({
      summary: "Did 5 searches in 1.1s",
    });
  });
  ...
  it("matches id-less live starts with later identified completions", () => {
    ...
    expect(webParts[0].result).toMatchObject({
      summary: "Did 5 searches in 1.1s",
    });
  });
  ...
  it("matches id-less live starts with later identified progress updates", () => {
    const started = upsertToolPart(
      ...
      {
        context:
          "reykjavik tashkent uzbekistan weather today and tomorrow forecast",
        name: "web_search",
      },
      "running",
    );
  ...
  it("uses structured live tool args for titles before hydrate", () => {
    const started = upsertToolPart(
      [],
      {
        args: {
          search_term:
            "reykjavik bishkek kyrgyzstan weather today and tomorrow forecast",
        },
        name: "web_search",
...
describe("mergeFinalAssistantText", () => {
  ...
  it("drops reasoning that the final text fully covers (reasoning ⊆ final)", () => {
    ...
    const result = mergeFinalAssistantText(
      parts,
      "Let me check the files. Everything looks good.",
    );
  ...
  it("keeps non-restating reasoning", () => {
    const parts = [
      reasoningPart(
        "I analyzed the issue and found a race condition in the event loop.",
      ),
      { type: "text" as const, text: "streamed" },
    ];
...
describe("collectUnspokenTurnSpeech", () => {
  ...
  it("keeps the binding id stable while later bubbles stream in", () => {
    const turnStart = [
      user("u1", "go"),
      assistant("a1", "Let me check.", { interim: true }),
    ];
    const first = collectUnspokenTurnSpeech(turnStart, null);

    const turnLater = [
      ...turnStart,
      assistant("a2", "Still work", { pending: true }),
    ];
  ...
  it("returns null when everything is spoken or there is no assistant text", () => {
    ...
    expect(
      collectUnspokenTurnSpeech([assistant("a1", "Done.")], "a1"),
    ).toBeNull();
    expect(
      collectUnspokenTurnSpeech(
        [user("u1", "hello"), assistant("a1", "")],
        null,
      ),
    ).toBeNull();
  });
...
describe("stripPendingClarifyProjectionForCache", () => {
  ...
  it("drops a synthetic request-id-only clarify row from the durable cache", () => {
    const messages: ChatMessage[] = [
      { id: "user", role: "user", parts: [{ type: "text", text: "choose" }] },
      {
        id: "synthetic",
        role: "assistant",
        parts: [clarifyPart("req-1")],
        pending: true,
      },
    ];

    expect(stripPendingClarifyProjectionForCache(messages, "req-1")).toEqual([
      messages[0],
    ]);
  });
  ...
  it("keeps a provider-authored clarify in position but strips its local running bit", () => {
    ...
    expect(cached.parts.map((part) => part.type)).toEqual([
      "text",
      "tool-call",
    ]);
  });
