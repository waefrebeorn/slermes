...
describe("filePathFromMediaPath", () => {
  it("passes through a plain path", () => {
    expect(filePathFromMediaPath("/home/u/.hermes/images/a.png")).toBe(
      "/home/u/.hermes/images/a.png",
    );
  });
...
describe("mediaExternalUrl", () => {
  ...
  it("passes through http(s) URLs untouched", () => {
    ...
    expect(mediaExternalUrl("https://example.com/a.png")).toBe(
...
describe("mediaGatewayStreamUrl", () => {
  ...
  it("rewrites gateway-local media to the main-process remote stream proxy", () => {
    ...
    expect(mediaGatewayStreamUrl("file:///tmp/a b.mp4")).toBe(
      "hermes-media://remote/%2Ftmp%2Fa%20b.mp4",
    );
  });
...
describe("resolveMediaDisplaySrc", () => {
  ...
  it("leaves web, data, and relative markdown image sources unchanged", async () => {
    ...
    await expect(
      resolveMediaDisplaySrc("https://example.com/a.png"),
    ).resolves.toBe("https://example.com/a.png");
    await expect(
      resolveMediaDisplaySrc("data:image/png;base64,ZHVtbXk="),
    ).resolves.toBe("data:image/png;base64,ZHVtbXk=");
    await expect(resolveMediaDisplaySrc("images/a.png")).resolves.toBe(
      "images/a.png",
    );
    await expect(resolveMediaDisplaySrc("./images/a.png")).resolves.toBe(
      "./images/a.png",
  ...
  it("reads local desktop file paths from the local desktop shell", async () => {
    ...
    await expect(
      resolveMediaDisplaySrc("file:///Users/me/project/a%20b.png"),
...
describe("resolveMediaPlaybackSrc", () => {
  ...
  it("keeps a remote HTTPS video URL unchanged", async () => {
    ...
    await expect(
      resolveMediaPlaybackSrc("https://cdn.example.com/render.mp4"),
    ).resolves.toBe("https://cdn.example.com/render.mp4");
  });
  ...
  it("routes OAuth gateway-local video through the authenticated main-process proxy", async () => {
    ...
    await expect(
      resolveMediaPlaybackSrc("/root/outputs/render.mp4"),
    ).resolves.toBe(
  ...
  it("uses the Electron streaming protocol for local desktop video", async () => {
    ...
    await expect(
      resolveMediaPlaybackSrc("C:\\renders\\demo.mp4"),
    ).resolves.toBe("hermes-media://stream/C%3A%5Crenders%5Cdemo.mp4");
  });
});
...
describe("gatewayMediaDataUrl", () => {
  ...
  it("reads gateway media through the desktop fs bridge instead of /api/media roots", async () => {
    const url = await gatewayMediaDataUrl(
      "/home/u/.hermes/skills/demo/images/a b.png",
    );
...
describe("downloadGatewayMediaFile", () => {
  ...
  it("downloads gateway files through the native desktop save bridge", async () => {
    await expect(
      downloadGatewayMediaFile("file:///Users/me/project/a%20b.md"),
    ).resolves.toEqual({
      path: "/Users/me/Downloads/report.md",
      saved: true,
    });
  ...
  it("rejects when the desktop bridge is unavailable", async () => {
    ...
    await expect(
      downloadGatewayMediaFile("/Users/me/project/report.md"),
    ).rejects.toThrow("Desktop file download bridge");
  });
});
