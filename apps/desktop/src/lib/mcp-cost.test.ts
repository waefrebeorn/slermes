...
import {
  ...
  mcpServerUsagePrefix,
  sanitizeMcpNameComponent,
  serverUsageCount,
} from "./mcp-cost";
...
describe("estimateServerTokens", () => {
  ...
  it("counts only ENABLED tools per the include/exclude filter", () => {
    ...
    expect(estimateServerTokens({ tools: { exclude: ["drop"] } }, tools)).toBe(
      10,
    );
    expect(estimateServerTokens({ tools: { include: ["keep"] } }, tools)).toBe(
      10,
    );
    // Everything filtered out → no estimate rather than a misleading 0.
    expect(estimateServerTokens({ tools: { include: ["other"] } }, tools)).toBe(
      null,
    );
  });
});
