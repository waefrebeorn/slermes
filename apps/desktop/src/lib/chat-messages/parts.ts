...
export function collectUnspokenTurnSpeech(
  ...
  const spokenIndex = lastSpokenId
    ? messages.findLastIndex((m) => m.id === lastSpokenId)
    : -1;
 ...
 */
export function dedupeRepeatedTextInParts(
...
export function mergeFinalAssistantText(
  ...
  const finalPart = assistantTextPart(
    finalText,
    previousText?.timestamp ?? fallbackTimestamp,
  );
...
function appendStreamPart(
  ...
  const STREAM_PART: Record<
    "reasoning" | "text",
    (text: string, timestamp?: number) => ChatMessagePart
  > = {
...
export function appendAssistantTextPart(
  ...
  const { index, parts: next } = appendStreamPart(
    parts,
    "text",
    delta,
    timestamp,
