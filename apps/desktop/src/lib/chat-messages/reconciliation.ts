...
const normalizedTimelineText = (message: ChatMessage) =>
...
const assistantTimelineMatch = (stored: ChatMessage, local: ChatMessage) => {
  ...
  const toolMatch = stored.parts.some(
    (part) => part.type === "tool-call" && localToolIds.has(part.toolCallId),
  );
...
const timelinePartMatch = (stored: ChatMessagePart, local: ChatMessagePart) => {
  ...
  if (
    ...
    local.type === stored.type
  ) {
    return (
      stored.text.replace(/\s+/g, " ").trim() ===
      local.text.replace(/\s+/g, " ").trim()
    );
  }
...
function reconcileLocalAssistantTimeline(
  nextMessages: ChatMessage[],
  currentMessages: ChatMessage[],
): ChatMessage[] {
  const localAssistants = currentMessages.filter(
  ...
  for (
    let nextIndex = nextMessages.length - 1;
    nextIndex >= 0;
    nextIndex -= 1
  ) {
  ...
  return nextMessages.map((message, messageIndex) => {
    ...
    const parts = message.parts.map((part) => {
      const localIndex = local.parts.findIndex(
        (candidate, index) =>
          unusedLocalParts.has(index) && timelinePartMatch(part, candidate),
      );
    ...
    return {
      ...message,
      completedAt: latestBoundary(
        message.completedAt,
        local.completedAt,
        ...parts.map((part) => part.completedAt),
      ),
      parts,
      timestamp: earliestBoundary(
        message.timestamp,
        local.timestamp,
...
export function preserveLocalAssistantErrors(
  ...
  const tailUserInNext = [...mergedNextMessages]
    .reverse()
    .find((message) => message.role === "user" && !message.hidden);
  const tailUserText = tailUserInNext
    ? normalize(chatMessageText(tailUserInNext))
    : "";
  const tailUserRefs = tailUserInNext
    ? (tailUserInNext.attachmentRefs ?? []).join("\n")
    : "";
