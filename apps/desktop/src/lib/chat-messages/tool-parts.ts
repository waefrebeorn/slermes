...
function collectToolMatchValues(
  query: string,
  context: string,
  preview: string,
): string[] {
  return [
    ...new Set(
      [query, context, preview].map(normalizeToolMatchValue).filter(Boolean),
    ),
  ];
}
...
function toolPayloadMatchValues(
  payload: GatewayEventPayload | undefined,
): string[] {
...
function completeOpenStreamParts(
  parts: ChatMessagePart[],
  completedAt: number,
): ChatMessagePart[] {
...
export function upsertToolPart(
  ...
  const id =
    stableId ||
    (prev && "toolCallId" in prev && typeof prev.toolCallId === "string"
      ? prev.toolCallId
      : "") ||
    nextLiveToolId(name);
...
function findPendingClarifyLocation(
  ...
  for (
    ...
    for (
      ...
      const contextual = hasToolMatchOverlap(
        matchValues,
        toolPartMatchValues(part),
      );
  ...
  if (questions.length > 0) {
    return {
      responses: questions.map((entry) => ({
        question: firstStringField(recordFromUnknown(entry) ?? {}, [
          "question",
        ]),
        user_response: "",
      })),
...
export function settlePendingClarifyToolCall(
  ...
  const next = [...messages];
  next[location.messageIndex] = {
    ...message,
    parts,
    pending: keepMessageRunning,
  };
 ...
 * snapshot re-arms it. */
export function stripPendingClarifyProjectionForCache(
  messages: ChatMessage[],
  requestId?: string,
): ChatMessage[] {
  let changed = false;
...
export function toolPartFromStoredCall(
  ...
  const id = String(
    row.id || row.tool_call_id || `stored-tool-${fallbackIndex}`,
  );
  ...
  const toolName = String(
    row.name ||
      row.tool_name ||
      fn?.name ||
      (recordFromUnknown(row.input)?.name as string | undefined) ||
      "tool",
  );
...
export function storedToolMessagePart(
  toolMessage: SessionMessage,
  fallbackIndex: number,
): ChatMessagePart {
  const name = toolMessage.tool_name || toolMessage.name || "tool";
  const context = textFromUnknown(
    toolMessage.context || toolMessage.text || toolMessage.content || "",
  );
  ...
  return {
    type: "tool-call",
    toolCallId:
      toolMessage.tool_call_id || `stored-tool-message-${fallbackIndex}`,
    toolName: name,
    args: args as never,
    argsText: Object.keys(args).length ? JSON.stringify(args) : "",
 ...
 */
export function withUniqueToolCallIdsWithinMessage(
  message: ChatMessage,
): ChatMessage {
