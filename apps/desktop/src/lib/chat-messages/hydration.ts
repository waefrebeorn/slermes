...
function displayContentForMessage(
...
export function toChatMessages(messages: SessionMessage[]): ChatMessage[] {
  ...
  const appendPartsToActiveAssistant = (
    ...
    active.timestamp = earliestTimestamp(
      active.timestamp,
      timestamp,
  ...
  messages.forEach((message, index) => {
    if (message.role === "tool") {
      const updatedPendingToolParts = applyStoredToolResultToParts(
        pendingToolParts,
        message,
      );
      ...
      pendingToolParts = [
        ...pendingToolParts,
        storedToolMessagePart(message, index),
      ];
    ...
    const rawDisplayContent = transcriptContent(
      ...
      timelineDisplayContent(
        message,
        displayContentForMessage(message.role, content),
      ),
    );
    ...
    const imageRefExtraction =
      displayRole === "user" && rawDisplayContent
        ? extractImageRefs(rawDisplayContent)
        : null;
    const displayContent = imageRefExtraction
      ? imageRefExtraction.cleanedText
      : rawDisplayContent;
    const extractedAttachmentRefs = imageRefExtraction?.refs.length
      ? imageRefExtraction.refs
      : undefined;
    ...
    if (message.role === "assistant" && Array.isArray(message.tool_calls)) {
      parts.push(
        ...message.tool_calls.map((call, callIndex) =>
          toolPartFromStoredCall(call, callIndex, message.timestamp),
        ),
      );
    }
    ...
    const isToolOnlyAssistant =
      ...
      parts.length > 0 &&
      parts.every((part) => part.type === "tool-call");
    ...
    if (message.role === "assistant") {
      ...
      const activeAssistant =
        activeAssistantIndex !== null &&
        result[activeAssistantIndex]?.role === "assistant"
          ? result[activeAssistantIndex]
          : null;

      const currentHasToolCall = parts.some(
        (part) => part.type === "tool-call",
      );
      const activeHasToolCall = Boolean(
        activeAssistant?.parts.some((part) => part.type === "tool-call"),
      );
    ...
    result.push({
      ...
      ...(extractedAttachmentRefs
        ? { attachmentRefs: extractedAttachmentRefs }
        : {}),
    });

    activeAssistantIndex =
      message.role === "assistant" ? result.length - 1 : null;
  });
  ...
  const withoutGeneratedImageEchoes = result.map((message) =>
    message.role === "assistant"
      ? {
          ...message,
          parts: dedupeRepeatedTextInParts(
