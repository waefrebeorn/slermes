...
describe("fetchSessionLinkTitle", () => {
  ...
  it("uses the local sidebar row before calling the API", async () => {
    ...
    await expect(
      fetchSessionLinkTitle("default/20260101_abc123"),
    ).resolves.toBe("Cached title");
  ...
  it("keeps separate cache entries per profile", async () => {
    vi.mocked(getSession).mockImplementation(async (id, scope) => {
      const profile =
        typeof scope === "string" ? scope : scope?.profile || "default";
    ...
    await expect(
      fetchSessionLinkTitle("default/20260101_abc123"),
    ).resolves.toBe("Home chat");
    await expect(fetchSessionLinkTitle("work/20260101_abc123")).resolves.toBe(
      "Work chat",
    );
    expect(sessionRefCacheKey("default/20260101_abc123")).not.toBe(
      sessionRefCacheKey("work/20260101_abc123"),
    );
  });

  it("falls back to the preview when the session has no title", async () => {
    vi.mocked(getSession).mockResolvedValue(
      makeSession({ preview: "Summarize this repo", title: null }),
    );

    await expect(fetchSessionLinkTitle("20260101_abc123")).resolves.toBe(
      "Summarize this repo",
    );
  });
