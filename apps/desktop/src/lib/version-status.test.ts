...
const client = (
  over: Partial<Parameters<typeof resolveVersionStatus>[0]> = {},
) =>
...
describe("resolveVersionStatus", () => {
  ...
  it("names the client as one of two versions in remote mode", () => {
    expect(client({ remote: true, version: "0.4.2" }).label).toBe(
      "client v0.4.2",
    );
  });
  ...
  it("drops the diff and the sha detail while an apply is in flight", () => {
    ...
    expect(
      client({ applying: true, restarting: true, version: "0.4.2" }).label,
    ).toBe("v0.4.2 · restart");
  });

  it("leads the tooltip with the apply message while applying", () => {
    expect(
      client({ applyMessage: "Pulling…", applying: true, version: "0.4.2" })
        .tooltip,
    ).toBe("Pulling… · Hermes Desktop v0.4.2");
  ...
  it("prefers the exact commit diff over the generic (update) hint", () => {
    expect(
      backend({ behind: 4, updateAvailable: true, version: "0.4.2" }).label,
    ).toBe("backend v0.4.2 (+4)");
  });
