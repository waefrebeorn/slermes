...
const MAX_ENTRIES = Math.min(
  24,
  Math.floor(MAX_SESSION_STORE_CHARS / MAX_ENTRY_CHARS),
);
...
function sessionStorageKey(storedSessionId: string): null | string {
  try {
    ...
    return encoded.length > 0 && encoded.length <= MAX_SESSION_KEY_CHARS
      ? `${STORAGE_PREFIX}${encoded}`
      : null;
  } catch {
    return null;
  }
    ...
    snapshot.messages.every(
      (message) =>
        ...
        (message.pending === undefined ||
          typeof message.pending === "boolean") &&
...
function boundedPart(part: ChatMessagePart): ChatMessagePart | null {
  if (part.type === "text") {
    return {
      ...
      ...(part.parentId === undefined
        ? {}
        : { parentId: boundedString(part.parentId, MAX_METADATA_CHARS) }),
    };
  }
  ...
  if (part.type === "tool-call") {
    ...
    return {
      ...
      ...(part.toolCallId === undefined
        ? {}
        : { toolCallId: boundedString(part.toolCallId, MAX_METADATA_CHARS) }),
...
function boundedMessages(messages: ChatMessage[]): ChatMessage[] | null {
  ...
  return bounded.map((message) => ({
    ...
    ...(message.error === undefined
      ? {}
      : { error: boundedString(message.error, MAX_METADATA_CHARS) }),
    ...(message.branchGroupId === undefined
      ? {}
      : {
          branchGroupId: boundedString(
...
function migrateLegacyStore(store: Storage): void {
  ...
  try {
    ...
    const entries = Object.entries(parsed.entries)
      .filter(
        (entry): entry is [string, InFlightTurnSnapshot] =>
          isSnapshot(entry[1]) && !isExpired(entry[1]),
      )
    ...
    for (const [storedSessionId, snapshot] of entries) {
      ...
      const value = messages
        ? serializeSnapshot({ ...snapshot, messages })
        : null;
...
function userMessagesMatch(left: ChatMessage, right: ChatMessage): boolean {
  return (
    left.role === "user" &&
    right.role === "user" &&
    normalizedText(chatMessageText(left)) ===
      normalizedText(chatMessageText(right)) &&
...
function recoverableTail(
  ...
  if (streamId) {
    assistantIndex = visible.findIndex(
      (message) =>
        message.id === streamId && assistantHasRecoverableContent(message),
    );
  }
...
function normalizeRecoveredTail(
  tail: ChatMessage[],
  keepPending: boolean,
): ChatMessage[] {
...
function overlayProjectionRow(
  projection: ChatMessage,
  journalRow: ChatMessage,
): ChatMessage {
...
function journalTailAlreadyCommitted(
  tailAssistants: ChatMessage[],
  baseMessages: ChatMessage[],
): boolean {
...
export function mergeInFlightMessages(
  ...
  const lastJournalRow =
    tailAssistants.findLast(assistantHasRecoverableContent) ?? null;
  const matchingUserIndex = tailUser
    ? baseMessages.findLastIndex((message) =>
        userMessagesMatch(message, tailUser),
      )
    : -1;
  ...
  const completedReply = afterUser.find(
    (message) =>
      assistantHasRecoverableContent(message) && !isLiveProjectionRow(message),
  );
  ...
  const merged = lastJournalRow
    ? overlayProjectionRow(projection, lastJournalRow)
    : projection;

  const sealedRows = tailAssistants.filter(
    (message) =>
      message !== lastJournalRow && assistantHasRecoverableContent(message),
  );
...
export function recoverInFlightTurnJournal(
  ...
  const recovered = mergeInFlightMessages(
    baseMessages,
    snapshot.messages,
    options,
  );
