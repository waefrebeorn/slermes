...
function lexIncrementally(text: string): null | string[] {
  const entry = appendCache.find(
    (cached) =>
      text.length > cached.text.length && text.startsWith(cached.text),
  );
  ...
  if (
    settledLength > entry.text.length ||
    !text.startsWith(entry.text.slice(0, settledLength), 0)
  ) {
    return null;
  }
...
export function parseMarkdownIntoBlocksCached(markdown: string): string[] {
  ...
  const blocks =
    lexIncrementally(markdown) ?? parseMarkdownIntoBlocks(markdown);
