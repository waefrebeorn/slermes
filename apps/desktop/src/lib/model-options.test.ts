...
describe("requestModelOptions", () => {
  ...
  it("uses the connected gateway even before a session exists", async () => {
    ...
    await expect(
      requestModelOptions({ gateway: gateway as never, sessionId: null }),
    ).resolves.toBe(gatewayPayload);
  ...
  it("recovers through profile-scoped REST when the gateway catalog request fails", async () => {
    ...
    const gateway = {
      request: vi.fn(() =>
        Promise.reject(new Error("gateway request unavailable")),
      ),
    };
  ...
  it("preserves the gateway error when its REST recovery path also fails", async () => {
    ...
    vi.mocked(getGlobalModelOptions).mockRejectedValueOnce(
      new Error("REST request unavailable"),
    );

    await expect(
      requestModelOptions({ gateway: gateway as never }),
  ...
  it("prefers an owner-routed request over the ambient gateway socket", async () => {
    ...
    const request = vi.fn(() => Promise.resolve(routedPayload)) as unknown as <
      T,
    >(
      method: string,
  ...
  it("does not recover an owner-routed failure through the ambient REST connection", async () => {
    ...
    await expect(
      requestModelOptions({ profile: "berry", request, sessionId: "tile-1" }),
  ...
  it("keeps an empty owner-routed catalog instead of replacing it from ambient REST", async () => {
    ...
    const request = vi.fn(() => Promise.resolve(ownerPayload)) as unknown as <
      T,
    >(
      method: string,
      params?: Record<string, unknown>,
...
describe("modelOptionsQueryKey", () => {
  it("isolates new-chat catalogs by active gateway profile", () => {
    expect(modelOptionsQueryKey("default")).toEqual([
      "model-options",
      "default",
      "global",
    ]);
    expect(modelOptionsQueryKey("compass")).toEqual([
      "model-options",
      "compass",
      "global",
    ]);
    expect(modelOptionsQueryKey("default")).not.toEqual(
      modelOptionsQueryKey("compass"),
    );
  });
  ...
  it("isolates identical profile and session names across registry connections", () => {
    ...
    queryClient.setQueryData(sourceAKey, {
      providers: [{ models: ["a/model"], slug: "a" }],
    });
    queryClient.setQueryData(sourceBKey, {
      providers: [{ models: ["b/model"], slug: "b" }],
    });

    expect(queryClient.getQueryData(sourceAKey)).toMatchObject({
      providers: [{ models: ["a/model"] }],
    });
    expect(queryClient.getQueryData(sourceBKey)).toMatchObject({
      providers: [{ models: ["b/model"] }],
    });
  });
});
...
describe("manualPickRemoved", () => {
  ...
  it("flags a pick whose model was dropped from a populated provider", () => {
    expect(manualPickRemoved(providers, "openrouter", "nemotron-removed")).toBe(
      true,
    );
  });
  ...
  it("never clobbers when the provider is absent (ambiguous / deauth)", () => {
    expect(manualPickRemoved(providers, "anthropic", "claude-sonnet-4.6")).toBe(
      false,
    );
  });
...
describe("reconcileSelectionAfterCatalogRefresh", () => {
  const zhipu = {
    name: "智谱2",
    slug: "zhipu",
    models: ["glm-4.5-air", "glm-5-turbo"],
  };
  ...
  it("switches to the first new-group model when the current pick is gone", () => {
    ...
    expect(
      reconcileSelectionAfterCatalogRefresh("glm-4.5-air", [moa, bytea]),
    ).toEqual({
      model: "deepseek-v4-flash",
      provider: "byteplus",
    });
  ...
  it("keeps the current pick when it is still in the refreshed catalog", () => {
    expect(
      reconcileSelectionAfterCatalogRefresh("glm-4.5-air", [zhipu, moa]),
    ).toBeNull();
  });

  it("does not wipe the pick when the refreshed catalog has no selectable models", () => {
    expect(
      reconcileSelectionAfterCatalogRefresh("glm-4.5-air", [moa]),
    ).toBeNull();
    expect(reconcileSelectionAfterCatalogRefresh("glm-4.5-air", [])).toBeNull();
    expect(
      reconcileSelectionAfterCatalogRefresh("glm-4.5-air", undefined),
    ).toBeNull();
  });
});
