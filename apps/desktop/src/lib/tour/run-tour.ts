...
export const isTourActive = (): boolean =>
  appHolder.driver?.isActive() ?? false;

/** What's on screen right now, durable (`stable`) handles first. */
export const listTourTargets = (surface?: TourSurface) =>
  runTour({ kind: "targets" }, surface);

/** Spotlight one element with a popover, replacing any current highlight. */
export const showTourStep = (step: TourStep, surface?: TourSurface) =>
  runTour({ ...step, kind: "show" }, surface);
...
export const nextTourStep = (surface?: TourSurface) =>
  runTour({ kind: "next" }, surface);

/** Step a running tour backward. */
export const previousTourStep = (surface?: TourSurface) =>
  runTour({ kind: "prev" }, surface);

/** End the tour and clear the overlay. Safe to call when none is running. */
export const stopTour = (surface?: TourSurface) =>
  runTour({ kind: "stop" }, surface);
