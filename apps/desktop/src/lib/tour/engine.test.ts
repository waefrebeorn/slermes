...
function makeFactory(calls: string[]) {
  return (config?: object) => {
    ...
    const instance: TourDriver & {
      ...
      clickNext: () =>
        cfg.onNextClick ? cfg.onNextClick() : instance.moveNext(),
      clickPrev: () =>
        cfg.onPrevClick ? cfg.onPrevClick() : instance.movePrevious(),
      destroy() {
        calls.push("destroy");
        active = false;
        cfg.onDestroyed?.();
...
describe("collectTourTargets", () => {
  it("reports resolving selectors, durable handles first", () => {
    ...
    expect(targets.some((t) => t.selector === '[data-tour="composer"]')).toBe(
      true,
    );
...
describe("runTourEngine", () => {
  ...
  it("show validates the selector before touching driver.js", () => {
    ...
    const bad = runTourEngine(
      factory,
      holder,
      { kind: "show", selector: "#nope" },
      collectTourTargets,
      document,
    );
  ...
  it("start honors startAt and rejects unknown selectors, naming each", () => {
    ...
    expect(
      runTourEngine(
        ...
        holder,
        {
          kind: "start",
          startAt: 1,
          steps: [{ selector: "#send-btn" }, { title: "Second" }],
        },
        collectTourTargets,
        document,
      ),
    ...
    const result = runTourEngine(
      ...
      {
        kind: "start",
        steps: [{ selector: "#ghost", title: "x" }, { selector: "#send-btn" }],
      },
      collectTourTargets,
      document,
    );
  ...
  it("is self-contained source (injectable into a guest page)", () => {
    ...
    for (const source of [
      runTourEngine.toString(),
      collectTourTargets.toString(),
    ]) {
...
describe("tours that move the app", () => {
  ...
  it("survives the surface re-rendering underneath it", async () => {
    ...
    expect(calls.filter((c) => c.startsWith("drive:")).length).toBeGreaterThan(
      drivesBefore,
    );
