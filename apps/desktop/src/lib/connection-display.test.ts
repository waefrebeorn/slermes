...
describe("connection display helpers", () => {
  ...
  it("matches labels, transport details, localized aliases, and accents", () => {
    ...
    expect(
      connectionMatchesQuery(gateway, "distant", ["Passerelle distante"]),
    ).toBe(true);
  ...
  it("keeps technical endpoints available on demand without exposing secrets", () => {
    ...
    expect(connectionTooltip(remote)).toBe(
    ...
    expect(connectionTooltip(connection("local", "This device", "local"))).toBe(
      "This device",
    );
  });
});
