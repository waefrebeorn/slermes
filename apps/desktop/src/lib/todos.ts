...
export const parseTodoPatch = (value: unknown): null | TodoPatch[] =>
  parsePatch(value, 0);

export const todoArgsWantMerge = (args: unknown): boolean =>
  isRecord(args) && args.merge === true;

/** Same as TodoStore.write(merge=True): update by id, append new items. */
export function mergeTodoItems(
  current: readonly TodoItem[],
  patch: readonly TodoPatch[],
): TodoItem[] {
  ...
  for (const item of patch) {
    ...
    if (index === undefined) {
      next.push({
        content: item.content?.trim() || "(no description)",
        id: item.id,
        status: item.status,
      });
...
function parseRevision(value: unknown, depth: number): null | number {
  ...
  return Object.hasOwn(value, "result")
    ? parseRevision(value.result, depth + 1)
    : null;
}

export const parseTodoRevision = (value: unknown): null | number =>
  parseRevision(value, 0);
...
export function todosFromMessageContent(content: unknown): null | TodoItem[] {
  ...
  for (const part of content) {
    ...
    const parsed =
      parseTodos(part.todos) ??
      parseTodos(part.result) ??
      parseTodos(part.args);
...
export function latestSessionTodos(
