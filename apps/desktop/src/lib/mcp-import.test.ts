...
describe("parseMcpImport", () => {
  ...
  describe("bare command lines", () => {
    ...
    it("parses a uvx line", () => {
      expect(parseMcpImport("uvx mcp-server-fetch")).toEqual([
        {
          config: { args: ["mcp-server-fetch"], command: "uvx" },
          name: "fetch",
        },
      ]);
    });
    ...
    it("parses a node line and names it after the script", () => {
      expect(parseMcpImport("node ./servers/weather.js --port 3010")).toEqual([
        {
          config: {
            args: ["./servers/weather.js", "--port", "3010"],
            command: "node",
          },
          name: "weather",
        },
    ...
    it("parses a docker run line, keeping placeholder env values verbatim", () => {
      expect(
        parseMcpImport(
          "docker run -i --rm -e GITHUB_TOKEN=YOUR_KEY mcp/github",
        ),
      ).toEqual([
        {
          config: {
    ...
    it("honors quoted arguments", () => {
      expect(
        parseMcpImport(
          'npx -y @modelcontextprotocol/server-filesystem "/My Files/docs"',
        ),
      ).toEqual([
        {
          config: {
  ...
  describe("cursor deeplinks", () => {
    ...
    it("rejects deeplinks with an invalid payload", () => {
      expect(
        parseMcpImport(
          "cursor://anysphere.cursor-deeplink/mcp/install?name=x&config=!!!notbase64!!!",
        ),
      ).toBeNull();
      expect(
        parseMcpImport("cursor://anysphere.cursor-deeplink/mcp/install?name=x"),
      ).toBeNull();
    });
  });
