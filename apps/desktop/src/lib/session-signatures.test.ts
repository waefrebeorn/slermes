...
): SessionInfo => ({ id, title, ...extra }) as SessionInfo;
...
describe("sessionMessagesSignature", () => {
  const msg = (role: string, content: string) =>
    ({ role, content }) as Parameters<
      typeof sessionMessagesSignature
    >[0][number];

  it("is stable for identical transcripts", () => {
    expect(sessionMessagesSignature([msg("user", "hi")])).toBe(
      sessionMessagesSignature([msg("user", "hi")]),
    );
  });

  it("changes when content changes", () => {
    expect(sessionMessagesSignature([msg("user", "hi")])).not.toBe(
      sessionMessagesSignature([msg("user", "yo")]),
    );
  });

  it("changes when a message is appended", () => {
    const one = [msg("user", "hi")];
    expect(sessionMessagesSignature(one)).not.toBe(
      sessionMessagesSignature([...one, msg("assistant", "hey")]),
    );
  });
});
