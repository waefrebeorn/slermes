...
function runtimeWith(
  ...
  return { repository } as unknown as Parameters<
...
describe("syncRepositoryIncrementally", () => {
  it("writes only the changed tail instead of the whole transcript", () => {
    ...
    const repository = (runtime as unknown as { repository: MessageRepository })
      .repository;
    ...
    const nextItems = [
      ...items.slice(0, -1),
      { message: nextTail, parentId: "m-198" },
    ];
  ...
  it("appends a new message through the full path", () => {
    ...
    const result = syncRepositoryIncrementally(
      runtime,
      exported(chain([first, second])),
    );
  ...
  it("rebuilds cleanly when a disjoint transcript is swapped in", () => {
    const runtime = runtimeWith(
      chain([message("old-1", "one"), message("old-2", "two")]),
    );
