...
const SKIP_PROTO_RE =
  /^(?:file|data|mailto|javascript|blob|chrome|about|hermes):/i;
const LOCAL_HOST_RE =
...
export function isTitleFetchable(value: string): boolean {
  ...
  return Boolean(
    url && /^https?:$/.test(url.protocol) && !LOCAL_HOST_RE.test(url.host),
  );
}
...
export function useLinkTitle(url?: null | string): string {
  ...
  const key = useMemo(
    () => (normalizedUrl ? titleCacheKey(normalizedUrl) : ""),
    [normalizedUrl],
  );
  const [title, setTitle] = useState(() =>
    key ? (titleCache.get(key) ?? "") : "",
  );
...
export function ExternalLinkIcon({ className }: { className?: string }) {
  return (
    <ArrowUpRight
      aria-hidden
      className={cn(
        "ml-1 inline size-[0.78em] align-[-0.08em] opacity-70",
        className,
      )}
    />
  );
...
export function LinkBrandIcon({
  ...
  return Icon ? (
    <Icon
      aria-hidden
      className={cn(
        "mr-1 inline size-[0.85em] align-[-0.12em] opacity-80",
        className,
      )}
      title=""
    />
  ...
  return (
    <a
      ...
      onClick={(event) => {
        ...
        event.preventDefault();
        openLink(target, {
          native: native || wantsNativeBrowser(event.nativeEvent),
        });
      }}
