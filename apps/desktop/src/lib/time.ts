...
export const fmtClock = new Intl.DateTimeFormat(undefined, {
  hour: "numeric",
  minute: "2-digit",
});
...
export const fmtMonthYear = new Intl.DateTimeFormat(undefined, {
  month: "long",
  year: "numeric",
});
...
export const nominalDayStart = (ms: number): number =>
  startOfLocalDay(ms - DAY_ROLLOVER_HOUR * HOUR);
...
export function localeWeekStartDay(): number {
  try {
    ...
    const firstDay = (withWeekInfo.getWeekInfo?.() ?? withWeekInfo.weekInfo)
      ?.firstDay;
...
export function calendarBucket(
  ...
  if (
    nominal >=
    new Date(ws.getFullYear(), ws.getMonth(), ws.getDate() - 7).getTime()
  ) {
  ...
  return sameYear
    ? { at: nominal, key: `m-${ym}`, kind: "month" }
    : { at: nominal, key: `my-${ym}`, kind: "monthYear" };
}
...
// in here.
export function coarseElapsed(deltaMs: number): {
  unit: ElapsedUnit;
  value: number;
} {
...
export function formatAgo(
  ...
  const by = {
    day: labels.ageDays,
    hour: labels.ageHours,
    minute: labels.ageMinutes,
  };
