...
export const NEEDS_AUTH_RE =
  /\b(401|unauthorized|forbidden|invalid[_ ]?token|authentication|oauth)\b/i;
...
export const probeCache = new Map<
  string,
  { at: number; result: McpTestResult }
>();
...
export const probeKey = (
  ...
  profileKey: string,
): string => `${profileKey}::${name}::${serverFingerprint(server ?? {})}`;

/** Read a still-fresh cached probe result, or null (miss / expired). */
export function freshProbe(
  key: string,
  now = Date.now(),
): McpTestResult | null {
