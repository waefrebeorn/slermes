...
export type HermesOpenTarget =
  string | { href: string } | { path: string; params?: Record<string, string> };
...
function isPluginDeepLinkHost(host: string): boolean {
  return (
    /^[a-z0-9][a-z0-9-]*$/.test(host) && !RESERVED_DEEP_LINK_KINDS.has(host)
  );
}
...
export function normalizeHermesOpenString(raw: string): string | null {
  ...
  if (
    ...
    trimmed.startsWith(`${HERMES_PROTOCOL}//`)
  ) {
    try {
...
export function resolveHermesOpenPath(
...
export function pathFromHermesDeepLink(
  ...
  return resolveHermesOpenPath({
    path: `/${kind}/${name.replace(/^\//, "")}`,
    params,
  });
}

/** Convenience for `hermes://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(
  name: string,
  params: Record<string, string> = {},
): string | null {
