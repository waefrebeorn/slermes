...
describe("nominalDayStart", () => {
  it("rolls the day boundary at 4 AM, not midnight", () => {
    ...
    expect(nominalDayStart(new Date(2026, 5, 20, 1, 30).getTime())).toBe(
      new Date(2026, 5, 19).getTime(),
    );
    expect(nominalDayStart(new Date(2026, 5, 20, 4, 30).getTime())).toBe(
...
describe("calendarBucket", () => {
  ...
  it("assigns the small hours to the previous evening", () => {
    ...
    expect(
      calendarBucket(secondsAt(2026, 5, 18, 23), smallHours, MONDAY).kind,
    ).toBe("today");
    expect(
      calendarBucket(secondsAt(2026, 5, 18, 10), smallHours, MONDAY).kind,
    ).toBe("today");
    expect(
      calendarBucket(secondsAt(2026, 5, 17, 15), smallHours, MONDAY).kind,
    ).toBe("yesterday");
  });
  ...
  it("respects a Sunday week start", () => {
    ...
    expect(calendarBucket(secondsAt(2026, 5, 14), THU_NOON, 0).kind).toBe(
      "thisWeek",
    );
    expect(calendarBucket(secondsAt(2026, 5, 13), THU_NOON, 0).kind).toBe(
      "lastWeek",
    );
  });

  it("keys same-month sessions together and disambiguates across years", () => {
    expect(calendarBucket(secondsAt(2026, 2, 3), THU_NOON, MONDAY).key).toBe(
      "m-2026-2",
    );
    expect(calendarBucket(secondsAt(2026, 2, 20), THU_NOON, MONDAY).key).toBe(
      "m-2026-2",
    );
    expect(calendarBucket(secondsAt(2025, 2, 3), THU_NOON, MONDAY).key).toBe(
      "my-2025-2",
    );
  });
});
...
describe("sessionBucketLabel", () => {
  ...
  const labelAt = (year: number, month: number, day: number) =>
    sessionBucketLabel(
      calendarBucket(secondsAt(year, month, day), THU_NOON, 1),
      labels,
    );
  ...
  it("formats month (same year) and month + year (prior year) via Intl", () => {
    ...
    expect(sessionBucketLabel(monthBucket, labels)).toBe(
      fmtMonth.format(monthBucket.at),
    );
    ...
    expect(sessionBucketLabel(monthYearBucket, labels)).toBe(
      fmtMonthYear.format(monthYearBucket.at),
    );
  });
});
