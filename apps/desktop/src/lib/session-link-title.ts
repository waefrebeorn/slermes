...
function profileMatches(
  sessionProfile: null | string | undefined,
  target?: string,
): boolean {
  if (!target) {
    return true;
  ...
  const row = $sessions
    ...
    .find(
      (session) =>
        ...
        profileMatches(session.profile, profile),
    );
...
function requestSessionRow(
  sessionId: string,
  profile?: string,
): Promise<null | SessionInfo> {
  try {
    return Promise.resolve(getSession(sessionId, profile ?? null)).catch(
      () => null,
    );
  } catch {
...
export function useSessionLinkTitle(
  value: string,
  ...
  const [title, setTitle] = useState(() =>
    key ? titleCache.get(key) || lookupLocalSessionTitle(value) : "",
  );
