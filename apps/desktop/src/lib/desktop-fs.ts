...
export async function readDesktopDir(
  path: string,
 ...
 */
export async function readDesktopFileDataUrlLocalFirst(
  path: string,
): Promise<string> {
  try {
...
export async function desktopGitRoot(path: string): Promise<string | null> {
  ...
  return (await remoteFsApi<{ root: string | null }>(fsPath("git-root", path)))
    .root;
}

export async function desktopDefaultCwd(): Promise<{
  branch: string;
  cwd: string;
} | null> {
  if (!isDesktopFsRemoteMode()) {
...
export async function selectDesktopPaths(
  ...
  return remotePicker
    ? remotePicker.selectPaths({ ...options, multiple: false })
    : [];
}
