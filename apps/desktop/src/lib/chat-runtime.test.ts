...
function attachment(
  overrides: Partial<ComposerAttachment> & Pick<ComposerAttachment, "kind">,
): ComposerAttachment {
...
describe("optimisticAttachmentRef", () => {
  ...
  it("does not render a full path-backed image while its bounded thumbnail is pending", () => {
    expect(
      optimisticAttachmentRef(
        attachment({ kind: "image", detail: "/tmp/shot.png" }),
      ),
    ).toBeNull();
  });
  ...
  it("returns null for an undefined attachment instead of throwing", () => {
    expect(() =>
      optimisticAttachmentRef(undefined as unknown as ComposerAttachment),
    ).not.toThrow();
    expect(
      optimisticAttachmentRef(undefined as unknown as ComposerAttachment),
    ).toBeNull();
  ...
  it("returns null for a null attachment instead of throwing", () => {
    expect(
      optimisticAttachmentRef(null as unknown as ComposerAttachment),
    ).toBeNull();
  });
});
...
describe("attachmentDisplayText", () => {
  it("returns null for undefined|null instead of reading .kind/.refText on a hole", () => {
    expect(() =>
      attachmentDisplayText(undefined as unknown as ComposerAttachment),
    ).not.toThrow();
    expect(
      attachmentDisplayText(undefined as unknown as ComposerAttachment),
    ).toBeNull();
    expect(
      attachmentDisplayText(null as unknown as ComposerAttachment),
    ).toBeNull();
  });
  ...
  it("still resolves a normal file ref", () => {
    expect(
      attachmentDisplayText(
        attachment({ kind: "file", refText: "@file:src/a.ts" }),
      ),
    ).toBe("@file:src/a.ts");
  });
  ...
  it("expands a review attachment into an anchored fenced block", () => {
    ...
    const block = attachmentDisplayText(
      attachment({ kind: "review", detail, refText: "@url:`https://x`" }),
    );
...
describe("coerceThinkingText", () => {
  it("strips streaming status prefixes from thinking deltas", () => {
    expect(
      coerceThinkingText("◉_◉ processing... checking the user's request"),
    ).toBe("checking the user's request");
    expect(coerceThinkingText("(¬‿¬) analyzing... reading the file")).toBe(
      "reading the file",
    );
  });
...
describe("parseCommandDispatch", () => {
  it("keeps the notice on a send directive (e.g. /goal set)", () => {
    ...
    const parsed = parseCommandDispatch({
      type: "send",
      notice: "⊙ Goal set",
...
describe("parseSlashCommand", () => {
  ...
  it("keeps a multiline arg intact instead of failing the whole parse (#41323)", () => {
    expect(
      parseSlashCommand("/goal Write a Python script\nthat prints Hello World"),
    ).toEqual({
...
describe("attachmentId", () => {
  it("normalizes a trailing slash on a url so a re-attach dedupes (#59305 P2)", () => {
    expect(attachmentId("url", "https://example.com/a")).toBe(
      attachmentId("url", "https://example.com/a/"),
    );
  });
  ...
  it("falls back to the trimmed raw value for a malformed url instead of throwing", () => {
    ...
    expect(attachmentId("url", "  not a url  ")).toBe(
      attachmentId("url", "not a url"),
    );
  });

  it("normalizes backslash path separators so a Windows and posix path dedupe", () => {
    expect(attachmentId("file", "a\\b.ts")).toBe(
      attachmentId("file", "a/b.ts"),
    );
  });

  it("normalizes a trailing slash on a folder path", () => {
    expect(attachmentId("folder", "src/app/")).toBe(
      attachmentId("folder", "src/app"),
    );
  });
  ...
  it("keeps distinct urls distinct", () => {
    expect(attachmentId("url", "https://example.com/a")).not.toBe(
      attachmentId("url", "https://example.com/b"),
    );
  });
});
...
describe("messageCreatedAt", () => {
  ...
  it("reads the authoritative Unix-seconds timestamp (not ms)", () => {
    ...
    expect(messageCreatedAt({ timestamp: 1785282262 }, NOW).getFullYear()).toBe(
      2026,
    );
  });
  ...
  it("treats a zero / non-finite timestamp as absent", () => {
    ...
    expect(messageCreatedAt({ timestamp: Number.NaN }, NOW).getTime()).toBe(
      NOW,
    );
  });
});
...
describe("toRuntimeMessage timeline metadata", () => {
  it("does not expose a fabricated visible timestamp for timestamp-less history", () => {
    ...
    expect(
      (runtime.metadata?.custom as { timelineTimestamp?: number })
        .timelineTimestamp,
    ).toBeUndefined();
  });
});
...
describe("coalesceToolOnlyAssistants toolCallId uniqueness", () => {
  ...
  it("folds a clean follow-up unchanged", () => {
    const merged = coalesceToolOnlyAssistants(
      [
        assistant("a1", [
          { type: "text", text: "ok" } as ChatMessagePart,
          tool("call-a"),
        ]),
