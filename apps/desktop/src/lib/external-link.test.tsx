...
const FORGEJO_URL =
  "https://forgejo.home.example/homelab/homelab-ops/issues/101";
...
function installTitleBridge(title: string) {
  ...
  installDesktopBridge({
    fetchLinkTitle:
      bridge as unknown as Window["hermesDesktop"]["fetchLinkTitle"],
  });
...
describe("external link helpers", () => {
  it("formats URL fallbacks as host + path", () => {
    expect(
      ...
      "getyourguide.com/culebra-island-l145468/from-fajardo-full-day-cordillera-islands-catamaran-tour-t19894",
    );
  });
  ...
  it("filters out local/non-http targets for title fetches", () => {
    expect(isTitleFetchable("https://www.expedia.com/things-to-do/foo")).toBe(
      true,
    );
  ...
  it("deduplicates in-flight title fetches and caches results", async () => {
    const bridge = vi
      .fn()
      .mockResolvedValue("El Yunque Tour Water Slide, Rope Swing & Pickup");
    installDesktopBridge({
      fetchLinkTitle:
        bridge as unknown as Window["hermesDesktop"]["fetchLinkTitle"],
    });
  ...
  it("shares cache across protocol/www URL variants", async () => {
    ...
    const first =
      "https://www.getyourguide.com/san-juan-puerto-rico-l355/sunset-tours-tc306/";
    const second =
      "http://getyourguide.com/san-juan-puerto-rico-l355/sunset-tours-tc306/";

    const [a, b] = await Promise.all([
      fetchLinkTitle(first),
      fetchLinkTitle(second),
    ]);
  ...
  it("opens a web link in the in-app browser", async () => {
    ...
    installDesktopBridge({
      openExternal:
        openExternal as unknown as Window["hermesDesktop"]["openExternal"],
    });
  ...
  it("sends every HUD web link to the OS browser", () => {
    ...
    try {
      ...
      render(
        <ExternalLink href="https://accounts.google.com/o/oauth2/auth">
          Sign in
        </ExternalLink>,
      );
  ...
  it("sends a setup-step link straight to the OS browser", () => {
    ...
    render(
      <MarkdownLinkText text="Enable the [Docs API](https://console.cloud.google.com/apis/library) first." />,
    );
  ...
  it("renders pretty links with fetched titles and no host suffix", async () => {
    ...
    expect(link.textContent).toContain(
      "From Fajardo Full Day Cordillera Islands Catamaran Tour",
    );
  ...
  it("still resolves a title when no label was authored", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTitle(FORGEJO_URL).textContent).toContain(
        "Homelab Ops Issue 101",
      );
    });
    expect(bridge).toHaveBeenCalledTimes(1);
  });
  ...
  it("normalizes scheme-less links before opening", () => {
    ...
    render(
      <LinkifiedText text="Source expedia.com/things-to-do/puerto-rico-el-yunque-rainforest-adventure" />,
    );
  ...
  it("explicitOnly skips bare filename/domain tokens and only links explicit URLs", () => {
    ...
    expect(
      screen.queryByText((content) => content.includes("agent.log")),
    ).toBeTruthy();
    expect(links.some((a) => (a.textContent ?? "").includes(".log"))).toBe(
      false,
    );
  });
