...
describe("messageStoreWeight", () => {
  it("charges large text and tool results by character cost, not only part count", () => {
    ...
    expect(
      messageStoreWeight([bigResult(RENDER_WEIGHT_CHARS * 100)]),
  ...
  it("bounds a single enormous payload", () => {
    const enormous = messageStoreWeight([
      bigResult(RENDER_WEIGHT_CHARS * 10_000),
    ]);
...
describe("messagePaintWeight", () => {
  ...
  it("charges a reasoning block one collapsed header", () => {
    const thought = [
      { type: "reasoning", text: "x".repeat(RENDER_WEIGHT_CHARS * 20) },
    ];
  ...
  it("prices an image card flat, however long its data URL", () => {
    ...
    expect(messagePaintWeight(card(10_000_000))).toBe(
      messagePaintWeight(card(80)),
    );
  });
    ...
    const hoisted = [
      {
        ...
        toolName: "todo",
        args: {
          todos: Array.from({ length: 40 }, (_, i) => ({ content: `t${i}` })),
        },
      },
      {
        type: "tool-call",
  ...
  it("keeps a tool-heavy turn far cheaper to paint than to hold", () => {
    ...
    const parts = Array.from({ length: 12 }, () => bigResult(4_000)).concat([
      { type: "text", text: "x".repeat(600) } as unknown as ReturnType<
        typeof bigResult
      >,
    ]);

    expect(messagePaintWeight(parts)).toBeLessThan(
      messageStoreWeight(parts) / 5,
    );
  });
