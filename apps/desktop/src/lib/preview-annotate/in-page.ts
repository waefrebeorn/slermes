...
export function annotateInPage(doc: Document): AnnotateInPage {
  ...
  const toView = (pageRect: AnnotatePageRect): AnnotatePageRect => {
    ...
    return {
      ...
      x: pageRect.x - offset.x,
      y: pageRect.y - offset.y,
    };
  };
  ...
  const markupOf = (el: Element): string => {
    ...
    const nested = clone.querySelectorAll(
      "input, textarea, select, [data-secret]",
    );
    ...
    for (const node of nodes) {
      ...
      for (const name of names) {
        ...
        if (lower === "value" && (secretField || node.getAttribute("value"))) {
          node.setAttribute(
            name,
            secretField ? "[redacted]" : node.getAttribute(name) || "",
          );
        }
  ...
  const relocate = () => {
    ...
    if (
      liveDraft &&
      draftBox &&
      draftBox.style.getPropertyValue("display") !== "none"
    ) {
  ...
  const paintOutline = (el: HTMLElement, heavy: boolean) => {
    style(el, {
      ...
      "border-radius": "4px",
      "box-shadow": heavy
        ? `0 0 0 1px ${blue}, 0 8px 24px rgba(15, 23, 42, 0.18)`
        : `0 0 0 1px ${blue}`,
  ...
  const showPins = (pins: AnnotatePinChrome[]) => {
    ...
    for (const pin of pins) {
      ...
      const page = el
        ? toPage(box(el))
        : pin.page || old?.page || toPage(pin.rect);
  ...
  const showDraft = (rect: AnnotatePageRect, number: number) => {
    ...
    place(
      draftBox,
      liveDraft.el && liveDraft.el.isConnected
        ? box(liveDraft.el)
        : viewOf(liveDraft),
    );
  ...
  return {
    ...
    getMarkerNumbers: () => {
      ...
      return Array.from(
        shadow.querySelectorAll("[data-annotate-pin] [data-annotate-marker]"),
      ).map((node) => Number(node.getAttribute("data-annotate-marker") || "0"));
    },
    getOutlineColor: () => {
      const outline = shadow?.querySelector(
        "[data-annotate-outline], [data-annotate-pin]",
      ) as HTMLElement | null;

      return (
        outline?.style.getPropertyValue("border-color") ||
