...
describe("annotateInPage overlay", () => {
    ...
    api.showPins([
      {
        kind: "element",
        number: 1,
        rect: { height: 24, width: 80, x: 10, y: 10 },
      },
      {
        kind: "area",
        number: 2,
        rect: { height: 40, width: 120, x: 20, y: 80 },
      },
    ]);

    expect(api.getMarkerNumbers()).toEqual([1, 2]);
    expect(api.getOutlineColor().replace(/\s/g, "").toLowerCase()).toMatch(
      /#2f80ed|rgb\(47,128,237\)/,
    );
  });
    ...
    api.showPins([
      {
        kind: "element",
        number: 1,
        rect: { height: 20, width: 80, x: 10, y: 40 },
        selector: "#target-word",
      },
    ]);

    const pin = () =>
      document
        .querySelector("hermes-annotate")!
        .shadowRoot!.querySelector('[data-annotate-pin="1"]') as HTMLElement;
  ...
  it("shows the draft marker before a capture can photograph it", async () => {
    ...
    const draft = shadow.querySelector(
      '[data-annotate-outline="draft"]',
    ) as HTMLElement;
  ...
  it("hides saved pins during a capture so a neighbour marker cannot land in the crop", async () => {
    ...
    api.showPins([
      ...
      },
      {
        kind: "element",
        number: 2,
        rect: { height: 24, width: 80, x: 10, y: 40 },
      },
    ]);
    ...
    const shadow = document.querySelector("hermes-annotate")!.shadowRoot!;
    const pins = () =>
      shadow.querySelector('[data-annotate-pin="1"]')!
    ...
    expect(
      (shadow.querySelector('[data-annotate-outline="draft"]') as HTMLElement)
        .style.display,
    ).toBe("block");
