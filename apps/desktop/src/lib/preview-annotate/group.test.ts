...
function item(number: number, selector?: string): ComposerReadyAnnotation {
  return {
    ...
    identity: selector
      ? { css: {}, html: "", selector, tag: "div", text: "" }
      : undefined,
  };
}
...
describe("annotateSplitDepth", () => {
  it("splits at the shallowest region where the comments disagree", () => {
    expect(
      annotateSplitDepth([
        "body>main>div.header>h1",
        "body>main>div.header>p",
  ...
  it("does not split siblings inside one component", () => {
    ...
    expect(
      groupAnnotations(
        selectors.map((selector, index) => item(index + 1, selector)),
      ),
    ).toHaveLength(1);
  });
...
describe("groupAnnotations", () => {
  ...
  it("produces groups whose subtrees do not overlap, so they can run in parallel", () => {
    ...
    const overlapping = keys.filter((key) =>
  ...
  it("keeps area pins in their own trailing group rather than guessing a subtree", () => {
    const groups = groupAnnotations([
      item(1, "body>main>div.a>h1"),
      item(2),
      item(3, "body>main>div.b>h1"),
      item(4),
    ]);
  ...
  it("returns one group when every comment lands in the same region", () => {
    const groups = groupAnnotations([
      item(1, "body>div.card>h1"),
      item(2, "body>div.card>p"),
    ]);
...
describe("groupAnnotations refinement", () => {
  ...
  const page = [
    ...["a.logo", "ul.links>li", "button.menu"].map((tail, index) =>
      item(index + 1, `body>header.nav>${tail}`),
    ),
    ...["h1", "p.sub", "a.cta", "img.art"].map((tail, index) =>
      item(index + 4, `body>main>section.hero>${tail}`),
    ),
    ...["table", "button.buy", "span.note"].map((tail, index) =>
      item(index + 8, `body>main>section.pricing>${tail}`),
    ),
    ...["details:nth-of-type(1)", "details:nth-of-type(4)"].map((tail, index) =>
      item(index + 11, `body>main>section.faq>${tail}`),
    ),
    ...["div.cols>ul", "small.copy"].map((tail, index) =>
      item(index + 13, `body>footer.foot>${tail}`),
    ),
  ];
  ...
  it("stops instead of looping when an oversized group cannot divide further", () => {
    const identical = Array.from({ length: 9 }, (_, index) =>
      item(index + 1, "body>div.card>span"),
    );
