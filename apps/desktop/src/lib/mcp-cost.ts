...
export function estimateServerTokens(
  ...
  for (const tool of tools) {
    if (
      typeof tool.schema_chars !== "number" ||
      !Number.isFinite(tool.schema_chars) ||
      tool.schema_chars <= 0
    ) {
      continue;
    }
...
export const sanitizeMcpNameComponent = (value: string): string =>
...
export const mcpServerUsagePrefix = (serverName: string): string =>
 ...
 */
export function serverUsageCount(
  serverName: string,
