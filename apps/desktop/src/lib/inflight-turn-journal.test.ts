...
const sessionStorageKey = (storedSessionId: string) =>
  `${STORAGE_PREFIX}${encodeURIComponent(storedSessionId)}`;
...
function journalState(
  overrides: Partial<JournalableSessionState> = {},
): JournalableSessionState {
  return {
...
describe("persistInFlightTurnState", () => {
  it("sweeps expired and oldest session entries once before the first write", () => {
    ...
    window.localStorage.setItem(
      ...
      JSON.stringify({
        messages: [
          user("expired-u", "expired"),
          assistant("expired-a", "expired", { pending: true }),
        ],
        streamId: "expired-a",
        turnStartedAt: 0,
    ...
    expect(
      window.localStorage.getItem(sessionStorageKey("expired")),
    ).toBeNull();
    expect(window.localStorage.getItem(sessionStorageKey("old-24"))).toBeNull();
    expect(
      window.localStorage.getItem(sessionStorageKey("stored-1")),
    ).not.toBeNull();
  });
  ...
  it("writes only the current session instead of reading and rewriting the aggregate journal", () => {
    ...
    const spyTarget =
      typeof storageConstructor === "function" &&
      localStorage instanceof storageConstructor
  ...
  it("keeps another session snapshot when one session settles", () => {
    ...
    expect(
      window.localStorage.getItem(sessionStorageKey("stored-2")),
    ).not.toBeNull();
  ...
  it("coalesces rapid updates into one write carrying the latest state", () => {
    ...
    persistInFlightTurnState(
      journalState({
        messages: [
          user("u1", "do the thing"),
          assistant("assistant-stream-1", "partial answer grew", {
            pending: true,
          }),
        ],
      }),
    ...
    expect(tail?.parts).toEqual([
      { type: "text", text: "partial answer grew" },
    ]);
  });
  ...
  it("preserves a long user prompt exactly so recovery still matches its transcript row", () => {
    ...
    const result = recoverInFlightTurnJournal("stored-1", [
      user("db-u1", prompt),
    ]);
  ...
  it("preserves user attachment refs exactly so recovery still matches its transcript row", () => {
    const attachmentRefs = Array.from(
      { length: 25 },
      (_, index) => `@file:/tmp/input-${index}.txt`,
    );
  ...
  it("trims oldest sealed rows when bounded parts exceed the entry cap", () => {
    ...
    expect(snapshot.messages.map((message: ChatMessage) => message.id)).toEqual(
      ["u1", "a3", "a4"],
    );
  });
  ...
  it("keeps an older recoverable snapshot when the newest row alone is too large", () => {
    ...
    persistInFlightTurnState(
      journalState({
        messages: [user("u1", "do the thing"), hugeAssistant],
        streamId: hugeAssistant.id,
      }),
    );
    vi.advanceTimersByTime(400);
  ...
  it("does not parse the legacy aggregate when a pathological write discards its session", () => {
    const legacy = {
      messages: [
        user("legacy-u1", "old prompt"),
        assistant("legacy-a1", "old partial", { pending: true }),
      ],
      streamId: "legacy-a1",
      turnStartedAt: 1,
      updatedAt: Date.now(),
    ...
    window.localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ entries: { "stored-1": legacy }, version: 1 }),
    );
  ...
  it("strips pathological 5 MiB tool payloads before attempting a storage write", () => {
    ...
    persistInFlightTurnState(
      journalState({ messages: [user("u1", "do the thing"), oversized] }),
    );
    vi.advanceTimersByTime(400);
    ...
    expect(snapshot.messages[1].parts[1]).toEqual({
      type: "text",
      text: "still useful",
    });
    expect(
      setItem.mock.calls.every(([, value]) => value.length <= 256 * 1024),
    ).toBe(true);
  });
  ...
  it("clears the entry the moment the turn settles, cancelling pending writes", () => {
    ...
    persistInFlightTurnState(
      journalState({ busy: false, awaitingResponse: false, streamId: null }),
    );
  ...
  it("does not journal a turn with no recoverable assistant content yet", () => {
    persistInFlightTurnState(
      journalState({ messages: [user("u1", "do the thing")], streamId: null }),
    );
...
describe("legacy journal migration", () => {
  it("migrates the bounded v1 aggregate once and recovers its sessions", () => {
    const first = {
      messages: [
        user("u1", "one"),
        assistant("a1", "partial one", { pending: true }),
      ],
      streamId: "a1",
      turnStartedAt: 1,
      updatedAt: Date.now(),
    ...
    window.localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ entries: { one: first, two: second }, version: 1 }),
    );

    expect(readInFlightTurnJournal("one")).toEqual(first);
    const migratedSecondRaw = window.localStorage.getItem(
      sessionStorageKey("two"),
    )!;
    ...
    window.localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ entries: { three: first }, version: 1 }),
    );
  ...
  it("does not overwrite a newer per-session snapshot while migrating another legacy session", () => {
    ...
    const legacyOther = {
      messages: [
        user("u2", "other prompt"),
        assistant("a2", "other partial", { pending: true }),
      ],
      streamId: "a2",
      turnStartedAt: 2,
      updatedAt: Date.now(),
    ...
    window.localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({
        entries: { "stored-1": legacyCurrent, other: legacyOther },
        version: 1,
      }),
    );

    expect(readInFlightTurnJournal("other")).toEqual(legacyOther);
    expect(readInFlightTurnJournal("stored-1")?.messages[0]).toEqual(
      user("u1", "do the thing"),
    );
  });
...
describe("recoverInFlightTurnJournal", () => {
  function journalEntry(messages: ChatMessage[]) {
    persistInFlightTurnState(
      journalState({ messages, streamId: messages.at(-1)?.id ?? null }),
    );
    vi.advanceTimersByTime(400);
  }
  ...
  it("appends the full tail when the base transcript never saw the turn", () => {
    journalEntry([
      user("u1", "do the thing"),
      assistantWithTool("assistant-stream-1", "working on it", {
        pending: true,
      }),
    ]);
  ...
  it("overlays the backend text-only projection instead of dropping local tool progress", () => {
    ...
    journalEntry([
      user("u1", "do the thing"),
      assistantWithTool("assistant-stream-old", "local part", {
        pending: true,
      }),
    ]);
    ...
    const base = [
      ...
      assistant(
        ...
        "local part and more from the backend snapshot",
        { pending: true },
      ),
    ];
    ...
    expect(merged.parts[1]).toMatchObject({
      type: "text",
      text: "local part and more from the backend snapshot",
    });
  ...
  it("keeps the journal text when it is longer than the projection text", () => {
    journalEntry([
      user("u1", "do the thing"),
      assistantWithTool(
        "assistant-stream-old",
        "a much longer locally journaled partial answer",
        { pending: true },
      ),
    ]);
    ...
    const base = [
      user("db-u1", "do the thing"),
      assistant("assistant-stream-rt9", "thin", { pending: true }),
    ];
    const result = recoverInFlightTurnJournal("stored-1", base, {
      keepPending: true,
    });
    ...
    expect(merged.parts[1]).toMatchObject({
      type: "text",
      text: "a much longer locally journaled partial answer",
    });
  });
  ...
  it("does not re-append committed answers when the journaled user row never persisted", () => {
    ...
    journalEntry([
      user(
        "user-inflight-a3c2beb1",
        "a stray user bubble that never persisted",
      ),
    ...
    const base = [
      ...
      assistant("db-a1", "the committed answer"),
    ];
    const result = recoverInFlightTurnJournal("stored-1", base, {
      keepPending: false,
    });
  ...
  it("keeps appending a genuinely unknown turn (crash recovery still works)", () => {
    ...
    journalEntry([
      user("u1", "the live prompt"),
      assistant("assistant-stream-1", "partial answer", { pending: true }),
    ]);

    const base = [
      user("db-u0", "an earlier turn"),
      assistant("db-a0", "earlier reply"),
    ];
    const result = recoverInFlightTurnJournal("stored-1", base, {
      keepPending: true,
    });
...
describe("mid-turn redirect corrections", () => {
  ...
  it("journals the whole user run, not just the correction", () => {
    ...
    const journaled =
      readInFlightTurnJournal("stored-redirect")?.messages ?? [];

    expect(
      journaled.map((message) =>
        message.parts.map((part) => (part as { text: string }).text).join(""),
      ),
    ).toEqual(["remove the session counts", "hurry up", "Moving."]);
  });
  ...
  it("still stops at an assistant boundary so prior turns are not journaled", () => {
    ...
    const journaled =
      readInFlightTurnJournal("stored-boundary")?.messages ?? [];

    expect(journaled.map((message) => message.id)).toEqual([
      "user-1",
      "assistant-stream-1",
    ]);
  });
