...
function longCode(lines: number): string {
  return Array.from(
    { length: lines },
    (_, i) =>
      `export function helper${i}(value: number) { return value * ${i} }`,
  ).join("\n");
}
...
describe("artifactSlug", () => {
  it("is stable across regenerations of the same artifact", () => {
    const a = artifactSlug({
      kind: "html",
      language: "html",
      title: "Pomodoro Timer",
    });
    const b = artifactSlug({
      kind: "html",
      language: "html",
      title: "Pomodoro Timer",
    });
  ...
  it("distinguishes different titles", () => {
    expect(
      artifactSlug({ kind: "html", language: "html", title: "Timer" }),
    ).not.toBe(
  ...
  it("handles empty/symbol-only titles", () => {
    expect(artifactSlug({ kind: "code", language: "ts", title: "!!!" })).toBe(
      "code:ts:untitled",
    );
  });
});
...
describe("artifactDownloadName", () => {
  it("keeps an existing extension", () => {
    expect(artifactDownloadName("code", "python", "server.py")).toBe(
      "server.py",
    );
  });

  it("appends by kind and language", () => {
    expect(artifactDownloadName("html", "html", "Pomodoro Timer")).toBe(
      "Pomodoro-Timer.html",
    );
    expect(artifactDownloadName("svg", "svg", "Org Chart")).toBe(
      "Org-Chart.svg",
    );
    expect(artifactDownloadName("code", "typescript", "buildDashboard")).toBe(
      "buildDashboard.ts",
    );
    expect(artifactDownloadName("code", "unknownlang", "")).toBe(
      "artifact.txt",
    );
  });
});
