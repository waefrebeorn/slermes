...
describe("fetchVoiceClientConfig", () => {
  ...
  it("fetches from /api/audio/voice-config with the ambient scope and caches per scope", async () => {
    ...
    const request = api.mock.calls[0][0] as {
      connectionId?: string;
      path: string;
      profile?: string;
    };
...
describe("transcribeAudioClientDirect", () => {
  ...
  it("unwraps Mistral Voxtral JSON instead of dumping it into the composer", async () => {
    ...
    const fetchMock = vi.fn(
      async () => new Response(JSON.stringify(voxtral), { status: 200 }),
    );
    vi.stubGlobal("fetch", fetchMock);

    expect(
      await transcribeAudioClientDirect(
        new Blob(["x"], { type: "audio/webm" }),
      ),
  ...
  it("surfaces provider rejections instead of silently relaying", async () => {
    ...
    await expect(transcribeAudioClientDirect(new Blob(["x"]))).rejects.toThrow(
  ...
  it("speaks the xai wire shape", async () => {
    ...
    const fetchMock = vi.fn(
      async () =>
        new Response(JSON.stringify({ text: "grok heard this" }), {
          status: 200,
        }),
    );
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("speaks the elevenlabs wire shape with xi-api-key auth", async () => {
    ...
    const fetchMock = vi.fn(
      async () =>
        new Response(JSON.stringify({ text: "scribe text" }), { status: 200 }),
    );
    vi.stubGlobal("fetch", fetchMock);
...
describe("synthesizeSpeechClientDirect", () => {
  ...
  it("throws on provider rejection", async () => {
    ...
    await expect(
      synthesizeSpeechClientDirect(openaiTts, "Hi."),
    ).rejects.toThrow(/openai TTS error.*429/);
  });
});

describe("transcriptFromOpenAiMultipartBody", () => {
  it("keeps Groq/OpenAI plain-text bodies", () => {
    expect(transcriptFromOpenAiMultipartBody("  hello world  ")).toBe(
      "hello world",
    );
  });
  ...
  it("treats a JSON envelope with empty text as silence", () => {
    expect(
      transcriptFromOpenAiMultipartBody(
        JSON.stringify({ text: "  ", model: "voxtral-mini-latest" }),
      ),
    ).toBe("");
  });

  it("leaves a non-envelope JSON object intact", () => {
    expect(transcriptFromOpenAiMultipartBody('{"error":"nope"}')).toBe(
      '{"error":"nope"}',
    );
  });
});
...
describe("cutSentences", () => {
  it("emits complete sentences and holds the incomplete tail", () => {
    const { sentences, rest } = cutSentences(
      "This is the first full sentence. And then it keeps goi",
      false,
    );
  ...
  it("flush drains everything including the tail", () => {
    const { sentences, rest } = cutSentences(
      "First complete sentence right here. tail bit",
      true,
    );

    expect(sentences).toEqual([
      "First complete sentence right here.",
      "tail bit",
    ]);
    expect(rest).toBe("");
  });
