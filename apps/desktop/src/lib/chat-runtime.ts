...
export function contextPath(path: string, cwd: string): string {
  ...
  return path.startsWith(normalizedCwd)
    ? path.slice(normalizedCwd.length)
    : path;
}
 ...
 */
export function optimisticAttachmentRef(
  attachment: ComposerAttachment,
): string | null {
  if (!attachment) {
    return null;
...
export function parseSlashCommand(command: string) {
  ...
  return match
    ? { name: match[1], arg: match[2].trim() }
    : { name: "", arg: "" };
}

export function parseCommandDispatch(
  raw: unknown,
): CommandDispatchResponse | null {
