...
const REMOTE_HTML_PREVIEW_MAX_BASE64_BYTES =
  Math.ceil(REMOTE_HTML_PREVIEW_MAX_BYTES / 3) * 4;
...
export function validatedRemoteHtmlDataUrl(value: string): string | null {
  ...
  if (
    payload.length > REMOTE_HTML_PREVIEW_MAX_BASE64_BYTES ||
    payload.length % 4 !== 0
  ) {
    return null;
  }
...
export function remoteHtmlPreviewDocument(dataUrl: string): string | null {
  ...
  const html = new TextDecoder().decode(
    Uint8Array.from(atob(validated.slice(validated.indexOf(",") + 1)), (char) =>
      char.charCodeAt(0),
    ),
  );
...
async function enrichPreviewTarget(
  ...
  if (target.previewKind === "html") {
    try {
      ...
      return dataUrl
        ? { ...target, dataUrl }
        : { ...target, renderMode: "source", transient: true };
    } catch {
