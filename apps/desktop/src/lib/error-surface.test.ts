...
describe("parseErrorSurface", () => {
  ...
  it("accepts every documented layer", () => {
    for (const layer of [
      ...
      expect(
        parseErrorSurface({ layer, code: "x", retryable: false })?.layer,
      ).toBe(layer);
    }
  });
  ...
  it("rejects unknown layers and non-objects", () => {
    expect(
      parseErrorSurface({ layer: "blockchain", code: "x", retryable: true }),
    ).toBeNull();
  ...
  it("defaults code and retryable when missing", () => {
    expect(parseErrorSurface({ layer: "gateway" })).toEqual({
      layer: "gateway",
      code: "unknown",
      retryable: true,
    });
  ...
  it("carries the failing session identity when present", () => {
    ...
    expect(
      parseErrorSurface({ layer: "provider", code: "x", retryable: true })
        ?.provider,
    ).toBeUndefined();
  });
});
