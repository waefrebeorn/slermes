...
describe("svgSize", () => {
  it("reads explicit pixel width/height", () => {
    expect(svgSize('<svg width="312" height="90"><rect/></svg>')).toEqual({
      width: 312,
      height: 90,
    });
  });
  ...
  it("falls back to the viewBox when width/height are absent", () => {
    expect(svgSize('<svg viewBox="0 0 400 100"><rect/></svg>')).toEqual({
      width: 400,
      height: 100,
    });
  });
  ...
  it("treats mixed-unit widths as absent (not parseFloat(100) == 100)", () => {
    expect(
      svgSize(
        '<svg width="100%" height="70" viewBox="0 0 500 200"><rect/></svg>',
      ),
    ).toEqual({
      width: 500,
      height: 200,
...
describe("normalizeSvgSize", () => {
  ...
  it("leaves svgs without a percentage width untouched", () => {
    const svg =
      '<svg xmlns="http://www.w3.org/2000/svg" width="312" viewBox="0 0 312 90"><rect/></svg>';
