...
describe("normalizeFilePreviewMath", () => {
  it("leaves single-dollar inline math untouched", () => {
    expect(normalizeFilePreviewMath("Price is $x^2 + y^2$ today")).toBe(
      "Price is $x^2 + y^2$ today",
    );
  });
  ...
  it("does not escape dollars inside inline code spans", () => {
    expect(normalizeFilePreviewMath("Run `echo $HOME` now")).toBe(
      "Run `echo $HOME` now",
    );
  });
  ...
  it("normalizes latex delimiters to dollar math", () => {
    ...
    expect(normalizeFilePreviewMath("A formula \\(x + 1\\) inline.")).toBe(
      "A formula $x + 1$ inline.",
    );
  });
