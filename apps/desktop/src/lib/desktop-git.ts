...
const remoteGit: GitBridge = {
  worktreeList: async (repoPath) =>
    (
      await gitGet<{ worktrees: HermesGitWorktree[] }>("worktrees", {
        path: repoPath,
      })
    ).worktrees,
  ...
  branchSwitch: (repoPath, branch) =>
    gitPost("branch/switch", { branch, path: repoPath }),

  branchList: async (repoPath) =>
    (
      await gitGet<{ branches: HermesGitBranch[] }>("branches", {
        path: repoPath,
      })
    ).branches,
  ...
  baseBranchList: async (repoPath) =>
    (
      await gitGet<{ branches: HermesGitBaseBranch[] }>("base-branches", {
        path: repoPath,
      })
    ).branches,

  repoStatus: (repoPath) =>
    gitGet<HermesRepoStatus | null>("status", { path: repoPath }),

  fileDiff: async (repoPath, filePath) =>
    (
      await gitGet<{ diff: string }>("file-diff", {
        file: filePath,
        path: repoPath,
      })
    ).diff,
  ...
  review: {
    list: (repoPath, scope, baseRef) =>
      gitGet<HermesReviewList>("review/list", {
        base: baseRef,
        path: repoPath,
        scope,
      }),
    ...
    revParse: async (repoPath, ref) =>
      (
        await gitGet<{ sha: null | string }>("review/rev-parse", {
          path: repoPath,
          ref,
        })
      ).sha,
    ...
    commitContext: (repoPath) =>
      gitGet("review/commit-context", { path: repoPath }),

    push: (repoPath) => gitPost("review/push", { path: repoPath }),

    shipInfo: (repoPath) =>
      gitGet<HermesReviewShipInfo>("review/ship-info", { path: repoPath }),
