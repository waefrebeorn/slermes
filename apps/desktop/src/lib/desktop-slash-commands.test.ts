...
describe("desktop slash command curation", () => {
  ...
  it("keeps core desktop chat commands in suggestions", () => {
    ...
    expect(resolveDesktopCommand("/approvals")?.surface).toEqual({
      kind: "exec",
    });
  ...
  it("/voice points at the composer voice button instead of the generic advanced message", () => {
    ...
    expect(resolveDesktopCommand("/voice")?.surface).toEqual({
      kind: "unavailable",
      reason: "composer-voice",
    });
  ...
  it("routes /pet through the desktop action handler and drops /pets", () => {
    expect(resolveDesktopCommand("/pet")?.surface).toEqual({
      kind: "action",
      action: "pet",
    });
    ...
    expect(resolveDesktopCommand("/pets")?.surface).toEqual({
      kind: "unavailable",
      reason: "settings",
    });
  ...
  it("routes /wake through the desktop wake action instead of the slash worker", () => {
    expect(resolveDesktopCommand("/wake")?.surface).toEqual({
      kind: "action",
      action: "wake",
    });
  ...
  it("routes /stop through the desktop action that cancels the active turn", () => {
    expect(resolveDesktopCommand("/stop")?.surface).toEqual({
      kind: "action",
      action: "stop",
    });
  ...
  it("treats /browser as an executable action command (local-gateway connect)", () => {
    ...
    expect(resolveDesktopCommand("/browser")?.surface).toEqual({
      kind: "action",
      action: "browser",
    });
  ...
  it("routes /compress through the session-compression action", () => {
    ...
    expect(resolveDesktopCommand("/compress")?.surface).toEqual({
      kind: "action",
      action: "compress",
    });
    ...
    expect(resolveDesktopCommand("/compact")?.surface).toEqual({
      kind: "action",
      action: "compress",
    });
    expect(isDesktopSlashCommand("/compact")).toBe(true);
  ...
  it("routes /btw to the prompt.btw side-question action", () => {
    expect(resolveDesktopCommand("/btw")?.surface).toEqual({
      kind: "action",
      action: "btw",
    });
    expect(isDesktopSlashCommand("/btw")).toBe(true);
  ...
  it("routes /journey (and aliases) to the memory graph overlay action", () => {
    expect(resolveDesktopCommand("/journey")?.surface).toEqual({
      kind: "action",
      action: "journey",
    });
    expect(resolveDesktopCommand("/memory-graph")?.surface).toEqual({
      kind: "action",
      action: "journey",
    });
    expect(resolveDesktopCommand("/learning")?.surface).toEqual({
      kind: "action",
      action: "journey",
    });
  ...
  it("uses desktop-specific labels for commands with different UI behavior", () => {
    expect(
      desktopSlashDescription("/branch", "Branch the current session"),
    ).toBe("Branch the latest message into a new chat");
    expect(
      desktopSlashDescription("/skin", "Show or change the display skin/theme"),
    ).toBe("Switch desktop theme or cycle to the next one");
  });
  ...
  it("explains known commands that desktop owns elsewhere", () => {
    expect(desktopSlashUnavailableMessage("/model sonnet")).toContain(
      "model picker",
    );
    expect(desktopSlashUnavailableMessage("/skills")).toContain(
      "desktop sidebar",
    );
    expect(desktopSlashUnavailableMessage("/clear")).toContain(
      "terminal interface",
    );
  });
  ...
  it("resolves commands and aliases to their declared surface", () => {
    expect(resolveDesktopCommand("/new")?.surface).toEqual({
      kind: "action",
      action: "new",
    });
    expect(resolveDesktopCommand("/reset")?.surface).toEqual({
      kind: "action",
      action: "new",
    });
    expect(resolveDesktopCommand("/resume")?.surface).toEqual({
      kind: "picker",
      picker: "session",
    });
    expect(resolveDesktopCommand("/usage")?.surface).toEqual({ kind: "exec" });
    expect(resolveDesktopCommand("/clear")?.surface).toEqual({
      kind: "unavailable",
      reason: "terminal",
    });
...
describe("rankSkillCommands", () => {
  ...
  it("drops never-used built-ins when browsing, keeping everything else", () => {
    const browsing = rankSkillCommands(rows, skills, {
      pruneUnusedBuiltins: true,
    }).map((row) => row.text);
  ...
  it("leaves the backend order untouched when the catalog carries no usage", () => {
    expect(
      rankSkillCommands(rows, undefined, { pruneUnusedBuiltins: true }),
    ).toEqual(rows);
  });

  it("ranks an alias by the canonical command it resolves to", () => {
    const ranked = rankSkillCommands(
      [{ text: "/sessions" }, { text: "/research" }],
      {
