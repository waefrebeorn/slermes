...
const group = (entries: SidebarSessionEntry[], nowMs = NOW) =>
  groupEntriesByRecency(entries, nowMs, MONDAY);

const dividerKeys = (
  rows: ReturnType<typeof groupEntriesByRecency>,
): string[] => rows.flatMap((row) => (row.kind === "divider" ? [row.key] : []));
...
describe("groupEntriesByRecency", () => {
  ...
  it("never slices a rapid-fire burst mid-run", () => {
    ...
    const rows = group([
      ...burst,
      entry(session("old", { last_active: at(2026, 5, 17, 15) })),
    ]);
...
describe("hideCollapsedGroupRows", () => {
  ...
  it("never hides the unlabelled head above the first divider", () => {
    ...
    expect(
      visible.map((row) =>
        row.kind === "session" ? row.entry.session.id : row.key,
      ),
    ).toEqual(["a", "b", "yesterday"]);
  });
});
