...
describe("coerceRemoteUrlScheme", () => {
  it("prepends http:// to scheme-less host:port input", () => {
    expect(coerceRemoteUrlScheme("100.64.0.1:9119")).toBe(
      "http://100.64.0.1:9119",
    );
    expect(coerceRemoteUrlScheme("mini.tailnet-1234.ts.net:9119")).toBe(
      "http://mini.tailnet-1234.ts.net:9119",
    );
    expect(coerceRemoteUrlScheme("localhost:9119")).toBe(
      "http://localhost:9119",
    );
    expect(coerceRemoteUrlScheme("gw.example.com")).toBe(
      "http://gw.example.com",
    );
  });
  ...
  it("leaves explicitly schemed URLs alone", () => {
    ...
    expect(coerceRemoteUrlScheme("https://gw.example.com/hermes")).toBe(
