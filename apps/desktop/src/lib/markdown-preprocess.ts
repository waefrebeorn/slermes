...
const HUGGING_DISPLAY_MATH_CLOSE_RE =
  /^([ \t]*(?:>[ \t]*)*[ \t]*)(\S[^\n]*?)\$\$[ \t]*\r?$/;
...
const LOCAL_PREVIEW_URL_RE =
  /(^|\s)https?:\/\/(?:localhost|127\.0\.0\.1|0\.0\.0\.0|\[::1\])(?::\d+)?\/?[^\s<>"'`]*/gi;
const LOCAL_PREVIEW_ONLY_RE =
...
function isUrlOnlyBlock(lines: string[]): boolean {
  ...
  return (
    nonEmpty.length > 0 && nonEmpty.every((line) => URL_ONLY_LINE_RE.test(line))
  );
}
...
function routeFileLinksToPreview(text: string): string {
  return text.replace(FILE_LINK_RE, (match: string, ...args: unknown[]) => {
    ...
    const href =
      mediaKind(target) === "file"
        ? previewMarkdownHref(target)
        : mediaMarkdownHref(target);
    ...
    .map((part) =>
      part.startsWith("`")
        ...
        : part
            ...
            .map((segment, index) =>
              index % 2 === 1 ? segment : rewriteProseSegment(segment),
            )
            .join(""),
    )
    .join("");
...
function normalizeProseMath(text: string): string {
  ...
  const normalized = splitHuggingDisplayMath(
    normalizeMathDelimiters(normalizeDisplayMathForMarkdown(text)),
  );
  ...
  return strippedEmptyFences
    ...
    .map((part) => {
      ...
      const transformed = clampHtmlNestingDepth(
        normalizeVisibleProse(stripPreviewTargets(normalizeProseMath(part))),
      );
  ...
  return text
    ...
    .map((part) => {
      ...
      return part
        .split(INLINE_CODE_SPLIT_RE)
        .map((segment) =>
          segment.startsWith("`") ? segment : normalizeProseMath(segment),
        )
        .join("");
    })
    .join("");
