...
const PUNCTUATED_PARAGRAPH_BREAK_RE =
  /([.!?])([*_~`>"'’”)}\]]*)[ \t]*\n{2,}[ \t]*/g;
const SOFT_BREAK_RE = /[ \t]*\n[ \t]*/g;
...
function parseMarkdownTableRow(line: string): MarkdownTableRow | null {
  ...
  const pipeIndexes = [...row.matchAll(/\|/g)]
    .map((match) => match.index)
    .filter((index) => isUnescapedPipe(row, index));
  ...
  if (
    cells.length < 2 &&
    !(hasLeadingPipe && hasTrailingPipe && cells.length === 1)
  ) {
    return null;
  }
...
function stripMarkdownTables(text: string): string {
  ...
  while (index < lines.length) {
    ...
    if (
      !delimiterRow ||
      !headerRow ||
      !delimiterRow.cells.every((cell) =>
        MARKDOWN_TABLE_DELIMITER_CELL_RE.test(cell),
      ) ||
    ...
    for (; rowIndex < lines.length; rowIndex += 1) {
      ...
      if (
        !bodyRow ||
        bodyRow.blockquoteDepth !== delimiterRow.blockquoteDepth
      ) {
        break;
      }
