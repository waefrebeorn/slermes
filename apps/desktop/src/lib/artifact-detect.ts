...
export function detectArtifact(
  ...
  if (HTML_LANGUAGES.has(clean)) {
    ...
    if (
      ...
      (!isDocument &&
        trimmed.length >= HTML_FRAGMENT_MIN_CHARS &&
      ...
      return {
        ...
        language: clean,
        title:
          titleFromTag(trimmed, "title") ||
          titleFromTag(trimmed, "h1") ||
          "HTML",
      };
    }
  ...
  if (clean === "svg") {
    ...
    if (trimmed.length >= SVG_MIN_CHARS && /<svg[\s>]/i.test(trimmed)) {
      return {
        kind: "svg",
        language: clean,
        title: titleFromTag(trimmed, "title") || "SVG",
      };
    }
...
export function artifactSlug(
  detection: Pick<ArtifactDetection, "kind" | "language" | "title">,
): string {
