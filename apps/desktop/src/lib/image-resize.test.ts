...
describe("downscaleDataUrlForPreview", () => {
  ...
  describe("without createImageBitmap (jsdom default)", () => {
    it("fails closed when createImageBitmap is unavailable", async () => {
      await expect(
        downscaleDataUrlForPreview("data:image/png;base64,ZmFrZQ=="),
      ).resolves.toBe(FALLBACK_PLACEHOLDER);
    });
  ...
  describe("with mocked createImageBitmap + OffscreenCanvas", () => {
    ...
    function setupMocks(bitmapWidth: number, bitmapHeight: number) {
      ...
      return {
        ...
        ctx,
        canvasSizes,
        maxActiveBitmaps: () => maxActiveBitmaps,
      };
    }
    ...
    it("keeps every thumbnail bounded for a 72-image composer prompt", async () => {
      const { canvasSizes, drawImage, maxActiveBitmaps } = setupMocks(
        6000,
        6000,
      );
      ...
      await Promise.all(
        Array.from({ length: 72 }, () =>
          downscaleDataUrlForPreview(TINY_PNG_DATA_URL),
        ),
      );

      expect(canvasSizes).toHaveLength(72);
      expect(
        canvasSizes.every(([width, height]) => width === 512 && height === 512),
      ).toBe(true);
