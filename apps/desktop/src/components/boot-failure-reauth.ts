...
export function isRemoteReauthError(error: string | null | undefined): boolean {
  ...
  return (
    ...
    (text.includes("oauth") &&
      (text.includes("not signed in") || text.includes("sign in")))
  );
}
 ...
 */
export function shouldApplyPostBootProgressError(
...
export function deriveProviderShape(
...
export function signInLabel(
  reauth: RemoteReauth | null,
  copy: SignInCopy = DEFAULT_SIGN_IN_COPY,
): string {
  ...
  const provider =
    reauth?.providerLabel === DEFAULT_SIGN_IN_COPY.identityProvider
      ? copy.identityProvider
      : reauth?.providerLabel;
