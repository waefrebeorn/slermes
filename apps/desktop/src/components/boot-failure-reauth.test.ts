...
function config(
  overrides: Partial<DesktopConnectionConfig> = {},
...
describe("isRemoteConfig", () => {
  it("true for remote/cloud with a URL, regardless of auth mode or connection", () => {
    expect(
      isRemoteConfig(
        config({ remoteAuthMode: "token", remoteOauthConnected: false }),
      ),
    ).toBe(true);
    expect(
      isRemoteConfig(config({ mode: "cloud", remoteOauthConnected: true })),
    ).toBe(true);
  });
  ...
  it("recognizes SSH as remote recovery without treating it as OAuth reauth", () => {
    ...
    expect(isRemoteReauthFailure(ssh, "SSH authentication failed.")).toBe(
      false,
    );
  });
  ...
  it("false for local, incomplete SSH, a remote with no URL, and nullish", () => {
    ...
    expect(
      isRemoteConfig(config({ mode: "ssh" as never, remoteUrl: "" })),
    ).toBe(false);
...
describe("isRemoteReauthFailure", () => {
  ...
  it("false for a connected cloud session", () => {
    expect(
      isRemoteReauthFailure(
        config({ mode: "cloud", remoteOauthConnected: true }),
      ),
    ).toBe(false);
  });

  it("false for a token (non-gated) remote gateway", () => {
    expect(isRemoteReauthFailure(config({ remoteAuthMode: "token" }))).toBe(
      false,
    );
  });
...
describe("isRemoteReauthError", () => {
  it("recognizes auth-shaped boot errors", () => {
    expect(
      isRemoteReauthError("Your remote gateway session has expired."),
    ).toBe(true);
  ...
  it("ignores non-auth boot errors and nullish", () => {
    expect(
      isRemoteReauthError("Hermes background process exited during startup."),
    ).toBe(false);
...
describe("shouldApplyPostBootProgressError", () => {
  it("applies only confirmed reauth after a healthy boot — not transient ticket blips", () => {
    ...
    expect(
      shouldApplyPostBootProgressError("Lost connection to the gateway"),
    ).toBe(false);
...
describe("sshFailureMessage", () => {
  it("localizes SSH failures without changing non-SSH errors", () => {
    ...
    expect(sshFailureMessage(ssh, "SSH authentication failed", copy)).toBe(
      "localized auth",
    );
    expect(sshFailureMessage(ssh, "unexpected failure", copy)).toBe(
      "localized unknown",
    );
    expect(sshFailureMessage(config(), "raw remote error", copy)).toBe(
      "raw remote error",
    );
  });
});
...
describe("deriveProviderShape", () => {
  it("generic copy when there are no providers", () => {
    expect(deriveProviderShape([])).toEqual({
      isPassword: false,
      providerLabel: "your identity provider",
    });
    expect(deriveProviderShape(null)).toEqual({
      isPassword: false,
      providerLabel: "your identity provider",
    });
  });
  ...
  it("falls back to name when displayName is empty", () => {
    expect(
      deriveProviderShape([
        { name: "basic", displayName: "", supportsPassword: true },
      ]).providerLabel,
    ).toBe("basic");
  });
});
