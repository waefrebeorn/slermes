...
describe("BootFailureOverlay", () => {
  ...
  it("drops local-only Repair and Use-local-gateway on a local failure", () => {
    ...
    expect(
      screen.queryByRole("button", { name: /use local gateway/i }),
    ).toBeNull();
  });
  ...
  it("leads with Gateway settings and drops Repair for a remote (token) failure", async () => {
    ...
    try {
      ...
      expect(
        screen.getByRole("button", { name: /gateway settings/i }),
      ).toBeTruthy();
      expect(
        screen.getByRole("button", { name: /use local gateway/i }),
      ).toBeTruthy();
    } finally {
      restore();
    }
  ...
  it("clears and signs in only the failed gateway once", async () => {
    ...
    const restore = stubDesktop(
      ...
      },
      {
        ...
        probeConnectionConfig: vi
          .fn()
          .mockResolvedValue({
            providers: [{ id: "basic", type: "password" }],
          }),
      },
    );
    ...
    try {
      render(<BootFailureOverlay />);
      fireEvent.click(
        await screen.findByRole("button", { name: /sign out & sign in/i }),
      );
  ...
  it("recovers a cloud connection through the portal cascade instead of native OAuth", async () => {
    ...
    const logout = vi.fn().mockResolvedValue({ ok: true, connected: false });
    const nativeLogin = vi
      .fn()
      .mockResolvedValue({ ok: true, connected: false });
    ...
    const cloudAgentSignIn = vi
      .fn()
      .mockResolvedValue({ baseUrl: gatewayUrl, connected: false });
    ...
    const restore = stubDesktop(
      ...
      },
      {
        ...
        probeConnectionConfig: vi
          .fn()
          .mockResolvedValue({ providers: [{ id: "nous", type: "oauth" }] }),
      },
    );
    ...
    try {
      ...
      await waitFor(() =>
        expect(cloudAgentSignIn).toHaveBeenCalledWith(gatewayUrl),
      );
      expect(logout).toHaveBeenCalledWith(gatewayUrl);
  ...
  it("shows the Nous Cloud down recovery when the backend flags isCloudBackendDown", async () => {
    ...
    $desktopBoot.set({
      error:
        "Nous Cloud agent ares-3009.agents.nousresearch.com is down (HTTP 503: server-side fault).",
      fakeMode: false,
    ...
    try {
      ...
      expect(
        screen.getByRole("button", { name: /check portal status/i }),
      ).toBeTruthy();
      expect(
        screen.getByRole("button", { name: /get help on discord/i }),
      ).toBeTruthy();
      ...
      expect(
        screen.getByText(/ares-3009\.agents\.nousresearch\.com/i),
      ).toBeTruthy();
    } finally {
      restore();
    }
