...
import {
  ConnectorLogo,
  type ConnectorLogoSubject,
} from "@/components/ui/connector-logo";
...
export type ConnectorCardState =
  "connected" | "disabled" | "needs_auth" | "not_configured";
...
export function outcomeMeta(
  ...
  if (outcome.status === "connected") {
    ...
    return toolCount > 0
      ? `${copy.stateConnected} · ${copy.toolCount(toolCount)}`
      : copy.stateConnected;
  }
    ...
    return (
      <Tip label={copy.trustVerifiedTip(connector.publisher)}>
        <span className="text-[0.6875rem] text-(--ui-text-tertiary)">
          {copy.trustVerified(connector.publisher)}
        </span>
      </Tip>
    );
  }
...
export function ConnectorCard({
  ...
  return (
    <div
      className={cn(SHELL_CLASS, "flex items-start gap-3")}
    ...
    >
      ...
      <div className="grid min-w-0 flex-1 gap-0.5">
        ...
        {connector.description ? (
          <p className="text-(--ui-text-secondary)">{connector.description}</p>
        ) : null}

        {failed && outcome.detail ? (
          <p className="text-[0.6875rem] text-destructive">{outcome.detail}</p>
        ) : null}
        ...
        {steps.length > 0 && (
          <ol className="mt-1.5 grid gap-1" data-slot="connector-card-steps">
            {steps.map((step, index) => (
              <li
                className="flex gap-1.5 text-[0.6875rem] text-(--ui-text-secondary)"
                key={step}
              >
            ...
            {envFields.map((env) => (
              <label className="grid gap-1" key={env.name}>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    onEnvChange?.(env.name, event.currentTarget.value)
                  }
                  type="password"
