...
const move = () =>
  act(
    () =>
      void window.dispatchEvent(new MouseEvent("mousemove", { bubbles: true })),
  );
const scroll = () =>
  act(
    () => void window.dispatchEvent(new WheelEvent("wheel", { bubbles: true })),
  );
...
describe("usePointerQuiet", () => {
  ...
  it("drops its listeners on unmount", () => {
    ...
    expect(remove.mock.calls.map(([type]) => type)).toEqual(
      expect.arrayContaining(["mousemove", "wheel"]),
    );
  });
});
