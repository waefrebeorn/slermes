...
export const PANE_TAB_STRIP_LINE_LEFT =
  "shadow-[inset_1px_0_0_var(--ui-stroke-tertiary)]";
export const PANE_TAB_STRIP_LINE_RIGHT =
...
const TAB_HORIZONTAL =
  "h-full min-w-0 max-w-48 not-first:border-l not-first:border-l-(--ui-stroke-quaternary)";
...
const TAB_ACTIVE_UNDERLINE =
  "shadow-[inset_0_-2px_0_var(--pane-tab-active-accent,var(--theme-primary))]";
 ...
 */
export const PaneTab = React.forwardRef<HTMLDivElement, PaneTabProps>(
  function PaneTab(
    {
      active = false,
      dirty = false,
...
export const PaneTabLabel = React.forwardRef<HTMLElement, PaneTabLabelProps>(
  function PaneTabLabel({ as = "span", className, children, ...props }, ref) {
 ...
 */
export const PaneTabStrip = React.forwardRef<HTMLDivElement, PaneTabStripProps>(
 ...
 */
export function PaneStripGlyph({
  active,
  disabled,
  icon,
  label,
  onSelect,
}: Omit<PaneStripTool, "id">) {
  return (
    <Tip label={label}>
      <Button
        aria-label={label}
