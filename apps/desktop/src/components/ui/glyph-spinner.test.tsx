...
describe("GlyphSpinner", () => {
  ...
  it("renders every frame in source order as the scroll strip", () => {
    ...
    const frames = [...strip().querySelectorAll(".glyph-spinner__frame")].map(
  ...
  it("feeds steps() and the duration from the spinner data, so cadence is unchanged", () => {
    ...
    expect(style.getPropertyValue("--glyph-spinner-frames")).toBe(
      String(BRAILLE.frames.length),
    );
    expect(style.getPropertyValue("--glyph-spinner-duration")).toBe(
      `${BRAILLE.frames.length * BRAILLE.interval}ms`,
    );
  });
  ...
  it("keeps per-variant cadence distinct", () => {
    ...
    expect(found.querySelectorAll(".glyph-spinner__frame")).toHaveLength(
      breathe.frames.length,
    );
  ...
  it("hides the decorative frames from assistive tech", () => {
    ...
    expect(
      status.querySelector(".glyph-spinner")?.getAttribute("aria-hidden"),
    ).toBe("true");
  });
