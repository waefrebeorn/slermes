  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          {active?.icon && (
            <TabDropdownIcon icon={active.icon} indent={active.indent} />
          )}
          <span className="min-w-0 truncate">{active?.label}</span>
          {active?.meta !== undefined && (
            <TextTabMeta>{tabMetaContent(active.meta)}</TextTabMeta>
          )}
          <Codicon
      ...
      >
        {items.map((item, index) => (
          <Fragment key={item.id}>
            ...
            >
              {item.icon && (
                <TabDropdownIcon icon={item.icon} indent={item.indent} />
              )}
              <span className="min-w-0 flex-1 truncate">{item.label}</span>
              {item.meta !== undefined && (
                <span className="text-xs text-muted-foreground">
                  {tabMetaContent(item.meta)}
                </span>
              )}
            </DropdownMenuItem>
          </Fragment>
...
export function ResponsiveTabs({
  ...
  return (
    <>
      <div
        className={cn(
          "hidden min-w-0 flex-wrap items-center gap-x-2 gap-y-1 md:flex",
          wideClassName,
        )}
      >
        {tabs.map((tab) => (
          ...
          >
            {tab.label}
            {tab.meta !== undefined && (
              <TextTabMeta>{tabMetaContent(tab.meta)}</TextTabMeta>
            )}
          </TextTab>
        ))}
      </div>
