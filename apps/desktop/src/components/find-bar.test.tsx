...
describe("findBarKeyAction", () => {
  ...
  it("maps Cmd+Shift+G / Ctrl+Shift+G to previous", () => {
    ...
    expect(findBarKeyAction({ key: "G", metaKey: true, shiftKey: true })).toBe(
      "previous",
    );
    expect(findBarKeyAction({ key: "g", ctrlKey: true, shiftKey: true })).toBe(
      "previous",
    );
  });
  ...
  it("steps with Enter / Shift+Enter only while focus is in the input", () => {
    ...
    expect(
      findBarKeyAction({ key: "Enter", shiftKey: true }, { inInput: true }),
    ).toBe("previous");
  ...
  it("ignores a bare g so typing never triggers a step", () => {
    ...
    expect(
      findBarKeyAction({ key: "G", shiftKey: true }, { inInput: true }),
    ).toBeNull();
  });

  it("falls through when Alt is held so ⌥⌘G is not swallowed", () => {
    expect(
      findBarKeyAction({ key: "g", metaKey: true, altKey: true }),
    ).toBeNull();
...
describe("FindBar", () => {
  ...
  it("excludes the semantic searchbox only while Chromium indexes the page", async () => {
    ...
    bridge.findInPage.mockReturnValueOnce(
      new Promise((resolve) => (resolveFind = () => resolve({ count: 0 }))),
    );
  ...
  it("does not fire a pending search after the bar closes", async () => {
    ...
    try {
      ...
      fireEvent.change(
        screen.getByRole("searchbox", { name: /find in page/i }),
        {
          target: { value: "needle" },
        },
      );
...
describe("FindBar files pane positioning", () => {
  ...
  afterEach(() => {
    ...
    for (const aside of document.querySelectorAll(
