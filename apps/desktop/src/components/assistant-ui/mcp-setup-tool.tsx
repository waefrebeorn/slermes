...
function readSetupArgs(args: unknown): SetupArgs {
  ...
  return {
    action:
      rawAction === "enable" || rawAction === "authorize"
        ? rawAction
        : "install",
...
function McpSetupSettled({ args, result }: ToolCallMessagePartProps) {
  ...
  const ok =
    status === "installed" || status === "enabled" || status === "authorized";
  const neutral = status === "declined" || status === "unanswered";
  const toolCount = Array.isArray(fromResult.tools)
    ? fromResult.tools.length
    : 0;
  const brand = brandFor(server);

  return (
    <div
      className={cn(SHELL_CLASS, "my-1.5 grid gap-1.5")}
      ...
      <SetupLine
        trailing={
          ok ? (
            <CheckCircle2
              aria-hidden
              className={cn(ICON_CLASS, "text-emerald-400")}
            />
          ) : neutral && brand ? (
            <brand.Icon
              aria-hidden
              className="mt-px size-4 shrink-0 opacity-60"
            ...
            <AlertCircle
              aria-hidden
              className={cn(ICON_CLASS, "text-destructive")}
            />
          )
        }
      >
        <span
          className={cn(
            "font-medium",
            neutral && "italic text-(--ui-text-tertiary)",
          )}
        >
          {line}
        </span>
        {ok && toolCount > 0 && (
          <span className="ml-2 text-(--ui-text-tertiary)">
            {copy.toolCount(toolCount)}
          </span>
        )}
        {!ok && !neutral && fromResult.detail ? (
          <p className="mt-0.5 text-(--ui-text-secondary)">
            {fromResult.detail}
          </p>
        ) : null}
      </SetupLine>
...
function McpSetupPending({ args }: ToolCallMessagePartProps) {
  ...
  const $request = useMemo(
    () => sessionMcpSetupRequest(sessionId),
    [sessionId],
  );
  ...
  const [entry, setEntry] = useState<McpCatalogEntry | null | undefined>(
    undefined,
  );
  ...
  const respond = useCallback(
    async (outcome: McpSetupOutcome) => {
      ...
      if (
        !request ||
        sessionMcpSetupRequest(request.sessionId).get()?.requestId !==
          request.requestId
      ) {
        return;
      }
    ...
    try {
      ...
      if (action === "authorize") {
        ...
        await respond({
          server,
          status: "authorized",
          tools: (flow.tools ?? []).map((tool) => tool.name),
        });
      ...
      if (resolved === undefined) {
        ...
        resolved =
          catalog.entries.find((candidate) => candidate.name === server) ??
          null;
        setEntry(resolved);
      }
      ...
      if (!resolved) {
        ...
        if (!known) {
          await respond({
            detail: copy.notInCatalog(server),
            server,
            status: "error",
          });
  ...
  if (!ready) {
    return (
      <div
        className={cn(SHELL_CLASS, "my-1.5 flex items-center gap-2")}
        data-slot="mcp-setup-inline"
      >
        <Loader2
          aria-hidden
          className="size-4 animate-spin text-(--ui-text-tertiary)"
        />
  ...
  return (
    ...
    >
      <SetupLine trailing={trailingIcon}>
        ...
        {reason ? (
          <p className="mt-0.5 text-(--ui-text-secondary)">{reason}</p>
        ) : null}
        {sourceLine && (
      ...
      <div className="flex items-center gap-2.5">
        <div className="inline-flex h-6 items-stretch overflow-hidden rounded-md border border-primary/25 bg-primary/10 text-primary">
          ...
          >
            ...
            {!working && (
              <span className="text-[0.625rem] text-primary/60">
                {isMac ? "⌘⏎" : "Ctrl⏎"}
              </span>
            )}
          </Button>
