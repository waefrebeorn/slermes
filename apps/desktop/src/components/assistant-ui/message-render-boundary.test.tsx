...
const lookupErrors = [
  ...
  [
    "tapClientLookup",
    new Error("tapClientLookup: Index 2 out of bounds (length: 2)"),
  ],
  [
    "tapClientResource",
    new Error("tapClientResource: Index 2 out of bounds (length: 2)"),
  ],
] as const;
...
describe("MessageRenderBoundary", () => {
  ...
  it.each(lookupErrors)(
    "swallows the transient %s out-of-bounds store race",
    (_label, error) => {
      const spy = vi
        .spyOn(console, "error")
