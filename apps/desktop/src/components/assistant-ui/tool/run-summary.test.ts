...
const searched = (query: string) =>
  tool("search_files", { query }, { hits: [] });
const ran = (command: string) =>
  tool("terminal", { command }, { exit_code: 0 });
...
describe("summarizeToolRun", () => {
  it("names a lone target and counts the rest", () => {
    expect(
      settled([searched("toolRuns"), read("a.ts"), read("b.ts"), read("c.ts")]),
    ).toBe("Explored 4 files");
  });

  it("orders clauses explore then run regardless of call order", () => {
    expect(
      settled([ran("ls"), read("a.ts"), read("b.ts"), ran("pwd"), ran("id")]),
    ).toBe("Explored 2 files, ran 3 commands");
  });
  ...
  it("counts commands rather than naming them once they have run", () => {
    ...
    ).toBe("Explored status.ts, ran 5 commands");
  });
  ...
  it("puts the running category in the present tense and leaves the rest past", () => {
    ...
    ).toBe("Exploring 2 files, ran 2 commands");
  });

  it("names the command that is still running", () => {
    expect(
      running([tool("terminal", { command: "npm run typecheck" })]),
    ).toMatch(/^Running /);
  });
  ...
  it("stays in the present tense between two sequential calls", () => {
    expect(running([read("a.ts"), ran("x"), ran("y")])).toBe(
      "Explored a.ts, running 2 commands",
    );
  });
  ...
  it("reads a run the turn left unresolved as finished", () => {
    expect(
      settled([read("a.ts"), tool("search_files", { query: "toolRuns" })]),
