...
import {
  PrettyLink,
  LinkifiedText as SharedLinkifiedText,
  urlSlugTitleLabel,
...
const TOOL_EXPANDED_SHELL_CLASS =
  "rounded-[0.3125rem] border border-(--ui-stroke-tertiary)";
      ...
      {hits.map((hit, index) => {
        ...
        return (
          <li className="grid min-w-0 gap-0.5" key={key}>
            ...
            {hit.snippet && (
              <p className={cn(TOOL_HEADER_SUBTITLE_CLASS, "m-0 line-clamp-3")}>
                {hit.snippet}
              </p>
            )}
          </li>
  ...
  return (
    <FadeText
      className={cn(
        ...
        legendary &&
          !isPending &&
          "tool-memory-legendary-title text-transparent",
      )}
    >
...
function ToolEntry({ part }: ToolEntryProps) {
  ...
  const inlineDiff =
    stripInlineDiffChrome(sideDiff) || inlineDiffFromResult(result);
  ...
  const enterRef = useEnterAnimation(
    messageRunning && !embedded,
    `tool-entry:${disclosureId}`,
  );
  ...
  const detailSections = useMemo(() => {
    ...
    const summaryDuplicatesSubtitle =
      summary && summary.toLowerCase() === subtitleNorm;
  ...
  const detailMatchesSubtitle = useMemo(
    () => looksRedundant(view.subtitle, view.detail),
    [view.subtitle, view.detail],
  );
  const detailMatchesTitle = useMemo(
  ...
  const showDetail =
    ...
    (Boolean(view.stdout || view.stderr) ||
      (view.status === "error" &&
        Boolean(detailSections.summary || detailSections.body)) ||
      (view.status !== "error" &&
        Boolean(view.detail) &&
        !detailMatchesTitle &&
  ...
  const searchResultsLabel =
    part.toolName === "web_search" ? "Search results" : view.detailLabel;
  ...
  const copyAction = useMemo(
    () => toolCopyPayload(stablePart, view),
    [stablePart, view],
  );
  ...
  const diffStats = useMemo(
    () =>
      isFileEdit && view.inlineDiff
        ? countDiffLineStats(view.inlineDiff)
        : null,
    [isFileEdit, view.inlineDiff],
  );
  ...
  return (
    ...
    >
      <div
        className={cn(
          open && "border-b border-(--ui-stroke-tertiary) px-2 py-1.5",
        )}
      >
        <DisclosureRow
        ...
        >
          ...
          >
            ...
            {!isPending && view.countLabel && (
              <span className={cn(SCAFFOLD_META_CLASS, memoryMetaClass)}>
                {view.countLabel}
              </span>
            )}
        ...
        <div className="relative grid w-full min-w-0 max-w-full gap-1.5 overflow-hidden p-1.5">
          ...
          {showDetail &&
            ...
            (view.status === "error" ? (
              ...
              <div className="max-w-full text-xs leading-relaxed text-(--ui-text-secondary)">
                {view.detailLabel && (
                  <p className={TOOL_SECTION_LABEL_CLASS}>{view.detailLabel}</p>
                )}
                {view.stdout && (
                  <div className="space-y-0.5">
                    {view.stderr && (
                      <p className={TOOL_SECTION_LABEL_CLASS}>stdout</p>
                    )}
                    <pre
                      className={cn(
              ...
              <div className="max-w-full text-xs leading-relaxed text-(--ui-text-secondary)">
                ...
                {renderDetailAsCode ? (
                  ...
                  >
                    {view.rendersAnsi ? (
                      <AnsiText text={clampForDisplay(view.detail)} />
                    ) : (
          ...
          {toolViewMode === "technical" && (
            <ToolPayloadDisclosure args={part.args} result={part.result} />
          )}
        </div>
      )}
    </div>
    ...
    <div className="flex min-w-0 items-center gap-2 rounded-[0.25rem] border border-(--ui-stroke-tertiary) bg-(--ui-bg-quinary) px-2 py-1.5 font-mono text-[0.7rem] leading-relaxed">
      {command && (
        <code className="min-w-0 flex-1 whitespace-pre-wrap wrap-anywhere text-(--ui-text-secondary)">
          <span
            aria-hidden
            className="select-none text-(--ui-accent-secondary)"
          >
            ${" "}
          </span>
          {command}
...
export type RunItem =
  { end: number; kind: "run"; start: number } | { index: number; kind: "card" };
    ...
    if (cache.current?.signature !== signature) {
      cache.current = {
        ...
        value: {
          ...
          count: tools.length,
          entryIds: tools.map((tool) =>
            toolEntryDisclosureId(state.message.id, tool),
          ),
          ...
          pendingApprovalTool: tools.some(
            (tool) =>
              tool.result === undefined && APPROVAL_TOOLS.has(tool.toolName),
          ),
...
> = ({ children, endIndex, startIndex }) => {
  ...
  const approval = useStore(
    useMemo(() => sessionApprovalRequest(sessionId), [sessionId]),
  );
  const disclosureId = `tool-run:${key}`;
  const persistedOpen = useStore($toolDisclosureOpen(disclosureId));
  const rowOpen = useStore(
    useMemo(() => $anyToolDisclosureOpen(entryIds), [entryIds]),
  );
  ...
  return (
    ...
    >
      ...
      {expanded && (
        <div className="grid min-w-0 max-w-full gap-(--tool-row-gap)">
          {children}
        </div>
      )}
    </div>
...
> = ({ children, endIndex, startIndex }) => {
  ...
  const items = useMemo(
    () => splitRunItems(toolNameKey.split("\u0000")),
    [toolNameKey],
  );
