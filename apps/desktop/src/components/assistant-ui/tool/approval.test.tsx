...
describe("PendingToolApproval", () => {
  ...
  it("renders nothing for tools that never raise approval", () => {
    setRequest();
    const { container } = render(
      <PendingToolApproval part={part("read_file")} />,
    );
  ...
  it.each(["patch", "write_file"])(
    "renders inline approval controls for protected %s writes",
    (toolName) => {
  ...
  it('offers "Always allow" in the options menu by default', async () => {
    ...
    expect(
      await screen.findByRole("menuitem", { name: /Always allow/ }),
    ).toBeTruthy();
    expect(
      screen.getByRole("menuitem", { name: /Allow this session/ }),
    ).toBeTruthy();
  });
  ...
  it("renders a floating fallback when no pending tool row is mounted", () => {
    ...
    expect(
      within(fallback as HTMLElement).getByRole("button", { name: /Run/ }),
    ).toBeTruthy();
    expect(
      within(fallback as HTMLElement).getByRole("button", { name: /Reject/ }),
    ).toBeTruthy();
  });
  ...
  it("hides the floating fallback once the inline approval bar is mounted", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        container.querySelector('[data-slot="tool-approval-fallback"]'),
      ).toBeNull();
    });
  });
});
