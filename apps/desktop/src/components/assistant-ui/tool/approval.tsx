...
export const PendingToolApproval: FC<{ part: ToolPart }> = ({ part }) => {
  ...
  const $request = useMemo(
    () => sessionApprovalRequest(sessionId),
    [sessionId],
  );
...
const InlineApprovalBar: FC<{ request: ApprovalRequest }> = ({ request }) => {
  useEffect(
    () => registerApprovalInlineAnchor(request.sessionId),
    [request.sessionId],
  );
...
export const PendingApprovalFallback: FC = () => {
  ...
  const $inlineVisible = useMemo(
    () => sessionApprovalInlineVisible(sessionId),
    [sessionId],
  );
  const request = useStore($request);
    ...
    >
      <div className="pointer-events-auto rounded-xl border border-primary/30 bg-(--ui-chat-surface-background) px-3 py-2 shadow-lg backdrop-blur-xl [-webkit-backdrop-filter:blur(1rem)]">
        <div className="flex min-w-0 items-center gap-2 text-sm text-primary">
          ...
          {request.description && (
            <span className="min-w-0 truncate text-(--ui-text-tertiary)">
              {request.description}
            </span>
          )}
        </div>
...
const ApprovalBar: FC<{
  ...
  const choices =
    request.choices ?? (request.smartDenied ? ["once", "deny"] : undefined);
  ...
  const respond = useCallback(
    async (choice: ApprovalChoice) => {
      ...
      try {
        ...
        void replayPendingApproval(gateway, request.sessionId).catch(
          () => undefined,
        );
      } catch (error) {
  ...
  return (
    <div
      ...
      data-slot={
        surface === "inline" ? "tool-approval-inline" : "tool-approval-actions"
      }
    >
