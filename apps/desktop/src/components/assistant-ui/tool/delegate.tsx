...
function statusGlyph(status: DelegateRowStatus, label: string): ReactNode {
  ...
  if (status === "failed" || status === "interrupted") {
    return (
      <AlertCircle aria-label={label} className="size-3.5 text-destructive" />
    );
  }
  ...
  if (status === "dispatched") {
    ...
    return (
      <span
        aria-hidden
        className="size-1.5 rounded-full bg-(--ui-text-tertiary)"
      />
    );
  }
...
function DelegateRowView({ row }: { row: DelegateRow }) {
  ...
  const open = sessionId
    ? () => void openSessionInNewWindow(sessionId, { watch: true })
    : undefined;

  return (
    <div
      className="grid min-w-0 max-w-full gap-0.5"
      data-conversation-scaffold=""
    >
      <div className="flex min-w-0 max-w-full items-center gap-1.5">
        <span className={SCAFFOLD_GLYPH_CLASS}>
          {statusGlyph(row.status, statusLabel)}
        </span>
        <button
          className={cn(
            SCAFFOLD_LABEL_CLASS,
        ...
        {meta.length > 0 && (
          <span className={SCAFFOLD_META_CLASS}>{meta.join(" · ")}</span>
        )}
        {live && (
          <ActivityTimerText
            className={cn(SCAFFOLD_META_CLASS, "ml-auto")}
            seconds={elapsed}
          />
        )}
      </div>
          ...
          <ToolRunTicker>
            {activity.map((text, index) => (
              <FadeText
                className={cn(
                  SCAFFOLD_LABEL_CLASS,
                  "text-(--conversation-scaffold-meta)",
                  live && "shimmer",
                )}
                key={`${row.id}:${index}`}
              >
 ...
 * them nothing.
 */
export const DelegateTool: FC<
  Pick<ToolPart, "args" | "result" | "toolCallId">
> = ({ args, result, toolCallId }) => {
  ...
  const rows = useMemo(
    () =>
      mergeDelegateRows(
        delegateRowsFromCall(args, result, toolCallId),
        live,
        toolCallId,
      ),
  ...
  return (
    <div
      className="grid min-w-0 gap-(--tool-row-gap)"
      data-delegate-card=""
