...
function readFileLineLabel(
  args: Record<string, unknown>,
  result: Record<string, unknown>,
): string {
  if (
    numericField(args, "offset") === undefined &&
    numericField(args, "limit") === undefined
  ) {
    return "";
  }
...
function titleForTool(name: string): string {
  ...
  return (
    normalized.split("_").filter(Boolean).map(capitalize).join(" ") || name
  );
}
...
function toolMeta(name: string): ToolMeta {
  if (isToolTitleKey(name)) {
    ...
    return {
      ...
      pendingAction: translateNow(
        `assistant.tool.titles.${name}.pendingAction`,
      ),
      icon: meta.icon,
      tone: meta.tone,
    };
  ...
  if (prefix) {
    const prefixLabel = translateNow(
      `assistant.tool.prefixes.${prefix.labelKey}`,
    );

    return {
      done: translateNow(
        "assistant.tool.titleTemplates.prefixedDone",
        prefixLabel,
        action,
      ),
      pending: translateNow(
        "assistant.tool.titleTemplates.runningPrefixedTool",
        prefixLabel,
        action,
      ),
...
const COUNT_NOUN_BY_FIELD: Partial<
  Record<(typeof COUNT_FIELD_KEYS)[number], string>
> = {
  count: "",
  total: "",
  result_count: "result",
...
function normalizeMetricForTool(
  toolName: string,
  metric: CountMetric,
): CountMetric {
...
function countFromText(
  ...
  const unitMatch =
    text.match(
      /\b(\d+)\s+(results?|items?|files?|matches?|documents?|sources?|searches?|steps?|rows?)\b/i,
    ) ||
    text.match(
      /\b(?:did|found|returned|listed|searched|matched|updated|created|deleted|processed)\s+(\d+)\b/i,
    );
...
function toolResultCount(
  ...
  const summaryText =
    firstStringField(resultRecord, ["summary", "message", "detail"]) ||
...
function summarizeBrowserSnapshot(snapshot: string): string {
  ...
  const labels = Array.from(
    snapshot.matchAll(/(?:button|link|combobox|textbox)\s+"([^"]+)"/g),
  )
    .map((m) => m[1].trim())
    .filter(Boolean)
    .slice(0, 4);
  ...
  return list
    .map((item) => {
      ...
      return {
        ...
        snippet: cleanVisibleText(
          firstStringField(r, ["snippet", "description", "body"]),
        ),
      };
    })
...
function toolErrorText(
  ...
  if (
    ...
    /\b(error|failed|failure)\b/i.test(result.status)
  ) {
    return (
      firstStringField(result, ["message", "reason", "detail"]) ||
      `Tool returned status "${result.status}".`
    );
  }
...
function toolPreviewTarget(
  ...
  if (
    ...
    const explicit =
      firstStringField(args, ["url", "search_term", "query"]) ||
  ...
  if (isFileEditTool(toolName)) {
    return htmlPathFromInlineDiff(
      firstStringField(result, ["inline_diff", "diff"]),
    );
  }
...
function cronjobSubtitle(
  ...
  const name =
    firstStringField(resultRecord, ["name"]) ||
    firstStringField(argsRecord, ["name", "job_id"]);
...
function toolSubtitle(
  ...
  if (toolName === "browser_click") {
    const clicked =
      firstStringField(resultRecord, ["clicked"]) ||
      firstStringField(argsRecord, ["ref", "target"]);
  ...
  if (toolName === "browser_fill" || toolName === "browser_type") {
    const field = firstStringField(argsRecord, [
      "label",
      "field",
      "ref",
      "target",
    ...
    return (
      [
        field && `Field: ${field}`,
        value && `Value: ${compactPreview(value, 42)}`,
      ]
        .filter(Boolean)
        .join(" · ") || "Filled page input"
    );
  }

  if (toolName === "web_search") {
    const query =
      firstStringField(argsRecord, ["search_term", "query"]) ||
      contextValue(argsRecord);

    return query ? `Query: ${query}` : "Queried web sources";
  }

  if (toolName === "terminal" || toolName === "execute_code") {
    const output = firstStringField(resultRecord, [
      "output",
      "stdout",
      "stderr",
    ]);
    ...
    const command =
      firstStringField(argsRecord, ["context", "preview", "command", "code"]) ||
      contextValue(argsRecord);
...
function toolDetailText(
  ...
  if (part.toolName === "web_extract") {
    const direct = firstStringField(resultRecord, [
      "content",
      "text",
      "markdown",
      "body",
  ...
  if (part.toolName === "read_file" && part.result !== undefined) {
    const content = firstStringField(resultRecord, [
      "content",
      "text",
      "data",
      "body",
  ...
  if (part.toolName === "web_search") {
    ...
    return detail
      .replace(
        /^\s*-\s*Duration\s+S\s*:\s*[-+]?[\d.]+(?:e[-+]?\d+)?\s*$/gim,
        `- Duration: ${duration}`,
      )
...
export function toolCopyPayload(
  ...
  if (part.toolName === "terminal" || part.toolName === "execute_code") {
    ...
    const command =
      firstStringField(args, ["command", "code"]) || contextValue(args);
  ...
  if (part.toolName === "web_extract") {
    ...
    const url =
      firstStringField(args, ["url", "target"]) || findFirstUrl(args, result);
  ...
  if (part.toolName === "web_search") {
    if (view.searchHits?.length) {
      const text = view.searchHits
        .map((hit) =>
          [hit.title, hit.url, hit.snippet].filter(Boolean).join("\n"),
        )
        .join("\n\n");
    ...
    const query =
      firstStringField(args, ["search_term", "query"]) || contextValue(args);
...
function dynamicTitle(
  ...
  const verb = (gerund: string, past: string) =>
    part.result === undefined ? gerund : past;
  ...
  if (part.toolName === "web_search") {
    ...
    return query
      ? titledAction(
          action,
          translateNow(
            "assistant.tool.titleTemplates.actionQuoted",
            action,
...
export function buildToolView(part: ToolPart, inlineDiff: string): ToolView {
  ...
  const titleParts = dynamicTitle(
    ...
    argsRecord,
    resultRecord,
    titlePartsFromAction(
      baseTitle,
      part.result === undefined ? meta.pendingAction : undefined,
    ),
  );
  ...
  const subtitle =
    titleEnriched && !error && !keepSubtitleWithTitle ? "" : baseSubtitle;
  ...
  const detail = error
    ? [error, detailBody]
        ...
        .filter(
          (value, index, list) =>
            list.findIndex((entry) => entry.trim() === value.trim()) === index,
        )
        .join("\n\n")
    : detailBody;

  const searchHits =
    part.toolName === "web_search" && status !== "error"
      ? extractSearchResults(part.result)
      : undefined;
  ...
  const resultCount =
    status === "error" ? null : toolResultCount(part, argsRecord, resultRecord);
  ...
  const stderrRaw = rendersAnsi
    ? firstStringField(resultRecord, ["stderr"])
    : "";
  ...
  const hasSplitStreams =
    rendersAnsi && (Boolean(stdout) || Boolean(stderrRaw));
  const terminalCommand =
    part.toolName === "terminal" ? shellCommand(argsRecord) : undefined;
  const terminalExitCode =
    part.toolName === "terminal"
      ? numericField(resultRecord, "exit_code")
      : undefined;
