    ...
    expect(
      deriveTimelineEntries([
        { id: "u1", role: "user", text: "   " },
        {
          id: "u2",
          role: "user",
          text: "[IMPORTANT: Background process 123 finished]",
        },
...
describe("sameTimelineEntries", () => {
  ...
  it("treats an identical derivation as unchanged, so the memo can reuse it", () => {
    expect(sameTimelineEntries(rail, [...rail.map((e) => ({ ...e }))])).toBe(
      true,
    );
  });
  ...
  it("detects a changed preview, id, or length", () => {
    expect(
      sameTimelineEntries(rail, [rail[0], { id: "u2", preview: "edited" }]),
    ).toBe(false);
    expect(
      sameTimelineEntries(rail, [rail[0], { id: "u9", preview: "second" }]),
    ).toBe(false);
  ...
  it("is stable when a filtered-out prompt joins the transcript", () => {
    const withNoise = deriveTimelineEntries([
      ...
      { id: "u2", role: "user", text: "second" },
      {
        id: "u3",
        role: "user",
        text: "[IMPORTANT: Background process 7 finished]",
      },
    ]);
