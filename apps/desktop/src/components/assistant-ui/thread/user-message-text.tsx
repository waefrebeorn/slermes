...
function inlineCodeOutsideDirectives(text: string): RegExpMatchArray[] {
  ...
  return Array.from(text.matchAll(INLINE_CODE_RE)).filter((match) => {
    ...
    return !directives.some(
      (directive) => start < directive.end && end > directive.start,
    );
  });
}
...
export const UserMessageText: FC<UserMessageTextProps> = ({
  className,
  text,
}) => {
...
const InlineSegmentView: FC<{ text: string }> = ({ text }) => {
  ...
  return (
    ...
    <span
      className="wrap-anywhere block whitespace-pre-line"
