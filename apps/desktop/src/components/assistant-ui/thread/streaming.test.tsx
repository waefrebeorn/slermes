...
function assistantReasoningMessage(
  text: string,
  running = false,
): ThreadMessage {
  return {
...
function assistantTodoMessage(
  ...
  const suffix =
    todos.map((todo) => `${todo.id}:${todo.status}`).join("|") || "empty";
    ...
    content: [
      {
        ...
        argsText: JSON.stringify({
          command: "npm run check --workspace=apps/desktop",
        }),
...
function StreamingHarness({
  onControls,
}: { onControls?: (controls: StreamingControls) => void } = {}) {
    ...
    if (onControls) {
      onControls({
        emitSecond: () => {
          setMessages([
            userMessage(),
            assistantMessage("first chunk second chunk"),
          ]);
        },
        complete: () => {
          setMessages([
...
function assistantInterimMessage(
  text: string,
...
function ReasoningHarness() {
  const runtime = useExternalStoreRuntime<ThreadMessage>({
    messages: [
      assistantReasoningMessage(" The user is asking what this file is."),
    ],
    isRunning: false,
    onNew: async () => {},
  });
...
function renderSettlingReasoning() {
  ...
  function SettlingReasoningHarness() {
    ...
    const runtime = useExternalStoreRuntime<ThreadMessage>({
      messages: [
        assistantReasoningMessage("The user asked a question.", running),
      ],
      isRunning: running,
      onNew: async () => {},
    });
...
function GroupedReasoningHarness() {
  const runtime = useExternalStoreRuntime<ThreadMessage>({
    messages: [
      assistantMultiReasoningMessage([" First thought.", " Second thought."]),
    ],
    isRunning: false,
    onNew: async () => {},
  });
...
describe("assistant-ui streaming renderer", () => {
  ...
  it("renders assistant text incrementally before completion", async () => {
    ...
    expect(
      screen.getByRole("status", { name: "Hermes is loading a response" }),
    ).toBeTruthy();
    ...
    expect(
      screen.queryByRole("status", { name: "Hermes is loading a response" }),
    ).toBeNull();
  ...
  it("does not render composer clearance for intro-only threads", () => {
    ...
    expect(
      container.querySelector('[data-slot="aui_composer-clearance"]'),
    ).toBeNull();
  });
  ...
  it("suppresses the action footer on sealed interim messages, keeping it on the final reply", () => {
    ...
    const finalRoot = [
      ...container.querySelectorAll('[data-slot="aui_assistant-message-root"]'),
    ].find((root) => root.textContent?.includes("All done — patch applied."));

    expect(
      finalRoot?.querySelector('[data-slot="aui_msg-actions"]'),
    ).toBeTruthy();
  });
  ...
  it("puts the turn duration on the action bar row instead of a line of its own", () => {
    ...
    const { container } = render(
      <TranscriptHarness messages={[userMessage(), settled]} />,
    );
  ...
  it("renders an incomplete streaming reasoning fenced code block as a code card", async () => {
    ...
    await waitFor(() => {
      expect(
        container.querySelector('[data-slot="aui_reasoning-text"]')
          ?.textContent,
  ...
  it("keeps the height-capped thinking preview scrollable after the turn settles", async () => {
    ...
    const live =
      container.querySelector('[data-slot="aui_thinking-body"]')?.className ??
      "";
    ...
    await waitFor(() => {
      expect(
        within(container).getByRole("button", { name: /thought/i }),
      ).toBeTruthy();
    });
  ...
  it("keeps streaming reasoning collapsed by default when the preference is enabled", () => {
    ...
    const { container } = render(<RunningReasoningHarness />);
    const thinkingToggle = within(container).getByRole("button", {
      name: /thinking/i,
    });
    ...
    expect(
      container.querySelector('[data-slot="aui_reasoning-text"]')?.textContent,
    ).toContain("const answer = 42");
  });
  ...
  it("renders reasoning text without a leading token space", () => {
    ...
    expect(
      container.querySelector('[data-slot="aui_reasoning-text"]')?.textContent,
    ).toBe("The user is asking what this file is.");
  });
  ...
  it("does not reopen an earlier completed thinking group when a later group is running", () => {
    const { container } = render(
      <RunningMessageHarness message={assistantSeparatedReasoningMessage()} />,
    );
    ...
    expect(
      disclosures[0].querySelector("button")?.getAttribute("aria-expanded"),
    ).toBe("false");
    expect(
      disclosures[1].querySelector("button")?.getAttribute("aria-expanded"),
    ).toBe("true");
  ...
  it("does not render an inline todo panel — todos live in the composer status stack", () => {
    ...
    expect(
      container.querySelector('[data-slot="aui_todo-hoisted"]'),
    ).toBeNull();
  });

  it("renders completed image generation results in the tool slot", async () => {
    const { container } = render(
      <MessageHarness message={assistantImageMessage()} />,
    );
    ...
    expect(
      container.querySelector('[data-slot="aui_generated-image"]'),
    ).toBeTruthy();
    expect(
      screen.queryByRole("status", { name: /rendering image/i }),
    ).toBeNull();
  });
  ...
  it("shows the command prompt and exit code for terminal calls", async () => {
    const { container } = render(
      <MessageHarness message={assistantTerminalMessage()} />,
    );
