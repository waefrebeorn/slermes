...
const timerText = (value: string) =>
  screen.getAllByText((_, node) => node?.textContent === value);
...
describe("the turn timer covers the gaps, not just the streaming", () => {
  ...
  it("times a settled tail bubble while the session is still working", () => {
    ...
    const { container } = render(
      <Harness
        messages={[
          ...
          assistant("a1", [{ type: "text", text: "On it." }], false),
        ]}
      />,
    );
    ...
    expect(
      container.querySelector('[data-slot="aui_turn-activity"]'),
    ).not.toBeNull();
  ...
  it("times the gap between a finished tool call and the next thing", () => {
    const { container } = render(
      <Harness
        messages={[
          userMessage("u1", "read it"),
          assistant("a1", [toolCall("read_file", true)], true),
        ]}
      />,
    );
    ...
    const { container } = render(
      <Harness
        messages={[
          ...
          assistant("a1", [toolCall("terminal", false)], true),
        ]}
      />,
    );
  ...
  it("stops when the session stops working", () => {
    ...
    const { container } = render(
      <Harness
        messages={[
          userMessage("u1", "do the thing"),
          assistant("a1", [{ type: "text", text: "Done." }], false),
        ]}
      />,
    );
