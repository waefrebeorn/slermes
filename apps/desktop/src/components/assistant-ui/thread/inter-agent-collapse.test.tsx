...
describe("inter-agent collapse gate", () => {
  ...
  it("leaves an ordinary reply expanded", async () => {
    render(
      <Harness
        messages={[
          user("u1", "ordinary question"),
          assistant("a1", "ordinary answer", false),
        ]}
      />,
    );
  ...
  it("clears the streaming marker once the turn settles", async () => {
    const { container } = render(
      <Harness messages={[user("u1", "q"), assistant("a1", "done", false)]} />,
    );
    ...
    expect(
      container.querySelector(
        '[data-slot="aui_assistant-message-root"] [data-slot="aui_message-streaming-marker"]',
      ),
    ).toBeTruthy();
  });
});
