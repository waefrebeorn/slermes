...
describe("isTapbackDoubleClick", () => {
  ...
  it("leaves double-click alone where it already means something", () => {
    ...
    expect(
      isTapbackDoubleClick({ detail: 2, target: document.createElement("a") }),
    ).toBe(false);
    expect(
      isTapbackDoubleClick({
        detail: 2,
...
describe("double-click to heart an assistant message", () => {
  it("hearts the message, and a second double-click retracts it", async () => {
    ...
    const message = (await screen.findByText("done")).closest(
    ...
    await waitFor(() =>
      expect($localReactions.get()["assistant-1"]?.[0]?.emoji).toBe("❤️"),
    );

    fireEvent.doubleClick(message!, { detail: 2 });
    await waitFor(() =>
      expect($localReactions.get()["assistant-1"]).toEqual([]),
    );
  });
