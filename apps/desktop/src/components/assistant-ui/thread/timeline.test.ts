...
describe("ownViewport", () => {
  it("resolves the viewport of the surface the timeline lives in", () => {
    ...
    expect(
      ownViewport(document.getElementById("timeline-session-tile:b"))?.id,
    ).toBe("viewport-session-tile:b");
    expect(ownViewport(document.getElementById("timeline-workspace"))?.id).toBe(
      "viewport-workspace",
    );
  });
  ...
  it("ignores a kept-alive tab that matches first", () => {
    document.body.innerHTML =
      surface("workspace", true) + surface("session-tile:b");
  ...
  it("falls back to the document when there is no surface around it", () => {
    document.body.innerHTML =
      '<div data-slot="aui_thread-viewport" id="viewport-lone"></div>';
