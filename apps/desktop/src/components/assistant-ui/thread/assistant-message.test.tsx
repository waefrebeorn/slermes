  ...
  return {
    ...
    metadata: {
      ...
      custom: {
        ...
        timelineTimestamp: createdAt.getTime() / 1000,
      },
    },
  } as unknown as ThreadMessage;
}
...
describe("message timeline timestamps", () => {
  it("always renders precise user and assistant lifecycle times", async () => {
    ...
    expect(stamps).toContain(
      formatTimelineRange(startedAt + 0.05, startedAt + 0.1),
    );
    expect(stamps).toContain(
      formatTimelineRange(startedAt + 0.125, startedAt + 0.5),
    );
  });
  ...
  it("suppresses an aggregate assistant stamp that exactly duplicates its sole part", async () => {
    ...
    const assistant = {
      ...assistantMessage(),
      content: [
        { completedAt, text: "done", timestamp: startedAt, type: "text" },
      ],
    } as unknown as ThreadMessage;
    ...
    expect(
      stamps.filter(
        (stamp) => stamp === formatTimelineRange(startedAt, completedAt),
      ),
    ).toHaveLength(1);
  });
});
