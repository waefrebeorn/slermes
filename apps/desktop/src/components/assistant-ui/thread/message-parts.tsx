...
} from "@/components/assistant-ui/thread/agent-delivery";
...
type TimelineTextPartProps = TextMessagePartProps & {
  completedAt?: number;
  timestamp?: number;
};
...
const ThinkingDisclosure: FC<{
  ...
  const showPreview =
    !reasoningCollapsedByDefault && (pending || sawLivePreview);
  ...
  useEffect(() => {
    ...
    const pin = (entries: readonly ResizeObserverEntry[]) => {
      const height =
        entries[entries.length - 1]?.borderBoxSize?.[0]?.blockSize ?? -1;
  ...
  return (
    ...
    >
      ...
      >
        <span className={cn(SCAFFOLD_LABEL_CLASS, pending && "shimmer")}>
          {thoughtLabel}
        </span>
      </ScaffoldRow>
      {open && (
...
const ReasoningAccordionGroup: FC<{
  children?: ReactNode;
  endIndex: number;
  startIndex: number;
  ...
  const timestamp = useAuiState((s) =>
    s.message.parts
      .slice(Math.max(0, startIndex), endIndex + 1)
      .reduce<number | undefined>((earliest, part) => {
        const value =
          part.type === "reasoning"
            ? (part as { timestamp?: number }).timestamp
            : undefined;
  ...
  const completedAt = useAuiState((s) =>
    s.message.parts
      ...
      .reduce<number | undefined>((latest, part) => {
        const value =
          part.type === "reasoning"
            ? (part as { completedAt?: number }).completedAt
            : undefined;
...
const ReasoningTextPart: ReasoningMessagePartComponent = () => {
  ...
  return (
    <MarkdownTextContent
      ...
      containerProps={
        { "data-slot": "aui_reasoning-text" } as ComponentProps<"div">
      }
      disableArtifacts
