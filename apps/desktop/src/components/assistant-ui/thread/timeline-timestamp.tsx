...
export const TimelineTimestamp: FC<{
  ...
  const completed =
    validUnixSeconds(completedAt) && completedAt > timestamp
      ? unixDate(completedAt)
      : null;

  const validCompletedAt =
    completed && validUnixSeconds(completedAt) ? completedAt : undefined;
  const startLabel = formatTimelineRange(timestamp, undefined);
  const completedLabel =
    validCompletedAt === undefined
      ? ""
      : formatTimelineRange(validCompletedAt, undefined);
  ...
  return (
    <span
      className={cn(
        "text-[0.625rem] leading-4 tabular-nums text-muted-foreground/55",
        className,
      )}
...
export const MessageTimelineTimestamp: FC<{
  ...
  const timestamp = useAuiState((s) => {
    const value = (
      s.message.metadata?.custom as { timelineTimestamp?: unknown } | undefined
    )?.timelineTimestamp;
  ...
  const completedAt = useAuiState((s) => {
    const value = (
      s.message.metadata?.custom as
        { timelineCompletedAt?: unknown } | undefined
    )?.timelineCompletedAt;
  ...
  const duplicatePart = useAuiState((s) => {
    ...
    const solePart =
      s.message.parts.length === 1
        ? (s.message.parts[0] as { completedAt?: unknown; timestamp?: unknown })
        : null;
    ...
    return (
      ...
      (solePart?.completedAt === custom.timelineCompletedAt ||
        (!validUnixSeconds(solePart?.completedAt) &&
          !validUnixSeconds(custom.timelineCompletedAt)))
    );
  });
