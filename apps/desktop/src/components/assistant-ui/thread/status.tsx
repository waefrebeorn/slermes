...
import {
  ...
  toolNarratesWait,
  TURN_QUIET_S,
} from "@/components/assistant-ui/thread/turn-activity";
...
const StatusRow: FC<
  { children: ReactNode; label: string } & React.ComponentPropsWithoutRef<"div">
...
const HintText: FC<{ children: ReactNode }> = ({ children }) => (
  <span className={cn(SCAFFOLD_LABEL_CLASS, "shimmer min-w-0 flex-1 truncate")}>
    {children}
  </span>
);
...
function useLocalModelLoad(
  active: boolean,
): (LocalModelLoadProgress & { model: string }) | null {
...
const WaitHint: FC<{ hint: string }> = ({ hint }) => {
  ...
  const label =
    load.kind === "load"
      ...
      : t.assistant.thread.processingPrompt;

  return <ProgressHint label={label} percent={load.percent} />;
};

const ProgressHint: FC<{ label: string; percent: null | number }> = ({
  label,
  percent,
}) => (
  <span className="flex min-w-0 flex-1 items-center gap-2">
    <span
      className={cn(SCAFFOLD_LABEL_CLASS, "shimmer min-w-0 shrink truncate")}
    >
      {label}
    </span>
    {percent !== null && (
      <>
        ...
        <span className={cn(SCAFFOLD_LABEL_CLASS, "shrink-0 tabular-nums")}>
          {percent}%
        </span>
      </>
    )}
...
function useThreadSessionStatus() {
  ...
  const compacting = useStore(
    useMemo(() => sessionCompacting(sessionId), [sessionId]),
  );
  const drafting = useStore(
    useMemo(() => sessionDraftingTool(sessionId), [sessionId]),
  );
  const providerWait = useStore(
    useMemo(() => sessionProviderWait(sessionId), [sessionId]),
  );
  ...
  const awaitingInput = useStore(
    useMemo(() => sessionAwaitingInput(sessionId), [sessionId]),
  );
...
export const ResponseLoadingIndicator: FC = () => {
  const { t } = useI18n();
  const { compacting, drafting, providerWait, turnStartedAt } =
  ...
  return (
    ...
    >
      ...
      {hint ? (
        ...
        <ProgressHint
          label={t.assistant.thread.loadingLocalModel(localLoad.model)}
          percent={localLoad.percent}
        />
      ) : null}
