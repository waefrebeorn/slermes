...
vi.mock(
  "@/components/assistant-ui/thread/use-message-reactions",
  async (importActual) => {
...
describe("streaming-status invalidation scope", () => {
  it("does not re-render the message root when the turn settles", async () => {
    const messages = [
      user("u1", "question"),
      assistant("a1", "partial answer", true),
    ];
    ...
    expect(
      container.querySelector('[data-message-streaming="true"]'),
    ).toBeTruthy();
    ...
    rerender(
      <Harness
        messages={[messages[0], assistant("a1", "partial answer", false)]}
      />,
    );
    ...
    expect(
      container.querySelector('[data-slot="aui_message-streaming-marker"]'),
    ).toBeTruthy();
  ...
  it("does not re-render the message root when streaming text arrives", async () => {
    ...
    rerender(
      <Harness messages={[messages[0], assistant("a1", "one two", true)]} />,
    );
  ...
  it("keeps the same root DOM node across the settle transition", async () => {
    ...
    const messages = [
      user("u1", delivery),
      assistant("a1", "working on it", true),
    ];
    const { container, findByText, rerender } = render(
      <Harness messages={messages} />,
    );
    ...
    rerender(
      <Harness
        messages={[messages[0], assistant("a1", "working on it", false)]}
      />,
    );
