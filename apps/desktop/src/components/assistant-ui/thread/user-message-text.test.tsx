...
describe("a sent reference renders as the chip the composer showed", () => {
  it("chips a backtick-quoted @url: instead of splitting it into code", () => {
    ...
    expect(
      screen.queryByTitle(
        "https://github.com/NousResearch/hermes-agent/pull/74790",
      ),
    ).not.toBeNull();
  ...
  it("chips a backtick-quoted @file: path with spaces", () => {
    render(
      <UserMessageText text="see @file:`apps/desktop/my notes.md` please" />,
    );
  ...
  it("chips every kind that travels in message text", () => {
    ...
    for (const kind of WIRE_REFERENCE_KINDS) {
      expect(`@${kind}:\`some value\``.match(referenceRe()), kind).toHaveLength(
        1,
      );
    }
  });
  ...
  it("renders code and a reference side by side", () => {
    render(
      <UserMessageText text="run `npm test` on @file:`apps/desktop/a b.ts` now" />,
    );

    expect(
      document.querySelector('[data-slot="aui_user-inline-code"]')?.textContent,
    ).toBe("npm test");
  ...
  it("leaves a fenced block alone", () => {
    ...
    expect(
      document.querySelector('[data-slot="aui_user-fence"]')?.textContent,
    ).toBe("const x = 1\n");
  });
});
