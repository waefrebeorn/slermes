...
const hoverProps = (
  index: number,
  paint: (index: number, on: boolean) => void,
) => ({
...
export const ownViewport = (root: HTMLElement | null): HTMLElement | null =>
  (
    root?.closest("[data-session-anchor]") ?? document
...
const ActiveThreadTimeline: FC = () => {
    ...
    for (const message of auiRef.current.thread().getState().messages) {
      if (message.role === "user") {
        rows.push({
          id: message.id,
          role: "user",
          text: userPromptText(message.content),
        });
      }
    }
  ...
  const jump = useCallback(
    (id: string) => scrollToPrompt(rootRef.current, id),
    [],
  );
