...
describe("agent message detection", () => {
  ...
  it("captures the @handle when present", () => {
    const m = AGENT_MESSAGE_RE.exec(
      "Message from 🤖 Eats Tests (@mr-tester): run them all",
    );
  ...
  it("does not match prose that merely contains the phrase", () => {
    expect(
      AGENT_MESSAGE_RE.test("I got a Message from 🤖 Hermes: earlier"),
    ).toBe(false);
    expect(
      AGENT_MESSAGE_RE.test("can you explain what Message from means?"),
    ).toBe(false);
  });
});
