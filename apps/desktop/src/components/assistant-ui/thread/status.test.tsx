...
describe("ResponseLoadingIndicator timer", () => {
  ...
  it("preserves each running session timer while switching between sessions", () => {
    ...
    expect(
      screen.getAllByText((_, node) => node?.textContent === "5s").length,
    ).toBeGreaterThan(0);
    sessionA.unmount();
    ...
    expect(
      screen.getAllByText((_, node) => node?.textContent === "3s").length,
    ).toBeGreaterThan(0);
    sessionB.unmount();
    ...
    expect(
      screen.getAllByText((_, node) => node?.textContent === "8s").length,
    ).toBeGreaterThan(0);
  });
  ...
  it("names a prolonged provider wait in the existing response status row", () => {
    ...
    expect(
      screen.getByText("⏳ waiting on local-model — 30s with no output yet"),
    ).toBeTruthy();
  });
});
  ...
  it("is marked as transcript scaffolding", () => {
    ...
    expect(
      container
        ...
        ?.hasAttribute("data-conversation-scaffold"),
    ).toBe(true);
  });
});
