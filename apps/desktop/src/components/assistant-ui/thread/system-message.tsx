    ...
    return (
      ...
      >
        <span className="tool-memory-legendary-glyph flex h-(--conversation-line-height) w-3.5 shrink-0 items-center justify-center">
          <ToolIcon
            className="text-(--tool-memory-legendary-icon)"
            name="brain"
            size="0.875rem"
          />
        </span>
        <span
          className={cn(
            ...
            "tool-memory-legendary-title shrink-0 text-transparent",
          )}
        >
        ...
        {detail && (
          <span
            className={cn(
              SCAFFOLD_LABEL_CLASS,
              "tool-memory-legendary-meta min-w-0 wrap-anywhere",
            )}
          >
            {detail}
          </span>
  ...
  if (slashStatus?.groups) {
    ...
    return (
      ...
      >
        ...
        <MessageTimelineTimestamp
          className={cn(multiline ? "mt-0.5 block" : "ml-1.5")}
        />
      </MessagePrimitive.Root>
    );
  }
