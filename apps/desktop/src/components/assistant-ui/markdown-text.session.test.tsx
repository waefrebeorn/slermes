...
describe("MarkdownTextContent session refs", () => {
  it("renders an agent-written @session ref as a link showing the session title", async () => {
    ...
    render(
      <MarkdownTextContent
        isRunning={false}
        text="Context lives in @session:work/20260101_abc123 today."
      />,
    );
  ...
  it("falls back to a short id when the session is unknown", async () => {
    render(
      <MarkdownTextContent
        isRunning={false}
        text="See @session:work/20260101_abc123 for context."
      />,
    );
  ...
  it("leaves a ref inside inline code as literal text", () => {
    render(
      <MarkdownTextContent
        isRunning={false}
        text="Write `@session:work/20260101_abc123` to link a chat."
      />,
    );
