...
export function refAttrs(
  kind?: string,
  extra?: string,
): { className: string; "data-ref"?: string } {
...
export const hermesDirectiveFormatter: Unstable_DirectiveFormatter = {
  serialize(item: Unstable_TriggerItem): string {
    const metadata = item.metadata as
      { rawText?: unknown; insertId?: unknown } | undefined;
    const rawText =
      typeof metadata?.rawText === "string" ? metadata.rawText : null;
    const insertId =
      typeof metadata?.insertId === "string" ? metadata.insertId : null;
...
export function DirectiveContent({ text }: { text: string }) {
  const { cleanedText, images } = useMemo(
    () => safeEmbeddedImages(text ?? ""),
    [text],
  );
  const segments = useMemo(
    () => safeDirectiveSegments(cleanedText),
    [cleanedText],
  );
  ...
  const imageSegments = useMemo(
    () =>
      segments.filter(
        (
          segment,
        ): segment is Extract<Unstable_DirectiveSegment, { kind: "mention" }> =>
...
export const DirectiveText: TextMessagePartComponent = ({
  text,
}: TextMessagePartProps) => <DirectiveContent text={text ?? ""} />;
...
const SlashChip: FC<{ kind: SlashChipKind; label: string; value: string }> = ({
  kind,
  label,
  value,
}) => (
...
const DirectiveChip: FC<{
  ...
  const activate =
    onClick ??
    (DIRECTIVE_ACTIONS[type]
      ? () => DIRECTIVE_ACTIONS[type]!.run(id)
      : undefined);
