...
describe("MarkdownLink filesystem hrefs", () => {
  ...
  it("routes an absolute file path link through the preview attachment", async () => {
    render(
      <MarkdownTextContent
        isRunning={false}
        text="Wrote it: [report](/home/user/report.md)"
      />,
    );
  ...
  it("routes file:// and ~/ links the same way", async () => {
    ...
    expect(
      screen.getAllByRole("button", { name: "Open preview" }),
    ).toHaveLength(2);
  });

  it("renders a media player for a media-extension path link", async () => {
    const { container } = render(
      <MarkdownTextContent isRunning={false} text="[clip](/tmp/demo.mp4)" />,
    );
  ...
  it("leaves anchors and relative links out of the preview pipeline", () => {
    render(
      <MarkdownTextContent
        isRunning={false}
        text={
          "[frag](#section-2) and [rel](docs/guide.md) and [site](https://example.com)"
        }
      />,
    );
