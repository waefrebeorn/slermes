...
describe("formatRefValue", () => {
  it("leaves simple paths untouched", () => {
    ...
    expect(formatRefValue("https://example.com/post")).toBe(
      "https://example.com/post",
    );
  });

  it("wraps paths with whitespace in backticks", () => {
    expect(formatRefValue("apple-touch-icon (1).png")).toBe(
...
describe("hermesDirectiveFormatter.parse", () => {
  it("keeps quoted file paths whole when parsing", () => {
    const segments = hermesDirectiveFormatter.parse(
      "see @image:`apple-touch-icon (1).png` for the icon",
    );
    ...
    expect(segments).toEqual([
      { kind: "text", text: "see " },
      {
        kind: "mention",
        type: "image",
        label: "apple-touch-icon (1).png",
        id: "apple-touch-icon (1).png",
      },
  ...
  it("still parses unquoted paths", () => {
    const segments = hermesDirectiveFormatter.parse(
      "@file:src/main.tsx the entry point",
    );
  ...
  it("parses session links with profile/id values", () => {
    const segments = hermesDirectiveFormatter.parse(
      "see @session:work/20260101_abc123 next",
    );
...
describe("inline skill references", () => {
  const skills = (text: string) =>
    [...hermesDirectiveFormatter.parse(text)]
      .filter(
        (segment) => segment.kind === "mention" && segment.type === "skill",
      )
  ...
  it("parses a skill chip alongside an @ reference", () => {
    const mentions = [
      ...hermesDirectiveFormatter.parse("run /clean on @file:`src/a.ts`"),
    ].filter((segment) => segment.kind === "mention");

    expect(
      mentions.map((segment) =>
        segment.kind === "mention" ? segment.type : "",
      ),
    ).toEqual(["skill", "file"]);
  });
});
