...
function claimFor(contributions: readonly Contribution[], name: string) {
  return contributions.find(
    (c) =>
      (c.data as TranscriptDirectiveContribution | undefined)?.name === name,
  );
}

export const TranscriptDirectiveLeaf: FC<{
  text: string;
  streaming?: boolean;
}> = ({ text, streaming }) => {
  ...
  const contribution = match?.data as
    TranscriptDirectiveContribution | undefined;
