...
describe("withInlineChrome", () => {
  ...
  it("puts the theme prelude FIRST so page styles override it", () => {
    const doc =
      "<html><head><style>body{color:red}</style></head><body><h1>hi</h1></body></html>";
  ...
  it("injects the measuring script before </body>", () => {
    ...
    expect(framed.indexOf("postMessage")).toBeGreaterThan(
      framed.indexOf("<h1>"),
    );
    expect(framed.indexOf("postMessage")).toBeLessThan(
      framed.indexOf("</body>"),
    );
...
describe("frameSizeFromMessage", () => {
  ...
  it("accepts our message with our token, height clamped", () => {
    expect(frameSizeFromMessage(msg(), "tok")).toEqual({
      height: 500,
      width: 300,
    });
    expect(frameSizeFromMessage(msg({ height: 12 }), "tok")?.height).toBe(120);
    expect(frameSizeFromMessage(msg({ height: 5000 }), "tok")?.height).toBe(
      1200,
    );
    expect(frameSizeFromMessage(msg({ height: 500.7 }), "tok")?.height).toBe(
      501,
    );
  });

  it("sanitizes width to 0 when missing or hostile", () => {
    expect(frameSizeFromMessage(msg({ width: undefined }), "tok")?.width).toBe(
      0,
    );
    expect(frameSizeFromMessage(msg({ width: "wide" }), "tok")?.width).toBe(0);
    expect(frameSizeFromMessage(msg({ width: Infinity }), "tok")?.width).toBe(
      0,
    );
...
describe("intentFromMessage", () => {
  ...
  it("caps runaway prompts to a sentence-sized budget", () => {
    expect(
      intentFromMessage(msg({ prompt: "x".repeat(9000) }), "tok"),
    ).toHaveLength(500);
  });
  ...
  it("rejects wrong token, wrong type, empty, and hostile shapes", () => {
    ...
    expect(
      intentFromMessage(msg({ type: "hermes-inline-preview-size" }), "tok"),
    ).toBeNull();
