...
describe("preprocessMarkdown", () => {
  ...
  it("strips orphan numeric citation markers outside code spans", () => {
    const output = preprocessMarkdown(
      "This is the source[0], but keep `items[0]` untouched.",
    );
  ...
  it("does not swallow trailing emphasis asterisks into an autolinked url", () => {
    const input =
      "**PR opened: https://github.com/NousResearch/hermes-agent/pull/12345**";
    ...
    expect(output).toContain(
      "<https://github.com/NousResearch/hermes-agent/pull/12345>",
    );
  ...
  it("keeps numeric inline math intact instead of escaping it as currency", () => {
    const input = [
      "- The observed outcome might be $4$",
      "- Because $4\\in A$, event $A$ occurred",
    ].join("\n");

    expect(preprocessMarkdown(input)).toBe(input);
  });

  it.each(["$4$", "$2/3$", "$5x=10$", "$4xy$", "$10kg$"])(
    "preserves balanced numeric inline math: %s",
    (input) => {
      expect(preprocessMarkdown(input)).toBe(input);
    },
  );
  ...
  it("does not mistake a numeric formula closer for a later price opener", () => {
    expect(preprocessMarkdown("Probability is $2/3$ and fee is $7.")).toBe(
      "Probability is $2/3$ and fee is \\$7.",
    );
  ...
  it("keeps escaping currency ranges instead of treating them as inline math", () => {
    ...
    expect(preprocessMarkdown("Costs $5 + tax; formula is $x$.")).toBe(
      "Costs \\$5 + tax; formula is $x$.",
    );
    expect(preprocessMarkdown("Costs $5 = base rate; formula is $x$.")).toBe(
      "Costs \\$5 = base rate; formula is $x$.",
    );
  });
  ...
  it("splits the hugging $$ form that the bracket rewrite itself produces", () => {
    ...
    expect(output).toBe(
      ["$$", "\\begin{aligned}", "a &= b", "\\end{aligned}", "$$"].join("\n"),
    );
  });
  ...
  it("keeps a radical index in math that arrived as bracket delimiters", () => {
    expect(preprocessMarkdown("\\(\\sqrt[3]{8}\\)")).toContain(
      "$\\sqrt[3]{8}$",
    );
  });
