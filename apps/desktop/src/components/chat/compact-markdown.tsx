...
const TAG_CLASSES = {
  blockquote:
    "mt-2 mb-2 border-l-2 border-(--ui-stroke-tertiary) pl-2.5 italic text-muted-foreground/85",
...
function MarkdownCode({ className, ...rest }: ComponentProps<"code">) {
  return (
    <code
      className={cn(
        "rounded bg-muted/80 px-1 py-px font-mono text-[0.86em] text-muted-foreground",
        className,
      )}
      {...rest}
    />
...
export const CompactMarkdown = memo(function CompactMarkdown({
  ...
  return (
    <div
      className={cn(
        "max-w-full text-xs leading-relaxed text-muted-foreground/90 wrap-anywhere",
        className,
      )}
    >
      <Streamdown
