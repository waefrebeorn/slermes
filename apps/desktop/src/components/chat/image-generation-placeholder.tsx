...
const drawAsciiDiffusion = (
  ...
  const denoise =
    cycle < 0.82
      ? smoothstep(0.02, 0.82, cycle)
      : 1 - smoothstep(0.82, 1, cycle);
  ...
  for (let row = -1; row <= rows + 1; row += 1) {
    for (let col = -1; col <= cols + 1; col += 1) {
      ...
      const bloom =
        ...
        Math.exp(
          -(
            (radius - (0.28 + Math.sin(angle * 5 + time * 0.16) * 0.035)) **
            2
          ) / 0.0028,
        ) *
          0.8;
      ...
      const contour =
        Math.exp(
          -(
            (Math.sin(angle * 3 + radius * 17 - time * 0.17) * 0.5 +
              0.5 -
              radius) **
            2
          ) / 0.016,
      ...
      const stem =
        Math.exp(-((sx + 0.05) ** 2 / 0.004 + (sy - 0.25) ** 2 / 0.08)) * 0.46;
      ...
      const glyph =
        RAMP[clamp(Math.floor(signal * (RAMP.length - 1)), 0, RAMP.length - 1)];
