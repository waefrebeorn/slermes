...
function parseFullFileDiff(diff: string, fullText: string): DiffLine[] {
  ...
  if (hunks.length === 0) {
    return fullLines.map((text, index) => ({
      kind: "context",
      newNo: index + 1,
      oldNo: index + 1,
      text,
      ...
      {lines.map((line, index) => (
        <span
          className={cn(
            ...
            !syntax && DIFF_KIND_TEXT[line.kind],
          )}
          key={`${index}-${line.text}`}
        >
          {line.text || " "}
...
function tokenStyle({
  ...
  color,
  fontStyle = 0,
}: ThemedToken): React.CSSProperties | undefined {
...
function PreviewDiffRows({
  ...
  return (
    <>
      {beforeLines > 0 && (
        <div aria-hidden style={{ height: beforeLines * PREVIEW_LINE_PX }} />
      )}
      {chunks.map((chunk) => (
      ...
      {afterLines > 0 && (
        <div aria-hidden style={{ height: afterLines * PREVIEW_LINE_PX }} />
      )}
    </>
  );
}
  ...
  return (
    <>
      {lines.map((line, index) => {
        ...
        return (
          <span
            className={cn(PREVIEW_DIFF_LINE_BASE, DIFF_KIND_TINT[line.kind])}
            key={`${index}-${line.text}`}
          >
            {rowTokens.length > 0
              ? rowTokens.map((token, tokenIndex) => (
...
export function DiffLines({ className, text, ...props }: DiffLinesProps) {
  ...
  return (
    <pre
      className={cn(DIFF_BOX_CLASS, className)}
...
function DiffOverviewRuler({ lines }: { lines: DiffLine[] }) {
  ...
  return (
    <div
      aria-hidden
      className="pointer-events-none absolute top-0 right-0 bottom-0 w-1.5 opacity-80"
    >
        ...
        {runs.map((run, index) => (
          <div
            className={cn(
              "absolute inset-x-0",
              run.kind === "add"
                ? "bg-(--ui-diff-add-border)"
                : "bg-(--ui-diff-remove-border)",
            )}
            key={index}
            style={{
              height: `max(0.125rem, ${run.sizePct}%)`,
              top: `${run.startPct}%`,
            }}
          />
        ))}
...
export function FileDiffPanel({
  ...
  const lineChunks = React.useMemo(
    () => chunkLines(lines, PREVIEW_CHUNK_LINES),
    [lines],
  );
  ...
  const canHighlight =
    Boolean(language) && !exceedsHighlightBudget(fullText ?? diff);
          ...
          <div className="grid min-w-max grid-cols-[auto_minmax(0,1fr)]">
            ...
            >
              {beforeRows > 0 && (
                <div
                  aria-hidden
                  style={{ height: beforeRows * PREVIEW_LINE_PX }}
                />
              )}
              ...
              {afterRows > 0 && (
                <div
                  aria-hidden
                  style={{ height: afterRows * PREVIEW_LINE_PX }}
                />
              )}
            </div>
