...
function DurationProbe({
  ...
  return (
    <span data-testid="measured">
      {measured === null ? "unknown" : measured}
    </span>
  );
}
...
describe("useMeasuredDuration", () => {
  ...
  it("freezes the duration at the moment the thing finishes", () => {
    const { rerender } = render(
      <DurationProbe active timerKey="reasoning:m1:0" />,
    );
  ...
  it("records the real finish time even if the UI clock was paused", () => {
    const probe = render(
      <DurationProbe active timerKey="reasoning:background" />,
    );
    vi.mocked(document.hasFocus).mockReturnValue(false);
    window.dispatchEvent(new Event("blur"));
    ...
    probe.rerender(
      <DurationProbe active={false} timerKey="reasoning:background" />,
    );
