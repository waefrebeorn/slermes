...
function StageRow({ descriptor, result, now }: StageRowProps) {
  ...
  const elapsed =
    state === "running" && typeof result?.startedAt === "number"
  ...
  const icon = useMemo(() => {
    switch (state) {
      ...
      default:
        return (
          <div className="size-1.5 rounded-full border border-(--ui-stroke-secondary)" />
        );
    }
  }, [state]);
  ...
  return (
    <li className="flex items-center gap-3 px-3 py-1">
      {state === "running" && (
        <div className="-mr-2 -ml-4 flex size-6 flex-shrink-0 items-center justify-center">
          {icon}
        </div>
      )}
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <span
            className={cn(
              "truncate text-sm",
              state === "running" ? "font-medium" : "text-muted-foreground",
            )}
          >
...
export function DesktopInstallOverlay({
  enabled = true,
}: DesktopInstallOverlayProps) {
  const { t } = useI18n();
  const copy = t.install;
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (
                    !desktop ||
                    typeof desktop.continueBootstrapLocal !== "function"
                  ) {
  ...
  const currentStage = stages.find(
    (s) => state.stages[s.name]?.state === "running",
  )?.name;

  const completedCount = stages.filter(
    (s) =>
      state.stages[s.name]?.state === "succeeded" ||
      state.stages[s.name]?.state === "skipped",
  ).length;
  ...
  const progressPct =
    totalCount > 0 ? Math.round((progressUnits / totalCount) * 100) : 0;
  const currentStartedAt = currentStage
    ? state.stages[currentStage]?.startedAt
    : null;
  const currentElapsed =
    typeof currentStartedAt === "number"
      ? formatElapsed(now - currentStartedAt)
      : "";
            ...
            <div className="mb-4">
              <div className="mb-1 flex items-center justify-between text-xs text-muted-foreground">
                <span>
                  ...
                  {currentStage &&
                    copy.currentStage(formatStageName(currentStage))}
        ...
        {state.active && !failed && (
          <div className="flex-shrink-0 bg-card p-4">
            <div className="flex items-center justify-end">
              ...
              >
                {cancelling ? (
                  <Loader className="size-4" type="fourier-flow" />
                ) : null}
