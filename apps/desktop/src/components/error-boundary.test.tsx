...
const CURRENT_LOOKUP_ERROR = new Error(
  "useClientLookup: Index 6 out of bounds (length: 2)",
);
...
const recoveryWarningCount = (calls: unknown[][]) =>
  calls.filter((call) =>
    call.some((value) =>
      String(value).includes("auto-recovering from assistant-ui lookup"),
    ),
  ).length;
...
describe("ErrorBoundary assistant-ui lookup recovery", () => {
  ...
  it.each([
    ...
    [
      "the legacy tapClientLookup error",
      new Error("tapClientLookup: Index 6 out of bounds (length: 2)"),
    ],
    [
      "the legacy tapClientResource error",
      new Error("tapClientResource: Index 6 out of bounds (length: 2)"),
    ],
  ])(
    "recovers the production root composition without StrictMode after %s clears",
    (_label, error) => {
      const warnSpy = vi
        .spyOn(console, "warn")
      ...
      expect(
        screen.queryByRole(RELOAD_WINDOW.role, { name: RELOAD_WINDOW.name }),
      ).toBeNull();
  ...
  it("stops retrying a persistent lookup error after the recovery budget is exhausted", () => {
    ...
    expect(
      screen.getByRole(RELOAD_WINDOW.role, { name: RELOAD_WINDOW.name }),
    ).toBeTruthy();
  ...
  it.each([
    [
      "a differently cased classifier near-miss",
      new Error("UseClientLookup: Index 6 out of bounds (length: 2)"),
    ],
    [
      "a non-bounds lookup error",
      new Error('useClientLookup: Key "missing" not found'),
    ],
    [
      "an unrelated render error",
      new Error("some unrelated application error"),
    ],
