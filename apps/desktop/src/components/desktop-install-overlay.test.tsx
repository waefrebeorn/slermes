...
function installDesktopMock(state: DesktopBootstrapState) {
  ...
  const desktop = {
    ...
    onBootstrapEvent: vi.fn(
      (listener: (event: DesktopBootstrapEvent) => void) => {
...
describe("DesktopInstallOverlay first-run setup", () => {
  ...
  it("surfaces a recoverable error when the local-bootstrap bridge is unavailable", async () => {
    ...
    const install = (await screen.findByText("Install Hermes locally")).closest(
      "button",
    ...
    expect(
      await screen.findByText(
        "Local installation could not start. Restart Hermes Desktop and try again.",
      ),
    ).toBeTruthy();
  ...
  it("keeps the local-start error when the first snapshot commits under the click", async () => {
    ...
    const install = (await whenPresent("Install Hermes locally")).closest(
      "button",
    ) as HTMLButtonElement;
    fireEvent.click(install);
  ...
  it("clears a stale local-start error when a repair presents a different root", async () => {
    ...
    fireEvent.click(
      (await screen.findByText("Install Hermes locally")).closest(
        "button",
      ) as HTMLButtonElement,
    );
    expect(
    ...
    act(() => {
      desktop.emitBootstrapEvent({
        ...
        platform: "win32",
        activeRoot:
          "C:\\Users\\me\\AppData\\Local\\hermes\\hermes-agent-repaired",
      });
    });
  ...
  it("opens the remote connection form from the first-run choice", async () => {
    installDesktopMock(
      bootstrapState({
        setupChoice: {
          platform: "linux",
          activeRoot: "/home/me/.hermes/hermes-agent",
        },
      }),
    );
  ...
  it("requires a successful token connection test before applying remote config", async () => {
    ...
    fireEvent.click(await screen.findByText("Connect to existing Hermes"));
    fireEvent.change(
      await screen.findByPlaceholderText("https://gateway.example.com/hermes"),
      {
    ...
    fireEvent.change(
      await screen.findByPlaceholderText("Paste session token"),
      {
    ...
    await screen.findByText(
      "Connected to https://gateway.example.com/hermes (0.17.0).",
    );
  ...
  it("ignores a completed probe after the gateway URL becomes invalid", async () => {
    ...
    let resolveProbe:
      ((result: DesktopConnectionProbeResult) => void) | undefined;

    const pendingProbe = new Promise<DesktopConnectionProbeResult>(
      (resolve) => {
  ...
  it("does not enable Apply when credentials change during a connection test", async () => {
    ...
    let resolveTest:
      | ((result: { baseUrl: string; ok: boolean; version: string }) => void)
      | undefined;
    ...
    fireEvent.click(screen.getByText("Test connection"));
    await waitFor(() =>
      expect(desktop.testConnectionConfig).toHaveBeenCalledTimes(1),
    );
  ...
  it("restores remote apply controls when applying the tested connection fails", async () => {
    ...
    desktop.applyConnectionConfig.mockRejectedValue(
      new Error("remote apply failed"),
    );
  ...
  it("signs in, tests, and applies a password-style remote gateway", async () => {
    ...
    await waitFor(() => {
      expect(desktop.oauthLoginConnectionConfig).toHaveBeenCalledWith(
        "https://gateway.example.com/hermes",
      );
    });
  ...
  it("offers remote connection from the unsupported packaged install screen", async () => {
    ...
    expect(
      await screen.findByText("Hermes needs a one-time install"),
    ).toBeTruthy();
    ...
    fireEvent.change(
      screen.getByPlaceholderText("https://gateway.example.com/hermes"),
      {
        target: { value: "https://gateway.example.com/hermes" },
      },
    );
