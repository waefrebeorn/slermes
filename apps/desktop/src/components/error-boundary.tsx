...
const isTransientAssistantUiLookupError = (error: Error): boolean =>
  ASSISTANT_UI_LOOKUP_ERROR.test(error.message);
...
> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    ...
    if (
      ...
      isTransientAssistantUiLookupError(error) &&
      this.takeAutoRecoveryAttempt()
    ) {
      console.warn(
        `${tag} auto-recovering from assistant-ui lookup render race`,
        error.message,
      );
  ...
  private takeAutoRecoveryAttempt(): boolean {
    ...
    if (
      this.autoRecoveryCount === 0 ||
      now - this.autoRecoveryWindowStart >= AUTO_RECOVERY_WINDOW_MS
    ) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Button
          onClick={() =>
            void window.hermesDesktop?.revealLogs()?.catch(() => undefined)
          }
          variant="text"
        >
          {t.errors.openLogs}
