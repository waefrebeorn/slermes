...
export function ModelVisibilityDialog({
  ...
  const modelOptions = useQuery({
    ...
    queryFn: (): Promise<ModelOptionsResponse> =>
      requestModelOptions({ gateway: gw, profile, sessionId }),
    enabled: open,
  });
  ...
  const matches = (provider: ModelOptionProvider, model: string) =>
    !q ||
    `${model} ${provider.name} ${provider.slug} ${displayModelName(model)}`
      .toLowerCase()
      .includes(q);
        ...
        <div className="max-h-[55vh] overflow-y-auto pb-1">
          {providers.length === 0 ? (
            ...
            providers.map((provider) => {
              const models = collapseModelFamilies(
                provider.models ?? [],
              ...
              const collapsed =
                collapsedProviders.includes(provider.slug) && !q;
                  ...
                  <div className="flex items-center gap-2 px-3 pb-0.5 pt-1">
                    ...
                    <Checkbox
                      ...
                      onCheckedChange={(next) =>
                        setProviderVisible(provider, next !== false)
                      }
                    />
                  </div>
                  {!collapsed &&
