...
function useDecoded(text: string): string {
  ...
  useEffect(() => {
    ...
    const tailIndices = chars
      .map((_, idx) => idx)
      .filter((idx) => scrambleable[idx]);
    let resolved = 0;
    ...
    const id = window.setInterval(() => {
      ...
      setOut(
        chars
          .map((ch, idx) =>
            scrambleable[idx] && !settled.has(idx) ? pickAscii() : ch,
          )
          .join(""),
      );
...
export function DecodedLabel({
  ...
  return (
    ...
    >
      ...
      <style>
        {
          "@keyframes ob-decode-cursor { 0%, 49% { opacity: 1 } 50%, 100% { opacity: 0 } }"
        }
      </style>
    </span>
  );
...
export function HackeryButton({
  ...
  return (
    ...
    >
      ...
      <span className="-mr-[0.25em] pl-[0.25em] tracking-[0.25em]">
        {label}
      </span>
      <span className="text-primary/40 transition-colors group-hover:text-primary">
        ]
