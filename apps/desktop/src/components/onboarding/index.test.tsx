...
describe("onboarding Picker", () => {
  ...
  it("shows Fireworks first in the expanded list, ahead of other OAuth providers", () => {
    ...
    const labels = screen
      ...
      .filter((text) =>
        /Nous Portal|Fireworks AI|ChatGPT or Codex|MiniMax|OpenRouter/.test(
          text,
        ),
      );

    const indexOf = (needle: string) =>
      labels.findIndex((text) => text.includes(needle));
    expect(indexOf("Nous Portal")).toBeGreaterThanOrEqual(0);
    expect(indexOf("Fireworks AI")).toBeGreaterThan(indexOf("Nous Portal"));
    expect(indexOf("ChatGPT or Codex")).toBeGreaterThan(
      indexOf("Fireworks AI"),
    );
  ...
  it('offers "choose later" on first run and persists the skip', () => {
    ...
    const skip = screen.getByRole("button", {
      name: "I'll choose a provider later",
    });

    fireEvent.click(skip);

    expect($desktopOnboarding.get().firstRunSkipped).toBe(true);
    expect(window.localStorage.getItem("hermes-onboarding-skipped-v1")).toBe(
      "1",
    );
  });
  ...
  it('hides "choose later" in manual (add-provider) mode', () => {
    ...
    expect(
      screen.queryByRole("button", { name: "I'll choose a provider later" }),
    ).toBeNull();
  });
});
