...
export function ModelPickerDialog({
  ...
  const modelOptions = useQuery({
    ...
    queryFn: () =>
      requestModelOptions({ gateway: gw, profile, request, sessionId }),
    enabled: open,
  });
  ...
  const { model: optionsModel, provider: optionsProvider } =
...
function ModelResults({
  ...
  const visibleDownloads = downloads.filter(
    (job) => !q || (job.target || "").toLowerCase().includes(q),
  );
    ...
    <>
      {configured.map((provider) => {
        ...
        const groupDownloads =
          provider.slug === LOCAL_PROVIDER_SLUG ? visibleDownloads : [];
        ...
        return (
          ...
          >
            ...
            {models.map((model) => {
              const isCurrent =
                model === currentModel && provider.slug === currentProvider;
      ...
      {!hasLocalGroup && visibleDownloads.length > 0 && (
        <CommandGroup
          heading={copy.localDownloadsHeading}
          key="local-downloads"
        >
