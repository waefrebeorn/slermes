...
export const hiddenPaneProps = (hidden: boolean): Record<string, string> =>
  hidden ? { [PANE_HIDDEN_ATTR]: "" } : {};
...
export const usePaneLifecycle = (): PaneLifecycle =>
  useContext(PaneLifecycleContext);
...
export const isElementInHiddenPane = (element: Element): boolean =>
  Boolean(element.closest(HIDDEN_PANE));

/** `querySelectorAll` minus anything inside an inactive tab. */
export const queryAllVisible = <T extends HTMLElement>(
  selector: string,
  root: ParentNode = document,
): T[] =>
  [...root.querySelectorAll<T>(selector)].filter(
    (el) => !isElementInHiddenPane(el),
  );

/** `querySelector` minus anything inside an inactive tab. */
export const queryVisible = <T extends HTMLElement>(
  selector: string,
  root: ParentNode = document,
): null | T => queryAllVisible<T>(selector, root)[0] ?? null;
