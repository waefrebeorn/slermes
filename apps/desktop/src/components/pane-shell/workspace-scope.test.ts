...
describe("workspace scope", () => {
  ...
  it("publishes a coherent mode and owner in one batch", () => {
    ...
    const capture = () =>
      snapshots.push([$workspaceMode.get(), $workspaceOwnerKey.get()]);
    ...
    expect(
      snapshots.every(
        (snapshot) =>
          snapshot[0] === "bots" && snapshot[1] === "connection-a::default",
      ),
    ).toBe(true);
  ...
  it("publishes the exact new-session route with its Bot owner", () => {
    ...
    expect(
      setWorkspaceScope("bots", "bot:connection-a::writer", {
        kind: "route",
        route,
      }),
    ).toBe(true);
  ...
  it("keeps a group owner explicit while publishing why it has no generic route", () => {
    const target = {
      ...
      message: "New group conversations start in the group composer.",
    };
...
describe("workspace owner title", () => {
  it("captions a bot chat by its bot instead of the canonical stored title, and leaves everything else alone (#99152)", () => {
    ...
    expect(workspaceOwnerTitle("Plan the launch", botChat)).toBe(
      "Plan the launch",
    );
    // A Sessions tab titled the same way is not a bot chat.
    expect(workspaceOwnerTitle("Bot Chat", { workspaceMode: "sessions" })).toBe(
      "Bot Chat",
    );
    // No label yet (roster not loaded): the stored title stands.
    expect(
      workspaceOwnerTitle("Bot Chat", {
        ...botChat,
...
describe("remembered active panes", () => {
  ...
  it("remembers and restores panes independently per owner key", () => {
    ...
    expect(
      resolveRememberedActivePane("conn-a:profile-x", ["pane-1", "pane-2"]),
    ).toBe("pane-1");
    expect(
      resolveRememberedActivePane("conn-b:profile-y", ["pane-1", "pane-2"]),
    ).toBe("pane-2");
  });
  ...
  it("falls back after the remembered pane is removed", () => {
    ...
    expect(resolveRememberedActivePane("bot-a", ["first", "second"])).toBe(
      "first",
    );
  ...
  it("forgets a single owner without touching others", () => {
    ...
    expect(resolveRememberedActivePane("bot-a", ["fallback-a", "pane-a"])).toBe(
      "fallback-a",
    );
    expect(resolveRememberedActivePane("bot-b", ["pane-a", "pane-b"])).toBe(
      "pane-b",
    );
  });
  ...
  it("forgets a removed pane across every owner that remembered it", () => {
    ...
    expect(resolveRememberedActivePane("bot-a", ["fallback-a"])).toBe(
      "fallback-a",
    );
    expect(resolveRememberedActivePane("bot-b", ["fallback-b"])).toBe(
      "fallback-b",
    );
  });
});
