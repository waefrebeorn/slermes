...
function resizerGeometry(resizer: GridResizer, zones: GridZone[]) {
  const all = [
    ...resizer.negativeSideIndices,
    ...resizer.positiveSideIndices,
  ].map((i) => zones[i]);
...
export function ZoneEditor() {
  ...
  useEffect(() => {
    ...
    const down = (e: KeyboardEvent) => {
      ...
      if (
        e.key === "Escape" &&
        !e.defaultPrevented &&
        isTopEscapeLayer(ESCAPE_PRIORITY.zoneEditor)
      ) {
        e.preventDefault();
  ...
  const updateSplitPreview = (clientX: number, clientY: number) => {
    ...
    setSplitPreview(
      canSplit(model, zone.index, position, orientation)
  ...
  const onCanvasPointerDown = (e: ReactPointerEvent<HTMLDivElement>) => {
    if (
      e.button !== 0 ||
      (e.target as HTMLElement).dataset.resizer !== undefined
    ) {
      return;
    }
      ...
      if (!dragged) {
        ...
        if (splitPreview) {
          setModel((m) =>
            splitZone(
              m,
              splitPreview.zoneIndex,
              splitPreview.position,
              splitPreview.orientation,
            ),
          );
          setSplitPreview(null);
        }
  ...
  const startResizerDrag = (
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      const total = Math.round(
        (px / (horizontal ? rect.height : rect.width)) * MULTIPLIER,
      );
  ...
  const save = () => {
    const paneIds = allPaneIds(
      $layoutTree.get() ?? { type: "group", id: "tmp", panes: [], active: "" },
    );
    ...
    const placed = paneIds.map((id) => ({
      id,
      placement: (
        contributions.find((c) => c.id === id)?.data as
          { placement?: PanePlacementHint } | undefined
      )?.placement,
    }));
    ...
    const id = saveLayoutPresetTree(
      name || t.zones.customZoneName(zones.length),
      tree,
    );
  ...
  return (
    <div
      ...
      style={{
        background: "color-mix(in srgb, var(--ui-bg-chrome) 88%, transparent)",
    ...
    >
      ...
      >
              ...
              style={
                horizontal
                  ? {
                      top: pct(geo.at),
                      left: pct(geo.from),
                      width: pct(geo.to - geo.from),
                    }
                  : {
                      left: pct(geo.at),
                      top: pct(geo.from),
                      height: pct(geo.to - geo.from),
                    }
              }
            >
              <span
                className={cn(
                  ...
                  horizontal ? "h-[4px] w-10" : "h-10 w-[4px]",
                )}
                style={{
                  background:
                    "color-mix(in srgb, var(--ui-text-primary) 55%, transparent)",
                }}
              />
            </div>
          );
        ...
        {mergeAt && selection.length > 1 && (
          <div
            ...
            style={{ left: mergeAt.x, top: mergeAt.y }}
          >
            <Button
              className="shadow-lg"
              onClick={merge}
      ...
      <div className="flex items-center gap-1.5">
        <Input
          ...
          placeholder={t.zones.layoutNamePlaceholder(
            t.zones.customZoneName(zones.length),
          )}
          value={name}
        />
        <Button
          disabled={!treeExpressible}
          onClick={save}
          size="sm"
          variant="outline"
        >
        ...
        <Button
          onClick={() => $zoneEditorOpen.set(false)}
          size="sm"
          variant="ghost"
        >
          {t.common.cancel}
        ...
        {!treeExpressible && (
          <span className="text-xs text-muted-foreground/80">
            {t.zones.notExpressible}
          </span>
        )}
        <span className="ml-auto text-xs text-muted-foreground/60">
          {t.zones.zoneCount(zones.length)}
        </span>
      </div>
    </div>
  );
