...
describe("hide-only strip tabs", () => {
  ...
  it("persists hides and clears them on reveal", () => {
    ...
    const persisted = JSON.parse(
      window.localStorage.getItem("hermes.desktop.hiddenStripTabs.v1") ?? "[]",
    );
  ...
  it("refuses to hide the strip that is the only handle for hide-only tabs", () => {
    ...
    expect(
      group && group.type === "group" ? tabStripVisibleForGroup(group) : false,
    ).toBe(true);
  });
  ...
  it("excludes hide-only tabs from every close verb", () => {
    ...
    $layoutTree.set(
      group(["sessions", "hermes-bots:pane", "session-tile:x"], {
        active: "sessions",
        id: "g-mixed",
      }),
    );
    ...
    expect(treeTabCloseTargets("session-tile:x")).toEqual({
      all: 1,
      others: 0,
      right: 0,
    });
