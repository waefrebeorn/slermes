...
function bindPaneCollapse(
  paneId: string,
  $open: ReturnType<typeof atom<boolean>>,
) {
  bindToolPaneCollapse(
    paneId,
    $open,
  ...
  const found =
    tree.type === "split"
      ? tree.children.find(
          (c) =>
            c.type === "group" &&
            (c.panes.includes("terminal") || c.panes.includes("logs")),
        )
      : null;
...
const stackTree = (options?: {
  active?: string;
  tabStrip?: "always" | "never";
}) => {
  $layoutTree.set(
    split("column", [
...
describe("collapsing the active terminal in a shared group with the workspace", () => {
  ...
  it("openNewSessionTile (+ / ⌘T): the new session tab fronts and the terminal tab goes hidden", () => {
    ...
    $layoutTree.set(
      group(["workspace", "files", "review", "terminal", tilePaneId], {
        active: "terminal",
        id: "g-focus",
      }),
    );
