...
export const $activePresetId = atom<string>(
  readKey("hermes.desktop.layoutPreset.active") ?? "default",
);
...
const paneShares: Record<string, number> =
  readJson<Record<string, number>>(PANE_SHARE_KEY) ?? {};
...
export const $hiddenStripTabs = atom<ReadonlySet<string>>(
  new Set(readJson<string[]>(HIDDEN_STRIP_TAB_KEY) ?? []),
);
...
function tabTargetGroup(
  eligible: (group: GroupNode) => boolean,
): GroupNode | null {
  const tree = $layoutTree.get();
...
const treeGroupOfEvent = (event: Event): null | string => {
  ...
  return (
    el?.closest<HTMLElement>("[data-tree-group]")?.dataset.treeGroup ?? null
  );
};
...
export function trackActiveTreeGroup(): () => void {
  ...
  const trackHover = (event: Event) =>
    noteHoveredTreeGroup(treeGroupOfEvent(event));
...
const isUncloseablePane = (paneId: string): boolean =>
  Boolean(
    (
      registry.getArea("panes").find((c) => c.id === paneId)?.data as
...
export function focusedSessionTabAnchor(): null | string {
  ...
  return active && isSessionStripPane(active)
    ? active
    : (group.panes.find(isSessionStripPane) ?? null);
}
...
export function closeFocusedSessionTab(): boolean {
  const active = tabTargetGroup((group) =>
    group.panes.some(isMainStripPane),
  )?.active;
...
function closeableTreeSiblings(paneId: string): {
  others: string[];
  right: string[];
} {
  const tree = $layoutTree.get();
  ...
  return {
    others: panes.filter(
      (id) => id !== paneId && !isUncloseablePane(id) && !isHideOnlyPane(id),
    ),
    right: panes.filter(
      (id, i) => i > idx && !isUncloseablePane(id) && !isHideOnlyPane(id),
    ),
  };
}

/** Closeable-tab counts for a tab's menu enablement (`all` includes self). */
export function treeTabCloseTargets(paneId: string): {
  all: number;
  others: number;
  right: number;
} {
  ...
  return {
    all:
      others.length +
      (isUncloseablePane(paneId) || isHideOnlyPane(paneId) ? 0 : 1),
...
export function closeAllTreeTabs(paneId: string): void {
  ...
  panes
    .filter((id) => !isUncloseablePane(id) && !isHideOnlyPane(id))
    .forEach(closeTabPane);
}
...
export function hideOnlyZoneTabs(
  groupId: string,
): { hidden: boolean; id: string; title: string }[] {
  const tree = $layoutTree.get();
...
export function activateTreeTabSlot(slot: number): null | string {
  const group = tabTargetGroup(
    (candidate) => shownPanesInGroup(candidate).length >= 2,
  );
...
function rootRow(): SplitNode | null {
  ...
  return (
    (tree.children.find(
      (child) =>
        child.type === "split" && child.orientation === "row" && hasMain(child),
    ) as SplitNode | undefined) ?? null
  );
}
...
export function paneRootSide(paneId: string): null | TreeSide {
  ...
  return child
    ? rootChildSide(child, (id) => panes.find((p) => p.id === id))
    : null;
}
...
export function layoutHasRootSide(side: TreeSide): boolean {
  ...
  return row.children.some(
    (child) =>
      rootChildSide(child, (id) => panes.find((p) => p.id === id)) === side,
  );
}
...
function restoreDismissedSidePanes(side: TreeSide) {
  ...
  for (const pane of registry.getArea("panes")) {
    ...
    const placement = (pane.data as { placement?: string } | undefined)
      ?.placement;
    const paneSide =
      placement === "left" ? "left" : placement === "main" ? null : "right";
...
export function revealTreePane(paneId: string) {
  ...
  if ($hiddenStripTabs.get().has(paneId)) {
    saveHiddenStripTabs(
      toggledSet($hiddenStripTabs.get(), paneId, false) ??
        $hiddenStripTabs.get(),
    );
  }
...
export const $sessionTileDragging = computed(
  $treeDragging,
  (dragging) => dragging === SESSION_TILE_DRAG,
);
...
export const $sessionTileEdgeHover = computed(
  ...
  (dragging, hint) =>
    dragging === SESSION_TILE_DRAG &&
    ((hint?.pos !== undefined && hint.pos !== "center") ||
...
function adoptMissingPanes(target: LayoutNode, source: LayoutNode): LayoutNode {
  ...
  for (const paneId of allPaneIds(source)) {
    ...
    const sibling = findGroupOfPane(source, paneId)?.panes.find((p) =>
      have.has(p),
    );
    const targetId =
    ...
    if (targetId) {
      ...
      next =
        insertAtGroup(next, targetId, paneId, "center", null, false) ?? next;
      have.add(paneId);
    }
  }
...
function enforceDockedPanes(
  ...
  dataOf: (
    paneId: string,
  ) => { dock?: PaneDockHint; placement?: string } | undefined,
): LayoutNode {
  let next = tree;
  ...
  for (const pane of registry.getArea("panes")) {
    ...
    const target = without
      ? findGroupOfPane(without, dock.pane)?.id
      : undefined;
    ...
    next =
      insertAtGroup(without, target, pane.id, "center", dock.before, false) ??
      next;
  }
...
function adoptContributedPanes(): void {
  ...
  for (const pane of missing) {
    ...
    const anchor =
      ...
      allPaneIds(next).find(
        (id) => id !== pane.id && placementOf(id) === placement,
      ) ??
      mainId;
...
export const $userPlacedPanes = atom<ReadonlySet<string>>(
  new Set(readJson<string[]>(USER_PLACED_KEY) ?? []),
);
...
export function dockPaneBeside(paneId: string, anchorPaneId: string) {
  ...
  if (
    !anchor ||
    $hiddenTreePanes.get().has(anchorPaneId) ||
    !panes.some((c) => c.id === anchorPaneId)
  ) {
    return;
  }
  ...
  const mainId = panes.find((c) => {
    const data = c.data as
      { placement?: string; uncloseable?: boolean } | undefined;
  ...
  const anchorRightOfMain =
    !!mainId &&
    !anchor.panes.includes(mainId) &&
    order.indexOf(anchorPaneId) > order.indexOf(mainId);
...
export function applyTree(tree: LayoutNode, presetId: string) {
  ...
  for (const paneId of allPaneIds(tree)) {
    const data = panes.find((c) => c.id === paneId)?.data as
      { revealOnPreset?: boolean } | undefined;
...
export function reorderTreePanes(
  groupId: string,
  paneIds: readonly string[],
  toIndex: number,
) {
    ...
    if (collapsed && group.active === paneId) {
      if (group.panes.some(isUncloseablePane)) {
        ...
        if (anchor && anchor !== paneId) {
          ...
          activateTreePane(
            group.id,
            group.panes[at - 1] ?? group.panes[at + 1],
          );
        }
      } else {
...
export function $paneVisible(paneId: string): ReadableAtom<boolean> {
  ...
  if (!cached) {
    cached = computed([$layoutTree, $dismissedPanes, $hiddenTreePanes], () =>
...
export function setTreeGroupTabStrip(
  groupId: string,
  tabStrip: TabStripMode | undefined,
) {
  const tree = $layoutTree.get();
 ...
 */
const tabStripTargetGroup = () =>
  tabTargetGroup((candidate) => shownPanesInGroup(candidate).length > 0);
...
export function toggleTargetZoneTabStrip(): TabStripMode | null {
  ...
  const next: TabStripMode = tabStripVisibleForGroup(group)
    ? "never"
    : "always";
 ...
 */
export function presetSplitWeights(
  splitId: string,
  length: number,
): number[] | null {
  const activeId = $activePresetId.get();
  const presets = [...registry.getArea("layouts")].sort(
    (a, b) => Number(b.id === activeId) - Number(a.id === activeId),
  );

  for (const preset of presets) {
    const weights =
      preset.data && isLayoutNode(preset.data)
        ? findSplitWeights(preset.data, splitId)
        : null;
