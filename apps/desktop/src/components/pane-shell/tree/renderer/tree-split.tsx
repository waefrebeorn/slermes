 ...
 */
function useSubtreeOverrides(
  paneIds: readonly string[],
): TrackContext["overrides"] {
  const key = paneIds.join(",");
  const cache = useRef<{
    sig: string;
    value: Record<string, PaneStateSnapshot>;
  }>({ sig: "\0", value: {} });
  ...
  const snapshot = useCallback(() => {
    ...
    const sig = paneIds
      .map(
        (id) =>
          `${id}:${all[id]?.widthOverride ?? ""}:${all[id]?.heightOverride ?? ""}`,
      )
      .join("|");
    ...
    if (cache.current.sig !== sig) {
      cache.current = {
        ...
        value: Object.fromEntries(
          paneIds.flatMap((id) => (all[id] ? [[id, all[id]]] : [])),
        ),
      };
    }
...
export function TreeSplit({
  ...
  const overrides = useSubtreeOverrides(
    useMemo(() => allPaneIds(node), [node]),
  );
  ...
  const childRootRow = (child: LayoutNode): boolean => {
    ...
    return allPaneIds(child).some(
      (id) => paneChrome(paneFor(id)).placement === "main",
    );
  };
  ...
  const isEmptyZone = (child: LayoutNode) =>
    child.type === "group" && child.panes.length === 0;
  const isCollapsed = (child: LayoutNode) =>
    subtreeGone(child, trackCtx) || (isEmptyZone(child) && !editMode);
  ...
  const startSash = useCallback(
    (aIndex: number, bIndex: number, e: ReactPointerEvent<HTMLDivElement>) => {
      ...
      const sideFor = (
        ...
        const toolZone =
          allPaneIds(child).length > 0 &&
          allPaneIds(child).every(isCollapsePane);
        ...
        return {
          ...
          min: toolZone
            ...
            : Math.max(
                floor,
                computedPx(horizontal ? cs.minWidth : cs.minHeight, 0),
              ),
          max: computedPx(
            horizontal ? cs.maxWidth : cs.maxHeight,
      ...
      const applyShift = (shiftPx: number) => {
        if (a.fixed) {
          a.paneIds.forEach((id) =>
            setOverride(id, Math.round(a0px + shiftPx)),
          );
        }

        if (b.fixed) {
          b.paneIds.forEach((id) =>
            setOverride(id, Math.round(b0px - shiftPx)),
          );
        }
      ...
      const onMove = (ev: PointerEvent) => {
        lastShift = Math.max(
          lo,
          Math.min(hi, (horizontal ? ev.clientX : ev.clientY) - start),
        );
  ...
  const resetBoundary = useCallback(
    (aIndex: number, bIndex: number) => {
      ...
      for (const i of [aIndex, bIndex]) {
        ...
        if (
          child.type !== "group" ||
          fixedTrackSize(child, axis, trackCtx) !== null
        ) {
          continue;
        }
        ...
        for (const paneId of shownPaneIds(child, trackCtx)) {
          ...
          const resolved = css
            ? resolveCssPx(container, css, horizontal)
            : null;
        ...
        const others = weights.reduce(
          (sum, w, j) => (j === i ? sum : sum + w),
          0,
        );
      ...
      setTreeSplitWeights(
        node.id,
        !preset && !pinned ? weights.map(() => 1) : weights,
      );
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [
  ...
  const isMinimized = (child: LayoutNode) =>
    child.type === "group" && Boolean(child.minimized);
  ...
  const semanticSides =
    rootRow && horizontal && collapsedSides.size > 0 && !editMode;
  ...
  const tracks = node.children.map((child, i) => {
    ...
    const track =
      minimized || collapsed ? null : fixedTrackSize(child, axis, trackCtx);
    ...
    const narrowCollapsed =
      narrow &&
      collapsed &&
      allPaneIds(child).some((id) => !hiddenPanes.has(id));
  ...
  const growable = tracks
    .map((_, i) => i)
    .filter((i) => !tracks[i].collapsed && !tracks[i].minimized);
  const allFixed =
    growable.length > 0 && growable.every((i) => tracks[i].track !== null);
  ...
  const absorberIndex = allFixed
    ? allFixedAbsorberIndex(growable, (i) =>
        horizontal ? tracks[i].sizing?.maxWidth : tracks[i].sizing?.maxHeight,
      )
    : -1;
  ...
  const flexTotal = growable.reduce(
    (sum, i) => sum + (tracks[i].track === null ? node.weights[i] : 0),
    0,
  );
  ...
  const visibleOrder = tracks
    .map((t, j) => (t.collapsed ? -1 : j))
    .filter((j) => j >= 0);

  const railSideFor = (i: number): "left" | "right" => {
    const pos = visibleOrder.indexOf(i);

    return pos >= 0 && (pos + 0.5) / visibleOrder.length > 0.5
      ? "right"
      : "left";
  };
  ...
  return (
    ...
    >
      {tracks.map(
        (
          { child, collapsed, minimized, narrowCollapsed, sizing, track },
          i,
        ) => {
                ...
                collapsed
                  ...
                  : minimized
                    ...
                    : {
                        ...
                        maxWidth:
                          horizontal && !absorbs ? sizing?.maxWidth : undefined,
                        minHeight: (!horizontal && sizing?.minHeight) || 0,
                        maxHeight:
                          horizontal || absorbs ? undefined : sizing?.maxHeight,
                      }
              }
            >
              {partner >= 0 && (
    ...
    >
      ...
      <span
        className={cn(
          ...
          horizontal
            ? "inset-y-0 left-[1px] w-px -translate-x-1/2"
            : "inset-x-0 top-[1px] h-px -translate-y-1/2",
        )}
      />
      {!disabled && (
        <span
