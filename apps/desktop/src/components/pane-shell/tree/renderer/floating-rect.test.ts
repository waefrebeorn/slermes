...
describe("anchoredRect", () => {
  it("spawns in the named corner, inset by the margin", () => {
    expect(anchoredRect("top-right", size, viewport)).toMatchObject({
      x: 1188,
      y: 46,
    });
    expect(anchoredRect("top-left", size, viewport)).toMatchObject({
      x: 12,
      y: 46,
    });
    expect(anchoredRect("bottom-right", size, viewport)).toMatchObject({
      x: 1188,
      y: 708,
    });
    expect(anchoredRect("bottom-left", size, viewport)).toMatchObject({
      x: 12,
      y: 708,
    });
  });
  ...
  it("never spawns over the reserved top chrome", () => {
    for (const anchor of [
      ...
      expect(
        anchoredRect(anchor, size, { width: 320, height: 120, top: 34 }).y,
...
describe("clampFloatingRect", () => {
  it("keeps a grabbable sliver on screen at either horizontal edge", () => {
    expect(clampFloatingRect({ ...size, x: 5000, y: 100 }, viewport).x).toBe(
      1392,
    );
    expect(clampFloatingRect({ ...size, x: -5000, y: 100 }, viewport).x).toBe(
      -192,
  ...
  it("hard-bounds the top so the drag handle is always reachable", () => {
    expect(clampFloatingRect({ ...size, x: 100, y: -500 }, viewport).y).toBe(
      34,
    );
    expect(clampFloatingRect({ ...size, x: 100, y: 5000 }, viewport).y).toBe(
      852,
    );
  });
...
describe("reflowRect", () => {
  ...
  it("holds position when left/top-anchored, but still clamps into view", () => {
    ...
    expect(reflowRect(start, "top-left", viewport, shrunk)).toMatchObject({
      x: 40,
      y: 60,
    });
    expect(
      reflowRect({ ...size, x: 1300, y: 60 }, "top-left", viewport, shrunk).x,
    ).toBe(952);
  });
