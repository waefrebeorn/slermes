...
const readStored = (): Record<string, StoredRect> =>
  readJson<Record<string, StoredRect>>(POSITIONS_KEY) ?? {};
...
function FloatingPane({ pane }: { pane: Contribution }) {
  ...
  const [collapsed, setCollapsed] = useState(
    () => readStored()[pane.id]?.collapsed ?? false,
  );
  ...
  const persist = useCallback(
    (next: FloatingRect, nextCollapsed: boolean) => {
      writeJson(POSITIONS_KEY, {
        ...readStored(),
        [pane.id]: { x: next.x, y: next.y, collapsed: nextCollapsed },
      });
    },
    [pane.id],
  );
  ...
  return (
    <div
      className={cn(
        "pointer-events-auto fixed z-45 flex flex-col overflow-hidden",
        HUD_SURFACE,
      )}
...
export function FloatingPanes() {
  ...
  const floating = panes.filter(
    (pane) =>
      paneChrome(pane).placement === FLOATING_PLACEMENT && !hidden.has(pane.id),
  );
