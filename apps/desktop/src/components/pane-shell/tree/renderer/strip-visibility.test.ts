...
import {
  resolveTabStripVisible,
  type StripPane,
  tabStripVisibleForZone,
} from "./strip-visibility";

const tile = (): StripPane => ({ collapsePane: false, placement: "main" });
const workspace = (): StripPane => ({
  collapsePane: false,
  placement: "main",
  uncloseable: true,
});
const toolPanel = (): StripPane => ({
  collapsePane: true,
  placement: "bottom",
});
const sideChrome = (): StripPane => ({
  collapsePane: false,
  placement: "right",
});
const hideOnlyChrome = (): StripPane => ({
  collapsePane: false,
  hideOnly: true,
  placement: "left",
});
...
describe("auto (no stored choice)", () => {
  it("gives a lone workspace no strip and a stack of two a strip", () => {
    ...
    expect(resolveTabStripVisible({ shown: [workspace(), sideChrome()] })).toBe(
      true,
    );
  });
...
describe("the stored choice", () => {
  it("overrides auto in both directions", () => {
    expect(
      resolveTabStripVisible({ mode: "always", shown: [workspace()] }),
    ).toBe(true);
    expect(
      resolveTabStripVisible({
        mode: "never",
        shown: [workspace(), sideChrome()],
      }),
    ).toBe(false);
  });
});
...
describe("no dead zone", () => {
  it("keeps the strip for a closeable tile even when the zone says never", () => {
    expect(resolveTabStripVisible({ mode: "never", shown: [tile()] })).toBe(
      true,
    );
  });
  ...
  it("keeps the strip for a lone tool panel even when the zone says never", () => {
    expect(
      resolveTabStripVisible({ mode: "never", shown: [toolPanel()] }),
    ).toBe(true);
  });
  ...
  it("keeps the strip for hide-only chrome even when the zone says never", () => {
    ...
    expect(
      resolveTabStripVisible({ mode: "never", shown: [hideOnlyChrome()] }),
    ).toBe(true);
    expect(
      resolveTabStripVisible({
        mode: "never",
        shown: [hideOnlyChrome(), hideOnlyChrome()],
      }),
    ).toBe(true);
  });
  ...
  it("still hides a zone that cannot strand anything", () => {
    ...
    expect(
      resolveTabStripVisible({ mode: "never", shown: [workspace()] }),
    ).toBe(false);
    expect(
      resolveTabStripVisible({
        mode: "never",
        shown: [toolPanel(), toolPanel()],
      }),
    ).toBe(false);
  });
});
