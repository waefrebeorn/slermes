...
function registerPane(id: string, data: Record<string, unknown>, title = id) {
  disposers.push(
    registry.register({ area: "panes", data, id, render: () => null, title }),
  );
}
...
function LiveTreeGroup({
  ...
  const zone = (
    node.type === "split" ? node.children[index] : node
  ) as GroupNode;
...
const tabEl = (paneId: string) =>
  globalThis.document.querySelector(`[data-tree-tab="${paneId}"]`);
...
describe("docked tool tile — collapsing keeps the restore chip", () => {
  ...
  it("tapping the strip gutter does not collapse a lone docked tile", () => {
    ...
    tap(
      globalThis.document.querySelector('[data-zone-tabstrip="g-routines"]')!,
    );
  ...
  it("chevron-collapse of a row-docked tile keeps the tab as a restore handle", () => {
    ...
    fireEvent.click(
      globalThis.document.querySelector(
        '[data-tree-group="g-routines"] button[aria-label="Minimize"]',
      )!,
    );
...
describe("a stacked tool zone collapsed in a row keeps the horizontal strip", () => {
  ...
  it("keeps both tab labels after minimize so either pane can be restored", () => {
    ...
    expect(
      globalThis.document.querySelector('[data-zone-tabstrip="g-tools"]'),
    ).toBeTruthy();
  });
});
