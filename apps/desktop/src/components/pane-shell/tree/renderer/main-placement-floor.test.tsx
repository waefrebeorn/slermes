...
const zoneWrapper = (groupId: string) =>
  document
    .querySelector<HTMLElement>(`[data-tree-group="${groupId}"]`)
...
describe("subtreeGone", () => {
  ...
  it("still collapses a tool zone whose panes a chrome toggle hid", () => {
    expect(
      subtreeGone(group(["terminal"], { id: "grp-tools" }), ctx(["terminal"])),
    ).toBe(true);
  });
  ...
  it("still collapses a main pane whose plugin has not registered yet", () => {
    ...
    expect(
      subtreeGone(
        group(["plugin-workspace:late"], { id: "grp-late" }),
        ctx(["plugin-workspace:late"]),
      ),
    ).toBe(true);
  });
});
