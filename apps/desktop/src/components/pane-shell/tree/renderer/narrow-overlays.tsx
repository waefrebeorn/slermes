...
export function NarrowOverlays() {
  ...
  const [reveal, setReveal] = useState<{ id: string; pinned: boolean } | null>(
    null,
  );
  ...
  useEffect(
    () =>
      revealActive ? pushEscapeLayer(ESCAPE_PRIORITY.narrowOverlay) : undefined,
    [revealActive],
  );
  ...
  useEffect(() => {
    ...
    const onToggle = (event: Event) => {
      ...
      const match = collapsiblesRef.current.find(
        (p) => p.id === id || paneChrome(p).revealAliases?.includes(id),
      );
      ...
      setReveal((current) => {
        ...
        return current?.id === match.id && current.pinned
          ? null
          : { id: match.id, pinned: true };
      });
    };
  ...
  const sideOf = (c: Contribution) =>
    paneChrome(c).placement === "left" ? "left" : "right";
  const revealed = reveal
    ? collapsibles.find((p) => p.id === reveal.id)
    : undefined;
  ...
  const zonePanes = (() => {
    ...
    const mates = zone
      ? zone.panes.map((id) => collapsibles.find((p) => p.id === id))
      : [];
      ...
      {sides.map((side) => (
        <div
          ...
          onMouseEnter={() => {
            ...
            if (first) {
              setReveal((current) =>
                current?.pinned ? current : { id: first.id, pinned: false },
              );
            }
          }}
        />
