...
export function snapshotZones(): EngineZone[] {
  return [...document.querySelectorAll<HTMLElement>("[data-tree-group]")].map(
    (el) => {
...
export function subZonePosition(
  zones: EngineZone[],
  groupId: string,
  x: number,
  y: number,
...
export function snapshotStrips(): StripSnapshot[] {
  return [
    ...document.querySelectorAll<HTMLElement>("[data-zone-tabstrip]"),
  ].map((el) => {
    const r = el.getBoundingClientRect();
  ...
  const publishHint = (next: DropHint | null) => {
    if (!sameHint($dropHint.get(), next)) {
      if (
        ...
        $dropHint.get()?.stack?.before !== next.stack.before
      ) {
        reorderStepHaptic();
      }
...
export function startPaneDrag(
  ...
  const moving: readonly string[] =
    selection && selection.length > 1 ? selection : [paneId];
  ...
  const markSource = () => {
    ...
    if (dimmed.length === 0 && reorder) {
      dimmed = moving
        .map((id) =>
          reorder.strip.querySelector<HTMLElement>(
            `[data-tree-tab="${CSS.escape(id)}"]`,
          ),
        )
  ...
  const withinStrip = (x: number, y: number) =>
    Boolean(reorder) &&
    rectContains(reorderStrip().rect, x, y, TEAR_OFF_SLACK_PX);
  ...
  startDragSession(e, {
    ...
    resolveMove(x, y, shift) {
      ...
      const groupId =
        groupIds.length > 0
          ? (primaryZone(zones, groupIds, point) ?? undefined)
          : undefined;
      ...
      const strip =
        groupIds.length === 1 && groupId
          ? strips.find(
              (s) => s.groupId === groupId && rectContains(s.rect, x, y),
            )
          : null;
      ...
      return groupIds.length > 0
        ? { kind: "group", groupId, groupIds, pos, stack }
        : null;
    },
    ...
    onCommit(hint) {
      ...
      if (mode === "reorder" && reorder && hint?.stack !== undefined) {
        ...
        const others = [
          ...reorder.strip.querySelectorAll<HTMLElement>("[data-tree-tab]"),
        ]
          .map((el) => el.dataset.treeTab)
          .filter((id): id is string => Boolean(id) && !moving.includes(id!));

        const toIndex = hint.stack.before
          ? others.indexOf(hint.stack.before)
          : others.length;
