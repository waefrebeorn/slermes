...
describe("focused chat zone drives the tab verbs", () => {
  ...
  async function setup() {
    ...
    tree.declareDefaultTree(
      model.split("row", [
        ...
        model.group(["session-tile:a", "session-tile:b"], {
  ...
  it("⌘W closes the focused zone active tab and leaves the workspace alone", async () => {
    ...
    expect(model.allPaneIds(tree.$layoutTree.get()!)).not.toContain(
      "session-tile:b",
    );
  ...
  it("⌘W and ⌃Tab serve a zone of preview/page tiles like any tab strip", async () => {
    ...
    tree.declareDefaultTree(
      model.split("row", [
        model.group(["workspace"], { active: "workspace", id: "grp-main" }),
        model.group(["preview-tile:url:x", "route-tile:/skills"], {
    ...
    expect(
      tree.$layoutTree.get() &&
        model.findGroup(tree.$layoutTree.get()!, "grp-view")?.active,
    ).toBe("route-tile:/skills");
