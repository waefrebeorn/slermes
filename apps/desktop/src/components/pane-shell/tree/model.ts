...
export const nodeId = (kind: string) =>
  `${kind}-${Date.now().toString(36)}-${(seq++).toString(36)}`;
...
export function findGroupOfPane(
  node: LayoutNode,
  paneId: string,
): GroupNode | null {
  if (node.type === "group") {
...
export function normalize(node: LayoutNode): LayoutNode | null {
  if (node.type === "group") {
    ...
    const active = node.panes.includes(node.active)
      ? node.active
      : node.panes[0];
  ...
  const walk = (n: LayoutNode): LayoutNode => {
    if (n.type === "group") {
      ...
      return {
        ...
        active:
          n.active === paneId
            ? (panes[Math.min(at, panes.length - 1)] ?? "")
            : n.active,
      };
    }
  ...
  const walk = (n: LayoutNode): LayoutNode => {
    if (n.type === "group") {
      ...
      if (pos === "center") {
        ...
        const panes =
          at >= 0
            ? [...n.panes.slice(0, at), paneId, ...n.panes.slice(at)]
            : [...n.panes, paneId];
...
function shapeSignature(node: LayoutNode): string {
  ...
  return children.length === 1
    ? children[0]
    : `${node.orientation}(${children.join("|")})`;
}
...
export function movePane(
  ...
  const next =
    insertAtGroup(without, target.groupId, paneId, target.pos, target.before) ??
    root;
...
export function groupLeafIds(node: LayoutNode): string[] {
  return node.type === "group"
    ? [node.id]
    : node.children.flatMap(groupLeafIds);
}
...
export function setActivePane(
  ...
  return mapGroups(root, (g) =>
    g.id === groupId && g.panes.includes(paneId) ? { ...g, active: paneId } : g,
  );
}
...
export function setGroupMinimized(
  root: LayoutNode,
  groupId: string,
  minimized: boolean,
): LayoutNode {
...
export function setGroupTabStrip(
  root: LayoutNode,
  groupId: string,
...
function replaceNode(
  ...
  return {
    ...node,
    children: node.children.map((c) => replaceNode(c, id, make)),
  };
}
...
export function setSplitWeights(
  ...
  if (root.type === "split") {
    ...
    return {
      ...root,
      children: root.children.map((c) => setSplitWeights(c, splitId, weights)),
    };
  }
...
export function migratePersistedTree(node: LayoutNode): LayoutNode {
  if (node.type === "group") {
    const { headerHidden, ...rest } = node as GroupNode & {
      headerHidden?: unknown;
    };
    const tabStrip =
      rest.tabStrip === "always" || rest.tabStrip === "never"
        ? rest.tabStrip
        : undefined;

    return headerHidden === undefined && rest.tabStrip === tabStrip
      ? node
      : { ...rest, tabStrip };
  }
