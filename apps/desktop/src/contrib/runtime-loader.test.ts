...
const readFileText =
  vi.fn<(path: string) => Promise<{ text: string; truncated?: boolean }>>();
const readPluginSource =
  vi.fn<(path: string) => Promise<{ text: string; truncated?: boolean }>>();
...
describe("scanDiskPlugins (#66899)", () => {
  it("scans the Electron-resolved local roots, never the backend hermes_home", async () => {
    ...
    expect(readDir).not.toHaveBeenCalledWith(
      "/remote/box/.hermes/desktop-plugins",
    );
  });
  ...
  it("treats a package without a Desktop half as metadata, not a throwing file read", async () => {
          ...
          entries: [
            {
              isDirectory: true,
              name: "my-feature",
              path: "/local/.hermes/plugins/my-feature",
            },
          ],
        };
      }
          ...
          entries: [
            {
              isDirectory: false,
              name: "plugin.yaml",
              path: "/local/.hermes/plugins/my-feature/plugin.yaml",
            },
          ],
        };
      }
    ...
    expect(readDir).not.toHaveBeenCalledWith(
      "/local/.hermes/plugins/my-feature/desktop",
    );
          ...
          entries: [
            {
              isDirectory: true,
              name: "odd",
              path: "/local/.hermes/desktop-plugins/odd",
            },
          ],
        };
      }
          ...
          entries: [
            {
              isDirectory: true,
              name: "plugin.js",
              path: "/local/.hermes/desktop-plugins/odd/plugin.js",
            },
          ],
        };
      }
  ...
  it("still scans the standalone root when agentPluginsRoot is absent (older shell)", async () => {
    delete (window.hermesDesktop as unknown as { agentPluginsRoot?: unknown })
      .agentPluginsRoot;
    desktopPluginsRoot.mockResolvedValue("/local/.hermes/desktop-plugins");
    readDir.mockResolvedValue({ entries: [] });
  ...
  it("loads a unified desktop half OPT-IN: inventoried but not activated by default", async () => {
          ...
          entries: [
            {
              isDirectory: true,
              name: "desktop",
              path: "/local/.hermes/plugins/uni/desktop",
            },
          ],
        };
      }
    ...
    (globalThis as unknown as { __uniRegister: unknown }).__uniRegister =
      register;
    ...
    try {
      ...
      expect($pluginRecords.get().uni).toMatchObject({
        kind: "disk",
        status: "disabled",
      });
      ...
      delete (globalThis as unknown as { __uniRegister?: unknown })
        .__uniRegister;
    }
  });
});
...
describe("watchRuntimePlugins dir watch (#66899)", () => {
  it("watches both Electron-resolved local roots, never the backend hermes_home", async () => {
    ...
    expect(watchDirectory).toHaveBeenCalledWith(
      "/local/.hermes/desktop-plugins",
    );
    expect(watchDirectory).toHaveBeenCalledWith("/local/.hermes/plugins");
    expect(watchDirectory).not.toHaveBeenCalledWith(
      "/remote/box/.hermes/desktop-plugins",
    );
    expect(getStatus).not.toHaveBeenCalled();
  });
...
describe("plugin source reads (512 KiB preview-cap bug)", () => {
  ...
  it("loads the full source via readPluginSource when the shell offers it", async () => {
    (
      window.hermesDesktop as unknown as { readPluginSource: unknown }
    ).readPluginSource = readPluginSource;
    desktopPluginsRoot.mockResolvedValue("/local/.hermes/desktop-plugins");
    agentPluginsRoot.mockResolvedValue("");
    standaloneRootWith("big");
    // The preview read would truncate this source — it must never be used.
    readFileText.mockResolvedValue({
      text: "// first 512 KiB only",
      truncated: true,
    });

    const register = vi.fn();

    (globalThis as unknown as { __bigRegister: unknown }).__bigRegister =
      register;
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith(
        "/local/.hermes/desktop-plugins/big/plugin.js",
      );
      expect(register).toHaveBeenCalledTimes(1);
      ...
      restore();
      delete (globalThis as unknown as { __bigRegister?: unknown })
        .__bigRegister;
    }
  });
  ...
  it("older shell, small plugin (not truncated): still loads through readFileText", async () => {
    ...
    (globalThis as unknown as { __smallRegister: unknown }).__smallRegister =
      register;
    readFileText.mockResolvedValue({
    ...
    try {
      ...
      expect(register).toHaveBeenCalledTimes(1);
      expect($pluginRecords.get().small).toMatchObject({
        kind: "disk",
        status: "loaded",
      });
    } finally {
      restore();
      delete (globalThis as unknown as { __smallRegister?: unknown })
        .__smallRegister;
    }
  });
});
...
describe("bundled-shadowed disk copies", () => {
  it("skips a disk copy of a bundled plugin but publishes a visible inventory row", async () => {
    ...
    try {
      ...
      expect(id).toBeNull();
      expect($pluginRecords.get()["hermes-bots"]).toMatchObject({
        kind: "bundled",
        status: "loaded",
      });
