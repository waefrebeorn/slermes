...
describe("createPluginContext.os", () => {
  it("dispatches a native notification attributed to the plugin", () => {
    ...
    expect(dispatchPluginNativeNotification).toHaveBeenCalledWith("demo", {
      body: "b",
      title: "t",
    });
  });
  ...
  it("resolves false (never throws) when the desktop bridge is missing", async () => {
    ...
    await expect(ctx.os.openExternal("https://example.com")).resolves.toBe(
      false,
    );
  ...
  it("file pickers return the chosen path, and null on cancel", async () => {
    ...
    try {
      ...
      await expect(ctx.os.pickSavePath({ title: "Save" })).resolves.toBe(
        "/tmp/out.tar.gz",
      );
      expect(bridge.selectSavePath).toHaveBeenCalledWith({ title: "Save" });

      await expect(ctx.os.pickOpenPath({ title: "Open" })).resolves.toBe(
        "/tmp/board.tar.gz",
      );
