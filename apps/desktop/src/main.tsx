...
if (
  ...
  import.meta.env.VITE_PERF_PROBE === "1"
) {
...
if (winParam === "overlay") {
  void import("./app/pet-overlay/overlay-root").then(({ mountPetOverlay }) =>
    mountPetOverlay(),
  );
} else if (winParam === "quick") {
  void import("./app/quick-entry/quick-entry-root").then(
    ({ mountQuickEntry }) => mountQuickEntry(),
  );
} else if (winParam === "wake") {
  void import("./app/wake-indicator/wake-indicator-root").then(
    ({ mountWakeIndicator }) => mountWakeIndicator(),
  );
} else {
