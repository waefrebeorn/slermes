  ...
  it("follows a focused session tile, not the primary", async () => {
    ...
    for (const id of ["workspace", "session-tile:tile-a"]) {
      registry.register({
        ...
        data:
          id === "workspace"
            ? { placement: "main", uncloseable: true }
            : { placement: "main" },
        id,
        render: () => null,
        title: id,
    ...
    tree.declareDefaultTree(
      model.split("row", [
        ...
        model.group(["session-tile:tile-a"], {
...
describe("host.connections", () => {
  ...
  it("folds the envelope-level primary id down onto the row that owns it", async () => {
    ...
    expect(
      (await host.connections()).map((entry) => [entry.id, entry.primary]),
    ).toEqual([
      ["local", false],
      ["homelab", true],
    ]);
  ...
  it("still rejects on a Desktop build without the connection registry", async () => {
    ...
    await expect(host.connections()).rejects.toThrow(
      "This Desktop build has no connection registry",
    );
  });
});
...
describe("host workspace scope", () => {
  ...
  it("registers plugin workspace chrome options", async () => {
    ...
    expect(
      registry
        .getArea("panes")
        .find((pane) => pane.id === "plugin-workspace:scope-test"),
    ).toMatchObject({
      data: {
        dock: { pane: "workspace", pos: "right" },
        headerVeto: true,
  ...
  it("publishes the active workspace scope through one host seam", async () => {
    const { $workspaceMode, $workspaceOwnerKey } =
  ...
  it("uses the shared tab action for an exact Bot owner without moving Sessions", async () => {
    const tree = await import("@/components/pane-shell/tree/store");
    const { $workspaceNewSessionTarget } =
      await import("@/components/pane-shell/workspace-scope");
    const opened: string[] = [];
    ...
    host.newChat(route, {
      workspaceMode: "bots",
      workspaceOwnerKey: "bot:connection-b::writer",
    });
