...
const readonlyAtom = <T>(atomLike: ReadableAtom<T>): ReadableAtom<T> =>
  atomLike;
  ...
  computed(
    ...
    (focused, selected, state, primary) =>
      !focused || focused === selected
        ? primary
        : Boolean(state && select(state)),
  );

const $focusedBusy = focusedTurnFlag(
  (state) => state.busy,
  PRIMARY_SESSION_VIEW.$busy,
);
...
const $focusedSessionOwner = computed(
  ...
  (
    ...
    for (const row of sessions.filter((session) =>
      sessionMatchesStoredId(session, focused),
    )) {
...
async function requestPluginProfile<T>(
  ...
  if (typeof route !== "string") {
    if (
      ...
      throw new Error(
        "Profile route must include connectionId, profile, and targetProfile",
      );
    }
  ...
  const soleLocalSource =
    roster.sources.length === 1 && roster.sources[0]?.kind === "local";
 ...
 *  identical. */
async function pluginRouteStillRegistered(
  ...
  try {
    const routes = await getProfileRoutes(
...
const $focusedUsage = computed(
  $focusedSessionState,
  (state) => state?.usage ?? null,
);
...
function waitForFocusedSessionHydration({
  ...
  return new Promise((resolve, reject) => {
    ...
    const check = () => {
      ...
      const storedTile = $sessionTiles
        .get()
        .find((tile) => tile.storedSessionId === storedSessionId);
      const tileMatches =
        $focusedStoredSessionId.get() === storedSessionId ||
        Boolean(storedTile);
      const focusedTileMatches =
        $focusedStoredSessionId.get() === storedSessionId;
      const tileRuntimeId = focusedTileMatches
        ? $focusedRuntimeId.get()
        : (storedTile?.runtimeId ?? null);
      ...
      new Promise<never>((_resolve, reject) => {
        ...
        timer = window.setTimeout(
          () =>
            reject(
              new Error(
                `Timed out loading ${targetProfile}'s session history.`,
              ),
            ),
          timeoutMs,
        );
...
export const host = {
  state: {
    ...
    focusedSessionOwner: readonlyAtom<PluginFocusedSessionOwner | null>(
      $focusedSessionOwner,
    ),
    ...
    focusedStoredSessionId: readonlyAtom<null | string>(
      $focusedStoredSessionId,
    ),
  ...
  deleteProfile: async (
    profile: string | PluginProfileRoute,
  ): Promise<void> => {
    const route =
    ...
    const name =
      typeof profile === "string" ? profile.trim() : route?.profile || "";

    if (
      route &&
      (!route.connectionId || !route.profile || !route.targetProfile)
    ) {
      throw new Error(
        "deleteProfile: route requires connectionId, profile, and targetProfile",
      );
    }
    ...
    // the deleted profile.
    const ambientConnectionId = route
      ? null
      : String(activeGatewayConnectionId() || "").trim();

    const ambientRemoteConnectionId =
      ambientConnectionId && ambientConnectionId !== "local"
        ? ambientConnectionId
        : null;
    ...
    const wasActive = route
      ? route.connectionId === ($activeConnectionId.get() || "") &&
        normalizeProfileKey(route.profile) ===
          normalizeProfileKey($activeGatewayProfile.get())
      : normalizeProfileKey(name) ===
        normalizeProfileKey($activeGatewayProfile.get());
  ...
  connections: async () => {
    ...
    if (!bridge) {
      throw new Error(
        "This Desktop build has no connection registry. Update Hermes Desktop.",
      );
    }

    const registryPayload = await bridge.list();
    const rows = Array.isArray(registryPayload?.connections)
      ? registryPayload.connections
      : [];

    return rows.map((connection) => ({
      ...connection,
      primary: connection.id === registryPayload.primary,
    }));
  },
  ...
  agents: async () => {
    ...
    if (!roster) {
      throw new Error(
        "This Desktop build cannot enumerate multi-source agents. Update Hermes Desktop.",
      );
    }
  ...
  openSession: async (
    ...
    const hydrationTimeoutMs = Math.max(
      1,
      options.hydrationTimeoutMs ?? DEFAULT_SESSION_HYDRATION_TIMEOUT_MS,
    );
    ...
    const maxAttempts =
      options.awaitHydration && options.retryHydrationTimeoutOnce ? 2 : 1;
    ...
    try {
      ...
      if (dial) {
        ...
        await (options.awaitHydration
          ? awaitProfileActivation(dial, targetProfile, hydrationTimeoutMs)
          : dial());
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
          ...
          if (
            ...
            (options.forceResume || !surfaceHealthy)
          ) {
            const existingTile = $sessionTiles
              .get()
              .some((tile) => tile.storedSessionId === storedSessionId);
            const tileDelegate = existingTile ? sessionTileDelegate() : null;

            if (tileDelegate) {
              try {
                await tileDelegate.resumeTile(storedSessionId, {
          ...
          if (options.awaitHydration) {
            await waitForFocusedSessionHydration({
              ...
              requireActiveProfile: ownerRoute
                ? false
                : plan.requireActiveProfileForHydration,
              storedSessionId,
          ...
          if (
            explicitRoute &&
            !(await pluginRouteStillRegistered(explicitRoute))
          ) {
            throw new Error(
              `The ${targetProfile} gateway is no longer available.`,
            );
          }
      ...
      if (
        ...
        console.warn("[bot-wake] hydration timed out", {
          ...
          hydrationWaitMs:
            wakePhase === "hydration" ? timedOutAt - profileActiveAt : 0,
          phase: wakePhase,
          profile: targetProfile,
          profileActivationMs:
            (wakePhase === "activation" ? timedOutAt : profileActiveAt) -
            wakeStartedAt,
  ...
  openWorkspace: (
    ...
    if (!key || typeof options?.render !== "function") {
      throw new Error(
        "openWorkspace: an id and a render function are required",
      );
    }
   ...
   *  "+" uses). */
  newChat: (
    profile?: null | string | PluginProfileRoute,
    options: PluginNewChatOptions = {},
  ): void => {
    if (options.workspaceMode === "bots") {
      if (
        !profile ||
        typeof profile === "string" ||
        !options.workspaceOwnerKey
      ) {
        notify({
          kind: "error",
          message: "Select a Bot before starting another chat.",
        });

        return;
      }

      publishWorkspaceScope("bots", options.workspaceOwnerKey, {
        kind: "route",
        route: { ...profile },
      });
    ...
    if (profile && typeof profile !== "string") {
      ...
      newSessionInProfile(
        (profile ?? "").trim() || $activeGatewayProfile.get(),
      );
    }
  ...
  paneVisibility: (paneId: string): ReadableAtom<boolean> =>
    $paneVisible(paneId),
  ...
  setPersistedSessionHidden: async (
    ...
    if (!profile || !options.sessionId.trim()) {
      throw new Error(
        "Persisted session updates require a profile and session id",
      );
    }
...
} from "@/app/chat/session-status-dot";
...
} from "@/app/chat/sidebar/row-geometry";
export {
  PALETTE_AREA,
  type PaletteContribution,
} from "@/app/command-palette/contrib";
...
export {
  type BudgetedLoop,
  type BudgetedLoopOptions,
  createBudgetedLoop,
} from "@/lib/budgeted-loop";
