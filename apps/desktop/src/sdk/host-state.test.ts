...
describe("host.state focused-session atoms", () => {
  ...
  it("mirrors the primary session while no tile is focused", async () => {
    ...
    expect(host.state.focusedSessionId.get()).toBe(
      states.$focusedRuntimeId.get(),
    );
    expect(host.state.focusedStoredSessionId.get()).toBe(
      states.$focusedStoredSessionId.get(),
    );
  });
  ...
  it("follows the interacted tile while the primary-only atom stays put", async () => {
    ...
    states.$sessionTiles.set([
      { storedSessionId: "tile-a", runtimeId: "runtime-tile-a" },
    ]);
    states.$sessionStates.set({
      "runtime-tile-a": {
        storedSessionId: "tile-a",
        usage: tileUsage,
      } as never,
    });
...
describe("host.state busy vs gateway", () => {
  ...
  it("exposes per-session turn-busy and does not treat gateway as busy", () => {
    ...
    expect(host.state.busyBySession.get()).toEqual({
      "runtime-a": true,
      "runtime-b": false,
    });
