...
export const ja = defineLocale({
  ...
  boot: {
    ...
    errors: {
      ...
      backgroundExitedDuringStartup:
        "起動中に Hermes バックグラウンドプロセスが終了しました。",
    ...
    failure: {
      ...
      openLogs: "ログを開く",
      repairHint:
        "修復はインストーラーを再実行します。新しいマシンでは数分かかる場合があります。",
  ...
  notifications: {
    ...
    errors: {
      ...
      diskFull:
        "ディスク容量不足です — 空きを作ってからもう一度お試しください。",
      gatewayAuthFailed:
        "ゲートウェイ認証に失敗しました — API_SERVER_KEY を確認してください。",
      methodNotAllowed:
  ...
  billingBlock: {
    ...
    fallbackMessage:
      "アカウントのクレジットが不足しています。続行するにはクレジットを追加してください。",
    openBilling: "請求を開く",
    addCredits: "クレジットを追加",
    dismiss: "閉じる",
  },
  ...
  sendDiagnostics: {
    ...
    copyLink: "リンクをコピー",
    uploadIdFallback: (id) =>
      `表示リンクが返されませんでした — サポートにアップロード ID ${id} をお伝えください`,
  ...
  titlebar: {
    ...
    unreadSessions: (count) =>
      count === 1 ? "未読セッション 1 件" : `未読セッション ${count} 件`,
  ...
  settings: {
    ...
    notifications: {
      ...
      focusedHint:
        "完了通知は Hermes がバックグラウンドにあるときのみ表示されます。",
      kinds: {
        approval: {
          label: "承認が必要",
        ...
        input: {
          label: "入力が必要",
          description:
            "Hermes が質問したか、パスワードやシークレットを必要としています。",
        },
        turnDone: {
          label: "応答完了",
          description:
            "Hermes がバックグラウンドのときにターンが完了しました。",
        },
        turnError: {
          label: "ターン失敗",
        ...
        plugin: {
          label: "プラグイン通知",
          description:
            "Hermes がバックグラウンドの間に、デスクトッププラグインが通知を送信しました。",
        },
      },
      test: "テスト通知を送信",
      testTitle: "Hermes",
      ...
      completionSoundDesc:
        "エージェントのターン終了時に再生されます。プリセットを選んでここで試聴できます。",
    ...
    appearance: {
      ...
      translucencyDesc:
        "テキストも含めウィンドウ全体を透過させてデスクトップを表示します。",
      translucencyGlassDesc:
        "マットガラス: デスクトップが滑らかなぼかしとして透け、テキストは鮮明なまま。",
      ...
      toursTitle: "ガイドツアー",
      toursDesc:
        "画面を暗くして各ステップを強調しながら、Hermes がアプリを案内します。",
      composerPopoutTitle: "フローティング入力欄",
      composerPopoutDesc:
        "入力欄をドックからドラッグして外せるようにします。オフにすると画面下部に固定されます。",
      ...
      themeTitle: "テーマ",
      themeDesc:
        "デスクトップ専用のパレットです。選択したモードの上に適用されます。",
      ...
      pet: {
        ...
        scaleTitle: "サイズ",
        scaleDesc:
          "浮遊マスコットの大きさを変更します。すべての画面に即時反映されます。",
        roamTitle: "散歩",
        ...
        chooseTitle: "ペットを選ぶ",
        chooseDesc:
          "選ぶと（必要に応じて）インストールされ、アクティブになります。",
        searchPlaceholder: "ペットを検索…",
        unreachable:
          "petdex ギャラリーに接続できませんでした。接続を確認してこのページを開き直してください。",
        noMatch: (query) => `「${query}」に一致するペットがありません。`,
        installedTag: "インストール済み",
        generatedTag: "生成",
        countCapped: (cap, total) =>
          `${total} 件中 ${cap} 件を表示中——入力して絞り込めます。`,
        count: (n) => `${n} 件のペット。`,
    ...
    fieldDescriptions: defineFieldCopy({
      model:
        "コンポーザーで別のモデルを選ばない限り、新しいチャットで使用されます。",
      modelContextLength:
        "0 のままにすると、選択したモデルから検出されたコンテキストウィンドウを使用します。",
      fallbackProviders:
        "デフォルトモデルが失敗したときに試す provider:model 形式のバックアップです。",
      display: {
      ...
      desktop: {
        repoScanEnabled:
          "ローカルフォルダを検索して Git リポジトリをプロジェクトに表示します。",
        repoScanRoots:
          "検索するフォルダです。空の場合はホームディレクトリを検索します。",
        repoScanExcludePaths:
          "リポジトリ検出時に除外するフォルダとその配下です。",
      },
      timezone:
      ...
      agent: {
        ...
        maxTurns:
          "Hermes が 1 回の実行を停止するまでのツール呼び出しターン上限です。",
      },
      terminal: {
        cwd: "ツールとターミナル作業のデフォルトプロジェクトフォルダーです。",
        persistentShell:
          "バックエンドが対応している場合、コマンド間でシェル状態を保持します。",
      ...
      fileReadMaxChars:
        "Hermes が 1 回のファイル読み取りで取得できる最大文字数です。",
      approvals: {
      ...
      security: {
        redactSecrets:
          "検出したシークレットを、可能な限りモデルから見える内容から隠します。",
      },
      checkpoints: {
      ...
      stt: {
        enabled:
          "ローカルまたはプロバイダーによる音声文字起こしを有効にします。",
        elevenlabs: {
          languageCode:
            "任意の ISO-639-3 言語コードです。空欄なら ElevenLabs が自動検出します。",
        },
      },
      updates: {
    ...
    about: {
      ...
      updateReady: (count) =>
        `新しい更新の準備ができました (${count} 件の変更を含みます)。`,
      updateReadyUnknown: "新しい更新の準備ができました。",
      lastChecked: (age) => `前回確認: ${age}`,
      justNowSuffix: " · たった今",
      automaticUpdates: "自動更新",
      automaticUpdatesDesc:
        "Hermes はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。",
      branchCommit: (branch, commit) =>
        `ブランチ ${branch} · コミット ${commit}`,
      never: "未確認",
      justNow: "たった今",
    ...
    config: {
      ...
      keepAwakeDesc:
        "本体のスリープを防ぎ、長時間や夜通しの実行を継続します。画面は暗転できます。",
    },
    quickEntry: {
      enabledTitle: "クイック入力",
      enabledDesc:
      ...
      shortcutDesc:
        "修飾キーが 1 つ以上必要です（例: CommandOrControl+Shift+Space）。",
      active: "ショートカットは有効です。",
      takenBy:
        "このショートカットは他のアプリが使用しています。別のものを選んでください。",
      invalidShortcut:
        "有効なショートカットではありません。修飾キーを 1 つ以上含めてください。",
    },
    credentials: {
      pasteKey: "キーを貼り付け",
    ...
    gateway: {
      ...
      unavailableDesc:
        "デスクトップ IPC ブリッジはゲートウェイ設定を公開していません。",
      title: "ゲートウェイ接続",
      ...
      keychainEncryptionFailed:
        "シークレット暗号化の設定を変更できませんでした",
      testRemote: "リモートをテスト",
      ...
      incompleteSignIn:
        "リモートに切り替える前にリモート URL を入力してサインインしてください。",
      incompleteToken:
        "リモートに切り替える前にリモート URL とセッショントークンを入力してください。",
      incompleteSignInTest:
        "テストする前にリモート URL を入力してサインインしてください。",
      incompleteTokenTest:
        "テストする前にリモート URL とセッショントークンを入力してください。",
      enterUrlFirst: "最初にリモート URL を入力してください。",
      restartingTitle: "ゲートウェイ接続を再起動中",
      savedTitle: "ゲートウェイ設定を保存しました",
      restartingMessage:
        "Hermes Desktop は保存された設定を使用して再接続します。",
      savedMessage: "次回起動時に保存されます。",
      connectedTo: (baseUrl, version) =>
        `${baseUrl}${version ? ` · Hermes ${version}` : ""} に接続しました`,
      ...
      sshTrustHint:
        "初回に提示されたホスト鍵を信頼して固定し、以後の変更は拒否します。",
      sshHostTitle: "ホスト",
      sshHostDesc: "user@host、または ~/.ssh/config の Host エイリアス。",
      sshHostPick: "ホストを選択…",
      sshHostPickTitle: "ホスト",
      sshHostPickDesc:
        "~/.ssh/config の Host エイリアス、または「カスタム」で手入力。",
      ...
      sshHermesPathDesc:
        "リモートの hermes バイナリへのフルパス。空欄 = 自動検出。",
      sshHermesPathPlaceholder: "自動検出",
      sshTestConnection: "SSH をテスト",
      sshConnect: "接続",
      sshButtonsHint:
        "「保存」は次回起動時に適用され、「接続」は今すぐ再接続します。",
      sshReachable: (host, platform) =>
        `接続可能: ${host}（${platform}）— Hermes を検出`,
      sshIncompleteHost: "接続する前に SSH ホストを入力してください。",
      sshErrUnreachable:
        "SSH でそのホストに到達できませんでした。ホスト、ポート、ネットワークを確認してください。",
      sshErrAuth:
      ...
      sshErrTimeout:
        "SSH 接続がタイムアウトしました。ホストが到達不能、またはスリープ中の可能性があります。",
    ...
    profileScope: {
      ...
      editsProfile: (profile) =>
        `このページの変更は「${profile}」プロファイルに適用されます。`,
    },
    mcp: {
      ...
      gatewayUnavailableMessage:
        "MCP を再読み込みする前にゲートウェイを再接続してください。",
      ...
      emptyDesc:
        "MCP ツールを公開するには stdio または HTTP サーバーを追加してください。",
      disabled: "無効",
      editServer: "サーバーを編集",
      name: "名前",
      ...
      deepLinkNameInvalid:
        "名前は 1〜64 文字の英数字、ドット、ハイフン、アンダースコアです。",
      ...
      deepLinkErrorConfig:
        "リンクの設定が有効な base64 エンコード JSON ではありません。",
      deepLinkErrorShape:
        "設定は文字列の `url` または `command` フィールドを持つ JSON オブジェクトである必要があります。",
      deepLinkErrorUrl:
        "サーバー URL は http:// と https:// のみ許可されます。",
      deepLinkErrorTooLarge: "設定ペイロードが 32KB の上限を超えています。",
      importButton: "インポート",
      importPlaceholder:
        "mcp.json スニペット、npx/docker コマンド、claude mcp add 行、URL、Cursor リンクを貼り付け…",
      importNoMatch:
        "貼り付けたテキストからサーバー設定を認識できませんでした。",
    ...
    localModels: {
      ...
      serverStopped:
        "ローカルサーバーを停止しました——GPU メモリを解放しました。",
    ...
    providers: {
      ...
      removeKeyManaged: (provider) =>
        `${provider} は API キーで設定されています。API Keys から削除してください。`,
      ...
      localEndpoint: {
        ...
        description:
          "OpenAI 互換のエンドポイント（Zyphra、vLLM、llama.cpp、Ollama など）を指定します。",
      },
    ...
    toolsets: {
      ...
      nousIncluded:
        "Nous サブスクリプションに含まれています。有効にするには Nous Portal にサインインしてください。",
      ...
      postSetupInstalledHint:
        "インストール済みです。問題がある場合のみセットアップを再実行してください。",
      ...
      terminalBackend: {
        ...
        selectedMessage: (backend) =>
          `ターミナルコマンドは ${backend} で実行されます。新しいセッションに適用されます。`,
        failedSelect: (backend) => `${backend} の選択に失敗しました`,
        needsSetupHint:
          "このバックエンドは今すぐ選択できますが、セットアップが完了するまでコマンドは失敗します。",
      },
      browserRealProfile: {
  ...
  agents: {
    ...
    emptyDesc:
      "ターンで作業を委任すると、子エージェントの進捗状況がここにストリームされます。",
    running: "実行中",
    failed: "失敗",
    done: "完了",
    streaming: "ストリーミング中",
  ...
  commandCenter: {
    ...
    generatePet: {
      ...
      backgroundHint:
        "このウィンドウは閉じても大丈夫です。完了したら Hermes が通知します。",
      slowProviderHint: "数分かかることがあります",
      remix: "リミックス",
      remixConfirmTitle: "この見た目でリミックスしますか？",
      remixConfirmBody:
        "これを起点に新しい候補を生成します。数分かかることがあります。",
      genericError:
        "生成に失敗しました。もう一度試すか、候補を選んでください。",
      referenceImageTooLarge:
        "参照画像が大きすぎます。16 MB 未満の画像を使ってください。",
      referenceImageInvalid:
        "参照画像を読み込めませんでした。PNG/JPG/WebP/GIF を試してください。",
      adopt: "迎え入れる",
      startOver: "やり直す",
    },
    installTheme: {
    ...
    hermesActiveSessions: (version, count) =>
      `Hermes ${version} · アクティブセッション ${count}`,
  ...
  messaging: {
    ...
    unknown: "不明",
    hintPendingRestart:
      "この変更を適用するにはステータスバーからゲートウェイを再起動してください。",
    hintGatewayStopped:
      "ステータスバーからゲートウェイを起動して接続してください。",
  ...
  profiles: {
    close: "プロファイルを閉じる",
    nameHint:
      "小文字、数字、ハイフン、アンダースコア。文字または数字で始める必要があります。",
    title: "プロファイル",
    ...
    remoteOverride: {
      ...
      description:
        "このプロファイルのセッションは、このパソコンではなく指定したリモートの Hermes で実行されます。",
      urlLabel: "リモートアドレス",
      urlPlaceholder: "https://hermes.example.com",
      urlInvalid:
        "http:// または https:// で始まる完全なアドレスを入力してください",
      tokenLabel: "アクセストークン",
      tokenPlaceholder: "リモートのセッショントークンを貼り付け",
      tokenSavedHint:
        "トークンは保存済みです。空欄のままにすると保持されます。",
      plainTextOptIn:
    ...
    notSet: "未設定",
    soulDesc:
      "このプロファイルに組み込まれたシステムプロンプトとペルソナの指示。",
    soulOptional: "省略可能",
    ...
    deleting: "削除中...",
    createDesc:
      "プロファイルは独立した Hermes 環境です：設定、スキル、SOUL.md が別々になります。",
    nameLabel: "名前",
    cloneFrom: "複製元",
    cloneFromNone: "なし（空）",
    cloneFromDesc:
      "選択したプロファイルから設定、スキル、SOUL.md をコピーします。",
    cloneFromDefault: "デフォルトプロファイルから設定を複製",
    cloneFromDefaultDesc:
      "デフォルトプロファイルから設定、スキル、SOUL.md をコピーします。",
  ...
  cron: {
    ...
    modelImpact: {
      ...
      message: (count) =>
        `モデル設定を確認するまで、${count} 件のスケジュール済みジョブがスキップされます。`,
      ...
      confirmAction: "確認",
      declined:
        "モデル変更をキャンセルしました — データ学習ティアの警告を拒否しました。",
    },
    search: "Cron ジョブを検索...",
    ...
    editDesc:
      "スケジュール、プロンプト、または配信先を更新します。変更は次回の実行時に適用されます。",
    createDesc:
    ...
    blueprints: {
      ...
      emptyDesc:
        "このバックエンドで利用できる自動化ブレーンプリントはありません。",
    },
  },
  ...
  artifacts: {
    ...
    noArtifactsDesc:
      "セッションで生成された画像やファイルの出力がここに表示されます。",
  ...
  sidebar: {
    ...
    allPinned:
      "ここにあるものはすべてピン留めされています。チャットのピン留めを解除すると最近のものに表示されます。",
    ...
    projects: {
      ...
      convertBranchDesc:
        "チェックアウト済みのブランチを開くか、空いているブランチのワークツリーを作成します。",
      ...
      removeWorktreeFailed:
        "ワークツリーを削除できませんでした（コミットされていない変更？）",
  ...
  composer: {
    ...
    attachUrlDesc:
      "Hermes がページを取得し、このターンのコンテキストとして含めます。",
    ...
    snippets: {
      ...
      implementationPlan: {
        label: "実装計画",
        description:
          "コードに手をつける前にアプローチを概説して、差分を集中させます。",
        text: "コードを変更する前に簡潔な実装計画を立ててください。",
      },
      explainThis: {
        label: "これを説明する",
        description:
          "選択したコードがどのように機能するかを説明し、主要なファイルにリンクします。",
  ...
  statusStack: {
    ...
    coding: {
      ...
      revertAll: "すべて取り消し",
      revertConfirm:
        "このファイルの変更を破棄してコミット済みの状態に戻しますか？この操作は元に戻せません。",
      revertAllConfirm:
        "すべての変更を破棄してコミット済みの状態に戻しますか？この操作は元に戻せません。",
      staged: "ステージ済み",
      noChanges: "変更なし",
      ...
      openPr: "PR を開く",
      ghMissing:
        "PR を開くには GitHub CLI (gh) をインストールしてサインインしてください",
  ...
  updates: {
    ...
    unsupportedMessage:
      "このバージョンの Hermes はアプリ内から自分を更新できません。",
    ...
    availableBody:
      "新しいバージョンの Hermes をインストールする準備ができています。",
    availableTitleBackend: "バックエンドの更新があります",
    availableBodyBackend:
      "接続中の Hermes バックエンドの新しいバージョンをインストールできます。",
    ...
    applyingBodyBackend:
      "リモートバックエンドが更新を適用して再起動します。復帰すると Hermes が自動的に再接続します。",
    applyingClose:
      "このウィンドウは更新中に閉じ、その後 Hermes が自動的に再度開きます。",
  ...
  install: {
    ...
    installLocalDesc:
      "Hermes をダウンロードし、Python 環境を作成して、このコンピューターでバックエンドを実行します。",
    ...
    remoteUrlDesc:
      "Hermes ゲートウェイのベース URL を使用します。リモートの場合は https:// を含めてください。",
    ...
    authTitle: "認証",
    authNeedsOauth: (provider) =>
      `このゲートウェイをテストする前に ${provider} でサインインしてください。`,
    ...
    signInIncomplete:
      "認証が完了する前にサインインウィンドウが閉じられました。",
    tokenTitle: "セッショントークン",
    tokenDesc:
      "リモートゲートウェイの .env ファイルからセッショントークンを貼り付けます。",
    pasteSessionToken: "セッショントークンを貼り付け",
    incompleteSignInTest:
      "OAuth で保護されたこのゲートウェイをテストする前にサインインしてください。",
    incompleteTokenTest:
      "このゲートウェイをテストする前にセッショントークンを入力してください。",
    testConnection: "接続をテスト",
    testSucceeded: (baseUrl, version) =>
      `${baseUrl}${version ? ` (${version})` : ""} に接続しました。`,
    applyRemote: "適用して再接続",
    backToSetup: "戻る",
  ...
  onboarding: {
    ...
    headerDesc:
      "チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。",
    preparingInstall:
      "Hermes はインストールを完了中です。初回実行では通常 1 分以内に完了します。",
    ...
    connected: "接続済み",
    featuredPitch:
      "1 つのサブスクリプションで 300 以上の最先端モデル — Hermes を実行するための推奨方法",
    fireworksPitch: "直接モデル API — Fireworks がホストする最先端モデル",
    localModelsTitle: "モデルをローカルで実行",
    localModelsPitch:
      "アカウント不要——モデルをダウンロードしてこのマシンで実行",
    ...
    apiKeyOptions: {
      ...
      openrouter: {
        short: "1 つのキーで多くのモデル",
        description:
          "1 つのキーで数百のモデルをホスト。新規インストールのデフォルトとして最適。",
      },
      openai: {
        short: "GPT クラスのモデル",
    ...
    connectedPicking: (provider) =>
      `${provider} が接続されました。デフォルトモデルを選択中...`,
    ...
    deviceCodeOpened: (provider) =>
      `${provider} をブラウザーで開きました。そこにこのコードを入力してください:`,
  ...
  modelPicker: {
    ...
    proNeedsSubscription:
      "Pro モデルには有料の Nous サブスクリプションが必要です。",
    free: "無料",
    freeTier: "無料プラン",
  ...
  shell: {
    ...
    statusbar: {
      ...
      systemResources: {
        ...
        ram: "RAM",
        unifiedNote:
          "ユニファイドメモリ——GPU とシステムがこのプールを共有します。",
        toggle: "システムリソース",
      },
      contextUsagePanel: {
        categories: {
      ...
      session: "セッション",
      yoloOn:
        "YOLO オン — 危険なコマンドを自動承認中。Shift+クリックで全体に切り替え。",
      ...
      modelPinned:
        "手動で固定中 — 新しいチャットは設定のデフォルトではなくこのモデルを使用します",
  ...
  rightSidebar: {
    ...
    noProjectBody:
      "プロジェクトを開くと、ファイルの閲覧と変更の確認ができます。",
    noProjectOpen: "プロジェクト未選択",
    noDiffs: "差分なし",
    unreadableTitle: "読み取り不可",
    unreadableBody: (error) =>
      `このフォルダーを読み取れませんでした (${error})。`,
    emptyTitle: "空",
    emptyBody: "このフォルダーは空です。",
    treeErrorTitle: "ツリーエラー",
    treeErrorBody:
      "ファイルツリーがこのフォルダーのレンダリング中にエラーが発生しました。",
    tryAgain: "再試行",
  ...
  preview: {
    ...
    sourceLineTitle:
      "クリックして選択 · Shift クリックで拡張 · コンポーザーにドラッグ",
    source: "ソース",
    ...
    console: {
      ...
      sentMessage: (count) =>
        `${count} 件のログエントリーがコンポーザーに追加されました`,
    },
    web: {
      ...
      lookingRestart: (taskId) =>
        `Hermes は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: "プレビューサーバーを再起動中",
      restartingMessage:
        "Hermes はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。",
      startRestartFailed: (message) =>
        `サーバー再起動を開始できませんでした: ${message}`,
      ...
      blankPageBody:
        "上のアドレス欄に入力するか、Hermes にページを開くよう頼んでください。",
      finishedRestarting: (message) =>
        `Hermes がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ""}`,
      failedRestarting: (message) =>
        `サーバーの再起動に失敗しました: ${message}`,
      unknownError: "不明なエラー",
      ...
      fileChanged: (url) =>
        `ファイルが変更され、プレビューを再読み込み中: ${url}`,
      filesChanged: (count, url) =>
        `${count} 件のファイルが変更され、プレビューを再読み込み中: ${url}`,
      watchFailed: (message) =>
        `プレビューファイルを監視できませんでした: ${message}`,
  ...
  zones: {
    ...
    editorHintPost:
      " で線の向きを反転 · ゾーンをまたいでドラッグで結合 · 共有辺をドラッグでリサイズ",
    templateColumns: "列",
    templateRows: "行",
    templateGrid: "グリッド",
    templatePriority: "優先",
    ...
    saveApply: "保存して適用",
    notExpressible:
      "この配置は互いに噛み合っています（風車型）— 入れ子の分割では表現できません",
  ...
  assistant: {
    thread: {
      ...
      restoreBody:
        "このプロンプト以降のメッセージは会話から削除され、ここからプロンプトが再実行されます。",
  ...
  desktop: {
    ...
    noProfileNamed: (target, available) =>
      `"${target}" という名前のプロファイルはありません。利用可能: ${available}`,
    newChatsProfile: (name) =>
      `新しいチャットはプロファイル ${name} を使用します。`,
    ...
    editTurnUnavailable:
      "このターンはサーバー履歴にありません（圧縮で削除された可能性があります）。",
    ...
    readOnlyTranscriptSendBlocked:
      "このチャットは読み取り専用トランスクリプトとして開いています。送信は無効です。",
    ...
    branchStopCurrent:
      "このチャットをブランチする前に現在のターンを停止してください。",
    ...
    restartToUseSaveImage:
      "画像を保存するには Hermes Desktop を再起動してください。",
    restartToSaveImages:
      "画像を保存するには Hermes Desktop を再起動してください",
    ...
    handoff: {
      pickPlatform: "送信先を選択",
      success: (platform) =>
        `${platform} に引き継ぎました。いつでもここで再開できます。`,
      systemNote: (platform) =>
        `↻ ${platform} に引き継ぎました — いつでもここで再開できます。`,
      failed: (error) => `引き継ぎに失敗しました: ${error}`,
      timedOut:
        "ゲートウェイの待機がタイムアウトしました。`hermes gateway` は起動していますか？",
    },
  },
  ...
  errors: {
    ...
    boundaryDesc:
      "ビューで予期しないエラーが発生しました。チャットと設定は安全です。",
    reloadWindow: "ウィンドウを再読み込み",
    openLogs: "ログを開く",
  },
