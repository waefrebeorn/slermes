...
const ruNum = (count: number | string) =>
  typeof count === "string" ? Number(count) || 0 : count;
...
export const ru = defineLocale({
  ...
  fileMenu: {
    ...
    deleteBody:
      "Элемент будет перемещён в корзину — его можно восстановить оттуда.",
  ...
  boot: {
    ...
    errors: {
      ...
      backgroundExitedDuringStartup:
        "Фоновый процесс Hermes завершился при запуске.",
    ...
    failure: {
      ...
      repairHint:
        "Восстановление перезапускает установщик — на чистой машине это может занять несколько минут.",
      ...
      remoteFailureHint:
        "Проверьте URL шлюза и вход в настройках шлюза или переключитесь на локальный шлюз.",
      ...
      signInIncompleteMessage:
        "Окно входа закрылось до завершения аутентификации.",
  ...
  notifications: {
    ...
    mcp: {
      ...
      needsAuthMessage: (name) =>
        `Для MCP ${name} требуется повторная аутентификация.`,
      errorTitle: "Сервер MCP недоступен",
      errorMessage: (name) =>
        `MCP ${name} не прошёл проверку работоспособности.`,
      signIn: "Войти",
      view: "Просмотр",
    },
    errors: {
      ...
      gatewayAuthFailed:
        "Аутентификация шлюза не удалась — проверьте API_SERVER_KEY.",
      methodNotAllowed:
      ...
      openaiTtsNeedsKey:
        "Для TTS OpenAI нужен VOICE_TOOLS_OPENAI_KEY или OPENAI_API_KEY.",
    },
    voice: {
      configureSpeechToText:
        "Настройте распознавание речи, чтобы использовать голосовой режим.",
      ...
      microphoneConstraintsUnsupported:
        "Ограничения микрофона не поддерживаются этим устройством.",
      ...
      microphoneUnsupported:
        "Это окружение не поддерживает запись с микрофона.",
      ...
      sayStopToEnd: (phrase) =>
        `Скажите «${phrase}», чтобы закончить голосовой чат.`,
  ...
  billingBlock: {
    ...
    fallbackMessage:
      "В вашем аккаунте закончились кредиты. Пополните баланс, чтобы продолжить.",
  ...
  titlebar: {
    ...
    layoutEditorTitle: (mod) =>
      `Редактор раскладки — ${mod}-клик сбрасывает раскладку`,
  },
  keybinds: {
    title: "Горячие клавиши",
    subtitle: (open) =>
      `Нажмите на сочетание, чтобы переназначить · ${open} снова открывает эту панель.`,
    ...
    actions: {
      ...
      "hud.snapToPointer":
        "Переместить HUD под курсор (глобально, пока HUD открыт)",
  ...
  settings: {
    ...
    plugins: {
      ...
      blurb:
        "Встроенные или добавленные в папку desktop-plugins. Отключите, чтобы выгрузить без перезапуска.",
      ...
      agent: {
        ...
        updateBackendToManage:
          "Обновите бэкенд Hermes, чтобы управлять этим плагином из приложения.",
        sources: {
          bundled: "встроенный",
      ...
      installModal: {
        ...
        description:
          "Перед установкой посмотрите, что содержит этот репозиторий.",
        repoLabel: "Репозиторий",
        includesHeading: "Состав пакета",
        agentLabel: "Плагин агента",
        desktopLabel: "UI приложения",
        agentTargetLocal: (profile) =>
          `Устанавливается в локальный бэкенд ${profile} (~/.hermes/plugins/)`,
        agentTargetRemote: (profile) =>
          `Устанавливается в подключённый бэкенд ${profile}`,
        desktopTarget:
          "Устанавливается в локальную папку desktop-plugins этого приложения",
        desktopOnlyNote:
          "Пакеты только для приложения не устанавливают плагин агента.",
        insecureWarning:
        ...
        forceReinstall:
          "Принудительная переустановка (заменить, если уже установлен)",
        install: "Установить",
        installing: "Установка…",
        probing: "Осмотр репозитория…",
        probeUnavailable: "Осмотр плагинов недоступен в этом окружении.",
        desktopUnavailable:
          "Установка плагинов приложения недоступна в этом окружении.",
        ...
        missingEnv: (vars) =>
          `Не хватает переменных окружения: ${vars}. Добавьте их в Настройки → Ключи.`,
      },
    },
    notifications: {
      title: "Уведомления",
      intro:
        "Системные уведомления (не всплывающие внутри приложения). Настраивается для каждого устройства.",
      enableAll: "Включить уведомления",
      enableAllDesc: "Если выключено — все уведомления ниже будут заглушены.",
      focusedHint:
        "Оповещения о завершении срабатывают только пока Hermes в фоне.",
      kinds: {
        approval: {
        ...
        plugin: {
          ...
          description:
            "Плагин приложения отправил уведомление, пока Hermes был в фоне.",
        },
      },
      ...
      completionSoundDesc:
        "Воспроизводится, когда ход агента завершён. Выберите пресет и прослушайте здесь.",
    ...
    appearance: {
      title: "Внешний вид",
      intro:
        "Только для приложения. Режим — это яркость, тема — палитра и оформление чата.",
      colorMode: "Цветовой режим",
      colorModeDesc:
        "Выберите фиксированный режим или позвольте Hermes следовать настройкам системы.",
      toolViewTitle: "Отображение вызовов инструментов",
      toolViewDesc:
        "Режим «Продукт» скрывает сырые данные инструментов, «Технический» показывает полный вход/выход.",
      ...
      sessionDensityDesc:
        "Выберите, сколько контекста показывать под заголовками сеансов в боковой панели.",
      ...
      tabStripDesc:
        "Показывать вкладки над зоной. Автоматически скрываются, когда в зоне только одна панель.",
      tabStripAuto: "Авто",
      ...
      reactionsDesc:
        "Эмодзи-тапбеки в стиле iMessage — реагируйте на сообщения, и Hermes сможет реагировать на ваши.",
      composerPopoutTitle: "Плавающий композер",
      composerPopoutDesc:
        "Позволяет вытягивать композер из его док-зоны. Отключите, чтобы он был закреплён снизу.",
      ...
      technicalDesc:
        "Показывать сырые аргументы/результаты инструментов и низкоуровневые детали.",
      themeTitle: "Тема",
      themeDesc:
        "Только палитры для приложения. Выбранный режим применяется поверх.",
      themeProfileNote: (profile) =>
        `Сохранено для профиля ${profile} — у каждого профиля своя тема.`,
      ...
      pet: {
        ...
        scaleTitle: "Размер",
        scaleDesc:
          "Меняет размер парящего питомца. Применяется мгновенно везде.",
        roamTitle: "Блуждание",
        roamDesc:
          "Позволить питомцу самому бродить по окну, пока всё бездействие.",
        chooseTitle: "Выбрать питомца",
        chooseDesc:
          "Выбор установит его (при необходимости) и сделает активным.",
        searchPlaceholder: "Поиск питомцев…",
        unreachable:
          "Не удалось связаться с галереей petdex. Проверьте соединение и откройте страницу заново.",
        noMatch: (query) => `Нет питомцев, подходящих под «${query}».`,
        installedTag: "установлен",
        generatedTag: "Сгенерирован",
        countCapped: (cap, total) =>
          `Показаны ${cap} из ${total} — начните вводить, чтобы сузить выбор.`,
        count: (n) => `${n} ${RU_PLURAL(n, "питомец", "питомца", "питомцев")}.`,
        uninstall: (name) => `Удалить ${name}`,
        delete: (name) => `Удалить навсегда ${name}`,
        deleteTitle: (name) => `Удалить ${name}?`,
        deleteBody:
          "Питомец будет удалён безвозвратно — переустановить его будет нельзя.",
    ...
    fieldLabels: defineFieldCopy({
      ...
      updates: {
        nonInteractiveLocalChanges:
          "Локальные изменения при обновлении из приложения",
      },
    }),
    fieldDescriptions: defineFieldCopy({
      model:
        "Используется для новых чатов, если вы не выберете другую модель в композере.",
      modelContextLength:
        "Оставьте 0, чтобы использовать обнаруженное окно контекста выбранной модели.",
      fallbackProviders:
      ...
      display: {
        ...
        showReasoning:
          "Показывать блоки рассуждений, когда бэкенд их предоставляет.",
      },
      desktop: {
        repoScanEnabled:
          "Сканировать локальные папки на Git-репозитории, чтобы показывать их в Проектах.",
        repoScanRoots:
          "Папки для сканирования. Оставьте пустым, чтобы сканировать домашний каталог.",
        repoScanExcludePaths:
          "Папки и их вложенные, которые нужно пропускать при обнаружении репозиториев.",
      },
      timezone:
        "Идентификатор часового пояса IANA. Пустое значение — системный часовой пояс.",
      agent: {
        imageInputMode:
          "Управляет тем, как вложения изображений отправляются модели.",
        maxTurns:
          "Верхний предел ходов с вызовами инструментов, после которого Hermes останавливает запуск.",
      },
      terminal: {
        cwd: "Папка проекта по умолчанию для инструментов и терминала.",
        persistentShell:
          "Сохранять состояние оболочки между командами, если бэкенд это поддерживает.",
        envPassthrough:
          "Переменные окружения, которые передавать в выполнение инструментов.",
        dockerImage:
          "Образ контейнера, используемый, когда бэкенд выполнения — Docker.",
        singularityImage:
          "Образ, используемый, когда бэкенд выполнения — Singularity.",
      ...
      fileReadMaxChars:
        "Максимальное число символов, которые Hermes может прочитать из одного запроса к файлу.",
      approvals: {
      ...
      security: {
        redactSecrets:
          "Скрывать обнаруженные секреты из видимого для модели содержимого, когда это возможно.",
      },
      checkpoints: {
      ...
      memory: {
        memoryEnabled:
          "Сохранять долговременные воспоминания, которые могут помочь будущим сеансам.",
        userProfileEnabled:
          "Поддерживать компактный профиль предпочтений пользователя.",
      },
      context: {
      ...
      compression: {
        enabled:
          "Сжимать более старый контекст, когда диалоги становятся большими.",
      },
      voice: {
      ...
      tts: {
        xai: {
          voiceId:
            "ID голоса xAI (например, eve) или ID пользовательского голоса.",
          language:
            "Код языка речи (например, en, pt-BR) или «auto» для автоопределения.",
          speed:
            "Скорость воспроизведения. 0.7 = медленнее, 1.0 = нормально, 1.5 = быстрее.",
          autoSpeechTags:
          ...
          sampleRate:
            "Частота дискретизации аудио в Гц. Выше = лучшее качество, большие файлы.",
      ...
      stt: {
        ...
        echoTranscripts:
          "Возвращать в чат «сырую» 🎙️ расшифровку голосовых сообщений.",
        elevenlabs: {
          languageCode:
            "Необязательный код языка ISO-639-3. Пустое значение — автоопределение ElevenLabs.",
        },
      },
      updates: {
        nonInteractiveLocalChanges:
    ...
    about: {
      ...
      justNow: "только что",
      minAgo: (count) =>
        `${count} ${RU_NOUN(count, "минуту", "минуты", "минут")} назад`,
      hoursAgo: (count) =>
        `${count} ${RU_NOUN(count, "час", "часа", "часов")} назад`,
      daysAgo: (count) =>
        `${count} ${RU_NOUN(count, "день", "дня", "дней")} назад`,
    },
    config: {
      none: "Нет",
      noneParen: "(нет)",
      ...
      attachmentSizeLabel:
        "Макс. размер превью / загрузки изображений в мегабайтах",
    },
    quickEntry: {
      ...
      shortcutDesc:
        "Нужен хотя бы один модификатор, например CommandOrControl+Shift+Space.",
      active: "Горячий ключ активен.",
      takenBy: "Это сочетание уже занято другим приложением — выберите другое.",
      invalidShortcut:
        "Некорректное сочетание. Включите хотя бы одну модифицирующую клавишу.",
    },
    credentials: {
    ...
    connections: {
      ...
      launchModeTitle:
        "При запуске возвращаться к «Сеансам» на последнем шлюзе",
      ...
      kindRemoteDesc:
        "Шлюз Hermes, доступный по HTTP(S) — LAN, Tailscale или интернет.",
      kindCloudDesc:
        "Хостинговая инстанция, обнаруженная через ваш аккаунт Hermes Cloud.",
      ...
      duplicateLocal:
        "Это приложение уже управляет локальным соединением — их может быть только одно.",
      duplicateUrl: (label) =>
        `Соединение с этим URL шлюза уже есть («${label}»).`,
      duplicateSsh: (label) =>
        `Соединение с этим SSH-хостом уже есть («${label}»).`,
      sameBackendHint: (label) => `Тот же бэкенд, что и «${label}»`,
      localAddHint:
        "Локальное недоступно: управляемое локальное соединение уже существует (их всегда только одно).",
      cloudAddHint:
    ...
    gateway: {
      ...
      envOverrideTitle:
        "Переменные окружения управляют этой сессией приложения.",
      envOverrideDesc:
      ...
      localDesc:
        "Запускает приватный бэкенд Hermes на localhost. Это значение по умолчанию, работает офлайн.",
      ...
      cloudDesc:
        "Войдите в Hermes Cloud один раз и выбирайте агентов из своего аккаунта — без вставки URL.",
      cloudSignInTitle: "Hermes Cloud",
      cloudSignIn: "Войти в Hermes Cloud",
      cloudSignedIn: "Вы вошли в Hermes Cloud",
      cloudNeedsSignIn:
        "Войдите в Hermes Cloud, чтобы обнаружить агентов в вашем аккаунте.",
      cloudSignedInDesc:
        "Вы вошли. Выберите агента ниже; сессия обновляется автоматически.",
      ...
      remoteUrlDesc:
        "Базовый URL удалённого бэкенда дашборда. Поддерживаются префиксы пути, например /hermes.",
      ...
      diagnosticsDesc:
        "Показать desktop.log в файловом менеджере — полезно, если шлюз не запускается.",
      openLogs: "Открыть логи",
      incompleteTitle: "Настройки удалённого шлюза неполные",
      incompleteSignIn:
        "Введите удалённый URL и войдите, прежде чем переключаться на удалённый режим.",
      incompleteToken:
        "Введите удалённый URL и токен сессии, прежде чем переключаться на удалённый режим.",
      incompleteSignInTest:
        "Введите удалённый URL и войдите, прежде чем проверять.",
      incompleteTokenTest:
        "Введите удалённый URL и токен сессии, прежде чем проверять.",
      ...
      restartingMessage:
        "Hermes Desktop переподключится с сохранёнными настройками — оболочка останется открытой.",
      ...
      sshHostPickDesc:
        "Псевдоним Host из ~/.ssh/config, либо «Свой», чтобы ввести вручную.",
      ...
      sshKeyDesc:
        "Путь к закрытому ключу. Пусто = ssh-agent или ~/.ssh/config.",
      sshHermesPathTitle: "Путь к Hermes (необязательно)",
      sshHermesPathDesc:
        "Полный путь к бинарнику hermes на удалённой машине. Пусто = автоопределение.",
      sshHermesPathPlaceholder: "автоопределение",
      sshTestConnection: "Проверить SSH",
      sshConnect: "Подключиться",
      sshButtonsHint:
        "«Сохранить» применится при следующем запуске. «Подключиться» переподключится сейчас.",
      sshReachable: (host, platform) =>
        `Доступен: ${host} (${platform}) — Hermes найден`,
      sshIncompleteHost: "Введите SSH-хост перед подключением.",
      sshErrUnreachable:
        "Не удалось достичь этого хоста по SSH. Проверьте хост, порт и сеть.",
      sshErrAuth:
      ...
      sshErrTimeout:
        "SSH-соединение истекло. Хост может быть недоступен или «спит».",
      sshErrUpdateRequired:
        "Перед подключением через SSH обновите Hermes на удалённой машине.",
    ...
    profileScope: {
      ...
      editsProfile: (profile) =>
        `Изменения на этой странице применяются к профилю «${profile}».`,
    },
    mcp: {
      ...
      gatewayUnavailableMessage:
        "Переподключите шлюз, прежде чем перезагружать MCP.",
      ...
      emptyDesc:
        "Добавьте stdio- или HTTP-сервер, чтобы получить инструменты MCP.",
      disabled: "отключён",
      ...
      serverDisabled: (name) =>
        `${name} отключён — применится к новым сеансам.`,
      ...
      catalogInstallStarted: (name) =>
        `Установка ${name}… применится к новым сеансам после завершения.`,
      ...
      usage30d: (uses) =>
        `${uses} ${RU_NOUN(uses, "использование", "использования", "использований")}/30д`,
      ...
      deepLinkNameInvalid:
        "Имя: 1–64 символа — буквы, цифры, точки, дефисы или подчёркивания.",
      deepLinkNameConflict: (name) =>
        `Сервер с именем ${name} уже существует — выберите другое имя или отмените.`,
      deepLinkErrorTitle: "Ссылка установки MCP отклонена",
      deepLinkErrorName: "Имя сервера в ссылке отсутствует или некорректно.",
      deepLinkErrorConfig:
        "Конфигурация в ссылке не является корректным JSON в base64.",
      deepLinkErrorShape:
        "Конфигурация должна быть JSON-объектом со строковым полем `url` или `command`.",
      ...
      importConfirmMany: (count) =>
        `Добавить ${count} ${RU_PLURAL(count, "сервер", "сервера", "серверов")} в mcp.json`,
    },
    model: {
      ...
      fallbackEmpty:
        "Запасных моделей нет — используется модель по умолчанию, если она не падает.",
      notInCatalog:
        "нет в списке моделей этого провайдера — вызовы могут уходить на запасную.",
      tasks: {
    ...
    providers: {
      ...
      removeExternalGeneric: (provider) =>
        `${provider} управляется собственным CLI — удалите его там.`,
      removeKeyManaged: (provider) =>
        `${provider} настроен по API-ключу. Удалите его в разделе API-ключи.`,
      ...
      localEndpoint: {
        ...
        description:
          "Направьте Hermes на любой OpenAI-совместимый эндпоинт (Zyphra, vLLM, llama.cpp, Ollama и т. д.).",
      },
    ...
    sessions: {
      ...
      messages: (count) =>
        `${count} ${RU_PLURAL(count, "сообщение", "сообщения", "сообщений")}`,
      restored: "Восстановлено",
      deleteConfirm: (title) =>
        `Безвозвратно удалить «${title}»? Это действие необратимо.`,
    ...
    toolsets: {
      ...
      noProviders:
        "Для этого набора инструментов сейчас нет доступных провайдеров.",
      ready: "Готово",
      ...
      nousIncluded:
        "Входит в подписку Nous — войдите в Nous Portal, чтобы активировать.",
      ...
      postSetupInstalledHint:
        "Установлено. Повторите настройку, только если что-то сломано.",
      ...
      webCapabilitySelectedMessage: (provider, capability) =>
        `${provider} теперь отвечает за веб-${capability}.`,
      ...
      modelCount: (count) =>
        `${count} ${RU_PLURAL(count, "модель", "модели", "моделей")}`,
      modelInUse: "В использовании",
      modelDefault: "по умолчанию",
      modelInactiveHint:
        "Сначала выберите этот бэкенд, чтобы изменить его модель.",
      ...
      terminalBackend: {
        ...
        needsSetupHint:
          "Этот бэкенд можно выбрать сейчас — команды будут падать, пока настройка не завершена.",
      },
    },
  },
  skills: {
    ...
    toolsetsEnabled: (enabled, total) =>
      `Включено наборов инструментов: ${enabled}/${total}`,
    configureToolset: (label) => `Настроить ${label}`,
    toggleToolset: (label, enabled) =>
      `${enabled ? "Включить" : "Отключить"} набор инструментов ${label}`,
    ...
    bulkUpdated: (count) =>
      `Обновлено ${count} ${RU_NOUN(count, "элемент", "элемента", "элементов")} для новых сеансов.`,
    ...
    hub: {
      ...
      findings: (count) =>
        `${count} ${RU_NOUN(count, "находка", "находки", "находок")}`,
      ...
      pickerHint:
        "Нажмите «+ Добавить к этому агенту» на любом навыке — он установится и появится в списке выше.",
  ...
  starmap: {
    ...
    emptyDesc:
      "По мере того как Hermes создаёт навыки и память для вашей работы, они появятся здесь.",
    ...
    importSuccess: (nodes) =>
      `Загружена карта с ${nodes} ${RU_NOUN(nodes, "узлом", "узла", "узлов")}.`,
  ...
  agents: {
    ...
    emptyDesc:
      "Когда ход делегирует работу, дочерние агенты стримят свой прогресс сюда.",
    running: "Выполняется",
    failed: "Ошибка",
    done: "Готово",
    streaming: "Стримится",
    files: "Файлы",
    moreFiles: (count) =>
      `+ещё ${count} ${RU_NOUN(count, "файл", "файла", "файлов")}`,
    delegation: (index) => `Делегирование ${index}`,
    workers: (count) =>
      `${count} ${RU_NOUN(count, "воркер", "воркера", "воркеров")}`,
    workersActive: (count) =>
      `${count} ${RU_NOUN(count, "активен", "активно", "активных")}`,
    agentsCount: (count) =>
      `${count} ${RU_PLURAL(count, "агент", "агента", "агентов")}`,
    activeCount: (count) =>
      `${count} ${RU_NOUN(count, "активен", "активно", "активных")}`,
    failedCount: (count) =>
      `${count} ${RU_NOUN(count, "с ошибкой", "с ошибками", "с ошибками")}`,
    toolsCount: (count) =>
      `${count} ${RU_NOUN(count, "инструмент", "инструмента", "инструментов")}`,
    filesCount: (count) =>
      `${count} ${RU_NOUN(count, "файл", "файла", "файлов")}`,
  ...
  commandCenter: {
    ...
    pets: {
      ...
      toggleFailed: (enabled) =>
        `Не удалось ${enabled ? "включить" : "отключить"} питомца.`,
    ...
    generatePet: {
      ...
      promptHint:
        "Введите описание и нажмите Enter, чтобы получить четыре варианта облика.",
      readyHint:
        "Нажмите Enter, чтобы получить четыре варианта облика по вашему описанию.",
      ...
      genericError:
        "Генерация не удалась — попробуйте снова или выберите подсказку.",
      referenceImageTooLarge:
        "Изображение-референс слишком большое. Используйте меньше 16 МБ.",
      referenceImageInvalid:
        "Не удалось прочитать это изображение-референс. Попробуйте PNG, JPG, WebP или GIF.",
      adopt: "Усыновить",
    ...
    installTheme: {
      ...
      installs: (count) =>
        `${count} ${RU_NOUN(count, "установка", "установки", "установок")}`,
    },
    ...
    hermesActiveSessions: (version, count) =>
      `Hermes ${version} · Активные сеансы: ${count}`,
    ...
    noUsage: (period) =>
      `Нет использования за последние ${period} ${RU_NOUN(period, "день", "дня", "дней")}.`,
    retry: "Повторить",
    ...
    actions: (count) =>
      `${count} ${RU_NOUN(count, "действие", "действия", "действий")}`,
    ...
    maintenance: {
      ...
      securityAuditDesc:
        "Сканирование конфигурации и навыков на предмет рискованных настроек",
      backup: "Создать резервную копию",
      backupDesc: "Сжатие конфигурации, памяти, навыков и сеансов в zip",
      debugShare: "Поделиться отладкой",
      debugShareDesc:
        "Загрузка анонимизированного отчёта + журналов, получение ссылок (автоудаление через 6 ч)",
  ...
  messaging: {
    ...
    unknown: "Неизвестно",
    hintPendingRestart:
      "Перезапустите шлюз из строки состояния, чтобы применить это изменение.",
    ...
    noTokenNeeded:
      "Этой платформе здесь не нужен токен. Используйте руководство выше, затем включите её ниже.",
    enabled: "Включено",
    disabled: "Отключено",
    ...
    restartToReconnect:
      "Новые учётные данные вступят в силу после перезапуска шлюза.",
    ...
    revokeDesc: (name) =>
      `${name} потеряет доступ и перестанет распознаваться начиная со следующего сообщения.`,
    approvedUser: (name) => `${name} одобрен`,
    approvedHint:
      "Они распознаются автоматически начиная с их следующего сообщения.",
    revokedUser: (name) => `${name} отозван`,
    failedApprove: (name) => `Не удалось одобрить ${name}`,
    failedRevoke: (name) => `Не удалось отозвать ${name}`,
    pairingLockedOut:
      "Слишком много неудачных одобрений — эта платформа заблокирована. Попробуйте позже.",
    waitingSince: (minutes) =>
      minutes < 1 ? "только что" : `${minutes}м назад`,
    fieldCopy: {
      TELEGRAM_BOT_TOKEN: {
        label: "Токен бота",
  ...
  webhooks: {
    ...
    restartNeeded:
      "Вебхуки включены, но шлюз всё ещё нужно перезапустить, чтобы приёмник заработал.",
    restartGateway: "Перезапустить шлюз",
    ...
    createdSecretHint:
      "Скопируйте секрет сейчас — он показывается только один раз.",
    ...
    fieldPromptPlaceholder:
      "Инструкции для агента при срабатывании этого вебхука (необязательно)",
  ...
  profiles: {
    ...
    title: "Профили",
    count: (count) =>
      `${count} ${RU_PLURAL(count, "профиль", "профиля", "профилей")}`,
    ...
    skills: (count) =>
      `${count} ${RU_PLURAL(count, "навык", "навыка", "навыков")}`,
    env: "env",
    ...
    notSet: "Не задано",
    soulDesc:
      "Системный промпт и инструкции по персоне, встроенные в этот профиль.",
    ...
    deleting: "Удаление...",
    createDesc:
      "Профили — это независимые среды Hermes: отдельные настройки, навыки и SOUL.md.",
    nameLabel: "Имя",
    cloneFrom: "Клонировать из",
    cloneFromNone: "Нет (пустой)",
    cloneFromDesc:
      "Копирует настройки, навыки и SOUL.md из выбранного исходного профиля.",
    cloneFromDefault: "Клонировать из профиля по умолчанию",
    cloneFromDefaultDesc:
      "Скопируйте настройки, навыки и SOUL.md из вашего профиля по умолчанию.",
    ...
    renameDescPrefix:
      "Переименование обновляет каталог профиля и все wrapper-скрипты в ",
    renameDescSuffix: ".",
  ...
  cron: {
    ...
    count: (count) =>
      `${count} ${RU_PLURAL(count, "задача", "задачи", "задач")}`,
    modelImpact: {
    ...
    editDesc:
      "Обновите расписание, промпт или место доставки. Изменения вступят в силу при следующем запуске.",
    createDesc:
    ...
    promptLabel: "Промпт",
    promptPlaceholder:
      "Суммируй мои непрочитанные треды Slack и пришли топ-5 на почту...",
    ...
    customHint:
      "Cron-выражение или фразы вроде «каждый час» или «будни в 9:00».",
    optional: "Необязательно",
    ...
    scriptOnlyEditHint:
      "Задача только со скриптом (без AI-промпта). ID задачи:",
    saveChanges: "Сохранить изменения",
  ...
  artifacts: {
    ...
    noArtifactsDesc:
      "Сгенерированные изображения и файловые результаты появятся здесь по мере их создания в сеансах.",
  ...
  artifactCard: {
    ...
    generating: (lines) =>
      `Генерация… ${lines} ${RU_NOUN(lines, "строка", "строки", "строк")}`,
    versionBadge: (count) =>
      `${count} ${RU_NOUN(count, "версия", "версии", "версий")}`,
    open: "Открыть",
  },
  ...
  sidebar: {
    ...
    allPinned:
      "Здесь всё закреплено. Открепите чат, чтобы он появился в недавних.",
    ...
    projects: {
      ...
      createDesc:
        "Назовите рабочее пространство и добавьте одну или несколько папок.",
      ...
      deleteConfirm:
        "Это удалит сохранённый проект из Hermes. Файлы, git-репозитории и worktrees не пострадают.",
      ...
      convertBranchDesc:
        "Откройте закоммиченные ветки или создайте worktree для свободной ветки.",
      ...
      removeWorktreeFailed:
        "Не удалось удалить worktree (есть незакоммиченные изменения?)",
      ...
      toggle: (label, open) =>
        `${open ? "Показать" : "Скрыть"} сеансы ${label}`,
      back: "Все проекты",
    },
    ...
    row: {
      ...
      deleteDesc: (title) =>
        `Это навсегда удалит «${title}». Это действие необратимо.`,
      deleting: "Удаление…",
  ...
  composer: {
    ...
    wakeWordPausedVoice: (phrase) =>
      `Слово-пробуждение: «${phrase}» — приостановлено во время голосового чата`,
    lookupLoading: "Ищем…",
    ...
    attachUrlDesc:
      "Hermes загрузит страницу и добавит её как контекст для этого хода.",
    ...
    attachments: (count) =>
      `${count} ${RU_NOUN(count, "вложение", "вложения", "вложений")}`,
    ...
    queueResumeTip:
      "Приостановлено стопом — продолжить отправку ходов из очереди",
    ...
    mcpSuggestions: {
      ...
      tip: (keyword) =>
        `Предложено, потому что вы упомянули «${keyword}» — нажмите, чтобы подключить`,
    ...
    skillSuggestions: {
      ...
      tip: (skill) =>
        `Вы упомянули «${skill}» — нажмите, чтобы начать с этого навыка`,
    ...
    cronSuggestions: {
      ...
      tip: (phrase) =>
        `«${phrase}» звучит как повторяющееся — запланируйте выполнение по расписанию`,
    ...
    snippets: {
      codeReview: {
        label: "Рецензия кода",
        description:
          "Аудит текущих изменений на предмет регрессий, упущенных граничных случаев и недостающих тестов.",
        text: "Пожалуйста, проверьте это на баги, регрессии и недостающие тесты.",
      },
      implementationPlan: {
        label: "План реализации",
        description:
          "Опишите подход перед правкой кода, чтобы diff остался сфокусированным.",
        text: "Пожалуйста, составьте краткий план реализации перед изменением кода.",
      },
      explainThis: {
        label: "Объяснить это",
        description:
          "Разберите, как работает выделенный код, и дайте ссылки на ключевые файлы.",
  ...
  statusStack: {
    agents: "Агенты",
    background: (count) =>
      `${count} ${RU_NOUN(count, "фоновая задача", "фоновые задачи", "фоновых задач")}`,
    goalActive: "Цель активна",
    goalDone: "Цель выполнена",
    goalPaused: "Цель на паузе",
    goalWaiting: "Цель ожидает",
    subagents: (count) =>
      `${count} ${RU_PLURAL(count, "субагент", "субагента", "субагентов")}`,
    ...
    coding: {
      ...
      revertAllConfirm:
        "Сбросить все изменения и вернуть файлы в закоммиченное состояние? Это действие необратимо.",
      staged: "В индексе",
      ...
      agentShipUnavailable:
        "Чат, которому принадлежат эти изменения, не на экране.",
      agentShipPrompt:
  ...
  updates: {
    ...
    unsupportedMessage:
      "Эта версия Hermes не может обновлять себя из приложения.",
    ...
    availableBodyBackend:
      "Новая версия подключённого бэкенда Hermes готова к установке.",
    availableBodyNoChangelog:
      "Новая версия готова. Заметки о выпуске недоступны для этого типа установки.",
    updateNow: "Обновить сейчас",
    maybeLater: "Возможно позже",
    moreChanges: (count) =>
      `+ ещё ${count} ${RU_NOUN(count, "изменение", "изменения", "изменений")} включено.`,
    ...
    applyingClose:
      "Это окно закроется во время обновления, затем Hermes откроется сам.",
    errorTitle: "Обновление не завершилось",
    errorBody:
      "Не переживайте — ничего не потеряно. Можно попробовать снова прямо сейчас.",
    ...
    applyStatus: {
      ...
      noReturn:
        "Бэкенд не вернулся в сеть. Обновление могло не завершиться — проверьте хост бэкенда.",
    },
  },
  install: {
    stageStates: {
    ...
    installLocalDesc:
      "Скачайте Hermes, создайте его Python-окружение и запустите бэкенд на этом компьютере.",
    localStartUnavailable:
      "Не удалось начать локальную установку. Перезапустите Hermes Desktop и попробуйте снова.",
    remoteSetupTitle: "Подключиться к существующему Hermes",
    remoteSetupDesc:
      "Введите URL вашего шлюза. Hermes Desktop определит, нужен токен или вход через браузер.",
    remoteUrlTitle: "URL шлюза",
    remoteUrlDesc:
      "Используйте базовый URL шлюза Hermes, включая https:// для удалённых.",
    ...
    authTitle: "Аутентификация",
    authNeedsOauth: (provider) =>
      `Сначала войдите через ${provider}, чтобы проверить этот шлюз.`,
    ...
    testSucceeded: (baseUrl, version) =>
      `Подключено к ${baseUrl}${version ? ` (${version})` : ""}.`,
    ...
    lines: (count) =>
      `${count} ${RU_PLURAL(count, "строка", "строки", "строк")}`,
    noOutput: "Вывода пока нет.",
    cancelling: "Отмена...",
  ...
  onboarding: {
    ...
    headerDesc:
      "Подключите провайдера модели, чтобы начать общение. Большинство вариантов — в один клик.",
    preparingInstall:
    ...
    connected: "Подключено",
    featuredPitch:
      "Одна подписка, 300+ передовых моделей — рекомендуемый способ запускать Hermes",
    fireworksPitch:
      "Прямой API моделей — передовые модели на хостинге Fireworks",
    ...
    apiKeyOptions: {
      ...
      openrouter: {
        ...
        description:
          "Сотни моделей за одним ключом. Хороший вариант по умолчанию для новых установок.",
      },
      openai: {
    ...
    localApiKeyPlaceholder:
      "API-ключ (необязательно — только если ваш endpoint его требует)",
    couldNotSave: "Не удалось сохранить учётные данные.",
    connecting: "Подключение",
    update: "Обновить",
    flowSubtitles: {
      ...
      device_code:
        "Откроет страницу подтверждения в браузере — Hermes подключится автоматически",
    ...
    connectedPicking: (provider) =>
      `${provider} подключён. Выбираем модель по умолчанию...`,
    ...
    deviceCodeOpened: (provider) =>
      `Мы открыли ${provider} в вашем браузере. Введите там этот код:`,
  ...
  shell: {
    ...
    approvalMode: {
      ...
      manual: "Ручной",
      manualDescription:
        "Спрашивать перед действиями, требующими подтверждения",
      smart: "Умный",
      smartDescription:
        "Автоматически оценивать действия и спрашивать при необходимости",
      off: "Выкл",
    ...
    statusbar: {
      ...
      commitsBehind: (count, branch) =>
        `${count} ${RU_NOUN(count, "коммит", "коммита", "коммитов")} позади ${branch}`,
      ...
      cacheHitRateTitle:
        "Доля попаданий в кэш промпта за сеанс — кэшированные токены дешевле, чем выше, тем дешевле",
      tokensPerSecondTitle:
        "Выходных токенов в секунду, среднее за последние 10 вызовов модели",
      agents: "Агенты",
      ...
      running: (count) =>
        `${count} ${RU_NOUN(count, "выполняется", "выполняются", "выполняется")}`,
      cron: "Cron",
      ...
      session: "Сеанс",
      yoloOn:
        "YOLO включён — автоматическое подтверждение опасных команд. Shift-клик переключает глобально.",
      ...
      modelPinned:
        "закреплено вами; новые чаты используют её вместо модели по умолчанию из настроек",
  ...
  rightSidebar: {
    ...
    noProjectBody:
      "Откройте проект, чтобы просматривать его файлы и проверять изменения.",
    ...
    treeErrorBody:
      "Дерево файлов столкнулось с ошибкой при отображении этой папки.",
    tryAgain: "Попробовать снова",
  ...
  preview: {
    ...
    sourceLineTitle:
      "Клик — выбрать · Shift-клик — расширить · перетащите в композер",
    source: "ИСТОЧНИК",
    ...
    largeBody: (label, size) =>
      `${label} — ${size}. Hermes покажет только первые 512 КБ.`,
    ...
    noInlineBody: (mimeType) =>
      `${mimeType || "Этот тип файла"} всё равно можно прикрепить как контекст.`,
    edit: "Изменить",
    editing: "Изменение",
    unsavedChanges: "Несохранённые изменения",
    ...
    console: {
      ...
      messages: (count) =>
        `${count} ${RU_PLURAL(count, "сообщение консоли", "сообщения консоли", "сообщений консоли")}`,
    ...
    web: {
      ...
      restartingMessage:
        "Hermes работает в фоне. Следите за прогрессом в консоли предпросмотра.",
      ...
      blankPageBody:
        "Введите адрес выше, чтобы просматривать, или попросите Hermes открыть страницу.",
      ...
      workspaceReloading:
        "Рабочее пространство изменилось, перезагружаем предпросмотр",
      fileChanged: (url) =>
        `Файл изменился, перезагружаем предпросмотр: ${url}`,
      ...
      loadFailedConsole: (code, message) =>
        `Не удалось загрузить${code ? ` (${code})` : ""}: ${message}`,
  ...
  zones: {
    ...
    pluginDisabledBody:
      "Включите его снова в Настройки → Плагины, чтобы вернуть панель.",
    ...
    mergeZones: (count) =>
      `Слить ${count} ${RU_NOUN(count, "зона", "зоны", "зон")}`,
    ...
    notExpressible:
      "эта расстановка зацеплена (pinwheel) — пока не выразима как вложенные разделения",
    zoneCount: (count) => `${count} ${RU_NOUN(count, "зона", "зоны", "зон")}`,
    tabCount: (count) =>
      `${count} ${RU_NOUN(count, "вкладка", "вкладки", "вкладок")}`,
  },
  contextMenu: {
    link: {
  ...
  assistant: {
    thread: {
      ...
      filesChanged: (count) =>
        `${count} ${RU_PLURAL(count, "файл изменён", "файла изменено", "файлов изменено")}`,
      ...
      restoreFromHere:
        "Восстановить чекпоинт — запустить снова с этого промпта",
      restoreTitle: "Восстановить до этого чекпоинта?",
      restoreBody:
        "Всё, что было после этого промпта, будет удалено из разговора, и промпт запустится снова отсюда.",
    ...
    clarify: {
      ...
      questionProgress: (answered, total) =>
        `Ответ дан на ${answered} из ${total}`,
      lateAnswer: (question, choice) =>
        `Re: «${question}» — мой ответ: ${choice}`,
      lateAnswerTip: "Составить этот ответ как продолжение",
      lateAnswerHint:
        "Этот промпт больше не ждёт. Выберите вариант, чтобы составить его как сообщение-продолжение.",
    },
    mcpSetup: {
      ...
      reloadFailed:
        "Сервер сохранён, но перезагрузка MCP-инструментов не удалась — они загрузятся в следующем сеансе",
      gatewayDisconnected: "Шлюз Hermes не подключён",
    },
    tool: {
      ...
      failedMany: (count) =>
        `${count} ${RU_NOUN(count, "шаг не удался", "шага не удалось", "шагов не удалось")}`,
  ...
  desktop: {
    ...
    yoloSystem: (active) =>
      `YOLO ${active ? "включён" : "выключен"} для этого сеанса`,
    yoloTitle: "YOLO",
    ...
    noProfileNamed: (target, available) =>
      `Нет профиля «${target}». Доступны: ${available}`,
    ...
    editTurnUnavailable:
      "Этого хода больше нет в истории сервера (возможно, он был сжат).",
    ...
    cwdStagedMessage:
      "Перезапустите бэкенд desktop, чтобы применить изменения cwd к этому активному сеансу.",
    ...
    restartToUseSaveImage:
      "Перезапустите Hermes Desktop, чтобы использовать «Сохранить изображение».",
    restartToSaveImages:
      "Перезапустите Hermes Desktop, чтобы сохранять изображения",
    ...
    handoff: {
      ...
      success: (platform) =>
        `Передаём в ${platform}. Возобновите здесь в любой момент.`,
      systemNote: (platform) =>
        `↻ Передано в ${platform} — возобновите здесь в любой момент.`,
      failed: (error) => `Передача не удалась: ${error}`,
      timedOut:
        "Превышено время ожидания шлюза. Выполняется ли `hermes gateway`?",
    },
  },
  errors: {
    ...
    boundaryDesc:
      "Вид столкнулся с неожиданной ошибкой. Ваши чаты и настройки в безопасности.",
    reloadWindow: "Перезагрузить окно",
    openLogs: "Открыть журналы",
  },
