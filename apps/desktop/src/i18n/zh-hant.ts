...
export const zhHant = defineLocale({
  ...
  boot: {
    ...
    failure: {
      title: "Hermes 無法啟動",
      description:
        "背景閘道未啟動。請嘗試下面的復原步驟。這裡的操作不會刪除您的聊天或設定。",
      remoteTitle: "需要重新登入遠端閘道",
      remoteDescription:
        "您的遠端閘道工作階段已過期。請重新登入以重新連線。這裡的操作不會刪除您的聊天或設定。",
      retry: "重試",
      repairInstall: "修復安裝",
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      "您的 Hermes 後端早於目前的桌面版本，可能無法正常運作。請更新以保持一致。",
  ...
  remoteDisplayBanner: {
    message: (reason) =>
      `軟體繪圖已啟用 — 偵測到遠端顯示（${reason}）。為防止畫面閃爍，已停用 GPU 加速。`,
  },
  ...
  sendDiagnostics: {
    ...
    doneTitle: "診斷資訊已傳送",
    doneDescription:
      "偵錯套件已私密上傳。在您的支援討論串中分享以下連結，團隊即可檢視您的日誌。",
    failedTitle: "上傳失敗",
    failedHint:
  ...
  titlebar: {
    ...
    unreadSessions: (count) =>
      count === 1 ? "1 個未讀工作階段" : `${count} 個未讀工作階段`,
    muteHaptics: "靜音觸感回饋",
  ...
  settings: {
    ...
    notifications: {
      title: "通知",
      intro:
        "原生桌面通知，與應用程式內提示不同。設定會依裝置保存，每台電腦各自獨立。",
      enableAll: "啟用通知",
      ...
      testBody: "通知運作正常。",
      testSent:
        "測試已傳送。若沒有出現，請檢查系統通知權限與專注模式／勿擾模式。",
    ...
    appearance: {
      title: "外觀",
      intro:
        "這些是僅限桌面端的顯示偏好。模式控制亮度；主題控制強調色與聊天介面樣式。",
      colorMode: "色彩模式",
      colorModeDesc: "選擇固定模式，或讓 Hermes 跟隨系統設定。",
      toolViewTitle: "工具呼叫顯示",
      toolViewDesc:
        "產品模式會隱藏原始工具 payload；技術模式會顯示完整輸入/輸出。",
      ...
      reactionsTitle: "訊息回應",
      reactionsDesc:
        "iMessage 風格的表情回應 — 你可以對訊息做出回應，Hermes 也能回應你的訊息。",
      tipsTitle: "應用程式內提示",
      tipsDesc:
        "指向應用程式某處的小氣泡：閒置時偶爾出現，需要時 Hermes 也會給你一則。關掉一則就不再出現。",
      tipsReset: (count: number) => `復原 ${count} 則已關閉的提示`,
      toursTitle: "導覽",
      toursDesc: "讓 Hermes 帶你認識應用程式：調暗畫面並逐步標示每個位置。",
      composerPopoutTitle: "懸浮輸入框",
      composerPopoutDesc:
        "允許將輸入框拖出底部停靠區。關閉後，輸入框會鎖定在底部。",
      vibeHeartsTitle: "心情愛心",
      vibeHeartsDesc:
        "當你說謝謝、愛你、good bot 或送出愛心時飄出的愛心。與上方的訊息回應是兩回事。",
      embedsTitle: "內嵌預覽",
      embedsDesc:
      ...
      resumeLastSessionDesc:
        "開啟後，應用冷啟動時重新打開最近的聊天。關閉則始終從空白新會話開始。",
      product: "產品",
      ...
      themeProfileNote: (profile) =>
        `已為「${profile}」設定檔儲存——每個設定檔保留各自的主題。`,
      installTitle: "從 VS Code 安裝",
      installDesc:
        "貼上 Marketplace 擴充功能 ID（例如 dracula-theme.theme-dracula），將其配色主題轉換為桌面調色盤。",
      ...
      pet: {
        ...
        restartHint:
          "寵物功能需要重新啟動——目前執行的應用在此功能加入前啟動。請結束並重新開啟 Hermes，然後回到此處。",
        scaleTitle: "大小",
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      stt: {
        ...
        elevenlabs: {
          languageCode:
            "可選的 ISO-639-3 語言代碼。留空讓 ElevenLabs 自動偵測。",
        },
      },
      updates: {
    ...
    quickEntry: {
      enabledTitle: "快速輸入",
      enabledDesc:
        "用全域快速鍵在任何地方喚出一個小輸入框，無需開啟 Hermes 即可送出提示。",
    ...
    gateway: {
      ...
      localTitle: "本機閘道",
      localDesc:
        "在 localhost 啟動私有 Hermes 後端。這是預設方式，可離線使用。",
      remoteTitle: "遠端閘道",
      remoteDesc:
      ...
      connectedTo: (baseUrl, version) =>
        `已連線至 ${baseUrl}${version ? ` · Hermes ${version}` : ""}`,
      ...
      sshReachable: (host, platform) =>
        `可連線：${host}（${platform}）——已找到 Hermes`,
      sshIncompleteHost: "連線前請輸入 SSH 主機。",
      sshErrUnreachable:
        "無法透過 SSH 連線到該主機。請檢查主機、連接埠和網路。",
      sshErrAuth:
      ...
      sshErrPlatform:
        "不支援的遠端平台。Hermes Desktop 的 SSH 模式支援 Linux、macOS 和 Windows 遠端主機。",
      sshErrTimeout: "SSH 連線逾時。主機可能無法存取或處於睡眠狀態。",
      sshErrUpdateRequired:
        "使用 Desktop SSH 連線前，請更新遠端主機上的 Hermes。",
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) =>
        `組建 ${tag}，${backend} 後端。Hermes 會為您啟動並管理伺服器。`,
      ...
      recommendedReason: {
        "best-quality-resident":
          "在完全駐留 GPU 且保持全速的模型中品質最高。推薦會在品質與該硬體的預計速度之間權衡。",
        "speed-gated-quality":
          "有更高品質的模型可以裝入這台機器，但受記憶體頻寬限制回應會太慢——這是保持流暢的最佳模型。",
        "fastest-resident":
          "沒有模型能在該硬體上達到全速；這是完全駐留 GPU 記憶體中最快的一個。",
        "least-painful-spilled":
          "沒有模型能完全裝入 GPU 記憶體——這是從系統記憶體執行表現最好的一個。",
      ...
      upToDateDetail: (tag, backend) =>
        `正在執行 llama.cpp ${tag}（${backend}）——Hermes 提供的最新組建。`,
      updateToast: (next) =>
        `本地引擎有新組建（${next}）。可在 設定 → 本地模型 中更新。`,
      ...
      runtimeRunningDetail:
        "本地伺服器執行中。關閉後將釋放全部顯示記憶體，新對話將不再使用本地模型，直到您重新開啟。",
      ...
      browseTitle: "發現更多模型",
      browseHint:
        "搜尋整個 Hugging Face。在這裡下載的模型會自動適配你的機器，但未經我們測試。",
    ...
    providers: {
      ...
      intro:
        "使用訂閱登入，無需複製 API 金鑰。Hermes 會在應用程式中為您完成瀏覽器登入。",
      connected: "已連線",
      collapse: "收合",
      ...
      removeKeyManaged: (provider) =>
        `${provider} 由 API 金鑰設定。請從 API Keys 中移除。`,
      removedTitle: "帳號已移除",
      ...
      localEndpoint: {
        title: "本地 / 自訂端點",
        description:
          "將 Hermes 指向任意 OpenAI 相容端點（Zyphra、vLLM、llama.cpp、Ollama 等）。",
      },
      loading: "正在載入提供方...",
    },
    sessions: {
    ...
    toolsets: {
      ...
      nousAuthNeededMessage: (provider) =>
        `已儲存 ${provider}，但在登入 Nous Portal 之前不會啟用。`,
      nousAuthSignIn: "登入",
      ...
      postSetupHint: (step) =>
        `此後端需要一次性安裝 (${step})。將在此機器上執行，可能需要幾分鐘。`,
  ...
  skills: {
    ...
    needsKeys: "需要金鑰",
    visionModelHint:
      "視覺功能使用你的輔助模型設定——支援影像的模型在那裡選擇，而不是在此處按供應商選擇。",
  ...
  commandCenter: {
    ...
    generatePet: {
      ...
      referenceImageInvalid:
        "無法讀取該參考圖片。請嘗試 PNG、JPG、WebP 或 GIF。",
      adopt: "領養",
      startOver: "重新開始",
    },
    installTheme: {
  ...
  messaging: {
    ...
    noTokenNeeded:
      "此平台不需要在此填寫 Token。請按照上方設定指南操作，然後在下方啟用。",
    enabled: "已啟用",
    disabled: "已停用",
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description:
        "此設定檔中的工作階段將在你指定的遠端 Hermes 上執行，而不是這台電腦。",
      urlLabel: "遠端位址",
      ...
      plainTextOptIn:
        "這台電腦沒有安全金鑰儲存空間，權杖將以未加密方式儲存到磁碟。仍要儲存。",
    ...
    deleting: "刪除中…",
    createDesc:
      "設定檔是獨立的 Hermes 環境：各自擁有獨立的設定、技能和 SOUL.md。",
    nameLabel: "名稱",
    cloneFrom: "複製來源",
    cloneFromNone: "無（空白）",
  ...
  cron: {
    ...
    createDesc:
      "排程一個提示詞以自動執行。使用 cron 語法或類似「每 15 分鐘」的自然語言。",
    nameLabel: "名稱",
  ...
  sidebar: {
    ...
    projects: {
      ...
      deleteConfirm:
        "這會從 Hermes 中移除已儲存的專案。檔案、git 儲存庫和工作樹維持不變。",
      startWork: "新增工作樹",
      ...
      worktreeStaleBackend:
        "請更新 Hermes 後端以在此遠端連線上建立工作樹 —— 該後端早於 git 工作樹 API。",
      ...
      removeWorktreeDirty:
        "此工作樹有未提交的變更。強制移除（捨棄這些變更），或僅隱藏軌道並保留在磁碟上。",
      forceRemove: "強制移除",
  ...
  statusStack: {
    ...
    coding: {
      ...
      revertAll: "全部還原",
      revertConfirm:
        "捨棄對此檔案的變更並將其還原至已提交狀態？此操作無法復原。",
      revertAllConfirm:
        "捨棄所有變更並將檔案還原至已提交狀態？此操作無法復原。",
      staged: "已暫存",
      noChanges: "沒有變更",
      notRepo: "不是 Git 儲存庫",
      noDiff: "沒有可顯示的差異",
      ...
      agentShipPrompt:
        "檢查目前的變更，使用清晰的約定式提交訊息提交，推送分支，並開啟一個拉取請求。",
      newBranch: "新增分支",
  ...
  updates: {
    ...
    manualTitle: "從終端機更新",
    manualBody:
      "您是從命令列安裝的 Hermes，因此更新也需要在那裡執行。請將此指令貼到終端機：",
  ...
  onboarding: {
    ...
    apiKeyOptions: {
      fireworks: {
        short: "直接模型 API",
        description: "直接存取 Fireworks AI 託管的模型。",
      },
      openrouter: {
        short: "一個金鑰，多個模型",
        description: "用一個金鑰存取數百個模型。適合新安裝的預設選擇。",
      },
      openai: { short: "GPT 等級模型", description: "直接存取 OpenAI 模型。" },
      gemini: {
        short: "Gemini 模型",
        description: "直接存取 Google Gemini 模型。",
      },
      xai: { short: "Grok 模型", description: "直接存取 xAI Grok 模型。" },
      local: {
        short: "自託管",
        description:
          "將 Hermes 指向本機或自託管的 OpenAI 相容端點（vLLM、llama.cpp、Ollama 等）。",
      },
    },
    backToSignIn: "返回登入",
    getKey: "取得金鑰",
  ...
  preview: {
    ...
    diskChangedBody:
      "此檔案自開啟以來已變更。用你的版本覆寫，還是放棄你的編輯並重新載入？",
    overwrite: "覆寫",
  ...
  zones: {
    ...
    lastTabKeptBody:
      "此區域至少需要一個可見分頁。請先顯示另一個分頁，或收合整個側邊欄。",
  ...
  assistant: {
    ...
    clarify: {
      ...
      lateAnswer: (question, choice) =>
        `關於「${question}」 — 我的回答: ${choice}`,
      lateAnswerTip: "將此回答起草為後續訊息",
      lateAnswerHint:
        "此問題已不再等待回答。選擇一個選項會將其起草為後續訊息。",
    },
    tool: {
      copyCode: "複製程式碼",
  ...
  prompts: {
    ...
    sudoTitle: "管理員密碼",
    sudoDesc:
      "Hermes 需要您的 sudo 密碼來執行特權指令。它只會傳送給您的本機代理。",
  ...
  desktop: {
    ...
    noProfileNamed: (target, available) =>
      `沒有名為「${target}」的設定檔。可用的：${available}`,
    ...
    readOnlyTranscriptSendBlocked:
      "此對話目前以唯讀逐字稿方式開啟——傳送已停用。",
    resumeStrandedTitle: "無法載入此工作階段",
    resumeStrandedBody:
      "與此工作階段的連線失敗，自動重試已停止。請確認閘道正在執行，然後重試。",
    resumeRetry: "重試",
