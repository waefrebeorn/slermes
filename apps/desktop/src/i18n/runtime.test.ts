...
describe("desktop i18n runtime translator", () => {
  ...
  it("translates settings copy for newly supported locales", () => {
    ...
    expect(translateNow("settings.appearance.reasoningCollapsedTitle")).toBe(
      "طي التفكير افتراضيًا",
    );
  ...
  it("keeps translated settings field copy addressable from schema keys", () => {
    ...
    expect(fieldCopyForSchemaKey(zh.settings.fieldLabels, field)).toBe(
      "推理过程块",
    );
    expect(fieldCopyForSchemaKey(zh.settings.fieldDescriptions, field)).toBe(
      "当后端提供推理内容时予以显示。",
    );
  });
