...
export const zh: Translations = {
  ...
  boot: {
    ...
    failure: {
      title: "Hermes 无法启动",
      description:
        "后台网关没有启动。请尝试下面的恢复步骤；这里不会删除你的对话或设置。",
      remoteTitle: "需要重新登录远程网关",
      remoteDescription:
        "你的远程网关会话已过期。请重新登录以恢复连接。这些操作不会删除你的对话或设置。",
      retry: "重试",
      repairInstall: "修复安装",
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      "你的 Hermes 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。",
    ...
    errors: {
      ...
      methodNotAllowed:
        "桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 Hermes Desktop。",
      microphonePermission: "麦克风权限已被拒绝。",
      openaiRejectedApiKey: "OpenAI 拒绝了该 API key。",
      openaiRejectedApiKeyWithStatus: (status) =>
        `OpenAI 拒绝了该 API key (${status} invalid_api_key)。`,
      openaiTtsNeedsKey:
        "OpenAI TTS 需要 VOICE_TOOLS_OPENAI_KEY 或 OPENAI_API_KEY。",
      codeSkewRestartRequired:
        "更新后此后端仍在运行旧代码。请重启以加载新代码。",
    },
    voice: {
  ...
  remoteDisplayBanner: {
    message: (reason) =>
      `软件渲染已启用 — 检测到远程显示（${reason}）。为防止画面闪烁，已禁用 GPU 加速。`,
  },
  ...
  sendDiagnostics: {
    ...
    doneTitle: "诊断信息已发送",
    doneDescription:
      "调试包已私密上传。在您的支持会话中分享以下链接，团队即可查看您的日志。",
    failedTitle: "上传失败",
    failedHint:
  ...
  settings: {
    ...
    notifications: {
      title: "通知",
      intro:
        "原生桌面通知，区别于应用内提示。设置按设备保存，每台电脑各自独立。",
      enableAll: "启用通知",
      ...
      testBody: "通知工作正常。",
      testSent:
        "测试已发送。如果没有出现，请检查系统通知权限和专注模式／勿扰模式。",
    ...
    appearance: {
      title: "外观",
      intro:
        "这些是仅桌面端的显示偏好。模式控制明暗；主题控制强调色与对话界面样式。",
      colorMode: "颜色模式",
      ...
      reactionsTitle: "消息回应",
      reactionsDesc:
        "iMessage 风格的表情回应 — 你可以给消息添加回应，Hermes 也能回应你的消息。",
      tipsTitle: "应用内提示",
      tipsDesc:
        "指向应用某处的小气泡：空闲时偶尔出现，需要时 Hermes 也会给你一条。关掉一条就不再出现。",
      tipsReset: (count: number) => `恢复 ${count} 条已关闭的提示`,
      toursTitle: "引导导览",
      toursDesc: "让 Hermes 带你熟悉应用：调暗界面并逐步高亮每个位置。",
      composerPopoutTitle: "悬浮输入框",
      composerPopoutDesc:
        "允许将输入框拖出底部停靠区。关闭后，输入框会锁定在底部。",
      vibeHeartsTitle: "心情爱心",
      vibeHeartsDesc:
        "当你说谢谢、爱你、good bot 或发送爱心时飘出的爱心。与上方的消息回应是两回事。",
      embedsTitle: "内嵌预览",
      embedsDesc:
      ...
      resumeLastSessionDesc:
        "开启后，应用冷启动时重新打开最近的聊天。关闭则始终从空白新会话开始。",
      product: "产品",
      ...
      themeProfileNote: (profile) =>
        `已为「${profile}」配置文件保存——每个配置文件保留各自的主题。`,
      installTitle: "从 VS Code 安装",
      installDesc:
        "粘贴 Marketplace 扩展 ID（例如 dracula-theme.theme-dracula），将其配色主题转换为桌面调色板。",
      ...
      pet: {
        ...
        restartHint:
          "宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 Hermes，然后回到此处。",
        scaleTitle: "大小",
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      stt: {
        ...
        elevenlabs: {
          languageCode:
            "可选的 ISO-639-3 语言代码。留空让 ElevenLabs 自动检测。",
        },
      },
      updates: {
    ...
    about: {
      ...
      bundleSwapPendingDesc:
        "更新后的应用已安装完成，只需重启 Hermes 即可加载新版本。聊天记录和设置不会受到影响。",
    ...
    config: {
      ...
      disableF12Desc:
        "阻止 F12 打开开发者工具。Ctrl+Shift+I（Mac 上为 Cmd+Opt+I）仍然可用。",
    ...
    quickEntry: {
      enabledTitle: "快速输入",
      enabledDesc:
        "用全局快捷键在任何地方唤出一个小输入框，无需打开 Hermes 即可发送提示。",
    ...
    connections: {
      title: "已注册网关",
      intro:
        "管理本机以及通过远程、SSH 或 Hermes Cloud 连接可访问的每个 Hermes 网关。",
      stagedNote:
      ...
      removeConfirmDesc: (label: string) =>
        `“${label}”将从本应用移除。实例本身不受影响——你可以随时重新添加。`,
      makePrimary: "设为主连接",
      testConnection: "测试",
      testOk: "可访问",
      testFailed: "连接测试失败",
      ...
      kindRemoteDesc:
        "可通过 HTTP(S) 访问的 Hermes 网关——局域网、Tailscale 或互联网。",
      kindCloudDesc: "通过你的 Hermes Cloud 账户发现的托管实例。",
      kindSshDesc: "通过 SSH 访问的 Hermes 安装。",
      labelTitle: "名称",
      labelDesc:
        "必填。此实例出现的所有位置都会显示该名称；必须唯一（例如“家庭服务器”、“工作笔记本”）。",
    ...
    gateway: {
      ...
      envOverrideDesc:
        "取消设置 HERMES_DESKTOP_REMOTE_URL 和 HERMES_DESKTOP_REMOTE_TOKEN 后才会使用下面保存的设置。",
      modeTitle: "连接模式",
      localTitle: "本地网关",
      localDesc:
        "在 localhost 启动私有 Hermes 后端。这是默认方式，并且可离线工作。",
      remoteTitle: "远程网关",
      remoteDesc: "将此桌面外壳连接到远程 Hermes 后端。",
      remoteAuthHint:
        "托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。",
      cloudTitle: "Hermes Cloud",
      cloudDesc:
        "只需登录 Hermes Cloud 一次，即可从你账户下的智能体中选择——无需粘贴 URL。",
      ...
      authSignedInPassword:
        "此网关使用用户名和密码。你已登录，会话会自动刷新。",
      authSignedInOauth: "此网关使用 OAuth。你已登录，会话会自动刷新。",
      authNeedsPassword: "此网关使用用户名和密码。请登录以授权此桌面应用。",
      authNeedsOauth: (provider) =>
        `此网关使用 OAuth。请使用 ${provider} 登录以授权此桌面应用。`,
      tokenTitle: "会话 token",
      tokenDesc:
        "用于 REST 和 WebSocket 访问的 dashboard 会话 token。留空则保留已保存的 token。",
      ...
      savedTitle: "网关设置已保存",
      restartingMessage:
        "Hermes Desktop 将使用已保存设置重新连接（界面保持打开）。",
      savedMessage: "已保存，下一次重启生效。",
      connectedTo: (baseUrl, version) =>
        `已连接到 ${baseUrl}${version ? ` · Hermes ${version}` : ""}`,
      ...
      sshErrPlatform:
        "不支持的远程平台。Hermes Desktop 的 SSH 模式支持 Linux、macOS 和 Windows 远程主机。",
      sshErrTimeout: "SSH 连接超时。主机可能无法访问或处于休眠状态。",
      sshErrUpdateRequired:
        "使用 Desktop SSH 连接前，请更新远程主机上的 Hermes。",
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) =>
        `构建 ${tag}，${backend} 后端。Hermes 会为您启动并管理服务器。`,
      installTitle: "安装本地运行时",
      installDetail:
        "下载 llama.cpp 推理引擎（几百 MB）。下载的模型完全在本机运行——无需账号，数据不会离开您的电脑。",
      installAction: "安装运行时",
      installing: "正在安装运行时…",
      ...
      recommendedReason: {
        "best-quality-resident":
          "在完全驻留 GPU 且保持全速的模型中质量最高。推荐会在质量与该硬件的预计速度之间权衡。",
        "speed-gated-quality":
          "有更高质量的模型可以装入这台机器，但受内存带宽限制响应会太慢——这是保持流畅的最佳模型。",
        "fastest-resident":
          "没有模型能在该硬件上达到全速；这是完全驻留 GPU 内存中最快的一个。",
        "least-painful-spilled":
          "没有模型能完全装入 GPU 内存——这是从系统内存运行表现最好的一个。",
      ...
      upToDateDetail: (tag, backend) =>
        `正在运行 llama.cpp ${tag}（${backend}）——Hermes 提供的最新构建。`,
      updateToast: (next) =>
        `本地引擎有新构建（${next}）。可在 设置 → 本地模型 中更新。`,
      ...
      placementSpilledTip:
        "模型的一部分从系统内存运行——可用但较慢。更紧凑的版本或更小的上下文可以完全放入显存。",
      loadingPill: "加载中…",
      ...
      runtimeRunningDetail:
        "本地服务器正在运行。关闭后将释放全部显存，新对话将不再使用本地模型，直到您重新开启。",
      ...
      browseTitle: "发现更多模型",
      browseHint:
        "搜索整个 Hugging Face。在这里下载的模型会自动适配你的机器，但未经我们测试。",
    ...
    providers: {
      ...
      intro:
        "使用订阅登录，无需复制 API 密钥。Hermes 会在应用中为你完成浏览器登录。",
      connected: "已连接",
      collapse: "收起",
      ...
      removeExternalGeneric: (provider) =>
        `${provider} 由其自身的 CLI 管理 — 请在那里移除。`,
      removeKeyManaged: (provider) =>
        `${provider} 由 API 密钥配置。请从 API Keys 中移除。`,
      removeTerminalConfirm: (provider, command) =>
        `断开 ${provider}？这将在终端中运行 "${command}" 以清除凭据。`,
      ...
      localEndpoint: {
        title: "本地 / 自定义端点",
        description:
          "将 Hermes 指向任意 OpenAI 兼容端点（Zyphra、vLLM、llama.cpp、Ollama 等）。",
      },
      loading: "正在加载提供方...",
    },
    sessions: {
      ...
      archivedTitle: "已归档会话",
      archivedIntro:
        "已归档对话会从侧边栏隐藏，但会保留全部消息。在侧边栏 Ctrl/⌘ 点击对话即可归档。",
      ...
      defaultDirDesc:
        "新会话默认从此文件夹开始，除非你选择其他目录。留空则使用你的 home 目录。",
    ...
    toolsets: {
      ...
      nousAuthNeededMessage: (provider) =>
        `已保存 ${provider}，但在登录 Nous Portal 之前不会激活。`,
      nousAuthSignIn: "登录",
      ...
      postSetupHint: (step) =>
        `此后端需要一次性安装 (${step})。将在此机器上执行，可能需要几分钟。`,
  ...
  skills: {
    ...
    needsKeys: "需要密钥",
    visionModelHint:
      "视觉功能使用你的辅助模型配置——支持图像的模型在那里选择，而不是在此处按提供商选择。",
    ...
    hub: {
      ...
      featured: "精选技能",
      landingHint:
        "搜索技能中心，浏览来自官方索引、GitHub 和社区来源的可安装技能。",
      noResults: "技能中心没有匹配的技能。",
      resultCount: (count, ms) =>
        `${count} 个结果${ms !== null ? `（${ms}ms）` : ""}`,
      ...
      pickerHint:
        "点击任意技能上的“+ Add to this Agent”即可安装，安装后会出现在上方列表中。",
      loadFailed: "技能中心加载失败",
  ...
  starmap: {
    ...
    share: "分享图谱",
    shareHint:
      "复制代码以分享此图谱，或粘贴代码以载入。仅包含布局，不含你的记忆或技能内容。",
    shareTitle: "导入 / 导出图谱",
  ...
  messaging: {
    ...
    noTokenNeeded:
      "此平台无需在此填写令牌。请按上方设置指南操作，然后在下方启用。",
    enabled: "已启用",
    disabled: "已禁用",
    ...
    platformIntro: {
      ...
      slack:
        "创建 Slack 应用，启用 Socket Mode，安装到你的工作区，然后复制 bot 令牌和 app 级令牌。",
      mattermost:
        "在你的 Mattermost 服务器上，创建机器人账户或个人访问令牌，然后在此粘贴服务器 URL 和令牌。",
      matrix:
        "用机器人账户登录你的 homeserver，然后复制访问令牌、用户 ID 和 homeserver URL。",
      signal:
        "在可访问的位置运行 signal-cli REST 桥接，然后把 Hermes 指向该 URL 和已注册的电话号码。",
      whatsapp:
        "启动 Hermes 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。",
      bluebubbles:
        "在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 Hermes 指向该 URL。",
      homeassistant:
        "在 Home Assistant 中打开你的个人资料并创建长期访问令牌。把它连同你的 HA URL 一起粘贴到这里。",
      email:
      ...
      dingtalk:
        "在开发者控制台创建钉钉应用，然后在此复制 Client ID(App key) 和 Client Secret。",
      feishu:
        "创建飞书 / Lark 应用，配置机器人能力，复制 App ID、App secret 和事件加密密钥。",
      wecom:
        "在企业微信中添加群机器人，复制其 webhook key 作为 WECOM_BOT_ID。仅可发送——双向请用企业微信 (应用) 选项。",
      wecom_callback:
        "设置一个企业微信自建应用，暴露其回调 URL，并提供 corp ID、secret、agent ID 和 AES key。",
      weixin:
        "运行 `hermes gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。Hermes 会通过腾讯 iLink Bot API 连接并保存凭据。",
      qqbot:
        "在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。",
      api_server:
        "把 Hermes 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。",
      webhook:
        "运行一个 HTTP 服务器，供其他工具 (GitHub、GitLab、自定义应用)POST。用 secret 验证签名。",
    },
  },
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description:
        "此配置档案中的会话将在你指定的远程 Hermes 上运行，而不是这台电脑。",
      urlLabel: "远程地址",
      ...
      plainTextOptIn:
        "这台电脑没有安全密钥存储，令牌将以未加密方式保存到磁盘。仍然保存。",
    ...
    deleting: "删除中…",
    createDesc:
      "配置档案是相互独立的 Hermes 环境：各自拥有独立的配置、技能和 SOUL.md。",
    nameLabel: "名称",
    cloneFrom: "克隆来源",
    cloneFromNone: "无（空白）",
    ...
    displayNameDesc:
      '设置一个在整个应用中显示的名称。内部配置档案 ID 仍为 "default"。',
  ...
  cron: {
    ...
    newCron: "新建定时任务",
    emptyDescNew:
      "按 cron 表达式排程一个提示词。Hermes 会运行它，并把结果发送到你选择的目的地。",
  ...
  sidebar: {
    ...
    projects: {
      ...
      deleteConfirm:
        "这会从 Hermes 中移除已保存的项目。文件、git 仓库和工作树保持不变。",
      startWork: "新建工作树",
      ...
      worktreeStaleBackend:
        "请更新 Hermes 后端以在此远程连接上创建工作树 —— 该后端早于 git 工作树 API。",
      ...
      removeWorktreeDirty:
        "此工作树有未提交的更改。强制移除（丢弃这些更改），或仅隐藏泳道并保留在磁盘上。",
      forceRemove: "强制移除",
  ...
  statusStack: {
    ...
    coding: {
      ...
      revertAll: "全部还原",
      revertConfirm:
        "放弃对此文件的更改并将其恢复到已提交状态？此操作无法撤销。",
      revertAllConfirm:
        "放弃所有更改并将文件恢复到已提交状态？此操作无法撤销。",
      staged: "已暂存",
      noChanges: "没有更改",
      notRepo: "不是 Git 仓库",
      noDiff: "没有可显示的差异",
      ...
      agentShipPrompt:
        "检查当前更改，使用清晰的约定式提交信息提交，推送分支，并开启一个拉取请求。",
      newBranch: "新建分支",
  ...
  updates: {
    ...
    manualTitle: "从终端更新",
    manualBody:
      "你是从命令行安装的 Hermes，因此更新也需要在那里运行。请将此命令粘贴到终端：",
    ...
    applyingBodyBackend:
      "远程后端正在应用更新并将重启。恢复后 Hermes 会自动重新连接。",
    applyingClose: "此窗口会在更新期间关闭，随后 Hermes 会自动重新打开。",
    errorTitle: "更新未完成",
    errorBody: "没有数据丢失。你可以现在重试。",
    blockerTitle: "关闭本地预览以更新 Hermes？",
    blockerBody:
      "Hermes 需要在更新前停止这些本地预览。这不会修改或删除你的文件。",
    foreignBlockerTitle: "关闭其他进程以更新 Hermes",
    foreignBlockerBody:
      "Hermes 无法安全地自动关闭这些进程。请关闭拥有这些进程的应用、终端或服务，然后重试更新。",
    mixedBlockerBody:
      "Hermes 可以关闭下方列出的本地预览。其他进程必须手动关闭，更新才能继续。",
  ...
  install: {
    ...
    remoteSetupDesc:
      "输入网关 URL。Hermes Desktop 会检测需要令牌还是浏览器登录。",
    ...
    activeDesc:
      "这是一次性设置。Hermes 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。",
  ...
  onboarding: {
    ...
    apiKeyOptions: {
      ...
      local: {
        short: "自托管",
        description:
          "将 Hermes 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。",
      },
    },
    backToSignIn: "返回登录",
    getKey: "获取密钥",
    ...
    autoBrowser: (provider) =>
      `已在浏览器中打开 ${provider}。请在那里授权 Hermes，连接会自动完成，无需复制或粘贴。`,
    reopenSignInPage: "重新打开登录页面",
    waitingAuthorize: "等待你授权...",
    externalPending: (provider) =>
      `${provider} 通过自己的 CLI 登录。请在终端运行此命令，然后回来选择“我已登录”：`,
    signedIn: "我已登录",
    deviceCodeOpened: (provider) =>
      `已在浏览器中打开 ${provider}。请在那里输入此代码：`,
  ...
  preview: {
    ...
    diskChangedBody:
      "此文件自打开以来已更改。用你的版本覆盖，还是放弃你的编辑并重新加载？",
    overwrite: "覆盖",
  ...
  zones: {
    ...
    lastTabKeptBody:
      "该区域至少需要一个可见标签。请先显示另一个标签，或折叠整个侧边栏。",
    toggleStripTab: (title) => `切换 ${title} 标签`,
    minimize: "最小化",
    restore: "还原",
    closeRunningTitle: "关闭正在运行的标签？",
    closeRunningBody:
      "此对话仍在运行（或正在等待你的输入）。关闭标签只会隐藏它——会话将保留进度，可从侧边栏重新打开。",
  ...
  assistant: {
    ...
    clarify: {
      ...
      lateAnswer: (question, choice) =>
        `关于"${question}" — 我的回答: ${choice}`,
      lateAnswerTip: "将此回答起草为后续消息",
      lateAnswerHint:
        "此问题已不再等待回答。选择一个选项会将其起草为后续消息。",
    },
    mcpSetup: {
  ...
  prompts: {
    ...
    sudoTitle: "管理员密码",
    sudoDesc:
      "Hermes 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。",
  ...
  desktop: {
    ...
    profileStatus: (current) =>
      `配置档案：${current}。使用 /profile <name> 或“新建会话”选择器在其他配置档案中开始对话。`,
    unknownProfile: "未知配置档案",
    noProfileNamed: (target, available) =>
      `没有名为“${target}”的配置档案。可用：${available}`,
    ...
    resumeStrandedBody:
      "与此会话的连接失败，自动重试已停止。请确认网关正在运行，然后重试。",
    resumeRetry: "重试",
