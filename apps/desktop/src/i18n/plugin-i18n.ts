...
function mergeMessages(
  ...
  for (const [key, value] of Object.entries(overrides)) {
    ...
    result[key] =
      isRecord(prev) && isRecord(value) ? mergeMessages(prev, value) : value;
  }
...
export function createPluginI18n(
  ...
  return {
    ...
    t: (key, ...args) =>
      translatePlugin(pluginId, getRuntimeI18nLocale(), key, args),
  };
}
...
export function usePluginI18n(pluginId: string): PluginTranslate {
  ...
  return useCallback(
    (key: string, ...args: unknown[]) =>
      translatePlugin(pluginId, locale, key, args),
