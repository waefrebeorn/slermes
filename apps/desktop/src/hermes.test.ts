...
describe("Hermes REST helpers", () => {
  ...
  it("routes session, profile, and model reads through the active registry source", async () => {
    ...
    for (const call of api.mock.calls) {
      expect(call[0]).toEqual(
        expect.objectContaining({ connectionId: "personal" }),
      );
    }
  });
  ...
  it("keeps an explicit This device source on REST requests", async () => {
    ...
    expect(api).toHaveBeenCalledWith(
      expect.objectContaining({ connectionId: "local", path: "/api/profiles" }),
    );
  });
  ...
  it("preserves ambient and explicit-local ownership for session and profile requests", async () => {
    ...
    for (const call of api.mock.calls) {
      expect(call[0]).toEqual(
        expect.objectContaining({ connectionId: "local", profile: "worker" }),
      );
    }
  });
  ...
  it("falls back to the per-slice endpoint when the batched route 404s on an older backend", async () => {
    ...
    const paths = api.mock.calls.map(
      (call) => (call[0] as { path: string }).path,
    );
    expect(
      paths.filter((p) => p.startsWith("/api/profiles/sessions/sidebar")),
    ).toHaveLength(1);
    expect(
      paths.filter((p) => p.startsWith("/api/profiles/sessions?")),
    ).toHaveLength(3);
    expect(
      paths.filter(
        (path) =>
          path.startsWith("/api/profiles/sessions?") &&
          path.includes("profile=work"),
      ),
    ).toHaveLength(3);
    ...
    expect(paths).toContainEqual(
      expect.stringContaining("exclude_sources=cron%2Ctool"),
    );
  });
  ...
  it("keeps per-slice errors on the legacy fallback so a cron failure does not taint recents", async () => {
    ...
    expect(result.cron.errors).toEqual([
  ...
  it("remembers endpoint-missing and skips re-probing the batched route on later refreshes", async () => {
    ...
    const batchedProbes = api.mock.calls.filter((call) =>
      (call[0] as { path: string }).path.startsWith(
  ...
  it("does NOT fall back on transient failures — only endpoint-missing shapes trigger legacy mode", async () => {
    ...
    expect((api.mock.calls[1][0] as { path: string }).path).toMatch(
      /^\/api\/profiles\/sessions\/sidebar\?/,
    );
  });
  ...
  it("gives the whole startup data burst the long timeout, not just profiles", async () => {
    ...
    for (const [call, path] of bootCalls) {
      ...
      await call();
      expect(api).toHaveBeenCalledWith(
        expect.objectContaining({ path, timeoutMs: 60_000 }),
      );
    }
  });
  ...
  it("scopes profile deletion and rejects default before Electron dispatch", async () => {
    ...
    await deleteProfile("backend-worker", {
      connectionId: "source-a",
  ...
  it("records tail truncation state under the requested and resolved session ids", async () => {
    ...
    expect(transcriptTailState("prefix-1")).toMatchObject({
      nextOffset: 120,
      possiblyTruncated: true,
    });
    expect(transcriptTailState("resolved-1")).toMatchObject({
      nextOffset: 120,
      possiblyTruncated: true,
    });
  });
  ...
  it("stops complete transcript loads before Desktop memory becomes unbounded", async () => {
    ...
    await expect(
      getAllSessionMessages("session-1", null, { maxJsonChars: 1 }),
    ).rejects.toThrow("Desktop safe-load limit");
  });

  it("bounds blocking TTS synthesis timeouts by text length", () => {
    expect(audioSpeakRequestTimeoutMs("short message")).toBe(
      AUDIO_SPEAK_MIN_REQUEST_TIMEOUT_MS,
    );
    expect(audioSpeakRequestTimeoutMs("x".repeat(8_000))).toBe(280_000);
    expect(audioSpeakRequestTimeoutMs("x".repeat(100_000))).toBe(
      AUDIO_SPEAK_MAX_REQUEST_TIMEOUT_MS,
    );
  });
  ...
  it("bounds blocking transcription timeouts by payload length", () => {
    expect(audioTranscribeRequestTimeoutMs("data:audio/webm;base64,AA==")).toBe(
      AUDIO_TRANSCRIBE_MIN_REQUEST_TIMEOUT_MS,
    );
    expect(audioTranscribeRequestTimeoutMs("x".repeat(3_000_000))).toBe(
      300_000,
    );
    expect(audioTranscribeRequestTimeoutMs("x".repeat(9_000_000))).toBe(
      AUDIO_TRANSCRIBE_MAX_REQUEST_TIMEOUT_MS,
    );
  });
  ...
  it("uses an extended timeout for blocking transcription", async () => {
    ...
    await expect(
      transcribeAudio("data:audio/webm;base64,AA==", "audio/webm"),
    ).resolves.toEqual({
      ok: true,
      provider: "openai",
      text: "transcribed text",
