...
async function loadToolCalls(
  ...
  const value = Object.fromEntries(
    (analytics.tools ?? []).map((e) => [e.tool, e.count]),
  );
...
const usageOf = (skill: SkillInfo): number =>
  typeof skill.usage === "number" ? skill.usage : 0;

const categoryFor = (skill: SkillInfo): string =>
  asText(skill.category) || "general";
...
const visibleToolsetCount = (toolsets: ToolsetInfo[]) =>
...
export function SkillsView({
  ...
  const gateway = useStoreSelector($gateway, (g) =>
    mode === "mcp" ? g : null,
  );
  ...
  const [scopeOverride, setScopeOverride] = useState<
    null | string | { connectionId: string; profile: string }
  >(null);
  ...
  const scopeConnectionId =
    scopeProfile && typeof scopeProfile === "object"
      ? (scopeProfile.connectionId ?? "").trim() || "local"
      : null;
  ...
  const crossBackendScope =
    ...
    scopeConnectionId !== (activeGatewayConnectionId() ?? "local");
  ...
  const setSkills = useCallback(
    (fn: (cur: SkillInfo[] | undefined) => SkillInfo[] | undefined) =>
      queryClient.setQueryData<SkillInfo[]>(
        [...SKILLS_QUERY_KEY, scopeKey],
        (prev) => fn(prev) ?? prev,
      ),
    [scopeKey],
  );
  ...
  const [toolCalls, setToolCalls] = useState<Record<string, number> | null>(
    null,
  );
  ...
  const refreshCapabilities = useCallback(async () => {
    ...
    if (toolCallsCache.size > 0) {
      ...
      loadToolCalls(scopeKey, scopeProfile, true)
        .then(
          (value) => toolCallsEpoch.current === epoch && setToolCalls(value),
        )
  ...
  const visibleOfficial = useMemo(() => {
    const catalog = (officialData?.skills ?? []).filter(
      (s) => !s.installed && !installedSkillNames.has(s.name),
    );
  ...
  const bulkToolsets = useMemo(
    () => (toolsets ?? []).filter((ts) => isDesktopToolsetVisible(ts.name)),
    [toolsets],
  );
  ...
  const searchHints = useMemo(() => {
    ...
    if (mode === "toolsets" && toolsets?.length) {
      return toolsets
        .filter(
          (ts) => isDesktopToolsetVisible(ts.name) && toolNames(ts).length > 0,
        )
        .slice(0, 5)
  ...
  const activeSkill = useMemo(
    () =>
      visibleSkills.find((s) => s.name === selectedSkill) ??
      visibleSkills[0] ??
      null,
  ...
  const activeToolset = useMemo(
    () =>
      visibleToolsets.find((ts) => ts.name === selectedToolset) ??
      visibleToolsets[0] ??
      null,
  ...
  function handleInstallOfficial(skill: OfficialSkillInfo) {
    notify({
      kind: "success",
      title: t.skills.hub.installStarted(skill.name),
      message: t.skills.hub.actionLog,
    });
    void installHubSkill(skill.identifier, scopeProfile).catch((err) =>
      notifyError(err, t.skills.hub.actionFailed),
    );
  }
  ...
  async function handleToggleToolset(toolset: ToolsetInfo, enabled: boolean) {
    ...
    writeScoped(
      (current) =>
        current?.map((row) =>
          row.name === toolset.name
            ? { ...row, enabled, available: enabled }
            : row,
        ) ?? current,
    );
    ...
    try {
      ...
      writeScoped(
        (current) =>
          current?.map((row) =>
            row.name === toolset.name
              ? { ...row, enabled: !enabled, available: !enabled }
              : row,
          ) ?? current,
      );
  ...
  async function bulkApply(
    skillTargets: SkillInfo[],
    toolsetTargets: ToolsetInfo[],
    enabled: boolean,
  ) {
    ...
    try {
      for (const row of skillTargets) {
        ...
        setSkills(
          (cur) =>
            cur?.map((r) => (r.name === row.name ? { ...r, enabled } : r)) ??
            cur,
        );
        done += 1;
      }
      ...
      for (const row of toolsetTargets) {
        ...
        queryClient.setQueryData<ToolsetInfo[]>(
          ...
          (cur) =>
            cur?.map((r) =>
              r.name === row.name ? { ...r, enabled, available: enabled } : r,
            ) ?? cur,
        );
        done += 1;
      }
      ...
      notify({
        kind: "success",
        title: t.skills.bulkUpdated(done),
        message: "",
      });
    } catch (err) {
      notifyError(
        err,
        t.skills.failedToUpdate(
          mode === "skills" ? t.skills.tabSkills : t.skills.tabToolsets,
        ),
      );
    } finally {
      invalidateSlashCompletions();
  ...
  const allSkillsEnabled =
    bulkSkills.length > 0 && bulkSkills.every((s) => s.enabled);
  const allToolsetsEnabled =
    bulkToolsets.length > 0 && bulkToolsets.every((ts) => ts.enabled);

  const sortButton = (desc: boolean, flip: () => void) => (
    <ListStripButton onClick={flip}>
      {desc ? t.skills.sortMostUsedDesc : t.skills.sortLeastUsedAsc}
    </ListStripButton>
  );
  ...
  const changeScope = (value: string) => {
    ...
    const next: ProfileScope =
      sep >= 0
        ? { connectionId: value.slice(0, sep), profile: value.slice(sep + 2) }
        : value;
    ...
    setScopeOverride(
      next as string | { connectionId: string; profile: string },
    );
    toolCallsEpoch.current += 1;
    setToolCalls(null);
    skillEditorEpoch.current += 1;
  ...
  const scopeOptions: { key: string; label: string; value: string }[] =
    useMemo(() => {
  ...
  return (
    <PageSearchShell
      ...
      searchPlaceholder={
        mode === "skills" ? t.skills.searchSkills : t.skills.searchToolsets
      }
      searchValue={query}
      tabs={[
        {
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          <div
            className={
              mode === "skills"
                ? "min-h-40 flex-1 overflow-hidden"
                : "min-h-0 flex-1"
            }
          >
            {mode === "mcp" ? (
              ...
              visibleSkills.length === 0 && visibleOfficial.length === 0 ? (
                ...
                <MasterDetail
                  pane={skillEditorPane}
                  resizeId="capabilities-split"
                  split="wide"
                >
                  <ListColumn
                    header={
                      <ListStrip
                        left={sortButton(skillsSortDesc, () =>
                    ...
                    {visibleOfficial.map((skill) => {
                      ...
                      return (
                        <CapRow
                          ...
                          active={
                            activeOfficial?.identifier === skill.identifier
                          }
                          enabled={false}
                  ...
                  <DetailColumn footer={t.skills.changesApplyNewSessions}>
                    {activeOfficial ? (
                      <OfficialSkillDetail
                        installing={runningInstalls.has(
                          activeOfficial.identifier,
                        )}
              ...
              <MasterDetail resizeId="capabilities-split" split="wide">
                <ListColumn
                  header={
                    <ListStrip
                      left={sortButton(toolsetsSortDesc, () =>
                        $toolsetsSortDesc.set(!$toolsetsSortDesc.get()),
                      )}
                      right={
                        <ListStripMenu
                          label={t.skills.tabToolsets}
                ...
                >
                  {visibleToolsets.map((toolset) => {
                    ...
                    const calls = toolCalls
                      ? toolsetCalls(toolset, toolCalls)
                      : null;
                    ...
                    return (
                      <CapRow
                        ...
                        title={label}
                        toggleLabel={t.skills.toggleToolset(
                          label,
                          !toolset.enabled,
                        )}
                      />
      ...
      {archiveTarget && (
        <ArchiveSkillConfirmDialog
          onApply={() => {
            ...
            setSkills(
              (current) =>
                current?.filter((skill) => skill.name !== name) ?? current,
            );
            invalidateSlashCompletions();
  ...
  const parsed = useMemo(
    () =>
      previewQuery.data?.skill_md
        ? parseFrontmatter(previewQuery.data.skill_md)
        : null,
    [previewQuery.data],
  );
  ...
  return (
    <>
      ...
      <DetailHeader
        ...
        pills={
          !toolset.configured && (
            <PanelPill tone="warn">{t.skills.needsKeys}</PanelPill>
          )
        }
        title={label}
      />
      ...
      {toolset.name === "computer_use" && (
        <ComputerUsePanel onConfiguredChange={onConfiguredChange} />
      )}
      ...
      {toolset.name === "browser" && (
        <BrowserRealProfilePanel profile={profile} />
      )}
      {toolset.name === "terminal" && (
        <TerminalBackendPanel onConfiguredChange={onConfiguredChange} />
      )}
      <ToolsetConfigPanel
