...
export const EmbeddedHubPicker = memo(function EmbeddedHubPicker({
  ...
  const updating = useStoreSelector(
    $hubActions,
    (actions) => actions[UPDATE_ALL_KEY]?.running ?? false,
  );
  ...
  const startDrag = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const columnMax = column
      ? column.clientHeight - HUB_LIST_RESERVED_PX
      : Number.POSITIVE_INFINITY;
    const max = Math.max(
      HUB_MIN_PX,
      Math.round(Math.min(window.innerHeight * HUB_MAX_VH, columnMax)),
    );
    setDragging(true);
    ...
    const onMove = (move: globalThis.PointerEvent) => {
      setPaneHeightOverride(
        ...
        Math.round(
          Math.min(
            max,
            Math.max(HUB_MIN_PX, startHeight + (startY - move.clientY)),
          ),
        ),
      );
    };
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (installedNames.has(label) || installedNames.has(target)) {
        notify({
          kind: "success",
          title: h.alreadyInstalled(label),
          message: "",
        });
      ...
      notify({
        kind: "success",
        title: h.installStarted(label),
        message: h.actionLog,
      });
      void installHubSkill(target, profile).catch((err) =>
  ...
  const updateAll = () => {
    ...
    void updateHubSkills(profile).catch((err) =>
      notifyError(err, h.actionFailed),
    );
  };
        ...
        <div
          className={cn(
            ...
            dragging
              ...
              : "group-hover/hubsash:bg-(--ui-stroke-secondary)",
          )}
        />
      </div>
