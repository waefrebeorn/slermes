...
vi.mock("@/hermes", async (importOriginal) => ({
  ...
  setSkillEnabled: (name: string, enabled: boolean, profile?: null | string) =>
    setSkillEnabled(name, enabled, profile),
  setToolsetEnabled: (
    name: string,
    enabled: boolean,
    profile?: null | string,
  ) => setToolsetEnabled(name, enabled, profile),
  getToolsetConfig: (name: string, profile?: null | string) =>
    getToolsetConfig(name, profile),
  selectToolsetProvider: (toolset: string, provider: string) =>
    selectToolsetProvider(toolset, provider),
  getUsageAnalytics: (days: number, profile?: null | string) =>
    getUsageAnalytics(days, profile),
  getProfiles: () => getProfiles(),
  getSkillContent: (name: string, profile?: null | string) =>
    getSkillContent(name, profile),
...
beforeEach(() => {
  ...
  setToolsetEnabled.mockResolvedValue({
    ok: true,
    name: "web",
    enabled: false,
  });
...
describe("SkillsView toolset management", { timeout: 60_000 }, () => {
  it("renders a switch for each toolset and toggles it off", async () => {
    ...
    const sw = await screen.findByRole("switch", {
      name: "Turn Web Search toolset off",
    });
  ...
  it("hub picker refuses to reinstall an already-installed skill", async () => {
    ...
    render(
      <EmbeddedHubPicker
        installedNames={new Set(["web-research"])}
        profile={null}
      />,
    );
    ...
    await waitFor(() =>
      expect(vi.mocked(notify)).toHaveBeenCalledWith(
        expect.objectContaining({
          title: '"web-research" is already installed',
        }),
      ),
    );
  });
  ...
  it("mounts the hub iframe lazily and keeps it (hidden) across tab switches", async () => {
    ...
    expect(iframe!.closest("section")!.classList.contains("hidden")).toBe(
      false,
    );
  ...
  it("shows a vision explainer that deep-links to Settings → Models", async () => {
    ...
    expect(
      await screen.findByText(/auxiliary model configuration/),
    ).toBeTruthy();
    const link = screen.getByRole("button", {
      name: /Choose vision model in Settings/,
    });
    ...
    await waitFor(() =>
      expect(navigateSpy).toHaveBeenCalledWith(
        "/settings?tab=config:model&aux=vision",
      ),
    );
  });
  ...
  it("fixedConnection pins every read to the target connection", async () => {
    ...
    expect(getSkills.mock.calls[0][0]).toEqual({
      connectionId: "homelab",
      profile: "inbox-bot",
    });
    expect(getToolsets.mock.calls[0][0]).toEqual({
      connectionId: "homelab",
      profile: "inbox-bot",
    });
  ...
  it("offers (connection, profile) scope rows on multi-connection desktops", async () => {
    ...
    try {
      ...
      expect(
        await screen.findByText("default — This device (current)"),
      ).toBeTruthy();
    } finally {
  ...
  it("lists the built-in optional-skills catalog with Install buttons that route through the hub pipeline", async () => {
    ...
    await waitFor(() =>
      expect(vi.mocked(installHubSkill)).toHaveBeenCalledWith(
