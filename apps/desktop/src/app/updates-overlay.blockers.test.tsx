...
describe("formatBlockerCommandLine", () => {
  ...
  it("redacts a sensitive query value and bounds the remaining diagnostic output", () => {
    ...
    const commandLine =
      `python.exe watcher.py https://example.test/?api_key=${queryValue}&mode=inspect ` +
      "x".repeat(600);
...
describe("BlockerView", () => {
  ...
  it("uses the blocker view for a foreign process instead of the generic update error", async () => {
    ...
    expect(
      screen.getByText("Close other processes to update Hermes"),
    ).toBeTruthy();
  ...
  it("identifies foreign blockers without offering automatic termination", async () => {
    ...
    expect(
      screen.getByText(/can’t safely close these processes automatically/i),
    ).toBeTruthy();
    expect(
      screen.getByText(/python.exe fenbi_session_refresh\.py/i),
    ).toBeTruthy();
    expect(
      screen.queryByRole("button", { name: /close previews/i }),
    ).toBeNull();
  ...
  it("keeps mixed blocker cleanup limited to safe previews", async () => {
    ...
    expect(
      screen.getByText(/can close the local previews listed below/i),
    ).toBeTruthy();
    expect(screen.getByText("Example Preview")).toBeTruthy();
    expect(screen.getByText("python.exe")).toBeTruthy();

    fireEvent.click(
      screen.getByRole("button", { name: "Close previews and check again" }),
    );
  ...
  it("explains safe local previews and offers one-click close-and-update", async () => {
    ...
    expect(
      screen.getByText("Close local previews to update Hermes?"),
    ).toBeTruthy();
    expect(screen.getByText("Example Preview")).toBeTruthy();
    expect(screen.getByText("Port 8766")).toBeTruthy();
    expect(
      screen.getByText(/will not modify or delete your files/i),
    ).toBeTruthy();

    fireEvent.click(
      screen.getByRole("button", { name: "Close previews and update" }),
    );
    expect(onStopAndUpdate).toHaveBeenCalledTimes(1);
  });
});
