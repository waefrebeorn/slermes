...
function Harness({
  ...
  const actions = useSessionActions({
    ...
    selectedStoredSessionIdRef:
      selectedStoredSessionIdRefOverride ?? ref(selectedStoredSessionId),
...
describe("desktop branch creation idempotency", () => {
  ...
  it("coalesces duplicate stored-session branch attempts onto one backend child", async () => {
    ...
    setSessions([
      storedSession({ id: "parent", message_count: 2, title: "Parent" }),
    ]);
    ...
    await waitFor(() =>
      expect(
        requestGateway.mock.calls.filter(
    ...
    expect(
      $sessions.get().filter((session) => session.id === "stored-branch"),
    ).toHaveLength(1);
  });
  ...
  it("does not coalesce two same-id parents that live on different connections", async () => {
    ...
    const pandoraCreate = deferred<{
      session_id: string;
      stored_session_id: string;
    }>();
    const otherCreate = deferred<{
      session_id: string;
      stored_session_id: string;
    }>();

    routedCreate.mockImplementation((async (
      connectionId: string,
      _profile: string,
      method: string,
    ) => {
      ...
      return connectionId === "pandora"
        ? pandoraCreate.promise
        : otherCreate.promise;
    }) as never);
    ...
    await act(async () => {
      ...
      await waitFor(() =>
        expect(
          routedCreate.mock.calls.filter(
            ([, , method]) => method === "session.create",
          ),
        ).toHaveLength(2),
      );
    ...
    await act(async () => {
      pandoraCreate.resolve({
        session_id: "rt-pandora",
        stored_session_id: "stored-pandora",
      });
      otherCreate.resolve({
        session_id: "rt-other",
        stored_session_id: "stored-other-box",
      });
      await expect(Promise.all([first, second])).resolves.toEqual([true, true]);
    });

    const creates = routedCreate.mock.calls.filter(
      ([, , method]) => method === "session.create",
    );

    expect(creates.map(([connectionId]) => connectionId)).toEqual([
      "pandora",
      "other-box",
    ]);
    // Two distinct children, not one child claimed twice.
    expect(
      $sessions.get().filter((session) => session.id === "stored-pandora"),
    ).toHaveLength(1);
    expect(
      $sessions.get().filter((session) => session.id === "stored-other-box"),
    ).toHaveLength(1);
  });
});
...
describe("connection-qualified session deletion", () => {
  ...
  it("tears down the selected session from synchronous refs when render state is stale", async () => {
    ...
    const requestGateway = vi.fn().mockResolvedValue({});
    const activeSessionIdRef: MutableRefObject<null | string> = {
      current: "runtime-shared",
    };
    const selectedStoredSessionIdRef: MutableRefObject<null | string> = {
...
describe("active stored-session id rotation routing", () => {
  ...
  it("follows a rotation while the same conversation still owns the foreground route", async () => {
    ...
    await waitFor(() =>
      expect(selectedStoredSessionIdRef.current).toBe("stored-A-next"),
    );
    expect($selectedStoredSessionId.get()).toBe("stored-A-next");
    expect(navigate).toHaveBeenCalledWith(sessionRoute("stored-A-next"), {
      replace: true,
    });
...
async function createWith(
  ...
  render(
    <Harness onReady={(h) => (handle = h)} requestGateway={requestGateway} />,
  );
...
describe("createBackendSessionForSend profile routing", () => {
  ...
  it("keeps a route-aware New Chat pinned when foreground activation changes before Send", async () => {
    ...
    expect(ambientRequest).not.toHaveBeenCalledWith(
      "session.create",
      expect.anything(),
    );
  });
...
function ResumeHarness({
  ...
  sessionStateByRuntimeIdRef?: MutableRefObject<
  ...
  const actions = useSessionActions({
    ...
    updateSessionState: (sessionId, updater, storedSessionId) => {
      ...
      const current =
        stateMapRef.current.get(sessionId) ??
...
describe("resumeSession failure recovery", () => {
  ...
  async function runResume(
    ...
    let resume:
      | ((storedSessionId: string, replaceRoute?: boolean) => Promise<unknown>)
      | null = null;
    render(
      <ResumeHarness
        onReady={(r) => (resume = r)}
  ...
  it.each([
    ...
    "restores a pending clarify as a running inline card for a %s transcript",
    async (_shape, content) => {
      ...
      const clarifyMessages =
        state?.messages.filter((message) =>
          message.parts.some(
            (part) => part.type === "tool-call" && part.toolName === "clarify",
          ),
        ) ?? [];
  ...
  it("restores a pending batch clarify whose resume snapshot has no top-level question", async () => {
          ...
          tool_calls: [
            {
              function: {
                arguments:
                  '{"questions":[{"question":"Color?"},{"question":"Size?"}]}',
                name: "clarify",
              },
    ...
    expect(state?.messages.filter((message) => message.pending)).toHaveLength(
      1,
    );
    expect(state?.streamId).toBe(
      state?.messages.find((message) => message.pending)?.id,
    );
  });
  ...
  it("paints the REST transcript before a cold session.resume settles and keeps it when resume rejects", async () => {
    ...
    vi.mocked(getLatestSessionMessages).mockResolvedValue({
      messages: [
        ...
        {
          content: "history visible before runtime",
          role: "assistant",
          timestamp: 2,
        },
      ],
    ...
    await waitFor(() =>
      expect(JSON.stringify($messages.get())).toContain(
        "history visible before runtime",
      ),
    );
  ...
  it("preserves an optimistic user message during a same-session reconnect", async () => {
    ...
    expect($messages.get().map((message) => message.id)).toContain(
      "user-optimistic",
    );
  });
  ...
  it("keeps the complete transcript with the live tail after a full renderer restart", async () => {
    const storedMessages = [
      {
        content: "older question removed by compression",
        role: "user",
        timestamp: 1,
      },
      {
        content: "older answer removed by compression",
        role: "assistant",
        timestamp: 2,
      },
  ...
  it("preserves a runtime-cache delta that arrives while cold resume waits for REST", async () => {
    const persisted =
      deferred<Awaited<ReturnType<typeof getLatestSessionMessages>>>();

    const sessionStateByRuntimeIdRef: MutableRefObject<
      Map<string, ClientSessionState>
    > = {
      current: new Map(),
    };
    ...
    const streamingAssistantRows = resumedState?.messages.filter((message) =>
  ...
  it("does not reuse an empty cached runtime view for a stored session with history", async () => {
    ...
    expect(requestGateway).not.toHaveBeenCalledWith("session.usage", {
...
describe("session.resume turn timer contract", () => {
  ...
  it("restores the canonical gateway turn timestamp in milliseconds", async () => {
    ...
    expect($turnStartedAt.get()).toBe(
      sessionResumeActiveTurn.turn_started_at * 1000,
    );
  });
  ...
  it("clears a stale timer when the running gateway response omits its timestamp", async () => {
    const missingTimestamp: Record<string, unknown> = JSON.parse(
      JSON.stringify(sessionResumeActiveTurn),
    );
  ...
  it("clears a stale timer when the running gateway response has a non-numeric timestamp", async () => {
    ...
    await resumeFrom({
      ...sessionResumeActiveTurn,
      turn_started_at: "not-a-timestamp",
    });
...
function BranchHarness({
  ...
  const selectedStoredSessionIdRef = ref<string | null>(
    selectedStoredSessionId,
  );
...
describe("branchStoredSession desktop source tagging", () => {
  ...
  it("opens the branch as the primary session in the main workspace (#93444)", async () => {
    ...
    let branchStoredSession:
      ((storedSessionId: string) => Promise<boolean>) | null = null;
    render(
      <BranchHarness
        navigate={navigate}
    ...
    expect(navigate).toHaveBeenCalledWith(sessionRoute("branch-stored"), {
      replace: true,
    });
    ...
    expect(
      $sessionTiles
        .get()
        .some((tile) => tile.storedSessionId === "branch-stored"),
    ).toBe(false);
  });
  ...
  it("tags the optimistic branch row with the parent connection owner", async () => {
    ...
    const row = $sessions
      .get()
      .find((session) => session.id === "branch-stored");
  ...
  it("branches an open live chat via session.branch with a trimmed message count (bug #1/#3 fix)", async () => {
    ...
    let branchCurrentSession:
      ((messageId?: string) => Promise<boolean>) | null = null;
    render(
      <BranchHarness
  ...
  it("hydrates the complete persisted display transcript before branching a compacted live chat", async () => {
    ...
    expect(getAllSessionMessages).toHaveBeenCalledWith(
      "stored-parent",
      undefined,
    );
...
describe("resumeSession drops a redundant tile when the session loads into main", () => {
  ...
  it("closes the tile so the session is not open in both main and its own tab", async () => {
    ...
    expect(
      $sessionTiles.get().some((t) => t.storedSessionId === "stored-1"),
    ).toBe(false);
  ...
  it("leaves OTHER sessions tiles untouched", async () => {
    ...
    expect($sessionTiles.get().map((t) => t.storedSessionId)).toEqual([
      "stored-2",
    ]);
  });
});
...
const clientState = (storedSessionId: string | null): ClientSessionState =>
...
describe("resumeSession warm-cache mapping integrity", () => {
  ...
  it("pins metadata, transcript, resume, activate, and usage to the captured connection", async () => {
    ...
    vi.mocked(getSession).mockImplementation(async (id) =>
      storedSession({ id, profile: "default" }),
    );
    vi.mocked(getLatestSessionMessages).mockImplementation(
      async (id) => ({ messages: [], session_id: id }) as never,
    );
    vi.mocked(requestGatewayForAgent).mockImplementation(
      async (_connectionId, _profile, method, params) => {
  ...
  it("rejects a cross-wired runtime mapping and falls through to a full resume", async () => {
    ...
    const resumeCalls = requestGateway.mock.calls.filter(
  ...
  it("paints the bounded latest transcript before the deferred resume acknowledgement without rebuilding it", async () => {
    ...
    const requestGateway = <T,>(
      method: string,
      params?: Record<string, unknown>,
    ): Promise<T> => requestGatewayMock(method, params) as Promise<T>;
  ...
  it("re-arms a pending clarify in place on the warm session.activate path", async () => {
          ...
          tool_calls: [
            {
              function: {
                arguments:
                  '{"question":"Which path?","choices":["safe","fast"]}',
                name: "clarify",
              },
              id: "call-provider",
            },
    ...
    expect(requestGateway.mock.calls.map(([method]) => method)).toContain(
      "session.activate",
    );
    expect(clarifyMessages).toHaveLength(1);
    expect(clarifyMessages[0].pending).toBe(true);
    expect(
      clarifyMessages[0].parts.find(
        (part) => part.type === "tool-call" && part.toolName === "clarify",
      ),
  ...
  it.each([
    ...
    "de-arms stale clarify state %s when session.activate reports no pending request",
    async (_case, keepStore) => {
      const runtimeIdByStoredSessionIdRef: MutableRefObject<
        Map<string, string>
      > = {
        current: new Map([["stored-A", "rt-A"]]),
      };
      ...
      expect(
        resumedState?.messages.find(
          (message) => message.id === resumedState.streamId,
        ),
      ).toBeUndefined();
  ...
  it("reads the terminal transcript after warm reconnect transport reattachment", async () => {
    ...
    vi.mocked(getLatestSessionMessages).mockImplementation(async () => {
      ...
      return {
        messages: [
          ...
          {
            content: transportAttached
              ? "complete answer persisted during disconnect"
    ...
    expect(getLatestSessionMessages).toHaveBeenCalledTimes(1);
    expect(JSON.stringify(resumedState?.messages)).toContain(
      "complete answer persisted during disconnect",
    );
    expect(JSON.stringify(resumedState?.messages)).not.toContain(
      "partial before disconnect",
    );
  });
  ...
  it("preserves cached image attachments through an idle persisted transcript refresh", async () => {
    ...
    expect(resumedState?.messages[0]?.attachmentRefs).toEqual([
      "@image:/tmp/photo.png",
    ]);
  });
  ...
  it("repairs an idle warm cache from a divergent equal-length persisted transcript", async () => {
    ...
    const persistedMessages = [
      ...
      {
        content: "answer saved after compression",
        role: "assistant",
        timestamp: 4,
      },
    ];
  ...
  it("keeps the complete persisted transcript when activating a compressed running session", async () => {
    ...
    const persistedMessages = [
      {
        content: "older prompt that compression removed",
        role: "user",
        timestamp: 1,
      },
      {
        content: "older answer that compression removed",
        role: "assistant",
        timestamp: 2,
      },
  ...
  it("preserves live cache updates that arrive while the persisted transcript is loading", async () => {
    ...
    const liveMessages = liveState.messages.map((message) =>
      message.id === "assistant-stream-live-123"
        ? {
            ...message,
            parts: [{ type: "text" as const, text: "partial A + delta B" }],
          }
        : message,
    );
  ...
  it("does not duplicate an in-flight user prompt already present in the persisted suffix", async () => {
    ...
    const persistedMessages = [
      {
        content: "older prompt removed by compression",
        role: "user",
        timestamp: -1,
      },
      {
  ...
  it("keeps a warm runtime and optimistic turn on a transient activation timeout", async () => {
    ...
    expect(requestGateway.mock.calls.map(([method]) => method)).not.toContain(
      "session.resume",
    );
    expect(runtimeIdByStoredSessionIdRef.current.get("stored-A")).toBe("rt-A");
    expect(
      sessionStateByRuntimeIdRef.current.get("rt-A")?.messages[0]?.id,
    ).toBe("user-optimistic");
  });
  ...
  it("never publishes a tail-only warm cache before the full persisted history", async () => {
    ...
    cachedState.messages = [
      {
        ...
        parts: [
          {
            type: "text",
            text: "latest question after long-context completion",
          },
        ],
      },
      {
        ...
        role: "assistant",
        parts: [
          { type: "text", text: "latest answer after long-context completion" },
        ],
      },
    ];
    ...
    vi.mocked(getLatestSessionMessages).mockReturnValue(
      persistedAuthority.promise as never,
    );
        ...
        onViewSync={(_sessionId, state) => {
          ...
          publications.push({
            latest: snapshot.includes(
              "latest question after long-context completion",
            ),
            older: snapshot.includes(
              "earlier question before long-context completion",
            ),
          });
        }}
        requestGateway={requestGateway}
    ...
    persistedAuthority.resolve({
      messages: [
        {
          content: "earlier question before long-context completion",
          role: "user",
          timestamp: 1,
        },
        {
          content: "earlier answer before long-context completion",
          role: "assistant",
          timestamp: 2,
        },
        {
          content: "latest question after long-context completion",
          role: "user",
          timestamp: 3,
        },
        {
          content: "latest answer after long-context completion",
          role: "assistant",
          timestamp: 4,
        },
      ],
    ...
    expect(
      publications.some((snapshot) => snapshot.latest && snapshot.older),
    ).toBe(true);
    expect(
      publications.filter((snapshot) => snapshot.latest && !snapshot.older),
...
describe("removeSession / archiveSession profile routing (#78836)", () => {
  ...
  it("resolves a profile-less messaging DELETE before drop, without leaking into recents", async () => {
    ...
    setMessagingSessions([
      storedSession({ id: "tg-1", source: "telegram", title: "QQ/TG" }),
    ]);
    mockGetSession.mockImplementation(
      async (id: string, scope?: ProfileScope) => {
        expect(
          $messagingSessions.get().some((session) => session.id === "tg-1"),
        ).toBe(true);
        const profile =
          scope && typeof scope === "object" ? scope.profile : scope;
  ...
  it("restores a failed DELETE to the messaging slice, not recents", async () => {
    ...
    expect($messagingSessions.get().map((session) => session.id)).toEqual([
      "tg-roll",
    ]);
    expect($sessions.get()).toEqual([]);
  ...
  it("archives a messaging row against its owning profile", async () => {
    ...
    setMessagingSessions([
      storedSession({ id: "tg-arch", profile: "winefox", source: "telegram" }),
    ]);
  ...
  it("still DELETEs a desktop-native session with its listed profile", async () => {
    mockDeleteSession.mockResolvedValue({ ok: true });
    setSessions([
      storedSession({ id: "desk-1", profile: "default", source: "desktop" }),
    ]);
  ...
  it("does not restore a failed cron DELETE into recents", async () => {
    setCronSessions([
      storedSession({ id: "cron-1", profile: "winefox", source: "cron" }),
    ]);
    ...
    expect($cronSessions.get().map((session) => session.id)).toEqual([
      "cron-1",
    ]);
    expect($sessions.get()).toEqual([]);
    expect($messagingSessions.get()).toEqual([]);
  ...
  it("fails closed when a listed profile-less messaging archive cannot resolve an owner", async () => {
    setMessagingSessions([
      storedSession({ id: "tg-arch-unresolved", source: "telegram" }),
    ]);
    mockGetSession.mockRejectedValue(new Error("404: Session not found"));
...
describe("routed fresh chat keeps its exact owner across turns", () => {
  const route: SessionProfileRoute = {
    connectionId: "local",
    mode: "local",
    profile: "omar",
  };
  ...
  async function createRoutedFreshChat() {
    ...
    const ambientRequest = vi.fn(
      async (method: string, params?: Record<string, unknown>) => {
        if (typeof params?.session_id === "string") {
          throw new Error(
            `Session not found: ${params.session_id} (ambient/default backend, ${method})`,
          );
        }
    ...
    vi.mocked(requestGatewayForAgent).mockImplementation(
      async (connectionId, profile, method) => {
        ...
        throw new Error(
          `Session not found (${connectionId}::${profile}, ${method})`,
        );
      },
    );
  ...
  it("unit: an explicitly routed fresh create never resolves its follow-up owner to the ambient default", async () => {
    ...
    const foregroundScope = registryBackendScopeKey(
      route.connectionId,
      route.profile,
    );
    ...
    expect(
      $sessions
        .get()
        .find((session) => sessionMatchesStoredId(session, STORED)),
    ).toMatchObject({
      connection_id: "local",
  ...
  it("integration: both turns hit local::omar with default ambient — no session-not-found, no orphaned runtime", async () => {
    ...
    const submits = vi
      ...
      .mock.calls.filter((call) => call[2] === "prompt.submit");

    expect(
      submits.map((call) => [
        call[0],
        call[1],
        (call[3] as { text: string }).text,
      ]),
    ).toEqual([
    ...
    expect(
      ambientRequest.mock.calls.filter(
        (call) => typeof call[1]?.session_id === "string",
      ),
    ).toEqual([]);
    ...
    expect(
      vi
        .mocked(requestGatewayForAgent)
        .mock.calls.filter((call) => call[2] === "session.close"),
    ).toEqual([]);
    expect(ambientRequest).not.toHaveBeenCalledWith(
      "session.close",
      expect.anything(),
