...
} from "@/components/assistant-ui/reference-kinds";
import {
  type ChatMessage,
...
describe("sessionMatchesStoredId", () => {
  it("matches on live id or lineage root", () => {
    ...
    expect(
      sessionMatchesStoredId(
        session({ id: "live", _lineage_root_id: "root" }),
        "root",
      ),
    ).toBe(true);
...
describe("sessionShouldHaveTranscript", () => {
  it("is true only when the session has messages", () => {
    expect(sessionShouldHaveTranscript(session({ message_count: 3 }))).toBe(
      true,
    );
    expect(sessionShouldHaveTranscript(session({ message_count: 0 }))).toBe(
      false,
    );
...
describe("selectBranchMessages", () => {
  it("uses the complete authoritative transcript for a whole-chat branch", () => {
    const local = [
      ...
      msg("tail", "assistant", "latest answer"),
    ];
    ...
    expect(
      selectBranchMessages(local, authoritative).map(
  ...
  it("maps a clicked local bubble to the authoritative row before slicing", () => {
    ...
    expect(
      selectBranchMessages(local, authoritative, "tail-assistant").map(
        (message) => message.content,
      ),
    ).toEqual([
      "first question",
...
describe("chatMessagesEquivalent", () => {
  ...
  it("returns false when message IDs differ", () => {
    expect(
      chatMessagesEquivalent(
        msg("msg-1", "user", "Hello"),
        msg("msg-2", "user", "Hello"),
      ),
    ).toBe(false);
  });
...
describe("reconcileResumeMessages", () => {
  ...
  it("prefers a richer local pending assistant over an empty projection shell", () => {
    const previous = [
      ...
      msg("assistant-stream-live", "assistant", "hello from stream", {
        pending: true,
      }),
    ];
  ...
  it("prefers a richer local pending assistant when the projection lags mid-stream", () => {
    const previous = [
      msg("1-user", "user", "question"),
      msg("assistant-stream-live", "assistant", "hello world", {
        pending: true,
      }),
    ];
  ...
  it("does not override when the authoritative assistant has advanced further", () => {
    ...
    const next = [
      msg("1-user", "user", "question"),
      msg("assistant-stream-sess", "assistant", "hello world", {
        pending: true,
      }),
    ];
  ...
  it("prefers a traces-only local pending row over an empty shell", () => {
    const previous = [
      ...
      streamingMsg("assistant-stream-live", ""),
    ];
    ...
    expect(reconciled[1].id).toBe("assistant-stream-live");
    expect(reconciled[1].parts.map((part) => part.type)).toEqual([
      "reasoning",
      "tool-call",
    ]);
  });
  ...
  it("leaves a shorter non-prefix authoritative assistant intact", () => {
    const previous = [
      ...
      msg(
        ...
        "assistant",
        "a long local reply about something else entirely",
        { pending: true },
      ),
    ];
    ...
    const next = [
      msg("1-user", "user", "question"),
      msg("9-assistant", "assistant", "short authoritative answer"),
    ];
  ...
  it("does not treat an errored authoritative row as an empty shell", () => {
    ...
    const next = [
      msg("1-user", "user", "do the thing"),
      msg("assistant-stream-sess", "assistant", "", {
  ...
  it("takes the local body but the authoritative settled state", () => {
    const previous = [
      msg("1-user", "user", "question"),
      msg("assistant-stream-live", "assistant", "streamed body", {
        pending: true,
      }),
    ];
    ...
    const next = [
      msg("1-user", "user", "question"),
      msg("assistant-stream-sess", "assistant", "", { pending: false }),
    ];
...
describe("preserveLocalPendingTurnMessages", () => {
  it("keeps an optimistic user turn and pending assistant when the server projection is behind", () => {
    ...
    expect(
      preserveLocalPendingTurnMessages(next, previous).map(
        (message) => message.id,
      ),
    ).toEqual([
      "1-user",
  ...
  it("drops stale optimistic history after compression and keeps only the live tail", () => {
    ...
    ).toEqual(["stored-user", "stored-assistant", "user-inflight"]);
  });
  ...
  it("keeps every optimistic user row in the live run after a mid-turn redirect", () => {
    ...
    expect(
      preserveLocalPendingTurnMessages([], previous).map(
        (message) => message.id,
      ),
    ).toEqual(["user-1000", "user-2000", "assistant-stream-1"]);
  });
  ...
  it("keeps the whole live run when sealed live output sits between prompt and correction", () => {
    const previous = [
      msg("user-1000", "user", "remove the session counts"),
      msg("assistant-stream-1", "assistant", "two screens of output", {
        interim: true,
      }),
      msg("user-2000", "user", "hurry up"),
      msg("assistant-stream-2", "assistant", "post-redirect output", {
        pending: true,
      }),
    ];
  ...
  it.each(
    WIRE_REFERENCE_KINDS.filter((kind) => kind !== "file" && kind !== "image"),
  )(
  ...
  it("does not duplicate a turn with multiple CRLF directives and Unicode payloads", () => {
    const refs = [
      "@file:`資料/über notes.md`",
      "@url:`https://example.com/café?q=✓`",
    ];
  ...
  it("strips only complete reference lines from visible text", () => {
    expect(textWithoutReferenceLines("see @file:X here")).toBe(
      "see @file:X here",
    );
    expect(textWithoutReferenceLines("@file:X trailing prose")).toBe(
      "@file:X trailing prose",
    );
  ...
  it("replaces an empty inflight shell with a fuller local pending assistant", () => {
    const previous = [
      msg("1-user", "user", "question"),
      msg("assistant-stream-live", "assistant", "partial answer so far", {
        pending: true,
      }),
    ];
  ...
  it("replaces a lagging same-id shell with the fuller local pending body", () => {
    const previous = [
      msg("1-user", "user", "question"),
      msg("assistant-stream-sess", "assistant", "full streamed content", {
        pending: true,
      }),
    ];
  ...
  it("still drops local pending when authoritative text is at least as complete", () => {
    ...
    const next = [
      msg("1-user", "user", "question"),
      msg("assistant-stream-sess", "assistant", "partial and more", {
        pending: true,
      }),
    ];
  ...
  it("replaces an empty shell with a traces-only local pending row", () => {
    ...
    expect(preserved[1].parts.map((part) => part.type)).toEqual([
      "reasoning",
      "tool-call",
    ]);
  });
  ...
  it("does not erase a retained failure with the local partial", () => {
    ...
    const assistant = preserveLocalPendingTurnMessages(next, previous).find(
  ...
  it("does not re-append a settled stream row the authoritative history already carries", () => {
    const next = [
      ...
      msg("2-assistant-stored", "assistant", "answer"),
    ];
    const settledLocalStream = msg(
    ...
    expect(
      preserveLocalPendingTurnMessages(next, [...next, settledLocalStream]),
    ).toBe(next);
  });
  ...
  it("keeps a settled stream row the authoritative history has not committed", () => {
    const previous = [
      msg("1-user", "user", "question"),
      msg("assistant-stream-sess", "assistant", "the finished reply", {
        pending: false,
      }),
    ];
    ...
    ).toEqual(["1-user", "assistant-stream-sess"]);
  });
  ...
  it("does not duplicate or rewrite committed history around the live turn", () => {
    ...
    const previous = [
      ...history,
      streamingMsg("assistant-stream-live", "here is the full reply"),
    ];
    const next = [
      ...history,
      msg("assistant-stream-sess", "assistant", "", { pending: true }),
    ...
    expect(
      preserved.filter((message) => message.role === "assistant"),
    ).toHaveLength(2);
  });
  ...
  it("does not re-append a pending stream row the authoritative history already carries", () => {
    ...
    const next = [
      msg("1-user", "user", "查金价"),
      msg("9-assistant", "assistant", "面板内容"),
    ];
  ...
  it("drops a pending stream row whose text the committed authoritative reply extends", () => {
    ...
    const next = [
      msg("1-user", "user", "查金价"),
      msg("9-assistant", "assistant", "面板内容完整版"),
    ];
  ...
  it("still keeps a pending stream row when the authoritative history has no reply", () => {
    ...
    ).toEqual(["1-user", "assistant-stream-live"]);
  });
});
...
describe("appendLiveSessionProjection", () => {
  ...
  it("does not re-project a correction the transcript already persisted", () => {
    const stored = [
      msg("stored-user", "user", "remove the session counts"),
      msg("stored-fix", "user", "hurry up"),
    ];
    ...
    expect(
      restored
        ...
        .map((message) => message.id),
    ).toEqual(["stored-user", "stored-fix"]);
  });
  ...
  it("preserves the original array when no live projection exists", () => {
    ...
    expect(
      appendLiveSessionProjection(stored, { session_id: "runtime-1" }),
    ).toBe(stored);
  });
  ...
  it("does not sandwich a structured mid-turn row with the inflight flat dump (#76444)", () => {
    ...
    expect(assistants[0].parts.some((part) => part.type === "reasoning")).toBe(
      true,
    );
    expect(assistants[0].parts.some((part) => part.type === "tool-call")).toBe(
      true,
    );
  ...
  it("still projects inflight when only a completed historical tool reply has structure", () => {
    ...
    expect(restored.map((message) => message.id)).toContain(
      "assistant-stream-runtime-1",
    );
    expect(restored.at(-1)).toMatchObject({
      id: "assistant-stream-runtime-1",
...
describe("dedupeInflightUserAgainstTranscript", () => {
  ...
  it("preserves an intentionally repeated prompt when the match is before the runtime anchor", () => {
    ...
    const unchanged = dedupeInflightUserAgainstTranscript(
      runtime,
      runtime,
      projection,
    );
  ...
  it("preserves a repeated in-flight prompt when the persisted match already has an answer", () => {
    ...
    const persisted = [
      ...
      msg("persisted-repeat-answer", "assistant", "finished repeat answer", {
        timestamp: 4,
      }),
    ];
...
describe("removeRepresentedLocalLiveProjection", () => {
  it("removes only matched synthetic rows from the open local tail", () => {
    ...
    const remaining = removeRepresentedLocalLiveProjection(
      previous,
      projection,
    );
  ...
  it("preserves an ambiguous text-identical local race prompt without a matching stream boundary", () => {
    ...
    expect(removeRepresentedLocalLiveProjection(previous, projection)).toBe(
      previous,
    );
  });
...
describe("overlayConcurrentMessageChanges", () => {
  it("does not replace an authoritative row with an unchanged baseline cache row", () => {
    const baseline = [
      msg("shared-assistant", "assistant", "stale cached answer"),
    ];
    const authoritative = [
      msg("shared-assistant", "assistant", "completed persisted answer"),
    ];
    ...
    expect(overlaid[0].parts).toEqual([
      { type: "text", text: "completed persisted answer" },
    ]);
  });

  it("replaces an activation stream placeholder and appends rows created after the baseline", () => {
    const baseline = [
      msg("assistant-stream-runtime", "assistant", "partial A", {
        pending: true,
      }),
    ];
    const authoritative = [
    ...
    expect(overlaid[0].parts).toEqual([
      { type: "text", text: "partial A + delta B" },
    ]);
  });
...
describe("preserveEquivalentTranscript", () => {
  it("keeps the current array BY REFERENCE when the replacement is content-equivalent", () => {
    ...
    const current = [
      msg("u-1", "user", "hello"),
      msg("a-1", "assistant", "const x = 1"),
    ];
    const freshObjects = current.map((message) => ({
      ...message,
      parts: [...message.parts],
    }));
