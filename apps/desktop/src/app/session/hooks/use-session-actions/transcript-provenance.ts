...
export function createPersistedDisplayTranscriptProvenance({
  ...
  const connectionId =
    typeof scope === "object" && scope ? (scope.connectionId ?? "").trim() : "";
...
export function withoutTranscriptProvenance(
  ...
  const { transcriptProvenance: _transcriptProvenance, ...withoutProvenance } =
    state;

  return withoutProvenance;
}

export function invalidatePersistedDisplayTranscriptAuthority(
  state: ClientSessionState,
): ClientSessionState {
  return {
    ...state,
