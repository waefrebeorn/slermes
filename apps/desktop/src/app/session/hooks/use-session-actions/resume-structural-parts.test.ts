...
describe("reconcileResumeMessages — structural parts on a mid-turn switch", () => {
  it("keeps tool-call parts the authoritative text-only row cannot carry", () => {
    ...
    expect(assistant.parts.filter((p) => p.type === "tool-call")).toHaveLength(
      1,
    );
    expect(assistant.parts.filter((p) => p.type === "reasoning")).toHaveLength(
      1,
    );
    // The newer authoritative text still wins.
    expect(
      assistant.parts.filter((p) => p.type === "text").at(-1),
    ).toMatchObject({
  ...
  it("keeps live-tail structure when the flat dump is not a strict text extension", () => {
    ...
    expect(assistant.parts.some((part) => part.type === "reasoning")).toBe(
      true,
    );
    expect(assistant.parts.some((part) => part.type === "tool-call")).toBe(
      true,
    );
    expect(
      assistant.parts
  ...
  it("does not graft historical structure onto a live text-only row after compression rewrote ordinals", () => {
    ...
    expect(assistant.parts).toEqual([
      { type: "text", text: "brand new partial" },
    ]);
  });
});
