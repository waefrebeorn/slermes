...
export function useHermesConfig({ activeSessionIdRef }: HermesConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(
    DEFAULT_VOICE_SECONDS,
  );
    ...
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        ...
        const personality = normalizePersonalityValue(
          typeof config.display?.personality === "string"
        ...
        setCurrentPersonality((prev) =>
          activeSessionIdRef.current ? prev || personality : personality,
        );
        setAvailablePersonalities([
          ...new Set([
            "none",
        ...
        setCurrentServiceTier((prev) =>
          activeSessionIdRef.current ? prev : tier,
        );
        ...
        setVoiceMaxRecordingSeconds(
          recordingLimit(config.voice?.max_recording_seconds),
        );
