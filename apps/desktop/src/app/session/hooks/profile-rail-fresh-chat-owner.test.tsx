...
const sessionScoped = (params: unknown) =>
  typeof (params as { session_id?: unknown } | undefined)?.session_id ===
  "string";
...
function answer(
  ...
  if (method === "session.create") {
    if (!isOmar) {
      throw new Error(
        `session.create landed on the wrong socket: ${socket.connectUrl}`,
      );
    }

    if (runtimeOwner) {
      throw new Error(
        `session.create attempted to replace runtime owner: ${socket.connectUrl}`,
      );
    }
...
vi.mock("@/hermes", async (importOriginal) => ({
  ...
  getSession: vi.fn(async () => {
    throw new Error(
      "REST cross-profile probe must not be needed: the owner is known",
    );
  }),
  ...
  (window as unknown as { hermesDesktop: unknown }).hermesDesktop = {
    ...
    getConnection: vi.fn(async (profile: null | string) => {
      ...
      return {
        port,
        profile,
        token: profile ? "v1-token" : "primary-token",
...
interface HarnessHandle {
  ...
  bindings: () => {
    runtimeForStored: null | string;
    storedForRuntime: null | string;
  };
...
function Harness({
  ...
  useEffect(() => {
    onReady({
      ...
      bindings: () => ({
        runtimeForStored:
          cache.runtimeIdByStoredSessionIdRef.current.get(mintedStoredId) ??
          null,
        storedForRuntime:
          cache.sessionStateByRuntimeIdRef.current.get(mintedRuntimeId)
            ?.storedSessionId ?? null,
      }),
      submitText: (...args) =>
        act(async () => submitText(...args)) as Promise<boolean>,
      updateSessionState:
        cache.updateSessionState as HarnessHandle["updateSessionState"],
    });
  }, [
    cache.runtimeIdByStoredSessionIdRef,
    cache.sessionStateByRuntimeIdRef,
...
describe("profile rail: a fresh Omar chat keeps its exact registry owner across turns (#94071)", () => {
  ...
  async function bootProfileRailOmar(
    startDraft: "newSessionInProfile" | "selectProfile" = "selectProfile",
  ) {
    // Primary / ambient source: a remote gateway on `default`.
    ...
    const omarSocket = sockets.find((socket) =>
      socket.connectUrl?.includes(`:${OMAR_PORT}`),
    );
    expect(
      omarSocket,
    ...
    const ambientRequest = vi.fn(
      async (method: string, params?: Record<string, unknown>) => {
        if (method === "session.create" || sessionScoped(params)) {
          throw new Error(
            `session traffic must not use ambient dispatcher: ${method}`,
          );
        }

        return (activeGateway() as unknown as MockGateway).request(
          method,
          params,
        );
      },
  ...
  const calls = (socket: MockGateway) =>
    socket.request.mock.calls.map((call) => call[0] as string);
  ...
  function expectUninterruptedOwner({
    ...
    expect(
      ambientRequest.mock.calls.filter(
        (call) => call[0] === "session.create" || sessionScoped(call[1]),
      ),
    ).toEqual([]);
    ...
    for (const socket of [primary, ...sockets]) {
      ...
      expect(
        calls(socket).filter((method) =>
          ["session.activate", "session.close", "session.resume"].includes(
            method,
          ),
        ),
      ).toEqual([]);
    ...
    expect(
      registryOnEvent.mock.calls.filter(
        ([event]) => event.type === "session.reclaimed",
      ),
    ).toEqual([]);
  }

  it("session.create and both prompt.submit calls ride the SAME conn:homelab::omar socket", async () => {
    const { ambientRequest, handle, omarSocket, primary } =
      await bootProfileRailOmar();
    ...
    expect(handle.bindings()).toEqual({
      runtimeForStored: RUNTIME_ID,
      storedForRuntime: STORED_ID,
    });
    ...
    expect(
      omarCalls.filter((method) => method === "prompt.submit"),
    ).toHaveLength(2);
    expect(
      omarSocket.request.mock.calls
        .filter((call) => call[0] === "prompt.submit")
        .map((call) => [
          (call[1] as { session_id: string }).session_id,
          (call[1] as { text: string }).text,
        ]),
    ).toEqual([
    ...
    expect(
      calls(primary).filter(
        (method) => method === "session.create" || method === "prompt.submit",
      ),
    ).toEqual([]);
    ...
    for (const socket of sockets) {
      if (socket !== omarSocket) {
        expect(
          socket.request.mock.calls.filter(
            (call) => sessionScoped(call[1]) || call[0] === "session.create",
          ),
        ).toEqual([]);
      }
    }
    ...
    expect(
      sockets.some((socket) => socket.connectUrl?.includes(`:${V1_PORT}`)),
    ).toBe(false);
    ...
    for (const socket of [primary, ...sockets]) {
      expect(
        calls(socket).filter((method) => method === "session.close"),
      ).toEqual([]);
    }

    expect(
      omarCalls.filter((method) => method === "session.create"),
    ).toHaveLength(1);
    expectUninterruptedOwner({
      ambientRequest: ambientRequest as GatewayRequestMock,
      omarSocket,
      primary,
    });
    expect(getSessionOwnerHint(STORED_ID)).toEqual({
      connectionId: SOURCE_ID,
      profile: "omar",
    });
    expect(
      $sessions
        .get()
        .find((session) => sessionMatchesStoredId(session, STORED_ID)),
    ).toMatchObject({
  ...
  it("turn two still rides conn:homelab::omar after the transient hint is evicted AND a refresh returned the row untagged", async () => {
    const { ambientRequest, handle, omarSocket, primary } =
      await bootProfileRailOmar("newSessionInProfile");
    ...
    expect(
      calls(omarSocket).filter((method) => method === "session.create"),
    ).toHaveLength(1);
    expectUninterruptedOwner({
      ambientRequest: ambientRequest as GatewayRequestMock,
      omarSocket,
  ...
  it("a pick on the explicit `local` source is a legacy profile pick: create and both turns ride the ONE v1 omar socket", async () => {
    ...
    const v1Socket = sockets.find((socket) =>
      socket.connectUrl?.includes(`:${V1_PORT}`),
    );
    expect(
      v1Socket,
      `no v1 socket dialed; dialed: ${sockets.map((s) => s.connectUrl).join(", ")}`,
    ).toBeDefined();
    expect(activeGateway()).toBe(v1Socket as never);
    expect(
      sockets.some((socket) => socket.connectUrl?.includes(`:${OMAR_PORT}`)),
    ).toBe(false);
    ...
    expect(handle!.bindings()).toEqual({
      runtimeForStored: LEGACY_RUNTIME_ID,
      storedForRuntime: LEGACY_STORED_ID,
    });
    ...
    expect(
      calls(v1Socket!).filter((method) => method === "session.create"),
    ).toHaveLength(1);
    expect(
    ...
    expect(
      $sessions
        .get()
        .find((session) => sessionMatchesStoredId(session, LEGACY_STORED_ID)),
    ).toMatchObject({
      profile: "omar",
    });
  });
