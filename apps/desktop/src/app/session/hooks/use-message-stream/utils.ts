...
export function sessionInfoStatePatch(
...
export function hasSessionInfoStatePatch(
...
export function applySessionInfoStatePatch(
  ...
  if (
    ...
    (patch.personality === undefined ||
      patch.personality === state.personality) &&
    (patch.provider === undefined || patch.provider === state.provider) &&
    (patch.reasoningEffort === undefined ||
      patch.reasoningEffort === state.reasoningEffort) &&
    (patch.serviceTier === undefined ||
      patch.serviceTier === state.serviceTier) &&
...
export function completionErrorText(finalText: string): string | null {
  ...
  return text && COMPLETION_ERROR_PATTERNS.some((re) => re.test(text))
    ? text
    : null;
}
...
export function toTodoPayload(
  ...
  const isTodo =
    payload.name === "todo" ||
    (!payload.name && Object.hasOwn(payload, "todos"));

  return isTodo
    ? { ...payload, name: "todo", tool_id: payload.tool_id || "todo-live" }
    : undefined;
}
...
export function delegateTaskPayloads(
  ...
  const status =
    phase === "complete" ? (failedResult ? "failed" : "completed") : "running";
  const toolId =
    payload.tool_id || payload.tool_call_id || payload.id || "delegate_task";
  const progressText = firstString(
    payload.preview,
    payload.message,
    payload.context,
  );
  ...
  return tasks.map((task, index) => {
    const goal =
      firstString(task.goal, args.goal, payload.context) || "Delegated task";
    ...
    return {
      ...
      task_index: index,
      text:
        eventType === "subagent.progress" ? progressText || goal : undefined,
      tool_name: eventType === "subagent.start" ? "delegate_task" : undefined,
      tool_preview: eventType === "subagent.start" ? progressText : undefined,
      toolsets: Array.isArray(task.toolsets)
        ? task.toolsets
        : Array.isArray(args.toolsets)
          ? args.toolsets
          : [],
