...
describe("live transcript timeline events", () => {
  ...
  it("records commentary, tool, resumed text, and turn-stop boundaries", () => {
    ...
    event("message.interim", 101.75, {
      already_streamed: true,
      text: "Let me inspect it.",
    });
    event("tool.start", 102.25, {
      args: { path: "README.md" },
      name: "read_file",
      tool_id: "call-1",
    });
    event("tool.complete", 104.5, {
      name: "read_file",
      result: { content: "ok" },
      tool_id: "call-1",
    });
    ...
    expect([assistants[0].timestamp, assistants[0].completedAt]).toEqual([
      101.125, 101.75,
    ]);
    expect(
      assistants[0].parts.map((part) => [part.timestamp, part.completedAt]),
    ).toEqual([[101.125, 101.75]]);

    expect([assistants[1].timestamp, assistants[1].completedAt]).toEqual([
      102.25, 106.875,
    ]);
    expect(assistants[1].parts.map((part) => part.type)).toEqual([
      "tool-call",
      "text",
    ]);
    expect(
      assistants[1].parts.map((part) => [part.timestamp, part.completedAt]),
    ).toEqual([
      [102.25, 104.5],
      [105.625, 106.875],
    ]);
  ...
  it("preserves cross-channel delta order inside one flush window", () => {
    ...
    event("tool.start", 203.5, {
      args: {},
      name: "terminal",
      tool_id: "call-2",
    });

    const assistant = stream
      .state(SID)
    ...
    expect(assistant?.parts.map((part) => part.timestamp)).toEqual([
      201.125, 202.25, 203.5,
    ]);
  });
  ...
  it("uses the gateway event time for an error boundary", () => {
    ...
    expect([assistant?.timestamp, assistant?.completedAt]).toEqual([
      301.875, 301.875,
    ]);
  });
