...
const completePreviewed = (text: string) =>
  act(() =>
    stream.handleEvent({
      payload: { text, response_previewed: true },
      session_id: SID,
      type: "message.complete",
    }),
  );
...
describe("useMessageStream interim text sealing", () => {
  ...
  it("hydrates an empty completion when the live transcript still ends with the user message", async () => {
    ...
    const hydrateFromStoredSession = vi.fn<() => Promise<void>>(
      async () => undefined,
    );
      ...
      states: new Map([
        [
          ...
          createClientSessionState("stored-session-1", [
            {
              id: "user-1",
              parts: [textPart("finish the task")],
              role: "user",
            },
          ]),
        ],
    ...
    expect(hydrateFromStoredSession).toHaveBeenCalledWith(
      3,
      "stored-session-1",
      SID,
    );
  ...
  it("marks sealed interim bubbles interim and leaves the final reply unmarked", async () => {
    ...
    const assistants = getState().messages.filter(
      (m) => m.role === "assistant" && !m.hidden,
    );
    const byText = (text: string) =>
      assistants.find((m) => chatMessageText(m) === text);
  ...
  it("dedupes interim text when the final response includes it", async () => {
    ...
    expect(texts).not.toContain(
      "Let me check the files.Let me check the files.",
    );
    expect(
      texts.some((t) =>
        t.includes("Let me check the files. Everything looks good."),
      ),
    ).toBe(true);
  });
  ...
  it("ignores malformed message.interim payload", async () => {
    ...
    // No payload at all
    await act(() =>
      stream.handleEvent({ type: "message.interim" } as RpcEvent),
    );
    // Empty text
    await act(() =>
      stream.handleEvent({
