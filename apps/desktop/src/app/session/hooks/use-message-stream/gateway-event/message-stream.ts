...
function surfaceBillingBlock(sessionId: string, raw: unknown): void {
  ...
  notify({
    ...
    message:
      firstBillingLine(block.message) ||
      translateNow("billingBlock.fallbackMessage"),
    // Sticky: a credit wall blocks every turn until resolved.
    durationMs: 0,
    action: {
      label: billingCtaLabel(block, ctaCopy),
...
export function handleMessageStreamEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === "message.start") {
    ...
    const seededAt =
      sessionStateByRuntimeIdRef.current.get(sessionId)?.turnStartedAt;

    if (typeof seededAt === "number") {
      console.debug("[turn-accept-latency]", {
        sessionId,
        ms: Date.now() - seededAt,
      });
    }
  ...
  if (event.type === "thinking.delta") {
    ...
    if (sessionId) {
      setSessionProviderWait(
        sessionId,
        providerWaitText(coerceGatewayText(payload?.text)),
      );
    }
  ...
  if (event.type === "reasoning.delta") {
    if (sessionId) {
      appendReasoningDelta(
        sessionId,
        coerceThinkingText(payload?.text),
        false,
        occurredAt,
      );
    }
  ...
  if (event.type === "moa.progress") {
    ...
    if (
      sessionId &&
      typeof payload?.refs_done === "number" &&
      typeof payload?.refs_total === "number"
    ) {
  ...
  if (event.type === "message.complete") {
    ...
    const finalText =
      coerceGatewayText(payload?.text) || coerceGatewayText(payload?.rendered);
    ...
    if (isActiveEvent) {
      ...
      flashPetActivity(
        { celebrate: true, reasoning: false, toolRunning: false },
        2200,
      );
