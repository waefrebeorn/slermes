 ...
 */
function sessionInfoDescribesSelectedSession(
  ...
  return $sessions
    ...
    .some(
      (session) =>
        sessionMatchesStoredId(session, infoStoredSessionId) &&
        sessionMatchesStoredId(session, selected),
    );
}
...
function maybeRebindPaneToRebuiltRuntime(ctx: GatewayEventContext): boolean {
  ...
  if (
    !selected ||
    !sessionInfoDescribesSelectedSession(payload.stored_session_id, false)
  ) {
    return false;
  }

  const activeId = $activeSessionId.get();
  const oldState = activeId
    ? deps.sessionStateByRuntimeIdRef.current.get(activeId)
    : undefined;
...
export function handleSessionInfoEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === "session.info") {
    ...
    const apply =
      (explicitSid ? isActiveEvent : !activeSessionIdRef.current) || rebound;
    ...
    const modelValueChanged =
      modelChanged &&
      payload!.model !== (knownState?.model ?? $currentModel.get());
    ...
    if (apply) {
      ...
      if (
        ...
        sessionInfoDescribesSelectedSession(
        ...
        const sameSession =
          !!sessionId && sessionId === lastCwdInfoSessionRef.current;
        ...
        (state) => {
          ...
          if (busy) {
            ...
            const gatewayTurnStartedAt =
              typeof payload!.turn_started_at === "number" &&
              payload!.turn_started_at > 0
            ...
            return {
              ...
              turnLive: true,
              turnStartedAt:
                state.turnStartedAt ?? gatewayTurnStartedAt ?? Date.now(),
            };
          }
          ...
          const withinPreStartGrace =
            ...
            Date.now() - armedAt < PRE_TURN_LIVE_SETTLE_GRACE_MS;
      ...
      void queryClient.invalidateQueries({
        queryKey:
          explicitSid && sessionId
            ? modelOptionsQueryKey(activeGatewayProfile, sessionId)
            : ["model-options"],
      });
    }
  ...
  if (event.type === "session.title") {
    ...
    if (storedId && nextTitle) {
      setSessions((prev) =>
        prev.map((s) =>
          sessionMatchesStoredId(s, storedId) ? { ...s, title: nextTitle } : s,
        ),
      );
    }
