...
const loadPreviewEngine = () => {
  ...
  return import(
    /* @vite-ignore */ "/src/app/chat/right-rail/preview-act.ts?hot=" +
      Date.now()
  )
    .catch(stable)
    .then(
      (mod) =>
        mod.actOnActivePreview as Awaited<
          ReturnType<typeof stable>
        >["actOnActivePreview"],
    );
};
...
export function handleDesktopBridgeEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === "window.read.request") {
    ...
    if (requestId) {
      ...
      void Promise.resolve(read ? read() : null).then(answer, () =>
        answer(null),
      );
    }
  ...
  if (event.type === "tour.request") {
    ...
    if (requestId) {
      ...
      if (!$toursEnabled.get()) {
        ...
        void answer({
          error: "The user has turned guided tours off.",
          success: false,
        });
    ...
    if (typeof reactedRowId === "number") {
      ...
      setMessages((messages) => {
        ...
        const byRowId = messages.find(
          (message) => message.rowId === reactedRowId,
        );
        ...
        const lastIndex = messages.findLastIndex(
          (message) =>
            message.role === reactedRole && message.rowId === undefined,
        );
        ...
        return messages.map((message, index) =>
          index === lastIndex
            ? { ...message, rowId: reactedRowId, reactions: nextReactions }
            : message,
        );
      });
    }
