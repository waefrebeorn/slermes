  ...
  beforeEach(() => {
    ...
    updateSessionState = vi.fn(
      (
        ...
        const next = updater(
          states.get(sessionId) ?? createClientSessionState(),
        );
...
describe("useMessageStream composed with the real useSessionStateCache", () => {
  ...
  let appendAssistantDelta:
    ((sessionId: string, delta: string) => void) | null = null;
  ...
  function cachedText() {
    const message = cache?.sessionStateByRuntimeIdRef.current
      .get(SID)
      ?.messages.at(-1);
