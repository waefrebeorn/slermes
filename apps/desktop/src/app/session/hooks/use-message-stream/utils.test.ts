...
const payload = (over: Record<string, unknown>): GatewayEventPayload =>
  over as GatewayEventPayload;

describe("completionErrorText", () => {
  it("flags provider/HTTP/retry failures, ignores normal text", () => {
    expect(
      completionErrorText("API call failed after 3 retries: boom"),
    ).toMatch(/^API call failed/);
    expect(completionErrorText("HTTP 500 upstream")).toMatch(/^HTTP 500/);
    expect(completionErrorText("Gateway error: nope")).toMatch(
      /^Gateway error/,
    );
...
describe("sessionInfoStatePatch / hasSessionInfoStatePatch", () => {
  it("extracts only present runtime fields", () => {
    const patch = sessionInfoStatePatch(
      payload({ model: "gpt", fast: true, branch: "main" }),
    );
    expect(patch).toMatchObject({ model: "gpt", fast: true, branch: "main" });
    expect(hasSessionInfoStatePatch(patch)).toBe(true);
    expect(hasSessionInfoStatePatch(sessionInfoStatePatch(payload({})))).toBe(
      false,
    );
  });
});
...
describe("delegateTaskPayloads", () => {
  it("returns [] for non-delegate events", () => {
    expect(
      delegateTaskPayloads(payload({ name: "web_search" }), "running"),
    ).toEqual([]);
  });
  ...
  it.each(["timeout", "error", "failed", "failure", "TIMEOUT"])(
    ...
    (resultStatus) => {
      const [spec] = delegateTaskPayloads(
        payload({
          name: "delegate_task",
          result: { status: resultStatus, summary: "timed out" },
        }),
        "complete",
      );
