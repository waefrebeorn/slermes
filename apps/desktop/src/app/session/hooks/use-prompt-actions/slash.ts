...
const wakeDeviceLabel = (device?: WakeInputDeviceStatus): string => {
  ...
  const name =
    device.name?.trim() ||
    (selector == null ? "system default" : String(selector));
...
export function useSlashCommand(deps: SlashCommandDeps) {
  ...
  return useCallback(
    async (
      rawCommand: string,
      options?: { sessionId?: string; recordInput?: boolean },
    ) => {
      ...
      async function runExec(ctx: SlashActionCtx): Promise<void> {
        ...
        if (!isDesktopSlashCommand(name)) {
          renderSlashOutput(
            desktopSlashUnavailableMessage(name) ||
              `/${name} is not available in the desktop app.`,
          );
        ...
        const handleDispatch = async (
          ...
          if (
            (dispatch.type === "send" || dispatch.type === "prefill") &&
            dispatch.notice?.trim()
          ) {
          ...
          const message =
            ("message" in dispatch ? dispatch.message : "")?.trim() ?? "";
          ...
          const projected =
            "display" in dispatch ? dispatch.display?.trim() : "";
          const displayText =
            projected || skillInvocationText(message) || undefined;
        ...
        try {
          ...
          if (
            slashExecError &&
            /not a quick\/plugin\/skill command/i.test(dispatchMessage)
          ) {
            const original =
      ...
      const actionHandlers: Record<
        DesktopActionId,
        (ctx: SlashActionCtx) => Promise<void>
      > = {
        new: async () => {
          startFreshSessionDraft();
        },
        ...
        stop: async (ctx) => {
          ...
          try {
            await withSessionNotFoundResume(
              ...
              storedSessionId,
              (liveId) =>
                requestGateway("session.interrupt", { session_id: liveId }),
              {
                requestGateway,
            ...
            lines.push(
              `Could not stop the active turn: ${err instanceof Error ? err.message : String(err)}`,
            );
          }
          ...
          try {
            ...
            lines.push(
              `Could not stop background processes: ${err instanceof Error ? err.message : String(err)}`,
            );
          }
        ...
        btw: async (ctx) => {
          ...
          try {
            const result = await requestGateway<{ task_id?: string }>(
              "prompt.btw",
              {
                session_id: sessionId,
                text: question,
        ...
        compress: async (ctx) => {
          ...
          try {
            ...
            const { result, sessionId: liveSessionId } =
              await withSessionNotFoundResume(
                sessionId,
                storedSessionId,
                (liveId) =>
            ...
            if (result?.status === "pending") {
              const pendingMessage =
                result.message || "compression still running in the background";
              notify({
                durationMs: 8_000,
                id: noticeId,
                kind: "info",
              ...
              setSessions((prev) =>
                prev.map((session) =>
                  session.id === sessionId
                    ? { ...session, title: result.info!.title || null }
                    : session,
                ),
              );
            }
            ...
            if (result?.summary?.headline) {
              const lines = [
                ...
                result.summary.token_line,
                result.summary.note,
              ].filter((line): line is string => Boolean(line));

              const aborted =
                result.status === "aborted" || result.summary.aborted === true;
        ...
        yolo: async ({ sessionHint }) => {
          ...
          if (!sid) {
            ...
            notify({
              kind: "success",
              message: next ? copy.yoloArmed : copy.yoloOff,
            });
        ...
        wake: async (ctx) => {
          ...
          const status = async (): Promise<WakeStatusResponse> => {
            const current = await requestGateway<WakeStatusResponse>(
              "wake.status",
              {
                client_capture: true,
                surface: "gui",
          ...
          try {
            ...
            if (action === "on") {
              ...
              applyWakeStopResult(
                await requestGateway<WakeStopResponse>("wake.stop", {
                  persist: true,
                }),
              );
            }
        ...
        handoff: async ({ arg, command, recordInput, sessionHint }) => {
          ...
          if (!result.ok && result.error) {
            appendSessionTextMessage(
              ...
              "system",
              recordInput
                ? slashStatusText(command, result.error)
                : result.error,
            );
          }
        },
        ...
        profile: async ({ arg }) => {
          ...
          try {
            ...
            notify({
              kind: "success",
              message: copy.newChatsProfile(match.name),
            });
          } catch (err) {
        ...
        skin: async ({ arg, command, recordInput, sessionHint }) => {
          ...
          appendSessionTextMessage(
            sid,
            "system",
            recordInput ? slashStatusText(command, message) : message,
          );
        },
        ...
        title: async (ctx) => {
          ...
          try {
            const result = await requestGateway<SessionTitleResponse>(
              "session.title",
              {
                session_id: sessionId,
                title: arg,
            ...
            setSessions((prev) =>
              prev.map((s) =>
                s.id === sessionId ? { ...s, title: finalTitle || null } : s,
              ),
            );
        ...
        help: async (ctx) => {
          ...
          try {
            const catalog = await requestGateway<CommandsCatalogLike>(
              "commands.catalog",
              { session_id: sessionId },
            );
        ...
        pet: async (ctx) => {
          ...
          if (lower === "scale") {
            ...
            if (!rawValue || Number.isNaN(value)) {
              const resolved = await withSlashOutput(ctx);
              resolved?.render(
                "usage: /pet scale <factor>  (e.g. /pet scale 0.5)",
              );
        ...
        browser: async (ctx) => {
          ...
          const [rawAction = "status", ...rest] = ctx.arg
            .trim()
            .split(/\s+/)
            .filter(Boolean);
          ...
          try {
            const result = await requestGateway<BrowserManageResponse>(
              "browser.manage",
              {
                action: cmdAction,
                session_id: sessionId,
            ...
            if (result?.connected) {
              ...
              renderSlashOutput(
                `Endpoint: ${result.url || "(url unavailable)"}`,
              );
              renderSlashOutput(
      ...
      const openPicker = async (
        ...
        const match =
          ...
          sessions.find((session) =>
            sessionTitle(session).toLowerCase().includes(lower),
          ) ||
          sessions.find((session) =>
            (session.preview ?? "").toLowerCase().includes(lower),
          );
