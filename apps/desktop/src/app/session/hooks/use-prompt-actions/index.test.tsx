...
interface HarnessHandle {
  ...
  editMessage: (
    edited: Parameters<ReturnType<typeof usePromptActions>["editMessage"]>[0],
  ) => Promise<void>;
...
function Harness({
  ...
  const selectedStoredSessionIdRef: MutableRefObject<string | null> =
    selectedStoredSessionIdRefProp ?? {
      current:
  ...
  useEffect(() => {
    onReady({
      ...
      reloadFromMessage: (
        ...args: Parameters<typeof actions.reloadFromMessage>
      ) => act(async () => actions.reloadFromMessage(...args)) as Promise<void>,
      restoreToMessage: (
        ...args: Parameters<typeof actions.restoreToMessage>
...
describe("usePromptActions /title", () => {
  ...
  it("falls through to the slash worker for a bare /title (show current title)", async () => {
    const refreshSessions = vi.fn(async () => undefined);
    const requestGateway = vi.fn(
      async () => ({ output: "Title: Old title" }) as never,
    );
    ...
    expect(requestGateway).toHaveBeenCalledWith(
      "slash.exec",
      expect.objectContaining({ command: "title" }),
    );
  });
    ...
    expect(calls).toEqual([
      {
        ...
        params: { session_id: "rt-target-session" },
      },
    ...
    const requestGateway = vi.fn(
      async (method: string, params?: Record<string, unknown>) => {
        ...
        if (method === "slash.exec") {
          return {
            output: "⊙ Goal (active, 1/20 turns): build a rocket",
          } as never;
        }
...
describe("usePromptActions /wake", () => {
  ...
  it("starts the GUI-owned listener through wake.start and never spawns the slash worker", async () => {
    ...
    expect(renderedSeedTexts(seeds).join("\n")).toContain(
      "Input: Microphone Array (Windows WASAPI)",
    );
  });
...
describe("usePromptActions /compress", () => {
  ...
  it("uses the compute-host response transcript and success output", async () => {
    ...
    expect(computeHostTexts).toEqual(
      expect.arrayContaining(["compute-host summary", "compute-host answer"]),
    );
    expect(
      computeHostTexts.some((text) =>
        text.includes("Compressed 4 → 2 messages"),
      ),
    ).toBe(true);
    expect($notifications.get()).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ message: "Compressed 4 → 2 messages" }),
      ]),
    );
  });
    ...
    expect($notifications.get()).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "error",
          message: expect.stringContaining("Compression aborted"),
        }),
      ]),
    );
  ...
  it("surfaces the RPC error verbatim (e.g. the busy guard) instead of a routing error", async () => {
    ...
    expect(
      texts.some((text) => text.includes("not a quick/plugin/skill command")),
    ).toBe(false);
  });
  ...
  it("falls back to the slash worker when an older gateway lacks session.compress", async () => {
    ...
    expect(requestGateway).toHaveBeenCalledWith(
      "slash.exec",
      expect.objectContaining({ command: "compress" }),
    );
    expect(
      renderedSeedTexts(seeds).some((text) =>
        text.includes("compressed by legacy gateway"),
      ),
    ).toBe(true);
  });
  ...
  it("does not clobber the foreground transcript when compression resolves after a session switch", async () => {
    ...
    const storedSessionIdRef: MutableRefObject<string | null> = {
      current: "stored-a",
    };
    const updates: Array<{
      sessionId: string;
    ...
    setMessages([
      {
        id: "foreground-b",
        parts: [textPart("session B transcript")],
        role: "user",
        timestamp: 0,
      },
    ]);
    ...
    await actRender(
      <Harness
        ...
        onUpdateState={(sessionId, storedSessionId) =>
          updates.push({ sessionId, storedSessionId })
        }
        refreshSessions={async () => undefined}
        requestGateway={requestGateway}
    ...
    resolveCompress({
      ...
      messages: [
        { content: "compressed session A transcript", role: "system" },
      ],
      removed: 5,
    });
    await act(async () => {
    ...
    expect($currentUsage.get()).toEqual(
      expect.objectContaining({ calls: 7, input: 70, output: 30, total: 100 }),
    );
  ...
  it("shows a compression progress toast outside the transcript", async () => {
    ...
    await waitFor(() =>
      expect(
        $notifications
          .get()
          .some((item) => item.message === "compressing context..."),
      ).toBe(true),
    );
    resolveCompress({
...
describe("usePromptActions /btw", () => {
  ...
  it("routes through prompt.btw (not slash.exec) and renders the start notice", async () => {
    ...
    expect(
      renderedSeedTexts(seeds).some((text) => text.includes("btw_ab12cd")),
    ).toBe(true);
  });
  ...
  it("shows usage when no question is typed", async () => {
    ...
    expect(
      renderedSeedTexts(seeds).some((text) => text.includes("Usage: /btw")),
    ).toBe(true);
  });
  ...
  it("falls back to the slash worker when an older gateway lacks prompt.btw", async () => {
    ...
    expect(requestGateway).toHaveBeenCalledWith(
      "slash.exec",
      expect.objectContaining({ command: "btw anything" }),
    );
    expect(
      renderedSeedTexts(seeds).some((text) => text.includes("legacy gateway")),
    ).toBe(true);
  });
});
...
describe("usePromptActions exec fallback error reporting", () => {
  ...
  it('surfaces the slash.exec failure when command.dispatch only adds "not a quick/plugin/skill command"', async () => {
    ...
    const texts = renderedSeedTexts(seeds);
    expect(texts.some((text) => text.includes("slash worker timed out"))).toBe(
      true,
    );
    expect(
      texts.some((text) => text.includes("not a quick/plugin/skill command")),
  ...
  it("falls back to slash.exec when an older gateway lacks a dedicated RPC", async () => {
    ...
    expect(
      renderedSeedTexts(seeds).some((text) =>
        text.includes("session status from slash worker"),
      ),
    ).toBe(true);
  });
  ...
  it("still reports a real command.dispatch failure for skill/quick commands", async () => {
    ...
    expect(
      texts.some((text) =>
        text.includes("quick command failed with exit code 1"),
      ),
    ).toBe(true);
  });
});
...
describe("usePromptActions slash.exec dispatch payloads", () => {
    ...
    const requestGateway = vi.fn(
      async (method: string, params?: Record<string, unknown>) => {
        if (method === "slash.exec") {
          ...
          return {
            output: "Approval mode: off (persistent profile setting).",
          } as never;
        }
  ...
  it("updates the goal card live when /goal resume returns a typed exec dispatch", async () => {
    ...
    const requestGateway = vi.fn(async (method: string) => {
      if (method === "slash.exec") {
        return {
          type: "exec",
          output: "▶ Goal resumed: ship the release notes",
        } as never;
      }
  ...
  it("queues the /goal kickoff instead of dropping it when the session is busy (#63352)", async () => {
    ...
    expect(renderedText).toContain(
      "⊙ Goal set (20-turn budget): ship the release notes",
    );
  ...
  it("gates the busy queue on the TARGET session, not the foreground busy flag", async () => {
    ...
    const requestGateway = vi.fn(async (method: string) => {
      ...
      return (
        method === "slash.exec"
          ? { type: "send", message: "audit the session states" }
          : {}
      ) as never;
    });
  ...
  it("still queues when the TARGET session is busy and the foreground flag is not", async () => {
    ...
    expect(
      getQueuedPrompts(RUNTIME_SESSION_ID).map((entry) => entry.text),
    ).toEqual(["audit the session states"]);
  ...
  it("binds slash output and the busy queue to the TARGET session, not the foreground selection", async () => {
    ...
    const requestGateway = vi.fn(
      async (method: string) =>
        (method === "slash.exec"
          ? { type: "send", message: "run it in the tab" }
          : {}) as never,
    );
    ...
    await actRender(
      <Harness
        onReady={(h) => (handle = h)}
        onUpdateState={(_sessionId, storedSessionId) =>
          boundStoredIds.push(storedSessionId)
        }
        refreshSessions={async () => undefined}
        requestGateway={requestGateway}
    ...
    await handle!.submitText("/audit-only run it in the tab", {
      sessionId: tileRuntimeId,
    });
    ...
    expect(getQueuedPrompts(tileStoredId).map((entry) => entry.text)).toEqual([
      "run it in the tab",
    ]);
        ...
        return (
          method === "slash.exec"
            ? {
                type: "skill",
                name: "work",
                message: "Load the work skill, then: fix the tab bug",
              }
            : {}
        ) as never;
      },
  ...
  it("slash status header carries the command token, not the full invocation", async () => {
    ...
    await handle!.submitText(
      "/goal build the whole thing end to end with tests",
    );
...
describe("usePromptActions desktop slash pickers", () => {
  beforeEach(() => {
    setSessions(() => [
      sessionInfo({ id: "20260610_120000_abcdef", title: "Loaded session" }),
    ]);
  });
...
describe("usePromptActions submit / queue drain semantics", () => {
  ...
  it("pins prompt.submit to the active registry connection when the remote session row is untagged", async () => {
    ...
    await actRender(
      <Harness
        ...
        runtimeIdByStoredSessionIdRef={{
          current: new Map([["stored-remote", "runtime-remote"]]),
        }}
  ...
  it("flags prompt.submit with interrupted:true after a voice-playback barge", async () => {
    const { markVoicePlaybackInterrupted } =
      await import("@/lib/voice-playback");
    const requestGateway = vi.fn(async () => ({}) as never);
  ...
  it("a fromQueue drain sends to its queued session even after the active session changes", async () => {
    ...
    render(
      <Harness
        getRuntimeIdForStoredSession={(storedId) =>
          storedId === "stored-session-a" ? "rt-session-a" : null
        }
        onReady={(h) => (handle = h)}
        onUpdateState={(sessionId, storedSessionId, state) =>
          updates.push({ sessionId, state, storedSessionId })
        }
        refreshSessions={async () => undefined}
        requestGateway={requestGateway}
      />,
    ...
    expect(
      updates.some(
        (update) =>
          update.sessionId === "rt-session-a" &&
          update.storedSessionId === "stored-session-a",
      ),
    ).toBe(true);
    ...
    expect(
      requestGateway.mock.calls.every(
        ([method, params]) =>
          ...
          (params as { session_id?: string }).session_id !== "rt-session-a",
      ),
    ).toBe(true);
    expect(
      updates.some(
        (update) =>
          update.sessionId === "rt-session-b" &&
  ...
  it("a fromQueue drain rebinds to the centrally recorded runtime when its explicit id is stale", async () => {
    ...
    render(
      <Harness
        getRuntimeIdForStoredSession={(storedId) =>
          storedId === "stored-session-b" ? "rt-session-b-live" : null
        }
        onReady={(h) => (handle = h)}
        refreshSessions={async () => undefined}
        requestGateway={requestGateway}
    ...
    const accepted = await handle!.submitText(
      "queued for B, B already re-bound",
      {
        fromQueue: true,
        sessionId: "rt-session-a",
        storedSessionId: "stored-session-b",
  ...
  it("a fromQueue drain with null runtime id does NOT land in the foreground session (cross-session leak guard)", async () => {
    ...
    expect(
      requestGateway.mock.calls.every(
        ([method, params]) =>
          method !== "prompt.submit" || params?.session_id !== "rt-foreground",
      ),
    ).toBe(true);
  });
    ...
    expect(
      seeds.some(
        (s) =>
          ...
          (s.messages as { error?: string }[]).some((m) => m.error),
      ),
    ).toBe(false);
  });
...
describe("usePromptActions redirectPrompt", () => {
  ...
  it("redirects the live turn with trimmed correction text", async () => {
    const requestGateway = vi.fn(
      async () => ({ status: "redirected" }) as never,
    );
  ...
  it("records the correction AFTER the assistant output that predates it (#73793, #83151)", async () => {
      ...
      <Harness
        ...
        seedMessages={[
          ...
          },
          {
            ...
            role: "assistant",
            parts: [
              { type: "text", text: "two screens of already-read output" },
            ],
            pending: true,
          },
        ]}
    ...
    const messages = capturedStates.at(-1)?.messages as {
      id: string;
      interim?: boolean;
      pending?: boolean;
    }[];
      ...
      <Harness
        ...
        seedMessages={[
          ...
          },
          {
            id: "assistant-1",
            role: "assistant",
            parts: [{ type: "text", text: "old committed reply" }],
          },
          {
            id: "user-2",
            role: "user",
          ...
          },
          {
            id: "assistant-2",
            role: "assistant",
            parts: [{ type: "text", text: "newer committed reply" }],
          },
        ]}
...
describe("usePromptActions restoreToMessage", () => {
  ...
  it("rewinds to the target user turn and resubmits its text", async () => {
    ...
    expect((lastState.messages as { id: string }[]).map((m) => m.id)).toEqual([
      "u1",
    ]);
  ...
  it("rethrows gateway failures and clears the busy flags for the dialog to surface", async () => {
    ...
    await expect(handle!.restoreToMessage("u2")).rejects.toThrow(
      "gateway exploded",
    );
...
describe("usePromptActions file attachment sync", () => {
  ...
  it("uploads file bytes via file.attach on a remote gateway and submits the rewritten ref", async () => {
    ...
    Object.defineProperty(window, "hermesDesktop", {
      configurable: true,
      value: {
        readFileDataUrl: vi.fn(async () => "data:text/plain;base64,aGVsbG8="),
      },
    });
    ...
    const ok = await handle!.submitText("convert this to epub", {
  ...
  it("uploads Windows file bytes when local mode fronts a POSIX WSL/Docker backend", async () => {
    ...
    const readFileDataUrl = vi.fn(
      async () => "data:text/plain;base64,aGVsbG8=",
    );
    Object.defineProperty(window, "hermesDesktop", {
      configurable: true,
    ...
    expect(
      await handle!.submitText("summarize", { attachments: [attachment] }),
    ).toBe(true);
    expect(readFileDataUrl).toHaveBeenCalledWith(
      "C:\\Users\\alice\\Downloads\\report.txt",
    );
  ...
  it("uses image.attach_bytes for a Windows image when the local backend cwd is POSIX", async () => {
    const readFileDataUrl = vi.fn(
      async () => "data:image/jpeg;base64,aGVsbG8=",
    );
    Object.defineProperty(window, "hermesDesktop", {
      configurable: true,
      value: { readFileDataUrl },
  ...
  it("merges image staging into the current occurrence without dropping its thumbnail", async () => {
    ...
    Object.defineProperty(window, "hermesDesktop", {
      configurable: true,
      value: {
        readFileDataUrl: vi.fn(async () => "data:image/jpeg;base64,aGVsbG8="),
      },
    });
  ...
  it("preserves a same-path replacement added while a successful main submit is in flight", async () => {
    ...
    const attachResult = new Promise<{ attached: boolean; path: string }>(
      (resolve) => {
  ...
  it("uploads file bytes when the terminal backend is a container (docker)", async () => {
    ...
    expect(
      await handle!.submitText("summarize", {
        attachments: [fileAttachment()],
      }),
    ).toBe(true);
    expect(readFileDataUrl).toHaveBeenCalledWith(
      "/Users/alice/Downloads/report.txt",
    );
    expect(calls[0]).toEqual({
      method: "file.attach",
      params: {
  ...
  it("passes a path-less @file: ref straight through (no path = nothing to upload)", async () => {
    ...
    $connection.set({ mode: "remote" } as never);
    const readFileDataUrl = vi.fn(
      async () => "data:application/pdf;base64,JVBERi0=",
    );
    Object.defineProperty(window, "hermesDesktop", {
      configurable: true,
      value: { readFileDataUrl },
    ...
    const ok = await handle!.submitText("read this file", {
      attachments: [pathlessRef],
    });
    ...
    expect(calls[0]?.params?.text).toContain(
  ...
  it("passes a Windows path directly for a native Windows local backend", async () => {
    ...
    const readFileDataUrl = vi.fn(
      async () => "data:text/plain;base64,c2hvdWxkLW5vdC1iZS1yZWFk",
    );
    Object.defineProperty(window, "hermesDesktop", {
      configurable: true,
      value: { readFileDataUrl },
  ...
  it("joins an in-flight eager upload at submit instead of staging the file twice", async () => {
    ...
    const requestGateway = vi.fn(async (method: string) => {
      ...
      if (method === "file.attach") {
        ...
        return {
          attached: true,
          ref_text: "@file:.hermes/desktop-attachments/doc.pdf",
          uploaded: true,
        } as never;
      }
...
describe("usePromptActions sleep/wake session recovery", () => {
        ...
        if (method === "prompt.submit") {
          ...
          if (!bindingPublished) {
            throw new JsonRpcGatewayError(
              "session not found on ambient gateway",
              { code: 4001 },
            );
          }
    ...
    await actRender(
      <Harness
        ...
        onUpdateState={(runtimeId, storedId) => {
          if (
            runtimeId === RECOVERED_SESSION_ID &&
            storedId === STORED_SESSION_ID
          ) {
  ...
  it('resumes the stored session and retries once when reloadFromMessage (regenerate) reports "session not found"', async () => {
    ...
    setMessages([
      {
        id: "u1",
        parts: [textPart("original prompt")],
        role: "user",
        timestamp: 0,
      },
  ...
  it("carries the owning profile from the cache into the recovery resume", async () => {
    setSessions(() => [
      sessionInfo({ id: STORED_SESSION_ID, profile: "work" }),
    ]);
  ...
  it("background queue resume uses the queued stored id and leaves foreground runtime selected", async () => {
    ...
    render(
      <Harness
        ...
        // tests above).
        getRuntimeIdForStoredSession={(storedId) =>
          storedId === STORED_SESSION_ID ? "rt-background-stale" : null
        }
        onReady={(h) => (handle = h)}
        refreshSessions={async () => undefined}
        requestGateway={requestGateway}
    ...
    const ok = await handle!.submitText(
      "queued background message after wake",
      {
        fromQueue: true,
  ...
  it("resumes the SELECTED stored session instead of minting a new one when activeSessionId is null (#55578 split)", async () => {
    ...
    const calls: { method: string; params?: Record<string, unknown> }[] = [];
    const createBackendSessionForSend = vi.fn(
      async () => "brand-new-session-WRONG",
    );
  ...
  it("never replaces a selected stored session when its direct runtime resume fails", async () => {
    ...
    expect(
      await handle!.submitText("keep me in the selected conversation"),
    ).toBe(false);
  ...
  it("resumes the ROUTED stored session instead of minting a new one when profile switching cleared both session refs", async () => {
    ...
    expect(
      await handle!.submitText(
        "follow-up while the profile route is rebinding",
      ),
    ).toBe(true);
  ...
  it("lets the durable route replace a stale selected session and runtime before submit", async () => {
    ...
    await actRender(
      <Harness
        ...
        resumeStoredSession={resumeStoredSession}
        runtimeIdByStoredSessionIdRef={{
          current: new Map([[STORED_SESSION_ID, RECOVERED_SESSION_ID]]),
        }}
        selectedStoredSessionIdRef={selectedStoredSessionIdRef}
        storedSessionId={STORED_SESSION_ID}
      />,
    ...
    expect(await handle!.submitText("stay in the routed profile session")).toBe(
      true,
    );
    expect(resumeStoredSession).toHaveBeenCalledWith(STORED_SESSION_ID);
    expect(requestGateway).toHaveBeenCalledWith(
  ...
  it("submits directly when the routed stored session already owns the live runtime", async () => {
    const activeSessionIdRef: MutableRefObject<string | null> = {
      current: RECOVERED_SESSION_ID,
    };
    const selectedStoredSessionIdRef: MutableRefObject<string | null> = {
      current: STORED_SESSION_ID,
    };
    const requestGateway = vi.fn(async () => ({}) as never);
...
describe("usePromptActions submit session-context isolation (#54527)", () => {
  ...
  it("aborts submit when the user switches sessions during session.resume (no misroute)", async () => {
    ...
    const submitting = handle!.submitText(
      "carefully composed prompt for project A",
    );
    await waitFor(() =>
  ...
  it("aborts submit when the composer scope disagrees with the resolved target (#59305)", async () => {
    ...
    const ok = await handle!.submitText("typed while B was on screen", {
      composerScope: STORED_SESSION_B,
    });
  ...
  it("submits normally when the composer scope agrees with the resolved target", async () => {
    ...
    const ok = await handle!.submitText("typed while A was on screen", {
      composerScope: STORED_SESSION_A,
    });
  ...
  it("aborts recovery submit when the user switches sessions during timeout resume", async () => {
    ...
    const submitting = handle!.submitText(
      "message that must not land in session B",
    );
    await waitFor(() =>
      expect(calls.some((c) => c.method === "session.resume")).toBe(true),
    );
  ...
  it("submits the first prompt of a new chat — the create pipeline re-homing selection/route is not user drift", async () => {
    ...
    expect(await handle!.submitText("first message of a brand-new chat")).toBe(
      true,
    );
    expect(createBackendSessionForSend).toHaveBeenCalledTimes(1);
    expect(
      calls.find((c) => c.method === "prompt.submit")?.params,
    ).toMatchObject({
  ...
  it("aborts when the user switches sessions during the tail of a successful create", async () => {
    ...
    expect(
      await handle!.submitText("message that must not land in session B"),
    ).toBe(false);
    expect(calls.some((c) => c.method === "prompt.submit")).toBe(false);
  });
});
...
describe("usePromptActions new-chat first-send delivery (#63078)", () => {
  ...
  it("aborts a new-session submit when sidebar navigation changes the route before its selected ref (#62562)", async () => {
    ...
    const submitting = handle!.submitText("first message", {
      attachments: [attachment],
    });
    await waitFor(() =>
  ...
  it("still aborts when the user genuinely switches chats after create, during attachment sync (#62805)", async () => {
    ...
    await waitFor(() =>
      expect(calls.some((c) => c.method === "file.attach")).toBe(true),
    );
...
describe("usePromptActions busy-gateway churn tolerance (#64327)", () => {
  ...
  it("does not abort a send when programmatic gateway churn fires mid-submit (selection null-reset + search/hash route churn)", async () => {
    ...
    const submitting = handle!.submitText(
      "send from a second chat on a busy gateway",
    );
    await waitFor(() =>
      expect(calls.some((c) => c.method === "session.resume")).toBe(true),
    );
  ...
  it("submits once through authoritative recovery when routed resume publication lags (#90428)", async () => {
    ...
    const calls: { method: string; params?: Record<string, unknown> }[] = [];
    const selectedStoredSessionIdRef: MutableRefObject<string | null> = {
      current: staleStoredId,
    };
    const activeSessionIdRef: MutableRefObject<string | null> = {
      current: staleRuntimeId,
    };
    ...
    render(
      <Harness
        ...
        getRuntimeIdForStoredSession={(storedId) =>
          runtimeIdByStoredSessionIdRef.current.get(storedId) ?? null
        }
        onReady={(h) => (handle = h)}
        refreshSessions={async () => undefined}
        requestGateway={requestGateway}
    ...
    expect(
      await handle!.submitText("deliver despite lagging local publication"),
    ).toBe(true);
  ...
  it("keeps an explicit background queue target isolated from foreground routed recovery (#90428)", async () => {
    ...
    const resumeStoredSession = vi.fn(async () => {
      ...
      runtimeIdByStoredSessionIdRef.current.set(
    ...
    render(
      <Harness
        ...
        onReady={(h) => (handle = h)}
        onUpdateState={(sessionId, storedSessionId) =>
          stateWrites.push({ sessionId, storedSessionId })
        }
        refreshSessions={async () => undefined}
        requestGateway={requestGateway}
        resumeStoredSession={resumeStoredSession}
    ...
    expect(
      stateWrites.some((write) => write.sessionId === FOREGROUND_RUNTIME_ID),
    ).toBe(false);
...
describe("usePromptActions submit entry-time runtime ownership proof (#64789/#65328)", () => {
  ...
  it("does not submit to runtime A when the cache proves B is bound to a different runtime (forward mismatch)", async () => {
    ...
    const selectedStoredSessionIdRef: MutableRefObject<string | null> = {
      current: STORED_SESSION_B,
    };
    const activeSessionIdRef: MutableRefObject<string | null> = {
      current: RUNTIME_SESSION_A,
    };
    ...
    await handle!.submitText(
      "ordinary text for the selected project B session",
    );
    ...
    expect(
      calls.find(
        (c) =>
          ...
          c.params?.session_id === RUNTIME_SESSION_A,
      ),
    ).toBeUndefined();
    expect(
      calls.find(
        (c) =>
          c.method === "prompt.submit" &&
          c.params?.session_id === RUNTIME_SESSION_B_RESUMED,
      ),
    ).toBeDefined();
  });
  ...
  it("does not submit to runtime A when the cache proves A belongs to a DIFFERENT stored session (reverse proof, no forward entry for B)", async () => {
    ...
    await handle!.submitText(
      "ordinary text for the freshly created project B session",
    );
...
describe("usePromptActions eager attachment upload (drop-time)", () => {
  ...
  it("uploads a dropped file the moment it lands (active session) and rewrites the chip with the gateway ref", async () => {
    ...
    await waitFor(() =>
      expect($composerAttachments.get()[0]?.attachedSessionId).toBe(
        RUNTIME_SESSION_ID,
      ),
    );

    const chip = $composerAttachments.get()[0]!;
    expect(chip.refText).toBe(
      "@file:.hermes/desktop-attachments/DEVIS_signed.pdf",
    );
    expect(chip.uploadState).toBeUndefined();
    expect(readFileDataUrl).toHaveBeenCalledWith(
      "/Users/mahmoud/Downloads/DEVIS_signed.pdf",
    );
  });
  ...
  it("flags the chip uploadState=error when the eager upload fails, keeping the path so submit can retry", async () => {
    ...
    $composerAttachments.set([
      { id: "file:x", kind: "file", label: "x.pdf", path: "/abs/x.pdf" },
    ]);
    ...
    await waitFor(() =>
      expect($composerAttachments.get()[0]?.uploadState).toBe("error"),
    );
...
describe("uploadComposerAttachment remote read failures", () => {
  ...
  it("turns the raw 16MB IPC cap error into a friendly remote-gateway message", async () => {
    ...
    Object.defineProperty(window, "hermesDesktop", {
      ...
      value: {
        readFileDataUrl: vi.fn(async () => {
          throw new Error(
            "File preview failed: file is too large (20971520 bytes; limit 16777216 bytes).",
          );
        }),
      },
    });
    ...
    await expect(
      ...
      "huge.csv is too large to upload to the remote gateway (max 16 MB).",
    );
...
describe("uploadComposerAttachment preview reuse", () => {
  ...
  it("reuses the chip previewUrl instead of re-reading the image off disk", async () => {
    ...
    const readFileDataUrl = vi.fn(
      async () => "data:image/png;base64,ZnJvbS1kaXNr",
    );
    Object.defineProperty(window, "hermesDesktop", {
      configurable: true,
      value: { readFileDataUrl },
...
describe("usePromptActions stale-closure session routing", () => {
  ...
  async function renderWithStaleClosure(
    requestGateway: GatewayMock,
    seedMessages?: unknown[],
  ) {
    const activeSessionIdRef: MutableRefObject<string | null> = {
      current: RUNTIME_SESSION_ID,
  ...
  it("restores a checkpoint in the CURRENT session, not the stale closure session", async () => {
    ...
    const { handle, updated } = await renderWithStaleClosure(requestGateway, [
      {
        id: "u1",
        parts: [textPart("first prompt")],
        role: "user",
        timestamp: 0,
      },
      {
        id: "a1",
        parts: [textPart("first reply")],
        role: "assistant",
        timestamp: 1,
      },
      {
        id: "u2",
        parts: [textPart("second prompt")],
        role: "user",
        timestamp: 2,
      },
    ]);
...
describe("usePromptActions editMessage stale-target recovery (#82462)", () => {
  ...
  it("surfaces a compressed-away notice instead of plain-resubmitting without an ordinal", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        titles.some((t) =>
          /no longer in server history|compressed/i.test(t || ""),
        ),
      ).toBe(true);
    });

    const submitCalls = (
      requestGateway as unknown as { mock: { calls: unknown[][] } }
    ...
    expect(
      (
        submitCalls[0]?.[1] as
          { truncate_before_user_ordinal?: unknown } | undefined
...
describe("usePromptActions reloadFromMessage failed-submit rollback (#95745)", () => {
  ...
  it("restores the full transcript when regenerate is rejected", async () => {
    ...
    const seed = [
      ...
      {
        id: "u2",
        parts: [textPart("later")],
        role: "user" as const,
        timestamp: 2,
      },
      {
        id: "a2",
        parts: [textPart("later reply")],
        role: "assistant" as const,
        timestamp: 3,
      },
    ];
    ...
    const rolledBack = latest?.messages as
      Array<{ hidden?: boolean; id: string }> | undefined;
