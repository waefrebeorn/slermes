...
describe("singleFlightSessionResume", () => {
  ...
  it("a rejected flight is not cached: the next caller retries", async () => {
    ...
    await expect(singleFlightSessionResume("stored-a", run)).rejects.toThrow(
      "boom",
    );
    await expect(singleFlightSessionResume("stored-a", run)).resolves.toEqual({
      session_id: "rt-second",
    });
...
describe("drift-abort recovered-runtime cache", () => {
  ...
  it("a later non-drifted recovery adopts the cached runtime instead of resuming again", async () => {
    ...
    const requestGateway = vi.fn(async () => {
      throw new Error(
        "session.resume must not be called when a cached runtime exists",
      );
    });
