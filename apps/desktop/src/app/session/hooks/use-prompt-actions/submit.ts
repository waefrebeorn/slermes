...
const MAIN_SUBMIT_SCOPE: NonNullable<SubmitPromptDeps["scope"]> = {
  removeAttachments: (attachments) =>
    mainComposerScope.removeOccurrences(attachments),
...
export function useSubmitPrompt(deps: SubmitPromptDeps) {
  ...
  return useCallback(
    async (rawText: string, options?: SubmitTextOptions) => {
      ...
      const attachments = (
        options?.attachments ?? scope.readAttachments()
      ).filter((a): a is ComposerAttachment => Boolean(a));

      const terminalContextBlocks =
        terminalContextBlocksFromDraft(rawText).join("\n\n");
      ...
      const buildContextText = (atts: ComposerAttachment[]): string => {
        ...
        return (
          [contextRefs, terminalContextBlocks, visibleText]
            .filter(Boolean)
            .join("\n\n") ||
      ...
      const hasSendable = Boolean(
        visibleText || terminalContextBlocks || attachments.length || hasImage,
      );
      ...
      let sessionId: null | string =
        options?.sessionId ??
        (isBackgroundQueueDrain ? null : activeSessionIdRef.current);
      ...
      if (
        ...
        const boundRuntimeId = getRuntimeIdForStoredSession(
          options.storedSessionId,
        );
      ...
      const targetIsCurrentView = (): boolean =>
        targetStartedInCurrentView && !sessionDriftReason();
      ...
      const seedOptimistic = (sid: string) => {
        ...
        const activity = bubbleText.trim()
          ? { preview: bubbleText.trim() }
          : undefined;
        ...
        updateSessionState(
          ...
          (state) => ({
            ...
            messages: state.messages.map((message) =>
              message.id === optimisticId ? buildUserMessage() : message,
            ),
          }),
          targetStoredSessionId,
        );
      ...
      const dropOptimistic = (sid: null | string) => {
        ...
        updateSessionState(
          ...
          (state) => ({
            ...
            turnStartedAt:
              state.streamId || state.sawAssistantPayload
                ? state.turnStartedAt
                : null,
          }),
          targetStoredSessionId,
      ...
      const abortForSessionSwitch = (
        optimisticSessionId: null | string,
      ): false => {
      ...
      const ownershipStoredSessionId = options?.sessionId
        ? null
        : targetStoredSessionId;

      if (sessionId && ownershipStoredSessionId) {
        const provenRuntimeId = runtimeIdByStoredSessionIdRef.current.get(
          ownershipStoredSessionId,
        );
        ...
        const runtimeOwnedByOtherStored = Array.from(
          runtimeIdByStoredSessionIdRef.current.entries(),
        ).some(
          ([storedId, runtimeId]) =>
            runtimeId === sessionId && storedId !== ownershipStoredSessionId,
        );
      ...
      if (
        ...
        if (routedResumeDrift) {
          console.warn("[submit-drift-abort]", routedResumeDrift, {
            phase: "post-routed-resume",
          });
          ...
          const publishedRuntimeId = getRuntimeIdForStoredSession(
            routedStoredSessionId,
          );
        ...
        const validatedRuntimeId = getRuntimeIdForStoredSession(
          routedStoredSessionId,
        );
      ...
      if (!sessionId && targetStoredSessionId) {
        ...
        try {
          ...
          const resumed = cachedRuntimeId
            ...
            : await singleFlightSessionResume(
                targetStoredSessionId,
                async () => {
                  const resumeProfile = await resolveSessionProfile(
                    targetStoredSessionId,
                  );
          ...
          if (resumeDrift) {
            console.warn("[submit-drift-abort]", resumeDrift, {
              phase: "post-resume",
            });
        ...
        if (resumeSettleDrift) {
          console.warn("[submit-drift-abort]", resumeSettleDrift, {
            phase: "post-resume-settle",
          });
      ...
      if (!sessionId) {
        ...
        if (!sessionId) {
          ...
          if (createNullDrift) {
            console.warn("[submit-drift-abort]", createNullDrift, {
              phase: "post-create-null",
            });
      ...
      try {
        ...
        const attachResult = await syncAttachmentsForSubmit(
          sessionId,
          attachments,
          {
        ...
        if (attachmentsDrift) {
          console.warn("[submit-drift-abort]", attachmentsDrift, {
        ...
        attachmentRefs = syncedAttachments
          .map(optimisticAttachmentRef)
          .filter((r): r is string => Boolean(r));
        ...
        try {
          ...
          if (firstErr instanceof SessionRecoveryAborted) {
            console.warn("[submit-drift-abort]", firstErr.reason, {
