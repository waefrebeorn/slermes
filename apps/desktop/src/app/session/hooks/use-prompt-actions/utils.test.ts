...
describe("recent interrupt cooldown", () => {
  it("is true within the cooldown and false after expiry", () => {
    ...
    expect(
      isSessionRecentlyInterrupted(
        sessionId,
        t0 + RECENT_INTERRUPT_COOLDOWN_MS - 1,
      ),
    ).toBe(true);
    expect(
      isSessionRecentlyInterrupted(
        sessionId,
        t0 + RECENT_INTERRUPT_COOLDOWN_MS,
      ),
    ).toBe(false);
  });
  ...
  it("shouldInterruptBeforeRewind is true when recently interrupted even if not busy", () => {
    ...
    expect(
      shouldInterruptBeforeRewind({ busy: false, sessionId, now: t0 + 500 }),
    ).toBe(true);
    expect(
      shouldInterruptBeforeRewind({
        busy: false,
        sessionId,
        now: t0 + RECENT_INTERRUPT_COOLDOWN_MS + 1,
      }),
    ).toBe(false);
  });
  ...
  it("shouldInterruptBeforeRewind stays false for idle sessions with no recent interrupt", () => {
    expect(
      shouldInterruptBeforeRewind({ busy: false, sessionId: "idle-sess" }),
    ).toBe(false);
    expect(
      shouldInterruptBeforeRewind({ busy: true, sessionId: "busy-sess" }),
    ).toBe(true);
  });
});
...
describe("isTargetSessionBusy", () => {
  it("reads the target session slice, not the leftover foreground flag", () => {
    expect(
      isTargetSessionBusy({ a: { busy: true }, b: { busy: false } }, "b", true),
    ).toBe(false);
...
describe("inlineErrorMessage", () => {
  it("unwraps an electron remote-method error", () => {
    expect(
      inlineErrorMessage(
        new Error("Error invoking remote method 'x': Error: boom"),
        "fallback",
      ),
    ).toBe("boom");
  });
  ...
  it("strips a leading Error: prefix", () => {
    expect(inlineErrorMessage(new Error("Error: nope"), "fallback")).toBe(
      "nope",
    );
  });
...
describe("withSessionNotFoundResume", () => {
  ...
  it.each([
    ...
    "resumes and retries once so %s can %s after a stale drop",
    async (rpc) => {
      const d = deps();
      let attempts = 0;
  ...
  it("aborts instead of retrying when the user moved on during the resume", async () => {
    ...
    await expect(
      withSessionNotFoundResume(DEAD, STORED, call, d),
  ...
  it("rethrows the original error when the resume itself 404s (never-persisted draft)", async () => {
    ...
    await expect(
      withSessionNotFoundResume(DEAD, STORED, call, d),
    ).rejects.toThrow("original symptom");
  });
  ...
  it("rethrows when no stored session id is available to resume", async () => {
    ...
    await expect(
      withSessionNotFoundResume(DEAD, null, call, d),
  ...
  it("does not recover a gateway timeout unless the caller opts in", async () => {
    ...
    await expect(
      withSessionNotFoundResume(DEAD, STORED, timeout, d),
...
describe("friendlyRemoteAttachError", () => {
  it("rewrites a too-large error with the parsed cap", () => {
    const err = friendlyRemoteAttachError(
      new Error("file is too large (20 bytes; limit 16777216 bytes)"),
      "pic.png",
    );
    expect(err.message).toBe(
      "pic.png is too large to upload to the remote gateway (max 16 MB).",
    );
  });
...
describe("readFileDataUrlForAttach", () => {
  it("prefers the attachment-specific desktop reader over the preview reader", async () => {
    ...
    const attachmentReader = vi.fn(
      async () => "data:application/zip;base64,UEs=",
    );
    Object.defineProperty(window, "hermesDesktop", {
      configurable: true,
      value: {
        readFileDataUrl: previewReader,
        readFileDataUrlForAttach: attachmentReader,
      },
    });

    await expect(readFileDataUrlForAttach("/tmp/archive.zip")).resolves.toBe(
  ...
  it("falls back to the preview reader on older shells", async () => {
    ...
    await expect(readFileDataUrlForAttach("/tmp/note.txt")).resolves.toBe(
...
describe("renderRpcResult", () => {
        ...
        renderRpcResult(
          {
            summary: {
              ...
              note: "No new turns since last compress",
              noop: true,
            },
          },
          "compress",
  ...
  describe("session.steer", () => {
    it("reports a queued steer with the original text", () => {
      expect(
        renderRpcResult({ status: "queued", text: "skip the docs" }, "steer"),
      ).toBe('Steered · "skip the docs" queued for next tool call');
    });

    it("reports a rejected steer without echoing user text", () => {
      expect(
        renderRpcResult({ status: "rejected", text: "whatever" }, "steer"),
      ).toBe("Steer rejected — agent declined input");
    });
  });

  describe("process.stop", () => {
    it("reports the numeric number of stopped processes", () => {
      expect(renderRpcResult({ killed: 2 }, "stop")).toBe(
        "Stopped 2 background processes.",
      );
    });

    it("reports nothing-to-stop when the numeric count is zero", () => {
      expect(renderRpcResult({ killed: 0 }, "stop")).toBe(
  ...
  describe("session.save", () => {
    it("echoes the saved file path", () => {
      expect(
        renderRpcResult(
          { file: "/home/user/.hermes/sessions/saved/x.json" },
          "save",
        ),
      ).toBe("Saved transcript to /home/user/.hermes/sessions/saved/x.json");
    });
  });
  ...
  describe("session.usage", () => {
    it("formats calls / input / output / total with thousands separators", () => {
      ...
      ).toBe("Usage: 12 calls · 1,234,567 in / 89,012 out · 1,323,579 total");
    });
  ...
  describe("agents.list", () => {
    it("reports no running tasks when the array is empty", () => {
      expect(renderRpcResult({ processes: [] }, "agents")).toBe(
  ...
  describe("fallback", () => {
    it("serialises unknown shapes as JSON so we never lose data", () => {
      expect(
        renderRpcResult({ custom: "value", nested: { a: 1 } }, "mystery"),
      ).toBe('/mystery: {"custom":"value","nested":{"a":1}}');
    });
