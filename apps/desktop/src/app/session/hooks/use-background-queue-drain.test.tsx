...
describe("useBackgroundQueueDrain", () => {
  ...
  it("drains an idle queued prompt for a non-selected background session", async () => {
    const runtimeMap = {
      current: new Map([["stored-session-a", "rt-session-a"]]),
    };
  ...
  it("treats a tip working id as busy for a root queue key via lineage", async () => {
    ...
    enqueueQueuedPrompt("root-a", {
      text: "wait for tip turn",
      attachments: [],
    });
    publishSessionState("rt-tip-a", {
      ...createClientSessionState("tip-a"),
      busy: true,
    });
