...
vi.mock("@/hermes", async (importOriginal) => ({
  ...
  listAllProfileSessions: (...args: unknown[]) =>
    listAllProfileSessions(...args),
...
describe("refreshSessions identity + loading hygiene", () => {
  it("keeps the previous $sessions array when the refresh is content-identical", async () => {
    ...
    const { result } = renderHook(() =>
      useSessionListActions({ profileScope: "default" }),
    );
  ...
  it("does not flicker the loading flag over a populated list", async () => {
    ...
    const off = $sessionsLoading.subscribe((value) =>
    ...
    listSidebarSessions.mockResolvedValue(
      sidebar({
        sessions: [
          row("a"),
          row("b"),
          row("c", { _lineage_root_id: "root-c" } as Partial<SessionInfo>),
        ],
      }),
    );
    ...
    try {
      ...
      await act(async () => {
        pending.resolve({
          ...
          cron: { sessions: [row("stale-cron", { source: "cron" })] },
          messaging: {
            sessions: [row("stale-message", { source: "telegram" })],
          },
        });
...
describe("refreshSessions batches slices into one request", () => {
  ...
  it("forwards the active profile scope + section limits to the batched call", async () => {
    listSidebarSessions.mockResolvedValue(sidebar({ sessions: [] }));
    const { result } = renderHook(() =>
      useSessionListActions({ profileScope: "work" }),
    );
  ...
  it("keeps the committed profile active when a later render is discarded", async () => {
    ...
    expect(listSidebarSessions).toHaveBeenCalledWith(
      expect.objectContaining({ recentsProfile: "work" }),
    );
  });
  ...
  it("ignores an in-flight response after the source changes with the same profile", async () => {
    ...
    listSidebarSessions
      ...
      .mockReturnValueOnce(personal.promise);
    const { result } = renderHook(() =>
      useSessionListActions({ profileScope: "default" }),
    );
    const workRefresh = result.current.refreshSessions();
    ...
    await act(async () => {
      personal.resolve(
        sidebar(
          { sessions: [row("personal-session")] },
          [],
          [row("personal-chat", { source: "telegram" })],
        ),
      );
      await personalRefresh;
      ...
      work.resolve(
        sidebar(
          { sessions: [row("work-session")] },
          [],
          [row("work-chat", { source: "signal" })],
        ),
      );
      await workRefresh;
    });
  ...
  it("requests cron jobs for the unified scope", async () => {
    const unified = renderHook(() =>
      useSessionListActions({ profileScope: "__all__" }),
    );
  ...
  it("ignores an out-of-order cron-jobs response from the previous profile", async () => {
    ...
    getCronJobs.mockImplementation((profile: string) =>
...
describe("messaging profile scope", () => {
  ...
  it("keeps loaded platform rows when pagination fails", async () => {
    ...
    await act(async () => {
      await expect(
        result.current.loadMoreMessagingForPlatform("signal"),
  ...
  it("ignores an in-flight response after the active profile changes", async () => {
    ...
    listAllProfileSessions.mockImplementation(
      (_limit, _min, _archived, _order, profile) =>
        profile === "work" ? work.promise : personal.promise,
    );
    ...
    await act(async () => {
      personal.resolve({
        sessions: [
          row("personal-message", { profile: "personal", source: "telegram" }),
        ],
        total: 1,
      });
      await personalRefresh;
      work.resolve({
        sessions: [row("work-message", { profile: "work", source: "signal" })],
        total: 1,
      });
      await workRefresh;
    });

    expect(listAllProfileSessions.mock.calls.map((call) => call[4])).toEqual([
      "work",
      "personal",
    ]);
    expect($messagingSessions.get().map((session) => session.id)).toEqual([
