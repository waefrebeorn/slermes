...
describe("useSessionStateCache — stored-id rotation provenance", () => {
  ...
  it("emits the previous, next, and runtime ids and removes the stale reverse mapping", () => {
    ...
    expect(cache.runtimeIdByStoredSessionIdRef.current.has("stored-A")).toBe(
      false,
    );
    expect(
      cache.runtimeIdByStoredSessionIdRef.current.get("stored-A-next"),
    ).toBe("runtime-A");
  });
...
describe("useSessionStateCache — per-session turn timer", () => {
  ...
  it("keeps a background session's running turn clock and never mirrors it to the view", () => {
    ...
    expect(
      cache.sessionStateByRuntimeIdRef.current.get("bg-runtime")?.turnStartedAt,
    ).toBe(startedAt);
  ...
  it("clears the global clock when the focused turn ends", () => {
    ...
    act(() => {
      cache.updateSessionState("fg-runtime", (state) => ({
        ...state,
        busy: false,
        turnStartedAt: null,
      }));
...
function transcriptForCache(id: string): ChatMessage[] {
  return [
    ...
    assistantText(`${id}-assistant`, `reply ${id}`),
  ];
}
...
describe("useSessionStateCache — cross-thread error isolation", () => {
  ...
  it("does not leak a failed turn into another thread on switch", () => {
    ...
    let cache!: Cache;
    const { rerender } = render(
      <ViewHarness activeSessionId="thread-A" onReady={(c) => (cache = c)} />,
    );
    ...
    act(() => {
      cache.updateSessionState(
        ...
        (state) => ({
          ...
          busy: false,
          messages: [
            userMessage("user-a", "do the thing"),
            assistantError("assistant-a-error", "Out of funds"),
          ],
        }),
        "stored-A",
      );
    ...
    expect(
      $messages.get().some((message) => message.error === "Out of funds"),
    ).toBe(true);
    ...
    rerender(
      <ViewHarness activeSessionId="thread-B" onReady={(c) => (cache = c)} />,
    );
    act(() => {
      cache.updateSessionState(
        "thread-B",
        (state) => ({
          ...
          messages: [
            ...
            assistantText("assistant-b", "hi there"),
          ],
        }),
        "stored-B",
      );
    ...
    expect($messages.get().map((message) => message.id)).toEqual([
      "user-b",
      "assistant-b",
    ]);
    expect(
  ...
  it("still preserves a same-session local error a heartbeat dropped", () => {
    ...
    $messages.set([
      userMessage("user-a", "do the thing"),
      assistantError("assistant-a-error", "OpenRouter 403"),
    ]);
    ...
    expect(
      $messages.get().some((message) => message.error === "OpenRouter 403"),
    ).toBe(true);
  });
  ...
  it("evicts the oldest warm transcript with its reverse ownership while retaining lightweight state", () => {
    ...
    act(() => {
      for (let index = 0; index < 25; index += 1) {
        cache.updateSessionState(
          ...
          (state) => ({
            ...state,
            messages: transcriptForCache(`message-${index}`),
          }),
          `stored-${index}`,
        );
      }
    ...
    expect(cache.sessionStateByRuntimeIdRef.current.has("runtime-0")).toBe(
      false,
    );
    expect(cache.runtimeIdByStoredSessionIdRef.current.has("stored-0")).toBe(
      false,
    );
    expect($sessionStates.get()["runtime-0"]).toMatchObject({
      storedSessionId: "stored-0",
      busy: false,
    });
  ...
  describe("reconnect-orphaned transcripts (#95189)", () => {
    ...
    it("releases a busy transcript once reconciliation settles the authoritative record", () => {
      ...
      render(
        <Harness
          activeSessionId={fg}
          onReady={(value) => (cache = value)}
          selectedStoredSessionId={fgStored}
        />,
      );
            ...
            messages: [
              {
                id: `${liveBusy}-u`,
                role: "user",
                parts: [{ type: "text", text: "long turn" }],
              },
            ],
          }),
          `${liveBusy}-stored`,
      ...
      expect(cache.runtimeIdByStoredSessionIdRef.current.has(bgStored)).toBe(
        false,
      );
    ...
    it("keeps a live background turn cached while its authoritative record is busy", () => {
      ...
      act(() => {
        cache.updateSessionState(fg, (state) => ({
          ...state,
          model: "test/model",
        }));
      });
      ...
      expect(cache.sessionStateByRuntimeIdRef.current.has(bg)).toBe(true);
      expect(cache.runtimeIdByStoredSessionIdRef.current.get(bgStored)).toBe(
        bg,
      );
    });
  });
...
describe("useSessionStateCache — reconnect busy reconcile (#93059)", () => {
  ...
  const asDelegate = (partial: Partial<SessionTileDelegate>) =>
    partial as SessionTileDelegate;
  ...
  it("retires the wiring cache entry, not just the store mirror", () => {
    ...
    setSessionTileDelegate(
      asDelegate({
        retireBusyClaim: (runtimeId) => {
          cache.updateSessionState(runtimeId, (state) => ({
            ...state,
    ...
    expect(
      cache.sessionStateByRuntimeIdRef.current.get("runtime-1")?.busy,
    ).toBe(true);
    ...
    expect(
      cache.sessionStateByRuntimeIdRef.current.get("runtime-1")
        ?.awaitingResponse,
    ).toBe(false);
