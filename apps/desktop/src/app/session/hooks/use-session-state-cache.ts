...
export function useSessionStateCache({
  ...
  const selectedStoredSessionIdRef = useRef<string | null>(
    selectedStoredSessionId,
  );
    ...
    sessionStateByRuntimeIdRef.current = new SessionStateCache({
          ...
          .some(
            (tile) =>
              ...
              (state.storedSessionId !== null &&
                tile.storedSessionId === state.storedSessionId),
          ),
      ...
      onEvict: (runtimeId, state) => {
        ...
        if (
          ...
          runtimeIdByStoredSessionIdRef.current.get(state.storedSessionId) ===
            runtimeId
        ) {
    ...
    (sessionId: string, storedSessionId?: string | null) => {
      ...
      if (existing) {
        if (
          ...
          storedSessionId !== existing.storedSessionId
        ) {
          ...
          const updated = invalidatePersistedDisplayTranscriptAuthority({
            ...existing,
            storedSessionId,
          });
  ...
  const syncSessionStateToView = useCallback(
    (sessionId: string, state: ClientSessionState) => {
      ...
      const viewState = suppressTranscriptForView(
        state,
        transcriptViewGateByRuntimeIdRef.current.has(sessionId),
      );
  ...
  return {
    ...
    sessionStateByRuntimeIdRef: sessionStateByRuntimeIdRef as MutableRefObject<
