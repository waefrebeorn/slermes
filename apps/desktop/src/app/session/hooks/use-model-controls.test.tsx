...
vi.mock("@/hermes", () => ({
  ...
  setGlobalModel: (...args: Parameters<typeof setGlobalModel>) =>
    setGlobalModel(...args),
}));
...
vi.mock("@/store/notifications", () => ({
  dismissNotification: (...args: Parameters<typeof dismissNotification>) =>
    dismissNotification(...args),
  notify: (...args: Parameters<typeof notify>) => notify(...args),
  notifyError: (...args: Parameters<typeof notifyError>) =>
    notifyError(...args),
}));
...
describe("useModelControls", () => {
  ...
  it("writes optimistic selections only to the owning connection cache", async () => {
    ...
    await act(() =>
      result.current.selectModel({ model: "a/model", provider: "a" }),
    );

    expect(
      queryClient.getQueryData(modelOptionsQueryKey("beta", null, "source-a")),
    ).toMatchObject({
      model: "a/model",
      provider: "a",
    });
    expect(
      queryClient.getQueryData(modelOptionsQueryKey("beta")),
    ).toBeUndefined();
    expect(
      queryClient.getQueryData(modelOptionsQueryKey("beta", null, "source-b")),
    ).toBeUndefined();
  });
  ...
  it("keeps a live session authoritative when Settings saves a new profile default", async () => {
    ...
    expect(
      queryClient.getQueryData(modelOptionsQueryKey("default", "runtime-1")),
    ).toMatchObject({
      model: "tencent/hy3:free",
      provider: "nous",
    });
    ...
    expect(getCurrentModelSource()).toBe("default");
    expect(
      queryClient.getQueryData(modelOptionsQueryKey("default")),
    ).toMatchObject({
      model: "poolside/laguna-xs-2.1:free",
      provider: "nous",
    });
  ...
  it("preserves a populated model catalog when painting a saved profile default", () => {
    const queryClient = new QueryClient();
    const providers = [
      { models: ["tencent/hy3:free"], name: "Nous", slug: "nous" },
    ];
  ...
  it("sends an active primary-session picker change without a scope flag so the gateway decides persistence", async () => {
    ...
    const requestGateway = vi.fn(
      async () => ({ key: "model", value: "claude-sonnet-4.6" }) as never,
    );
  ...
  it("keeps a mid-turn pick painted and skips the refetch that would repaint the old model", async () => {
    ...
    const requestGateway = vi.fn(
      async () =>
        ({ deferred: true, key: "model", value: "grok-4.5" }) as never,
    );
    ...
    await expect(
      controls.selectModel({ model: "grok-4.5", provider: "xai" }),
    ).resolves.toBe(true);
  ...
  it("still refetches after a switch that applied immediately", async () => {
    ...
    const requestGateway = vi.fn(
      async () =>
        ({ key: "model", scope: "session", value: "grok-4.5" }) as never,
    );
    const invalidate = vi.spyOn(QueryClient.prototype, "invalidateQueries");
    let controls!: Controls;
  ...
  it("still rolls back and reports a real switch failure", async () => {
    ...
    await expect(
      controls.selectModel({ model: "bogus", provider: "xai" }),
    ).resolves.toBe(false);
  ...
  it("session-scopes MoA preset selections so they cannot persist as the global gateway default", async () => {
    $activeSessionId.set("session-1");
    const requestGateway = vi.fn(
      async () => ({ key: "model", value: "BeastMode" }) as never,
    );
    let controls!: Controls;
  ...
  it("updates only the active profile new-chat cache", async () => {
    ...
    expect(
      queryClient.getQueryData(modelOptionsQueryKey("compass")),
    ).toMatchObject({
      model: "qwen3.6:35b-65k",
      provider: "custom:local-ollama",
    });
  ...
  it("reseeds a sticky manual pick that was removed from the catalog", async () => {
    ...
    queryClient.setQueryData(modelOptionsQueryKey("compass"), {
      providers: [
        { models: ["openai/gpt-5.5"], name: "OpenRouter", slug: "openrouter" },
      ],
    });
    ...
    const { result } = renderHook(() =>
      useModelControls({ queryClient, requestGateway: vi.fn() }),
    );
  ...
  it("keeps a sticky manual pick that is still in the catalog", async () => {
    ...
    queryClient.setQueryData(modelOptionsQueryKey("default"), {
      providers: [
        {
          models: ["openrouter/glm-4.7", "openai/gpt-5.5"],
          name: "OpenRouter",
          slug: "openrouter",
        },
      ],
  ...
  it("does not let a stale forced profile refresh overwrite a newer picker choice", async () => {
    const profileDefault =
      deferred<Awaited<ReturnType<typeof getGlobalModelInfo>>>();
  ...
  it("does not let an older profile refresh overwrite a newer profile", async () => {
    ...
    vi.mocked(getGlobalModelInfo)
      ...
      .mockReturnValueOnce(profileC.promise);
    ...
    profileC.resolve({
      model: "profile-c-model",
      provider: "profile-c-provider",
    });
    await refreshC;
    profileB.resolve({
      model: "profile-b-model",
      provider: "profile-b-provider",
    });
    await refreshB;
  ...
  it("keeps an active-A focused-B selection cache and request on B", async () => {
    ...
    const requestGateway = vi.fn(
      async () => ({ key: "model", value: "tile-model" }) as never,
    );
    ...
    expect(
      queryClient.getQueryData(
        modelOptionsQueryKey("profile-b", "runtime-b", "connection-b"),
      ),
    ).toMatchObject({
      model: "tile-model",
      provider: "anthropic",
    ...
    expect(
      queryClient.getQueryData(modelOptionsQueryKey("profile-a", "runtime-b")),
    ).toBeUndefined();
    expect(
      queryClient.getQueryData(
        modelOptionsQueryKey("profile-b", "runtime-b", "connection-a"),
      ),
    ).toBeUndefined();
  ...
  it("rolls a failed focused-B selection back only in B cache", async () => {
    const queryClient = new QueryClient();
    const ownerBKey = modelOptionsQueryKey(
      "profile-b",
      "runtime-b",
      "connection-b",
    );
    const ambientAKey = modelOptionsQueryKey(
      "profile-a",
      "runtime-b",
      "connection-a",
    );
    queryClient.setQueryData(ownerBKey, {
      model: "old-b",
      provider: "provider-b",
      providers: [],
    });
    queryClient.setQueryData(ambientAKey, {
      model: "model-a",
      provider: "provider-a",
      providers: [],
    });
    ...
    expect(queryClient.getQueryData(ownerBKey)).toMatchObject({
      model: "old-b",
      provider: "provider-b",
    });
    expect(queryClient.getQueryData(ambientAKey)).toMatchObject({
      model: "model-a",
      provider: "provider-a",
    });
    expect(notifyError).toHaveBeenCalled();
