...
function settled(
  ...
  text = storedSessionId,
): ClientSessionState {
  return {
    ...createClientSessionState(storedSessionId),
    messages: transcript(storedSessionId, text),
  };
}
...
describe("SessionStateCache", () => {
    ...
    const cache = new SessionStateCache(
      {
        ...
        onEvict: (runtimeId, state) => {
          if (
            ...
            owners.get(state.storedSessionId) === runtimeId
          ) {
  ...
  it.each([
    ...
    [
      "awaiting",
      (state: ClientSessionState) => ({ ...state, awaitingResponse: true }),
      false,
    ],
    [
      "needs input",
      (state: ClientSessionState) => ({ ...state, needsInput: true }),
      false,
    ],
  ...
  it("keeps unsaved drafts and pending messages out of the eviction pool", () => {
    const draft = {
      ...
      messages: transcript("draft"),
    };
    const pending = settled("pending");
    pending.messages = [
      { id: "pending-assistant", role: "assistant", parts: [], pending: true },
    ];
  ...
  it("retains lightweight status while releasing an evicted transcript", () => {
    ...
    expect($sessionStates.get().runtime).toMatchObject({
  ...
  describe("authoritative liveness probe (#95189)", () => {
    ...
    it.each([
      ...
      "evicts an orphaned %s transcript once the authoritative record settles",
      (_label, decorate) => {
        const evicted: string[] = [];
        ...
        $sessionStates.set({
          orphaned: { ...orphaned, busy: false, awaitingResponse: false },
        });
        cache.prune();
    ...
    it.each([
      ...
      "still never evicts %s transcripts when no authority probe is wired",
      (_label, decorate) => {
        ...
        $sessionStates.set({
          working: { ...working, busy: false, awaitingResponse: false },
        });
