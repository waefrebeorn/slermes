...
export const MASTER_DETAIL_WIDE_COLS =
  "sm:grid-cols-[minmax(0,var(--md-split,0.75fr))_minmax(0,1fr)]";
...
export function MasterDetail({
  ...
  const startSplitDrag = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const startWidth = (grid.children[0] as HTMLElement).getBoundingClientRect()
      .width;
    const max = Math.max(
      SPLIT_MIN_LEFT_PX,
      grid.getBoundingClientRect().width - SPLIT_MIN_RIGHT_PX,
    );
    setDragging(true);
    ...
    const onMove = (move: globalThis.PointerEvent) => {
      setPaneWidthOverride(
        ...
        Math.round(
          Math.min(
            max,
            Math.max(SPLIT_MIN_LEFT_PX, startWidth + (move.clientX - startX)),
          ),
        ),
      );
    };
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div
        ...
        style={
          override !== undefined
            ? ({ "--md-split": `${override}px` } as CSSProperties)
            : undefined
        }
      >
        {resizeId ? (
                ...
                <div
                  className={cn(
                    ...
                    dragging
                      ...
                      : "group-hover/vsash:bg-(--ui-stroke-secondary)",
                  )}
                />
              </div>
              {rest}
  ...
  const startDrag = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const onMove = (move: globalThis.PointerEvent) => {
      setPaneHeightOverride(
        ...
        Math.min(
          max,
          Math.max(0, Math.round(startHeight + (startY - move.clientY))),
        ),
      );
    };
      ...
      >
        <div
          className={cn(
            ...
            dragging
              ? "bg-(--ui-stroke-secondary)"
              : "group-hover/sash:bg-(--ui-stroke-secondary)",
          )}
        />
      </div>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1.5">
          ...
          <Tip label={collapsed ? t.common.expand : t.common.collapse}>
            <Button
              ...
              onClick={() =>
                setPaneHeightOverride(id, collapsed ? undefined : 0)
              }
              size="icon"
              variant="ghost"
            >
    ...
    >
      ...
      {action != null ? (
        ...
        <Switch
          ...
          checked={enabled}
          className={cn(
            "mr-1.5 shrink-0 cursor-pointer",
            !enabled && "opacity-60",
          )}
          disabled={busy}
