 ...
 */
export function useHudTranscriptBand(
  ...
  useEffect(() => {
    ...
    const measure = () => {
      const el =
        viewport ??
        root.querySelector<HTMLElement>('[data-slot="aui_thread-viewport"]');
      ...
      const rows = el?.querySelectorAll<HTMLElement>(
        '[data-slot="aui_thread-content"] > *:not([data-slot])',
      );
      ...
      const visible = hudTranscriptHeight({
        barHeight:
          root
            .querySelector<HTMLElement>('[data-slot="composer-dock"]')
