  ...
  for (const node of roots) {
    ...
    if (node.delegationId) {
      const byId = groups.find(
        (g) => g.id === `delegation:${node.delegationId}`,
      );
    ...
    const sameShape =
      prev &&
      !prev.id.startsWith("delegation:") &&
      node.taskCount > 1 &&
      prev.taskCount === node.taskCount;
    ...
    <section className="grid min-w-0 gap-3">
      <p className="text-[0.66rem] font-medium uppercase tracking-wider text-muted-foreground/70">
        {group.delegationIndex > 0
          ? t.agents.delegation(group.delegationIndex)
          : ""}{" "}
