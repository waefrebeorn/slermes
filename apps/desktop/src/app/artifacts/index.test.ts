...
${payload}
  ...
  it("keeps native browser screenshots without indexing embedded image data", () => {
    const artifacts = collectArtifactsForSession(
      ...
      [
        ...
        {
          content:
            "Image attached. Screenshot path: /tmp/hermes browser/summary screenshot.png",
          role: "tool",
          timestamp: 1_781_774_002,
          tool_name: "browser_vision",
        },
        {
          content:
            "Image attached. Screenshot path: C:\\Users\\Example User\\.hermes\\screenshot.png",
          role: "tool",
          timestamp: 1_781_774_003,
          tool_name: "browser_vision",
        },
  ...
  it("resolves remote image artifact thumbnails through the desktop fs bridge", async () => {
    ...
    const path =
      "/Users/me/.hermes/skills/work-esab/references/images/manual-step03.jpeg";
    ...
    const result = await loadArtifactsForSessions(sessions, async (session) => {
      ...
      try {
        ...
        if (session.id === "session-2") {
          throw new Error(
            "Session transcript exceeds the Desktop safe-load limit",
          );
        }
