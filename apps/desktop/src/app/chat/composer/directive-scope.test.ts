...
function paste(editor: HTMLDivElement, text: string) {
  insertComposerContentsAtCaret(
    editor,
    pathifyRefs(linkifyUrls(text)),
    openDirectiveScope(editor),
  );
}
...
describe("directive scope is a browse mode, not text to maintain", () => {
  it("Tab-descend carries the scope down instead of dropping to a bare path", () => {
    ...
    expect(result.current.trigger).toMatchObject({
      kind: "@",
      scope: "folder",
      value: "apps/deskt",
    });
    ...
    act(() =>
      result.current.replaceTriggerWithChip(folderItem("apps/desktop"), {
        descend: true,
      }),
    );
  ...
  it("a pick mid-message keeps the trailing prose and consumes the whole token", () => {
    ...
    const result = withTrigger(
      editor,
      "@folder:apps/deskt and some trailing words",
    );
    ...
    expect(composerPlainText(editor)).toBe(
      "@folder:`apps/desktop/` and some trailing words",
    );
  ...
  it("pasting into an open @url: scope consumes it instead of stacking", () => {
    ...
    expect(composerPlainText(editor)).toBe(
      "refer to @url:`https://github.com/NousResearch/hermes-agent/pull/74533`",
    );
  ...
  it("a normal paste with no open scope is untouched", () => {
    ...
    expect(composerPlainText(editor)).toBe(
      "look at @url:`https://example.com/x`",
    );
  });

  it("scope parsing leaves an unscoped @ query alone", () => {
    expect(detectTrigger("@apps/desk")).toMatchObject({
      kind: "@",
      value: "apps/desk",
    });
