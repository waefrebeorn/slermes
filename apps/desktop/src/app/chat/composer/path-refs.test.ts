...
const spaceOn = (text: string) => {
  ...
  return {
    editor,
    event: { currentTarget: editor, key: " " } as KeyboardEvent<HTMLDivElement>,
  };
};

describe("pathifyRefs", () => {
  it("promotes a Tab-descended folder token", () => {
    expect(pathifyRefs("look at @apps/desktop/")).toBe(
      "look at @folder:`apps/desktop`",
    );
  });

  it("promotes a typed file path", () => {
    expect(pathifyRefs("read @src/main.ts please")).toBe(
      "read @file:`src/main.ts` please",
    );
  });

  it("promotes every bare path in one message", () => {
    expect(pathifyRefs("@apps/desktop/ and @src/main.ts")).toBe(
      "@folder:`apps/desktop` and @file:`src/main.ts`",
    );
  });
  ...
  it("leaves simple refs alone", () => {
    expect(pathifyRefs("check @diff and @staged")).toBe(
      "check @diff and @staged",
    );
  });

  it("leaves an email alone", () => {
    expect(pathifyRefs("mail me at brooklyn@nous.dev/x")).toBe(
      "mail me at brooklyn@nous.dev/x",
    );
  });
