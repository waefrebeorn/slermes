...
const TODO_GLYPHS: Record<
  Exclude<TodoStatus, "in_progress" | "pending">,
  ...
  return (
    <span
      ...
      className={cn(
        ...
        item.state === "failed" ? "bg-destructive/80" : "bg-emerald-500/70",
      )}
    />
  );
}
        ...
        leading={
          item.depth ? (
            <span
              ...
              style={{ paddingLeft: `${Math.min(item.depth, 4) * 0.8}rem` }}
            >
              {leadingGlyph(item, s)}
            </span>
          ) : (
