...
describe("CodingStatusRow", () => {
  ...
  it("opens the review pane from the branch and the diff counts, never the bar itself", () => {
    ...
    const { container } = render(
      <CodingStatusRow onOpen={onOpen} repoPath="/repo" />,
    );
  ...
  it("wraps the click targets without adding a layout box", () => {
    ...
    expect(
      screen
        .getByText("bb/hitbox")
        .parentElement?.classList.contains("contents"),
    ).toBe(true);
    expect(
      screen.getByText("12").closest("button")?.classList.contains("contents"),
    ).toBe(true);
  ...
  it("parks the copy glyph against the end of the path, not the end of the row", () => {
    render(
      <CodingStatusRow
        onOpen={() => undefined}
        repoPath="/Users/someone/www/repo"
      />,
    );
  ...
  it("copies the absolute cwd inline — checkmark feedback, no toast", async () => {
    ...
    await waitFor(() =>
      expect(writeText).toHaveBeenCalledWith("/Users/someone/www/repo"),
    );
