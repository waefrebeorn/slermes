...
const groupLabel = (group: StatusGroup, s: Translations["statusStack"]) => {
  ...
  if (group.type === "todo") {
    return s.todos(
      group.items.filter((i) => i.todoStatus === "completed").length,
      group.items.length,
    );
  }

  return group.type === "subagent"
    ? s.subagents(group.items.length)
    : s.background(group.items.length);
};

const hasRunningTodo = (group: StatusGroup) =>
  group.type === "todo" &&
  group.items.some(
    (item) => item.todoStatus === "in_progress" && item.state === "running",
  );
...
export function ComposerStatusStack({
  ...
  const hasRunningBackground = groups.some(
    (g) =>
      g.type === "background" && g.items.some((i) => i.state === "running"),
  );
  ...
  const visiblePreviews = previews.filter(
    (item) => hasRunningBackground || !isLocalhostPreview(item.target),
  );
    ...
    visiblePreviews.length > 0 && sessionId
      ? visiblePreviews.map((item) => (
          <PreviewStatusRow
            item={item}
            key={item.id}
            onDismiss={(id) => dismissPreviewArtifact(sessionId, id)}
          />
        ))
      : [];
  ...
  if (billing && sessionId && billing.sessionId === sessionId) {
    sections.push({
      key: "billing",
      node: <BillingBanner sessionId={sessionId} />,
    });
  }
        ...
        >
          {group.items.map((item) => (
            <StatusItemRow
              ...
              onDismiss={
                sessionId
                  ? (id) => dismissBackgroundProcess(sessionId, id)
                  : undefined
              }
      ...
      {sections.length > 0 && (
        <div
          className={cn(
            ...
            scrolledUp
              ? "opacity-30 group-hover/composer:opacity-100"
              : "opacity-100",
          )}
        >
