...
describe("detectTrigger", () => {
  ...
  it("detects a slash command query", () => {
    expect(detectTrigger("/skill")).toEqual({
      kind: "/",
      query: "skill",
      tokenLength: 6,
      value: "skill",
  ...
  it("detects an at-mention query", () => {
    expect(detectTrigger("@file")).toEqual({
      kind: "@",
      query: "file",
      tokenLength: 5,
      value: "file",
  ...
  it("keeps the at-mention live while walking into subfolders", () => {
    ...
    expect(detectTrigger("@./src")).toEqual({
      kind: "@",
      query: "./src",
      tokenLength: 6,
      value: "./src",
  ...
  it("treats a chip edge as a token boundary, like whitespace", () => {
    ...
    expect(detectTrigger("\uFFFC@Desk")).toEqual({
      kind: "@",
      query: "Desk",
      tokenLength: 5,
      value: "Desk",
  ...
  it("splits a typed ref kind off as the browse scope", () => {
    ...
    expect(detectTrigger("@url:")).toEqual({
      kind: "@",
      query: "url:",
      scope: "url",
      tokenLength: 5,
  ...
  it("treats a mid-message slash as an inline reference", () => {
    ...
    expect(detectTrigger("hello /")).toEqual({
      kind: "/",
      inline: true,
      query: "",
      tokenLength: 1,
...
describe("extractClipboardImageBlobs", () => {
  ...
  it("keeps inline HTML images when the copy is image-only", () => {
    const clipboard = {
      ...
      getData: (type: string) =>
        type === "text/html"
          ? `<img src="data:image/png;base64,${"A".repeat(20_000)}">`
          : "",
      items: [],
    } as unknown as DataTransfer;
  ...
  it("drops sub-thumbnail inline images — spacers, trackers, blurhash placeholders", () => {
    const clipboard = {
      ...
      getData: (type: string) =>
        type === "text/html"
          ? `<img src="data:image/png;base64,${"A".repeat(64)}">`
          : "",
      items: [],
    } as unknown as DataTransfer;
