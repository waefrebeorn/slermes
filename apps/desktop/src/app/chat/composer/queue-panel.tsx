...
const entryPreview = (entry: QueuedPromptEntry, c: Translations["composer"]) =>
  (entry.displayText ?? entry.text).trim() ||
  (entry.attachments.length > 0 ? c.attachmentOnly : c.emptyTurn);
        ...
        return (
          <StatusRow
            className={cn(
              ...
              isEditing &&
                "border-[color-mix(in_srgb,var(--dt-composer-ring)_40%,transparent)] bg-accent/25",
            )}
            key={entry.id}
            trailing={
              <>
          ...
          >
            <div className="min-w-0 flex-1">
              <p className="truncate text-[0.73rem] leading-4 text-foreground/92">
                {entryPreview(entry, c)}
              </p>
              {(attachmentsCount > 0 || isEditing) && (
                <div className="mt-0.5 flex items-center gap-1.5 text-[0.64rem] text-muted-foreground/75">
                  {attachmentsCount > 0 && (
                    <span>{c.attachments(attachmentsCount)}</span>
                  )}
                  {isEditing && (
