...
export type InlineRefInput =
  string | { kind: string; label?: string; value: string };
...
export function sessionInlineRef(payload: SessionDragPayload): InlineRefInput {
  return {
    ...
    value: `${payload.profile || "default"}/${payload.id}`,
  };
}

export function dragHasAttachments(
...
export function droppedFileInlineRef(
  candidate: DroppedFile,
...
export function droppedFileInlineRefs(
...
function parseInlineRef(
  ref: InlineRefInput,
): { kind: string; label?: string; rawValue: string } | null {
...
function buildRefFragment(
  refs: readonly { kind: string; label?: string; rawValue: string }[],
  {
    needsBeforeSpace,
    needsAfterSpace,
  }: { needsAfterSpace: boolean; needsBeforeSpace: boolean },
) {
