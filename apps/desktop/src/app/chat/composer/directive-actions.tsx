...
function actionableChipAt(
  ...
  return chip &&
    kind &&
    chip.dataset.refId &&
    editor.contains(chip) &&
    DIRECTIVE_ACTIONS[kind]
    ? chip
    : null;
}
...
function anchorFor(chip: HTMLElement): Anchor | null {
  ...
  const action = chip.dataset.refKind
    ? DIRECTIVE_ACTIONS[chip.dataset.refKind]
    : undefined;
 ...
 */
export function ComposerDirectiveActions({
  editorRef,
}: {
  ...
  useEffect(() => {
    const onPointerOver = (event: PointerEvent) => {
      ...
      cancelHide();
      setAnchor((current) =>
        current?.chip === chip ? current : anchorFor(chip),
      );
    };
    ...
    const onPointerOut = (event: PointerEvent) => {
      ...
      if (
        chip &&
        editor &&
        chip === actionableChipAt(event.relatedTarget, editor)
      ) {
        return;
      }
    ...
    const reanchor = () =>
      setAnchor((current) => (current ? anchorFor(current.chip) : null));
