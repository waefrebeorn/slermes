...
export function refChipElement(
  kind: string,
  rawValue: string,
  displayLabel?: string,
) {
  ...
  chip.append(
    directiveIconElement(kind),
    document.createTextNode(displayLabel || refChipLabel(kind, id)),
  );
...
export function slashChipElement(
  command: string,
  kind: SlashChipKind,
  label?: string,
) {
  ...
  chip.append(
    slashIconElement(kind),
    document.createTextNode(label || command),
  );
...
function chipSpans(text: string, options: SlashCommandScanOptions) {
  ...
  const refs = Array.from(text.matchAll(REF_RE)).map((match) => {
    ...
    return {
      ...
      node: () => refChipElement(match[1] || "file", match[2] || ""),
      start,
    };
  });
...
export function renderComposerContents(
  target: HTMLElement,
  text: string,
  options?: SlashCommandScanOptions,
) {
...
export function serializeTextBefore(
  editor: HTMLElement,
  container: Node,
  offset: number,
): string {
...
export function insertComposerContentsAtCaret(
  editor: HTMLElement,
  text: string,
  consumeBefore = 0,
) {
  const scoped =
    consumeBefore > 0 ? rangeBeforeCaret(editor, consumeBefore) : null;
...
export function rangeBeforeCaret(
  editor: HTMLElement,
  length: number,
): Range | null {
  const hit = composerSelectionRange(editor);
...
export function deleteChipBeforeCaret(editor: HTMLElement): boolean {
  ...
  if (startContainer === editor) {
    chip =
      startOffset > 0 ? (editor.childNodes[startOffset - 1] ?? null) : null;
  ...
  if (
    ...
    !(chip as HTMLElement).dataset.refText
  ) {
    return false;
  }
...
export function composerPlainText(node: Node): string {
  ...
  return block && text && el.dataset.slot !== RICH_INPUT_SLOT
    ? `${text}\n`
    : text;
}
...
export function normalizeComposerEditorDom(editor: HTMLElement) {
  ...
  if (
    ...
    if (
      (wrapper.tagName === "DIV" || wrapper.tagName === "P") &&
      wrapper.dataset.slot !== RICH_INPUT_SLOT
    ) {
