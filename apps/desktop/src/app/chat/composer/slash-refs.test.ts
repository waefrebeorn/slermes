...
const commands = (
  text: string,
  options?: Parameters<typeof slashCommandMatches>[1],
) =>
  slashCommandMatches(text, options).map(
    (match) => `${match.kind}:${match.command}`,
  );
...
describe("slashCommandMatches", () => {
  ...
  it("recognizes a leading command and a skill named mid-prose", () => {
    expect(commands("/some-skill clean this with /other-skill please")).toEqual(
      ["skill:/some-skill", "skill:/other-skill"],
    );
  });
  ...
  it("holds a trailing token as still-typed unless the text is inert", () => {
    ...
    expect(commands("/some-skill", { trailingCommitted: true })).toEqual([
      "skill:/some-skill",
    ]);
  });
  ...
  it("offers a built-in only as an invocation, never mid-message", () => {
    ...
    expect(commands("start over with /some-skill ")).toEqual([
      "skill:/some-skill",
    ]);
  });
