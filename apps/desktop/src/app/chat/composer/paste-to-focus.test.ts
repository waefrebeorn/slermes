...
const image = (name = "shot.png") =>
  new File([new Uint8Array(8192)], name, { type: "image/png" });
...
describe("routeClipboardToComposer", () => {
  it("routes pasted text to the composer as an insert", async () => {
    ...
    expect(routeClipboardToComposer(clipboard({ text: "hello there" }))).toBe(
      true,
    );
    await flushBus();
    off();
  ...
  it("chips a pasted link the same way a focused paste does", async () => {
    ...
    routeClipboardToComposer(
      clipboard({ text: "see https://example.com/docs" }),
    );
    await flushBus();
    off();
  ...
  it("attaches clipboard images and pulls focus on an image-only paste", async () => {
    ...
    const offAttach = onComposerAttachImagesRequest(({ blobs }) =>
      attached.push(blobs),
    );
    const offFocus = onComposerFocusRequest(() => focused.push(true));

    expect(routeClipboardToComposer(clipboard({ files: [image()] }))).toBe(
      true,
    );
    await flushBus();
    offAttach();
  ...
  it("takes both from a mixed paste — images attach AND the text inserts", async () => {
    ...
    routeClipboardToComposer(
      clipboard({ files: [image()], text: "look at this" }),
    );
    await flushBus();
    offAttach();
    offInsert();
