...
describe("the inline reference contract", () => {
  ...
  it("ships no colour from TypeScript — the theme owns every accent", () => {
    ...
    for (const [kind, style] of Object.entries(REFERENCE_STYLES)) {
      expect(style, `${kind} must not carry a colour`).not.toHaveProperty(
        "color",
      );
    }
  ...
  it("keeps commands and skills visually distinct", () => {
    ...
    expect(refAttrs("skill")["data-ref"]).not.toBe(
      refAttrs("command")["data-ref"],
    );
    expect(referenceStyle("skill").codicon).not.toBe(
      referenceStyle("command").codicon,
    );
  });
});
