    ...
    >
      ...
      {items.length === 0 ? (
        loading ? (
          ...
          <CompletionDrawerEmpty title={copy.lookupNoMatches}>
            {kind === "@" ? (
              <>
                {copy.lookupTry}{" "}
                <span className="font-mono text-foreground/80">@file:</span>{" "}
                {copy.lookupOr}{" "}
              ...
              <>
                {copy.lookupTry}{" "}
                <span className="font-mono text-foreground/80">:joy:</span>.
              </>
            ) : (
              <>
                {copy.lookupTry}{" "}
        ...
        items.map((item, index) => {
          ...
          const display =
            meta?.display ?? (isSlash ? `/${item.label}` : item.label);
            ...
            <Fragment key={item.id}>
              ...
              >
                {isEmoji ? (
                  ...
                  // as decoration.
                  <span className="min-w-0 shrink truncate leading-5 text-foreground">
                    {display}
                  </span>
                ) : (
                  <>
                    <span
                      className="grid size-4 shrink-0 place-items-center text-(--ref-color)"
                      data-ref={refKind}
                    >
                      <Codicon
                        name={referenceStyle(refKind).codicon}
                        size="0.875rem"
                      />
                    </span>
                    <span className="min-w-0 shrink truncate font-medium leading-5 text-foreground">
                      {display}
                    </span>
                    {description && (
