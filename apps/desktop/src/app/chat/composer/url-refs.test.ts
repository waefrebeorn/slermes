...
describe("linkifyUrls", () => {
  it("rewrites a bare link as a url directive", () => {
    expect(linkifyUrls("https://example.dev/a/b")).toBe(
      "@url:`https://example.dev/a/b`",
    );
  });

  it("keeps the link in place mid-sentence and leaves its punctuation behind", () => {
    expect(linkifyUrls("read https://example.dev/a. then stop")).toBe(
      "read @url:`https://example.dev/a`. then stop",
    );
  });
  ...
  it("rewrites every link in a multi-link paste", () => {
    expect(linkifyUrls("http://a.dev and https://b.dev")).toBe(
      "@url:`http://a.dev` and @url:`https://b.dev`",
    );
  });

  it("leaves a link that is already a directive alone", () => {
    expect(linkifyUrls("@url:`https://example.dev`")).toBe(
      "@url:`https://example.dev`",
    );
  });

  it("leaves text without a scheme alone", () => {
    expect(linkifyUrls("example.dev/a and src/foo.ts")).toBe(
      "example.dev/a and src/foo.ts",
    );
  });
});
