...
describe("AttachmentList", () => {
  ...
  it("renders valid attachments", async () => {
    const attachments = [
      makeAttachment("a", "doc.pdf"),
      makeAttachment("b", "img.png"),
    ];
  ...
  it("renders the thumbnail in the pill but opens the full-resolution image in the lightbox", async () => {
    ...
    expect(
      screen.getByAltText<HTMLImageElement>("shot.png").getAttribute("src"),
    ).toBe(THUMBNAIL_URL);
  ...
  it("loads a path-backed full image only when opened and releases it when closed", async () => {
    ...
    const { container } = await renderWithI18n(
      <AttachmentList attachments={[image]} />,
    );
  ...
  it("falls back to the original host path after an image was staged for a different filesystem", async () => {
    ...
    expect(
      (await screen.findByRole("dialog")).querySelector<HTMLImageElement>("img")
        ?.src,
    ).toBe(DATA_URL);
  });
  ...
  it("does not let an old occurrence open a replacement lightbox after a deferred read", async () => {
    ...
    const readFileDataUrl = vi.fn((path: string) =>
      path === "/tmp/old.png"
        ? oldRead
        : Promise.resolve("data:image/png;base64,replacement"),
    );
    ...
    const { rerender } = await renderWithI18n(
      <AttachmentList attachments={[oldOccurrence]} />,
    );
    ...
    expect(
      (await screen.findByRole("dialog")).querySelector<HTMLImageElement>("img")
        ?.src,
    ).toBe("data:image/png;base64,replacement");
  });
  ...
  it("removes an attachment from the composer chip", async () => {
    ...
    await renderWithI18n(
      <AttachmentList
        attachments={[makeAttachment("a", "doc.pdf")]}
        onRemove={onRemove}
      />,
    );
  ...
  it("still routes a non-image attachment to the preview rail", async () => {
    ...
    expect(screen.queryByRole("dialog")).toBeNull();
    expect($previewTabs.get().map((tab) => tab.target.path)).toEqual([
      "/tmp/notes.md",
    ]);
  });
});
