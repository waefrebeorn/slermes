...
import {
  cachedPathCompletion,
  hasCachedPathCompletion,
...
function classify(entry: CompletionEntry): {
  ...
  if (match) {
    ...
    return {
      ...
      display: rest
        ? refChipLabel(kind, rest)
        : textValue(entry.display, `@${kind}:`),
...
export function useAtCompletions(options: {
  ...
  const contributedEntries = useCallback(
    (query: string): CompletionEntry[] => {
      ...
      for (const contribution of contributed) {
        const source = contribution.data as
          ComposerAtCompletionSource | undefined;
  ...
  const cacheKey = useCallback(
    (query: string) => `${cwd ?? ""}|${sessionId ?? ""}|${query}`,
    [cwd, sessionId],
  );
    ...
    async (query: string): Promise<CompletionPayload> => {
      ...
      try {
        ...
        const result = await cachedPathCompletion(cacheKey(query), () =>
          gateway.request<{ items?: CompletionEntry[] }>(
            "complete.path",
            params,
          ),
        );
  ...
  const isCached = useCallback(
    (query: string) => hasCachedPathCompletion(cacheKey(query)),
    [cacheKey],
  );
