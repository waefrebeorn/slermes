...
export function useSlashCompletions(options: {
  ...
  useEffect(() => {
    ...
    void cachedSlashCompletion("catalog", () =>
  ...
  const fetcher = useCallback(
    async (query: string): Promise<CompletionPayload> => {
      ...
      if (skinArg && skinThemes) {
        const items = desktopSkinSlashCompletions(
          skinThemes,
          activeSkin ?? "",
          skinArg[1] ?? "",
        ).map((entry) => ({
      ...
      try {
        if (!query) {
          ...
          for (const [command, meta] of catalog.pairs ?? []) {
            if (
              !categorized.has(command.toLowerCase()) &&
              isDesktopSlashExtensionCommand(command)
            ) {
              skillRows.push({
                text: command,
                display: command,
          ...
          items.push(
            ...rankSkillCommands(skillRows, catalog.skills, {
        ...
        const result = await cachedSlashCompletion(
          `slash:${text.toLowerCase()}`,
          () =>
            gateway.request<{
        ...
        const decorated = (result.items ?? [])
          ...
          .filter(
            (item) => isArgCompletion || isDesktopSlashSuggestion(item.text),
          )
          .map((item) => ({
            ...item,
            ...
            group: isArgCompletion
              ? "Options"
              : slashCompletionGroup(item.text, item.kind),
  ...
  const toItem = useCallback(
    (entry: CompletionEntry, index: number): Unstable_TriggerItem => {
  ...
  const isCached = useCallback(
    (query: string) => {
      ...
      return hasCachedSlashCompletion(
        query ? `slash:${text.toLowerCase()}` : "catalog",
      );
    },
    [skinThemes],
  );
