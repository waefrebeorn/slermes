...
function gatewayStub(
  items = [
    {
      text: "@file:src/research-notes.md",
...
async function searchAndRead(
  result: {
    current: {
      adapter: { search?: (q: string) => readonly { label: string }[] };
    };
  },
  q: string,
): Promise<string[]> {
...
describe("contributed @ completion sources", () => {
  it("merges contributed rows ahead of gateway path results", async () => {
    vi.useFakeTimers();
    addSource("bots", (q) =>
      "researcher".startsWith(q)
        ? [{ insert: "@researcher", meta: "Bot · Researcher" }]
        : [],
    );

    const { result } = renderHook(() =>
      useAtCompletions({
        gateway: gatewayStub() as never,
        sessionId: "s1",
        cwd: "/repo",
      }),
    );
  ...
  it("deduplicates contributed and core profile handles without collapsing distinct references", async () => {
    ...
    const gateway = gatewayStub([
      {
        text: "@researcher",
        display: "@researcher",
        meta: "Research and intelligence analyst",
      },
      {
        text: "@researcher-homelab",
        display: "@researcher-homelab",
        meta: "agent profile",
      },
      {
        text: "@file:src/researcher.md",
        display: "researcher.md",
        meta: "file",
      },
    ]);
    ...
    const rows = await searchAndRead(result, "rese");
    expect(
      rows.filter((label) => label.toLowerCase() === "@researcher"),
    ).toEqual(["@Researcher"]);
