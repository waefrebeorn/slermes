...
import {
  clearWakeIndicator,
  syncWakeIndicatorWithVoice,
} from "@/lib/wake-indicator";
import {
...
export function useComposerVoice({
  ...
  const pendingTurnResponse = () => {
    ...
    return collectUnspokenTurnSpeech(
      messages,
      resolveSpokenReply(sessionId, messages)?.id ?? null,
    );
  };
  ...
  useEffect(() => {
    ...
    if (
      syncWakeIndicatorWithVoice(voiceConversationActive, conversation.status)
    ) {
  ...
  useEffect(
    () =>
      onComposerVoiceToggleRequest(
        (toggled) => toggled === target && toggleVoiceConversation(),
      ),
