...
function isDockDragPlatform(target: EventTarget | null) {
  return (
    ...
    Boolean(target.closest('[data-slot="composer-drag-region"]'))
  );
}
...
function dockProximityOf(rect: DOMRect, area?: PopoutBounds) {
  ...
  const horizontalDist = Math.abs(
    rect.left + rect.width / 2 - (a.left + a.right) / 2,
  );
  ...
  const v =
    verticalGap <= 0
      ? 1
      : Math.max(0, 1 - verticalGap / DOCK_VERTICAL_FALLOFF_PX);
...
export function useComposerPopoutGestures({
  ...
  const peelOffFromDock = useCallback(
    (state: PressState, clientX: number, clientY: number) => {
      ...
      const next = popoutPositionUnderPointer(
        clientX,
        clientY,
        grabX,
        grabY,
  ...
  useEffect(() => {
    ...
    const flush = () => {
      ...
      const composer = composerRef.current;
      const size = composer
        ? { height: composer.offsetHeight, width: composer.offsetWidth }
        : undefined;
      ...
      if (composer) {
        setDockProximity(
          dockProximityOf(composer.getBoundingClientRect(), area),
        );
      }
    };
      ...
      if (!state.armed) {
        ...
        if (state.mode === "dock") {
          ...
          if (-deltaY > PEEL_OUT_PX && -deltaY > Math.abs(deltaX)) {
            ...
            Math.abs(deltaX) > PEEL_OUT_PX ||
            deltaY > LONG_PRESS_MOVE_TOLERANCE
          ) {
            resetGesture();
          }
        } else if (
          Math.abs(deltaX) > LONG_PRESS_MOVE_TOLERANCE ||
      ...
      if (state.armed && state.mode === "float") {
        ...
        if (rect && dockProximityOf(rect, area) >= 1) {
          ...
          const size = composer
            ? { height: composer.offsetHeight, width: composer.offsetWidth }
            : undefined;
          setComposerPopoutPosition(groupId, liveRef.current, {
            area,
            persist: true,
            size,
          });
