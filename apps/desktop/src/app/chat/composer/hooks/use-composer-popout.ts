...
function usePopoutPlacement(
  ...
  const reclamp = useCallback(() => {
    ...
    const next = clampPopoutPosition(
      getComposerPopoutZone(groupId).position,
      size,
      readPopoutBounds(el),
    );
    ...
    setPlacement((prev) =>
      prev.bottom === next.bottom && prev.right === next.right ? prev : next,
    );
  }, [composerRef, groupId]);
...
export function useComposerPopout({ composerRef }: UseComposerPopoutOptions) {
  ...
  const popoutPosition = usePopoutPlacement(
    composerRef,
    groupId,
    zone.position,
    dragging,
