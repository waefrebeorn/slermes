...
function renderSubmitHook({
  ...
  const resolvedSurfaceId =
    surfaceId === undefined ? `test-surface-${++surfaceSequence}` : surfaceId;
  ...
  const Wrapper = ({ children }: PropsWithChildren) => {
    ...
    return (
      <ComposerScopeProvider
        value={{ ...MAIN_COMPOSER_SCOPE, target: scopeTarget }}
      >
...
describe("useComposerSubmit external request routing", () => {
  ...
  it("does not fan out a main ship across keep-alives or other projects", async () => {
    ...
    const hiddenMain = renderSubmitHook({
      sessionKey: "session-b",
      visible: false,
    });
    const visibleTile = renderSubmitHook({
      scopeTarget: "tile:project-b",
      sessionKey: "tile-session",
    });
    ...
    expect(requestComposerSubmit("ship this branch", { target: "main" })).toBe(
      true,
    );
  ...
  it("uses the captured surface id when two visible composers share a target", async () => {
    ...
    requestComposerSubmit("ship exactly one session", {
      surfaceId: second.composerSurfaceId,
      target: "main",
    });
  ...
  it("submits to the session visible at click time even when the same click switches tabs", async () => {
    const hiddenA = renderSubmitHook({
  ...
  it("does not fan out when visible composers do not have queue session keys yet", async () => {
    ...
    await waitFor(() =>
      expect(firstNewSession.onSubmit).toHaveBeenCalledTimes(1),
    );
  ...
  it("fails closed when a pinned origin surface is no longer visible", () => {
    const hidden = renderSubmitHook({
      sessionKey: "hidden-origin",
      visible: false,
    });
...
describe("useComposerSubmit busy-turn routing", () => {
  ...
  it("treats a payload mid-turn as send (steer), not stop", async () => {
    const { hook, onCancel, onSteer, onSubmit, queueCurrentDraft } =
      renderSubmitHook({
        busy: true,
        text: "change course",
      });
  ...
  it("runs slash commands immediately while busy", async () => {
    const { clearDraft, hook, onCancel, onSteer, onSubmit, queueCurrentDraft } =
      renderSubmitHook({
        busy: true,
  ...
  it("submits a normal turn while idle", async () => {
    const { hook, onCancel, onSteer, onSubmit, queueCurrentDraft } =
      renderSubmitHook({ text: "ordinary question" });
...
describe("useComposerSubmit with a clarify parked on the session", () => {
  ...
  const parkClarify = (sessionId: string) => {
    ...
    $gateway.set({ request: gatewayRequest } as unknown as ReturnType<
      typeof $gateway.get
    >);
  };
