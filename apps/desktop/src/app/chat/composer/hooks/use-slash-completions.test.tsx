...
async function completions(
  api: { search: (query: string) => readonly Unstable_TriggerItem[] },
  query: string,
) {
  await act(async () => {
    api.search(query);
...
describe("useSlashCompletions", () => {
  ...
  it("offers skill commands on a bare slash, not just built-ins", async () => {
    ...
    const work = items.find(
      (item) => (item.metadata as { command?: string })?.command === "/work",
    );
  ...
  it("leaves only skills for a mid-message slash", async () => {
    ...
    expect(
      inline.map((item) => (item.metadata as { command?: string })?.command),
    ).toEqual(["/work"]);
  });
  ...
  it("keeps backend order on a typed query and does not hide matches", async () => {
    ...
    expect(commandsOf(await completions(api, "research"))).toEqual([
  ...
  it("keeps a registry command in Commands even when the desktop table has no row", async () => {
              ...
              items: [
                {
                  ...
                  display: "/refine",
                  kind: "command",
                  meta: "Review this conversation",
                },
                {
                  text: "/docx",
                  display: "/docx",
                  kind: "skill",
                  meta: "Edit Word documents",
                },
                {
                  text: "/compress",
                  display: "/compress",
    ...
    const groupOf = (command: string) =>
      (
        items.find(
          (item) =>
            (item.metadata as { command?: string })?.command === command,
        )?.metadata as { group?: string }
      )?.group;
