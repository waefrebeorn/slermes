...
describe("useComposerQueue park integration", () => {
  ...
  it("sendQueuedNow while busy unparks so the settle drain flows (no stale latch)", async () => {
    ...
    const target = getQueuedPrompts(SESSION_KEY).find(
      (e) => e.id !== first!.id,
    )!;
  ...
  it("steerQueuedNow delivers via onSteer without cancelling and removes the entry", async () => {
    ...
    const { hook, onCancel, onSubmit } = renderQueueHook({
      busy: true,
      onSteer,
    });
  ...
  it("steerQueuedNow refuses unsteerable entries (slash commands execute, never steer)", async () => {
    const slash = enqueueQueuedPrompt(SESSION_KEY, {
      attachments: [],
      text: "/compress",
    });
    const onSteer = vi.fn(async () => true);
    ...
    await act(async () => {
      expect(await busy.hook.result.current.steerQueuedNow(slash!.id)).toBe(
        false,
      );
    });
  ...
  it("a delivered steer lifts the park so the rest of the queue flows", async () => {
    const steerable = enqueueQueuedPrompt(SESSION_KEY, {
      attachments: [],
      text: "redirect",
    });
    ...
    await act(async () => {
      expect(await hook.result.current.steerQueuedNow(steerable!.id)).toBe(
        true,
      );
    });
