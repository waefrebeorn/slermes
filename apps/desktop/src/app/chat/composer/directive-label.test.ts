...
function sentLabel(draft: string) {
  return hermesDirectiveFormatter
    .parse(draft)
    .filter(
      (s): s is Extract<typeof s, { kind: "mention" }> => s.kind === "mention",
    )
    .map((s) => s.label)
    .join(",");
}
...
describe("one label per reference, on every surface", () => {
  it("the popover row, the committed chip, and the sent chip all read the same", () => {
    const cases = [
      ...
      {
        text: "@file:apps/desktop/src/main.tsx",
        display: "main.tsx",
        meta: "apps/desktop/src",
      },
  ...
  it("a folder pick reads as its path, not a bare basename", () => {
    ...
    expect(editor.querySelector("[data-ref-text]")?.textContent).toBe(
      "apps/desktop/",
    );
  });
  ...
  it("Tab-descend leaves the live query, and the scope when there is one", () => {
    ...
    act(() =>
      result.current.replaceTriggerWithChip(
        backendRow("@folder:apps/desktop/", "desktop/", "dir"),
        {
          descend: true,
        },
      ),
