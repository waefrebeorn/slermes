...
const SkillsView = lazy(async () => ({
  default: (await import("../skills")).SkillsView,
}));
const MessagingView = lazy(async () => ({
  default: (await import("../messaging")).MessagingView,
}));
const ArtifactsView = lazy(async () => ({
  default: (await import("../artifacts")).ArtifactsView,
}));

// Built-in page views + their pane titles, keyed by route.
const BUILTIN_PAGES: Record<
  string,
  { render: () => ReactNode; title: string }
> = {
...
function routeTitle(path: string): string {
  ...
  return (
    contributedRoutes().find((r) => r.path === path)?.title ??
    humanizePath(path)
  );
}
...
function RouteTilePane({ path }: { path: string }) {
  ...
  const contrib = builtin
    ? null
    : contributedRoutes().find((r) => r.path === path);
