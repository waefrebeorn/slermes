...
const osDrop = (path: string): DroppedFile => ({
  file: new File(["x"], path.split("/").pop() || "f"),
  path,
});
const inAppRef = (
  path: string,
  extra: Partial<DroppedFile> = {},
...
describe("partitionDroppedFiles", () => {
  it("routes File-bearing OS drops to osDrops and path-only in-app drags to inAppRefs", () => {
    ...
    const { inAppRefs, osDrops } = partitionDroppedFiles([
      finderPdf,
      projectFile,
    ]);
  ...
  it("routes an OS folder drop (path-only, isDirectory) to inAppRefs, not the upload pipeline", () => {
    ...
    const folder = inAppRef("/Users/jeff/projects/hermes", {
      isDirectory: true,
    });
...
function stubTransfer(
  ...
  const files = entries.map(
    (entry) => new File(["x"], entry.path.split("/").pop() || "f"),
  );
...
describe("extractDroppedFiles", () => {
  ...
  const stubBridge = (
    transfer: DataTransfer & { _pathByFile: Map<File, string> },
  ) => {
...
describe("attachmentPreviewDataUrl", () => {
  ...
  it("reads a local path via the local bridge even in remote mode (paperclip/paste/OS drop)", async () => {
    ...
    await expect(
      attachmentPreviewDataUrl("/Users/me/Pictures/pic.png"),
    ).resolves.toBe(LOCAL_PREVIEW);
  ...
  it("falls back to the remote fs bridge when the path is not on this machine (project-tree drag)", async () => {
    ...
    await expect(
      attachmentPreviewDataUrl("/home/gateway/shot.png"),
    ).resolves.toBe(REMOTE_PREVIEW);
...
describe("useComposerActions native image drops", () => {
  ...
  it("copies dropped screenshot bytes before trusting a transient macOS path", async () => {
    ...
    const durablePath =
      "/Users/test/Library/Application Support/Hermes/composer-images/composer_saved.png";
    const previewUrl = "data:image/png;base64,c2NyZWVuc2hvdA==";

    const screenshot = new File(
      [new Uint8Array([1, 2, 3])],
      "Screen Shot 2026-08-11.png",
      {
        type: "image/png",
      },
    );
    ...
    await act(async () => {
      attached = await result.current.attachDroppedItems([
        { file: screenshot, path: transientPath },
      ]);
    });
...
describe("attachImagePath thumbnail separation", () => {
  ...
  it("does not apply a removed occurrence thumbnail to the same path when reattached", async () => {
    ...
    const readFileDataUrl = vi
      .fn()
      .mockResolvedValueOnce(FULL_RES)
    ...
    class MockOffscreenCanvas {
      getContext = () => ({ drawImage: vi.fn() });
      convertToBlob = vi.fn(
        async () => new Blob(["first"], { type: "image/png" }),
      );
      constructor(_width: number, _height: number) {}
    }
    ...
    expect($composerAttachments.get()[0]?.previewUrl).toBe(
  ...
  it("merges a delayed thumbnail into the latest staged state of the same occurrence", async () => {
    ...
    class MockOffscreenCanvas {
      getContext = () => ({ drawImage: vi.fn() });
      convertToBlob = vi.fn(
        async () => new Blob(["thumbnail"], { type: "image/png" }),
      );
      constructor(_width: number, _height: number) {}
    }
    ...
    act(() => {
      pending = result.current.attachImagePath(
        "C:\\Users\\alice\\Pictures\\photo.png",
      );
    });
  ...
  it("processes 72 image reads one at a time and retains only bounded thumbnails", async () => {
    ...
    await act(async () => {
      await Promise.all(
        Array.from({ length: 72 }, (_, index) =>
          result.current.attachImagePath(`/tmp/${index}.png`),
        ),
      );
    });
    ...
    expect(
      attachments.every((attachment) =>
        attachment.thumbnailUrl?.startsWith("data:image/"),
      ),
    ).toBe(true);
    expect(
      attachments.every((attachment) => attachment.previewUrl === undefined),
    ).toBe(true);
