    ...
    t: {
      ...
      profiles: {
        ...
        fleet: {
          ...
          switchTo: (name: string, gateway: string) =>
            `Switch to ${name} on ${gateway}`,
        },
...
const hasMultipleConnections =
  connectionsStore.$hasMultipleConnections as ReturnType<typeof atom<boolean>>;
const activeConnectionId = connectionsStore.$activeConnectionId as ReturnType<
...
const profiles = $profiles as ReturnType<
  typeof atom<Array<{ is_default: boolean; name: string }>>
>;
...
describe("ProfileRail fleet mode", () => {
  it("stays on the single-gateway path with one registered gateway", async () => {
    ...
    expect(
      container.querySelector('[data-slot="profile-rail-divider"]'),
    ).toBeNull();
    expect(
      screen.getByRole("button", { name: "Manage gateways…" }),
    ).toBeTruthy();
  });
  ...
  it("lays every other gateway on the strip as an at-rest group, in switcher order", async () => {
    ...
    const groups = Array.from(
      container.querySelectorAll('[data-slot="profile-rail-gateway"]'),
    ).map((node) => [
    ...
    const dividers = Array.from(
      container.querySelectorAll('[data-slot="profile-rail-divider"]'),
    ).map((node) => node.getAttribute("data-connection-id"));

    expect(dividers).toEqual(["local", "pandora", "vps"]);

    const local = screen.getByRole("group", {
      name: "Profiles on This device",
    });
    expect(
      within(local).getByRole("button", { name: "default · This device" }),
    ).toBeTruthy();
    expect(
      within(local).getByRole("button", { name: "omer · This device" }),
    ).toBeTruthy();
    ...
    expect(
      screen.getByRole("button", { name: "All profiles on this gateway" }),
    ).toBeTruthy();
    expect(
      screen.queryByRole("button", { name: "Manage gateways…" }),
    ).toBeNull();
  });
  ...
  it("marks an unreachable gateway but never hides it", async () => {
    ...
    const vps = container.querySelector(
      '[data-slot="profile-rail-gateway"][data-connection-id="vps"]',
    );
    ...
    expect(
      within(vps as HTMLElement).getByRole("button", { name: "default · VPS" }),
    ).toBeTruthy();
  });
  ...
  it("re-homes onto another gateway default from its home square", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "default · This device" }),
    );
  ...
  it("keeps every group in its slot when a different gateway is active", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "scout · Pandora" }),
    ).toBeTruthy();
  ...
  it("counts the whole fleet toward the condensed threshold and sections the menu by gateway", async () => {
    ...
    expect(
      container.querySelector('[data-slot="profile-rail-rest-square"]'),
    ).toBeNull();
  });
});
