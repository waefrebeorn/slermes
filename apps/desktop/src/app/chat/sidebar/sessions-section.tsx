  ...
  return (
    <div className="group/section flex shrink-0 items-center justify-between gap-1 pb-1 pt-1.5">
      {collapsible ? (
        ...
        <div className="flex w-fit min-w-0 items-center gap-1 leading-none">
          {labelBody}
        </div>
      )}
      {action}
...
export function SidebarSessionsSection({
  ...
  const isListGroupOpen = useCallback(
    (key: string) => nodeOpen[listGroupNodeId(key)] ?? true,
    [nodeOpen],
  );
  const sectionOpen = collapsible ? open : true;
  const hasGroupedSessions = Boolean(
  ...
  const dividerToggle = useMemo(
    () => ({
      ariaLabel: (label: string, open: boolean) =>
        t.sidebar.projects.toggle(label, !open),
      onToggle: (key: string) =>
        toggleWorkspaceNodeCollapsed(listGroupNodeId(key)),
  ...
  const renderListRow = useCallback(
    (row: SidebarListRow, draggable: boolean, action?: React.ReactNode) => {
      ...
      const label =
        "label" in row
          ? row.label
          : sessionBucketLabel(row.bucket, dividerLabels);
  ...
  const flatRows: SidebarListRow[] = useMemo(() => {
    ...
    return manualOrderIds?.length
      ? orderRowsWithinGroups(rows, manualOrderIds)
      : rows;
  }, [
    grouping,
    displayEntries,
  ...
  const sortableRowIds = useMemo(
    () => reorderableRowIds(visibleRows),
    [visibleRows],
  );
  const allSortableRowIds = useMemo(
    () => reorderableRowIds(flatRows),
    [flatRows],
  );

  const persistSessionOrder = useCallback(
    (ids: string[]) =>
      onReorderSessions?.(mergeVisibleReorder(allSortableRowIds, ids)),
  ...
  useEffect(() => {
    ...
    $sidebarListGroupIds.set(
      flatRows.flatMap((row) =>
        row.kind === "divider" ? [listGroupNodeId(row.key)] : [],
      ),
    );
  ...
  if (showProjectsSkeleton) {
    ...
    const home = projectOverview[0]?.isNoProject
      ? projectOverview[0]
      : undefined;
    const sortableProjects = home ? projectOverview.slice(1) : projectOverview;
    const projectsDraggable =
      sortableProjects.length > 1 && !!onReorderProjects;
    const Row = projectsDraggable
      ? SortableProjectOverviewRow
      : ProjectOverviewRow;

    const projectRow = (
      project: SidebarProjectTree,
      Component: typeof ProjectOverviewRow,
    ) => (
      <Component
  ...
  const resolvedContentClassName = cn(
    contentClassName,
    flatVirtualized && "overflow-visible",
  );
...
function SortableSidebarSessionRow(props: SortableSessionRowProps) {
  return (
    <SidebarSessionRow {...props} {...useSortableBindings(props.session.id)} />
  );
}

function SortableProjectOverviewRow(
  props: React.ComponentProps<typeof ProjectOverviewRow>,
) {
  return (
    <ProjectOverviewRow {...props} {...useSortableBindings(props.project.id)} />
  );
}
