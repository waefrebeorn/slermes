...
const sessionTitle = vi.fn(
  (s: SessionInfo) => (s as unknown as { title: string }).title,
);
...
vi.mock("@/lib/session-source", () => ({
  handoffOriginSource: (state?: string, platform?: string) =>
...
vi.mock("./session-actions-menu", () => ({
  ...
  SessionContextMenu: ({ children }: { children: React.ReactNode }) => (
    <>{children}</>
  ),
}));
...
function makeSession(
  overrides: Partial<SessionInfo> & { title: string },
): SessionInfo {
  return {
...
const tipTrigger = (el: HTMLElement) =>
...
describe("SidebarSessionRow running arc", () => {
  ...
  it("repaints only the session whose turn started", () => {
    render(
      <>
        {[
          makeSession({ id: "s1", title: "One" }),
          makeSession({ id: "s2", title: "Two" }),
        ].map((session) => (
          <SidebarSessionRow
            isPinned={false}
            isSelected={false}
    ...
    expect(sessionTitle).toHaveBeenCalledWith(
...
describe("SidebarSessionRow", () => {
  ...
  describe("full-title overflow tooltip", () => {
    ...
    const title =
      "A very long session title that the sidebar cannot possibly fit";
  ...
  it("exposes the exact session time through a focusable Tip trigger", () => {
    ...
    expect(age.getAttribute("datetime")).toBe(
...
describe("Inbox-style session card", () => {
  it("gives truncated card lines room for glyph ink instead of clipping them", () => {
    ...
    const title = screen.getByText(
      "Ruff lint and pytest verification",
    ).parentElement;
