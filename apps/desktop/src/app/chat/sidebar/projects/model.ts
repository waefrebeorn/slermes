...
const projectActivityTime = (project: SidebarProjectTree): number =>
  Math.max(
    ...
    projectSessions(project).reduce(
      (latest, s) => Math.max(latest, sessionRecency(s)),
      0,
    ),
  );
...
const homeFirst = (projects: SidebarProjectTree[]): SidebarProjectTree[] =>
  projects[0]?.isNoProject || !projects.some((project) => project.isNoProject)
    ? projects
    : [
        ...projects.filter((project) => project.isNoProject),
        ...projects.filter((project) => !project.isNoProject),
      ];
...
export function sortProjectsForOverview(
  ...
  const sorted = [...projects].sort((a, b) => {
    const aActive = Boolean(
      activeProjectId && a.id === activeProjectId && !a.isAuto,
    );
    const bActive = Boolean(
      activeProjectId && b.id === activeProjectId && !b.isAuto,
    );
...
export function orderProjectsByIds(
  ...
  const ordered = orderIds
    .map((id) => byId.get(id))
    .filter((p): p is SidebarProjectTree => Boolean(p));
...
export function useRepoWorktreeMap(
  ...
  useEffect(() => {
    ...
    void mapPool(repoPaths, WORKTREE_PROBE_CONCURRENCY, async (repoPath) => {
      ...
      .then(
        (entries) => void (cancelled || setMap(Object.fromEntries(entries))),
      )
...
export function useWorkspaceNodeOpen(
  id: string,
  defaultOpen = true,
): [boolean, () => void] {
  ...
  return [
    ...
    () => toggleWorkspaceNodeCollapsed(id, defaultOpen),
  ];
}
