...
export function SidebarWorkspaceGroup({
  ...
  const visibleSessions = sessions.slice(
    0,
    isProfileGroup ? PROJECT_PREVIEW_COUNT : visibleCount,
  );
  const hiddenCount = isProfileGroup
    ? 0
    ...
    <SidebarRowStack>
      {isProfileGroup ? (
        ...
        <SidebarGroupRow
          ...
          toggle={{
            ariaLabel: s.projects.toggle(group.label, !open),
            onToggle: toggleOpen,
            open,
          }}
            ...
            action={
              (onNewSession || onRemove) && (
                <div className="flex items-center">
                  {addButton}
                  {onRemove && (
                    <WorkspaceMenu onRemove={onRemove} path={group.path} />
                  )}
                </div>
              )
            }
        ...
        <>
          ...
          {hiddenCount > 0 && (
            <WorkspaceShowMoreButton
              ...
              label={group.label}
              onClick={() =>
                setVisibleCount((count) => count + SIDEBAR_GROUP_PAGE)
              }
            />
          )}
        </>
