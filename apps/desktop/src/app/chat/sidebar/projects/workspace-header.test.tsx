...
const tipTrigger = (button: HTMLElement) =>
  button.closest('[data-slot="tooltip-trigger"]');

describe("WorkspaceAddButton", () => {
  it('wraps the "+" button in a Tip', () => {
    render(
      <WorkspaceAddButton label="New session in Test D" onClick={vi.fn()} />,
    );

    const button = screen.getByRole("button", {
      name: "New session in Test D",
    });
  ...
  it("still fires onClick", () => {
    const onClick = vi.fn();
    render(
      <WorkspaceAddButton label="New session in Test D" onClick={onClick} />,
    );

    fireEvent.click(
      screen.getByRole("button", { name: "New session in Test D" }),
    );
...
describe("WorkspaceShowMoreButton", () => {
  it("wraps the ellipsis button in a Tip with the composed label", () => {
    render(
      <WorkspaceShowMoreButton count={5} label="Test D" onClick={vi.fn()} />,
    );

    const button = screen.getByRole("button", {
      name: "Show 5 more in Test D",
    });
    expect(tipTrigger(button)).toBeTruthy();
  });
});
