...
const lane = (
  over: Partial<SidebarSessionGroup> &
    Pick<SidebarSessionGroup, "id" | "label">,
): SidebarSessionGroup => ({
  path: null,
  sessions: [],
  ...over,
...
describe("kanbanWorktreeDir", () => {
  it("matches a kanban task worktree (t_<hex>) and returns its .worktrees dir", () => {
    expect(kanbanWorktreeDir("/repo/.worktrees/t_aaaaaaaa")).toBe(
      "/repo/.worktrees",
    );
  });
...
describe("sortWorktreeGroups", () => {
  it("pins trunk to the top, sinks kanban to the bottom, and orders the rest by recency", () => {
    ...
    expect(sortWorktreeGroups(groups).map((g) => g.label)).toEqual([
      "main",
      "busy-worktree",
      "stale-branch",
      "kanban",
  ...
  it("pins the live home checkout above trunk, even when it has no sessions yet", () => {
    const groups = [
      lane({
        ...
        label: "main",
        isMain: true,
        sessions: [makeCwdSession("/x", { last_active: 999 })],
      }),
      lane({
        id: "home",
        label: "bb/projects-paradigm",
        isMain: true,
        isHome: true,
      }),
    ];
...
describe("mergeRepoWorktreeGroups (visual enhancer)", () => {
  ...
  it("never spawns a lane per kanban task worktree", () => {
    ...
    const discovered: HermesGitWorktree[] = [
      {
        branch: "wt/t_aaaaaaaa",
        detached: false,
        isMain: false,
        locked: false,
        path: "/repo/.worktrees/t_aaaaaaaa",
      },
      {
        branch: "wt/t_bbbbbbbb",
        detached: false,
        isMain: false,
        locked: false,
        path: "/repo/.worktrees/t_bbbbbbbb",
      },
    ];

    expect(
      mergeRepoWorktreeGroups(repo, discovered).map((g) => g.label),
    ).toEqual(["main"]);
  });
    ...
    const discovered: HermesGitWorktree[] = [
      {
        ...
        isMain: false,
        locked: false,
        path: "/repo/.worktrees/main-mirror",
      },
    ];
  ...
  it("re-anchors a lane whose path drifted from git truth back to its branch path", () => {
    ...
    expect(merged.some((g) => g.path === "/repo/.worktrees/attempts")).toBe(
      false,
    );
  });
  ...
  it("keeps the dir label for a detached-HEAD worktree (no branch to show)", () => {
    ...
    expect(
      mergeRepoWorktreeGroups(repo, discovered).find((g) => g.id === "/repo-ci")
        ?.label,
    ).toBe("repo-ci");
  });
  ...
  it('labels the home lane "main" (still home-flagged) when the root is on trunk', () => {
    ...
    const home = mergeRepoWorktreeGroups(repo, discovered).find(
      (g) => g.isHome,
    );
...
const projectNode = (
  over: Partial<SidebarProjectTree> & Pick<SidebarProjectTree, "id">,
): SidebarProjectTree => ({
  label: over.id,
  path: over.id,
  repos: [],
...
describe("liveSessionProjectId", () => {
  it("maps a brand-new (unpersisted) session to its auto project (the repo root)", () => {
    expect(liveSessionProjectId(makeCwdSession("/www/app"), [])).toBe(
      "/www/app",
    );
  });
  ...
  it("anchors a cwd-less session on its git_repo_root (backend groups it there too)", () => {
    ...
    expect(
      liveSessionProjectId(
        makeCwdSession(null, { git_repo_root: "/www/app" }),
        [],
      ),
    ).toBe("/www/app");
    expect(
  ...
  it("skips cwd-less, kanban-task, and out-of-tree (sibling) worktree sessions", () => {
    ...
    expect(
      liveSessionProjectId(makeCwdSession("/repo/.worktrees/t_aaaaaaaa"), []),
    ).toBeNull();
    // Sibling worktree OUTSIDE the repo root → project can't be derived from the row.
    expect(
      liveSessionProjectId(
        makeCwdSession("/elsewhere/wt", { git_repo_root: "/repo" }),
        [],
      ),
    ).toBeNull();
  });
  ...
  it("places an in-tree worktree session under its repo project (the root is in the path)", () => {
    ...
    expect(
      liveSessionProjectId(
        makeCwdSession("/www/app/.worktrees/test1", {
          git_repo_root: "/www/app",
        }),
        [],
      ),
  ...
  it("places a cwd-outside-root session under an explicit project matching either path", () => {
    ...
    expect(
      liveSessionProjectId(
        makeCwdSession("/www/elsewhere", { git_repo_root: "/home/u/proj" }),
    ...
    expect(
      liveSessionProjectId(
        makeCwdSession("/www/elsewhere/sub", { git_repo_root: "/home/u/proj" }),
  ...
  it("matches a mixed-case/separator Windows cwd to its explicit project in the live overlay", () => {
    ...
    const id = liveSessionProjectId(makeCwdSession("c:/work/notes/SUB"), [
      makeProject("p_notes", ["C:\\Work\\Notes"]),
    ]);
  ...
  it("matches a root-relative WSL cwd (single backslash) case-insensitively", () => {
    const id = liveSessionProjectId(
      makeCwdSession("//wsl.localhost/Ubuntu/home/alice/PROJ"),
  ...
  it("keeps POSIX cwd matching case-sensitive (no false project match)", () => {
    ...
    expect(
      liveSessionProjectId(makeCwdSession("/work/notes"), [
        makeProject("p_notes", ["/Work/Notes"]),
      ]),
    ).toBe("/work/notes");
  });
});
...
describe("sessionProjectColor", () => {
  ...
  it("inherits the color of the explicit project the session belongs to", () => {
    const session = makeCwdSession("/www/app/src", {
      git_repo_root: "/www/app",
    });

    expect(
      sessionProjectColor(session, [colored("p_app", ["/www/app"], "#4a9eff")]),
    ).toBe("#4a9eff");
  });
  ...
  it("returns null when the owning project has no color set", () => {
    ...
    expect(
      sessionProjectColor(session, [makeProject("p_app", ["/www/app"])]),
    ).toBeNull();
  });
  ...
  it("colors a cwd-outside-root session when an explicit project folder matches", () => {
    ...
    const session = makeCwdSession("/www/elsewhere", {
      git_repo_root: "/home/u/proj",
    });

    expect(
      sessionProjectColor(session, [
        colored("p_proj", ["/home/u/proj"], "#4a9eff"),
      ]),
    ).toBe("#4a9eff");
  });
  ...
  it("returns null for an unplaceable (cwd-less) session", () => {
    expect(
      sessionProjectColor(makeCwdSession(null), [
        colored("p_app", ["/www/app"], "#4a9eff"),
      ]),
    ).toBeNull();
  });

  it("uses the longest-prefix project when nested projects both match", () => {
    const session = makeCwdSession("/www/app/packages/api/src", {
      git_repo_root: "/www/app",
    });
...
describe("overlayLiveLanes", () => {
  it("keeps an overview preview visible when the hydrated drill-in is stale", () => {
    ...
    const overlaySessions = reconcileEnteredProjectSessions(
      [],
    ...
    expect(
      overlaid.repos[0].groups[0].sessions.map((session) => session.id),
    ).toEqual(["current-preview", "stale-history"]);
  });

  it("keeps the live row when it is also present in the overview preview", () => {
    const live = makeCwdSession("/www/app", {
      id: "same",
      title: "live title",
    });
    const preview = makeCwdSession("/www/app", {
      id: "same",
      title: "preview title",
    });
  ...
  it("injects a live session into the matching main lane instantly", () => {
    ...
    const live = [
      makeCwdSession("/www/app", { id: "fresh", git_branch: "main" }),
    ];
  ...
  it("keeps cwd-less repo sessions visible in both the overview and project drill-in", () => {
    ...
    const live = makeCwdSession(null, {
      id: "fresh",
      git_branch: "main",
      git_repo_root: "/www/app",
    });
    ...
    expect(
      overlayLivePreviews([project], [live], [], 3)["/www/app"].map(
        (session) => session.id,
      ),
    ).toEqual(["fresh"]);
    ...
    expect(
      overlaid.repos[0].groups.flatMap((group) =>
  ...
  it("places a cwd-less repo session only in its exact repo subtree", () => {
    ...
    expect(
      overlaid.repos[1].groups.flatMap((group) =>
        group.sessions.map((session) => session.id),
      ),
    ).toEqual(["fresh"]);
    expect(overlaid.sessionCount).toBe(1);
  ...
  it("injects a session created in a fresh worktree into that worktree lane (no git_repo_root yet)", () => {
    ...
    const overlaid = overlayLiveLanes(project, live);
    const lane = overlaid.repos[0].groups.find(
      (g) => g.id === "/www/app/.worktrees/baby",
    );
  ...
  it("folds a kanban-task worktree session into the kanban lane", () => {
    ...
    const live = [
      makeCwdSession("/www/app/.worktrees/t_abc12345", { id: "k" }),
    ];
  ...
  it("does not duplicate a session already present in a backend lane", () => {
    const existing = makeCwdSession("/www/app", {
      id: "dup",
      git_branch: "main",
    });
    ...
    expect(
      overlaid.repos[0].groups.flatMap((g) => g.sessions.map((s) => s.id)),
    ).toEqual(["dup"]);
  });
  ...
  it("does not fork a phantom main lane for a non-git backend workspace lane", () => {
    ...
    expect(
      groups.some((g) => g.label === "main" || g.id.endsWith("::branch::main")),
    ).toBe(false);
  });
  ...
  it("preserves backend recency order when live sessions overlay a lane", () => {
    ...
    expect(
      overlaid.repos[0].groups[0].sessions.map((session) => session.id),
    ).toEqual(["recently-active", "newly-created"]);
  });
  ...
  it("adds a new session to an existing worktree lane keyed by a divergent id (matches by path)", () => {
    ...
    const overlaid = overlayLiveLanes(project, [existing, fresh]);
    const lanes = overlaid.repos[0].groups.filter(
      (g) => g.path === "/www/app/.worktrees/baby",
    );
  ...
  it("places into a visual-only discovered worktree lane after merge", () => {
    ...
    const groups = mergeRepoWorktreeGroups(
      { id: "/www/app", path: "/www/app", groups: [] },
      discovered,
    );
  ...
  it("evicts a deleted/archived snapshot row (and drops the lane once empty)", () => {
    ...
    expect(overlaid.repos[0].groups.map((g) => g.id)).toEqual([
      "/www/app::branch::main",
    ]);
    expect(overlaid.repos[0].groups[0].sessions.map((s) => s.id)).toEqual([
      "keep",
    ]);
    expect(overlaid.sessionCount).toBe(1);
  });
  ...
  it("adds a brand-new detached chat to Home, and evicts a deleted one", () => {
    ...
    const overlaid = overlayLiveLanes(
      home,
      [makeCwdSession(null, { id: "fresh", started_at: 9 })],
      new Set(["gone"]),
    );
  ...
  it("leaves Home alone for a session that has a cwd", () => {
    ...
    expect(
      overlayLiveLanes(home, [makeCwdSession("/www/app", { id: "fresh" })]),
    ).toBe(home);
  });
  ...
  it("evicts a session from the main lane when the live overlay places it into a worktree lane", () => {
    ...
    const session = makeCwdSession("/www/app/.worktrees/feature", {
      id: "moved",
      git_branch: "feature",
    });
    ...
    const featureLane = overlaid.repos[0].groups.find(
      (g) => g.path === "/www/app/.worktrees/feature",
    );
...
describe("overlayLivePreviews", () => {
  ...
  it("ranks by the active sort key before trimming, so the preview is its top rows", () => {
    ...
    const previews = overlayLivePreviews([project], [], [], 2, {
      rankIds: ["priciest", "newest", "cheap"],
    });

    expect(previews["/www/app"].map((s) => s.id)).toEqual([
      "priciest",
      "newest",
    ]);
  });
...
describe("excludeProjectSessions", () => {
  it("drops matching rows from every lane and recounts the subtree", () => {
    ...
    expect(filtered.repos[0].groups[0].sessions.map((s) => s.id)).toEqual([
      "keep",
    ]);
      ...
      repos: [
        {
          ...
          groups: [
            lane({
              ...
              isMain: true,
              label: "main",
              sessions: [makeCwdSession("/www/app", { id: "keep" })],
            }),
          ],
        },
      ],
