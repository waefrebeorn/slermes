    ...
    t: {
      sidebar: {
        ...
        projects: {
          ...
          toggle: (label: string, open: boolean) =>
            `${open ? "Show" : "Hide"} ${label} sessions`,
        },
      },
    },
  }),
...
describe("ProjectOverviewRow", () => {
  ...
  it("does not render the disclosure toggle when there is nothing to preview", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Show Test D sessions" }),
    ).toBeNull();
  });
  ...
  it('offers the "new session" add button on Home, which starts one with no folder', () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "New session in Home" }),
    );
  ...
  it("tags the row with data-sessions-project so a skin can target one project", () => {
    ...
    expect(
      container.querySelector('[data-sessions-project="p1"]'),
    ).toBeTruthy();
  });
});
