...
const ids = (projects: SidebarProjectTree[]) =>
...
describe("orderProjectsByIds", () => {
  ...
  it("applies the saved manual order", () => {
    ...
    expect(ids(orderProjectsByIds(projects, ["c", "a", "b"]))).toEqual([
      "c",
      "a",
      "b",
    ]);
  ...
  it("keeps freshly-scanned zero-session repos below the hand-ordered list", () => {
    ...
    const projects = [
      makeProject("scanned-1", 0),
      makeProject("mine", 4),
      makeProject("scanned-2", 0),
    ];

    expect(ids(orderProjectsByIds(projects, ["mine"]))).toEqual([
      "mine",
      "scanned-1",
      "scanned-2",
    ]);
  ...
  it("still surfaces a new project that has real activity", () => {
    ...
    const projects = [
      makeProject("ordered", 1),
      makeProject("just-started", 3),
    ];

    expect(ids(orderProjectsByIds(projects, ["ordered"]))).toEqual([
      "just-started",
      "ordered",
    ]);
  });
  ...
  it("keeps Home on top of a hand-picked order", () => {
    ...
    expect(ids(orderProjectsByIds(projects, ["b", "a"]))).toEqual([
      NO_PROJECT_ID,
      "b",
      "a",
    ]);
...
describe("sortProjectsForOverview", () => {
  it("puts Home above the active project", () => {
    ...
    expect(ids(sortProjectsForOverview(projects, "active"))).toEqual([
      NO_PROJECT_ID,
      "active",
      "scanned",
    ]);
