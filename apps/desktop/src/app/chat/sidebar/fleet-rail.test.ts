...
describe("buildRestGroups", () => {
  it("lists every gateway except the active one, in switcher order, regardless of which is active", () => {
    const fromPandora = buildRestGroups({
      activeConnectionId: "pandora",
      connections,
      roster,
    });
    const fromLocal = buildRestGroups({
    ...
    expect(fromPandora.map((group) => group.connectionId)).toEqual([
      "local",
      "vps",
    ]);
    expect(fromLocal.map((group) => group.connectionId)).toEqual([
      "pandora",
      "vps",
    ]);
  });
  ...
  it("carries each gateway default as its own square plus named profiles alphabetically", () => {
    ...
    const [pandora] = buildRestGroups({
      activeConnectionId: "local",
      connections,
      roster,
    });
    expect(pandora.named.map((agent) => agent.profile)).toEqual([
      "omer",
      "scout",
    ]);
  });
