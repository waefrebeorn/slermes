...
export function SidebarSectionMeta({
  children,
}: {
...
export function SidebarRowStack({
  className,
  ...props
}: React.ComponentProps<"div">) {
  return (
    <div
      className={cn("grid grid-cols-[minmax(0,1fr)] gap-px", className)}
      {...props}
    />
  );
}
...
export function SidebarRowNest({
  className,
  ...props
}: React.ComponentProps<"div">) {
...
export function SidebarDateDivider({
  ...
  const rule = (
    <span
      aria-hidden="true"
      className="h-px min-w-4 flex-1 bg-(--ui-stroke-tertiary)"
    />
  );
  ...
  return (
    ...
    <div
      className={cn(
        "group/workspace flex select-none items-center gap-2 px-2 pb-0.5 pt-2",
        className,
      )}
      {...props}
    >
...
export function SidebarRowCluster({
  className,
  ...props
}: React.ComponentProps<"div">) {
...
export function SidebarRowLink({
  ...
  return (
    <RowButton
      className={cn("min-w-0 shrink bg-transparent p-0 text-left", className)}
      {...props}
    >
...
export function SidebarGroupRow({
  ...
  const facts = [
    totals && rowMeta.includes("tokens") && totals.tokens > 0
    ...
    totals && rowMeta.includes("cost") && totals.costUsd >= 0.01
