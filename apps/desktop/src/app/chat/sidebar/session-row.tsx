...
const TAIL_HIDES =
  "min-w-5 transition-opacity group-hover:opacity-0 group-has-[[data-pr-link]:hover]:opacity-100";
const KEBAB_YIELDS =
  "group-has-[[data-pr-link]:hover]:pointer-events-none group-has-[[data-pr-link]:hover]:opacity-0";
...
function SidebarSessionRowImpl({
  ...
  const pr = useStoreSelector($pullRequestsByBranch, (prs) =>
    rowMeta.includes("pr") && prKey ? prs[prKey] : undefined,
  );
  ...
  const openUnfocused = useStoreSelector(
    ...
    (open) => !isSelected && open.has(session.id),
  );
  ...
  const figures = [
    rowMeta.includes("tokens") && totalTokens > 0
  ...
  if ((showProfile || pinnedProfile) && hasProfileTag) {
    trailing.push({
      key: "profile",
      node: <ProfileTag profile={session.profile} />,
    });
  }
  ...
  const handoffSource = handoffOriginSource(
    session.handoff_state,
    session.handoff_platform,
  );
  const handoffLabel = handoffSource
    ? (sessionSourceLabel(handoffSource) ?? handoffSource)
    : null;
  ...
  const context = useStoreSelector($projects, (projects) =>
    card
      ? (sessionProjectLabel(session, projects) ??
        (pathLeaf(session.cwd) || t.sidebar.projects.home))
      : null,
  );
  ...
  const size =
    card && session.message_count > 0
      ? r.messageCount(session.message_count)
      : "";
  ...
  const todoProgress = useStoreSelector($todoProgressBySession, (progress) =>
    card ? progress[session.id] : undefined,
  );
  ...
  const actionsNode = (
    ...
    >
      {trailing.map(({ key, node }, index) => (
        <span
          className={
            chipEndsSlot && index === trailing.length - 1
              ? cn("inline-flex justify-end", TAIL_HIDES)
              : undefined
          }
          key={key}
        >
    ...
    >
      <SidebarRowShell
        ...
        className={cn(
          ...
          dragging &&
            "z-10 cursor-grabbing bg-(--ui-sidebar-surface-background)",
          className,
        )}
      ...
      >
        ...
        <SidebarRowBody
          ...
          className={cn(
            ...
            card &&
              "flex-col items-stretch justify-center py-1.5 [--card-gap:0.4rem] gap-(--card-gap)",
          )}
...
function rowPropsEqual(
  a: SidebarSessionRowProps,
  b: SidebarSessionRowProps,
): boolean {
  return (
    a.session === b.session &&
