...
describe("resolveSessionRowClick", () => {
  ...
  it("pins on ⇧-click", () => {
    expect(
      resolveSessionRowClick({ ...NO_MODS, shiftKey: true }, WINDOW_OK),
    ).toBe("pin");
  });

  it("opens a new tab on ⌘/⌃-click", () => {
    expect(
      resolveSessionRowClick({ ...NO_MODS, metaKey: true }, WINDOW_OK),
    ).toBe("newTab");
    expect(
      resolveSessionRowClick({ ...NO_MODS, ctrlKey: true }, WINDOW_OK),
    ).toBe("newTab");
    expect(
      resolveSessionRowClick({ ...NO_MODS, ctrlKey: true }, NO_WINDOW),
    ).toBe("newTab");
  });

  it("opens a new window on ⌘/⌃+⇧-click when supported", () => {
    expect(
      resolveSessionRowClick(
        { ...NO_MODS, metaKey: true, shiftKey: true },
        WINDOW_OK,
      ),
    ).toBe("newWindow");
    expect(
      resolveSessionRowClick(
        { ...NO_MODS, ctrlKey: true, shiftKey: true },
        WINDOW_OK,
      ),
    ).toBe("newWindow");
  });
  ...
  it("archives on ⌥+⇧-click", () => {
    expect(
      resolveSessionRowClick(
        { ...NO_MODS, altKey: true, shiftKey: true },
        WINDOW_OK,
      ),
    ).toBe("archive");
  });
  ...
  it("does not archive on ⌥-click alone", () => {
    expect(
      resolveSessionRowClick({ ...NO_MODS, altKey: true }, WINDOW_OK),
    ).toBe("resume");
  });
});
