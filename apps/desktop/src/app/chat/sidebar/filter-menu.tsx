...
const STATUS_FILTERS: Option<SessionStatusBucket>[] = [
  {
    dot: sessionDotClassName("needs-input"),
    id: "needs-input",
    label: "Needs input",
  },
...
export function SidebarFilterMenu({ className }: { className?: string }) {
  ...
  const profileNames = useStore($profiles).map((profile) =>
  ...
  const foldCollapsed =
    foldIds.length > 0 && foldIds.every((id) => nodeOpen[id] === false);

  const groupingLabel = GROUPINGS.find(
    (option) => option.id === grouping,
  )?.label;
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="min-w-52">
        <DropdownMenuGroup>
          <DropdownMenuSub>
            ...
            <DropdownMenuSubContent>
              <DropdownMenuRadioGroup
                onValueChange={(value) =>
                  setSidebarGrouping(value as SidebarGrouping)
                }
                value={grouping}
              >
          ...
          <DropdownMenuSub>
            ...
            <DropdownMenuSubContent>
              <DropdownMenuRadioGroup
                onValueChange={(value) =>
                  setSidebarOrdering(value as SidebarOrdering)
                }
                value={ordering}
              >
        ...
        <DropdownMenuGroup>
          ...
          {viewCustomized && (
            <DropdownMenuItem onSelect={resetSidebarView}>
              Reset to defaults
            </DropdownMenuItem>
          )}
        </DropdownMenuGroup>
        ...
        {foldIds.length > 0 && (
          <DropdownMenuItem
            onSelect={() => setWorkspaceNodesOpen(foldIds, foldCollapsed)}
          >
