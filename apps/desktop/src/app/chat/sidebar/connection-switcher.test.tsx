...
vi.mock("@/i18n", () => ({
  useI18n: () => ({
    t: {
      profiles: {
        switchConnectionFailed: (name: string) =>
...
const $activeConnectionId = connectionStore.$activeConnectionId as ReturnType<
  typeof atom<null | string>
>;
...
const initializeConnectionsRegistry = vi.mocked(
  connectionStore.initializeConnectionsRegistry,
);
const refreshConnectionsRegistry = vi.mocked(
  connectionStore.refreshConnectionsRegistry,
);
...
describe("ConnectionSwitcher", () => {
  it("waits for primary boot fetches before restoring the launch source", async () => {
    ...
    await waitFor(() =>
      expect(initializeConnectionsRegistry).toHaveBeenCalledTimes(1),
    );
  });
  ...
  it("adds no source chrome for a local-only setup", () => {
    $connectionsRegistry.set(
      registry([connection("local", "This device", "local")]),
    );
    render(<ConnectionSwitcher onConnect={onConnect} />);

    expect(
      screen.queryByRole("group", { name: "Registered gateways" }),
    ).toBeNull();
  });
  ...
  it("keeps source controls stable while a remote is opening", () => {
    ...
    expect(
      screen
        .getByRole("group", { name: "Registered gateways" })
        .getAttribute("aria-busy"),
    ).toBe("true");
    expect(
      screen
        .getByRole("button", { name: "Registered gateways: This device" })
        .querySelector(".animate-spin"),
    ).toBeTruthy();
  });

  it.each([2, 20])(
    "uses the same stable source selector for %i registered backends",
    (count) => {
      $connectionsRegistry.set(
        registry([
          connection("local", "This device", "local"),
          ...Array.from({ length: count - 1 }, (_, index) =>
            connection(`remote-${index}`, `Remote ${index}`),
          ),
        ]),
      );
      render(<ConnectionSwitcher onConnect={onConnect} />);
  ...
  it("keeps small gateway lists simple and naturally sorted", () => {
    ...
    ).toEqual(["This device", "alpha", "Studio 2", "Studio 10", "Zulu"]);
  });
  ...
  it("adds search at eight gateways and filters stable results without moving the connect action", async () => {
    ...
    expect(
      screen.getByRole("menuitem", { name: "Manage gateways…" }),
    ).toBeTruthy();
    expect(
      screen.getAllByRole("menuitemradio").map((item) => item.textContent),
    ).toEqual([
    ...
    fireEvent.pointerDown(trigger, { button: 0, pointerType: "mouse" });
    expect(
      (screen.getByPlaceholderText("Search gateways…") as HTMLInputElement)
        .value,
    ).toBe("");
  ...
  it("explains an empty large-list search", () => {
    ...
    fireEvent.pointerDown(
      screen.getByRole("button", { name: "Registered gateways: This device" }),
      {
        button: 0,
        pointerType: "mouse",
      },
    );
    fireEvent.change(screen.getByPlaceholderText("Search gateways…"), {
  ...
  it("repaints the menu after a connections.save without reload (#95393)", async () => {
    ...
    let onChangedCallback:
      ((payload: { connectionId: string; reason: string }) => void) | null =
      null;
    ...
    try {
      ...
      fireEvent.pointerDown(trigger, { button: 0, pointerType: "mouse" });
      expect(
        screen.queryByRole("menuitemradio", { name: "W2Probe" }),
      ).toBeNull();
      ...
      (
        onChangedCallback as unknown as (payload: {
          connectionId: string;
          reason: string;
        }) => void
      )({
      ...
      await waitFor(() =>
        expect(refreshConnectionsRegistry).toHaveBeenCalledTimes(2),
      );
      ...
      expect(
        screen.getByRole("menuitemradio", { name: "W2Probe" }),
      ).toBeTruthy();
    } finally {
