...
describe("resolvePinnedSessions", () => {
  it("resolves local pin ids in their hand-picked order", () => {
    ...
    expect(
      resolvePinnedSessions(["c", "a"], index, sessions, settled).map(
        (s) => s.id,
      ),
    ).toEqual(["c", "a"]);
  });
  ...
  it("falls back to the server pinned flag when localStorage is cold (#85969)", () => {
    ...
    expect(
      resolvePinnedSessions([], index, sessions, settled).map((s) => s.id),
    ).toEqual(["a"]);
  });
  ...
  it("does not duplicate a session held both locally and server-side", () => {
    ...
    expect(
      resolvePinnedSessions(["a"], index, sessions, settled).map((s) => s.id),
    ).toEqual(["a"]);
  });
  ...
  it("does not duplicate a server-pinned row whose pin is stored on the lineage root", () => {
    ...
    expect(
      resolvePinnedSessions(["root"], index, sessions, settled).map(
        (s) => s.id,
      ),
    ).toEqual(["tip"]);
  });
  ...
  it("keeps locally pinned rows ahead of server-only fallback pins", () => {
    ...
    expect(
      resolvePinnedSessions(["local-pin"], index, sessions, settled).map(
        (s) => s.id,
      ),
    ).toEqual(["local-pin", "server-pin"]);
  });
  ...
  it("does not resurrect a session the user just unpinned", () => {
    ...
    expect(
      resolvePinnedSessions([], index, sessions, new Set(["just-unpinned"])),
    ).toEqual([]);
  });
  ...
  it("fences a stale row under the lineage root the pin was written on", () => {
    ...
    expect(
      resolvePinnedSessions([], index, sessions, new Set(["root"])),
    ).toEqual([]);
  });
  ...
  it("still adopts a foreign pin while an unrelated write is in flight", () => {
    ...
    expect(
      resolvePinnedSessions([], index, sessions, new Set(["other"])).map(
        (s) => s.id,
      ),
    ).toEqual(["foreign"]);
  });
  ...
  it("does not reshuffle Show-all pins that already live in the local set", () => {
    ...
    expect(
      resolvePinnedSessions(["foreign", "local"], index, sessions, settled).map(
        (s) => s.id,
      ),
    ).toEqual(["foreign", "local"]);
    ...
    expect(
      resolvePinnedSessions(["foreign", "local"], index, clicked, settled).map(
        (s) => s.id,
      ),
    ).toEqual(["foreign", "local"]);
  });
