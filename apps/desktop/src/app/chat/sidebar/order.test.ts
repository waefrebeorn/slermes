...
describe("orderByIds", () => {
  ...
  it("reorders by the given ids and drops missing ones", () => {
    ...
    expect(orderByIds(items, id, ["c", "gone", "a"])).toEqual([
      { id: "c" },
      { id: "a" },
      { id: "b" },
    ]);
  ...
  it("surfaces items absent from the order first", () => {
    ...
    expect(orderByIds(items, id, ["b", "a"])).toEqual([
      { id: "fresh" },
      { id: "b" },
      { id: "a" },
    ]);
  ...
  it("never duplicates an item when the persisted order repeats its id", () => {
    const items = [{ id: "a" }, { id: "b" }];
    expect(orderByIds(items, id, ["a", "a", "b"])).toEqual([
      { id: "a" },
      { id: "b" },
    ]);
  });
...
describe("rankSessions", () => {
  ...
  it("applies the active sort key to a lane the flat list never renders", () => {
    expect(
      rankSessions(sessions, ["oldest", "newest", "middle"]).map((s) => s.id),
...
describe("reconcileOrderIds", () => {
  ...
  it("puts newly-seen ids ahead of the retained saved order", () => {
    expect(reconcileOrderIds(["fresh", "a", "b"], ["b", "a", "gone"])).toEqual([
      "fresh",
      "b",
      "a",
    ]);
...
const ids = (rows: SidebarListRow[]): string[] =>
  rows.map((row) =>
    row.kind === "divider"
      ? `--${row.key}--`
      : `${row.entry.branchStem ?? ""}${row.entry.session.id}`,
  );
...
describe("orderRowsWithinGroups", () => {
  ...
  it("leaves rows the order does not name in their recency slot", () => {
    ...
    expect(ids(orderRowsWithinGroups(rows, ["c", "a"]))).toEqual([
      "c",
      "b",
      "a",
    ]);
  ...
  it("carries branch children with their parent", () => {
    ...
    expect(ids(orderRowsWithinGroups(rows, ["other", "parent"]))).toEqual([
      "other",
      "parent",
      "└─ child",
    ]);
...
describe("mergeVisibleReorder", () => {
  it("returns the visible order when nothing is hidden", () => {
    expect(mergeVisibleReorder(["a", "b", "c"], ["c", "a", "b"])).toEqual([
      "c",
      "a",
      "b",
    ]);
  ...
  it("keeps hidden ids in their original slots", () => {
    ...
    expect(mergeVisibleReorder(["a", "b", "c", "d"], ["b", "a"])).toEqual([
      "b",
      "a",
      "c",
      "d",
