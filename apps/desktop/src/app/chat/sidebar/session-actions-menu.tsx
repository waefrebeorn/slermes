...
export async function renameSessionPreferringRpc(
  ...
  if (title && runtimeId && gateway) {
    try {
      const result = await gateway.request<SessionTitleResponse>(
        "session.title",
        {
      ...
      console.warn(
        "session.title RPC rename failed; falling back to REST",
        err,
      );
    }
  }
...
function SessionColorSwatches({ sessionId }: { sessionId: string }) {
  ...
  const session = useStore($sessions).find((s) =>
...
function MoveToProjectItems({
  ...
  const targets = tree.filter(
    (node) =>
      node.id !== currentProjectId && !node.isNoProject && projectRootCwd(node),
  );
...
function useSessionActions({
  ...
  const alreadyTabbed =
    ...
    tiles.some((tile) => tile.storedSessionId === sessionId);

  const spec = (
    partial: Omit<ActionItemSpec, "onSelect"> & { onSelect: () => void },
  ): ActionItemSpec => partial;
  ...
  const closeTargets =
    surface === "tab" && tabPaneId ? treeTabCloseTargets(tabPaneId) : null;
...
interface SessionActionsMenuProps
  extends
    SessionActions,
    Pick<React.ComponentProps<typeof ActionsMenu>, "align" | "sideOffset"> {
...
export function SessionActionsMenu({
  ...
  const { t } = useI18n();
  const { deleteDialog, onCloseAutoFocus, renameDialog, renderItems } =
