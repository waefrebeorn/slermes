...
async function openDialog(profile = "work") {
  ...
  await waitFor(() =>
    expect(getConnectionConfig).toHaveBeenCalledWith(profile),
  );
...
describe("ProfileRemoteOverrideDialog", () => {
  it("walks the first-time connect through the risk confirmation before writing", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Remote address/), {
      target: { value: "https://box.example.com" },
    });
    ...
    expect(
      screen.getByText("Connect this profile to a remote host?"),
    ).toBeTruthy();
  ...
  it("offers removal for an overridden profile and clears via mode local", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Remove remote connection" }),
    );
  ...
  it("requires the explicit unencrypted-token opt-in on keyring-less machines", async () => {
    ...
    await waitFor(() =>
      expect(applyConnectionConfig).toHaveBeenCalledWith(
        expect.objectContaining({ allowPlainTextToken: true }),
      ),
    );
  });
  ...
  it("warns when the profile name collides with a registered gateway", async () => {
    ...
    await screen.findByText(
      /A gateway named “Work” already exists in Settings/,
    );
  });
  ...
  it("keeps the dialog open and shows the error when the write fails", async () => {
    ...
    applyConnectionConfig.mockRejectedValue(
      new Error("Remote gateway session token is required."),
    );
    await openDialog();
