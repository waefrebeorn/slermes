...
declare global {
  interface Window {
    __PERF_PROBE__?: {
      ...
      summary: () => Record<
        string,
        { count: number; total: number; max: number; p50: number; p95: number }
      >;
    };
    __PERF_DRIVE__?: {
      ...
      rightPaneSetup: (opts: { cwd: string; terminals?: number }) => {
        procId: string;
        terminalIds: string[];
      };
      rightPaneGit: (
        path: string,
        kind?: "added" | "conflicted" | "modified",
      ) => void;
      rightPaneReset: () => void;
...
const onRender: ProfilerOnRenderCallback = (
  ...
  const probe =
    typeof window !== "undefined" ? window.__PERF_PROBE__ : undefined;
  ...
  const syntheticTurn = (i: number): ReturnType<typeof $messages.get> => {
    const user = {
      ...
      parts: [
        {
          type: "text" as const,
          text: `Question ${i}: how does the widget in module ${i} handle back-pressure?`,
        },
      ],
      timestamp: Date.now(),
    };
