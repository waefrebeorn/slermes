...
export function registerPreviewNav(
  tabId: string,
  handle: PreviewNavHandle,
): () => void {
...
export function commandFocusedPreview(
  command: keyof PreviewNavHandle,
): boolean {
  const host = document.activeElement?.closest(`[${PREVIEW_BROWSER_ATTR}]`);
  const nav = host
    ? handles.get(host.getAttribute(PREVIEW_BROWSER_ATTR) || "")
    : undefined;
