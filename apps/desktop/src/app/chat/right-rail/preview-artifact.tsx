...
export function ArtifactPreview({ target }: { target: PreviewTarget }) {
  ...
  const record = useMemo(
    () => findArtifact(registry, artifactId),
    [artifactId, registry],
  );

  if (!record) {
    return (
      <PreviewEmptyState body={copy.missingBody} title={copy.missingTitle} />
    );
  }

  const renderable = record.kind === "html" || record.kind === "svg";
  const modes: PreviewViewMode[] = renderable
    ? ["rendered", "source"]
    : ["source"];
  const mode = userMode && modes.includes(userMode) ? userMode : modes[0]!;
  const versionIndex = Math.min(
    versionSelection[artifactId] ?? record.versions.length - 1,
                ...
                <button
                  ...
                  onClick={() =>
                    void openHtmlInBrowser(version.content).catch((error) =>
                      notifyError(error, copy.openInBrowserFailed),
                    )
                  }
                  type="button"
                >
