...
function stubPdfObjectUrls() {
  ...
  const createObjectURL = vi.fn(
    (_blob: Blob) => `blob:pdf-preview-${(objectUrlIndex += 1)}`,
  );
...
describe("PreviewPane console state", () => {
  ...
  it("does not watch backend-only remote filesystem previews locally", async () => {
    const watchPreviewFile = vi.fn(async () => ({
      id: "watch-1",
  ...
  it("streams console logs into the tab-keyed console store", async () => {
    ...
    expect(previewConsoleState(tabId).$logs.get().at(-1)?.message).toBe(
      "streamed log line",
    );
  ...
  it("drives the webview from the bar and tracks its history", async () => {
    ...
    const back = rendered.getByRole("button", {
      name: "Back",
    ...
    const address = rendered.getByRole("textbox", {
      name: "Address",
    }) as HTMLInputElement;
  ...
  it("continues comment numbering in one conversation and resets it when the conversation changes", async () => {
    ...
    fireEvent.click(
      await rendered.findByRole("button", { name: "Add 1 comment" }),
    );
    ...
    expect(
      await rendered.findByRole("form", { name: "Comment 2" }),
    ).toBeTruthy();
    ...
    await waitFor(() =>
      expect(rendered.queryByRole("form", { name: "Comment 2" })).toBeNull(),
    );
    expect(
      rendered.queryByRole("button", { name: "Add 1 comment" }),
    ).toBeNull();
  ...
  it("stays quiet for a non-loopback host on a remote gateway", async () => {
    ...
    await waitFor(() =>
      expect(rendered.container.textContent).toContain("ERR_NAME_NOT_RESOLVED"),
    );
    expect(rendered.container.textContent).not.toContain(
      "machine running your agent",
    );
  ...
  it("accepts case-insensitive metadata and percent-escaped base64", async () => {
    const readFileDataUrl = vi.fn(
      async () => "data:APPLICATION/PDF;BASE64,%4AVBERi0xLjQ=",
    );
  ...
  it.each([
    [
      "a non-PDF MIME type",
      "data:text/html;base64,JVBERi0xLjQ=",
      "Invalid PDF data URL type",
    ],
    [
      "bytes without a PDF header",
      "data:application/pdf;base64,PGh0bWw+",
    ...
    "rejects %s before creating an object URL",
    async (_case, dataUrl, expectedError) => {
      const readFileDataUrl = vi.fn(async () => dataUrl);
      const { createObjectURL } = stubPdfObjectUrls();
      $connection.set({ mode: "local" } as never);
      vi.stubGlobal("window", {
      ...
      await waitFor(
        () => expect(rendered.container.textContent).toContain(expectedError),
        {
          container: rendered.container,
        },
      );
