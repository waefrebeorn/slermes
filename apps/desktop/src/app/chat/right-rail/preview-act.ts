...
const NAV_ACTIONS: readonly (keyof PreviewNavHandle)[] = [
  "back",
  "forward",
  "reload",
];
...
const NOTHING_OPEN =
  "No live page is open in the in-app browser — open one with open_preview first.";
...
type Trip =
  ...
  | { kind: "answered"; result: PreviewActResult }
  | { kind: "silent" };
...
async function runJson(run: PreviewScriptRunner, code: string): Promise<Trip> {
  ...
  if (raw instanceof Error) {
    return {
      error: "The page rejected the action: " + raw.message,
      kind: "failed",
    };
  }
...
async function driveAction(
  ...
  const trip = await runJson(
    run,
    buildLocateScript(action, action.kind === "press"),
  );
  ...
  if (!found.point) {
    return {
      error: "Could not work out where that element is on screen.",
      success: false,
    };
  }
  ...
  if (!hit && CLICKS.indexOf(action.kind) !== -1) {
    return {
      ...result,
      error:
        "The pointer input never reached the page, so nothing was " +
        acted.split(" ")[0] +
        ".",
      success: false,
    };
...
async function driveScroll(
  ...
  const anchor = trip.result as PreviewActResult & {
    page?: number;
    span?: number;
  };
  ...
  if (!anchor.span) {
    return {
      ...
      note: "The page has nothing to scroll — it all fits already.",
    };
  }
...
export async function actOnActivePreview(
  ...
  if (nav) {
    ...
    return {
      acted: nav,
      note: "Page is loading — call elements to see what is on it.",
      success: true,
    };
  }
  ...
  if (
    ...
    return trip.kind === "answered"
      ? trip.result
      : { acted: typed.kind, note: NAVIGATED, success: true };
  }
  ...
  const plain =
    typed.kind === "scroll" && !typed.to && !typed.ref && !typed.selector;
