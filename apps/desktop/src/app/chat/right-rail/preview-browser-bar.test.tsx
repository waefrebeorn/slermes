...
describe("normalizePreviewAddress", () => {
  it("passes through http and https URLs", () => {
    expect(normalizePreviewAddress("https://example.com/path")).toBe(
      "https://example.com/path",
    );
    expect(normalizePreviewAddress("http://localhost:3000/")).toBe(
      "http://localhost:3000/",
    );
  });
  ...
  it("gives a bare host https and a loopback host http", () => {
    ...
    expect(normalizePreviewAddress("localhost:5173")).toBe(
      "http://localhost:5173",
    );
    expect(normalizePreviewAddress("127.0.0.1:8080/app")).toBe(
  ...
  it("passes through the other schemes a browser can navigate", () => {
    ...
    expect(normalizePreviewAddress("file:///etc/hosts")).toBe(
      "file:///etc/hosts",
    );
    expect(normalizePreviewAddress("view-source:https://example.com")).toBe(
  ...
  it("trims whitespace before validating", () => {
    expect(normalizePreviewAddress("  https://example.com  ")).toBe(
      "https://example.com",
    );
  });
});
...
describe("PreviewBrowserBar", () => {
  it("renders the navigation controls and the page toggles", () => {
    ...
    expect(
      rendered.getByRole("button", { name: "Show preview console" }),
    ).toBeTruthy();
    expect(
      rendered.getByRole("button", { name: "Open preview DevTools" }),
    ).toBeTruthy();
  ...
  it("renders the Annotate control and a blue Commenting status while the mode is on", () => {
    ...
    expect(
      rendered.getByText("Commenting").getAttribute("data-annotate-status"),
    ).toBe("commenting");
  });
  ...
  it("flushes stacked comments from the bar without implying a send on save", () => {
    ...
    const rendered = render(
      <PreviewBrowserBar
        {...baseProps}
        commentCount={2}
        onFlushComments={onFlushComments}
  ...
  it("disables back and forward when there is no history", () => {
    ...
    expect(
      (rendered.getByRole("button", { name: "Back" }) as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    expect(
      (rendered.getByRole("button", { name: "Forward" }) as HTMLButtonElement)
        .disabled,
    ).toBe(true);
  });
  ...
  it("enables back and forward when there is history", () => {
    const rendered = render(
      <PreviewBrowserBar {...baseProps} canGoBack canGoForward />,
    );
  ...
  it("toggles the console and DevTools, and labels them by current state", () => {
    ...
    rendered.rerender(
      <PreviewBrowserBar {...baseProps} consoleOpen devToolsOpen />,
    );

    expect(
      rendered
        .getByRole("button", { name: "Hide preview console" })
        .getAttribute("aria-pressed"),
    ).toBe("true");
    expect(
      rendered
        .getByRole("button", { name: "Hide preview DevTools" })
        .getAttribute("aria-pressed"),
    ).toBe("true");
  });
  ...
  it("shows the live url while idle and follows it as the page navigates", () => {
    ...
    rendered.rerender(
      <PreviewBrowserBar {...baseProps} url="https://example.com/next" />,
    );
  ...
  it("does not clobber the draft while the user is typing", () => {
    ...
    rendered.rerender(
      <PreviewBrowserBar {...baseProps} url="https://example.com/redirected" />,
    );
  ...
  it("normalizes and navigates on Enter, and holds the address it asked for", () => {
    ...
    const rendered = render(
      <PreviewBrowserBar {...baseProps} onNavigate={onNavigate} />,
    );
    const input = address(rendered);
  ...
  it("releases the asked-for address once the page lands somewhere", () => {
    ...
    fireEvent.keyDown(input, { key: "Enter" });
    rendered.rerender(
      <PreviewBrowserBar {...baseProps} url="https://www.example.org/home" />,
    );
  ...
  it("shows progress inside the address field only while loading", () => {
    const { container, rerender } = render(
      <PreviewBrowserBar {...baseProps} loading />,
    );
  ...
  it("spins the reload glyph only while loading", () => {
    ...
    expect(container.querySelector(".codicon-refresh")?.className).toContain(
    ...
    expect(
      container.querySelector(".codicon-refresh")?.className,
    ).not.toContain("codicon-modifier-spin");
  });
  ...
  it("copies the new address after the page navigates", async () => {
    ...
    await waitFor(() =>
      expect(writeClipboard).toHaveBeenCalledWith("https://example.com/next"),
    );
  });
  ...
  it("shows Open in browser when only the external handler is provided", () => {
    ...
    expect(
      rendered.getByRole("button", { name: "Open in browser" }),
    ).toBeTruthy();
  ...
  it("prefers pop-out over open-in-browser when both handlers are provided", () => {
    const rendered = render(
      <PreviewBrowserBar {...baseProps} onOpenExternal={vi.fn()} />,
    );

    expect(rendered.getByRole("button", { name: "Pop out" })).toBeTruthy();
    expect(
      rendered.queryByRole("button", { name: "Open in browser" }),
    ).toBeNull();
  });
