...
function register(
  ...
  const result = upsertArtifact(
    "session-1",
    { kind, language: kind === "code" ? "python" : kind, title },
    content,
  );
...
async function renderArtifact(artifactId: string) {
  const record = $artifactRegistry
    .get()
    ["session-1"]!.find((item) => item.id === artifactId)!;
...
describe("ArtifactPreview", () => {
  ...
  it("shows the version stepper once an artifact has history, and follows the selection", async () => {
    ...
    expect(
      (screen.getByTitle("Dashboard") as HTMLIFrameElement).srcdoc,
    ).toContain("v2");
