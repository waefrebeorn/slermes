...
describe("actOnActivePreview (drive_preview tool)", () => {
    ...
    withRunner((code) => {
      ...
      return Promise.resolve(
        JSON.stringify({
          acted: "clicked",
          elements: [
            { label: "Log out", ref: "@e1", role: "button", selector: "#out" },
          ],
          success: true,
  ...
  it("reports a page that answers with nothing", async () => {
    ...
    expect(
      (await actOnActivePreview({ kind: "click", ref: "@e1" })).error,
  ...
  const sentTypes = (send: ReturnType<typeof vi.fn>) =>
    send.mock.calls.map(([event]) => event.type);
  ...
  it("clicks with real input, walking the pointer to where the page said", async () => {
    ...
    expect(types.filter((type) => type === "mouseMove").length).toBeGreaterThan(
      1,
    );
  ...
  it("types by pressing keys, after selecting whatever the field held", async () => {
    ...
    expect(
      events.filter(
        (event) => event.type === "keyDown" && event.keyCode === "a",
      )[0],
    ).toMatchObject({
  ...
  it("does not expect a click witness from a verb that never clicks", async () => {
    ...
    cleanups.push(
      registerPreviewScriptRunner(tabId, async (code) =>
        code.includes('"kind":"locate"')
          ? JSON.stringify({
              acted: 'looking at textbox "Search"',
              point: { x: 40, y: 20 },
              success: true,
            })
    ...
    cleanups.push(
      registerPreviewInput(tabId, { focus: vi.fn(), send: vi.fn() }),
    );
  ...
  it("scrolls by wheeling for real, not by scripting the page", async () => {
    ...
    const wheels = send.mock.calls
      .map(([event]) => event)
      .filter((event) => event.type === "mouseWheel");
  ...
  it("says so when the overlay itself swallowed the click", async () => {
    ...
    expect(
      (await actOnActivePreview({ kind: "click", ref: "@e1" })).note,
    ).toContain("overlay intercepted");
  });
  ...
  it("routes history verbs to the pane instead of the guest page", async () => {
    ...
    const tabId = openBrowserTab();
    cleanups.push(
      registerPreviewNav(tabId, { back, forward: vi.fn(), reload: vi.fn() }),
    );
  ...
  it("reports history verbs with no pane to drive", async () => {
    expect((await actOnActivePreview({ kind: "reload" })).error).toContain(
      "open_preview",
    );
  });
});
