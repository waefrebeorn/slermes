...
function mountStackedTabs() {
  ...
  for (const composer of document.querySelectorAll(
...
function dragTo(source: HTMLElement, x: number, y: number) {
  startSessionDrag(
    { id: "dragged", profile: "default", title: "Dragged chat" },
    {
      button: 0,
      clientX: 0,
      clientY: 0,
...
describe("session drop targeting across stacked tabs", () => {
  it("links into the visible tab’s composer, not the tab kept alive behind it", () => {
    ...
    expect(requestComposerInsertRefs).toHaveBeenCalledWith(expect.anything(), {
      target: "tile:visible",
    });
  });
