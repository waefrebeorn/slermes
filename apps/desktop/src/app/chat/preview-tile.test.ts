...
describe("browserTabLabel", () => {
  ...
  it("falls back to the surface when there is no page and no host", () => {
    ...
    expect(browserTabLabel(target, { title: "", url: "about:blank" })).toBe(
      "Browser",
    );
  });
  ...
  it("names an unreported tab from its target", () => {
    expect(browserTabLabel({ ...target, url: "https://github.com/nous" })).toBe(
      "github.com",
    );
  });
});
...
describe("browserTabExternalUrl", () => {
  const openBrowser = (url: string) => {
    openPreview(
      { kind: "url", label: "Browser", source: url, url },
      "explicit-link",
    );
  ...
  it("falls back to the target when the tab has not reported a page yet", () => {
    expect(browserTabExternalUrl(openBrowser("https://github.com/nous"))).toBe(
...
function dockOf(paneId: string) {
  return (
    registry.getArea("panes").find((entry) => entry.id === paneId)
      ?.data as DockData
  )?.dock;
}
