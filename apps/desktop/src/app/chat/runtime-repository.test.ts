...
const feedToRepository = (repository: ExportedRepository) => {
  ...
  } as unknown as Parameters<typeof syncRepositoryIncrementally>[0];
...
describe("useRuntimeMessageRepository", () => {
  ...
  it("anchors a branch group to its fork point, and a windowed cut keeps it", () => {
    ...
    const parents = new Map(
      result.current.messages.map((item) => [item.message.id, item.parentId]),
    );
    ...
    const { result: windowed } = renderHook(() =>
      useRuntimeMessageRepository(messages.slice(1)),
    );

    const windowedParents = new Map(
      windowed.current.messages.map((item) => [item.message.id, item.parentId]),
    );
  ...
  it("renames a duplicated toolCallId within one message instead of crashing useResources (#87857)", () => {
    ...
    const { result } = renderHook(() =>
      useRuntimeMessageRepository([text("user-1", "user", "go"), duplicated]),
    );

    const assistant = result.current.messages.find(
      (item) => item.message.id === "assistant-dup",
    );
    expect(assistant).toBeDefined();

    const toolParts = (
      assistant!.message.content as readonly {
        type: string;
        toolCallId?: string;
      }[]
  ...
  it("drops the carried-over copy when coalescing folds a repeated toolCallId into one message (#87857)", () => {
    ...
    const assistant = result.current.messages.find(
      (item) => item.message.id === "committed-49-assistant",
    );
    expect(assistant).toBeDefined();
  ...
  it("leaves a toolCallId repeated across DIFFERENT non-folded messages untouched", () => {
    ...
    const { result } = renderHook(() =>
      useRuntimeMessageRepository([...turn(1), ...turn(2)]),
    );

    for (const id of ["assistant-1", "assistant-2"]) {
      const item = result.current.messages.find(
        (entry) => entry.message.id === id,
      );

      const ids = (
        item!.message.content as readonly {
          type: string;
          toolCallId?: string;
        }[]
      )
