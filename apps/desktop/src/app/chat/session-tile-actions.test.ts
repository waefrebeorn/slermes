...
const { $activeSessionId, $sessions, setSessions } =
...
const { listTileSessionRow, useSessionTileActions } =
  await import("./session-tile-actions");
...
describe("useSessionTileActions sleep/wake session recovery", () => {
  beforeEach(() => {
    ...
    setSessions([]);
    $sessionTiles.set([
      { runtimeId: RUNTIME_SESSION_ID, storedSessionId: STORED_SESSION_ID },
    ]);
    setSessionTileDelegate({
  ...
  it('resumes the stored session and retries once when session.redirect (steer) reports "session not found"', async () => {
    ...
    const ok = await act(async () =>
      result.current.steerPrompt("actually use Postgres"),
    );
  ...
  it("rebinds prompt.submit recovery to the tile without changing the foreground session", async () => {
    ...
    await act(async () => {
      await expect(
        result.current.submitText("continue the bot chat"),
      ).resolves.toBe(true);
    });
...
describe("useSessionTileActions reloadFromMessage failed-submit rollback (#95745)", () => {
  ...
  beforeEach(() => {
    ...
    publishSessionState(
      RUNTIME_SESSION_ID,
      createClientSessionState(STORED_SESSION_ID, seed as never),
    );
    setSessionTileDelegate({
      archiveSession: vi.fn(async () => undefined),
      branchSession: vi.fn(async () => undefined),
