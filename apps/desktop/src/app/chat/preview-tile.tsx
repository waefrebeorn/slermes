...
function existingPreviewAnchor(tabId: string): string | undefined {
  ...
  const inTree = treePanesWithPrefix(`${PREVIEW_TILE_PREFIX}:`).find(
    (id) => id !== own,
  );
...
export function watchPreviewTiles(): void {
  ...
  const follow = () => {
    ...
    const tabId = active.slice(
      PREVIEW_TILE_PREFIX.length + 1,
    ) as RightRailTabId;
...
const watchPreviewTileMirror = paneMirror<{ id: string }>({
  ...
  newTab: (tabId) =>
    targetFor(tabId)?.kind === "url" ? newBrowserTab : undefined,
