...
function installDelegate(): void {
  const updateSession: SessionTileDelegate["updateSession"] = (
    runtimeId,
    updater,
  ) => {
    const current =
      $sessionStates.get()[runtimeId] ?? createClientSessionState(STORED_ID);
...
describe("session tile attachment occurrence ownership", () => {
  ...
  it("preserves a thumbnail that resolves before tile submit staging", async () => {
    ...
    expect(
      scope.attachments.updateIfCurrent(original, { thumbnailUrl: THUMBNAIL }),
    ).toBe(true);
