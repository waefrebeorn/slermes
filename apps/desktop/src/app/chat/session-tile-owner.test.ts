...
const tile = (
  over: Partial<SessionTile> & Pick<SessionTile, "storedSessionId">,
): SessionTile => over as SessionTile;
...
describe("tileOwnerRoute", () => {
  ...
  it("prefers the tile own explicit route", () => {
    const route = tileOwnerRoute(
      [
        tile({
          ownerRoute: { connectionId: "pandora", profile: "work" },
          storedSessionId: "s1",
        }),
      ],
  ...
  it("falls back to the owner hint when neither tile nor row is tagged", () => {
    ...
    expect(
      tileOwnerRoute([tile({ storedSessionId: "s1" })], [], "s1"),
    ).toMatchObject({ connectionId: "pandora" });
  });
  ...
  it("carries a targetProfile through, and omits it when absent", () => {
    ...
    expect(
      tileOwnerRoute(
        [
          tile({
            ownerRoute: { connectionId: "p", profile: "w" },
            storedSessionId: "s1",
          }),
        ],
        [],
  ...
  it("narrows a bare profile owner away", () => {
    ...
    expect(
      tileOwnerRoute([], [row({ id: "s1", profile: "work" })], "s1"),
    ).toBeUndefined();
  });
