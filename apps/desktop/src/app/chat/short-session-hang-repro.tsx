...
function turn(
  index: number,
  fixtureClass: FixtureTurn["class"],
  assistantParts: ChatMessage["parts"],
): FixtureTurn {
    ...
    messages: [
      {
        ...
        parts: [
          text(
            `Deterministic ${fixtureClass} prompt ${index}: preserve renderer responsiveness.`,
          ),
        ],
        role: "user",
        timestamp,
...
const fixtures: FixtureTurn[] = [
  turn(1, "plain", [
    text(
      "Plain response one. The transcript should remain selectable and scrollable.",
    ),
  ]),
  turn(2, "code", [
    text(
  ...
  turn(5, "plain", [
    text(
      "Plain response five is the first required responsiveness checkpoint.",
    ),
  ]),
  turn(6, "code", [
    text(
  ...
  turn(8, "mixed", [
    text(
      "Mixed response eight is the final checkpoint. `inline code` and **markdown** remain interactive.",
    ),
    {
      args: { path: "/tmp" },
      argsText: '{"path":"/tmp"}',
...
if (typeof window !== "undefined" && !window.__SHORT_SESSION_HANG_REPRO__) {
  ...
  const record = (sample: HeartbeatSample) => {
    if (
      ...
      sample.at < sampleWindowStartedAt
    ) {
      return;
    }
  ...
  const afterTwoFramesWhileVisible = () =>
    new Promise<void>((resolve, reject) => {
      ...
      requestAnimationFrame(() => {
        if (document.visibilityState !== "visible") {
          finish(
            new Error("renderer became hidden before the first paint frame"),
          );
        ...
        requestAnimationFrame(() => {
          if (document.visibilityState !== "visible") {
            finish(
              new Error("renderer became hidden before the second paint frame"),
            );
  ...
  window.__SHORT_SESSION_HANG_REPRO__ = {
    ...
    load: async (count) => {
      ...
      const next = fixtures
        .slice(0, bounded)
        .flatMap((fixture) => fixture.messages);
      const expectedUserIds = next
        .filter((message) => message.role === "user")
        .map((message) => message.id);
      const expectedAssistantIds = next
        .filter((message) => message.role === "assistant")
        .map((message) => message.id);
      const expectedAssistantMessages = expectedAssistantIds.length;
      const expectedTools = next
      ...
      const expectedCodeCards = next
        ...
        .reduce(
          (total, part) =>
            total + Math.floor((part.text.match(/```/g)?.length ?? 0) / 2),
          0,
        );
      ...
      const paintedCodeCards = assistantRoots
        .flatMap((root) => [
          ...root.querySelectorAll('[data-slot="code-card"]'),
        ])
        .filter(isPainted).length;
      ...
      const paintedAssistantMessages = assistantRoots.filter((root) => {
        ...
        const hasPaintedTool = [
          ...root.querySelectorAll("[data-tool-row]"),
        ].some(isPainted);
        const hasPaintedCode = [
          ...root.querySelectorAll('[data-slot="code-card"]'),
        ].some(isPainted);
    ...
    manifest: async () => {
      ...
      return {
        ...
        messageRecords: fixtures.reduce(
          (total, fixture) => total + fixture.messages.length,
          0,
        ),
