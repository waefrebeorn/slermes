...
function primaryField<T>(
  ...
  const $field: ReadableAtom<T> = computed(
    [$primaryState, $draft],
    (state, draft: T) => (state ? select(state) : draft),
  );

  return $field;
}

const $primaryMessages = primaryField<ChatMessage[]>(
...
export const PRIMARY_SESSION_VIEW: SessionView = {
  ...
  $reasoningEffort: primaryField<string>(
    (state) => state.reasoningEffort,
    $currentReasoningEffort,
  ),
  $runtimeId: $activeSessionId,
  $storedId: $selectedStoredSessionId,
  $turnStartedAt: primaryField<number | null>(
