...
describe("shouldShowIntro", () => {
  ...
  it("hides on surfaces that are not an empty primary draft", () => {
    ...
    expect(shouldShowIntro({ ...showing, routedSessionView: true })).toBe(
      false,
    );
    expect(
      shouldShowIntro({ ...showing, selectedSessionId: "session-1" }),
    ).toBe(false);
    expect(shouldShowIntro({ ...showing, activeSessionId: "session-1" })).toBe(
      false,
    );
