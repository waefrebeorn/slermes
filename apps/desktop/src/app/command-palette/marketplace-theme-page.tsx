...
export function MarketplaceThemePage({
  search,
  onPickTheme,
}: MarketplaceThemePageProps) {
  const { t } = useI18n();
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      window.hermesDesktop?.themes?.searchMarketplace(debouncedSearch) ??
      Promise.resolve([]),
    staleTime: 5 * 60 * 1000,
  });
  ...
  return (
    <div role="listbox">
      {installError && (
        <p className="px-2 pb-1 pt-1.5 text-[0.6875rem] text-(--ui-red)">
          {installError}
        </p>
      )}
      {results.map((item) => {
