...
const rankGroups = (groups: PaletteGroup[], search: string): PaletteGroup[] => {
  ...
  return groups
    .map((group) => {
      ...
      return {
        group: { ...group, items: scored.map((entry) => entry.item) },
...
const paletteValue = (item: PaletteItem): string =>
  `${item.label}\u0001${item.id}`;
...
const PaletteRow = memo(function PaletteRow({
  ...
  const combo =
    (item.action ? bindingsFor(item.action, bindings)[0] : undefined) ??
    item.comboHint;
  ...
  return (
    ...
    >
      ...
      <span
        className={cn("truncate", modPreview && "text-muted-foreground/80")}
      >
        {modPreview ? (
          item.modLabel
        ) : (
      ...
      {item.detail && (
        <span
          className={cn(
            HUD_NOTE,
            HUD_NOTE_VARIANT[item.detailVariant ?? "muted"],
          )}
        >
          {item.detail}
        </span>
      ...
      {combo && (
        <KbdCombo
          className={cn("ml-auto", modPreview ? "opacity-90" : "opacity-55")}
          combo={combo}
          size="sm"
        />
      )}
...
const NON_CONFIG_SETTINGS: ReadonlyArray<{
  ...
  {
    icon: Archive,
    keywords: ["history", "archived"],
    labelKey: "archivedChats",
    tab: "sessions",
  },
  {
    icon: Info,
...
function themeSupportsMode(name: string, target: "light" | "dark"): boolean {
  ...
  const background =
    target === "dark"
      ? (resolved.darkColors ?? resolved.colors).background
...
function CommandPaletteBody({ onExited }: { onExited: () => void }) {
  ...
  const resolveThemeMode = useCallback(
    (target: ThemeMode): "light" | "dark" =>
      target === "system" ? (systemDark ? "dark" : "light") : target,
    [systemDark],
  );
  ...
  const mcpServers = useMemo(
    () => Object.keys(getServers(configQuery.data ?? null)).sort(),
    [configQuery.data],
  );

  const sessions = useMemo(
    () => (sessionsQuery.data?.sessions ?? []).map(toSessionEntry),
    [sessionsQuery.data],
  );
  const archivedSessions = useMemo(
    () => (archivedQuery.data?.sessions ?? []).map(toSessionEntry),
    [archivedQuery.data],
  );
  ...
  const go = useCallback(
    (path: string) => () => navigateToWorkspacePage(navigate, path),
    [navigate],
  );
    ...
    (sessionId: string) =>
      (event?: {
        ...
        openSession(
          sessionId,
          navigate,
          openSessionIntentFromModifiers(event, "stack"),
        );
      },
    [navigate],
  );
        ...
        ? [
            {
              ...
              items: worktrees.map((wt) => {
                const name =
                  wt.branch?.trim() || wt.path.split("/").pop() || wt.path;
  ...
  const baseGroups = useMemo<PaletteGroup[]>(() => {
    ...
    const projectGroup: PaletteGroup = {
      ...
      items: [
        ...
        ...filterVisibleProjects(projectTree, dismissedAutoProjects).map(
          (project) => ({
            comboHint: "mod+enter",
            icon: codiconIcon(
              project.icon || (project.isNoProject ? "home" : "folder-library"),
            ),
            ...
            runWithEvent: (event?: {
              ...
              goToProject(project.id, {
                newSession: Boolean(event?.metaKey || event?.ctrlKey),
              }),
          }),
        ),
      ],
  ...
  const searchGroups = useMemo<PaletteGroup[]>(() => {
    ...
    result.push({
      ...
      items: THEME_MODES.map((entry) => ({
        ...
        keywords: [
          ...
          "brightness",
          entry.mode,
          t.settings.modeOptions[entry.mode].label,
        ],
        label: t.settings.modeOptions[entry.mode].label,
        onHighlight: () =>
          previewTheme(themeName, resolveThemeMode(entry.mode)),
    ...
    if (fieldItems.length > 0) {
      result.push({
        heading: t.commandCenter.settingsFields,
        items: fieldItems,
      });
    }
    ...
    if (archivedSessions.length > 0) {
      result.push({
        ...
        items: archivedSessions.map((session) => ({
          ...
          label: session.title,
          run: go(
            `${SETTINGS_ROUTE}?tab=sessions&session=${encodeURIComponent(session.id)}`,
          ),
        })),
      });
    }
  ...
  const visibleGroups = useMemo(
    () => rankGroups(unrankedGroups, search),
