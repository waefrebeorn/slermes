...
export function OverlaySplitLayout({
  children,
  className,
}: OverlaySplitLayoutProps) {
  return (
    <div
      className={cn(
  ...
  return (
    <>
      <OverlaySidebar className={RAIL_HIDDEN}>
        ...
        {footer && (
          <div className="mt-auto flex items-center gap-1 pt-2">{footer}</div>
        )}
      </OverlaySidebar>
            ...
            items={groups.flatMap((group) => [
              {
                active:
                  group.active &&
                  !group.children?.some((child) => child.active),
                icon: group.icon,
                id: group.id,
                label: group.label,
