...
function installPrimaryDesktop(authMode: "oauth" | "token") {
  const getConnection = vi.fn(async (profile?: null | string) => ({
    ...
    wsUrl:
      authMode === "oauth"
        ? "wss://gateway.example.test/api/ws?ticket=stale"
        : "ws://127.0.0.1:5151/api/ws",
  }));
...
describe("useGatewayRequest", () => {
  ...
  it.each([
    ...
    "recovers the registered remote source after a $label failure",
    async ({ error }) => {
      ...
      await act(async () => {
        await expect(
          result.current.requestGateway("prompt.submit", { text: "first" }),
        ).resolves.toEqual({ turn: 1 });
      });
      gateway.connectionState = "closed";
      await act(async () => {
        await expect(
          result.current.requestGateway("prompt.submit", { text: "second" }),
      ...
      expect(gateway.connect).toHaveBeenLastCalledWith(
        expect.stringContaining("ticket=fresh-2"),
      );
    },
  );
  ...
  it("does not reconnect for a non-transport request failure", async () => {
    ...
    await expect(result.current.requestGateway("session.resume")).rejects.toBe(
      failure,
    );
  ...
  it("surfaces a real secondary OAuth reauth rejection as the original transport failure", async () => {
    ...
    await expectSecondaryRecoveryFailure(
      gateway,
      result.current.requestGateway,
    );
  ...
  it("surfaces a failed secondary OAuth ticket mint without using the stale ticket or local bridges", async () => {
    const { desktop, gateway } = await activateRemoteGateway();
    desktop.getGatewayWsUrlFor.mockRejectedValue(
  ...
  it.each([
    ...
    await act(async () => {
      await expect(
        result.current.requestGateway("session.resume"),
  ...
  it("rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)", async () => {
    ...
    const pending = expect(
      result.current.requestGateway("some.method"),
