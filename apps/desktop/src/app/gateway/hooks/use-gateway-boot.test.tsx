...
describe("primaryRuntimeConnectionId", () => {
  it("uses the registry identity when the primary connection has one", () => {
    expect(
      primaryRuntimeConnectionId({ connectionId: " tower ", mode: "remote" }),
    ).toBe("tower");
  });
...
function fakeDesktop() {
  let bootProgressHandler: ((payload: Record<string, unknown>) => void) | null =
    null;
  ...
  return {
    ...
    getGatewayWsUrl: vi.fn(
      async (conn?: { wsUrl?: string }) => conn?.wsUrl ?? primaryConn.wsUrl,
    ),
...
describe("useGatewayBoot remote reconnect loop (real hook, fake socket)", () => {
  ...
  it("#93937: the Sessions switcher never publishes the new source while the previous backend's runtime id is still bound", async () => {
    ...
    const setLastUsed = vi.fn(async (id: string) => ({
      ok: true,
      registry: { ...registryConnections, lastUsed: id },
    }));
    ...
    desktop.getConnectionFor = vi.fn(
      async ({
        connectionId,
        profile,
      }: {
    ...
    expect(published).toEqual(
      published.map(() => ({ activeSessionId: null, switching: true })),
    );
    ...
    await expect(
      requestGatewayForAgent("primary-vps", "default", "ping"),
  ...
  it("a Settings switch superseded while reading its descriptor cannot publish over a newer Sessions switch", async () => {
    ...
    let releaseSettings: (connection: typeof settingsConn) => void = () =>
      undefined;

    desktop.api = vi.fn(async ({ path }: { path: string }) =>
      path === "/api/profiles/active"
        ? { active: "coder", current: "coder" }
        : { profiles: [] },
    );
    desktop.getConnectionFor = vi.fn(
      async ({
    ...
    desktop.connections = {
      list: vi.fn(async () => registryConnections),
      setLastUsed: vi.fn(async (id: string) => ({
        ok: true,
        registry: { ...registryConnections, lastUsed: id },
      })),
    };
    ...
    const profileRefreshes = vi.mocked(desktop.api as ReturnType<typeof vi.fn>)
      .mock.calls.length;
  ...
  it("forwards failed-switch recovery ownership through its registered lifecycle", async () => {
    const refreshSessions = vi.fn(
      async (_shouldPublish?: () => boolean) => undefined,
    );
  ...
  it("a superseded Settings switch cannot publish delayed cwd or config work after the winner", async () => {
    ...
    const sanitizeWorkspaceCwd = vi.fn((cwd: string) => {
      ...
      return sanitizeRead === 1
        ? staleSanitize.promise
        : Promise.resolve({ cwd });
    });

    (window as { hermesDesktop?: unknown }).hermesDesktop = desktop;

    const refreshHermesConfig = async (
      _force = false,
      shouldPublish?: () => boolean,
    ) => {
      if (!shouldPublish) {
    ...
    act(() => connectionApplied?.());
    await vi.waitFor(() =>
      expect(sanitizeWorkspaceCwd).toHaveBeenCalledTimes(1),
    );
  ...
  it("re-fetches the profile rail from the NEW backend after a connection apply (#85731)", async () => {
    ...
    expect($profiles.get().map((profile) => profile.name)).toEqual([
      "default",
      "cloud-eric",
    ]);
  });
  ...
  it("a revalidateConnection() that hangs on reconnect does not permanently latch the backoff loop (#93454)", async () => {
    ...
    expect(desktop.getConnection.mock.calls.length).toBeGreaterThan(
      callsBeforeDrop,
    );
    expect($gatewayState.get()).toBe("open");
  });
  ...
  it("onActiveConnectionInvalidated: a fallback getConnection() that hangs rejects on its own instead of latching $connection forever (#93454 sibling)", async () => {
    ...
    desktop.getConnection.mockImplementation(
      () => new Promise(() => undefined),
    );
  ...
  it("manual reconnect revalidates, re-resolves, re-mints, and re-dials the dropped socket", async () => {
    ...
    expect(
      lastCall.length === 0 || lastCall[0] == null || lastCall[0] === "",
    ).toBe(true);
  ...
  it("FIX: a failed session-list fetch during boot is non-fatal — the app still boots", async () => {
    ...
    const refreshSessions = vi.fn(async () => {
      throw new Error(
        '404: {"detail":"No such API endpoint: /api/profiles/sessions/sidebar"}',
      );
    });
  ...
  it("FIX: primary sleep/wake reconnect dials the window backend, not the active secondary profile", async () => {
    ...
    const reconnectCalls =
      desktop.getConnection.mock.calls.slice(callsBeforeDrop);
    expect(
      reconnectCalls.some((args) => (args[0] ?? "").trim() === "coder"),
    ).toBe(false);
    expect(
      reconnectCalls.some(
        (args) => args.length === 0 || args[0] == null || args[0] === "",
      ),
    ).toBe(true);
  ...
  it("FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection", async () => {
    ...
    desktop.getConnection = vi
      ...
      .mockRejectedValueOnce(
        new Error("Could not verify the existing SSH backend."),
      )
    ...
    desktop.getBootProgress = vi.fn(async () => ({
      ...
      fakeMode: false,
      message:
        "Desktop boot failed: Could not verify the existing SSH backend.",
      phase: "backend.error",
      progress: 24,
      retryable: true,
      running: false,
  ...
  it("wake probe: a timeout while a turn is IN FLIGHT defers the force-close", async () => {
    ...
    expect($gatewayState.get()).toBe("open");
    expect(FakeWebSocket.instances.length).toBeGreaterThanOrEqual(
      socketCountBefore,
    );
  });
