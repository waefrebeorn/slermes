...
export function UpdatesOverlay() {
  ...
  const updateBlockers =
    !isBackend && apply.error === "venv-blocked" && apply.blockers?.length
      ? apply.blockers
      : null;
  ...
  return (
    <Dialog onOpenChange={handleClose} open={open}>
      ...
      >
        {phase === "applying" && (
          <ApplyingView apply={apply} isBackend={isBackend} />
        )}
        ...
        {phase === "error" && updateBlockers ? (
          <BlockerView
            ...
            onStopAndUpdate={() =>
              void applyUpdates({ stopSafeBlockers: true })
            }
          />
        ) : null}
    ...
    <div className="grid gap-5 px-6 pb-6 pt-7 pr-8">
      ...
      <button
        className={cn(
          ...
          copied
            ? "border-primary/50"
            : "border-(--stroke-nous) hover:border-(--ui-stroke-secondary)",
        )}
        onClick={handleCopy}
        type="button"
      >
...
const SENSITIVE_QUERY_ARGUMENT = new RegExp(
  `([?&]${SENSITIVE_ARGUMENT_NAME}=)[^&#\\s]+`,
  "gi",
);
...
export function BlockerView({
  ...
  const safeBlockers = blockers.filter(
    (blocker) => blocker.kind === "local-preview" && blocker.safeToStop,
  );
    ...
    <div className="grid gap-5 px-6 pb-6 pt-7 pr-8">
      ...
      <div className="grid gap-2">
        {blockers.map((blocker) => {
          const isSafePreview =
            blocker.kind === "local-preview" && blocker.safeToStop;
          ...
          return (
            ...
            >
              ...
              <div className="text-xs text-muted-foreground">
                {isSafePreview && blocker.port
                  ? u.portLabel(blocker.port)
                  : u.pidLabel(blocker.pid)}
              </div>
            </div>
          );
        })}
