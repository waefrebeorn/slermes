...
export function jobIsScriptOnly(
  job: Pick<CronJob, "no_agent" | "script">,
): boolean {
  return Boolean(job.no_agent) && Boolean(asText(job.script).trim());
}

export type CronEditorValidationError =
  "prompt" | "prompt_and_schedule" | "schedule";
...
export function validateCronEditor(
  input: CronEditorValidationInput,
): CronEditorValidationError | null {
