...
describe("validateCronEditor", () => {
  it("requires prompt and schedule for agent-backed jobs", () => {
    expect(
      validateCronEditor({ prompt: "", schedule: "", scriptOnlyJob: false }),
    ).toBe("prompt_and_schedule");
    expect(
      validateCronEditor({
        prompt: "",
        schedule: "0 9 * * *",
    ...
    expect(
      validateCronEditor({ prompt: "go", schedule: "", scriptOnlyJob: false }),
    ).toBe("schedule");
  });
  ...
  it("still requires schedule for script-only jobs", () => {
    expect(
      validateCronEditor({ prompt: "", schedule: "", scriptOnlyJob: true }),
    ).toBe("schedule");
  });
});
...
describe("cron delivery targets", () => {
  it("parses comma-separated targets and removes duplicates", () => {
    expect(parseCronDeliveryTargets("local, telegram,local")).toEqual([
      "local",
      "telegram",
    ]);
  });
  ...
  it("adds a second target in the scheduler comma-separated format", () => {
    expect(toggleCronDeliveryTarget("local", "origin", true)).toBe(
      "local,origin",
    );
  });

  it("removes one target while keeping the other selection", () => {
    expect(toggleCronDeliveryTarget("local,origin", "local", false)).toBe(
      "origin",
    );
  });
