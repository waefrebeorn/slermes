...
const SPILLED_MODEL: LocalCatalogModel = {
  ...
  fit_summary:
    "starts at 64K and grows toward 256K as you use it (larger than your GPU memory — runs slower)",
};
...
beforeEach(() => {
  ...
  mocked.getLocalCatalog.mockResolvedValue({
    models: [FITTING_MODEL, SPILLED_MODEL, REFUSED_MODEL],
  });
...
describe("LocalModelsSettings", () => {
  it("offers the runtime install with a plain-language explanation", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /install runtime/i }),
    ).toBeTruthy();
  });
  ...
  it("shows every catalog model with fit pills; unaffordable ones stay visible with the reason", async () => {
    ...
    expect(screen.getByText("Full 256K context").className).toContain(
      "emerald",
    );
  ...
  it("never greens the full-context pill on a system-RAM model", async () => {
    ...
    expect(screen.getByText("Full 256K context").className).not.toContain(
      "emerald",
    );
  });
  ...
  it("explains the Recommended pick on hover", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getAllByText(/would respond too slowly on its memory bandwidth/)
          .length,
      ).toBeGreaterThan(0),
    );
  });
  ...
  it("enables downloads only once the runtime is installed", async () => {
    ...
    await screen.findByText("Qwen3.6 27B");
    const [fittingButton] = screen.getAllByRole("button", {
      name: /download · 17\.6 GB/i,
    });
  ...
  it("tracks a download job to completion and refreshes", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        mocked.getLocalModelsStatus.mock.calls.length,
  ...
  it("renders progress for a download discovered from the store (survives pane remount)", async () => {
    ...
    expect(screen.getAllByText(/0\.0 GB of 0\.0 GB|of/).length).toBeGreaterThan(
      0,
    );
...
describe("quickstart", () => {
  it("leads with one button on a fresh machine and fires the quickstart job", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /set up for me/i }),
    ).toBeTruthy();
...
describe("BrowseSection", () => {
  it("searches HF after a pause and shows fit-priced files on demand", async () => {
    ...
    try {
        ...
        files: [
          {
            fit: "fits-gpu",
            label: "Q4_K_M",
            paths: ["Qwen3.8-27B-Q4_K_M.gguf"],
            total_bytes: 17 * 2 ** 30,
          },
          {
            fit: "too-big",
            label: "F16",
            paths: ["Qwen3.8-27B-F16.gguf"],
            total_bytes: 56 * 2 ** 30,
          },
        ],
      });
      ...
      vi.mocked(hermes.downloadBrowsedModel).mockResolvedValue({
        job_id: "j1",
