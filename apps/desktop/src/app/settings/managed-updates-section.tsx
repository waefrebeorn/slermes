...
function stateTone(
  state: ManagedUpdateState | undefined,
): "muted" | "primary" | "warn" {
...
export function ManagedUpdatesSection() {
  ...
  const sshConnections = (registry?.connections ?? []).filter(
  ...
  const receiptLine = (state: ManagedUpdateState): string | null => {
    ...
    const parts = [
      m.receipt(state.receipt.correlationId.slice(0, 8), state.receipt.outcome),
    ];

    if (state.receipt.preVersion && state.receipt.postVersion) {
      parts.push(
        m.receiptVersions(state.receipt.preVersion, state.receipt.postVersion),
      );
    }
    ...
    <section className="mt-8">
      ...
      <div className="grid gap-1">
        {sshConnections.map((connection) => {
          ...
          const restored =
            state?.scopes
              .filter((scope) => scope.restored)
              .map((scope) => scope.profile) ?? [];
          const unrestored =
            state?.scopes.filter((scope) => !scope.restored) ?? [];
            ...
            <ListRow
              action={
                updating ? (
                  ...
                  <Button
                    onClick={() => void runManagedUpdate(connection.id)}
                    size="sm"
                  >
                    <Download /> {m.update}
                  </Button>
              ...
              below={
                ...
                (updating ||
                  ...
                  <div className="mt-1 grid gap-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                    ...
                    {receipt ? (
                      <p className="font-mono text-[0.68rem]">{receipt}</p>
                    ) : null}
                    {!updating && restored.length > 0 ? (
                      <p>{m.scopesRestored(restored.join(", "))}</p>
                    ) : null}
                    {!updating &&
