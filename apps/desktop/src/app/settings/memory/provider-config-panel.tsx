...
function seedValues(config: MemoryProviderConfig): Record<string, string> {
  return Object.fromEntries(
    config.fields
      ...
      .map((field) => [field.key, field.kind === "secret" ? "" : field.value]),
  );
}
...
export function ProviderConfigPanel({
            ...
            (current) =>
              current && {
                ...
                fields: current.fields.map((f) =>
                  f.key === field.key ? { ...f, is_set: true } : f,
                ),
              },
          );
        } else {
  ...
  if (!config) {
    ...
    return (
      <PageLoader
        className="min-h-24"
        label="Loading memory provider settings..."
      />
    );
  }
      ...
      <div className="flex items-center gap-2 py-2">
        ...
        >
          ...
          {secretFields.map((field) => (
            <Pill key={field.key}>
              {field.is_set ? `${field.label} set` : `${field.label} not set`}
            </Pill>
          ))}
        </button>
        {hasFullConfig && (
