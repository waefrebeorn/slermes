    ...
    <section className="min-h-0 overflow-hidden">
      <div
        className={cn(
          ...
          bare ? "px-5 pb-6" : cn("pb-20", PAGE_INSET_X),
        )}
      >
        {children}
      </div>
...
export function Pill({
  ...
  children,
  ...props
}: { tone?: keyof typeof PILL_VARIANT; children: ReactNode } & Omit<
    ...
    <div className={cn("@container", className)} data-tour={dataTour} id={id}>
      ...
      >
        <div className="min-w-0">
          ...
          {hint && (
            <div className="mt-1 block font-mono text-[0.68rem] text-muted-foreground/45">
              {hint}
            </div>
          )}
          {below}
        ...
        {action && (
          <div className={cn("min-w-0", !wide && "@2xl:justify-self-end")}>
            {action}
          </div>
        )}
      </div>
  ...
  return (
    <div className="@container">
      ...
      >
        ...
        {!wide && (
          <Skeleton className="h-8 w-full @2xl:w-72 @2xl:justify-self-end" />
        )}
      </div>
    </div>
  );
