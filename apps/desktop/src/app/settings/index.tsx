...
import type {
  SettingsPageProps,
  SettingsView as SettingsViewId,
} from "./types";
...
export function SettingsView({
  ...
  const [activeView, setActiveView] = useRouteEnumParam(
    "tab",
    SETTINGS_VIEWS,
    "config:model" as SettingsViewId,
  );
  ...
  const openKeysView = useCallback(
    (view: KeysView) => openSubView("keys", "kview", view, "tools"),
    [openSubView],
  );
    ...
    () => [
      ...
      },
      {
        ...
        children: [
          ...
          {
            active:
              activeView === "providers" && providerView === "custom-endpoints",
            icon: Globe,
          ...
          ...($localModelsEnabled.get()
            ? [
                {
                  active:
                    activeView === "providers" && providerView === "local",
                  icon: Cpu,
                  id: "pview:local",
