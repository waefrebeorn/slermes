...
function ConfigSettingsInner({
  ...
  const {
    ...
    queryKey:
      scopeProfile == null
        ...
        : ["hermes-config-schema", normalizeProfileKey(scopeProfile)],
  ...
  useEffect(() => {
    if (loadedConfig && !configSeeded.current) {
      ...
      savedDiscoverySignatureRef.current = repoDiscoveryPolicySignature(
        repoDiscoveryPolicyFromConfig(loadedConfig),
      );
    ...
    getElevenLabsVoices(scopeProfile)
      .then((result) => {
        ...
        setElevenLabsVoiceLabels(
          Object.fromEntries(
            result.voices.map((voice) => [voice.voice_id, voice.label]),
          ),
        );
      })
      .catch(() => {
        ...
        try {
          ...
          if (saveVersionRef.current === v) {
            ...
            if (scopeProfile == null) {
              const discoverySignature = repoDiscoveryPolicySignature(
                repoDiscoveryPolicyFromConfig(snapshot),
              );
  ...
  const updateConfig = (next: HermesConfigRecord) => {
    ...
    if (config && clearsEnabledToolsets(config, next)) {
      void confirm({ destructive: true, title: c.toolsetsWipeConfirm }).then(
        (ok) => {
          if (ok) {
            applyConfig(next);
          }
          ...
          {visibleFields.map(([key, field]) => (
            ...
            >
              <ConfigField
                descriptionExtra={
                  key === "memory.provider" &&
                  isExternalMemoryProvider(getNested(config, key)) ? (
                    <MemoryConnect
