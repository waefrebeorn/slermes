      ...
      settings: {
        toolsets: {
          browserRealProfile: {
            ...
            description:
              "Copies your default browser profile into a managed snapshot.",
...
vi.mock("../hooks/use-config-record", () => ({
  hermesConfigCacheWriter: () => (config: Record<string, unknown>) =>
    mocks.cache(config),
...
describe("BrowserRealProfilePanel", () => {
  ...
  it("renders off for a config without the key and turns it on", async () => {
    ...
    const toggle = screen.getByRole("switch", {
      name: "Use My Real Browser Profile",
    });
  ...
  it("rolls the optimistic cache write back when the save fails", async () => {
    ...
    await act(async () => {
      fireEvent.click(
        screen.getByRole("switch", { name: "Use My Real Browser Profile" }),
      );
    });
