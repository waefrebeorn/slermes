...
describe("settings helpers", () => {
  it("surfaces repository discovery config in Workspace with user-facing copy", () => {
    ...
    expect(
      fieldCopyForSchemaKey(FIELD_LABELS, "desktop.repo_scan_enabled"),
    ).toBeTruthy();
    expect(
      fieldCopyForSchemaKey(
        FIELD_DESCRIPTIONS,
  ...
  describe("defineFieldCopy", () => {
    it("flattens nested field copy paths", () => {
      ...
      expect(copy[["stt", "elevenlabs", "language_code"].join(".")]).toBe(
        "Language",
      );
    });
    ...
    it("maps schema keys to camelCase translation keys", () => {
      expect(schemaKeyToFieldCopyKey("model_context_length")).toBe(
        "modelContextLength",
      );
      expect(schemaKeyToFieldCopyKey("display.show_reasoning")).toBe(
        "display.showReasoning",
      );
      expect(schemaKeyToFieldCopyKey("tool_output.max_line_length")).toBe(
        "toolOutput.maxLineLength",
      );
      expect(
        schemaKeyToFieldCopyKey("updates.non_interactive_local_changes"),
      ).toBe("updates.nonInteractiveLocalChanges");
    });
    ...
    it("looks up camelCase field copy by schema key with legacy fallback", () => {
      ...
      expect(fieldCopyForSchemaKey(copy, "model_context_length")).toBe(
        "Context Window",
      );
      expect(fieldCopyForSchemaKey(copy, "display.show_reasoning")).toBe(
        "Reasoning Blocks",
      );
      expect(fieldCopyForSchemaKey(copy, "tool_output.max_line_length")).toBe(
        "Line Length Limit",
      );
      expect(fieldCopyForSchemaKey(copy, "file_read_max_chars")).toBe(
        "Legacy File Read Limit",
      );
    });
  ...
  it("rejects prototype-polluting config paths", () => {
    ...
    expect(() => setNested(config, "__proto__.polluted", true)).toThrow(
      "Unsafe config path",
    );
    expect(() =>
      setNested(config, "constructor.prototype.polluted", true),
  ...
  describe("stripToolsetLabel", () => {
    it("removes leading emoji prefixes from registry labels", () => {
      ...
      expect(stripToolsetLabel("❓ Clarifying Questions")).toBe(
        "Clarifying Questions",
      );
      expect(stripToolsetLabel("🌐 Browser Automation")).toBe(
        "Browser Automation",
      );
    ...
    it("leaves plain titles unchanged", () => {
      expect(stripToolsetLabel("Terminal & Processes")).toBe(
        "Terminal & Processes",
      );
    });
  });
  ...
  describe("toolsetDisplayLabel", () => {
    it("strips emoji from toolset rows", () => {
      expect(
        toolsetDisplayLabel({ name: "cronjob", label: "⏰ Cron Jobs" }),
      ).toBe("Cron Jobs");
    });
  });
  ...
  describe("enumOptionsFor — backend selector dropdowns", () => {
    ...
    it("renders dropdowns for per-backend model/device sub-fields", () => {
      expect(enumOptionsFor("stt.openai.model", "whisper-1", config)).toContain(
        "gpt-4o-transcribe",
      );
      expect(
        enumOptionsFor("tts.openai.model", "gpt-4o-mini-tts", config),
      ).toContain("tts-1-hd");
      expect(enumOptionsFor("tts.neutts.device", "cpu", config)).toEqual([
        "cpu",
        "cuda",
        "mps",
      ]);
    ...
    it("narrows OpenAI TTS voice suggestions to what the selected model supports", () => {
      ...
      const full = enumOptionsFor("tts.openai.voice", "alloy", {
      ...
      for (const model of ["tts-1", "tts-1-hd"]) {
        const narrowed = enumOptionsFor("tts.openai.voice", "alloy", {
          tts: { openai: { model } },
        });
      ...
      const custom = enumOptionsFor("tts.openai.voice", "my-cloned-voice", {
    ...
    it("appends a hand-typed value not in the known list so it stays selected", () => {
      const opts = enumOptionsFor(
        "tts.provider",
        "my-custom-command-tts",
        config,
      );
  ...
  describe("sectionFieldEntries", () => {
    it("renders memory.provider from config even when the backend schema omits it", () => {
      ...
      const memoryKeys = (
        sectionFieldEntries(schema, config).get("memory") ?? []
      ).map(([key]) => key);
    ...
    it("infers the field type from the config value when the schema omits the key", () => {
      ...
      const fields = new Map(
        sectionFieldEntries({}, config).get("memory") ?? [],
      );
  ...
  describe("clearsEnabledToolsets", () => {
    it("flags a non-empty → empty transition", () => {
      const prev: HermesConfigRecord = {
        toolsets: ["memory", "terminal", "web_search"],
      };
  ...
  describe("diffConfig", () => {
    it("omits a top-level key the draft never touched", () => {
      ...
      const baseline: HermesConfigRecord = {
        fallback_providers: ["nara1"],
        timezone: "UTC",
      };
      const draft: HermesConfigRecord = {
        fallback_providers: ["nara1"],
    ...
    it("includes a nested key only when it actually changed, leaving siblings out", () => {
      ...
      const draft: HermesConfigRecord = {
        display: { personality: "default", show_reasoning: false },
      };
