...
describe("settings search index", () => {
  it("builds config results from renderable schema fields with exact deep links", () => {
    ...
    expect(
      entries.some((entry) => entry.id === "config-field:tts.openai.voice"),
    ).toBe(false);
  });
  ...
  it("discovers future tool and setting entries entirely from backend metadata", () => {
    const vars = {
      ...
      FUTURE_GATEWAY_URL: envVar("setting", {
        description: "Route gateway traffic.",
      }),
    ...
    expect(entries[0]).toMatchObject({
      ...
      target: {
        key: "FUTURE_CRAWLER_API_KEY",
        keysView: "tools",
        view: "keys",
      },
    });
    expect(
      filterSettingsSearchEntries(entries, "structured crawler"),
    ).toHaveLength(1);
    expect(filterSettingsSearchEntries(entries, "future_crawl")[0]?.id).toBe(
      "credential:FUTURE_CRAWLER_API_KEY",
    );
    expect(filterSettingsSearchEntries(entries, "gateway traffic")[0]?.id).toBe(
      "credential:FUTURE_GATEWAY_URL",
    );
  });
  ...
  it("shares the Tools and Settings category boundary with the rendered page", () => {
    ...
    expect(
      credentialSettingsView(envVar("messaging", { channel_managed: true })),
    ).toBeNull();
  ...
  it("uses AND matching across labels, context, descriptions, and raw keys", () => {
    ...
    expect(filterSettingsSearchEntries(entries, "brave tools")[0]?.id).toBe(
      "credential:BRAVE_SEARCH_API_KEY",
    );
    expect(
      filterSettingsSearchEntries(entries, "firecrawl extract")[0]?.id,
    ).toBe("credential:FIRECRAWL_API_KEY");
