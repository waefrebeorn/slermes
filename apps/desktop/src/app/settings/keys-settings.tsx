...
export function KeysSettings({ view }: KeysSettingsProps) {
  ...
  const entries = useMemo(() => {
    ...
    return Object.entries(vars)
      .filter(
        ([, info]) =>
          !info.channel_managed && cats.includes(asText(info.category)),
      )
  ...
  const deepLinkReady = useCallback(
    (key: string) => renderableKeys.has(key),
    [renderableKeys],
  );
      ...
      {entries.length > 0 ? (
        <div className="grid gap-2">
          {entries.map(([key, info]) => {
            ...
            return (
              <div
                className="scroll-mt-6 rounded-[6px]"
