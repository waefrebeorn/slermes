...
const KIND_ORDER: Record<PluginRecord["kind"], number> = {
  disk: 0,
  runtime: 1,
  bundled: 2,
};
...
async function revealPluginsDir() {
  try {
    ...
    if (!dir) {
      notifyError(
        "Desktop plugins are unavailable",
        "Could not resolve the plugins folder",
      );
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    const result = await request<{ home?: string }>("config.get", {
      key: "profile",
    });
    ...
    if (!home) {
      notifyError(
        "The backend did not report its home directory",
        "Could not open the plugins folder",
      );
...
function AgentPluginRowView({
  ...
  return (
    <PluginLine
      controls={
        key ? toggle : <Tip label={p.agent.updateBackendToManage}>{toggle}</Tip>
      }
      description={
        row.description || (row.version ? `v${row.version}` : undefined)
      }
...
function AgentPluginsSection() {
  ...
  const requestProfile =
    scopeOverride && scopeOverride !== activeProfile ? scopeOverride : null;
    ...
    >
      ...
      {profiles.length > 1 && (
        <div className="mb-2 flex items-center gap-2">
          ...
          <Select
            onValueChange={(name) =>
              setScopeOverride(name === activeProfile ? null : name)
            }
            value={scopeProfile ?? ""}
          >
