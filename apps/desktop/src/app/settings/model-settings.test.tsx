...
vi.mock("@/hermes", () => ({
  ...
  getGlobalModelOptions: (opts?: unknown, profile?: null | string) =>
  ...
  getRecommendedDefaultModel: (slug: string) =>
    getRecommendedDefaultModel(slug),
...
beforeEach(() => {
  ...
  getRecommendedDefaultModel.mockResolvedValue({
    provider: "nous",
    model: "hermes-4",
    free_tier: null,
  });
...
describe("ModelSettings profile scope", () => {
  ...
  it("follows the active profile (undefined, never null) when unscoped", async () => {
    ...
    await waitFor(() =>
      expect(getGlobalModelInfo).toHaveBeenCalledWith(undefined),
    );
...
describe("ModelSettings", () => {
  ...
  it("deep-links a known OAuth provider row into its setup flow", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "Set up Anthropic" }),
    );
  ...
  it("replaces the selected provider and model when the active profile changes", async () => {
    ...
    await renderModelSettings();
    expect((await screen.findAllByRole("combobox"))[0].textContent).toContain(
      "Custom A",
    );
    ...
    expect(
      screen.queryByRole("button", { name: "Set up provider" }),
    ).toBeNull();
  });
  ...
  it("preserves a user-defined provider endpoint when applying the main model", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("option", { name: "qwen3:latest" }),
    );
    ...
    await waitFor(() =>
      expect(saveHermesConfig).toHaveBeenCalledWith(
        expect.objectContaining({
          agent: expect.objectContaining({ service_tier: "fast" }),
        }),
      ),
    );
  });
  ...
  it("renders the auxiliary task rows", async () => {
    ...
    expect(screen.getAllByText("auto · use main model").length).toBeGreaterThan(
      0,
    );
  });
  ...
  it("assigns an auxiliary task to the main model via setModelAssignment", async () => {
    ...
    const setToMainButtons = await screen.findAllByRole("button", {
      name: "Set to main",
    });
...
describe("ModelSettings MoA preset editor", () => {
  ...
  it("saves once the model pick completes the slot", async () => {
    ...
    try {
      ...
      const sent = saveMoaModels.mock.calls[0][0] as ReturnType<
        typeof moaConfig
      >;
  ...
  it("saves a disabled reference model without removing it (per-slot enabled toggle)", async () => {
    ...
    try {
      ...
      fireEvent.click(
        screen.getByRole("switch", { name: "Disable reference 1" }),
      );
      await vi.advanceTimersByTimeAsync(700);
...
describe("ModelSettings code-skew 503", () => {
  ...
  it("unwraps the stale-backend 503 instead of dumping IPC JSON", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/running old code after an update/i),
      ).toBeTruthy();
    });
    expect(
      screen.getByRole("button", { name: "Restart backend" }),
    ).toBeTruthy();
  ...
  it("recycles the Desktop-owned backend and reloads the catalog", async () => {
    ...
    (
      window as unknown as {
        hermesDesktop: { recycleBackend: typeof recycleBackend };
      }
    ).hermesDesktop = {
      recycleBackend,
    };
    ...
    await waitFor(() =>
      expect(getGlobalModelOptions.mock.calls.length).toBeGreaterThan(1),
    );
  });
});
