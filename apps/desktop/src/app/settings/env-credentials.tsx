...
export function SettingsCategoryHeading({
  count,
  icon: Icon,
  title,
...
export function useEnvCredentials(profile?: string): UseEnvCredentials {
  ...
  function patchVar(
    key: string,
    patch: Partial<Pick<EnvVarInfo, "is_set" | "redacted_value">>,
  ) {
  ...
  async function handleSave(key: string) {
    ...
    try {
      ...
      notify({
        ...
        message: toolsets.savedMessage(key),
      });
    } catch (err) {
  ...
  async function saveValue(
    ...
    try {
      ...
      return {
        message: err instanceof Error ? err.message : credentials.couldNotSave,
        ok: false,
      };
    } finally {
      setSaving(null);
  ...
  async function handleClear(key: string) {
    if (
      !(await confirm({
        destructive: true,
        title: toolsets.removeConfirm(key),
      }))
    ) {
      return;
    }
