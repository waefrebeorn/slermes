...
vi.mock("@/hermes", () => ({
  ...
  getToolsetModels: (name: string, provider?: string) =>
    getToolsetModels(name, provider),
  selectToolsetModel: (name: string, model: string, provider?: string) =>
    selectToolsetModel(name, model, provider),
  ...
  runToolsetPostSetup: (name: string, key: string) =>
    runToolsetPostSetup(name, key),
  getActionStatus: (name: string, lines?: number) =>
    getActionStatus(name, lines),
  startOAuthLogin: (providerId: string) => startOAuthLogin(providerId),
  pollOAuthSession: (providerId: string, sessionId: string) =>
    pollOAuthSession(providerId, sessionId),
...
beforeEach(() => {
  ...
  selectToolsetModel.mockResolvedValue({
    ok: true,
    name: "image_gen",
    model: "z-image-turbo",
  });
...
describe("ToolsetConfigPanel", () => {
  it("renders inline voice/model fields for a TTS provider row carrying tts_provider", async () => {
    ...
    const saved = saveHermesConfig.mock.calls.at(-1)?.[0] as Record<
      string,
  ...
  it("expands a provider on row click and activates it via the explicit button", async () => {
    ...
    const elevenlabs = await screen.findByRole("button", {
      name: /ElevenLabs/,
    });
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: /Use this backend/ }),
    );

    await waitFor(() =>
      expect(selectToolsetProvider).toHaveBeenCalledWith("tts", "ElevenLabs"),
    );
  });
  ...
  it("serializes provider selection while a previous choice is pending", async () => {
    ...
    const useBackend = await screen.findByRole("button", {
      name: /Use this backend/,
    });
    ...
    await waitFor(() =>
      expect(screen.getByText("This is your active backend")).toBeTruthy(),
    );
  });
  ...
  it("runs a provider post-setup install hook and tails its log", async () => {
    ...
    runToolsetPostSetup.mockResolvedValue({
      ok: true,
      pid: 4321,
    ...
    getActionStatus
      ...
      .mockResolvedValue({
        exit_code: 0,
        lines: [
          "Installing Camofox browser server...",
          "Post-setup 'camofox' complete",
        ],
        name: "tools-post-setup",
        pid: 4321,
        running: false,
    ...
    await waitFor(() =>
      expect(runToolsetPostSetup).toHaveBeenCalledWith("browser", "camofox"),
    );
    ...
    await waitFor(
      () =>
        expect(getActionStatus).toHaveBeenCalledWith("tools-post-setup", 300),
      {
        timeout: 4000,
      },
    );
  ...
  it("surfaces a non-zero exit code from the setup process", async () => {
    ...
    await waitFor(
      () => expect(screen.getByText(/npm ERR! install failed/)).toBeTruthy(),
      {
        timeout: 4000,
      },
    );
  ...
  it("swaps the install hint for the installed one-liner when the provider is ready", async () => {
    ...
    expect(
      await screen.findByText(
        /Installed\. Re-run setup only if something is broken\./,
      ),
    ).toBeTruthy();
  ...
  describe("readiness pills", () => {
    ...
    it("flips a needs_keys provider to Ready locally after its key is saved", async () => {
      ...
      fireEvent.click(await screen.findByRole("menuitem", { name: "Set" }));
      fireEvent.change(
        await screen.findByPlaceholderText("ElevenLabs API key"),
  ...
  describe("post-setup installed state", () => {
    ...
    it("still runs the hook from the Re-run setup affordance", async () => {
      ...
      await waitFor(() =>
        expect(runToolsetPostSetup).toHaveBeenCalledWith(
          "browser",
          "agent_browser",
        ),
      );
  ...
  describe("managed Nous provider activation", () => {
    ...
    it("drives the existing Nous OAuth device-code flow from the sign-in action and refetches", async () => {
      ...
      try {
        ...
        await waitFor(() =>
          expect(startOAuthLogin).toHaveBeenCalledWith("nous"),
        );
        ...
        await waitFor(
          () => expect(pollOAuthSession).toHaveBeenCalledWith("nous", "sess-1"),
          { timeout: 8000 },
        );
  ...
  describe("API key deep link", () => {
    it('offers "Manage in API Keys" on a set key and navigates to Settings → Keys', async () => {
      ...
      fireEvent.click(
        await screen.findByRole("menuitem", { name: "Manage in API Keys" }),
      );

      await waitFor(() =>
        expect(navigateSpy).toHaveBeenCalledWith(
          "/settings?tab=keys&key=ELEVENLABS_API_KEY",
        ),
      );
    });
    ...
    it('hides "Manage in API Keys" while the key is unset', async () => {
      ...
      fireEvent.click(elevenLabs);
      await waitFor(() =>
        expect(elevenLabs.getAttribute("aria-expanded")).toBe("true"),
      );
      ...
      expect(
        screen.queryByRole("menuitem", { name: "Manage in API Keys" }),
      ).toBeNull();
    });
  });
  ...
  describe("web capability split", () => {
    ...
    it('hides "Use for Extract" on a search-only provider and wires capability selection', async () => {
      ...
      expect(
        await screen.findByRole("button", { name: "Use for Search" }),
      ).toBeTruthy();
      expect(
        screen.queryByRole("button", { name: "Use for Extract" }),
      ).toBeNull();
      ...
      const useForSearch = await screen.findByRole("button", {
        name: "Use for Search",
      });
      ...
      await waitFor(() =>
        expect(screen.getByText("Search: firecrawl")).toBeTruthy(),
      );
    });
    ...
    it("does not render capability chrome for non-web toolsets", async () => {
      ...
      expect(
        screen.queryByRole("button", { name: "Use for Search" }),
      ).toBeNull();
    });
  });
});
