...
vi.mock("../hooks/use-config-record", () => ({
  setHermesConfigCache: (config: Record<string, unknown>) =>
    mocks.cache(config),
...
describe("TerminalFontSetting", () => {
  ...
  it("selects MesloLGS NF and persists only the terminal font field", async () => {
    ...
    expect(
      (screen.getByLabelText("Glyph preview") as HTMLElement).style.fontFamily,
    ).toContain("MesloLGS NF");
  ...
  it("accepts an arbitrary CSS stack and resets to the bundled default", async () => {
    mocks.loadedConfig = {
      terminal: {
        backend: "local",
        font_family: "'Hack Nerd Font', monospace",
      },
    };
    render(<TerminalFontSetting />);
    const input = screen.getByRole("combobox", { name: "Terminal Font" });
    ...
    fireEvent.change(input, {
      target: { value: "'Custom Powerline', monospace" },
    });
    await flushAutosave();
    ...
    expect(mocks.save.mock.calls[0][0]).toMatchObject({
      terminal: {
        backend: "local",
        font_family: "'Custom Powerline', monospace",
      },
    });
