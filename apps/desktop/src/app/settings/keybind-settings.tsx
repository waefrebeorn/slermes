      ...
      {isSearching ? (
        <div className="px-2 py-1.5">
          {filteredActions?.length === 0 && filteredReadonly?.length === 0 ? (
            <p className="px-2.5 py-4 text-center text-[0.82rem] text-muted-foreground">
              —
            </p>
          ) : (
            <>
        ...
        <div className="px-2 py-1.5">
          {KEYBIND_CATEGORIES.map((category) => {
            ...
            const readonly = KEYBIND_READONLY.filter(
...
function KeybindRow({ action }: { action: KeybindActionMeta }) {
  ...
  const conflict = combos
    .flatMap((combo) =>
      conflictsFor(action.id, combo).map((other) => k.actions[other] ?? other),
    )
    .find(Boolean);

  return (
    <div className="group flex items-center gap-2.5 rounded-lg px-2.5 py-1 transition-colors hover:bg-(--chrome-action-hover)">
      <span className="min-w-0 flex-1 truncate text-[0.82rem] text-foreground/90">
        {label}
      </span>

      {conflict && (
        <span
          className="flex size-4 items-center justify-center text-amber-500/90"
...
function ReadonlyRow({ shortcut }: { shortcut: KeybindReadonly }) {
  ...
  return (
    <div className="flex items-center gap-2.5 rounded-lg px-2.5 py-1">
      <span className="min-w-0 flex-1 truncate text-[0.82rem] text-foreground/75">
        {label}
      </span>
