...
describe("BillingSettings", () => {
  it("renders the deployed-today payload with buy controls hidden and usage rows visible", async () => {
    ...
    expect(
      screen.getByText(
        "Charges $10 automatically when your balance falls below $5.",
      ),
    ).toBeTruthy();
    expect(screen.getByText("$120 of $220 left")).toBeTruthy();
    expect(screen.getByText("$876.47")).toBeTruthy();
    expect(
      screen.getByText("$10 of $100 used").classList.contains("tabular-nums"),
    ).toBe(true);
  ...
  it("renders the post-train payload with enabled buy controls and card provenance", async () => {
    apiMocks.fetchBillingState.mockResolvedValue(
      okBilling(postTrainBillingState),
    );
    apiMocks.fetchSubscriptionState.mockResolvedValue(
    ...
    expect(
      screen.getByRole("button", { name: "$25" }).hasAttribute("disabled"),
    ).toBe(false);
    expect(
      screen.getByRole("button", { name: "$50" }).hasAttribute("disabled"),
    ).toBe(false);
    expect(
      screen.getByRole("button", { name: "$100" }).hasAttribute("disabled"),
    ).toBe(false);
    expect(
      screen.getByRole("spinbutton", { name: "Custom credit amount" }),
    ).toBeTruthy();
    expect(
      screen.getByRole("button", { name: /^Buy$/ }).hasAttribute("disabled"),
    ).toBe(false);
  });
  ...
  it("disables buy controls when no card is on file", async () => {
    ...
    expect(
      screen
        .getByRole("spinbutton", { name: "Custom credit amount" })
  ...
  it("saves enabled auto-refill edits and refreshes billing state", async () => {
    ...
    fireEvent.change(
      screen.getByRole("spinbutton", { name: "Auto-refill threshold" }),
      {
        target: { value: "15" },
      },
    );
    fireEvent.change(
      screen.getByRole("spinbutton", { name: "Auto-refill reload-to amount" }),
      {
        target: { value: "20" },
      },
    );
  ...
  it("rejects auto-refill amounts outside the billing bounds", async () => {
    ...
    expect(
      screen.getByText(`Threshold: minimum is ${formatMoney(10)}.`),
    ).toBeTruthy();
    expect(
  ...
  it("renders the enabled auto-refill row without crashing when the card is null", async () => {
    apiMocks.fetchBillingState.mockResolvedValue(
      okBilling({
        ...todayBillingState,
        auto_reload: { ...todayBillingState.auto_reload, card: null },
      }),
    );
  ...
  it("opens auto-refill edit without a validation error even when the saved config is below the minimum", async () => {
    ...
    expect(
      screen.queryByText(`Threshold: minimum is ${formatMoney(10)}.`),
    ).toBeNull();
  ...
  it("runs an in-app downgrade: preview → confirm → schedule with the tier id → refetch → overview", async () => {
    ...
    await waitFor(() =>
      expect(apiMocks.previewSubscriptionChange).toHaveBeenCalledWith(
        "cltier000free0000personal",
      ),
    );
    ...
    expect(
      screen.getByText(/Monthly credits change: −\$88\/mo\./),
    ).toBeTruthy();

    fireEvent.click(screen.getByRole("button", { name: "Confirm downgrade" }));

    await waitFor(() =>
      expect(apiMocks.scheduleSubscriptionChange).toHaveBeenCalledWith(
        "cltier000free0000personal",
      ),
    );
    await waitFor(() =>
  ...
  it("surfaces the step-up affordance when scheduling a downgrade needs approval", async () => {
    ...
    expect(
      await screen.findByText("Remote Spending needs approval:"),
    ).toBeTruthy();
    expect(
  ...
  it("locks out the other downgrade tiles and Back while a schedule is in flight", async () => {
    ...
    apiMocks.fetchBillingState.mockResolvedValue(
      billingDevFixtures["subscriber-personal"].billing,
    );
    apiMocks.fetchSubscriptionState.mockResolvedValue(
      okSubscription({
        ...todaySubscriptionState,
    ...
    expect(
      screen
        .getByRole("button", { name: "Back to billing" })
        .hasAttribute("disabled"),
    ).toBe(true);
  ...
  it("keeps the auto-refill edit form mounted so the row height is reserved before editing", async () => {
    ...
    expect(
      screen.queryByRole("spinbutton", { name: "Auto-refill threshold" }),
    ).toBeNull();
    expect(
      screen.getByRole("spinbutton", {
        name: "Auto-refill threshold",
        hidden: true,
      }),
    ).toBeTruthy();
  ...
  it("keeps disabled auto-refill portal-only with no enable control", async () => {
    ...
    expect(
      screen.getByText("Turn on auto-refill from the portal"),
    ).toBeTruthy();
  ...
  it("disables buy controls while polling and renders the settled outcome", async () => {
    ...
    expect(
      await screen.findByText("Processing… checking settlement"),
    ).toBeTruthy();
    expect(
      screen.getByRole("button", { name: "$25" }).hasAttribute("disabled"),
    ).toBe(true);
    ...
    await waitFor(() =>
      expect(
        screen.getByText(`${formatMoney(25)} added. Balance is refreshing.`),
      ).toBeTruthy(),
    );
  });
  ...
  it("renders logged-out as a connect card without normal account rows", async () => {
    apiMocks.fetchBillingState.mockResolvedValue(
      okBilling(loggedOutBillingState),
    );
    apiMocks.fetchSubscriptionState.mockResolvedValue(
  ...
  it("renders danger value text for overdrawn subscription credits", async () => {
    ...
    expect(
      (
        await screen.findByText("$0 of $220 left · $0.79 over")
      ).classList.contains("text-destructive"),
    ).toBe(true);
    const subscriptionTrack = screen.getByRole("progressbar", {
      name: "Subscription credits remaining",
    });
  ...
  it("renders an empty neutral usage track when a row has no bar data", async () => {
    ...
    const subscriptionTrack = screen.getByRole("progressbar", {
      name: "Subscription credits usage",
    });

    expect(subscriptionTrack.getAttribute("aria-valuenow")).toBe("0");
    expect(subscriptionTrack.classList.contains("text-destructive")).toBe(
      false,
    );
    ...
    const monthlyCapTrack = screen.getByRole("progressbar", {
      name: "Monthly spend cap used",
    });
