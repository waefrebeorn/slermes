...
function DowngradeConfirm({
  ...
  const { phase } = active;
  const captionCn =
    "text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)";
  const refusal =
    phase.kind === "previewFailed" || phase.kind === "scheduleFailed"
      ? phase.refusal
      : null;
        ...
        {tier.state === "downgrade" &&
          (confirming ? (
            ...
            <Button
              ...
              onClick={() =>
                flow.begin({ tierId: tier.tierId, tierName: tier.name })
              }
              size="sm"
              type="button"
              variant="outline"
