...
export type BillingPlanCardView = {
  ...
  | { action: { label: string }; link?: undefined }
...
export function deriveBillingView(
  ...
  return {
    ...
    summary: [
      ...
      {
        label: "Auto-refill",
        tone: billing.auto_reload?.enabled
          ? "primary"
          : billing.auto_reload
            ? "muted"
            : undefined,
...
function refusalNotice(refusal: BillingRefusal): BillingNoticeView {
  ...
  const portalUrl =
    resolved.action.type === "portal" ? resolved.action.url : undefined;
...
): SubscriptionTierOption | undefined {
  const current = subscription?.current;

  return subscription?.tiers?.find(
    (tier) => tier.is_current || tier.tier_id === current?.tier_id,
  );
}
...
function plansCapable(
  ...
  return (
    subscription.context !== "team" && Boolean(subscription.can_change_plan)
  );
}
...
function creditsPerMonthDisplay(
  ...
  return Number.isFinite(credits) && credits > 0
    ? `$${credits.toLocaleString("en-US")} credits/mo`
    : undefined;
}
...
function derivePlanCard(
  ...
  const renewal = formatBillingDate(
  ...
  const hasActionableTier = tiers.some(
  ...
  return {
    ...
    link: {
      label: "Adjust plan ↗",
      url: buildManageSubscriptionUrl(
        subscription,
        subscription?.portal_url ?? billing.portal_url,
      ),
    },
    pending,
    price,
...
function pendingTransition(
  ...
  if (current?.pending_downgrade_tier_name && current.pending_downgrade_at) {
    return {
      ...
      when:
        current.pending_downgrade_display ??
        formatBillingDate(current.pending_downgrade_at),
    };
  }
  ...
  if (current?.cancel_at_period_end && current.cancellation_effective_at) {
    return {
      kind: "cancellation",
      when:
        current.cancellation_effective_display ??
        formatBillingDate(current.cancellation_effective_at),
    };
  }
...
function derivePlanTiers(
  ...
  const gridTiers = allTiers
    .filter(
      (tier) => tier.is_enabled || tier.tier_id === explicitCurrent?.tier_id,
    )
    .slice()
  ...
  const currentTier =
    explicitCurrent ?? (current == null ? gridTiers[0] : undefined);
...
function buyCreditsRow(billing: BillingStateResponse): BillingAccountRowView {
  if (!billing.card) {
    ...
    return {
      ...
      chips: billing.charge_presets.map((amount) => ({
        disabled: true,
...
function autoReloadRow(billing: BillingStateResponse): BillingAccountRowView {
  ...
  if (autoReload.card?.kind === "distinct") {
    ...
    const cardLabel =
      brand && last4 ? `${capitalize(brand)} ••${last4}` : "a different card";
    const portalUrl = billing.portal_url ?? FALLBACK_PORTAL_BILLING_URL;
