...
const PLATFORM_NOTE: Record<string, string> = {
  linux:
    "Drives your desktop via the X11/XWayland accessibility stack — no permission prompt.",
  win32:
    ...
    <div className="grid gap-2">
        ...
        <div className="min-w-0">
          {status.can_grant ? (
            <p className="text-[0.72rem] text-muted-foreground">
              Grants attach to CuaDriver&apos;s own identity
              (com.trycua.driver), not Hermes — so the dialog is attributed to
              the process that drives your Mac.
            </p>
          ) : (
            <p className="text-[0.72rem] text-muted-foreground">
      ...
      {status.ready ? (
        <div className="flex items-center gap-1.5 px-1 text-xs text-muted-foreground">
          ...
          Computer Use is ready. Ask the agent to capture an app and click
          around.
        </div>
      ) : (
        status.can_grant && (
