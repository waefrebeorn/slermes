...
beforeEach(() => {
  ...
  save.mockResolvedValue({
    connection: registry.connections[1],
    ok: true,
    registry,
  });
  remove.mockResolvedValue({
    ok: true,
    registry: { ...registry, connections: [registry.connections[0]] },
  });
  setLaunchMode.mockResolvedValue({
    ok: true,
    registry: { ...registry, launchMode: "last-used" },
  });
  setPrimary.mockResolvedValue({
    ok: true,
    registry: { ...registry, primary: "homelab" },
  });
...
describe("ConnectionsRegistrySection", () => {
  ...
  it("opens the add-connection editor and saves with a required label", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Homelab"), {
      target: { value: "Spark box" },
    });
  ...
  it("offers every kind on create and disables Local while the managed entry exists", async () => {
    ...
    const localKind = screen.getByRole("button", {
      name: "Local",
  ...
  it("rejects a duplicate gateway URL in the save path with an inline error", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(
          "A connection to this gateway URL already exists (“Homelab”).",
        ),
      ).toBeTruthy(),
    );
  ...
  it("lets users opt into restoring the last-used source", async () => {
    ...
    const launchSetting = await screen.findByText(
      "At startup, return to Sessions on the last-used gateway",
    );
    const addConnection = screen.getByText("Add connection");

    expect(
      addConnection.compareDocumentPosition(launchSetting) &
        Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    fireEvent.click(
      screen.getByRole("switch", {
        name: "At startup, return to Sessions on the last-used gateway",
      }),
    );

    await waitFor(() =>
      expect(setLaunchMode).toHaveBeenCalledWith("last-used"),
    );
  });
  ...
  it("offers the launch preference even for a single source", async () => {
    ...
    list.mockResolvedValueOnce({
      ...registry,
      connections: [registry.connections[0]],
    });
  ...
  it("keeps search out of the way for a small registry", async () => {
    ...
    await waitFor(() => expect(screen.getByText("Homelab")).toBeTruthy());
    expect(
      screen.queryByRole("searchbox", { name: "Search gateways…" }),
    ).toBeNull();
  });
...
describe("dedupe helpers", () => {
  it("normalizes gateway URLs (trim, trailing slashes, lowercase)", () => {
    expect(normalizeGatewayUrl(" HTTP://Homelab.LAN:9119// ")).toBe(
      "http://homelab.lan:9119",
    );
  });
  ...
  it("finds at most one local entry", () => {
    expect(
      findDuplicateConnection(
        { host: "", id: null, kind: "local", remoteProfile: "", url: "" },
