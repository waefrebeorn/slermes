...
const { requestGateway, getProfiles } = vi.hoisted(() => ({
  ...
  getProfiles: vi.fn<
    () => Promise<{ profiles: { name: string; is_default: boolean }[] }>
  >(async () => ({
    profiles: [],
  })),
}));
...
describe("PluginsSettings", () => {
  ...
  it("keeps duplicate-named keyless rows distinct (no React key collision)", () => {
    ...
    expect(
      screen.getAllByRole("switch", { name: "Enable Legacy plugin" }),
    ).toHaveLength(2);
  ...
  it("keeps using the canonical key when the backend provides one", async () => {
    ...
    requestGateway.mockResolvedValue({
      ok: true,
      plugin: { ...keyedRow, status: "enabled" },
    });

    renderSettings();
    fireEvent.click(
      screen.getByRole("switch", { name: "Enable Legacy plugin" }),
    );
  ...
  it("hides repo-bundled built-ins and keeps the count pill in sync", () => {
    ...
    $agentPlugins.set([
      ...
      },
      {
        ...legacyRow,
        name: "chronos",
        key: "cron_providers/chronos",
        source: "bundled",
      },
      {
        ...legacyRow,
        name: "deepinfra",
        key: "model-providers/deepinfra",
        source: "bundled",
      },
    ]);
  ...
  it("sends toggles through the selected profile scope", async () => {
    ...
    requestGateway.mockImplementation(
