...
vi.mock("@/store/onboarding", () => ({
  ...
  startManualProviderOAuth: (providerId: string) =>
    startManualProviderOAuth(providerId),
  startManualLocalEndpoint: (reason: null | string) =>
...
describe("ProvidersSettings", () => {
  it("disconnects a connected provider account and refreshes the accounts list", async () => {
    ...
    await waitFor(() =>
      expect(disconnectOAuthProvider).toHaveBeenCalledWith("nous"),
    );
  ...
  it("leaves the account connected when the removal prompt is dismissed", async () => {
    ...
    await act(async () => {
      fireEvent.click(
        await screen.findByRole("button", { name: "Remove Nous Portal" }),
      );
    });
  ...
  it("does not offer removal for externally managed providers", async () => {
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider("qwen-oauth", true, {
          ...
          disconnect_hint:
            "Use `hermes auth add qwen-oauth` or that provider's CLI to remove it.",
    ...
    expect(
      screen.queryByRole("button", { name: "Remove Qwen Code" }),
    ).toBeNull();
  ...
  it("orders API-key providers by priority then name, and filters them via search", async () => {
    ...
    const labels = screen
      .getAllByText(/Acme|Middle|Zebra/)
