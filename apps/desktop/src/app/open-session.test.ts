...
vi.mock("@/store/session-states", () => ({
  ...
  setSessionTileWorkspaceScope: (...args: unknown[]) =>
    setSessionTileWorkspaceScope(...args),
}));

vi.mock("@/store/windows", () => ({
  canOpenSessionWindow: () => canOpenSessionWindow(),
  openSessionInNewWindow: (...args: unknown[]) =>
    openSessionInNewWindow(...args),
}));
...
describe("openSessionIntentFromModifiers", () => {
  ...
  it("reads ⌘/⌃ as tab and ⇧+mod as window", () => {
    ...
    expect(
      openSessionIntentFromModifiers({ metaKey: true, shiftKey: true }),
    ).toBe("window");
    expect(openSessionIntentFromModifiers({ shiftKey: true })).toBe("in-place");
  });

  it("lets modifiers override the base", () => {
    expect(openSessionIntentFromModifiers({ metaKey: true }, "stack")).toBe(
      "tab",
    );
    expect(
      openSessionIntentFromModifiers(
