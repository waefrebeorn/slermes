...
describe("ModelMenuPanel current selection", () => {
  it("keeps the checkmark on the live SessionView model when a stale options response disagrees", async () => {
    ...
    const currentRow = (await content.findByText(/Gemini 3\.1 Pro/i)).closest(
      '[role="menuitem"]',
    );
    const staleRow = content
...
describe("ModelMenuPanel search", () => {
  ...
  const rowWithText = (
    content: ReturnType<typeof renderPanel>["content"],
    pattern: RegExp,
  ) =>
    content.queryByText(
      (_, element) =>
        element?.tagName === "SPAN" && pattern.test(element.textContent ?? ""),
    );
...
describe("ModelMenuPanel provider collapse", () => {
  ...
  it("toggles collapse via keyboard Enter on header", async () => {
    ...
    fireEvent.keyDown(header.closest('[role="menuitem"]') ?? header, {
      key: "Enter",
    });
...
describe("ModelMenuPanel refresh reconcile × guarded-switch confirm handshake", () => {
    ...
    return (
      <QueryClientProvider client={client}>
        <DropdownMenu open>
          <DropdownMenuContent>
            <ModelMenuPanel
              onSelectModel={controls.selectModel}
              requestGateway={requestGateway as never}
            />
          </DropdownMenuContent>
        </DropdownMenu>
  ...
  it("reconcile-triggered switch to a guarded model surfaces confirm, not a silent retry", async () => {
    ...
    getGlobalModelOptions
      .mockResolvedValueOnce({
        providers: [
          { models: ["glm-4.5-air"], name: "Zhipu", slug: "zhipu" },
          MOA_PROVIDER,
        ],
      })
    ...
    const requestGateway = vi.fn(
      async (method: string, _params?: Record<string, unknown>) => {
        ...
        if (configSets === 1) {
          return {
            confirm_message:
              "CONTRIBUTOR TIER: this model may train on your data.",
    ...
    const content = render(
      <ConfirmHarness requestGateway={requestGateway as never} />,
    );
    ...
    const configSetCalls = requestGateway.mock.calls.filter(
    ...
    const lastNotify = notify.mock.calls.at(-1)?.[0] as {
    ...
    await vi.waitFor(() => {
      const resend = requestGateway.mock.calls.filter(
        ([method]) => method === "config.set",
      );
      expect(resend).toHaveLength(2);
      expect(resend[1][1]).toMatchObject({
