...
export function ModelMenuPanel({
  ...
  const defaultEffort =
    useStore($defaultReasoningEffort) || DEFAULT_REASONING_EFFORT;
  ...
  const refreshModels = async () => {
    ...
    try {
      const queryKey = modelOptionsQueryKey(
        profile,
        activeSessionId,
        ownerConnectionId,
      );
  ...
  const controller: ModelMenuController = {
    ...
    presetFor: (provider, model) =>
      modelPresets[modelPresetKey(provider, model)] ?? {},
