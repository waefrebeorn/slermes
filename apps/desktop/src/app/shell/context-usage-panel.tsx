...
export function ContextUsagePanel({
  ...
  const contextPercent = Math.max(
    0,
    Math.min(100, Math.round(usage.context_percent ?? 0)),
  );
  ...
  const categories = useMemo(
    () =>
      (breakdown?.categories ?? []).map((category) => ({
        ...category,
        label:
          copy.categories[category.id as keyof typeof copy.categories] ??
          category.label,
      })),
  ...
  return (
    <div
      className="flex w-72 flex-col gap-3 p-3 text-[0.75rem]"
