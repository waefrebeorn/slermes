...
export type SetStatusbarItemGroup = (
  id: string,
  items: readonly StatusbarItem[],
  side?: StatusbarItemSide,
) => void;
...
function StatusbarHideHint() {
  ...
  return hint ? (
    <span className="ml-auto pl-2 text-(--ui-text-quaternary)">{hint}</span>
  ) : null;
}
...
const StatusbarItemView = memo(function StatusbarItemView({
  ...
  if (
    item.variant === "menu" &&
    (item.menuContent || !!item.menuItems?.length)
  ) {
          ...
          {item.menuContent
            ...
            : (item.menuItems ?? [])
                ...
                .map((menuItem) => (
                  <DropdownMenuItem
                    className={cn(
                      "gap-2 text-foreground focus:bg-accent [&_svg]:size-4",
                      menuItem.className,
                    )}
