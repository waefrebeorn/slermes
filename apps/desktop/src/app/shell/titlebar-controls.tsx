...
export type SetTitlebarToolGroup = (
  id: string,
  tools: readonly TitlebarTool[],
  side?: TitlebarToolSide,
) => void;
  ...
  return (
    <>
      ...
      <span
        className={cn(
          "relative hidden",
          modHeld && "group-hover/tool:inline-flex",
        )}
      >
        <TitlebarIcon name="layout" />
        <span className="absolute -bottom-1 -right-1.5 grid place-items-center rounded-full bg-(--ui-bg-chrome) p-px">
          <TitlebarIcon
            ...
            name="refresh"
            size={titlebarIconSizeCss(TITLEBAR_ICON_BADGE_SCALE)}
          />
        </span>
      </span>
    </>
...
export function TitlebarControls({
  ...
  const unreadHint = unreadBadge
    ? ` · ${t.titlebar.unreadSessions(unreadBadge)}`
    : "";
  ...
  const leftLabel = leftEdge.open
    ? t.titlebar.hideSidebar
    : t.titlebar.showSidebar;
  const rightLabel = rightEdge.open
