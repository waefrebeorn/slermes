...
import {
  StatusbarControls,
  type StatusbarItem,
} from "@/app/shell/statusbar-controls";
import {
  $statusbarHiddenIds,
  $statusbarVisible,
...
describe("reset to defaults", () => {
  it("puts a customized bar back to the shipped show/hide set", async () => {
    ...
    openContextMenu(statusbar);
    fireEvent.click(
      await screen.findByRole("menuitem", { name: /reset to defaults/i }),
    );
  ...
  it("enables the row as soon as one item differs, in either direction", async () => {
    ...
    $statusbarHiddenIds.set(
      STATUSBAR_HIDDEN_BY_DEFAULT.filter((id) => id !== "cron"),
    );
    openContextMenu(statusbar);
    expect(
      (
...
describe("whole-bar visibility", () => {
  it("hides the bar from the context menu, leaving the keybind as the way back", async () => {
    ...
    openContextMenu(statusbar);
    fireEvent.click(
      await screen.findByRole("menuitem", { name: /hide status bar/i }),
    );
  ...
  it("offers the hide row even when no item opted into the show/hide list", async () => {
    const statusbar = bar([
      { id: "plugin-thing", label: "Plugin thing", variant: "action" },
    ]);
