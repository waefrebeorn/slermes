...
export function useStatusbarItems({
  ...
  const subagentsRunning = useStoreSelector($subagentsBySession, (bySession) =>
    Object.values(bySession).reduce(
  ...
  const focusedTurnStartedAt = useStoreSelector(
    $focusedSessionState,
    (state) => state?.turnStartedAt ?? null,
  );
  ...
  const primaryFocused =
    !focusedStoredSessionId ||
    focusedStoredSessionId === selectedStoredSessionId;

  const activeSessionId = primaryFocused
  ...
  const currentUsage = primaryFocused
    ? primaryUsage
    : (focusedUsage ?? EMPTY_USAGE);

  const turnStartedAt = primaryFocused
    ? primaryTurnStartedAt
    : focusedTurnStartedAt;
  ...
  const focusedRowCwd = useStoreSelector($sessions, (sessions) => {
    ...
    const row = sessions.find((s) =>
      sessionMatchesStoredId(s, focusedStoredSessionId),
    );
  ...
  const contextItemHidden =
    useStore($statusbarHiddenIds).includes("context-usage");

  const { breakdown: contextBreakdown, loading: contextBreakdownLoading } =
    useContextBreakdown({
      busy,
  ...
  const backendVersionItem = useMemo<StatusbarItem | null>(() => {
    ...
    const applying =
      backendUpdateApply.applying || backendUpdateApply.stage === "restart";
    ...
    () => [
      {
        ...
        title: commandCenterOpen
          ? copy.closeCommandCenter
...
function StatusbarGatewaySwitcher() {
  ...
  return (
    <ConnectionSwitcher
      compact
      onConnect={() => navigate(`${SETTINGS_ROUTE}?tab=connections`)}
    />
  );
}
