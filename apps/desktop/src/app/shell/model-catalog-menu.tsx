...
export function ModelCatalogMenu({
  ...
  const loadingModels: Record<string, LocalModelLoadProgress> =
    localStatus.data?.loading ?? {};
  ...
  const moaPresets = useMemo(
    () =>
      includeMoa
        ? (providers?.find((p) => p.slug.toLowerCase() === "moa")?.models ?? [])
        : [],
  ...
  const shownDownloads = q
    ? downloads.filter((job) => (job.target || "").toLowerCase().includes(q))
    : downloads;
  const hasLocalGroup = pickerProviders.some(
    (provider) => provider.slug === LOCAL_PROVIDER_SLUG,
  );
  ...
  const shownMoaPresets = useMemo(
    () =>
      q
        ? moaPresets.filter((preset) =>
            `moa ${preset}`.toLowerCase().includes(q),
          )
        : moaPresets,
    [moaPresets, q],
  );
  ...
  const selectFamily = async (
    ...
    const targetId =
      variantFast && preset.fast === true ? family.fastId! : family.id;
  ...
  const kbRows = useMemo<KbRow[]>(
    () => [
      ...
      ...shownMoaPresets.map((preset): KbRow => ({
        key: `moa:${preset}`,
        kind: "moa",
        preset,
      })),
  ...
  const autoIndex = q
    ...
    : kbRows.findIndex(
        (row) =>
          rowIsCurrent(row) ||
          (row.kind === "family" && row.family.fastId === current.model),
      );

  const kbIndex =
    kbOverride !== null && kbOverride < kbRows.length ? kbOverride : autoIndex;
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <div
          className={cn(
            "max-h-[max(150px,30dvh)] overflow-y-auto py-0.5",
            quietRows,
          )}
          ref={listRef}
        >
              ...
              <DropdownMenuGroup className="py-0.5" key={slug}>
                ...
                {!collapsed &&
                  group.families.map((family) => {
                    ...
                    const loadProgress =
                      loadingModels[family.id] ??
                      (family.fastId
                        ? loadingModels[family.fastId]
                        : undefined);
                    ...
                    const preset = controller.presetFor(
                      group.provider.slug,
                      family.id,
                    );
                    ...
                    const meta = [
                      fastControl.kind !== "none" && fastControl.on
                        ? copy.fast
                        : null,
                      (caps?.reasoning ?? true)
                      ...
                      >
                            ...
                            >
                              <span className="h-1 w-14 overflow-hidden rounded-full bg-(--ui-bg-tertiary)">
                                <span
                                  ...
                                  style={{
                                    width: `${Math.max(2, loadProgress.percent)}%`,
                                  }}
                                />
                              </span>
                        ...
                        <ModelEditSubmenu
                          ...
                          onSelectModel={(nextModel) =>
                            controller.select(nextModel, group.provider.slug)
                          }
                          onSetOptions={(patch) =>
      ...
      {shownMoaPresets.length > 0 ? (
        <div className={cn(quietRows)}>
          ...
          {shownMoaPresets.map((preset) => {
            const isCurrentMoa =
              current.provider === "moa" && current.model === preset;
...
function groupModels(
  ...
  for (const provider of providers) {
    ...
    if (q) {
      ...
      shown = new Set(
        allFamilies
          .slice(0, DEFAULT_VISIBLE_PER_PROVIDER)
          .map((family) => family.id),
      );
    }
    ...
    const activeId =
      !q && isCurrentProvider(provider, current.provider) && current.model
        ? allFamilies.find(
            (family) =>
              family.id === current.model || family.fastId === current.model,
          )?.id
        : undefined;

    const families = allFamilies.filter(
      (family) => shown.has(family.id) || family.id === activeId,
    );
