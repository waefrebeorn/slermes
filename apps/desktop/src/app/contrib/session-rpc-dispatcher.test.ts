...
const probe = vi.hoisted(() => ({
  resolveSessionOwner: vi.fn(async () => undefined as unknown),
}));
const sessionMocks = vi.hoisted(() => ({ requestSessionResume: vi.fn() }));

vi.mock(
  "@/app/session/hooks/use-session-actions/utils",
  async (importActual) => ({
    ...(await importActual<Record<string, unknown>>()),
...
const { $removedSessionIds, $sessionMutationsInFlight } =
  await import("@/store/session-removal");
...
const { isSessionOwnerResolutionError } =
  await import("@/store/session-owner-resolution");
  ...
  return {
    ...
    request: createSessionRpcDispatcher({
      ...
      runtimeIdByStoredSessionIdRef: {
        current: new Map([["stored-omar", "rt-omar"]]),
      },
...
describe("createSessionRpcDispatcher: fail closed", () => {
  it("rejects with an explicit owner-resolution error instead of riding the ambient socket", async () => {
    ...
    await expect(
      request("prompt.submit", { session_id: "rt-orphan", text: "hi" }),
  ...
  it("keeps the legacy single-backend Desktop on the ambient socket: no registry source, one profile", async () => {
    ...
    await expect(
      request("session.resume", { session_id: "stored-legacy" }),
    ).resolves.toEqual({ ambient: true });
    expect(ambientRequest).toHaveBeenCalledWith("session.resume", {
  ...
  it("fails closed as soon as there is somewhere to misroute to: a second profile, or a live registry source", async () => {
    ...
    $profiles.set([{ name: "default" }, { name: "omar" }] as never);
    await expect(
      dispatcher().request("session.resume", { session_id: "stored-x" }),
    ).rejects.toSatisfy(isSessionOwnerResolutionError);
...
describe("createSessionRpcDispatcher: exact owner rungs", () => {
  it("routes by the connection-tagged row when the hint is gone (runtime id translated to the stored id)", async () => {
    ...
    await expect(
      request("prompt.submit", { session_id: "rt-omar", text: "again" }),
  ...
  it("prefers the exact hint over an untagged row profile, and the probe result over nothing", async () => {
    ...
    setSessionOwnerHint("stored-omar", {
      connectionId: "local",
      profile: "omar",
    });

    await expect(
      dispatcher().request("session.interrupt", { session_id: "rt-omar" }),
    ).resolves.toEqual({
      routed: true,
    });
    ...
    _resetSessionOwnerHintsForTests();
    setSessions([]);
    probe.resolveSessionOwner.mockResolvedValue({
      connectionId: "homelab",
      profile: "worker",
    });

    await expect(
      dispatcher().request("session.activate", { session_id: "stored-hidden" }),
    ).resolves.toEqual({
      routed: true,
    });
    expect(gatewayMocks.requestGatewayForAgent).toHaveBeenLastCalledWith(
  ...
  it("resolves owners from the cron and messaging sidebar slices, not just recents (cron approval.respond)", async () => {
    ...
    setCronSessions([
      makeSessionInfo({ id: "stored-cron", profile: "omar", source: "cron" }),
    ]);
    const { ambientRequest, request } = dispatcher();
    ...
    await expect(
      request("prompt.submit", { session_id: "stored-tg", text: "hi" }),
    ).resolves.toEqual({ routed: true });
    expect(gatewayMocks.requestGatewayForAgent).toHaveBeenLastCalledWith(
      "homelab",
      "bots",
...
describe("createSessionRpcDispatcher: stale runtime recovery", () => {
  it("requests a durable rebind for the visible session after a structured 4001", async () => {
    ...
    await expect(
      request("process.list", { session_id: "rt-omar" }),
    ).rejects.toThrow("runtime was reaped");

    expect(sessionMocks.requestSessionResume).toHaveBeenCalledWith(
      "stored-omar",
      {
        connectionId: "local",
        profile: "omar",
  ...
  it.each([
    ...
    "still reports the 4001 for a selected session that is %s",
    async (_state, sessions) => {
  ...
  it("does not interpret an unrelated coded RPC failure as a stale runtime", async () => {
    ...
    gatewayMocks.requestGatewayForAgent.mockRejectedValueOnce(
      Object.assign(new Error("tool output says session not found"), {
        code: 5007,
      }),
    );
    const { request } = dispatcher(undefined, "stored-omar");
    ...
    await expect(
      request("process.list", { session_id: "rt-omar" }),
    ).rejects.toThrow("tool output says session not found");
  ...
  it("leaves the warm resume lifecycle to recover its own session.activate failure", async () => {
    ...
    await expect(
      request("session.activate", { session_id: "rt-omar" }),
    ).rejects.toThrow("session not found");
