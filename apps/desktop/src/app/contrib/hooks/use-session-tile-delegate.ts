...
interface SessionTileDelegateParams {
  ...
  executeSlashCommand: ReturnType<
    typeof usePromptActions
  >["executeSlashCommand"];
...
export function useSessionTileDelegate({
  ...
  useEffect(() => {
    ...
    const rebindTileRuntime =
      (deadRuntimeId: string) => (recoveredId: string) => {
    ...
    setSessionTileDelegate({
      ...
      interruptSession: async (runtimeId) => {
        ...
        await withSessionNotFoundResume(
          ...
          storedSessionId,
          (liveId) =>
            routedRequest("session.interrupt", { session_id: liveId }),
          {
      ...
      resumeTile: async (storedSessionId, options) => {
        ...
        const cached = existing
          ? sessionStateByRuntimeIdRef.current.get(existing)
          : undefined;
        ...
        const prefetchPromise = getLatestSessionMessages(
          storedSessionId,
          restScope,
        ).catch(() => null);
        ...
        const outcome = await resumeWithStoredTranscriptFallback(
          ...
          () => {
            ...
            return singleFlightSessionResume(storedSessionId, () =>
              requestForSessionProfile<SessionResumeResponse>(
                owner,
                requestGateway,
                "session.resume",
                {
          ...
          async () => {
            ...
            if (!stored) {
              throw new Error(
                "stored transcript unavailable on every reachable backend",
              );
            }
            ...
            (state) => ({
              ...
              messages:
                state.messages.length > 0
                  ? state.messages
                  : toChatMessages(outcome.transcript?.messages ?? []),
            }),
            storedSessionId,
          );
      ...
      submitToSession: async (runtimeId, text) => {
        ...
        if (isReadOnlyRuntimeId(runtimeId)) {
          notify({
            kind: "info",
            message: translateNow("desktop.readOnlyTranscriptSendBlocked"),
          });
