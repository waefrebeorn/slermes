...
vi.mock("@/hermes", async (importActual) => ({
  ...
  getLatestSessionMessages: vi.fn(async () => ({
    messages: [],
    session_id: "",
  })),
}));
...
const { requestGatewayForAgent, requestGatewayForProfile } =
...
describe("useSessionTileDelegate resumeTile", () => {
  ...
  it("reuses a warm binding that still carries a transcript", async () => {
    ...
    const runtimeIdByStoredSessionIdRef = {
      current: new Map([["stored-a", "runtime-a"]]),
    };
    const sessionStateByRuntimeIdRef = {
      current: new Map([["runtime-a", stateA]]),
    };
    const requestGateway = vi.fn(async () => ({}) as never);
  ...
  it("merges persisted messages into a warm tile on explicit reopen (#96183)", async () => {
    const stateA = {
      busy: false,
      messages: [
        { id: "m1", parts: [{ type: "text", text: "old" }], role: "user" },
      ],
      storedSessionId: "stored-a",
    };
    ...
    const updater = updateSessionState.mock.calls[0][1] as (
      state: typeof stateA,
    ) => {
    ...
    const texts = next.messages.flatMap((message) =>
  ...
  it("falls through to a real resume when the warm binding has no transcript (post-wake empty tile)", async () => {
    ...
    const runtimeIdByStoredSessionIdRef = {
      current: new Map([["stored-b", "runtime-dead"]]),
    };
    const sessionStateByRuntimeIdRef = {
      current: new Map([["runtime-dead", staleState]]),
    };
  ...
  it("hydrates the tile model and provider from resume info", async () => {
    ...
    const updater = updateSessionState.mock.calls[0][1] as (state: {
      messages: unknown[];
  ...
  it("invalidateRuntimeBindings clears the stored→runtime map so tiles re-resume after reconnect", async () => {
    ...
    const runtimeIdByStoredSessionIdRef = {
      current: new Map([["stored-c", "runtime-dead"]]),
    };
    const sessionStateByRuntimeIdRef = {
      current: new Map([["runtime-dead", liveState]]),
    };
...
describe("useSessionTileDelegate retireBusyClaim", () => {
  it("retires a stale busy claim through the session-state write path (#93059)", () => {
    ...
    const sessionStateByRuntimeIdRef = {
      current: new Map([["runtime-dead", busyState]]),
    };
    const updateSessionState = vi.fn();
  ...
  it("reports a miss instead of minting a cache entry for a runtime it never held", () => {
    ...
    const idle = {
      ...
      busy: false,
      messages: [{ id: "m1" }],
      storedSessionId: "stored-e",
    };
    const sessionStateByRuntimeIdRef = {
      current: new Map([["runtime-idle", idle]]),
    };
    const updateSessionState = vi.fn();
    ...
    expect(sessionTileDelegate()!.retireBusyClaim!("runtime-unknown")).toBe(
      false,
    );
...
describe("useSessionTileDelegate interruptSession", () => {
  ...
  afterEach(async () => {
    setSessions([]);
    const { clearSessionRecentlyInterrupted } =
      await import("../../session/hooks/use-prompt-actions/utils");
  ...
  it("marks the session recently interrupted so a quick tile edit/resend still interrupt-firsts (#83855)", async () => {
    const { isSessionRecentlyInterrupted } =
      await import("../../session/hooks/use-prompt-actions/utils");
