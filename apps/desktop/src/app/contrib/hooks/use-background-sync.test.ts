...
function makeRefresh(
  ...
  const updateSessionStateRef = {
    updateSessionState: vi.fn(
      (sessionId: string, updater: (value: typeof state) => typeof state) => {
        const next = updater(
          states.get(sessionId) ?? createClientSessionState(ACTIVE_STORED_ID),
        );
...
function useSyncHarness({
  ...
  const updateSessionState: Parameters<
    typeof useBackgroundSync
  >[0]["updateSessionState"] = vi.fn((sessionId, updater) => {
...
describe("active transcript refresh", () => {
  ...
  it("refreshes a hidden session through its unique complete owner route", async () => {
    ...
    renderSync(fixture.refresh, {
      activeStoredSessionId: hiddenStoredSessionId,
    });
    ...
    expect(
      fixture.states.get(ACTIVE_RUNTIME_ID)?.messages.at(-1)?.parts[0],
    ).toMatchObject({
  ...
  it("reconciles a workspace TILE transcript when sessions.changed ticks (#94255 review: behavior, not source-grep)", async () => {
    ...
    const updateSessionState: Parameters<
      typeof reconcileTileTranscriptsForTest
    >[0]["updateSessionState"] = vi.fn((sessionId, updater) => {
      updaterCallCount += 1;
      const current = {} as Parameters<typeof updater>[0];
        ...
        if (storedId === TILE_STORED_ID) {
          return {
            messages: [
              ...
              {
                content: "background delivery answer",
                role: "assistant",
                timestamp: 2,
              },
            ],
    ...
    await act(async () => {
      await reconcileTileTranscriptsForTest({
        tiles: [
          { storedSessionId: TILE_STORED_ID, runtimeId: TILE_RUNTIME_ID },
        ],
        requestSequenceRef,
        signatureRef,
        updateSessionState,
  ...
  it("reconciles an idle tile while the main pane is busy", async () => {
    ...
    vi.mocked(getLatestSessionMessages).mockResolvedValue(
      transcript("idle tile update", storedId) as never,
    );

    const updateSessionState = vi.fn(
      (
        sessionId: string,
        updater: (state: typeof idleState) => typeof idleState,
      ) => {
  ...
  it("isolates tile transcript reads by connection and profile while preserving the legacy local path", async () => {
    vi.mocked(getLatestSessionMessages).mockImplementation(
      async (storedId) => transcript(storedId, storedId) as never,
    );

    const updateSessionState: Parameters<
      typeof reconcileTileTranscriptsForTest
    >[0]["updateSessionState"] = vi.fn((_sessionId, updater) =>
      updater({} as Parameters<typeof updater>[0]),
    );
  ...
  it("refreshes a local/Desktop session when sessions.changed ticks", async () => {
    ...
    const fixture = makeRefresh(resolveActiveTranscriptSession);
    vi.mocked(getLatestSessionMessages).mockResolvedValue(
      transcript("external answer") as never,
    );
  ...
  it("pulls the open transcript once per (re)connect, not on session switches (#94779)", () => {
    ...
    rerender({
      ...
      activeStoredSessionId: "stored-other",
      gatewayState: "open",
    });
    expect(refresh).toHaveBeenCalledTimes(1);
...
describe("reconcileActiveTranscript", () => {
  it("resolves and hydrates a messaging session from the messaging sessions store", async () => {
    ...
    const fixture = makeRefresh(resolveActiveTranscriptSession);
    vi.mocked(getLatestSessionMessages).mockResolvedValue(
      transcript("telegram answer") as never,
    );
  ...
  it("resolves and hydrates a cron session from the cron sessions store", async () => {
    ...
    const fixture = makeRefresh(resolveActiveTranscriptSession);
    vi.mocked(getLatestSessionMessages).mockResolvedValue(
      transcript("cron progress") as never,
    );
  ...
  it("publishes changed authoritative messages once without duplicates", async () => {
    ...
    vi.mocked(getLatestSessionMessages).mockResolvedValue(
      transcript("new answer") as never,
    );
...
describe("windowIsActivelyViewed", () => {
  it("requires both DOM visibility and keyboard focus", () => {
    expect(
      windowIsActivelyViewed({ focused: true, visibilityState: "visible" }),
    ).toBe(true);
    expect(
      windowIsActivelyViewed({ focused: false, visibilityState: "visible" }),
    ).toBe(false);
    expect(
      windowIsActivelyViewed({ focused: true, visibilityState: "hidden" }),
    ).toBe(false);
  });
});
...
describe("rehydrateLiveSessionStatuses", () => {
  ...
  it("restores a waiting turn as working and needing attention", () => {
    rehydrateLiveSessionStatuses({
      sessions: [
        {
          id: "runtime-needs-user",
          session_key: "needs-user",
          status: "waiting",
        },
      ],
    });
