...
const { hudWindowMock } = vi.hoisted(() => ({
...
describe("useDesktopIntegrations", () => {
  ...
  describe("profile-ready gate", () => {
    it("does NOT restore before profileReady is true", () => {
      ...
      window.localStorage.setItem(
        "hermes.desktop.lastRoute.profile.default",
        "/remembered-session",
      );
      window.localStorage.setItem(
        "hermes.desktop.lastSessionId.profile.default",
        "remembered-session",
      );
    ...
    it("restores on profileReady when remembered route exists and owns the session", () => {
      ...
      const sessions = [
        session({ id: "remembered-session", profile: "default" }),
      ];

      render({ profileReady: true, sessions });

      expect(navigate).toHaveBeenCalledWith("/remembered-session", {
        replace: true,
      });
    });
    ...
    it("waits for sessions before validating a remembered session route", () => {
      ...
      expect(navigate).not.toHaveBeenCalled();
      expect(
        window.localStorage.getItem("hermes.desktop.lastRoute.profile.default"),
  ...
  describe("display.resume_last_session", () => {
    it("stays on the fresh chat when the setting is off, and keeps remembering the open chat", () => {
      ...
      result.rerender({
        ...
        sessions: [
          ...sessions,
          session({ id: "other-session", profile: "default" }),
        ],
      });
  ...
  describe("ownership validation", () => {
    ...
    it("clears stale remembered route owned by wrong profile", () => {
      window.localStorage.setItem(
        "hermes.desktop.lastRoute.profile.ai-engineer",
        "/ai-session",
      );
  ...
  describe("two profiles with distinct sessions", () => {
    it("restores profile A session when profile A is active", () => {
      window.localStorage.setItem(
        "hermes.desktop.lastRoute.profile.coder",
        "/coder-session",
      );
      ...
      expect(navigate).toHaveBeenCalledWith("/coder-session", {
        replace: true,
      });
    });
  ...
  describe("legacy key behavior", () => {
    it("discards legacy global keys on read and does NOT restore from them", () => {
      ...
      expect(
        window.localStorage.getItem("hermes.desktop.lastSessionId"),
      ).toBeNull();
      expect(
        window.localStorage.getItem("hermes.desktop.lastRoute"),
      ).toBeNull();
  ...
  describe("stale-result suppression during profile switch", () => {
    it("remembers route for the new profile after switch, not the old one", () => {
      ...
      expect(
        window.localStorage.getItem(
          "hermes.desktop.lastSessionId.profile.coder",
        ),
      ).toBe("coder-session");
      ...
      expect(
        window.localStorage.getItem("hermes.desktop.lastSessionId.profile.ops"),
      ).toBe("ops-session");
  ...
  describe("notification activate + plugin deep links", () => {
    it("navigates when a plugin notification activate payload arrives", () => {
      ...
      desktopWindow.hermesDesktop = {
        ...
        onNotificationActivate: (
          cb: (payload: { activate?: string }) => void,
        ) => {
          activate = cb;
    ...
    it("routes hermes://mcp/install to the pending-install dialog, not navigation", () => {
      ...
      expect(requestMcpInstallFromDeepLink).toHaveBeenCalledWith({
        name: "context7",
      });
      expect(navigate).not.toHaveBeenCalled();
    });
