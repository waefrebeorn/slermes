...
vi.mock("@/hermes", () => ({
  ...
  getMessagingPlatforms: (profile?: null | string) =>
    getMessagingPlatforms(profile),
...
function platform(
  patch: Partial<MessagingPlatformInfo> = {},
...
describe("MessagingView profile scope", () => {
  it("follows the active profile instead of targeting primary when there is no override", async () => {
    ...
    await waitFor(() =>
      expect(getMessagingPlatforms).toHaveBeenCalledWith(undefined),
    );
...
describe("MessagingView setup-guide link", () => {
  it("hides the setup-guide button for a plugin platform with no docs URL", async () => {
    ...
    getMessagingPlatforms.mockResolvedValue({
      platforms: [platform({ docs_url: "" })],
    });

    await renderMessaging();

    expect(
      (await screen.findAllByText("Microsoft Teams")).length,
    ).toBeGreaterThan(0);
  ...
  it("opens a real docs URL through the validated external opener", async () => {
    ...
    getMessagingPlatforms.mockResolvedValue({
      platforms: [platform({ docs_url: docsUrl })],
    });
...
describe("MessagingView pairing", () => {
  ...
  it("approves the listed request by its request id, never by a code", async () => {
    ...
    approvePairing.mockResolvedValue({
      ok: true,
      user: { user_id: "7712345", user_name: "Bee" },
    });
  ...
  it("refetches pending rows on pairing.changed, not on platforms.changed", async () => {
    ...
    const { $changeEventsAvailable, $pairingChangeTick, $platformsChangeTick } =
      await import("@/store/live-sync");
