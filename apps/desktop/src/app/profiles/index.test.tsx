...
describe("ProfilesView", () => {
  ...
  it("re-homes to default when the active profile is deleted", async () => {
    ...
    const retireLocalProfileGatewaysMock = vi.mocked(
    ...
    await waitFor(() =>
      expect(deleteProfile).toHaveBeenCalledWith(NAMED_PROFILE),
    );
    expect(retireLocalProfileGateways).toHaveBeenCalledWith(NAMED_PROFILE);
    expect(
      retireLocalProfileGatewaysMock.mock.invocationCallOrder[0],
    ).toBeLessThan(deleteProfileMock.mock.invocationCallOrder[0]);
