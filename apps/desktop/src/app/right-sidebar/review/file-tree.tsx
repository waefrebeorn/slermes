    ...
    >
      ...
      >
        ...
        <span className="flex min-w-0 flex-1 items-baseline gap-1.5">
          ...
          {node.dir && (
            <span
              className="min-w-0 shrink-[9999] truncate text-[0.68rem] text-(--ui-text-tertiary)"
              title={node.dir}
            >
              {node.dir}
            </span>
          ...
          <Tip label={file.staged ? c.unstage : c.stage}>
            <Button
              ...
              onClick={(event) => {
                ...
                void (file.staged
                  ? unstageReviewFile(file.path)
                  : stageReviewFile(file.path));
              }}
              size="icon-xs"
              variant="ghost"
            >
        ...
        {file.staged && (
          <span
            aria-hidden
            className="size-1.5 shrink-0 rounded-full bg-(--ui-green)/70"
            title={c.staged}
          />
        )}
      </div>
...
function ReviewFileContextMenu({
  ...
  return (
    <ContextMenu>
      ...
      <ContextMenuContent>
        <ContextMenuItem onSelect={onOpenChanges}>
          {c.openChanges}
        </ContextMenuItem>
        ...
        <ContextMenuSeparator />
        <ContextMenuItem onSelect={() => revealFileInTree(dragPath)}>
          {m.revealInSidebar}
        </ContextMenuItem>
        {localFs && (
        ...
        <ContextMenuSeparator />
        <ContextMenuItem onSelect={() => void copyFilePath(dragPath)}>
          {m.copyPath}
        </ContextMenuItem>
        {cwd && (
          <ContextMenuItem
            onSelect={() => void copyFilePath(toRelativePath(dragPath, cwd))}
          >
            {m.copyRelativePath}
          </ContextMenuItem>
        )}
        {shouldOfferRemoteFileDownload(false) && (
          <>
            <ContextMenuSeparator />
            <ContextMenuItem onSelect={() => void downloadRemoteFile(dragPath)}>
              {m.download}
            </ContextMenuItem>
          </>
        )}
