...
describe("buildReviewTree", () => {
  it("nests files under their folders and sorts dirs before files", () => {
    const tree = buildReviewTree(
      [file("src/a.ts"), file("readme.md"), file("src/b.ts")],
      false,
    );
  ...
  it("aggregates +/- onto directories", () => {
    const tree = buildReviewTree(
      [file("src/a.ts", 5, 2), file("src/b.ts", 3, 1)],
      false,
    );
  ...
  it("does not compact when a directory has multiple children", () => {
    const tree = buildReviewTree(
      [file("a/b/one.ts"), file("a/other.ts")],
      true,
    );

    expect(tree[0].name).toBe("a");
    expect(tree[0].children?.map((n) => n.name).sort()).toEqual([
      "b",
      "other.ts",
    ]);
  });
...
describe("countAllNodes", () => {
  ...
  it("counts a single deep folder holding thousands of files as heavy", () => {
    const files = Array.from({ length: 40_000 }, (_, i) =>
      file(`publish/lib-${i}.so`),
    );
