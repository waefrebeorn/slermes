...
describe("readProjectDir", () => {
  ...
  it("filters gitignored entries when readDir returns Windows-style paths", async () => {
    ...
    readDir.mockImplementation(async (path) => {
      if (path === "C:\\repo\\src") {
        return ok([
          {
            name: "debug.log",
            path: "C:\\repo\\src\\debug.log",
            isDirectory: false,
          },
          {
            name: "临时.txt",
            path: "C:\\repo\\src\\临时.txt",
            isDirectory: false,
          },
          {
            name: "keep.ts",
            path: "C:\\repo\\src\\keep.ts",
            isDirectory: false,
          },
        ]);
      }
    ...
    readFileDataUrl.mockResolvedValue(
      dataUrl("# Unicode 路径规则\nsrc/*.log\nsrc/临时.txt\n"),
    );
