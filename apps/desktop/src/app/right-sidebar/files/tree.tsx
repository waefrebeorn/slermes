...
function ProjectTreeRowContainer({
  attrs,
  children,
  innerRef,
  node,
...
function ProjectTreeRow({
  ...
  const row = (
    <div
      ...
      onDragStart={(event) => {
        ...
        const payload = JSON.stringify([
          { isDirectory: isFolder, path: node.data.id },
        ]);
    ...
    >
      ...
      <span
        aria-hidden
        className="flex w-3.5 items-center justify-center text-(--ui-text-tertiary)"
      >
        {isPlaceholder && !isErrorPlaceholder ? (
          ...
          <Codicon
            name={node.isOpen ? "folder-opened" : "folder"}
            size="0.875rem"
          />
        ) : (
      ...
      {editing ? (
        ...
        <span
          className={cn(
            ...
            changeKind && CHANGE_TINT[changeKind],
          )}
        >
          {node.data.name}
        </span>
