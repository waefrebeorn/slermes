...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  try {
    ...
    if (
      !listing.entries.some((e) => e.name === ".gitignore" && !e.isDirectory)
    ) {
      return null;
    }

    const text = decodeDataUrl(
...
async function filterIgnored(
  ...
  const rules = (
    await Promise.all(ancestorDirs(root, dirPath).map(gitignoreFor))
  ).filter((r): r is GitignoreRule => Boolean(r));

  return rules.length > 0
    ? entries.filter((entry) => !ignoredBy(rules, entry))
    : entries;
}
...
export async function readProjectDir(
  ...
  const entries = (result?.entries ?? []).filter(
    (entry) => !ALWAYS_EXCLUDED.has(entry.name),
  );

  return {
    ...result,
    entries: await filterIgnored(entries, rootPath, dirPath),
  };
}
