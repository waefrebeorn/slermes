...
beforeEach(() => {
  ...
  readDir.mockReset();
  (
    window as unknown as { hermesDesktop: { readDir: typeof readDir } }
  ).hermesDesktop = { readDir };
});
...
describe("useProjectTree", () => {
  ...
  it("loads root entries on mount and sorts folders before files", async () => {
    ...
    expect(result.current.data.map((n) => n.name)).toEqual([
      "README.md",
      "src",
    ]);
    // Folder children start undefined (lazy load on first expand).
    expect(
      result.current.data.find((n) => n.name === "src")?.children,
    ).toBeUndefined();
    expect(result.current.data.find((n) => n.name === "src")?.isDirectory).toBe(
      true,
    );
    expect(
      result.current.data.find((n) => n.name === "README.md")?.isDirectory,
    ).toBe(false);
  });
  ...
  it("does not fall back after a failed root read from a superseded connection", async () => {
    ...
    const sanitizeWorkspaceCwd = vi.fn(async () => ({
      cwd: "/fallback",
      sanitized: true,
    }));
    ...
    readDir.mockResolvedValueOnce(
      ok([{ name: "from-b", path: "/shared/from-b", isDirectory: false }]),
    );
  ...
  it("clears root loading and recovers when a root read rejects", async () => {
    ...
    readDir.mockResolvedValueOnce(
      ok([{ name: "IDEA.md", path: "/remote/IDEA.md", isDirectory: false }]),
    );
  ...
  it("lazy-loads children on loadChildren and replaces the placeholder", async () => {
    readDir.mockResolvedValueOnce(
      ok([{ name: "src", path: "/p/src", isDirectory: true }]),
    );
    readDir.mockResolvedValueOnce(
      ok([
  ...
  it("reads gitignore from the real path while caching per connection", async () => {
    const readFileDataUrl = vi.fn(
      async () => `data:text/plain;base64,${btoa("ignored.log\n")}`,
    );
    ...
    expect(readDir).not.toHaveBeenCalledWith(
      expect.stringContaining("local-a"),
    );
    ...
    expect(
      readDir.mock.calls.filter(([path]) => path === "/repo"),
    ).toHaveLength(2);
  });

  it("captures per-folder error code and shows an error placeholder child", async () => {
    readDir.mockResolvedValueOnce(
      ok([{ name: "priv", path: "/p/priv", isDirectory: true }]),
    );
    readDir.mockResolvedValueOnce({ entries: [], error: "EACCES" });
  ...
  it("clears child loading and allows retry when a child read rejects", async () => {
    ...
    readDir.mockResolvedValueOnce(
      ok([{ name: "index.ts", path: "/p/src/index.ts", isDirectory: false }]),
    );
    ...
    expect(result.current.data[0].children?.map((node) => node.name)).toEqual([
      "index.ts",
    ]);
  });
  ...
  it("dedupes concurrent loadChildren calls for the same id", async () => {
    ...
    await act(async () => {
      ...
      void result.current.loadChildren("/p/src");
      resolveChildren?.(
        ok([{ name: "a.ts", path: "/p/src/a.ts", isDirectory: false }]),
      );
    });
  ...
  it("refreshRoot reloads the root and clears prior error", async () => {
    readDir.mockResolvedValueOnce({ entries: [], error: "EACCES" });
    readDir.mockResolvedValueOnce(
      ok([{ name: "README.md", path: "/p/README.md", isDirectory: false }]),
    );
  ...
  it("discards a stale live refresh after the active registered connection changes", async () => {
    let resolveRefreshFromA:
      ((result: HermesReadDirResult) => void) | undefined;
    readDir.mockResolvedValueOnce(
      ok([{ name: "from-a", path: "/shared/from-a", isDirectory: false }]),
    );
    readDir.mockImplementationOnce(
      () =>
        new Promise<HermesReadDirResult>((resolve) => {
    ...
    await waitFor(() =>
      expect(result.current.data.map((node) => node.name)).toEqual(["from-a"]),
    );
    ...
    await act(async () => {
      resolveRefreshFromA?.(
        ok([{ name: "stale-a", path: "/shared/stale-a", isDirectory: false }]),
      );
    });
  ...
  it("discards a stale child read after the active registered connection changes", async () => {
    ...
    readDir.mockResolvedValueOnce(
      ok([{ name: "src", path: "/shared/src", isDirectory: true }]),
    );
    readDir.mockImplementationOnce(
      () =>
        new Promise<HermesReadDirResult>((resolve) => {
    ...
    await act(async () => {
      resolveChildFromA?.(
        ok([
          { name: "from-a", path: "/shared/src/from-a", isDirectory: false },
        ]),
      );
    });

    expect(result.current.data[0]).not.toMatchObject({
      children: [{ name: "from-a" }],
    });
  });
  ...
  it("reloads when cwd changes", async () => {
    readDir.mockResolvedValueOnce(
      ok([{ name: "one", path: "/a/one", isDirectory: false }]),
    );
    readDir.mockResolvedValueOnce(
      ok([{ name: "two", path: "/b/two", isDirectory: false }]),
    );
  ...
  it("falls back to the sanitized workspace dir when the session cwd is gone", async () => {
    ...
    readDir.mockImplementation(async (path) => {
      ...
      if (path === "/home/me/projects") {
        return ok([
          { name: "repo", path: "/home/me/projects/repo", isDirectory: true },
        ]);
      }
