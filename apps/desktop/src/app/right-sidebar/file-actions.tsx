...
export function isRenameShortcut(
...
export function pickRevealLabel(
  finder: string,
  explorer: string,
  fileManager: string,
): string {
...
export function FileEntryContextMenu({
  ...
  const revealLabel = pickRevealLabel(
    m.revealFinder,
    m.revealExplorer,
    m.revealFileManager,
  );
  ...
  return (
    <ContextMenu>
      ...
      <ContextMenuContent onCloseAutoFocus={(event) => event.preventDefault()}>
        {localFs && (
          <>
            <ContextMenuItem onSelect={() => void revealFile(path)}>
              {revealLabel}
            </ContextMenuItem>
            <ContextMenuSeparator />
          </>
        )}
        <ContextMenuItem onSelect={() => void copyFilePath(path)}>
          {m.copyPath}
        </ContextMenuItem>
        {relativeTo && (
          <ContextMenuItem
            onSelect={() => void copyFilePath(toRelativePath(path, relativeTo))}
          >
            {m.copyRelativePath}
          </ContextMenuItem>
        )}
        {remoteDownload && (
          <>
            <ContextMenuSeparator />
            <ContextMenuItem onSelect={() => void downloadRemoteFile(path)}>
              {m.download}
            </ContextMenuItem>
          </>
        )}
        {localFs && (
          <>
            <ContextMenuSeparator />
            <ContextMenuItem onSelect={() => beginInlineRename(path)}>
              {m.rename}
            </ContextMenuItem>
            <ContextMenuItem
              onSelect={() => requestFileDelete(target)}
              variant="destructive"
            >
              {m.delete}
            </ContextMenuItem>
