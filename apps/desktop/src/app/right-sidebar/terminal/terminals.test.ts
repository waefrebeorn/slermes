...
describe("terminal store persistence", () => {
  ...
  it("never attaches a revive buffer to an agent tab", async () => {
    const { $terminals, ensureAgentTerminal, updateTerminalReviveBuffer } =
      await loadTerminalStore();
    ...
    expect(
      $terminals.get().find((term) => term.id === agentId)?.reviveBuffer,
    ).toBeUndefined();
    expect(window.localStorage.getItem(STORAGE_KEY)).toBeNull();
  });

  it("tail-trims an oversized revive buffer to stay under the storage budget", async () => {
    const { $terminals, createTerminal, updateTerminalReviveBuffer } =
      await loadTerminalStore();
    ...
    const stored =
      $terminals.get().find((term) => term.id === userId)?.reviveBuffer ?? "";
  ...
  it("restores and persists the last observed cwd so a reopened tab lands where the user cd-d", async () => {
    ...
    expect(
      JSON.parse(window.localStorage.getItem(STORAGE_KEY) ?? "{}").terminals[0]
        .restoreCwd,
  ...
  it("never attaches a restore cwd to an agent tab and ignores empty values", async () => {
    ...
    expect(
      $terminals.get().find((term) => term.id === agentId)?.restoreCwd,
    ).toBeUndefined();
    expect(
      $terminals.get().find((term) => term.id === userId)?.restoreCwd,
    ).toBeUndefined();
  });
});
...
describe("session cwd → terminal tab linking", () => {
  ...
  it("re-selects the tab already pointed at the new session cwd (trailing slash tolerated)", async () => {
    const { $activeTerminalId, $currentCwd, createTerminal } =
      await loadTerminalStore();
