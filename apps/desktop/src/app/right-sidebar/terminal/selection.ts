...
export function terminalTheme(
  mode: "light" | "dark",
  palette?: DesktopTerminalPalette,
): ITheme {
...
export function terminalSelectionLabel(
  ...
  if (pos) {
    return pos.start.y === pos.end.y
      ? `${shellName}:${pos.start.y}`
      : `${shellName}:${pos.start.y}-${pos.end.y}`;
  }
...
export function terminalSelectionAnchor(
  ...
  const rect = Array.from(
    host.querySelectorAll<HTMLElement>(".xterm-selection div"),
  )
