...
import {
  ...
  setDocumentHidden,
  type WindowStateBridge,
} from "../../../test/window-state";
...
let mutationObserveCalls: Array<{
...
describe("PersistentTerminal rect tracking", () => {
  ...
  it("remeasures when layout moves the slot without resizing it", () => {
    ...
    expect(
      mutationObserveCalls.every((call) => call.options?.subtree === false),
    ).toBe(true);
  ...
  it("hides the overlay but keeps its workspace mounted when the terminal tab becomes inactive", () => {
    ...
    const overlay = mount.container!.lastElementChild as HTMLElement;
    const workspace = mount.container!.querySelector(
    ...
    expect(
      mutationObserveCalls.some((call) =>
        call.options?.attributeFilter?.includes(PANE_HIDDEN_ATTR),
      ),
    ).toBe(true);
    ...
    expect(
      mount.container!.querySelector('[data-testid="terminal-workspace"]'),
    ).toBe(workspace);
