...
function sanitizePersistedTerminal(
  value: unknown,
): PersistedTerminalEntry | null {
  ...
  const restoreCwd =
    typeof record.restoreCwd === "string" && record.restoreCwd
      ? record.restoreCwd
      : undefined;
  const reviveBuffer =
    typeof record.reviveBuffer === "string" ? record.reviveBuffer : undefined;
...
function loadPersistedTerminals(): PersistedTerminalState {
  const fallback: PersistedTerminalState = {
  ...
  try {
    ...
    const active =
      ...
      terminals.some((term) => term.id === record.activeTerminalId)
...
function persistTerminals(
  ...
  const active = terminals.some((term) => term.id === activeTerminalId)
    ? activeTerminalId
    : (terminals[0]?.id ?? null);
  writeKey(
    TERMINALS_STORAGE_KEY,
    JSON.stringify({ activeTerminalId: active, terminals }),
  );
}
...
export function createTerminal(cwd: string = $currentCwd.get()): string {
  ...
  $terminals.set([
    ...$terminals.get(),
    { id, title: "Terminal", auto: true, cwd, kind: "user" },
  ]);
...
const findByProc = (procId: string) =>
  $terminals.get().find((term) => term.procId === procId);
...
const terminalCwd = (term: TerminalEntry) =>
  normalizePath(term.restoreCwd || term.cwd);
...
$currentCwd.listen((cwd) => {
  ...
  const match = list.find(
    (term) => term.kind === "user" && terminalCwd(term) === target,
  );
...
export function closeAgentTerminalByProc(procId: string): boolean {
  const term = $terminals
    .get()
    .find((t) => t.kind === "agent" && t.procId === procId);
...
export function updateTerminalReviveBuffer(
  ...
  $terminals.set(
    $terminals
      .get()
      .map((term) =>
        term.id === id && term.kind === "user"
          ? { ...term, reviveBuffer: capped }
          : term,
      ),
  );
}
...
export function renameTerminal(id: string, title: string): void {
  ...
  $terminals.set(
    $terminals
      ...
      .map((term) =>
        term.id === id
          ? { ...term, title: trimmed || term.title, auto: false }
          : term,
      ),
  );
}
...
export function reportTerminalShell(id: string, shell: string): void {
  ...
  $terminals.set(
    $terminals
      .get()
      .map((term) =>
        term.id === id && term.auto ? { ...term, title: name } : term,
      ),
  );
}
