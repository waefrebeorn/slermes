...
async function clickLinkIn(text: string, event: MouseEvent) {
  ...
  const links = await new Promise<ILink[]>((resolve) =>
    providers[0].provideLinks(1, (found) => resolve(found ?? [])),
  );
...
describe("terminal links", () => {
  it("opens a ⌘-clicked URL in the in-app browser, not the window.open Electron denies", async () => {
    ...
    expect(
      await clickLinkIn(uri, new MouseEvent("click", { metaKey: true })),
    ).toBe(uri);
    expect(openExternal).not.toHaveBeenCalled();
    await vi.waitFor(() =>
      expect($previewTabs.get().at(-1)?.target.url).toBe(uri),
    );
  });
...
describe("isTerminalLinkActivation", () => {
  it("takes the platform modifier: ⌘ on macOS, Ctrl elsewhere", () => {
    ...
    expect(isTerminalLinkActivation(click({ ctrlKey: true }), false)).toBe(
      true,
    );
  });

  it("keeps Ctrl+click free on macOS, where the OS reads it as a right-click", () => {
    expect(isTerminalLinkActivation(click({ ctrlKey: true }), true)).toBe(
      false,
    );
  });
});
