...
describe("terminalClipboardIntent", () => {
  it("never claims a bare Ctrl+C with nothing selected, on either platform", () => {
    for (const isMac of [true, false]) {
      expect(
        terminalClipboardIntent(key({ ctrlKey: true, key: "c" }), {
          hasSelection: false,
          isMac,
        }),
      ).toBeNull();
  ...
  it("reserves plain Ctrl+C for the shell on macOS, where ⌘C is the copy chord", () => {
    ...
    expect(
      terminalClipboardIntent(key({ key: "c", metaKey: true }), {
        hasSelection: true,
        isMac: true,
      }),
    ).toBe("copy");
  ...
  it("claims paste regardless of selection, since paste has nothing to do with one", () => {
    expect(
      terminalClipboardIntent(key({ key: "v", metaKey: true }), {
        hasSelection: false,
        isMac: true,
      }),
    ).toBe("paste");
    expect(
      terminalClipboardIntent(
        key({ ctrlKey: true, key: "v", shiftKey: true }),
        { hasSelection: false, isMac: false },
      ),
    ).toBe("paste");
  });
...
describe("mirrorSelection", () => {
  ...
  it("is a no-op before xterm has mounted its textarea", () => {
    expect(() =>
      mirrorSelection(document.createElement("div"), "text"),
    ).not.toThrow();
  });
});
