...
function previewSelectionLabel(): string {
  ...
  return (
    source.split(/[\\/]/).filter(Boolean).pop() || target?.label?.trim() || ""
  );
}
...
export function useTerminalSession({
  ...
  const activeTheme = useMemo(
    () => terminalTheme(renderedMode, ansiPalette),
    [renderedMode, ansiPalette],
  );
  ...
  const { latestFontFamilyRef, mountedRef } = useTerminalFontController({
    fitRef,
    termRef,
    webglRef,
  });
  ...
  const [selectionStyle, setSelectionStyle] = useState<CSSProperties | null>(
    null,
  );
  ...
  const addSelectionToChat = useCallback(() => {
    const termSelection = (
      termRef.current?.getSelection() || selectionRef.current
    ).trim();
    const selectedText =
      termSelection || window.getSelection()?.toString() || "";
  ...
  useEffect(() => {
    ...
    const probeCwd = () => {
      ...
      if (
        !sessionId ||
        !terminalApi.cwd ||
        Date.now() - cwdProbeAt < CWD_PROBE_THROTTLE_MS
      ) {
        return;
      }
    ...
    const onDrop = (e: DragEvent) => {
      ...
      if (
        !id ||
        !e.dataTransfer ||
        !transferHasDropCandidates(e.dataTransfer)
      ) {
        return;
      }
    ...
    const fitAndResize = (visible: boolean) => {
      ...
      try {
        fit.fit();
        markRightPanePerf(
          visible ? "terminal-fit-active" : "terminal-fit-hidden",
          id,
        );
      } catch {
        return;
      ...
      if (
        sessionId &&
        (lastSentSize?.cols !== term.cols || lastSentSize?.rows !== term.rows)
      ) {
        lastSentSize = { cols: term.cols, rows: term.rows };
        void terminalApi.resize(sessionId, {
          cols: term.cols,
          rows: term.rows,
        });
      }
    ...
    const selectionDisposable = term.onSelectionChange(() => {
      ...
      selectionLabelRef.current = next.trim()
        ? terminalSelectionLabel(term, shellNameRef.current, next)
        : "";
    ...
    const startSession = () =>
      void terminalApi
        ...
        .then(async (session) => {
          ...
          selectionLabelRef.current = initial
            ? terminalSelectionLabel(term, shellNameRef.current, initial)
            : "";
          ...
          if (!attached) {
            throw new Error(
              "Terminal session disappeared before its output stream attached",
            );
          }
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn(
          "[hermes-terminal] WebGL unavailable; falling back to DOM",
          err,
        );
      }
  ...
  useEffect(() => {
    ...
    return term
      ? registerTerminalReader(id, makeTerminalReader(term))
      : undefined;
  }, [id, status]);
