...
export function PersistentTerminal({
  ...
  useLayoutEffect(() => {
    ...
    const unsubscribeLayout = $layoutTree.listen(() =>
      scheduleMeasure("layout-tree"),
    );
    const unsubscribePanes = $paneStates.listen(() =>
      scheduleMeasure("pane-state"),
    );
  ...
  const visible = Boolean(
    rect && !rect.hidden && rect.width > 0 && rect.height > 0,
  );
