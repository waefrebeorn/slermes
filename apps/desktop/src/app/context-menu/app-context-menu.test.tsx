...
describe("resolveDomTarget", () => {
  ...
  it("resolves an image, and the anchor wrapping it", () => {
    const host = attach(
      '<a href="https://example.com/page"><img src="https://example.com/pic.png"></a>',
    );
  ...
  it("resolves editables, skipping disabled and readonly fields", () => {
    const host = attach(
      '<textarea></textarea><input readonly><div contenteditable="true"><span>x</span></div>',
    );

    expect(
      resolveDomTarget(host.querySelector("textarea")).editable,
    ).toBeTruthy();
  ...
  it("resolves the enclosing dialog as the menu portal container", () => {
    const host = attach(
      '<div data-slot="dialog-content"><a href="https://example.com">link</a></div>',
    );
    const dialog = host.firstElementChild;

    expect(
      resolveDomTarget(host.querySelector("a")).dialogPortalContainer,
    ).toBe(dialog);
  });
});
...
describe("AppContextMenu", () => {
  ...
  it("opens the in-app browser from the link menu", async () => {
    ...
    await waitFor(() =>
      expect($previewTabs.get().at(-1)?.target.url).toBe(
  ...
  it("skips Open in in-app browser on the HUD — that window has no browser pane", async () => {
    ...
    try {
      ...
      mountMenu();
      const host = attach(
        '<a href="https://accounts.google.com/o/oauth2/auth">Sign in</a>',
      );
  ...
  it("offers the resolved copy only for loopback links on a remote gateway", async () => {
    ...
    installBridge({
      reachPreviewUrl:
        reachPreviewUrl as unknown as Window["hermesDesktop"]["reachPreviewUrl"],
      writeClipboard:
        writeClipboard as unknown as Window["hermesDesktop"]["writeClipboard"],
    });
    mountMenu();
    ...
    await waitFor(() =>
      expect(writeClipboard).toHaveBeenCalledWith("http://127.0.0.1:45173/"),
    );
  });
  ...
  it("shows edit verbs without icons and with faded accelerators", async () => {
    ...
    const pasteItem = (await screen.findByText("Paste")).closest(
      '[data-slot="dropdown-menu-item"]',
    )!;

    expect(
      pasteItem.querySelector('[data-slot="dropdown-menu-shortcut"]')
        ?.textContent,
    ).toBe(formatCombo("mod+v"));
    ...
    expect(
      selectAllItem.querySelector('[data-slot="dropdown-menu-shortcut"]')
        ?.textContent,
    ).toBe(formatCombo("mod+a"));
  ...
  it("runs edit verbs after the menu closed, with focus back on the editable", async () => {
    ...
    installBridge({
      contextMenuEdit:
        contextMenuEdit as unknown as Window["hermesDesktop"]["contextMenuEdit"],
    });
    mountMenu();
  ...
  it("keeps a modal textarea paste menu inside its dialog and restores focus", async () => {
    ...
    const paste = (await screen.findByText("Paste")).closest(
      '[data-slot="dropdown-menu-item"]',
    ) as HTMLElement;
  ...
  it("splits select all into its own section under the edit verbs", async () => {
    ...
    const selectAllItem = (await screen.findByText("Select all")).closest(
      '[data-slot="dropdown-menu-item"]',
    )!;
    ...
    expect(
      selectAllItem.parentElement?.querySelector(
  ...
  it("keeps paste clickable even when the clipboard probe reports empty", async () => {
    ...
    installBridge({
      readClipboard:
        readClipboard as unknown as Window["hermesDesktop"]["readClipboard"],
    });
    mountMenu();
    const host = attach("<textarea>text</textarea>");
  ...
  it("leaves surfaces with their own radix menu alone", () => {
    ...
    mountMenu();
    const host = attach(
      '<div data-slot="context-menu-trigger"><span>session row</span></div>',
    );
  ...
  it("shows the terminal menu through a registered handle", async () => {
    ...
    const unregister = registerTerminalContextMenu(
      host.querySelector("[data-terminal]")!,
      {
...
describe("AppContextMenu guest (in-app browser)", () => {
  const guestHandle = (
    overrides: Partial<GuestMenuHandle> = {},
  ): GuestMenuHandle => ({
  ...
  const guestParams = (
    overrides: Partial<GuestMenuParams> = {},
  ): GuestMenuParams => ({
  ...
  it("draws a line between select all and inspect element", async () => {
    ...
    const inspectItem = screen
      .getByText("Inspect element")
      .closest('[data-slot="dropdown-menu-item"]')!;

    expect(selectAllItem.parentElement).not.toBe(inspectItem.parentElement);
    expect(
      inspectItem.parentElement?.querySelector(
        '[data-slot="dropdown-menu-separator"]',
      ),
    ).not.toBeNull();
  });
  ...
  it("adds a link section above the tools for guest links", async () => {
    ...
    openGuestContextMenu(
      10,
      10,
      guestParams({ linkURL: "https://example.com/deep" }),
      guestHandle(),
    );
  ...
  it("dispatches guest edit verbs a frame after the menu closes", async () => {
    ...
    await waitFor(() =>
      expect(guest.editCommand).toHaveBeenCalledWith("selectAll"),
    );
    expect($contextMenu.get()).toBeNull();
  });
