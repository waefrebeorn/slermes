...
import {
  type PetZoomAnchor,
  usePetZoomGesture,
} from "@/components/pet/use-pet-zoom-gesture";
...
export function PetOverlayApp() {
  ...
  const onPetPointerMove = (e: React.PointerEvent) => {
    ...
    if (
      Math.hypot(e.screenX - drag.startX, e.screenY - drag.startY) >
      CLICK_SLOP_PX
    ) {
      drag.moved = true;
    }
  ...
  usePetZoomGesture(
    petRef,
    onScale,
    Boolean(info.enabled && info.spritesheetBase64),
  );
  ...
  useEffect(() => {
    ...
    const bounds = {
      ...
      y: Math.round(
        window.screenY +
          ay -
          (ay - (curH - PET_PADDING_BOTTOM)) * ratio -
          (height - PET_PADDING_BOTTOM),
      ),
    };
    ...
    >
      ...
      >
        ...
        <div style={{ lineHeight: 0, position: "relative" }}>
          ...
          <PetHeartField
            petH={
              (info.frameH ?? DEFAULT_FRAME_H) * (info.scale ?? DEFAULT_SCALE)
            }
            petW={
              (info.frameW ?? DEFAULT_FRAME_W) * (info.scale ?? DEFAULT_SCALE)
            }
          />
