...
export function PetGenerateContent() {
  ...
  const [remixPending, setRemixPending] = useState<{ dataUri: string } | null>(
    null,
  );
  ...
  const pickReference = (file: File | undefined) => {
    ...
    const mapReferenceError = (reason: unknown): string => {
      ...
      return message.includes("too large")
        ? copy.referenceImageTooLarge
        : copy.referenceImageInvalid;
    };
    ...
    void readReferenceImage(file)
      .then((dataUrl) => {
        ...
        if (
          $petGenStatus.get() === "error" &&
          $petGenDrafts.get().length === 0
        ) {
