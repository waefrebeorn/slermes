  ...
  handlersRef.current = {
    ...
    "nav.commandPalette": () => {
      if (
        ...
        appViewForPath(location.pathname) === "settings"
      ) {
    ...
    "view.closeTerminal": () =>
      isPaneVisible("terminal") && closeActiveTerminal(),
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (
        ...
        !actionAllowedInInput(actionId, combo)
      ) {
        return;
      }
      ...
      const handler =
        handlersRef.current[actionId] ?? contributedKeybindHandler(actionId);
