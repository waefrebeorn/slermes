  ...
  useEffect(() => {
    ...
    for (let y = 0; y < FIELD_H; y += 1) {
      ...
      for (let x = 0; x < FIELD_W; x += 1) {
        const rgb = oklchToSrgb255({
          l,
          c: (x / (FIELD_W - 1)) * CHROMA_MAX,
          h: hue,
        });
...
function AccentPicker() {
  ...
  const [lch, setLch] = useState<Oklch>(
    () => hexToOklch(painted) ?? { l: 0.5, c: 0.15, h: 260 },
  );
  ...
  const surface =
    (renderedMode === "dark" ? theme.darkColors : theme.colors) ?? theme.colors;
  const ratio = contrastRatio(
    painted,
    surface.sidebarBackground ?? surface.background,
  );

  return (
    <div className="flex flex-col gap-2 p-2" style={{ width: FIELD_W + 16 }}>
      <ChromaLightnessField
        hue={lch.h}
        lch={lch}
        onPick={(l, c) => commit({ ...lch, c, l })}
      />
      <HueRail lch={lch} onPick={(h) => commit({ ...lch, h })} />

      <div className="flex items-center gap-1.5">
        <span
          className="size-5 shrink-0 rounded-sm border border-(--dt-border)"
      ...
      <div className="flex items-center justify-between font-mono text-[10px] tabular-nums text-(--ui-text-tertiary)">
        <span>
          ...
          L{lch.l.toFixed(2)} C{lch.c.toFixed(3)}
        </span>
