...
vi.mock("./group-membership", () => ({
  groupMemberKey: (member: GroupMember) =>
    member?.route?.connectionId
      ? `${member.route.connectionId}::${member.name}`
      : member?.name,
}));

vi.mock("./routing", () => ({
  backendTargetProfile: (route: null | ProfileRoute, fallback: string) =>
    route?.targetProfile || fallback,
...
function hiddenCalls() {
  return hostMock.setPersistedSessionHidden.mock.calls as Array<
    [
      null | ProfileRoute,
      { hidden: boolean; profile: string; sessionId: string },
    ]
  >;
}
...
describe("the id half: group room member sessions", () => {
  ...
  it("routes a remote member session through its immutable persisted owner", async () => {
    ...
    requestForBotMock.mockRejectedValue(
      new Error("gateway RPC must not be used"),
    );
  ...
  it("keeps two remote owners of the same session id on their own routes", async () => {
    ...
    groupChats.value = {
      A: {
        sessionOwners: { "source-a::worker": owner("source-a") },
        sessions: { "source-a::worker": "same-id" },
      },
      B: {
        sessionOwners: { "source-b::worker": owner("source-b") },
        sessions: { "source-b::worker": "same-id" },
      },
    };
    requestForBotMock.mockRejectedValue(
      new Error("gateway RPC must not be used"),
    ...
    expect(
      hiddenCalls().every(([, options]) => options.sessionId === "same-id"),
    ).toBe(true);
  });
...
describe("the title half: each roster bot’s own profile listing", () => {
  ...
  beforeEach(() => {
    lastRoster.value = [
      { name: "alpha" },
      { connectionId: "mini", name: "remy", remoteSource: true },
    ] as RosterRow[];
  ...
  it("hides Bot-Mode plumbing titles per roster bot, and only those", async () => {
    ...
    expect(lists.map(([, options]) => options.profile).sort()).toEqual([
      "alpha",
      "remy",
    ]);
    ...
    expect(
      hiddenCalls().find(([, options]) => options.sessionId === "r-1")?.[0]
        ?.connectionId,
    ).toBe("mini");
  });
  ...
  it("runs beside the id half, and a throwing title sweep never breaks it", async () => {
    ...
    groupChats.value = { Core: { sessions: { alpha: "room-core-a" } } };
    hostMock.listPersistedSessions.mockRejectedValue(
      new Error("gateway not ready"),
    );

    await runSweep();

    expect(hiddenCalls().map(([, options]) => options.sessionId)).toEqual([
      "room-core-a",
    ]);
  });
});
