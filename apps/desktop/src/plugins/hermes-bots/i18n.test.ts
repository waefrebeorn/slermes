...
function leafEntries(node: unknown, prefix = ""): Array<[string, Leaf]> {
  ...
  return Object.entries(node as Record<string, unknown>).flatMap(
    ([key, value]) => leafEntries(value, prefix ? `${prefix}.${key}` : key),
  );
}
...
describe("BOTS_LOCALES", () => {
  ...
  it("keeps interpolator arguments in the translated string", () => {
    ...
    for (const locale of [en, ja, zh, zhHant]) {
      const byPath = Object.fromEntries(leafEntries(locale));
      const queryFn = byPath["roster.noMatchQuery"] as (
        query: string,
      ) => string;
      const bothFn = byPath["roster.noMatchQueryOn"] as (
        query: string,
        gateway: string,
      ) => string;
      const reasonFn = byPath["roster.rosterUnavailable"] as (
        reason: string,
      ) => string;
