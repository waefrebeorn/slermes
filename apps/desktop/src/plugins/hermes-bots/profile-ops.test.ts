...
vi.mock("./canonical-chat", () => ({
  ensureBotMetadata: ensureBotMetadataMock,
}));
...
describe("duplicating a bot", () => {
  it("copies the look but neither the chat pointer nor the creation stamp", async () => {
    ...
    const name = await duplicateBot(
      { description: "finds things", name: "researcher" } as RosterRow,
    ...
    expect(
      calls.find((call) => call.method === "profiles.create")?.params,
    ).toMatchObject({
    ...
    const configure = calls
      .filter((call) => call.method === "profiles.configure")
      .at(-1);
    const uiMeta = (
  ...
  it("duplicates the look of a bot that never had a pointer", async () => {
    $botMeta.set({
      painter: { color: "#38bdf8", shape: "cloud", title: "Painter" },
    });

    const name = await duplicateBot({ name: "painter" } as RosterRow, [
      { name: "painter" } as RosterRow,
    ]);

    expect($botMeta.get()[name]).toMatchObject({
      shape: "cloud",
      title: "Painter (copy)",
    });
  ...
  it("walks past taken suffixes to the first free slot", async () => {
    const roster = ["ops", "ops-2", "ops-3"].map(
      (name) => ({ name }) as RosterRow,
    );

    expect(await duplicateBot({ name: "ops" } as RosterRow, roster)).toBe(
      "ops-4",
    );
  });
  ...
  it("truncates the BASE so a max-length name still gets a distinct suffix (#19)", async () => {
    ...
    const name = await duplicateBot({ name: base } as RosterRow, [
      { name: base } as RosterRow,
    ]);
