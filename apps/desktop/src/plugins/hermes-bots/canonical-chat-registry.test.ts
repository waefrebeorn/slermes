...
const { hostMock, persistMock, requestForBotMock, saveBotMetaMock } =
  vi.hoisted(() => ({
...
vi.mock("./routing", () => ({
  backendTargetProfile: (
    route: { targetProfile?: string } | null,
    name: string,
  ) => route?.targetProfile ?? name,
  botConnectionRoute: () => null,
  botRosterMeta: () => ({}),
  botWorkspaceOwnerKey: (
    bot: { connectionId?: string; name?: string } | null,
  ) =>
...
function respondWith(
  ...
  requestForBotMock.mockImplementation(
    async (_bot: unknown, method: string, params: Record<string, unknown>) => {
...
describe("the registry row wins, always", () => {
  it('resolves the profile\u2019s "Bot Chat" row by exact title and opens it', async () => {
    const calls = respondWith((method) => {
      if (method === "session.list") {
        return {
          sessions: [
            { id: "forever-chat", message_count: 930, title: "Bot Chat" },
          ],
        };
      }
    ...
    expect(opened).toEqual({
      openedId: "forever-chat",
      registryId: "forever-chat",
    });
  ...
  it("never reads or writes a stored pointer while opening", async () => {
    const calls = respondWith((method) =>
      method === "session.list"
        ? { sessions: [{ id: "forever-chat", title: "Bot Chat" }] }
        : {},
    );
    ...
    expect(
      calls.every((call) => !("preferred_session_ids" in call.params)),
    ).toBe(true);
...
describe("no registry row → create", () => {
  it('mints a hidden "Bot Chat" WITHOUT an intro kickoff', async () => {
    ...
    expect(
      calls.find((call) => call.method === "session.create")?.params,
    ).toMatchObject({
      hidden: true,
      title: "Bot Chat",
    });
    ...
    expect(
      calls.find((call) => call.method === "session.title")?.params,
    ).toMatchObject({ session_id: "rt-1" });
    expect(
      calls.find((call) => call.method === "prompt.submit"),
    ).toBeUndefined();
  });
  ...
  it("never claims an ordinary titled session", async () => {
    respondWith((method) => {
      ...
      return method === "session.create"
        ? { session_id: "rt-2", stored_session_id: "fresh-2" }
        : {};
    });
    ...
    expect(
      hostMock.openSession.mock.calls.every(([id]) => id !== "scratch"),
    ).toBe(true);
  });
  ...
  it("surfaces a failed open of the registry row instead of forking a replacement", async () => {
    respondWith((method) => {
      if (method === "session.list") {
        return {
          sessions: [
            { id: "forever-chat", message_count: 12, title: "Bot Chat" },
          ],
        };
      }
      ...
      if (method === "session.create") {
        throw new Error(
          "must not create: a transient open failure is not ownership loss",
        );
      }
    ...
    await expect(openBotCanonicalChat("ops")).rejects.toThrow(
      "backend restarting",
    );
  });
});
...
describe('a failed lookup fails CLOSED — never "no chat exists"', () => {
  ...
  const refuseToMint = (method: string) => {
    ...
    if (method === "session.create") {
      throw new Error(
        'must not create: a failed lookup is not "no chat exists"',
      );
    }
  ...
  it("refuses to mint when the adoption lookup inside creation fails", async () => {
    ...
    await expect(createCanonicalChat("ops")).rejects.toThrow(
      /Bot Chat registry/,
    );
    expect(calls.some((call) => call.method === "session.create")).toBe(false);
  });
  ...
  it("refuses to mint on an empty lookup when the roster already confirmed a canonical chat", async () => {
    const calls = respondWith((method) => {
      ...
      if (method === "session.create") {
        throw new Error(
          "must not create: an empty result is not confirmed absence",
        );
      }
    ...
    const bot = {
      canonical_session: { id: "forever-chat" },
      name: "ops",
    } as RosterRow;
    const { openBotCanonicalChat } = await loadModule();

    await expect(openBotCanonicalChat(bot)).rejects.toThrow(
      /Bot Chat registry/,
    );
    expect(calls.some((call) => call.method === "session.create")).toBe(false);
    expect(hostMock.openSession).not.toHaveBeenCalled();
