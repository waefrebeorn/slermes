...
export function resolveBotConnectionRoute(
  ...
  const profile =
    String(candidate?.profile || bot?.name || "").trim() || "default";
  const targetProfile =
    String(candidate?.targetProfile || profile).trim() || profile;
  ...
  return {
    ...
    route: Object.freeze({
      connectionId,
      mode:
        candidate.mode === "local" || connectionId === "local"
          ? "local"
          : "remote",
      profile,
      targetProfile,
...
export function botConnectionRoute(
  bot: Partial<RosterRow> | null | undefined,
): null | ProfileRoute {
...
export function setBotsWorkspaceOwner(
  ...
  const target:
    ...
    | { kind: "route"; route: ProfileRoute } = route
    ? {
        kind: "route",
        route,
      }
...
export function backendTargetProfile(
  route: null | ProfileRoute | undefined,
...
function rewriteCliProfileOperands(
  argv: string[],
  logical: string,
  target: string,
) {
...
export function botBackendProfileScope(
  route: null | ProfileRoute | undefined,
  fallbackProfile = "default",
) {
  if (!route) {
...
export async function requestForBot<T = unknown>(
  ...
  if (route) {
    if (typeof host.requestProfile !== "function") {
      throw new Error(
        `Cannot route ${method} for ${route.connectionId}::${route.profile}`,
      );
    }

    try {
      return await host.requestProfile(
        route,
        method,
        scopedBotParams(route, method, params),
      );
    } catch (error) {
...
function asRpcError(value: unknown, fallback: string): unknown {
  ...
  const hasStack =
    isObject && typeof (value as RpcErrorLike).stack === "string";
  ...
  if (isObject) {
    const text =
      hasStringMessage && String(message).trim() ? String(message) : fallback;
...
export function indexAliasRoutes(
  routes: ProfileRoute[],
  epoch = beginAliasRouteIndex(),
) {
...
export function aliasIdentityFor(
  bot: Partial<RosterRow> | null | undefined,
): AliasIdentity | null {
  ...
  const connectionId = String(
    bot?.connectionId ||
      ...
      (!bot?.remoteSource && !bot?.sourceScoped
  ...
  const target =
    String(
      bot?.targetProfile || bot?.route?.targetProfile || bot?.name || "",
    ).trim() || "default";
