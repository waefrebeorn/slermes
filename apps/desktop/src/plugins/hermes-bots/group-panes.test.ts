...
describe("group composer drafts", () => {
  it("workspace retirement and re-registration restore the exact room draft", async () => {
    ...
    const key = drafts.groupComposerDraftKey(
      "Launch room",
      room({ roomId: "room-1" }),
    );
  ...
  it("legacy name-keyed drafts migrate when an immutable room id appears", async () => {
    ...
    const current = drafts.groupComposerDraftKey(
      "Renamed room",
      room({ roomId: "room-1" }),
    );

    drafts.updateGroupComposerDraft(legacy, (state) => ({
      ...state,
      main: "keep me",
    }));
  ...
  it("a failed send cannot overwrite text entered after the optimistic clear", async () => {
    ...
    const cleared = drafts.updateGroupComposerDraft(key, (state) => ({
      ...state,
      main: "",
    }));
    ...
    expect(
      drafts.restoreGroupComposerDraft(key, cleared.revision, before),
    ).toBeNull();
  ...
  it("disband removes only that room draft", async () => {
    ...
    drafts.updateGroupComposerDraft("id:a", (state) => ({
      ...state,
      main: "a",
    }));
    drafts.updateGroupComposerDraft("id:b", (state) => ({
      ...state,
      main: "b",
    }));
...
describe("main-window tab registry", () => {
  ...
  it("closing the tab runs its closer and releases the workspace", async () => {
    vi.resetModules();
    const [panes, chat] = await Promise.all([
      import("./group-panes"),
      import("./group-chat"),
    ]);
    const close = vi.fn();
