...
const RELAY_DELIVER_TIMEOUT_MS =
  RELAY_DELIVER_BACKEND_CEILING_MS + RELAY_DELIVER_SETTLEMENT_MARGIN_MS;
...
const relayAgentsCache = new LruCache<string, RelayAgentRow[]>(
  RELAY_AGENTS_CACHE_MAX,
);
      ...
      connections.map(async (connection) => {
        ...
        if (agents === null) {
          ...
          agentsByConnection.set(
            connection.id,
            relayAgentsCache.get(connection.id) || [],
          );
        } else {
...
export function startBotRelay() {
  ...
  if (relay.pushUnsub === null && typeof host.onEvent === "function") {
    relay.pushUnsub = host.onEvent("bot_relay.outbox.pending", () =>
      scheduleRelayPushDrain(),
    );
  }
}
