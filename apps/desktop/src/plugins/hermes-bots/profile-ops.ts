    ...
    Promise.resolve(assetRequest as Promise<ProfilesGetAssetResult>)
      .then((res) => {
        if (res?.found && res.data) {
          ...
          if (
            isBackfilledFacePng(res.data) &&
            mine.imageKind !== "photo" &&
            !mine.pet
          ) {
            return;
          }
    ...
    if (
      !roster.some(
        ...
        (b) =>
          b.name === candidate &&
          (!ownerKey ||
            botMetaKey(b)?.startsWith(`${ownerRoute!.connectionId}::`)),
      )
    ) {
      name = candidate;
...
export async function deleteBot(bot: RosterRow) {
  ...
  if (
    isDefaultBot(bot) ||
    String(route?.targetProfile || "").toLowerCase() === "default"
  ) {
  ...
  if (typeof host.deleteProfile === "function") {
    ...
    if (route) {
      throw new Error(
        "Source-scoped profile deletion requires host.deleteProfile.",
      );
    }
    ...
    if (result?.blocked || result?.code !== 0) {
      throw new Error(
        result?.hint ||
          result?.output ||
          `Could not delete profile ${bot.name}.`,
      );
    }
  }
  ...
  try {
    if (route) {
      ...
      await Promise.resolve(
        getPluginCtx()?.storage?.set?.(BOT_META_V1_KEY, meta),
      );
    }
  } catch {
  ...
  // compression lineage.
  forgetSessionUnread(
    [bot.canonical_session?.id, bot.canonical_session?.resolved_id],
    bot.name,
  );
  ...
  const deletedOwnerIsActive = route
    ? activeOwner?.connectionId === route.connectionId &&
      activeOwner?.name === route.profile
