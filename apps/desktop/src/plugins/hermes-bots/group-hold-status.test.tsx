...
vi.mock("@hermes/plugin-sdk", async () => {
  ...
  return {
    ...
    Codicon: ({ name }: { name: string }) => (
      <span aria-hidden data-icon={name} />
    ),
    ConfirmDialog: () => null,
    CopyButton: () => null,
    Dialog: () => null,
    ...
    relativeTime: () => "now",
    RowButton: (props: React.ComponentProps<"button">) => (
      <button type="button" {...props} />
    ),
...
vi.mock("./group-chat-parts", () => ({
  ...
  GroupMentionInput: (props: { "aria-label"?: string }) => (
    <textarea aria-label={props["aria-label"]} />
  ),
}));
...
describe("durable group holds", () => {
  ...
  it("names a partial hold and disappears after the production resume directive clears it", async () => {
    const [{ GroupHoldStatus }, { applyGroupHoldDirective }] =
      await Promise.all([
    ...
    const released = applyGroupHoldDirective(
      held,
  ...
  it("projects hydrated holds into the real group workspace", async () => {
    const [{ GroupChatWorkspace }, chat] = await Promise.all([
