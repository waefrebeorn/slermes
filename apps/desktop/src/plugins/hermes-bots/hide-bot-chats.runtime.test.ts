...
const setPersistedSessionHidden = vi.fn(
  async (
    _route: unknown,
    _options: { hidden: boolean; profile: string; sessionId: string },
  ) => ({
    hidden: true,
    ok: true,
  }),
...
describe("Bot Mode hidden-session reconciliation lifecycle", () => {
  it("uses persisted REST on load/reconnect and stops with plugin disposal", async () => {
    ...
    expect(
      listPersistedSessions.mock.calls.map(([, options]) => options.profile),
    ).toEqual(["alpha", "beta", "alpha", "beta", "alpha", "beta"]);
    expect(setPersistedSessionHidden).toHaveBeenCalledTimes(6);
    expect(
      setPersistedSessionHidden.mock.calls.map(([, options]) => options),
    ).toEqual(
    ...
    expect(
      request.mock.calls.some(
        ([method]) =>
          method === "session.list" || method === "session.set_hidden",
      ),
    ).toBe(false);
