...
const {
  clearBotAttentionMock,
  hostMock,
  noteBotAttentionMock,
  UnboundedCache,
} = vi.hoisted(() => ({
...
function trackRetention() {
  ...
  (hostMock.retainProfileSocket as ReturnType<typeof vi.fn>).mockImplementation(
    (pinned: ProfileRoute) => {
...
async function pushAndSettle(times = 1) {
  const listener = (hostMock.onEvent as ReturnType<typeof vi.fn>).mock.calls.at(
    -1,
  )?.[1] as () => void;
...
describe("push-notified drain (#93091)", () => {
  it("collapses a burst of pending signals into ONE drain", async () => {
    ...
    expect(
      calls.filter((call) => call.method === "bot_relay.outbox.drain"),
    ).toHaveLength(2);
  ...
  it("feature-detects the event door and disposes its subscription", async () => {
    ...
    startBotRelay();
    expect(hostMock.onEvent).toHaveBeenCalledWith(
...
describe("relay-route socket retention (#93594)", () => {
  ...
  it("releases exactly the pin of a connection that left the registry", async () => {
    ...
    expect(
      pins.filter((pin) => pin.released).map((pin) => pin.route.connectionId),
    ).toEqual(["b"]);
...
describe("the roster loop pushes the OTHER connections’ agents", () => {
  it("gives each gateway a union roster that excludes its own agents", async () => {
    const calls = respondWith((call) => {
      if (call.method === "profiles.list") {
        return {
          profiles: [{ name: call.connectionId === "a" ? "default" : "ops" }],
        };
      }
    ...
    const syncs = calls.filter(
      (call) => call.method === "bot_relay.roster.sync",
    );
  ...
  it("never conflates a transient fetch failure with an empty connection", async () => {
    ...
    const pushedToA = calls.find(
      (call) =>
        call.method === "bot_relay.roster.sync" && call.connectionId === "a",
    );
  ...
  it("drops the cached rows of a connection that genuinely disconnected", async () => {
    const calls = respondWith((call) =>
      call.method === "profiles.list"
        ? { profiles: [{ name: call.connectionId }] }
        : {},
    );
    ...
    expect(pushedToA?.params.agents).toEqual([
      expect.objectContaining({ profile: "c" }),
    ]);
...
describe("the drain loop wires drain → deliver → reply", () => {
  ...
  it("delivers on the target’s own socket and posts the reply to the sender", async () => {
    ...
    expect(
      calls.find((call) => call.method === "bot_relay.deliver"),
    ).toMatchObject({
      connectionId: "b",
      params: { message: "status?", profile: "ops" },
    });
    expect(
      calls.find((call) => call.method === "bot_relay.reply"),
    ).toMatchObject({
      connectionId: "a",
  ...
  it("still posts a reply when the target connection is gone — the waiter must never dangle", async () => {
    const calls = respondWith((call) =>
      call.method === "bot_relay.outbox.drain"
        ? {
            envelopes:
              call.connectionId === "a"
                ? [{ ...envelope, target_connection: "ghost" }]
                : [],
          }
        : {},
    );
    ...
    expect(calls.some((call) => call.method === "bot_relay.deliver")).toBe(
      false,
    );
    expect(
      calls.find((call) => call.method === "bot_relay.reply")?.params.error,
    ).toMatch(/'ghost' is not connected to this Desktop right now/);
  ...
  it("forwards the gateway’s typed failure reason to both the reply and the badge", async () => {
    ...
    expect(
      calls.find((call) => call.method === "bot_relay.reply")?.params,
    ).toMatchObject({
      error: "401 invalid x-api-key",
      id: "env-1",
      reason: "provider_auth_or_access",
    });
    expect(noteBotAttentionMock).toHaveBeenCalledWith(
      "b::ops",
  ...
  it("skips an older backend that rejects the drain RPC, and keeps going", async () => {
    ...
    expect(
      calls
        .filter((call) => call.method === "bot_relay.outbox.drain")
        .map((call) => call.connectionId),
    ).toEqual(["a", "b"]);
