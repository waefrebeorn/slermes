...
describe("alias identity survives the hosted handoff (#89131)", () => {
  it("names the backend row after the alias, through every meta generation", () => {
    ...
    expect(displayName(hostedRow, botRosterMeta(hostedRow, metaV2))).toBe(
      "Moxie",
    );

    const metaV1 = { moxie: { title: "Moxie ✨" } };

    expect(displayName(hostedRow, botRosterMeta(hostedRow, metaV1))).toBe(
      "Moxie ✨",
    );
  });
  ...
  it('renders the sole Cloud-only default as the alias, not "Hermes"', () => {
    ...
    expect(displayName({ name: "default" }, { title: "Custom" })).toBe(
      "Custom",
    );
  });
...
describe("a row without a reachable owner", () => {
  it("reports a typed status instead of throwing, while dispatch still fails closed", () => {
    expect(resolveBotConnectionRoute({ name: "plain" })).toEqual({
      route: null,
      status: "not_scoped",
    });
    expect(
    ...
    expect(resolveBotConnectionRoute(orphan)).toMatchObject({
      profile: "ops",
      route: null,
...
describe("requestForBot rides the bot’s own source", () => {
  it("pins same-name bots to their own connection under concurrent requests", async () => {
    ...
    const answers = await Promise.all(
      rows.map((bot) =>
        requestForBot<{ from: string }>(bot, "profiles.list", {}),
      ),
    );
  ...
  it("rewrites the logical profile to the backend target on the wire", async () => {
    ...
    await requestForBot(
      { name: "moxie", route: MOXIE_ROUTE, sourceScoped: true } as RosterRow,
      "profiles.configure",
      {
        name: "moxie",
        soul: "# hi",
  ...
  it("fails closed rather than falling back to the ambient request", async () => {
    ...
    await expect(requestForBot(scoped, "profiles.list", {})).rejects.toThrow(
      /Cannot route profiles\.list/,
    );
    expect(hostMock.request).not.toHaveBeenCalled();
  ...
  it("coerces a JSON-RPC rejection into an Error with a string name (#94471)", async () => {
    ...
    hostMock.request.mockRejectedValue({
      code: -32000,
      message: "profile busy",
      name: -32000,
    });

    const error = await requestForBot({ name: "ops" }, "cron.list", {}).catch(
