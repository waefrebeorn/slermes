...
describe("the @handle a bot answers to", () => {
  ...
  it("prefers a precomputed multi-source handle over the bare name", () => {
    expect(
      botHandle("default", row({ handle: "default-vera", name: "default" })),
    ).toBe("default-vera");
  });
});
...
describe("renamed bots stay taggable", () => {
  ...
  it("reduces a friendly name to its slugged and collapsed forms", () => {
    expect(mentionNameForms("Research Buddy")).toEqual([
      "research-buddy",
      "researchbuddy",
    ]);
  ...
  it("inserts the renamed slug, falling back to the profile handle", () => {
    ...
    expect(
      botMentionTag(row({ display_name: "Deal Finder", name: "scout" })),
    ).toBe("deal-finder");
  });
  ...
  it("resolves a renamed bot by its friendly tag AND its old handle", () => {
    ...
    const names = (text: string) =>
      resolveRosterMentions(text, roster, live).map((bot) => bot.name);
...
describe("resolving @mentions against the roster", () => {
  ...
  it("reaches Connections bots by bare name and by @name-device handle", () => {
    ...
    expect(
      resolveRosterMentions("@dixie how is disk space?", roster, live).map(
        (bot) => bot.name,
      ),
    ).toEqual(["dixie"]);
    expect(
      resolveRosterMentions("ping @bob-mac-mini", roster, live).map(
        (bot) => bot.name,
      ),
    ).toEqual(["bob"]);
  });
  ...
  it("never treats @hermes in your own chat as a handoff to yourself", () => {
    expect(
      resolveRosterMentions("@hermes do it", roster, {
        connectionId: "local",
        name: "default",
      }),
    ).toEqual([]);
  ...
  it("ignores tags inside code spans and fences", () => {
    ...
    expect(
      resolveRosterMentions("run `@dixie` literally", roster, live),
    ).toEqual([]);
  ...
  it("refuses an ambiguous bare name — the device-qualified handle is required", () => {
    ...
    expect(
      resolveRosterMentions("@ops ping", twins, {
        connectionId: "local",
        name: "default",
      }),
    ).toEqual([]);
    expect(
      resolveRosterMentions("@ops-b ping", twins, {
        connectionId: "local",
        name: "default",
      }).map((bot) => bot.connectionId),
    ).toEqual(["b"]);
  });
});
...
describe("which session speaks for a bot", () => {
  ...
  it("degrades to whichever summary an older gateway sent", () => {
    expect(
      botActivitySession(
        row({ last_session: { last_active: 1, preview: "visible only" } }),
      )?.preview,
    ).toBe("visible only");
    expect(
      botActivitySession(
...
describe("roster search narrows without re-ranking", () => {
  ...
  it("matches profile handles and preserves roster order", () => {
    ...
    expect(filterBots(roster, meta, "default").map((bot) => bot.name)).toEqual([
      "default",
    ]);
  });
  ...
  it("also matches roles, message previews, and the source device name", () => {
    const richer = [
      row({
        ...
        last_session: {
          id: "s1",
          last_active: 1,
          preview: "Checked the deployment checklist",
        },
        name: "reviewer",
      }),
    ...
    expect(filterBots(richer, {}, "deployment checklist")[0].name).toBe(
      "reviewer",
    );
...
describe("unreachable same-name twins (#92286)", () => {
  ...
  it("drops a dead twin when a live copy exists", () => {
    expect(
      preferReachableSameNameRows([live, dead]).map((bot) => bot.connectionId),
    ).toEqual(["local"]);
  });
  ...
  it("keeps a ghost so a selected-but-offline owner is never replaced by a twin", () => {
    expect(
      preferReachableSameNameRows([live, { ...dead, ghost: true }]),
    ).toHaveLength(2);
  });
...
describe("needs-attention badge (#93091 item 3)", () => {
  ...
  it("classifies the raw gateway error strings current backends emit", () => {
    ...
    expect(
      reasonFor("No LLM provider configured. Run hermes model to pick one."),
    ).toBe("missing_config");
    expect(reasonFor("No access token found for profile")).toBe(
      "missing_config",
    );
    expect(reasonFor("Your account is out of funds")).toBe(
      "provider_quota_limit",
    );
    expect(reasonFor("quota exceeded for this billing period")).toBe(
      "provider_quota_limit",
    );
    expect(reasonFor("agent is blocked awaiting approval")).toBe(
      "agent_blocked",
    );
  });
