...
const { botBackendProfileScope, botConnectionRoute, requestForBot } =
...
describe("every source-scoped row resolves to an immutable owner", () => {
  it("routes only source-annotated rows, and freezes the descriptor", () => {
    expect(
      botConnectionRoute({ connectionId: "local", name: "writer" }),
    ).toBeNull();
...
describe("requestForBot dispatches on the owner, not the active gateway", () => {
  it("sends source-scoped rows through requestProfile and local rows through request", async () => {
    await requestForBot({ name: "local-bot" }, "session.create", {
      title: "x",
    });
    await requestForBot(
      { connectionId: "local", name: "local-bot", sourceScoped: true },
      "session.create",
      {
        title: "x",
      },
    ...
    await requestForBot(
      { connectionId: "mac-mini", name: "dixie", remoteSource: true },
      "prompt.submit",
      {
        text: "hi",
      },
    ...
    expect(
      requestProfile.mock.calls.map(([route, method]) => [
        route.connectionId,
        method,
      ]),
    ).toEqual([
  ...
  it("translates every backend-profile RPC shape for a non-identity alias", async () => {
    ...
    await requestForBot(aliasBot, "profiles.configure", {
      name: "worker",
      soul: "x",
    });
    await requestForBot(aliasBot, "profiles.create", {
      clone_from: "worker",
      name: "worker-2",
    });
    await requestForBot(aliasBot, "session.create", {
      profile: "worker",
      title: "Bot Chat",
    });
    await requestForBot(aliasBot, "cli.exec", {
      argv: ["--profile", "worker", "config", "unset", "model"],
    });
    await requestForBot(aliasBot, "cli.exec", {
    ...
    expect(
      requestProfile.mock.calls.every(([route]) => route.profile === "worker"),
    ).toBe(true);
  });
});
...
describe("a group room seats members from several machines", () => {
  ...
  it("resolves @name-device to the remote member, keeping same-named agents distinct", () => {
    const members = [
      ...
      {
        connectionId: "mac-mini",
        handle: "dixie-mac-mini",
        name: "dixie",
        remoteSource: true,
      },
    ] as GroupMember[];

    const parsed = parseGroupChatMentions(
      "hey @dixie-mac-mini can you check disk space",
      members,
    );
  ...
  it("badges cross-connection speakers with their device in lines and turn prompts", () => {
    expect(
      formatGroupChatLine(
        {
          at: 1,
          from: { kind: "member", name: "dixie", source: "Mac Mini" },
          text: "done",
        },
        "ops",
      ),
