...
vi.mock("./data", async () => {
  ...
  saveBotMeta.mockImplementation(
    async (bot: { name: string }, patch: Record<string, unknown>) => {
      $botMeta.set({
        ...$botMeta.get(),
        [bot.name]: { ...$botMeta.get()[bot.name], ...patch },
      });
...
vi.mock("./routing", () => ({
  botRosterMeta: (bot: { name: string }, meta: Record<string, unknown>) =>
    meta[bot.name],
}));
...
describe("user sections", () => {
  it("filing writes one sectionId per bot through saveBotMeta and survives a reload", async () => {
    ...
    expect(saveBotMeta).toHaveBeenCalledWith(bot("nanox"), {
  ...
  it("groups every row exactly once; unknown or missing sections fall to Unassigned, drawn last", () => {
    ...
    expect(groupRowsBySection(rows, [], meta)).toEqual([
      { id: null, key: UNASSIGNED_SECTION_KEY, name: "", rows },
    ]);
  });
  ...
  it("deleting a section returns its bots to Unassigned, and undo refiles them", async () => {
    ...
    await vi.waitFor(() =>
      expect($botMeta.get().nanox?.sectionId).toBe(section.id),
    );

    const { members, undo } = deleteBotSection(section.id, [
      bot("nanox"),
      bot("scout"),
    ]);
