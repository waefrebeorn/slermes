...
const {
  McpTab,
  ToolsetConfigPanel,
}: Partial<Pick<typeof sdk, "McpTab" | "ToolsetConfigPanel">> = sdk;
export const SkillsView =
  typeof sdk === "undefined" ? undefined : sdk.SkillsView;
...
export const skillsViewRoutesConnections = Boolean(
  SkillsView && SkillsView.supportsFixedConnection,
);
...
interface AdvancedProfileConfigProps {
  bot: RosterRow;
  setState: (
    update: (prev: AdvancedConfigState) => AdvancedConfigState,
  ) => void;
  state: AdvancedConfigState;
}

export function AdvancedProfileConfig({
  bot,
  state,
  setState,
}: AdvancedProfileConfigProps) {
  const b = useBots();
  ...
  const backendProfile =
    botRoute?.targetProfile || botRoute?.profile || bot.name;
  ...
  if (unsupported) {
    return (
      <div className="px-2 py-3 text-center text-xs text-(--ui-text-tertiary)">
        Full configuration needs a newer gateway (restart it after updating
        Hermes).
      </div>
    );
  }
    ...
    return (
      <div className="grid gap-4">
        ...
        <div className="rounded-md border border-(--ui-stroke-secondary) px-3 py-2 text-xs text-(--ui-text-tertiary)">
          Remote capabilities require a newer desktop. Model and SOUL changes
          remain staged until you save.
        </div>
        {labeled(
          "SOUL.md (persona + agent-messaging protocol)",
          <Textarea
                  ...
                  <label className="flex items-center gap-2 text-xs font-medium text-(--ui-text-secondary)">
                    <Checkbox
                      ...
                      onCheckedChange={(value) =>
                        toggleToolset(tset.name, Boolean(value))
                      }
                    />
                    <span>{tset.name}</span>
                  </label>
...
export async function applyAdvancedConfig(
  ...
  if (state.dirtySoul) {
    payload.soul = ensureMessagingProtocol(
      state.soul,
      bot.name,
      $lastRoster.get(),
    );
