...
interface HubSkillsSectionProps {
  ...
  forProfile:
    null | string | { connectionId?: null | string; profile?: null | string };
...
export function HubSkillsSection({
  ...
  const installRef = useRef<
    ((name: string, displayName?: string) => Promise<void>) | null
  >(null);
  ...
  const search = async () => {
    ...
    try {
      const res: { results?: HubSkillResult[] } = await host.request(
        "skills.manage",
        {
          action: "search",
          query: q,
  ...
  return (
    <div className="grid gap-1.5 border-t border-(--ui-stroke-secondary) pt-2">
      ...
      {results === null ? null : results.length === 0 ? (
        <div className="px-1 py-1.5 text-[0.7rem] text-(--ui-text-quaternary)">
          No hub skills matched.
        </div>
      ) : (
        <div
