...
describe("removed-connection sweep", () => {
  it("annotates matching members and persists; other rows are untouched", async () => {
    ...
    chat.$groupChats.set({
      Crew: {
        log: [
          { at: 1, from: { kind: "user", name: "You" }, id: "1", text: "hi" },
        ],
        members: [
          member(),
          member({
    ...
    expect(chat.sweepGroupChatMembersForRemovedConnection("conn-gone")).toBe(
      true,
    );
...
describe("hydrate-time annotate", () => {
  it("marks lost-connectionId rows even without a registry", async () => {
    ...
    const { rooms: next, changed } = hygiene.annotateOrphanedGroupChatMembers(
      rooms,
      null,
    );
    ...
    expect(data.botSourceStatus(next.Crew.members?.[0]).label).toBe(
      "Gateway removed",
    );
  });
  ...
  it("with a live registry, marks members on dead connections and keeps live and local ones", async () => {
    ...
    const seated = (name: string) =>
      next.Crew.members?.find((row) => row.name === name);
...
describe("render-path callers", () => {
  it("degrade on an orphaned row instead of throwing", async () => {
    const { chat } = await load();
    const [membership, routing] = await Promise.all([
    ...
    expect(() =>
      routing.setBotsWorkspaceOwner("bot:halakukhan", orphaned),
    ).not.toThrow();
