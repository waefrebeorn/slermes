...
const ROUTED_MEMBER: GroupMember = {
  connectionId: "mini",
  name: "helper",
  remoteSource: true,
};
...
describe("session resolution", () => {
  it("pins session titles to the roomId, with a legacy fallback to the display name", async () => {
    ...
    const legacy = await room.turns.ensureGroupChatSession("Legacy", {
      name: "research",
      title: "",
    });

    expect(room.gateway.sessions.get(String(legacy.stored))?.title).toBe(
      "Group: Legacy",
    );
    ...
    const fresh = await room.turns.ensureGroupChatSession("New", {
      name: "research",
      title: "",
    });

    expect(room.gateway.sessions.get(String(fresh.stored))?.title).toBe(
      "Group: r-abc",
    );
  });
  ...
  it("creates member sessions with the room_plumbing + follow_profile_config contracts", async () => {
    ...
    const handle = await room.turns.ensureGroupChatSession("Contract", {
      name: "research",
      title: "",
    });
  ...
  it("mints fresh member sessions when a same-name group is recreated after disband", async () => {
    ...
    expect(room.gateway.sessions.get(String(first.stored))?.title).toBe(
      "Group: r-one",
    );
    expect(room.gateway.sessions.get(String(second.stored))?.title).toBe(
      "Group: r-two",
    );
  });
  ...
  it("fails closed on a transient resume failure instead of forking the member session", async () => {
    ...
    host.request = async (method: string, params: Record<string, unknown>) => {
      if (method === "session.resume" && params.session_id === first.stored) {
        throw Object.assign(new Error("gateway temporarily unavailable"), {
          code: 5000,
        });
      }
    ...
    await expect(
      room.turns.ensureGroupChatSession("Alpha", member),
    ).rejects.toThrow(/Could not check .*group session/);
  ...
  it("falls through a genuine 4007 to the title lookup, then creates", async () => {
    ...
    const result = await room.turns.ensureGroupChatSession("Alpha", {
      name: "research",
      title: "",
    });
...
describe("session-gone classification", () => {
  it('treats 4001 and "not in memory" as recoverable, 4007 as not', async () => {
    ...
    expect(
      turns.isSessionGoneError(Object.assign(new Error("x"), { code: 4001 })),
    ).toBe(true);
    expect(
      turns.isSessionGoneError(new Error("session_id=rt-1 not in memory")),
    ).toBe(true);
    expect(
      turns.isSessionGoneError(
        Object.assign(new Error("session not found"), { code: 4007 }),
      ),
    ).toBe(false);
  ...
  it("recovers a 4001 on the first submit via the STORED id and delivers", async () => {
    const room = await loadRoom({
      failFirstSubmitWith: Object.assign(
        new Error(
          "session-scoped RPC rejected: session_id='rt-1' not in memory",
        ),
        {
          code: 4001,
        },
  ...
  it("does not retry a persistent non-4001 submit failure", async () => {
    const room = await loadRoom({
      failEverySubmitWith: new Error("backend exploded"),
    });

    await expect(
      room.turns.runGroupChatMemberTurn("Room", LOCAL_MEMBER, "hi", "t1", []),
...
describe("per-turn socket lease", () => {
  ...
  it("is released after the turn — the refcount returns to zero", async () => {
    ...
    await room.turns.runGroupChatMemberTurn(
      "Room",
      ROUTED_MEMBER,
      "hi",
      "t1",
  ...
  it("is released even when the turn fails", async () => {
    ...
    await expect(
      room.turns.runGroupChatMemberTurn("Room", ROUTED_MEMBER, "hi", "t1", []),
    ).rejects.toThrow();
    expect(room.gateway.refcount()).toBe(0);
  });
...
describe("push-woken poll", () => {
  it("wakes on the member session's message.complete instead of sleeping out the backstop", async () => {
    ...
    const turn = room.turns.runGroupChatMemberTurn(
      "Room",
      LOCAL_MEMBER,
      "hi",
      "t1",
    ...
    const runtime = room.gateway.sessions.get(
      room.gateway.calls[0]?.stored,
    )?.runtime;
...
describe("clarify and approvals (#90694)", () => {
  ...
  it("holds the turn open while a member is blocked on clarify, then lands the reply", async () => {
    ...
    const replies = log(room, "Core").filter(
      (entry) => entry.thread === thread && entry.from.kind === "member",
    );
  ...
  it("never mirrors a question for older backends without pending_clarify", async () => {
    ...
    expect(
      turns.syncGroupClarify("Core", { name: "research" }, { messages: [] }),
    ).toBe(false);
    expect(Object.keys(chat.$groupClarify.get())).toHaveLength(0);
  });
  ...
  it("routes an answer through clarify.respond and clears the mirror", async () => {
    ...
    await room.turns.answerGroupClarify(
      Object.values(room.chat.$groupClarify.get())[0],
      member,
      "staging",
    );

    expect(
      room.gateway.rpcFor("clarify.respond").map((call) => call.params),
    ).toEqual([{ answer: "staging", request_id: "req-clarify-1" }]);
    expect(Object.keys(room.chat.$groupClarify.get())).toHaveLength(0);
  });
  ...
  it("clears only the disbanded room’s mirrored questions", async () => {
    ...
    room.turns.syncGroupClarify(
      "Other",
      { name: "ops" },
      { pending_clarify: { ...CLARIFY, request_id: "req-2" } },
    );
  ...
  it("falls back to once/deny when the server sends no choice set", async () => {
    ...
    expect(Object.values(chat.$groupClarify.get())[0].choices).toEqual([
      "once",
      "deny",
    ]);
  });
  ...
  it("routes approvals through approval.respond with the session and choice", async () => {
    ...
    expect(
      room.gateway.rpcFor("approval.respond").map((call) => call.params),
    ).toEqual([
      {
        choice: "once",
        request_id: "req-approval-1",
  ...
  it("lets clarify outrank approval when a snapshot carries both", async () => {
    ...
    turns.syncGroupClarify(
      "Core",
      { name: "research" },
      { pending_approval: APPROVAL, pending_clarify: CLARIFY },
    );
...
describe("stranded harvest", () => {
  ...
  it("prefers the substantive answer over a trailing synthetic (pass)", async () => {
    ...
    await room.turns.harvestStrandedGroupReply("Rescue", {
      name: "research",
      title: "",
    });
    ...
    expect(
      room.chat.$groupChats.get().Rescue.stranded?.research,
    ).toBeUndefined();
  });
  ...
  it("consumes the marker without posting when the late reply is a pass", async () => {
    ...
    await room.turns.harvestStrandedGroupReply("Quiet2", {
      name: "builder",
      title: "",
    });
    ...
    expect(
      room.chat.$groupChats.get().Quiet2.stranded?.builder,
    ).toBeUndefined();
  });
  ...
  it("never re-submits into a member the harvest just confirmed is still running", async () => {
    ...
    const room = await loadRoom({
      ...
      turn: ({ profile }) =>
        profile === "builder" ? "builder here, all good" : "(pass)",
    });
    ...
    expect(
      room.gateway.calls.filter((call) => call.profile === "research"),
    ).toHaveLength(0);
    ...
    expect(
      room.gateway.calls.filter((call) => call.profile === "builder"),
    ).toHaveLength(1);
  });
});
...
describe("room record", () => {
  it("persists the roomId alongside the room and survives another room’s disband", async () => {
    ...
    room.chat.updateGroupChat("Keep", (current) => {
      current.log = [
        { at: 1, from: { kind: "user", name: "You" }, id: "k1", text: "hi" },
      ];
    ...
    const durable = (room.gateway.storage.get("group-chats") || {}) as Record<
      string,
      GroupChat
    >;
