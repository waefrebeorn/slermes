...
function feed(room: Room, group: string): ActivityRow[] {
  return (room.activity.$groupActivity.get()[group]?.events ||
    []) as ActivityRow[];
}
...
describe("turn arc", () => {
  it("a settled turn records the full truthful arc: queued, working, replies and passes, settled", async () => {
    const room = await loadRoom({
      turn: ({ profile, prompt }) =>
        profile === "research" && !prompt.includes("(you)")
          ? "I looked into it — ship it."
          : "(pass)",
    });
    ...
    expect(
      kinds.filter((kind) => kind === "working").length,
    ).toBeGreaterThanOrEqual(3);
    expect(kinds).toContain("replied");
    expect(
      kinds.filter((kind) => kind === "passed").length,
    ).toBeGreaterThanOrEqual(2);
    expect(events.find((event) => event.kind === "replied")?.member).toBe(
      "research",
    );
  });
  ...
  it("a failed member turn preserves and prefers its typed reason", async () => {
    ...
    await drain(() =>
      Boolean(room.chat.$groupChats.get()["Typed failure"]?.running),
    );

    const failed = feed(room, "Typed failure").find(
      (event) => event.kind === "failed" && event.member === "builder",
    );

    expect(failed?.reason).toBe("provider_auth_or_access");
    expect(Object.values(room.data.$botAttention.get())[0]?.reason).toBe(
  ...
  it("an untyped failed member turn keeps the message-classification fallback", async () => {
    ...
    await drain(() =>
      Boolean(room.chat.$groupChats.get()["Untyped failure"]?.running),
    );

    const failed = feed(room, "Untyped failure").find(
      (event) => event.kind === "failed" && event.member === "builder",
    );
...
describe("epoch scoping", () => {
  it("a newer send interrupts the previous run and records cancelled in the CURRENT epoch", async () => {
    ...
    expect(feed(room, "Busy").some((event) => event.kind === "cancelled")).toBe(
      true,
    );
    // The view shows only the current run: every visible event is this epoch.
    expect(
      room.activity
        .currentGroupActivity("Busy")
        .every((event) => (event.epoch || 0) === epoch),
    ).toBe(true);
    expect(
      room.activity
        .currentGroupActivity("Busy")
        .some((event) => event.kind === "cancelled"),
    ).toBe(true);
  });
  ...
  it("epoch filtering drops events from a superseded run", async () => {
    ...
    room.activity.recordGroupActivity("Epoch", {
      kind: "queued",
      member: "You",
    });
    room.activity.recordGroupActivity("Epoch", {
...
describe("feed shape", () => {
  it("the feed is bounded: it stops growing and keeps the newest events", async () => {
    ...
    for (let i = 0; i < 500; i++) {
      room.activity.recordGroupActivity("Cap", {
        kind: "working",
        member: `member-${i}`,
      });
    }
  ...
  it("labels read like a person wrote them, with settled/cancelled as room-level lines", async () => {
    ...
    expect(label({ kind: "replied", member: "research" })).toBe(
      "research replied",
    );
    expect(label({ kind: "timed-out", member: "ops" })).toBe(
      "ops took too long",
    );
    expect(label({ kind: "cancelled", member: null })).toBe(
