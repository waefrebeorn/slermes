...
const petFrameCache = new LruCache<string, Promise<null | string>>(
  PET_FRAME_CACHE_MAX,
);
...
export function PetTab({ image, onImage }: PetTabProps) {
  ...
  const ranked = filtered.slice().sort((a, b) => {
    const rank = (pet: PetGalleryEntry) =>
      pet.installed ? 0 : pet.curated ? 1 : 2;
  ...
  return (
    <div className="grid w-full gap-2">
      ...
      {filtered.length === 0 ? (
        <div className="py-3 text-center text-xs text-(--ui-text-quaternary)">
          No pets match.
        </div>
      ) : (
        <div
