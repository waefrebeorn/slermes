...
const { hostMock, persistMock, pluginCtx, requestForBotMock, saveBotMetaMock } =
  vi.hoisted(() => ({
    ...
    pluginCtx: {
      current: null as null | { i18n?: { t: (key: string) => string } },
    },
...
describe("the lazy row is materialized before anything else touches it", () => {
  ...
  it("sends the one intro turn on New Bot creation (kickoff: true)", async () => {
    ...
    expect(await createCanonicalChat("ops", { kickoff: true })).toBe(
      "stored-1",
    );
    expect(events).toEqual([
      "session.list",
  ...
  it("speaks the intro in the active locale (#91827)", async () => {
    ...
    pluginCtx.current = {
      i18n: {
        t: (key) =>
          key === "bot.kickoff" ? "こんにちは、自己紹介をしてください！" : key,
      },
    };

    expect(await kickoffTextSent()).toBe(
      "こんにちは、自己紹介をしてください！",
    );
  });
  ...
  it("still returns the created registry row when the intro fails", async () => {
    ...
    expect(await createCanonicalChat("newbie", { kickoff: true })).toBe(
      "new-bot-chat",
    );
  });
});
...
describe("a superseded click completes registry-side but never navigates", () => {
  it("creates the canonical row without stealing the workspace", async () => {
    ...
    expect(
      await createCanonicalChat("ops", { openingStillCurrent: () => current }),
    ).toBe("new-stored");
  ...
  it("lets a newer same-bot open take over the in-flight creation navigation", async () => {
        ...
        return new Promise((resolve) => {
          releaseCreate = () =>
            resolve({
              ...
              stored_session_id: "shared-stored",
            });
        });
      }
    ...
    const first = createCanonicalChat("ops", {
      openingStillCurrent: () => firstCurrent,
    });
