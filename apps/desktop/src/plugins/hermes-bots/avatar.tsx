...
export const blobatarSvg =
  typeof sdk === "undefined" ? undefined : sdk.blobatarSvg;
...
const createBudgetedLoop =
  typeof sdk === "undefined" ? undefined : sdk.createBudgetedLoop;
...
export function isBlobShape(shape: null | string | undefined) {
  return (
    shape === "blobatar" ||
    (typeof shape === "string" && shape.startsWith("blobatar:"))
  );
}
...
export function parseBlobShape(
  shape: null | string | undefined,
...
function blobMarkup(
  ...
  try {
    return blobatarSvg(seed, opts).replace(
      "<svg ",
      "<svg data-bot-face=" + JSON.stringify(name) + " ",
    );
  } catch {
    return null;
  }
...
function sampleFaceRing(
  ...
  for (let i = 0; i < steps; i++) {
    ...
    if (kind === "circle") {
      ...
      const d =
        Math.pow(Math.abs(c) ** 8 + Math.abs(s / 0.72) ** 8, 1 / 8) || 1;
      rx = ry = 16 / d;
    } else if (
      kind === "triangle" ||
      kind === "tetrahedron" ||
...
function paintMathFace(svg: SVGSVGElement, t: number) {
  ...
  if (body) {
    if (shape === "cloud") {
      ...
      const ring = sampleFaceRing(shape).map(([x, y]) =>
        projectFacePoint(x, y, pose.turn, pose.tilt, pose.roll),
      );
...
function walkMathFaces(
  ...
  root
    .querySelectorAll<SVGSVGElement>("svg[data-hb-math]")
...
export function startFaceClock() {
  ...
  const idle = () =>
    faces.length === 0 || (observer && visibleFaces.size === 0);
...
export function BotFace({
  ...
  const eyeFill = isDarkColor(color)
  ...
  const hlFill = isDarkColor(color)
    ? "rgba(0,0,0,0.6)"
