...
import {
  groupMemberReferencesConnection,
  markOrphanedGroupMemberDescriptor,
} from "./hygiene";
...
function normalizeGroupChatSyncSnapshot(
  ...
  if (Number(snapshot.version || 0) >= 3) {
    return {
      ...
      rooms:
        snapshot.rooms && typeof snapshot.rooms === "object"
          ? snapshot.rooms
          : {},
      deleted:
        snapshot.deleted && typeof snapshot.deleted === "object"
          ? snapshot.deleted
          : {},
    };
  }
  ...
  for (const [name, at] of Object.entries(snapshot.deleted || {})) {
    ...
    deleted[`name:${name}`] =
      Number(snapshot.version || 0) >= 2 ? Math.max(0, Number(at || 0)) : 0;
  }
...
export function groupChatSyncSnapshot(
  ...
  const ranked = Object.entries(all || {})
    ...
    .filter(
      ([, room]) => room && Array.isArray(room.log) && room.log.length > 0,
    )
    .sort(([, left], [, right]) => {
  ...
  for (const [name, room] of ranked) {
    const log: GroupMessage[] = room.log
      .slice(-GROUP_CHAT_SYNC_MESSAGES)
      .map((entry) => ({
        ...(entry?.id
          ? {
        ...
        from: {
          ...
          name: String(
            entry?.from?.name ||
              (entry?.from?.kind === "member" ? "Bot" : "You"),
          ).slice(0, 128),
          ...(entry?.from?.source
            ? {
    ...
    while (
      ...
      groupChatGatewayJsonSize(envelope) > GROUP_CHAT_SYNC_MAX_BYTES
    ) {
      compact.log.shift();
    }
...
export function mergeGroupChatSyncSnapshots(
  ...
  for (const source of [remoteNorm, localNorm]) {
    for (const [key, at] of Object.entries(source.deleted || {})) {
      deleted[key] = Math.max(
        Number(deleted[key] || 0),
        Math.max(0, Number(at || 0)),
      );
    }
  }
  ...
  for (const label of deletedRooms) {
    for (const key of new Set([
      ...keysFor(label, remoteNorm),
      ...keysFor(label, localNorm),
    ])) {
  ...
  const roomKeys = new Set([
    ...Object.keys(remoteNorm.rooms || {}),
    ...Object.keys(localNorm.rooms || {}),
  ]);
  ...
  for (const key of roomKeys) {
    ...
    if (
      (!remoteRoom || !Array.isArray(remoteRoom.log)) &&
      (!localRoom || !Array.isArray(localRoom.log))
    ) {
      continue;
    }
    ...
    if (localRevision > remoteRevision) {
      ...
      members = [...byId.values()];
      image = Object.prototype.hasOwnProperty.call(localRoom || {}, "image")
        ? localRoom.image
        : remoteRoom?.image;
    }
    ...
    rooms[key] = {
      ...
      log: [...entries.values()].sort((left, right) => {
        ...
        return (
          byTime ||
          groupChatSyncEntryKey(left).localeCompare(
            groupChatSyncEntryKey(right),
          )
        );
      }),
      members,
  ...
  for (const [key, deletedRevision] of Object.entries(deleted)) {
    if (key.startsWith("id:")) {
      ...
      Number(deletedRevision || 0) >= Number(rooms[key]?.revision || 0)
    ) {
      delete rooms[key];
    } else {
      delete deleted[key];
...
export function mergeRemoteGroupChatSnapshotIntoRooms(
  ...
  current: Record<string, GroupChat> = $groupChats.get(),
  {
    preserveRooms = [],
    deletedRooms = [],
  }: { deletedRooms?: string[]; preserveRooms?: string[] } = {},
) {
  const remoteNorm = normalizeGroupChatSyncSnapshot(remote);
  ...
  for (const [key, projected] of Object.entries(remoteNorm.rooms || {})) {
    ...
    const displayName = String(
      projected.name ||
        localName ||
        (key.startsWith("name:") ? key.slice(5) : key),
    );

    if (
      locallyDeleted.has(displayName) ||
      (localName && locallyDeleted.has(localName))
    ) {
    ...
    const entries = new Map<string, GroupMessage>(
      (Array.isArray(existing.log) ? existing.log : []).map((entry) => [
        groupChatSyncEntryKey(entry),
        entry,
      ]),
    );

    const members = new Map<string, GroupMember>(
      (Array.isArray(existing.members) ? existing.members : []).map(
        (member) => [groupChatSyncMemberKey(member), member],
      ),
    );
    ...
    const isPreserved =
      preserved.has(displayName) || (localName && preserved.has(localName));
    ...
    if (!isPreserved) {
      ...
      for (const member of Array.isArray(projected.members)
        ? projected.members
        : []) {
    ...
    rooms[targetName] = {
      ...
      sessions:
        existing.sessions && typeof existing.sessions === "object"
          ...
          : {},
      stranded:
        existing.stranded && typeof existing.stranded === "object"
          ? existing.stranded
          : {},
      ...
      image: isPreserved
        ...
        : remoteRevision >= localRevision &&
            Object.prototype.hasOwnProperty.call(projected, "image")
          ? projected.image || null
          : existing.image || null,
      syncRevision: isPreserved
        ? localRevision
        : Math.max(remoteRevision, localRevision),
...
export function durableGroupChatRooms(
  ...
  for (const [name, room] of Object.entries(all || {})) {
    ...
    durable[name] = {
      ...
      // already carries.
      roomId:
        typeof room.roomId === "string" && room.roomId ? room.roomId : null,
...
export function persistGroupChatRooms(
  all: Record<string, GroupChat> = $groupChats.get(),
) {
  try {
    return Promise.resolve(
      getPluginCtx()?.storage?.set?.("group-chats", durableGroupChatRooms(all)),
    ).catch(() => undefined);
  } catch {
    return Promise.resolve();
  }
...
export function sweepGroupChatMembersForRemovedConnection(
  connectionId: string,
) {
  ...
  for (const [name, room] of Object.entries($groupChats.get())) {
    ...
    if (
      !members.some(
        (member) =>
          groupMemberReferencesConnection(member, id) && !member?.sourceMissing,
      )
    ) {
      continue;
    }
    ...
    updateGroupChat(name, (current: GroupChat) => ({
      ...current,
      members: (Array.isArray(current.members) ? current.members : []).map(
        (member) =>
          groupMemberReferencesConnection(member, id)
            ? markOrphanedGroupMemberDescriptor(member)
            : member,
      ),
    }));
  }
...
async function groupChatSyncRequest<T>(
  ...
  if (
    ...
    const route = (Array.isArray(routes) ? routes : []).find((candidate) => {
      const profile = String(
        candidate?.targetProfile || candidate?.profile || "",
      );

      return (
        String(candidate?.connectionId || "") === job.connectionId &&
        profile === "default"
      );
    });
  ...
  if (
    job.connectionId &&
    currentConnectionId &&
    job.connectionId !== currentConnectionId
  ) {
...
async function groupChatRemoteSnapshot(job: GroupChatSyncJob) {
  const result = await groupChatSyncRequest<{ profiles?: RosterRow[] }>(
    job,
    "profiles.list",
    {
  ...
  const profile = (Array.isArray(result?.profiles) ? result.profiles : []).find(
    (row) => row?.name === "default",
  );
  const snapshot = profile?.ui_meta?.[GROUP_CHAT_SYNC_META_KEY] as
    GroupChatSyncSnapshot | undefined;
  const supportsCas = Boolean(
    profile &&
    Object.prototype.hasOwnProperty.call(profile, "ui_meta_revisions"),
  );
  ...
  return {
    ...
    revision: Math.max(
      0,
      Number(profile?.ui_meta_revisions?.[GROUP_CHAT_SYNC_META_KEY] || 0),
    ),
    supportsCas,
  };
}
...
export async function pullGroupChatServerState(
  ...
  const pending = groupChatSyncPendingByConnection.get(
    String(connectionId || ""),
  );
...
function mergeGroupChatSyncJobs(
...
async function flushGroupChatServerSync(connectionId?: string) {
  ...
  if (
    ...
    groupChatSyncInFlightConnections.has(id) ||
    !groupChatSyncPendingByConnection.has(id)
  ) {
    return;
  }
  ...
  try {
    ...
    if (
      ...
      if (remoteState.snapshot) {
        ...
        const mergedRooms = mergeRemoteGroupChatSnapshotIntoRooms(
    ...
    if (
      ...
      Number(
        result?.applied?.ui_meta_revisions?.[GROUP_CHAT_SYNC_META_KEY] || 0,
      ) !== writeRevision
    ) {
    ...
    if (!groupChatSyncDisposed) {
      ...
      groupChatSyncPendingByConnection.set(
        id,
        mergeGroupChatSyncJobs(groupChatSyncPendingByConnection.get(id), job),
      );
      groupChatSyncRetryCounts.set(id, retries);

      if (
        typeof setTimeout === "function" &&
        !groupChatSyncRetryTimers.has(id)
      ) {
...
export function groupSpeakerLabel(name?: null | string) {
  ...
  const row = Array.isArray(roster)
    ? roster.find(
        (bot) =>
          bot?.name === trimmed && !bot?.remoteSource && !bot?.sourceScoped,
      )
    : null;
...
export function updateGroupChat(
  ...
  try {
    ...
    Promise.resolve(
      getPluginCtx()?.storage?.set?.("group-chats", durable),
    ).catch(() => undefined);
  } catch {
...
export function setGroupChatImage(
  group: string,
...
export function assignLegacyThreads(log: GroupMessage[]): GroupMessage[] {
  ...
  return (log || []).map((entry, i) => {
    ...
    const prev = log[i - 1];
    const lull =
      !prev || (entry.at || 0) - (prev.at || 0) > GROUP_THREAD_GAP_MS;
