...
function boundedModelOptionsFetch<T>(
  fetch: Promise<T>,
  settleMs = MODEL_OPTIONS_SETTLE_MS,
): Promise<T> {
  ...
  const deadline = new Promise<never>((_, reject) => {
    timerId = scope.setTimeout(() => {
      reject(
        new Error(
          `model.options did not answer within ${Math.round(settleMs / 1000)}s (#95279 settle guard)`,
        ),
      );
    }, settleMs);
  });

  return Promise.race([fetch, deadline]).finally(() =>
...
export function ModelPicker({
  ...
  const isKnown =
    !value.provider ||
    value.provider === NONE ||
    providers.some((p) => p.slug === value.provider);
          ...
          onValueChange={(v) => {
            if (v === NONE) {
              ...
              onChange({
                provider: v,
                model:
                  prov && provModels.includes(value.model)
                    ? value.model
                    : first,
              });
            }
