...
describe("the facts the row never showed", () => {
  ...
  it("promises a next run only while the job is active", () => {
    ...
    const paused = routineDetailRows({
      ...activeJob,
      enabled: false,
      state: "paused",
    });
  ...
  it("shows the raw schedule only when the humanized label dropped something", () => {
    ...
    expect(valueOf(routineDetailRows(activeJob), "Schedule (raw)")).toBe(
      "every 1440m",
    );

    // A schedule the label passes through unchanged would only be duplicated.
    const passthrough = routineDetailRows({
      ...activeJob,
      schedule: "0 9 * * 1-5",
    });
  ...
  it("spells out every last_status the scheduler writes", () => {
    ...
    expect(routineLastResult("delivery_failed")).toBe(
      "Ran, but delivery failed",
    );
    expect(routineLastResult("blocked_config")).toBe(
      "Blocked by configuration (not run)",
    );
    ...
    expect(
      valueOf(
        routineDetailRows({ ...activeJob, last_status: "delivery_failed" }),
        "Last result",
      ),
    ).toBe("Ran, but delivery failed");
  });

  it("explains a failing or scheduler-paused job in failure order", () => {
    expect(routineDetailIssue(activeJob)).toBeNull();
    expect(
      routineDetailIssue({ ...activeJob, paused_reason: "too many failures" }),
    ).toBe("too many failures");
    expect(
      routineDetailIssue({
        ...activeJob,
...
describe("the row is reachable", () => {
  it("opens THIS job from its own title control", async () => {
    ...
    render(
      <RoutineRow
        job={activeJob}
        onOpen={(job) => opened.push(job)}
        owner={{ name: "notetaker" }}
      />,
    );
  ...
  it("refuses to run a legacy delegated routine, and says why", () => {
    const legacy: RoutineJob = {
      ...activeJob,
      prompt_preview:
        "You are running the scheduled routine \"Morning digest\" for agent 'notetaker'.",
    };
...
describe("the inspector", () => {
  it("renders the job\u2019s instruction and its failure", () => {
    ...
    expect(
      within(dialog).getByText("Summarize yesterday and post it."),
    ).toBeTruthy();
