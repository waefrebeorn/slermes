...
vi.mock("@hermes/plugin-sdk", async () => {
  ...
  return {
    atom,
    blobatarSvg: (seed: string, opts: unknown) =>
      blobatarSvgMock(seed, opts) as string,
    createBudgetedLoop: undefined,
    host: { state: { connectionId: { get: () => "local" } } },
    profileColor: (name: string) =>
      name === "inbox-triage" ? "#38bdf8" : "#8b5cf6",
...
const lastBlobCall = () =>
  blobatarSvgMock.mock.calls.at(-1) as [string, BlobOptions];

beforeEach(() => {
  vi.clearAllMocks();
  blobatarSvgMock.mockReturnValue(
...
describe("blob shape strings round-trip through parse/build", () => {
  ...
  it("parses the seed/silhouette segments, ignoring an unknown silhouette", async () => {
    ...
    expect(parseBlobShape("blobatar", "inbox-triage")).toEqual({
      kind: "",
      seed: "inbox-triage",
      seedPart: "",
    });
...
describe("rendering a blob face", () => {
  it("tags the markup data-bot-face so the roster PNG backfill can find it", async () => {
    ...
    expect(
      container.querySelector('svg[data-bot-face="inbox-triage"]'),
    ).toBeTruthy();
  ...
  it("falls back to the legacy math face when the renderer throws", async () => {
    ...
    const { BotFace } = await import("./avatar");
    const { container } = render(
      <BotFace color="#38bdf8" name="agent" shape="blobatar" size={32} />,
    );
...
describe("catchlight contrast follows the pupil, not the body", () => {
  const catchlights = (container: HTMLElement) =>
    ["l", "r"].map((side) =>
      container.querySelector(`[data-hb-hl-${side}]`)?.getAttribute("fill"),
    );

  it("puts a DARK sparkle on the cream pupils of a dark body", async () => {
    const { BotFace } = await import("./avatar");
    const { container } = render(
      <BotFace color="#3b0910" name="oxblood" shape="circle" />,
    );

    expect(catchlights(container)).toEqual([
      "rgba(0,0,0,0.6)",
      "rgba(0,0,0,0.6)",
    ]);
  });
  ...
  it("keeps the white sparkle on the dark pupils of a light body", async () => {
    const { BotFace } = await import("./avatar");
    const { container } = render(
      <BotFace color="#f5e6c8" name="cream" shape="circle" />,
    );
...
describe("an SDK that predates blobatarSvg", () => {
  it("renders the legacy deterministic shape instead of nothing", async () => {
    ...
    expect(
      container
        .querySelector("svg[data-hb-math]")
        ?.getAttribute("data-hb-shape"),
    ).toBe(defaultShapeFor("agent"));
  });
});
