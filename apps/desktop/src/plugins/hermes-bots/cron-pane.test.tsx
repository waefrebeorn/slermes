...
const $focused = host.state.focusedSessionOwner as unknown as ReturnType<
  typeof atom
>;
...
describe("the pane follows the roster hydrating after mount", () => {
  it("starts fail-closed, then paints once the row arrives", async () => {
    ...
    expect(
      screen.getByText("This bot has to appear in the roster first."),
    ).toBeTruthy();
    ...
    await waitFor(() =>
      expect(
        screen.queryByText("This bot has to appear in the roster first."),
      ).toBeNull(),
    );
