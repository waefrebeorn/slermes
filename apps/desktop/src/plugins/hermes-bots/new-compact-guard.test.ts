...
function bot(
  canonical: null | { id?: string; resolved_id?: string },
): RosterRow {
...
describe("isCanonicalChatOnScreen", () => {
  it("matches the durable registry row", () => {
    expect(
      isCanonicalChatOnScreen(bot({ id: "forever-chat" }), "forever-chat"),
    ).toBe(true);
  });
  ...
  it("leaves an ordinary session alone", () => {
    ...
    expect(
      isCanonicalChatOnScreen(bot({ id: "forever-chat" }), "scratch"),
    ).toBe(false);
  });
  ...
  it("declines on a missing id rather than matching everything", () => {
    ...
    // a loose comparison.
    expect(isCanonicalChatOnScreen(bot({ id: "forever-chat" }), null)).toBe(
      false,
    );
    expect(isCanonicalChatOnScreen(bot({ id: "forever-chat" }), "")).toBe(
      false,
    );
