...
interface RowFixture extends Omit<
  Partial<RosterRow>,
  "last_session" | "worker_session"
> {
...
describe("previewKind classifies a roster preview", () => {
  const fromBot = (preview: null | string | undefined) =>
    previewKind(preview).fromBot;
  ...
  it("parses the current 🤖 delivery prefix and the legacy agent shape", () => {
    ...
    expect(fromBot("Message from agent 'researcher': here is the paper")).toBe(
      "researcher",
    );
  });

  it("surfaces the primary profile as @hermes, never @default (#89484)", () => {
    expect(fromBot("Message from agent 'default': deploy is green")).toBe(
      "hermes",
    );
...
describe("the canonical session id everything core-keyed reads", () => {
  it("prefers the compression-lineage tip over the durable registry id", () => {
    expect(
      botCanonicalSessionId(
        row({
          canonical_session: { id: "root", last_active: 1, resolved_id: "tip" },
        }),
      ),
    ).toBe("tip");
    expect(
      botCanonicalSessionId(
        row({ canonical_session: { id: "root", last_active: 1 } }),
      ),
    ).toBe("root");
  });
  ...
  it("is never last_session — that is a different conversation entirely", () => {
    expect(
      botCanonicalSessionId(
        row({ last_session: { id: "scratch", last_active: 1 } }),
      ),
    ).toBeNull();
  });
});
...
describe("which bots are working right now", () => {
  const roster = [
    row({
      last_session: { id: "a", last_active: secondsAgo(10) },
      name: "researcher",
    }),
    row({
      last_session: { id: "b", last_active: secondsAgo(400) },
      name: "scribe",
    }),
    row({ name: "analyst" }),
  ];
  ...
  it("includes the gateway-busy profile before its first response lands", () => {
    ...
    expect(
      activeBots(roster, "analyst", "busy", NOW).map((bot) => bot.name),
    ).toContain("analyst");
  });

  it("includes activity inside the liveness window and excludes activity outside it", () => {
    const names = activeBots(roster, "default", "open", NOW).map(
      (bot) => bot.name,
    );
  ...
  it("counts Bot Chat activity that last_session cannot see", () => {
    ...
    expect(
      activeBots(bots, "other", "open", NOW).map((bot) => bot.name),
    ).toContain("default");
  });
  ...
  it("counts a live kanban/tool worker heartbeat (#90268)", () => {
    ...
    const working = row({
      ...
      worker_session: {
        id: "w1",
        last_active: secondsAgo(30),
        source: "kanban",
      },
    });

    expect(
      activeBots([working], "other", "open", NOW).map((bot) => bot.name),
    ).toContain("coding");
...
describe("the roster activity filter", () => {
  ...
  it("splits recent from older on the week boundary", () => {
    ...
    expect(
      rosterActivityMatches({ activity: NOW - week + 1000 }, "recent", NOW),
    ).toBe(true);
    expect(
      rosterActivityMatches({ activity: NOW - week - 1000 }, "recent", NOW),
    ).toBe(false);
    expect(
      rosterActivityMatches({ activity: NOW - week - 1000 }, "older", NOW),
    ).toBe(true);
...
describe("which row owns the workspace highlight", () => {
  ...
  it("follows the explicit selection while no bot chat has focus", () => {
    expect(botRowOwnsWorkspace(bot, null, false, null, "local::ops")).toBe(
      true,
    );
    expect(botRowOwnsWorkspace(bot, null, false, null, "local::other")).toBe(
      false,
    );
  });
  ...
  it("follows the FOCUSED chat’s owner once a bot chat has focus", () => {
    ...
    expect(botRowOwnsWorkspace(bot, null, true, focused, "local::other")).toBe(
      true,
    );
    expect(
      botRowOwnsWorkspace(
  ...
  it("never highlights a bot row while a group room is active", () => {
    expect(botRowOwnsWorkspace(bot, "team", false, null, "local::ops")).toBe(
      false,
    );
  });
});
