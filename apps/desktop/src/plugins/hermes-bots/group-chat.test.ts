...
const durable = (room: Room) =>
  (room.gateway.storage.get("group-chats") || {}) as Record<string, GroupChat>;
...
function published(room: Room) {
  ...
  return (
    configure?.params.ui_meta as Record<string, Record<string, unknown>>
  )?.["hermes-bots-groups"];
}
...
describe("log window", () => {
  it("trimming keeps watermarks consistent with the trimmed array", async () => {
    ...
    const trimmed = chat.trimGroupChatLog(
      log,
      { builder: 10, research: 150 },
      96,
    );
...
describe("room naming", () => {
  it("same-name dedup reserves suffix length at the 64-char cap", async () => {
    ...
    const next = chat.uniqueGroupChatName(
      base,
      new Set([base, `${base.slice(0, 62)} 2`]),
    );
...
describe("speaker labels", () => {
  ...
  it("never borrow a remote row's display_name for a local speaker", async () => {
    ...
    data.$lastRoster.set([
      { display_name: "HomelabBot", name: "default", remoteSource: true },
    ]);
...
describe("threads", () => {
  it("hydration assigns legacy thread ids — a lull splits, follow-ups stay together", async () => {
    const { chat } = await loadRoom();
    const minute = 60000;
    const user = (text: string, at: number) =>
      ({ at, from: { kind: "user", name: "You" }, text }) as GroupMessage;
...
describe("gateway mirror", () => {
  it("mirrors room messages and members through bounded profile metadata", async () => {
    ...
    expect(
      (rooms[key!].members as { name: string }[]).map((member) => member.name),
    ).toEqual(["research", "builder"]);
    expect(room.chat.groupChatGatewayJsonSize(envelope)).toBeLessThanOrEqual(
      48000,
    );
  });
    ...
    const snapshot = chat.groupChatSyncSnapshot({
      Large: {
        log: Array.from({ length: 100 }, (_, index) => ({
          at: index,
          from: {
            kind: index % 2 ? "member" : "user",
            name: index % 2 ? "research" : "You",
          },
          text: `${index}:${long}`,
        })),
      },
  ...
  it("an explicit final-room disband may clear the mirror", async () => {
    ...
    room.chat.scheduleGroupChatServerSync(
      {},
      { allowEmpty: true, deletedRooms: ["Research"] },
    );
    ...
    expect(
      (envelope.deleted as Record<string, number>)["name:Research"],
    ).toBeGreaterThan(0);
  });
});
...
describe("snapshot merge", () => {
  it("pull-before-push preserves disjoint rooms, messages and members", async () => {
    ...
    const merged = chat.mergeGroupChatSyncSnapshots(
      {
        rooms: {
          RemoteOnly: {
            log: [
              { at: 2, from: { kind: "member", name: "ops" }, text: "kept" },
            ],
            members: [{ name: "ops" }],
          },
          Shared: {
    ...
    expect(merged.rooms["name:Shared"].log.map((entry) => entry.text)).toEqual([
      "remote",
      "local",
    ]);
    expect(
      merged.rooms["name:Shared"].members?.map((member) => member.name),
    ).toEqual(["research", "builder"]);
  ...
  it("lets a conflicting writer merge the winner and preserve both stable message ids", async () => {
    ...
    expect(
      loserRetry.rooms["name:Shared"].log.map((entry) => entry.id).sort(),
    ).toEqual(["writer-a:1", "writer-b:1"]);
...
describe("cold hydrate", () => {
  it("projects gateway rooms without dropping local runtime fields", async () => {
    ...
    const merged = chat.mergeRemoteGroupChatSnapshotIntoRooms(
      {
        rooms: {
          Shared: {
            ...
            members: [
              { connectionId: "mini", name: "research", sourceScoped: true },
            ],
          },
        },
        version: 3,
    ...
    expect(merged.Shared.log.map((entry) => entry.text)).toEqual([
      "remote question",
      "remote answer",
    ]);
  ...
  it("read-back does not clobber a newer local edit queued during an in-flight write", async () => {
    ...
    expect(merged.Shared.members?.map((member) => member.name)).toEqual([
      "new-member",
    ]);
    expect(merged.Shared.image).toBe("data:image/png;base64,new");
    expect(merged.Shared.syncRevision).toBe(4);
    expect(merged.Shared.log.map((entry) => entry.id).sort()).toEqual([
      "local",
      "remote",
    ]);
  });
...
describe("room identity", () => {
  ...
  it("a rename with a roomId is a same-key field update, never delete+create", async () => {
    ...
    const after = chat.mergeGroupChatSyncSnapshots(
      before,
      {
        rooms: { "id:room-42": { ...before.rooms["id:room-42"], name: "New" } },
        version: 3,
      },
    ...
    expect(
      chat.mergeGroupChatSyncSnapshots(before, after).rooms["id:room-42"].name,
    ).toBe("New");
  });
  ...
  it("renaming an id-keyed room via the rename job shape never tombstones it", async () => {
    ...
    const after = chat.mergeGroupChatSyncSnapshots(
      remote,
      {
        rooms: { "id:room-3": { ...remote.rooms["id:room-3"], name: "New" } },
        version: 3,
      },
...
describe("sync worker", () => {
  it("retries a gateway CAS conflict and publishes the merged room", async () => {
    ...
    await drain(
      () => (room.gateway.uiMetaRevisions["hermes-bots-groups"] || 0) < 2,
      60,
    );
    ...
    expect(
      stored.rooms["name:Shared"].log.map((entry) => entry.id).sort(),
    ).toEqual(["writer-a:1", "writer-b:1"]);
  ...
  it("fans a room write out to every reachable default-profile gateway", async () => {
    ...
    host.requestProfile = async (
      ...
      if (method === "profiles.list") {
        return {
          profiles: [{ name: "default", ui_meta: {}, ui_meta_revisions: {} }],
        };
      }
    ...
    room.chat.$groupChats.set({
      Shared: {
        log: [
          { at: 1, from: { kind: "user", name: "You" }, id: "w1", text: "hi" },
        ],
        members: [{ name: "research" }],
        sessions: {},
        syncRevision: 0,
    ...
    await drain(
      () =>
        remote.filter((entry) => entry.method === "profiles.configure").length <
        2,
      80,
    );
    ...
    const configured = new Set(
      remote
        .filter((entry) => entry.method === "profiles.configure")
