...
export function groupComposerDraftKey(
  group: string,
  room: GroupChatRoom,
): string {
...
export function updateGroupComposerDraft(
  key: string,
  ...
  const next = mutate({
    ...
    pendingAttachments: Object.fromEntries(
      Object.entries(current.pendingAttachments || {}).map(
...
export function restoreGroupComposerDraft(
  key: string,
  expectedRevision: number,
  snapshot: GroupComposerDraft,
) {
  const current = groupComposerDraftSnapshot(key);
  ...
  const restored = {
    ...snapshot,
    pendingAttachments: Object.fromEntries(
      Object.entries(snapshot.pendingAttachments || {}).map(
        ([thread, attachments]) => [thread, [...(attachments || [])]],
      ),
    ),
    replies: {
...
export function shouldRenderGroupChatInPane(
  group: null | string,
): group is string {
