...
function messagingProtocolSection(
  name: string,
...
export function backfillMessagingProtocol(
  roster: RosterRow[] | null | undefined,
) {
  ...
  for (const bot of roster || []) {
    ...
    if (
      !name ||
      soulProtocolChecked.has(name) ||
      soulProtocolInflight.has(name)
    ) {
      continue;
    }
...
export function composeSoul({
  name,
  title,
  description,
  roster,
  customSoul,
}: ComposeSoulOptions): string {
