...
vi.mock("./canonical-chat", () => ({
  createCanonicalChat: mocks.createCanonicalChat,
}));
...
async function renderDialog(hasSkillsView: boolean) {
  ...
  const view = render(
    withQueryClient(
      <CreateAgentDialog onClose={() => undefined} open roster={roster} />,
    ),
  );
...
describe("materializing the draft profile", () => {
  it("creates it once when the Capabilities tab opens, pinned to the new slug", async () => {
    ...
    await waitFor(() =>
      expect(mocks.skillsView.at(-1)).toMatchObject({
        fixedProfile: "inbox-triage",
      }),
    );
...
describe("the clone-from default", () => {
  it('regression: reset restores the mounted default, not "fresh profile"', async () => {
    ...
    fireEvent.click(
      await screen.findByRole("option", { name: /Fresh profile/ }),
    );
