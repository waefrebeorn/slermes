...
beforeEach(() => {
  ...
  openBotCanonicalChat
    .mockReset()
    .mockResolvedValue({ openedId: "stored-chat", registryId: "stored-chat" });
});
...
describe("opening a bot from a fronted room", () => {
  it("retires the group tab BEFORE the canonical open", async () => {
    ...
    expect(timeline.filter((event) => event.includes(":group:"))).toHaveLength(
      1,
    );
    expect(
      timeline.findIndex((event) => event.includes(":group:")),
    ).toBeLessThan(timeline.indexOf("canonicalOpen"));
...
describe("a failed open must not steal the center", () => {
  ...
  it("restores the room by its immutable roomId, so a rename mid-open still finds it", async () => {
    ...
    openBotCanonicalChat.mockImplementation(async () => {
      ...
      room.chat.$groupChats.set({ Renamed: rooms.Core } as unknown as Record<
        string,
        GroupChat
      >);
