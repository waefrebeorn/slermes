...
const seed = (key: unknown[], value: unknown) =>
  cache.set(JSON.stringify(key), { key, value });
...
describe("cachedUnionRoster", () => {
  ...
  it("falls back to another connection’s entry when the window has moved", async () => {
    ...
    seed(["hermes-bots", "roster", "vera"], {
      profiles: [{ connectionId: "vera", name: "default" }],
    });
    connection.id = "local";

    expect(cachedUnionRoster()?.profiles?.[0]).toMatchObject({
      connectionId: "vera",
    });
  });
  ...
  it("prefers the freshest snapshot among several cached connections", async () => {
    ...
    seed(["hermes-bots", "roster", "old"], {
      fetchedAt: 1_000,
      profiles: [{ name: "stale" }],
    });
    seed(["hermes-bots", "roster", "new"], {
      fetchedAt: 9_000,
      profiles: [{ name: "fresh" }],
    });
