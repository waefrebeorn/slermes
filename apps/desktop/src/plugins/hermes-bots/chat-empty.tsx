...
function botForStoredId(
  ...
  return (
    roster.find((bot) => {
      ...
      return (
        String(canonical?.id ?? "") === storedId ||
        String(canonical?.resolved_id ?? "") === storedId
      );
    }) ?? null
  );
}
...
function botForChat(
  ...
  return (
    botForStoredId(roster, sessionId) ??
    botForStoredId(roster, focusedStoredId())
  );
}
