...
function attentionReasonFromError(errorTextOrReason: unknown) {
  ...
  if (
    /rate.?limit|too many requests|\b429\b|\b5\d\d\b|server error|overloaded|timed?.?out|timeout|temporar/.test(
      text,
    )
  ) {
    return null;
  ...
  if (
    /no llm provider|no access token|not configured|no api key|missing api key/.test(
      text,
    )
  ) {
  ...
  if (
    /quota|out of funds|insufficient (credits?|funds|balance)|payment required|\b402\b|billing/.test(
      text,
    )
  ) {
...
export const $botAttention = atom<
  Record<string, { at: number; message: string; reason: string }>
>({});
...
export function commitBotMetaV2(
  storage: BotMetaStorage | undefined,
  snapshot: BotMetaSnapshot,
) {
  const commit = botMetaV2Commit.then(async () => {
    ...
    try {
      ...
      if (hasCommittedPrevious) {
        ...
        await Promise.allSettled(
          [BOT_META_MIGRATION_KEY, BOT_META_V2_KEY].map((key) =>
            storage.remove(key),
          ),
        );
      }
...
export async function saveBotMeta(
  owner: RosterRow | string,
  patch: StoredBotMeta,
): Promise<BotMetaSaveResult> {
  ...
  try {
    const persisted =
      route || botMetaV2Active
        ...
        : Promise.resolve(
            getPluginCtx()?.storage?.set?.(BOT_META_V1_KEY, next),
          );
...
export async function migrateBotMeta(
  storage: BotMetaStorage | undefined = getPluginCtx()?.storage,
) {
  ...
  const localAgents = (union?.agents || []).filter(
    (agent) => agent?.connectionKind === "local",
  );
  ...
  for (const [name, meta] of Object.entries(v1)) {
    const route =
      (routes || []).find(
        (candidate) =>
          candidate?.mode === "local" && candidate?.profile === name,
      ) ||
      (() => {
...
export function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const activeBot = {
        name:
          String(host.state.profile?.get?.() || "default").trim() || "default",
      };
      ...
      if (typeof host.agents === "function") {
        try {
          ...
          const previous: RosterRow[] = $lastRoster
            .get()
...
function mergeMultiSourceRoster(
  ...
  let activeId =
    liveId ||
    (liveProvided ? "" : String(union?.primaryConnectionId || "").trim());
  ...
  for (const profile of localProfiles) {
    ...
    if (
      profile?.sourceScoped &&
      activeId &&
      profile.connectionId !== activeId
    ) {
      continue;
    }
  ...
  if (Array.isArray(previous) && previous.length > 0) {
    const present = new Set(
      profiles.map((row) => `${row.connectionId || ""}::${row.name}`),
    );
    ...
    const omitted = new Set(
      (Array.isArray(union?.sources) ? union.sources : [])
        .filter(
          (source) =>
            source?.error === "connect-on-demand" ||
...
export function mentionNameForms(value: null | string | undefined): string[] {
  ...
  return [...new Set([slug, collapsed])].filter(
    (form) =>
      ...
      !["all", "everyone", "user", "default", "hermes"].includes(form),
  );
}
...
export function botFriendlyNames(
  bot: Partial<RosterRow> | null | undefined,
): Array<null | string | undefined> {
  const metaByName: Record<string, BotMeta | undefined> | null =
    typeof $botMeta !== "undefined" ? $botMeta.get() : null;
  const localTitle = !bot?.remoteSource
    ? metaByName?.[bot?.name!]?.title
    : null;
  const alias = aliasIdentityFor(bot);

  const aliasTitle = alias
    ? alias.metaKeys
        .map((key) => metaByName?.[key]?.title)
...
export function botSelectionKey(
  bot: Partial<RosterRow> | null | undefined,
): string | undefined;
...
export function persistBotMetaSnapshot(
  value: Record<string, BotMeta>,
  scoped = false,
): Promise<void> {
  try {
...
export function sourceByConnection(
  sources: GatewaySource[] | null | undefined,
): Map<string, GatewaySource> {
  return new Map(
...
export function annotateBotSource(
  bot: RosterRow,
  sources: GatewaySource[] | null | undefined,
): RosterRow {
...
export function botSourceStatus(
  bot: BotSourceFields | null | undefined,
): BotSourceStatus {
...
export function preferReachableSameNameRows(
  bots: RosterRow[] | null | undefined,
): RosterRow[] {
  ...
  return rows.filter(
    (bot) =>
      botSourceStatus(bot).available ||
      bot?.ghost ||
      !reachableNames.has(bot?.name),
  );
}
...
export function filterBots(
  ...
  return roster.filter((bot) => {
    ...
    const role =
      `${meta?.description || ""} ${bot.description || ""}`.toLowerCase();
    const preview = String(
      botActivitySession(bot)?.preview || "",
    ).toLowerCase();
...
export function botActivitySession(
  bot: null | RosterRow | undefined,
): CanonicalSession | SessionPreview | null {
  ...
  return (preferred.last_active || 0) >= (last.last_active || 0)
    ? preferred
    : last;
}
