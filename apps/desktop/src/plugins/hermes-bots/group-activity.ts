...
export interface GroupActivityEntry extends Omit<
  GroupActivityEvent,
  "group" | "member"
> {
  epoch: number;
...
export const $groupActivity = atom<
  Record<string, { events: GroupActivityEntry[] }>
>({});

export function recordGroupActivity(
  group: string,
  event: Omit<GroupActivityEntry, "at" | "epoch">,
) {
...
export function currentGroupActivity(group: string) {
  ...
  return (
    ($groupActivity.get()[group] || {}).events?.filter(
      (event) => (event.epoch || 0) === epoch,
    ) || []
  );
}
...
export function groupActivityLabel(event: GroupActivityEntry) {
  ...
  const who =
    event?.member === "You"
      ? "You"
      : groupSpeakerLabel(event?.member || "A bot");
