...
export function isSessionGoneError(
  error: GatewayErrorLike | null | undefined,
): boolean {
  ...
  return (
    message.includes("not in memory") || /session not found/i.test(message)
  );
}
...
export function syncGroupClarify(
  ...
  const clarify =
    state && typeof state.pending_clarify === "object"
  ...
  const approval = (
    state && typeof state.pending_approval === "object"
  ...
  $groupClarify.set({
    ...
    [key]: clarify
      ? {
          ...
          kind: "clarify",
          question:
            typeof clarify.question === "string" ? clarify.question : "",
          choices: Array.isArray(clarify.choices)
            ? clarify.choices.filter((c) => typeof c === "string" && c)
            : [],
          ...
          questions: Array.isArray(clarify.questions)
            ? clarify.questions
            : null,
        }
      : {
...
export async function runGroupChatMemberTurn(
  ...
  try {
    return await runGroupChatMemberTurnLeased(
      group,
      member,
      prompt,
      thread,
...
async function runGroupChatMemberTurnLeased(
  ...
  try {
    ...
    before = Array.isArray(pre?.messages)
      ? pre.messages.length
      : pre?.message_count || 0;
  ...
  const liveRuntime = await submitGroupTurnPrompt(
    member,
    runtime,
    stored,
    turnText,
  ...
  while (Date.now() < deadline) {
    ...
    quickRechecks = signalled
      ? GROUP_TURN_SETTLE_RECHECKS
      : Math.max(0, quickRechecks - 1);
    ...
    if (
      ...
      (roomDuringPoll.holds || {})[memberKey]
    ) {
      return null;
    }
...
export async function harvestStrandedGroupReply(
  ...
  const strandedThread =
    (typeof marker === "object" && marker?.thread) || "legacy";
