...
async function mergedRoster(
  ...
  if (union) {
    ...
    hostMock.agents.mockRejectedValue(
      new Error("no union roster on this build"),
    );
  }
...
const identities = (rows: RowFixture[]) =>
  rows.map((row) => `${row.connectionId}:${row.name}`);
...
describe("no union roster", () => {
  it("leaves the local list exactly as it was", async () => {
    const rows = await mergedRoster(
      {
        profiles: [
          { last_session: { id: "s1", last_active: 1 }, name: "default" },
        ],
      },
      null,
    );
...
describe("the active source annotates; other sources append", () => {
  it("keeps rich fields on the annotated row and tags the rest by source", async () => {
    const rows = await mergedRoster(
      {
        profiles: [
          { last_session: { id: "s1", last_active: 1 }, name: "research" },
        ],
      },
      {
        agents: [
    ...
    const annotated = rows.find(
      (row) => row.name === "research" && !row.remoteSource,
    )!;
    ...
    expect(
      rows.find((row) => row.name === "research" && row.remoteSource),
    ).toMatchObject({
      connectionId: "homelab",
      connectionLabel: "Homelab",
      handle: "research-homelab",
    });
    expect(rows.find((row) => row.name === "coder")).toMatchObject({
      handle: "coder",
      remoteSource: true,
    });
  });
    ...
    const rows = await mergedRoster(
      {
        profiles: [
          {
            last_session: { id: "remote-session", last_active: 1 },
            name: "default",
          },
        ],
      },
    ...
    const rows = await mergedRoster(
      {
        profiles: [
          {
            last_session: { id: "s-default", last_active: 1 },
            name: "default",
          },
  ...
  it("lets the live id beat primaryConnectionId", async () => {
    ...
    expect(
      rows.filter((row) => row.remoteSource).map((row) => row.connectionId),
    ).toEqual(["local"]);
  });
...
describe("a null live id", () => {
  it("does not treat the registry primary as active on a local window", async () => {
    ...
    const rows = await mergedRoster(
      {
        profiles: [
          {
            last_session: { id: "this-chat", last_active: 1 },
            name: "default",
          },
        ],
      },
    ...
    expect(rows.find((row) => row.name === "default")?.last_session?.id).toBe(
      "this-chat",
    );
    expect(
      rows.filter((row) => row.remoteSource && row.connectionId === "local"),
    ).toHaveLength(0);
    expect(
      rows
  ...
  it("infers a matching remote primary when the local inventory does not match", async () => {
    ...
    const rows = await mergedRoster(
      {
        profiles: [
          {
            last_session: { id: "noah-chat", last_active: 1 },
            name: "default",
          },
        ],
      },
    ...
    expect(rows.find((row) => row.name === "archie")).toMatchObject({
      connectionId: "local",
      remoteSource: true,
    });
  });
...
describe("connect-on-demand sources", () => {
  ...
  it("carries a previously painted remote row through an empty union", async () => {
    ...
    const rows = await mergedRoster(
      ...
      localOnlyUnion([
        { connectionId: "spark", error: "connect-on-demand", kind: "ssh" },
      ]),
    );

    expect(
      rows.find((row) => row.name === "bob" && row.connectionId === "spark"),
    ).toMatchObject({ remoteSource: true });
    expect(rows.filter((row) => row.name === "default")).toHaveLength(1);
  });
