 ...
 *  identity preference. */
export const $openBotChat = atom<{
  key: string;
  openedRegistryId: string;
...
export function saveSelectedRosterBot(bot: RosterRow) {
  ...
  try {
    Promise.resolve(
      getPluginCtx()?.storage?.set?.("selected-roster-bot-v1", key),
    ).catch(() => undefined);
  } catch {
...
export function clearSelectedRosterKey(key: string) {
  ...
  try {
    Promise.resolve(
      getPluginCtx()?.storage?.set?.("selected-roster-bot-v1", ""),
    ).catch(() => undefined);
  } catch {
...
function fallbackFocusedBotOwner(profile: string = $focusedBotProfile.get?.()) {
  ...
  const connectionId = String(
    host.state.connectionId?.get?.() ||
      (typeof host.activeConnectionId === "function"
...
export const $focusedBotOwner = host.state.focusedSessionOwner || {
  ...
  listen: (
    listener: (value: ReturnType<typeof fallbackFocusedBotOwner>) => void,
  ) => {
    const emit = (profile: string) =>
      listener(fallbackFocusedBotOwner(profile));
    const unbindProfile = $focusedBotProfile.listen(emit);
    const unbindConnection = host.state.connectionId?.listen?.(() =>
      emit($focusedBotProfile.get?.()),
    );
