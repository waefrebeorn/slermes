...
const en: BotsMessages = {
  roster: {
    ...
    noMatchQueryOn: (query, gateway) =>
      `No bots or group chats match “${query}” on ${gateway}`,
    noMatchFiltersOn: (gateway) =>
      `No bots or group chats match these filters on ${gateway}`,
  ...
  bot: {
    ...
    descriptionHint:
      "Leave blank to generate from the bot’s name and description.",
  ...
  avatar: {
    ...
    savedLocallyDescriptionFailed:
      "Saved look locally; description update failed",
    generate: "Generate",
  ...
  group: {
    ...
    manageDesc:
      "A bot can join multiple group chats. Memberships sync to every machine.",
    manageTitle: "Manage groups",
    settingsTitle: "Group settings",
    settingsDesc:
      "Rename the group or set a room picture. Members and history are kept.",
    ...
    stop: "Stop",
    stopHint:
      "Stop this run — interrupts the member on turn and holds the rest",
    allHeldStatus: (count) => `All ${count} bots are paused`,
    heldMembersStatus: (members) => `Paused: ${members}`,
    holdReleaseHint:
      "Mention a paused bot or send @all resume to release them.",
    ...
    settingsHint: (group) =>
      `Group settings — rename ${group} or set a room picture`,
  ...
  cron: {
    ...
    staleNotice:
      "Could not refresh scheduled jobs. Showing the last list we had.",
    readFailure:
      "The list may still be there — this was a read failure, not a delete.",
    createDesc: (bot) =>
      `A recurring task ${bot} runs on a schedule. Runs land in its own chat history.`,
...
const ja: BotsMessages = {
  roster: {
    ...
    noMatchQuery: (query) =>
      `「${query}」に一致するボットやグループチャットはありません`,
    noMatchQueryOn: (query, gateway) =>
      `${gateway} に「${query}」に一致するボットやグループチャットはありません`,
    noMatchFiltersOn: (gateway) =>
      `${gateway} にこれらのフィルタに一致するボットやグループチャットはありません`,
    noMatchFilters:
      "これらのフィルタに一致するボットやグループチャットはありません。",
    ...
    waitingForGateway:
      "ゲートウェイ接続を待っています…（リモートは数秒かかることがあります。自動で再試行します）",
  },
  sections: {
    newSection: "新しいセクション",
    newTitle: "新しいセクション",
  ...
  bot: {
    ...
    openAnotherChatUnsupported:
      "別のボットチャットを開くには Hermes Desktop を更新してください。",
    remoteConnectionsUnsupported:
      "他の接続上のボットとチャットするには Hermes Desktop を更新してください。",
  ...
  avatar: {
    ...
    describeHint:
      "空欄のままにすると、名前・タイトル・説明と agent-messaging の名簿から自動生成します。",
    matchTheName: "名前に合わせる",
    pickPet: "このボットのプロフィール画像としてペットを選びます。",
    petLoadFailed:
      "そのペットを読み込めませんでした。別のペットを試してください。",
    imageTooLarge: "画像が大きすぎます（最大 15MB）。",
    generationFailed: "アバターの生成に失敗しました",
    savedLocally:
      "見た目はローカルに保存されましたが、リモートへの保存に失敗しました",
    savedLocallyDescriptionFailed:
      "見た目はローカルに保存されましたが、説明の更新に失敗しました",
    generate: "生成",
    generating: "生成中…",
  },
  group: {
    ...
    manageDesc:
      "ボットは複数のグループチャットに参加できます。メンバーシップはすべてのマシンに同期されます。",
    manageTitle: "グループを管理",
    settingsTitle: "グループ設定",
    settingsDesc:
      "グループ名の変更や部屋の画像の設定ができます。メンバーと履歴は保持されます。",
    nameLabel: "グループ名",
    ...
    deleteAction: "削除",
    composerPlaceholder:
      "何か書いてください — このグループのすべてのボットが部屋の内容を受け取ります。",
    ...
    holdReleaseHint:
      "一時停止中のボットにメンションするか、@all resume を送信して再開します。",
    ...
    stopped: (group) =>
      `${group}を停止しました — 残りのターンは再開するまで保留されます`,
    ...
    newThreadPlaceholder: (group) =>
      `${group}で新しいスレッド…（@名前で個別、@everyoneで全員）`,
    everyoneMeta: "ルーム内のすべてのボット",
    commandApproval: "コマンドの承認",
    answerFailed: (handle, error) =>
      `@${handle}に回答を送信できませんでした: ${error}`,
    wantsToRunCommand: (handle) =>
      `@${handle}がコマンドを実行しようとしています:`,
  ...
  cron: {
    ...
    staleNotice:
      "定期実行ジョブを更新できませんでした。最後に取得したリストを表示しています。",
    readFailure:
      "リストはまだ存在している可能性があります — これは読み取りの失敗で、削除ではありません。",
    createDesc: (bot) =>
      `${bot}がスケジュールに沿って実行する定期タスクです。実行結果は専用のチャット履歴に残ります。`,
    instruction: "指示",
    whenToRun: "実行するタイミング",
    dayOfMonth: "日付",
    ...
    continuity:
      "継続: 各実行が前回の出力を参照します（重複を避け、続きから実行）",
...
const zh: BotsMessages = {
  roster: {
    ...
    noMatchQueryOn: (query, gateway) =>
      `${gateway} 上没有机器人或群聊匹配“${query}”`,
    noMatchFiltersOn: (gateway) =>
      `${gateway} 上没有机器人或群聊匹配这些筛选条件`,
    ...
    retryNow: "立即重试",
    rosterUnavailable: (reason) =>
      `无法获取名单：${reason}。如果网关早于 profiles.list，请更新 Hermes 并重启网关。`,
  ...
  bot: {
    ...
    openAnotherChatUnsupported:
      "请更新 Hermes Desktop 以打开另一个机器人聊天。",
    remoteConnectionsUnsupported:
      "请更新 Hermes Desktop 以与其他连接上的机器人聊天。",
    chatEmpty: "说点什么开始吧。",
    kickoff: "你好，介绍一下你自己吧！",
  },
  avatar: {
  ...
  group: {
    ...
    newThreadPlaceholder: (group) =>
      `在 ${group} 中开启新讨论串…（@名称指定，@everyone 全体）`,
  ...
  cron: {
    ...
    createDesc: (bot) =>
      `由 ${bot} 按计划运行的重复任务。运行结果会保存在它自己的聊天记录中。`,
    instruction: "指令",
    whenToRun: "运行时间",
    dayOfMonth: "每月日期",
    sendResultsTo: "结果发送到",
...
const zhHant: BotsMessages = {
  roster: {
    ...
    noMatchQueryOn: (query, gateway) =>
      `${gateway} 上沒有機器人或群組聊天符合「${query}」`,
    noMatchFiltersOn: (gateway) =>
      `${gateway} 上沒有機器人或群組聊天符合這些篩選條件`,
    ...
    retryNow: "立即重試",
    rosterUnavailable: (reason) =>
      `無法取得名單：${reason}。如果閘道早於 profiles.list，請更新 Hermes 並重新啟動閘道。`,
  ...
  bot: {
    ...
    openAnotherChatUnsupported:
      "請更新 Hermes Desktop 以開啟另一個機器人聊天。",
    remoteConnectionsUnsupported:
      "請更新 Hermes Desktop 以與其他連線上的機器人聊天。",
    chatEmpty: "說點什麼開始吧。",
    kickoff: "你好，介紹一下你自己吧！",
  },
  avatar: {
  ...
  group: {
    ...
    newThreadPlaceholder: (group) =>
      `在 ${group} 中開啟新討論串…（@名稱指定，@everyone 全體）`,
  ...
  cron: {
    ...
    createDesc: (bot) =>
      `由 ${bot} 按排程執行的重複工作。執行結果會保存在它自己的聊天紀錄中。`,
    instruction: "指示",
    whenToRun: "執行時間",
    dayOfMonth: "每月日期",
    sendResultsTo: "結果傳送到",
