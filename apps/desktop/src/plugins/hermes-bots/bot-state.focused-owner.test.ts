    ...
    host: {
      ...
      state: {
        ...
        get focusedSessionOwner() {
          return state.focusedOwner
            ? store(() => state.focusedOwner)
            : undefined;
        },
        get focusedSessionProfile() {
          return state.focusedProfile === null
            ? undefined
            : store(() => state.focusedProfile);
        },
...
async function load() {
  ...
  const [botState, data] = await Promise.all([
    import("./bot-state"),
    import("./data"),
  ]);
...
describe("a desktop that publishes the complete focused owner", () => {
  it("uses it verbatim and treats it as authoritative", async () => {
    ...
    const { $focusedBotOwner, focusedRosterOwner, isActiveRosterBot } =
      await load();
    ...
    expect(
      isActiveRosterBot(
        { connectionId: "source-b", name: "worker", remoteSource: true },
        owner,
      ),
    ).toBe(true);
    ...
    expect(
      isActiveRosterBot(
        { connectionId: "source-a", name: "worker", remoteSource: true },
        owner,
      ),
    ).toBe(false);
  });
...
describe("a desktop with neither focus atom", () => {
  it("falls back to the socket home profile", async () => {
    ...
    const { $focusedBotOwner, focusedMentionProfile, focusedRosterOwner } =
      await load();
...
describe("focusedRosterOwner", () => {
  it("reports nothing for an owner with no profile", async () => {
    ...
    expect(
      focusedRosterOwner({ connectionId: "source-a", profile: "  " }),
    ).toBeNull();
  });
});
