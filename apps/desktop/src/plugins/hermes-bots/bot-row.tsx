...
function rowAge(ms: number, r: SidebarRowLabels): string {
  ...
  return unit === "second"
    ? r.ageNow
    : `${value}${unit === "day" ? r.ageDay : unit === "hour" ? r.ageHour : r.ageMin}`;
}
...
export function BotRow({
  ...
  const rowAgeTs = workerActive
    ? Math.max(
        ...
        bot.worker_session?.last_active || 0,
      )
    : activitySession?.last_active || 0;

  const botMood =
    workerActive || (isGatewayHome && gatewayState === "busy")
      ? "work"
      : "idle";
  ...
  const attention =
    ...
    attentionByKey[
      `${bot?.connectionId || activeConnectionId}::${bot?.name || "default"}`
    ] ||
    null;
  ...
  const gatewayLabel =
    bot.connectionLabel || (bot.connectionId === "local" ? "This device" : "");
      ...
      <div className="min-w-0 flex-1">
        ...
        {showDetailsRow ? (
          <div className="flex min-w-0 items-center gap-1.5 text-xs text-(--ui-text-tertiary)">
            ...
            {displayPreview ? (
              <span className={cn("min-w-0 truncate", fromBot && "italic")}>
                {displayPreview}
              </span>
            ) : null}
          </div>
      ...
      <ContextMenuContent>
        ...
        <ContextMenuItem
          onSelect={() => {
            void ensureBotMetadata(bot)
              ...
              .catch((error) =>
                host.notifyError?.(error, "Could not load bot metadata"),
              );
          }}
        >
        ...
        <ContextMenuItem
          onSelect={() =>
            void ensureBotMetadata(bot)
              .then(() => onGroup(bot))
              .catch((error) =>
                host.notifyError?.(error, "Could not load bot groups"),
              )
          }
        >
        ...
        <ContextMenuSub>
          ...
          <ContextMenuSubContent>
            ...
            {currentSectionId ? (
              <ContextMenuItem
                onSelect={() => void moveBotsToSection([bot], null)}
              >
  ...
  const row = (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="min-w-0 truncate text-xs text-(--ui-text-tertiary)">
          {preview}
        </div>
      </div>
    </RowButton>
