...
const log = (room: Room, group: string) =>
  room.chat.$groupChats.get()[group]?.log || [];
...
describe("routing", () => {
  ...
  it("answers only @-mentioned members; @everyone or no mention means all", async () => {
    ...
    expect(
      rounds
        .resolveGroupResponders(user("@builder take this one"), MEMBERS)
        .map((m) => m.name),
    ).toEqual(["builder"]);
    expect(
      rounds.resolveGroupResponders(user("hello team"), MEMBERS),
    ).toHaveLength(3);
    expect(
      rounds.resolveGroupResponders(user("@everyone standup"), MEMBERS),
    ).toHaveLength(3);
  });
  ...
  it("resolves display titles to the member and never matches @user against a bot", async () => {
    ...
    const parsed = rounds.parseGroupChatMentions(
      "@theops please check, then ping @user",
      MEMBERS,
    );
  ...
  it("rotates the lead speaker each round", async () => {
    ...
    expect(rounds.rotateGroupSpeakers(MEMBERS, 1).map((m) => m.name)).toEqual([
      "builder",
      "ops",
      "research",
    ]);
  ...
  it("pulls a member mentioned by another bot into the NEXT round", async () => {
    ...
    const lines = log(room, "Core").map(
      (entry) => `${entry.from.name}: ${entry.text}`,
    );
...
describe("round lifecycle", () => {
  ...
  it("stops chatty members at GROUP_CHAT_MAX_MESSAGES", async () => {
    const room = await loadRoom({
      turn: ({ n }) => `message ${n} — @everyone keep going`,
    });

    room.rounds.sendToGroupChat("Loud", MEMBERS, "go wild");
    await settle(room, "Loud");

    const posted = log(room, "Loud").filter(
      (entry) => entry.from.kind === "member",
    );

    expect(posted.length).toBeLessThanOrEqual(
      room.chat.GROUP_CHAT_MAX_MESSAGES,
    );
  });
  ...
  it("badges needs-you when a member addresses @user, and clears it on the next user send", async () => {
    const room = await loadRoom({
      turn: ({ profile }) =>
        profile === "research"
          ? "Blocked on billing access — @user which account?"
          : "(pass)",
    });
  ...
  it('converts an "(empty)" member reply like the gateway does, never appending it raw', async () => {
    const room = await loadRoom({
      turn: ({ profile }) => (profile === "research" ? "(empty)" : "(pass)"),
    });
    ...
    const replies = log(room, "Sentinel").filter(
      (entry) => entry.from.kind === "member",
    );
  ...
  it("leaves normal replies untouched", async () => {
    const room = await loadRoom({
      turn: ({ profile }) =>
        profile === "research" ? "I am not empty." : "(pass)",
    });
    ...
    const replies = log(room, "Sentinel2").filter(
      (entry) => entry.from.kind === "member",
    );
...
describe("per-member delta", () => {
  it("feeds a second send only the NEW messages", async () => {
    ...
    const second = room.gateway.calls
      .slice(firstCount)
      .find((call) => call.prompt.includes("second message"));
  ...
  it("keeps concurrent rooms sharing one member isolated in sessions, deltas and context", async () => {
    ...
    const quiet = () =>
      !room.chat.$groupChats.get().Alpha?.running &&
      !room.chat.$groupChats.get().Beta?.running;
    ...
    const alphaFirst = room.gateway.calls.find(
      (call) => call.title === "Group: Alpha",
    );
    const betaFirst = room.gateway.calls.find(
      (call) => call.title === "Group: Beta",
    );
    ...
    expect(room.chat.$groupChats.get().Alpha.sessions?.research).toBe(
      alphaFirst?.stored,
    );
    expect(room.chat.$groupChats.get().Beta.sessions?.research).toBe(
      betaFirst?.stored,
    );
    ...
    expect(
      alphaSession?.messages.some((message) =>
        message.content.includes("BETA_ONLY"),
      ),
    ).toBe(false);
    expect(
      betaSession?.messages.some((message) =>
        message.content.includes("ALPHA_ONLY"),
      ),
    ).toBe(false);
  });
});
...
describe("threads", () => {
  ...
  it("continues an explicit thread and scopes the member delta to it", async () => {
    const room = await loadRoom({
      turn: ({ prompt }) =>
        prompt.includes("billing") ? "On the billing fix." : "(pass)",
    });

    const member: GroupMember[] = [{ name: "research", title: "" }];

    const billing = room.rounds.sendToGroupChat(
      "Scoped",
      member,
      "fix the billing bug",
    );
    ...
    await settle(room, "Scoped");
    const again = room.rounds.sendToGroupChat(
      "Scoped",
      member,
      "billing follow-up: ship it",
      billing,
    );
    await settle(room, "Scoped");

    const followUp = room.gateway.calls.find((call) =>
      call.prompt.includes("ship it"),
    );
    const replies = log(room, "Scoped").filter(
      (entry) => entry.from.kind === "member",
    );
...
describe("attachments", () => {
  it("stages them into EVERY responding member session before that member submits", async () => {
    ...
    expect(room.gateway.attaches.map((entry) => entry.profile).sort()).toEqual([
      "builder",
      "research",
    ]);
  ...
  it("scopes attachments by mention: only the mentioned member receives the image", async () => {
    ...
    expect(room.gateway.attaches.map((entry) => entry.profile)).toEqual([
      "builder",
    ]);
  });
  ...
  it("routes PDFs through pdf.attach and files through file.attach, per member", async () => {
    ...
    expect(byMethod).toEqual({
      "file.attach": 2,
      "image.attach_bytes": 2,
      "pdf.attach": 2,
    });

    const staged = room.gateway.attaches.find(
      (attach) => attach.method === "pdf.attach",
    );
  ...
  it("names attachments in the transcript line, labelling PDFs and files distinctly", async () => {
    ...
    const line = (entry: Partial<GroupMessage>) =>
      rounds.formatGroupChatLine(entry as GroupMessage, "research");
    ...
    ).toBe("You (user): see attached [attached image: screenshot.png]");
    expect(line({ from: { kind: "user", name: "You" }, text: "plain" })).toBe(
      "You (user): plain",
    );
    expect(
      line({
...
describe("member holds (#93129)", () => {
  ...
  it("holds nobody when a stop word carries no mention", async () => {
    ...
    expect([
      ...rounds.classifyGroupHoldDirective("stop", [], false).hold,
    ]).toEqual([]);
  });
  ...
  it('still holds on "don\'t stop @x" — the documented conservative trade-off', async () => {
    ...
    expect([
      ...rounds.classifyGroupHoldDirective("don't stop @impl", ["impl"], false)
        .hold,
    ]).toEqual(["impl"]);
  });
  ...
  it('does not trigger on "stop" inside another word', async () => {
    ...
    const action = rounds.classifyGroupHoldDirective(
      "@impl unstoppable work ahead",
      ["impl"],
      false,
    );
  ...
  it("sets a hold on stop and clears it on resume for the same member", async () => {
    ...
    const held = rounds.applyGroupHoldDirective(
      {},
...
describe("stopGroupThread (#91868/#94569)", () => {
  ...
  it("records a stopped activity event visible in the CURRENT run", async () => {
    ...
    expect(room.activity.groupActivityLabel(stopped!)).not.toMatch(
      /[\u4e00-\u9fff]/,
    );
  ...
  it("falls back to the durable room roster when called without members", async () => {
    ...
    expect(
      Object.keys(room.chat.$groupChats.get().Room.holds || {}),
    ).toHaveLength(STOP_MEMBERS.length);
  ...
  it("abandons an in-flight turn once a stop bumps the epoch and holds the member", async () => {
    ...
    const room = await loadRoom({
      onResumePoll: (polls) => {
        ...
        if (polls === 2) {
          void live?.rounds.stopGroupThread("Room", "t1", [
            { name: "helper", title: "" },
          ]);
        }
      },
