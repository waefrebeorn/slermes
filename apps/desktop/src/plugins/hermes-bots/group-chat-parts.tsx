...
export function GroupImageControls({
  image,
  onImage,
  seedName,
  seedMembers,
}: GroupImageControlsProps) {
  const b = useBots();
  const { t } = useI18n();
  ...
  const generate = async () => {
    ...
    try {
      const who = [
        seedName,
        seedMembers?.length ? `a team of ${seedMembers.join(", ")}` : "",
      ]
        .filter(Boolean)
        .join(" — ");
...
export function GroupClarifyCard({ entry, members }: GroupClarifyCardProps) {
  ...
  const member =
    members.find((m) => groupMemberKey(m) === entry.memberKey) ||
    members.find((m) => m.name === entry.member);
    ...
    entry.questions && entry.questions.length
      ? entry.questions.map((q, i) => ({
          ...
          choices: Array.isArray(q?.choices)
            ? q.choices.filter((c) => typeof c === "string" && c)
            : [],
  ...
  const submit = async () => {
    ...
    try {
      ...
      host.notify({
        kind: "error",
        message: b.group.answerFailed(
          botHandle(entry.member, member),
    ...
    <div className="grid gap-1.5 rounded-md border border-(--ui-accent)/50 bg-(--ui-accent)/5 px-2.5 py-2">
      ...
      {questions.map((q) => (
        <div className="grid gap-1" key={`q:${q.qid}`}>
          {q.question ? (
            <div className="text-xs whitespace-pre-wrap">{q.question}</div>
          ) : null}
          {q.choices.length ? (
