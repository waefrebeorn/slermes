...
async function loadModules() {
  ...
  const ops: StorageOp[] = [];
  const [data, profileOps, shared] = await Promise.all([
    import("./data"),
    import("./profile-ops"),
    import("./shared"),
  ]);
...
beforeEach(() => {
  ...
  stateAtoms.focusedSessionOwner.set({
    connectionId: "local",
    profile: "default",
  });
...
describe("the SDK delete verb is preferred over the CLI", () => {
  it("deletes through host.deleteProfile and never shells out", async () => {
    ...
    expect(
      request.mock.calls.filter(([method]) => method === "cli.exec"),
    ).toHaveLength(0);
  });
  ...
  it("falls back to the non-interactive profile CLI on an older desktop", async () => {
    ...
    expect(exec[0][1]).toEqual({
      argv: ["profile", "delete", "researcher", "--yes"],
    });
  });
});
...
describe("deletes that must not happen", () => {
  it("rejects an alias whose backend target is the default profile", async () => {
    ...
    expect(request.mock.calls.some(([method]) => method === "cli.exec")).toBe(
      false,
    );
  });
...
describe("plugin-local state is cleaned up behind the delete", () => {
  ...
  it("drops meta, unread and selection, then refreshes the roster", async () => {
    ...
    expect(forgetSessionUnread).toHaveBeenCalledWith(
      ["reg-1", "tip-9"],
      "researcher",
    );
    ...
    expect(
      (ops.at(-1)?.value as Record<string, unknown>).researcher,
    ).toBeUndefined();
