...
async function mount(initial = "") {
  ...
  return {
    input: screen.getByLabelText("Message Core") as HTMLTextAreaElement,
    onChange,
    onSubmitDraft,
  };
}
...
describe("the @-token at the caret", () => {
  ...
  it("narrows to @everyone as the query grows", async () => {
    ...
    expect(
      options().filter((label) => label.startsWith("@everyone")),
    ).toHaveLength(1);
...
describe("insertion", () => {
  it('writes exactly "@handle " — the shape parseGroupChatMentions resolves', async () => {
    ...
    const option = screen
      .getAllByRole("button")
      .find((button) => button.textContent?.startsWith("@alpha"));
  ...
  it("replaces the whole token, not just the typed suffix", async () => {
    ...
    const option = screen
      .getAllByRole("button")
      .find((button) => button.textContent?.startsWith("@builder"));
    fireEvent.mouseDown(option!);
  ...
  it("lists every seated member under a bare @, keyed by handle", async () => {
    ...
    expect(options().some((label) => label.startsWith("@undefined"))).toBe(
      false,
    );
  });
});
