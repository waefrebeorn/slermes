...
function hideOwnedBotSessions() {
  ...
  const roomEntries = Object.values($groupChats.get()).flatMap((room) =>
    Object.entries(room?.sessions || {})
      .map(([key, id]) => {
        ...
        const derived = (room?.members || []).find(
          (member: GroupMember) => groupMemberKey(member) === key,
        );
  ...
  const rooms = [
    ...new Map(roomEntries.map((entry) => [entry.dedupe, entry])).values(),
  ];
...
function hidePersistedBotSession(
  bot: RosterRow,
  sessionId: string,
  profileOverride = "",
) {
...
function isBotModeSweepCandidate(
  row: SweepSessionRow | null | undefined,
...
async function sweepBotProfileSessions(nowSeconds = Date.now() / 1000) {
  if (
    typeof host.listPersistedSessions !== "function" ||
    typeof host.setPersistedSessionHidden !== "function"
  ) {
    return;
  }

  const cached = $lastRoster.get();
  let roster: RosterRow[] | null =
    Array.isArray(cached) && cached.length ? cached : null;
