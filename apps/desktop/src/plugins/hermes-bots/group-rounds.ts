...
export function parseGroupChatMentions(text: unknown, members: GroupMember[]) {
  ...
  for (const member of members) {
    ...
    const handle = String(
      member.handle || botHandle(member.name, member) || "",
    ).trim();
    ...
    const forms = new Set([
      ...
      ...(handle
        ? [handle.toLowerCase(), handle.toLowerCase().replace(/[\s_-]+/g, "")]
        : []),
      ...(title
        ? [
            title.toLowerCase(),
  ...
  for (const match of source.matchAll(/@([a-z0-9][a-z0-9._-]*)/gi)) {
    ...
    const resolved =
      handles.get(handle) || handles.get(handle.replace(/[._-]+/g, ""));
...
export function buildGroupChatTurnPrompt({
  ...
  const peerNames = peers
    .map((m) => {
      const handle = m.title
        ? `${m.title} (@${botHandle(m.name, m)})`
        : `@${botHandle(m.name, m)}`;

      return m.remoteSource
        ? `${handle} [on ${m.connectionLabel || m.connectionId}]`
        : handle;
    })
    .join(", ");
    ...
    for (const key of parsed.mentioned) {
      const citingMemberKey = (() => {
        const m = members.find(
          (mm: GroupMember) => mm.name === entry.from?.name,
        );
...
export async function stopGroupThread(
  ...
  const room = $groupChats.get()[group] || {};
  const roster =
    Array.isArray(members) && members.length ? members : room.members || [];
  ...
  const onTurn = turnName
    ? roster.find((member: GroupMember) => member?.name === turnName)
    : null;
  const sessionId = onTurn
    ? (room.sessions || {})[groupMemberKey(onTurn)]
    : null;
...
export async function runGroupChatRounds(
  ...
  const isCurrent = () =>
    (($groupChats.get()[group] || {}).epoch || 0) === startEpoch;
  let posted = 0;
  let continuations = 0;
  ...
  try {
    for (let round = 0; round < GROUP_CHAT_MAX_ROUNDS; round++) {
      ...
      for (const member of responders) {
        ...
        const anchorId = room.log.length
          ? room.log[room.log.length - 1].id
          : null;
        const anchorIdx =
          anchorId === null
            ? -1
            : roomNow.log.findIndex((e: GroupMessage) => e.id === anchorId);
        // Anchor trimmed away ⇒ every pre-turn entry was dropped, so every
        // surviving entry is newer — scanning the whole log stays exact.
        const turnTail =
          anchorIdx >= 0 ? roomNow.log.slice(anchorIdx + 1) : roomNow.log;

        const newerUserEntryInThread = turnTail.some(
          (e: GroupMessage) =>
            e.from?.kind === "user" && groupThreadOf(e) === thread,
        );
      ...
      if (spokeThisRound === 0) {
        ...
        if (
          ...
          const citedMembers = members.filter((member: GroupMember) =>
          ...
          if (citedMembers.length && posted < GROUP_CHAT_MAX_MESSAGES) {
            ...
            for (const member of continuationResponders) {
              ...
              if (
                continuationReply !== null &&
                !isGroupPassText(continuationReply)
              ) {
                appendGroupChatEntry(
                  group,
                  {
    ...
    if (isCurrent()) {
      ...
      const strandedLeft = Object.keys(
        ($groupChats.get()[group] || {}).stranded || {},
      );
...
async function harvestStrandedUntilSettled(
  ...
  for (let attempt = 0; attempt < HARVEST_MAX_TRIES; attempt++) {
    await new Promise((resolve) =>
      window.setTimeout(resolve, HARVEST_INTERVAL_MS),
    );
    const room = $groupChats.get()[group];
    ...
    for (const member of members) {
      if (
        Object.prototype.hasOwnProperty.call(stranded, groupMemberKey(member))
      ) {
        try {
          await harvestStrandedGroupReply(group, member);
        } catch {
...
export function sendToGroupChat(
  ...
  const attached = Array.isArray(images)
    ? images.filter((img: Attachment) => img && img.data)
    : [];
