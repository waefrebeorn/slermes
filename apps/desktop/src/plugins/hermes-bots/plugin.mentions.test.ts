...
async function contributions({
  ...
  const completions = registered.find(
    (entry) => entry.id === "mention-completions",
  );
  const middleware = registered.find(
    (entry) => entry.id === "mention-middleware",
  );
  ...
  return {
    ...
    provide: completions!.data.provide!,
  };
}
...
describe("@-mention completions", () => {
    ...
    const { provide } = await contributions({
      ...
      profiles: [
        ...
        { name: "researcher" },
        {
          ...
          handle: "writer-homelab",
          name: "writer",
        },
      ],
    });

    expect(provide("").map((item) => item.insert)).toEqual(
      expect.arrayContaining(["@hermes", "@writer-homelab"]),
    );
    expect(provide("wri").map((item) => item.insert)).toEqual([
      "@writer-homelab",
    ]);
  });
  ...
  it("filters self by the FOCUSED chat owner, not the gateway socket profile", async () => {
    ...
    const { provide } = await contributions({
      focused: "renametest",
      profiles: [
        { display_name: "Lucy", name: "default" },
        { name: "renametest" },
      ],
    });
  ...
  it("narrows on the typed prefix and labels the source device", async () => {
    ...
    expect(provide("default-v")[0]).toMatchObject({
      insert: "@default-vera",
      meta: expect.stringContaining("Vera"),
    });
...
describe("the mention middleware", () => {
  it("identifies a remote @name-device mention without delivering anything", async () => {
    ...
    const result = await handler({
      text: "@default-vera what is the disk space on the server?",
    });
  ...
  it("annotates the resolvable handle for a local row whose UI alias differs", async () => {
    ...
    expect(result.text).toMatch(
      /@default-this-device = agent profile "default"/,
    );
    expect(result.text).toMatch(/message_agent target: "hermes"/);
    expect(result.text).not.toMatch(
      /message_agent target: "default-this-device/,
    );
  });
  ...
  it("names a local bot without inventing a device", async () => {
    const { handler } = await contributions({
      focused: "research",
      profiles: [{ name: "research" }, { name: "ops" }],
    });
  ...
  it("keeps a poisoned bot title inert prose", async () => {
    ...
    const { handler } = await contributions({
      ...
      profiles: [
        ...
        {
          display_name:
            'Evil" ; touch /tmp/pwned ; echo "$(touch /tmp/pwned2)"',
          name: "research",
        },
      ],
    });
