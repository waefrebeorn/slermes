...
const relaySource = readFileSync(
  join(process.cwd(), "src/plugins/hermes-bots/relay.ts"),
  "utf8",
);
const repoRoot = join(process.cwd(), "..", "..");
const configDefaults = readFileSync(
  join(repoRoot, "hermes_cli/config_defaults.py"),
  "utf8",
);
const relayHandler = readFileSync(
  join(repoRoot, "tui_gateway/methods_bot_relay.py"),
  "utf8",
);
...
function tsConstant(name: string): number {
  ...
  expect(
    match,
    `${name} must stay a literal so this test can read it`,
  ).toBeTruthy();
...
describe("bot_relay.deliver budget mirrors", () => {
  it("mirrors the backend turn-lock default", () => {
    ...
    expect(
      lockWaitMatch,
      "bot_mode.turn_wait_seconds default must exist in config_defaults.py",
    ).toBeTruthy();
    expect(tsConstant("RELAY_TURN_LOCK_WAIT_MS")).toBe(
      Number(lockWaitMatch![1]) * 1000,
    );
  });

  it("mirrors the backend per-attempt turn timeout", () => {
    const attemptTimeouts = [...relayHandler.matchAll(/timeout=(\d+)/g)].map(
      (m) => Number(m[1]),
    );

    expect(
      attemptTimeouts.length,
      "expected the attempt and its policy-gated retry",
    ...
    expect(
      new Set(attemptTimeouts).size,
      `attempt timeouts diverged: ${attemptTimeouts}`,
    ).toBe(1);
  ...
  it("keeps the client deadline strictly greater than the backend ceiling", () => {
    ...
    expect(drain).toMatch(
      /'bot_relay\.deliver'[\s\S]{0,400}RELAY_DELIVER_TIMEOUT_MS/,
    );
