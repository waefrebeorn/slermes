...
vi.mock("./routing", () => ({
  ...
  botRosterMeta: () => ({}),
  botWorkspaceOwnerKey: (bot: { name?: string } | null) =>
    String(bot?.name || ""),
...
function respondWith(handler: (method: string) => unknown) {
  requestForBotMock.mockImplementation(
    async (_bot: unknown, method: string) => {
      events.push(method);
...
describe("a title-uniqueness rejection means someone else won the registry", () => {
  it("adopts the racing winner instead of forking into the stray row", async () => {
    ...
    respondWith((method) => {
      ...
      if (method === "session.title") {
        throw new Error(
          "Title 'Bot Chat' is already in use by session winner-1",
        );
      }
  ...
  it("adopts the winner but does not navigate once the user has clicked away", async () => {
    ...
    expect(
      await createCanonicalChat("ops", { openingStillCurrent: () => false }),
    ).toBe("winner-1");
