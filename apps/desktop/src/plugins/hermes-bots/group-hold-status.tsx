...
export function GroupHoldStatus(props: GroupHoldStatusProps) {
  ...
  const heldMembers = props.members.filter((member) =>
    held.has(groupMemberKey(member)),
  );
  ...
  const heldLabels = [
    ...heldMembers.map(props.memberLabel),
    ...unavailableHeldLabels,
  ];
