...
export function previewKind(preview: null | string | undefined): {
  fromBot: null | string;
} {
...
export function botCanonicalSessionId(
  ...
  return (
    bot?.canonical_session?.resolved_id ?? bot?.canonical_session?.id ?? null
  );
}
...
export function workerActiveAt(
  bot: null | RosterRow | undefined,
  now = Date.now(),
): boolean {
...
export function rosterActivityMatches(
  ...
  const recent = Boolean(
    activity && now - activity <= RECENT_ACTIVITY_WINDOW_S * 1000,
  );
