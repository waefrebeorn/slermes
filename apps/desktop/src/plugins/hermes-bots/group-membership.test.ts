...
function rooms(
  map: Record<string, Partial<GroupChat>>,
): Record<string, GroupChat> {
  return Object.fromEntries(
    Object.entries(map).map(([name, room]) => [
      name,
      { log: [], watermarks: {}, ...room },
    ]),
  );
...
describe("room listing", () => {
  it("groupChatNames unions bot-meta groups with room records that carry members or a log", () => {
    ...
    const known = rooms({
      Chatty: {
        log: [
          { at: 5, from: { kind: "user", name: "You" }, id: "1", text: "hi" },
        ],
      },
  ...
  it("groupLastActivity is the newest room-log timestamp, 0 for silence", () => {
    ...
    expect(
      groupLastActivity({ log: [{ at: 3 }, { at: 9 }] } as GroupChat),
    ).toBe(9);
  ...
  it("stripPreviewMarkdown flattens bold, quotes, code and links out of row previews", () => {
    ...
    expect(stripPreviewMarkdown("**Plan**: ship the `thing`")).toBe(
      "Plan: ship the thing",
    );
    expect(stripPreviewMarkdown("> quoted wisdom")).toBe("quoted wisdom");
    expect(stripPreviewMarkdown("see [the doc](https://x.y/z) now")).toBe(
      "see the doc now",
    );
...
describe("seating a room", () => {
  it("seats local meta members plus stored remote descriptors, preferring live rows", () => {
    ...
    expect(members.map((row) => row.name)).toEqual([
      "researcher",
      "builder",
      "spark",
    ]);
  ...
  it("keeps a persisted unreachable source member seated next to a live same-name twin", () => {
    ...
    modules.chat.$groupChats.set(
      rooms({ Research: { log: [], members: [stored] } }),
    );

    const members = modules.membership.groupChatMemberBots(
      "Research",
      [live, stored],
      {
...
describe("legacy display-name descriptors", () => {
  const TAIYI: RosterRow = {
    connectionId: "local",
    display_name: "大司命",
    name: "taiyi",
    remoteSource: false,
  };
  const TESTBOT: RosterRow = {
    connectionId: "local",
    display_name: "",
    name: "testbot",
    remoteSource: false,
  ...
  it("seat their live row once, not as extra ghosts", () => {
    ...
    modules.data.$botMeta.set({
      taiyi: { groups: ["room"] },
      testbot: { groups: ["room"], title: "Testbot" },
    });

    const seated = modules.membership.groupChatMemberBots(
      "room",
      [TAIYI, TESTBOT],
      {
        taiyi: { groups: ["room"] },
  ...
  it("still seat a genuinely unknown descriptor as a degraded ghost", () => {
    modules.chat.$groupChats.set(
      rooms({
        room: {
          log: [],
          members: [{ connectionId: "gone", name: "vanished" }],
        },
      }),
    );

    const seated = modules.membership.groupChatMemberBots("room", [TAIYI], {
      taiyi: { groups: ["room"] },
    });
  ...
  it("resolve a connectionless pre-scoping descriptor against local rows", () => {
    ...
    modules.chat.$groupChats.set(
      rooms({ room: { log: [], members: [{ name: "大司命" }] } }),
    );

    expect(
      modules.membership
        .groupChatMemberBots("room", [TAIYI], {})
        .map((row) => row.name),
    ).toEqual(["taiyi"]);
  });
  ...
  it("never match a friendly name across connections", () => {
    ...
    modules.chat.$groupChats.set(
      rooms({
        room: { log: [], members: [{ connectionId: "local", name: "大司命" }] },
      }),
    );
  ...
  it("pass an exact slug descriptor through untouched", () => {
    ...
    modules.chat.$groupChats.set(
      rooms({ room: { log: [], members: [descriptor] } }),
    );

    expect(modules.membership.groupChatMemberBots("room", [TAIYI], {})[0]).toBe(
      TAIYI,
    );
  });
});
