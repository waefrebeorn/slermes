...
export async function disbandGroupChat(group: string, members: RosterRow[]) {
  ...
  const cleanup = groupDisbandMetadataPlan(
    group,
    members,
    prior,
    $lastRoster.get(),
  ...
  if (cleanup.patches.size) {
    ...
    metadataPersistence = persistBotMetaSnapshot(
      nextMeta,
      botMetaV2Active ||
        Object.keys(nextMeta).some((key) => key.includes("::")),
    );
  }
...
async function renameGroupChat(
  ...
  if (room) {
    migrateGroupComposerDraft(
      groupComposerDraftKey(oldName, room),
      groupComposerDraftKey(next, room),
    );
  }
  ...
  for (const member of members || []) {
    ...
    const groups = [
      ...new Set(botGroups(meta).map((g) => (g === oldName ? next : g))),
    ];
...
export function GroupChatWorkspace({
  ...
  const [composerDraft, setComposerDraft] = useState(() =>
    groupComposerDraftSnapshot(composerKey),
  );
  ...
  const updateComposerDraft = (
    mutate: (draft: GroupComposerDraft) => GroupComposerDraft,
  ) => {
  ...
  const setReplyDrafts = (value: GroupDraftSetter<Record<string, string>>) =>
    updateComposerDraft((current) => ({
      ...current,
      replies:
        typeof value === "function" ? value(current.replies || {}) : value,
    }));
    ...
    updateComposerDraft((current) => ({
      ...
      activeReplyThread:
        typeof value === "function"
          ? value(current.activeReplyThread || null)
          : value,
    }));

  const setPendingImages = (
    value: GroupDraftSetter<Record<string, Attachment[]>>,
  ) =>
    updateComposerDraft((current) => ({
      ...current,
      pendingAttachments:
        typeof value === "function"
          ? value(current.pendingAttachments || {})
          : value,
    }));
  ...
  const imagesFor = (thread: null | string) =>
    pendingImages[thread ?? "main"] || [];
  ...
  const pasteImages = (
    ...
    event.preventDefault();
    void filesToGroupAttachments(files).then((picked) =>
      addImages(thread, picked),
    );
  };
  ...
  const availableMembers = members.filter(
    (member) => botSourceStatus(member).available,
  ).length;
  const availabilityLabel = `${availableMembers} of ${members.length} available`;

  const memberNames =
    members.map((b) => displayName(b, botRosterMeta(b, allMeta))).join(", ") ||
    "No bots in this group chat";
  ...
  const latestActivity = activityEvents.length
  ...
  const stopRoomRun = async () => {
    await stopGroupThread(
      group,
      latestActivity?.thread || null,
      memberDescriptors(),
    );
    host.notify({
      kind: "success",
  ...
  const submit = () => {
    ...
    if (minted) {
      ...
      const restored = restoreGroupComposerDraft(
  ...
  const attachButton = (thread: null | string) => (
    <Button
      className="shrink-0 text-(--ui-text-tertiary) hover:text-foreground"
      onClick={() =>
        void pickGroupAttachments().then((picked) => addImages(thread, picked))
      }
      size="sm"
  ...
  const threads = [...threadsById.values()].sort(
    (a, b) =>
      (a.entries[a.entries.length - 1].entry.at || 0) -
      (b.entries[b.entries.length - 1].entry.at || 0),
  );
  ...
  threads.forEach((threadBucket) => {
    ...
    const head =
      entries.find(({ entry }) => entry.from.kind === "user")?.entry ||
      entries[0].entry;
      ...
      logChildren.push(
        ...
        >
          ...
          <span className="min-w-0 flex-1 truncate">
            {headText || b.group.threadFallback}
          </span>
  ...
  return (
    ...
    >
      ...
      <GroupHoldStatus
        holds={room.holds}
        memberLabel={(member) =>
          displayName(member, botRosterMeta(member, allMeta))
        }
        members={members}
      />
      {activityPanel}
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        <div className="grid gap-1.5 px-2.5 pb-2">
          ...
          {roomClarifies.map((entry) => (
            <GroupClarifyCard
              entry={entry}
              key={`clarify:${entry.memberKey}:${entry.requestId}`}
              members={members}
            />
          ))}
          {room.running ? (
