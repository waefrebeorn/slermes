...
vi.mock("./group-chat", async () => {
  ...
  return {
    $groupChats: atom({}),
    $groupChatWorkspace: atom<null | string>(null),
  };
});
...
describe("new activity after the seed", () => {
  it("marks the canonical chat unread through core’s store", async () => {
    ...
    expect(markUnreadMock).toHaveBeenCalledWith(
      "researcher-chat",
      "researcher",
    );
  });
  ...
  it("titles a bot-to-bot delivery differently from ordinary activity", async () => {
    ...
    trackInboundActivity([
      chatting(
        "researcher",
        6000,
        "Message from 🤖 manager (@manager): ship it",
      ),
    ]);
  ...
  it("keeps marking a roster-hidden bot but never toasts it", async () => {
    const { $activityToasts, $botMeta, trackInboundActivity } =
      await loadActions();
