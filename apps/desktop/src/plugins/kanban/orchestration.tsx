...
function ProfilePicker({
  ...
  return (
    <label className="flex min-w-0 flex-col gap-1">
      ...
      <Select
        onValueChange={(name) => onSave(name === DEFAULT_SENTINEL ? "" : name)}
  ...
  const auto = useMutation({
    ...
    onSuccess: (result) => {
      if (result.ok) {
        ...
        host.notify({
          kind: "warning",
          message: result.reason || "Auto-describe failed",
        });
      }
    },
  });
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="w-24 shrink-0 truncate text-[0.75rem] font-medium text-(--ui-text-secondary)">
        {profile.name}
        {profile.is_default && (
          <span className="ml-1 text-[0.625rem] text-(--ui-text-quaternary)">
            {k.defaultParen}
          </span>
        )}
      </span>
        ...
        <label className="flex cursor-pointer items-center gap-2 pb-1.5 text-[0.75rem] text-(--ui-text-secondary)">
          <Switch
            ...
            onCheckedChange={(checked) =>
              save.mutate({ auto_decompose: checked })
            }
            size="xs"
          />
          {k.autoDecompose}
