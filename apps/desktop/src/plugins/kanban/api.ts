...
function onEventsFrame(slug: string, data: unknown): void {
  ...
  for (const taskId of new Set(
    events.map((event) => event.task_id).filter(Boolean),
  )) {
...
export function bindApi(
  ...
  const open = (slug: string) => {
    close?.();
    close = socket(
      slug ? `/events?board=${encodeURIComponent(slug)}` : "/events",
      (data) => onEventsFrame(slug, data),
    );
  };
...
export const boardKey = (slug: string, archived: boolean) =>
  ["kanban", "board", slug, archived] as const;
export const taskKey = (slug: string, id: string) =>
  ["kanban", "task", slug, id] as const;
export const logKey = (slug: string, id: string) =>
  ["kanban", "log", slug, id] as const;
...
export const fetchBoard = (archived: boolean) =>
  call<KanbanBoard>(
    withBoard("/board", archived ? { include_archived: "true" } : {}),
  );

export const fetchTask = (id: string) =>
  call<KanbanTaskDetail>(withBoard(`/tasks/${id}`));

/** Worker stdout/stderr tail (last 16 KiB — plenty for the drawer). */
export const fetchLog = (id: string) =>
  call<WorkerLog>(withBoard(`/tasks/${id}/log`, { tail: "16384" }));

export const fetchBoards = () => call<BoardsResponse>("/boards");

export const fetchProfiles = () =>
  call<{ profiles: KanbanProfile[] }>("/profiles");

/** First-class Hermes projects, for scoping a board's default workspace. */
export const fetchProjects = () =>
  call<{ projects: KanbanProject[] }>("/projects");

export const fetchOrchestration = () =>
  call<OrchestrationSettings>("/orchestration");
...
export const createTask = (body: Record<string, unknown>) =>
  nudged(
    call<{ task: KanbanTask | null; warning?: string }>(withBoard("/tasks"), {
      method: "POST",
      body,
    }),
  );
...
export const deleteTask = (id: string) =>
  nudged(call(withBoard(`/tasks/${id}`), { method: "DELETE" }));
...
export const bulkTasks = (ids: string[], patch: Record<string, unknown>) =>
  nudged(
    call<{ results: Array<{ id: string; ok: boolean; error?: string }> }>(
      withBoard("/tasks/bulk"),
      {
        method: "POST",
        body: { ids, ...patch },
      },
...
export const addComment = (id: string, body: string) =>
  call(withBoard(`/tasks/${id}/comments`), {
    method: "POST",
...
export const reassignTask = (id: string, profile: string) =>
  nudged(
    call(withBoard(`/tasks/${id}/reassign`), {
      method: "POST",
...
export const estimateTask = (id: string) =>
  call<TaskEstimate>(withBoard(`/tasks/${id}/estimate`), {
    method: "POST",
    body: {},
  });
...
export const updateBoard = (slug: string, patch: Record<string, unknown>) =>
  call<{ board: BoardMeta }>(`/boards/${encodeURIComponent(slug)}`, {
    method: "PATCH",
    body: patch,
  });
...
export const exportBoard = (slug: string, output: string) =>
  call<BoardExportResult>(`/boards/${encodeURIComponent(slug)}/export`, {
    method: "POST",
    body: { output },
  });

export const importBoard = (archive: string) =>
  call<BoardImportResult>("/boards/import", {
    method: "POST",
    body: { archive },
  });

export const nudgeDispatcher = () =>
  call<{ spawned?: unknown[] }>(withBoard("/dispatch"), {
    method: "POST",
    body: {},
  });
...
export const saveProfileDescription = (name: string, description: string) =>
  call(`/profiles/${encodeURIComponent(name)}`, {
    method: "PATCH",
    body: { description },
  });
