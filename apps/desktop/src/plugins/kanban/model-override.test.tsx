...
describe("override label", () => {
  it("reads as inherited until something is pinned", () => {
    ...
    expect(overrideLabel(EMPTY_OVERRIDE, "Profile default")).toBe(
      "Profile default",
    );
  });
  ...
  it("shows provider: model · Effort", () => {
    expect(
      overrideLabel(
        { effort: "high", model: "gemini-3.1-pro", provider: "google" },
        "x",
      ),
    ).toBe("google: gemini-3.1-pro · High");
  });

  it("a depth-only pin is still an override", () => {
    const depthOnly: TaskModelOverride = {
      effort: "ultra",
      model: "",
      provider: "",
    };
    ...
    expect(overrideLabel(depthOnly, "Profile default")).toBe(
...
describe("REST field mapping", () => {
  it("create omits untouched fields", () => {
    ...
    expect(
      overrideCreateFields({ effort: "high", model: "m", provider: "p" }),
    ).toEqual({
      model_override: "m",
      provider_override: "p",
  ...
  it("patch clears explicitly", () => {
    expect(overridePatch(EMPTY_OVERRIDE)).toEqual({
  ...
  it("a provider is never sent without its model", () => {
    expect(
      overrideCreateFields({ effort: "", model: "", provider: "google" }),
    ).toEqual({});
  });
});
