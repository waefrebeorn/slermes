...
function RenameBoardDialog({
  ...
  const save = useBoardWrite(
    () => updateBoard(slug, { name: name.trim() }),
    onClose,
  );
...
function BoardSettingsDialog({
  ...
  const save = useBoardWrite(
    () => updateBoard(slug, { project_id: project }),
    onClose,
  );
  ...
  return (
    <BoardDialog
      ...
      open={Boolean(board)}
      title={
        board ? k.boardSettingsFor(board.name || board.slug) : k.settingsDots
      }
    >
      <ProjectPicker onChange={setProject} value={project} />
    </BoardDialog>
    ...
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            className="h-7 max-w-56 gap-1.5 px-2"
            size="sm"
            variant="ghost"
          >
      ...
      <ConfirmDialog
        ...
        title={
          deleteFor
            ? k.deleteBoardTitle(deleteFor.name || deleteFor.slug)
            : t.common.delete
        }
      />
    </>
