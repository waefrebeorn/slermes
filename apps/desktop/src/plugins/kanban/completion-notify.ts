...
const TERMINAL_NOTIFY = new Map<string, { titleKey: string; toast: ToastKind }>(
  [
    ...
    [
      "block_loop_detected",
      { titleKey: "notify.blockLoopTitle", toast: "warning" },
    ],
...
export function bindCompletionNotify(
  r: Rest,
  pluginTranslate?: PluginTranslate,
  os?: PluginOs,
): void {
  rest = r;
...
async function ensureBaseline(slug: string): Promise<void> {
  ...
  try {
    const board = (await rest!<{ latest_event_id?: unknown }>(
      `/board?board=${encodeURIComponent(slug)}`,
    )) as {
      latest_event_id?: unknown;
    };

    seenEventIdByBoard.set(
      slug,
      typeof board.latest_event_id === "number" ? board.latest_event_id : 0,
    );
  } catch {
