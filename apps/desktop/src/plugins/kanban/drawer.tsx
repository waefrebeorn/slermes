...
function eventText(
  ...
  switch (event.kind) {
    case "created":
      return {
        label: k.evtCreated(col("status") ?? "", str("assignee") ?? ""),
      };
    case "status": {
      ...
      return {
        ...
        detail:
          reason === "parent_reopened"
            ? k.evtParentReopened(str("parent") ?? "")
            : (reason ?? undefined),
      };
    }
    ...
    case "spawned":
      return {
        label: k.evtWorkerStarted,
        detail: p.pid != null ? `pid ${p.pid}` : undefined,
      };
  ...
  const act = (action: DiagnosticAction) => {
    if (action.kind === "reclaim") {
      ...
      void navigator.clipboard.writeText(
        String(action.payload?.command ?? action.label),
      );
    ...
    <div className="flex flex-col gap-2">
      {items.map((diag) => {
        ...
        const actions = diag.actions.filter(
          (action) => action.kind === "reclaim" || action.kind === "cli_hint",
        );
        ...
        {(roster?.profiles ?? []).map((profile) => (
          ...
          >
            ...
            {profile.name === current && (
              <Codicon className="ml-auto" name="check" size="0.8rem" />
            )}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="relative">
        <Textarea
          className={cn(
            "field-sizing-content max-h-40 min-h-0 resize-none",
            running ? "pr-[3.5rem]" : "pr-[5rem]",
          )}
...
const isAdminSummary = (summary: string) =>
    ...
    <Section
      action={
        <>
          ...
          >
            <Codicon
              name={pending ? "sync" : "cloud-upload"}
              size="0.8rem"
              spinning={pending}
            />
          </Button>
    ...
    <Section label={k.estimate}>
      {result?.ok ? (
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Codicon
              name={est.isPending ? "loading" : "dashboard"}
              size="0.75rem"
              spinning={est.isPending}
            />
      ...
      <header className="flex flex-col gap-2 px-4 pt-3.5 pb-3">
        <div className="flex items-center gap-2">
          {task ? (
            ...
            <span className="font-mono text-sm text-(--ui-text-tertiary)">
              {shortId(id)}
            </span>
          )}
          {task && (
        ...
        {errorMessage ? (
          ...
          <div className="flex flex-col gap-4 text-sm">
            <div className="grid grid-cols-[6rem_minmax(0,1fr)] gap-x-3 gap-y-1 text-[0.71rem]">
              <MetaRow label={k.assignee}>
                <AssigneeMenu
                  ...
                  onReassign={(profile) =>
                    void mutate(() => reassignTask(task.id, profile))()
                  }
                />
              </MetaRow>
              {typeof task.priority === "number" && (
                <MetaRow label={k.metaPriority}>{task.priority}</MetaRow>
              )}
              {task.tenant && (
                <MetaRow label={k.metaTenant}>{task.tenant}</MetaRow>
              )}
              {task.workspace_path && (
              ...
              <MetaRow label={k.model}>
                <ModelOverrideField
                  onChange={(next) =>
                    void mutate(() => patchTask(task.id, overridePatch(next)))()
                  }
                  value={{
              ...
              {task.created_by && (
                <MetaRow label={k.metaCreatedBy}>{task.created_by}</MetaRow>
              )}
              {ago(task.created_at) && (
                <MetaRow label={k.metaCreated}>{ago(task.created_at)}</MetaRow>
              )}
              {running && task.worker_pid ? (
                <MetaRow label={k.metaWorkerPid}>{task.worker_pid}</MetaRow>
              ) : null}
            </div>

            {task.status === "ready" && !task.assignee && !defaultAssignee && (
              <Callout
                title={k.readyUnassignedTitle}
            ...
            <DescriptionSection
              body={task.body}
              onSave={(body) =>
                void mutate(() => patchTask(task.id, { body }))()
              }
            />
            ...
            {(detail.links.parents.length > 0 ||
              detail.links.children.length > 0) && (
                  ...
                  <ul className="flex flex-col gap-1.5">
                    {detail.runs.map((run) => {
                      const failed = [
                        ...
                        "timed_out",
                        "gave_up",
                      ].includes(run.outcome ?? run.status);
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <Badge
                              size="xs"
                              variant={failed ? "destructive" : "muted"}
                            >
