...
export const en: KanbanMessages = {
  ...
  col: {
    ...
    scheduled: {
      label: "Scheduled",
      help: "Waiting for a scheduled time to arrive.",
    },
    ready: {
      label: "Ready",
  ...
  locked: {
    review:
      "Review is entered by the dispatcher when a review agent takes the card.",
    running: "Running is set by the dispatcher when a worker claims the card.",
    scheduled:
      "Scheduled needs a wake-up time — agents set it; it can’t be dragged into.",
  },
  arcRunning: "An agent is working on this now.",
  arcStale:
    "Claimed, but no worker heartbeat for 2+ minutes — the dispatcher will reclaim it.",
  title: "Kanban",
  ...
  attachedTip: (name) =>
    `${name} is attached — the dispatcher hands this over on its next tick (≤1m).`,
  orchestratorTip: (name) =>
    `${name} (the orchestrator) picks this up on the next tick and writes the spec.`,
  autoAssignTip: (name) =>
    `Auto-assigns to “${name}” (kanban.default_assignee) on the next dispatch tick.`,
  wontRun: "won't run",
  wontRunTip:
  ...
  bulkFailed: (failed, total, err) =>
    `${failed} of ${total} failed — ${err}. Failed cards stay selected.`,
  ...
  modelHint:
    "Runs this task on a specific model and thinking depth. Unset uses the assigned profile’s own.",
  workspaceInherit: "Inherits the board’s project directory",
  workspaceInheritDir: (dir) => `Leave empty to inherit ${dir}`,
  workspaceInheritGeneric:
  ...
  estimateTip:
    "Rough token + complexity estimate from the auxiliary model — makes a model call.",
  estimateTipLong:
...
const ja: KanbanMessages = {
  ...
  col: {
    triage: {
      label: "トリアージ",
      help: "生のアイデア — スペシファイアが仕様に整えます。",
    },
    todo: { label: "Todo", help: "依存関係の待ち、または未割り当て。" },
    scheduled: { label: "スケジュール", help: "予定時刻を待っています。" },
    ready: {
      label: "Ready",
      help: "依存関係が解決済み — プロフィールを割り当てるとディスパッチャが実行します。",
    },
    running: {
      label: "実行中",
      help: "ワーカーが取得済み — エージェントが作業中。ディスパッチャが設定します。",
    },
    blocked: {
      label: "ブロック",
      help: "ワーカーが人間の入力を求めています。",
    },
    review: {
      label: "レビュー",
      help: "レビューエージェントが作業を確認中。ディスパッチャが設定します。",
    },
    done: {
      label: "完了",
      help: "完了。依存する子タスクが Ready になります。",
    },
    archived: {
      label: "アーカイブ",
      help: "デフォルトのボード表示から非表示。",
    },
  },
  locked: {
    review:
      "レビューは、レビューエージェントがカードを取得するとディスパッチャによって設定されます。",
    running:
      "実行中は、ワーカーがカードを取得するとディスパッチャによって設定されます。",
    scheduled:
      "スケジュールには起動時刻が必要です — エージェントが設定します。ドラッグでは移動できません。",
  },
  arcRunning: "エージェントが現在作業中です。",
  arcStale:
    "取得済みですが、2分以上ワーカーのハートビートがありません — ディスパッチャが再取得します。",
  title: "カンバン",
  ...
  attachedTip: (name) =>
    `${name} が担当 — ディスパッチャが次のティック（≤1分）で引き渡します。`,
  orchestratorTip: (name) =>
    `${name}（オーケストレーター）が次のティックでこれを取得し、仕様を書きます。`,
  autoAssignTip: (name) =>
    `次のディスパッチティックで「${name}」（kanban.default_assignee）に自動割り当てされます。`,
  wontRun: "実行されません",
  wontRunTip:
  ...
  refused: "拒否されました",
  bulkFailed: (failed, total, err) =>
    `${total} 件中 ${failed} 件が失敗 — ${err}。失敗したカードは選択されたままです。`,
  ...
  estimateTip:
    "補助モデルによるトークン数と複雑度の概算 — モデル呼び出しを行います。",
  estimateTipLong:
    "補助モデルを呼び出してトークン数と複雑度を概算します。目安であり、請求ではありません。",
  roughEstimate: "概算",
  tokUnit: "tok",
  ...
  boardImportedAs: (slug) =>
    `その名前は使用中のため ${slug} としてインポートしました`,
  renameBoardTitle: "ボード名を変更",
  deleteBoardTitle: (name) => `「${name}」を削除しますか？`,
  deleteBoardConfirm:
    "ボードは消去されずアーカイブされます。タスクと添付ファイルはディスクに残り、復元できます。",
...
const zh: KanbanMessages = {
  ...
  col: {
    ...
    ready: {
      label: "就绪",
      help: "依赖已满足 — 分配一个配置档，调度器即会运行它。",
    },
    running: {
      label: "运行中",
      help: "已被工作单元领取 — 有代理在处理。由调度器设置。",
    },
  ...
  attachedTip: (name) =>
    `${name} 已接手 — 调度器将在下一个周期（≤1 分钟）移交。`,
  orchestratorTip: (name) => `${name}（编排者）将在下一个周期领取并撰写规格。`,
  autoAssignTip: (name) =>
    `将在下一个调度周期自动分配给“${name}”（kanban.default_assignee）。`,
  wontRun: "不会运行",
  wontRunTip:
    "就绪卡片只有在分配了配置档后才会运行。打开卡片设置负责人，或在编排设置中配置默认负责人。",
  noHeartbeat: "无心跳",
  ...
  refused: "被拒绝",
  bulkFailed: (failed, total, err) =>
    `${total} 个中有 ${failed} 个失败 — ${err}。失败的卡片仍保持选中。`,
  ...
  modelClear: "清除模型覆盖",
  modelHint:
    "让该任务使用指定的模型与思考深度。未设置时使用所指派配置文件自身的设置。",
  ...
  commentsHelp:
    "评论会添加到任务讨论串。工作单元领取任务时会将其作为上下文的一部分读取。",
  send: "发送",
  comment: "评论",
...
const zhHant: KanbanMessages = {
  ...
  col: {
    ...
    ready: {
      label: "就緒",
      help: "相依項目已滿足 — 指派一個設定檔，排程器便會執行它。",
    },
    running: {
      label: "執行中",
      help: "已被工作單元領取 — 有代理在處理。由排程器設定。",
    },
  ...
  attachedTip: (name) =>
    `${name} 已接手 — 排程器將在下一個週期（≤1 分鐘）移交。`,
  orchestratorTip: (name) => `${name}（編排者）將在下一個週期領取並撰寫規格。`,
  autoAssignTip: (name) =>
    `將在下一個排程週期自動指派給「${name}」（kanban.default_assignee）。`,
  wontRun: "不會執行",
  wontRunTip:
    "就緒卡片只有在指派了設定檔後才會執行。開啟卡片設定負責人，或在編排設定中設定預設負責人。",
  noHeartbeat: "無心跳",
  ...
  refused: "被拒絕",
  bulkFailed: (failed, total, err) =>
    `${total} 個中有 ${failed} 個失敗 — ${err}。失敗的卡片仍保持選取。`,
  ...
  modelClear: "清除模型覆寫",
  modelHint:
    "讓此任務使用指定的模型與思考深度。未設定時使用所指派設定檔本身的設定。",
  ...
  estimateTipLong:
    "快速呼叫輔助模型來估算 token 數與複雜度。僅供參考，並非帳單。",
  roughEstimate: "粗略估算",
  tokUnit: "tok",
  ...
  commentsHelp:
    "留言會加入任務討論串。工作單元領取任務時會將其作為脈絡的一部分讀取。",
  send: "傳送",
  comment: "留言",
...
export const columnLabel = (k: KanbanText, name: string) =>
  k.col[name as keyof KanbanText["col"]]?.label ?? name;
export const columnHelp = (k: KanbanText, name: string) =>
  k.col[name as keyof KanbanText["col"]]?.help ?? "";
export const lockedReason = (k: KanbanText, name: string) =>
  k.locked[name as keyof KanbanText["locked"]] ?? "";
