...
int cgw_u_read_systemd_unit_environment(const char *arg) {
    /* Python: Environment= parse. Arg = "state\tresult". */
    if (!arg || !*arg) { printf("{}\n"); return 0; }
    const char *tab = strchr(arg, '\t');
    int state = arg[0] == '1';
    if (!state) { printf("{}\n"); return 0; }
...
int cgw_u_find_legacy_hermes_units(const char *arg) {
    /* Python: allowlist scan. Arg = "count\tstate\tresult". */
    if (!arg || !*arg) { printf("[]\n"); return 0; }
    const char *t1 = strchr(arg, '\t');
    const char *t2 = t1 ? strchr(t1 + 1, '\t') : NULL;
    int state = t1 && t1[1] == '1';
...
int cgw_u_remap_path_for_user(const char *arg) {
    /* Python: home prefix swap. Arg = "path\thome\ttarget\tresult". */
    if (!arg || !*arg) { printf("\n"); return 0; }
    const char *t1 = strchr(arg, '\t');
    const char *t2 = t1 ? strchr(t1 + 1, '\t') : NULL;
    const char *t3 = t2 ? strchr(t2 + 1, '\t') : NULL;
    ...
    const char *target = t2 ? t2 + 1 : "";
    size_t hlen = strlen(home);
    if (hlen && strncmp(path, home, hlen) == 0 && (path[hlen] == '/' || path[hlen] == '\0')) {
        printf("%s/%s\n", target, path + hlen);
        return 0;
    }
    printf("%s\n", path);
