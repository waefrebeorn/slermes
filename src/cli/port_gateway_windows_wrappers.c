...
int gw_u_print_deep_probes(const char *arg) {
    /* Python: 6-signal block. Arg =
     * "passes\tstate\tresult". */
    if (!arg || !*arg) { printf("\n"); return 0; }
    const char *t1 = strchr(arg, '\t');
    const char *t2 = t1 ? strchr(t1 + 1, '\t') : NULL;
    int state = t1 && t1[1] == '1';
    if (!state) { printf("\n"); return 0; }
    printf("Deep probes: %s/6 PASS ([1] pid file [2] lock held [3] get_running_pid [4] _pid_exists [5] state json fresh [6] exit-diag log)\n", t2 ? t2 + 1 : "?");
    return 0;
}
