...
> Generated from a real full scan (last refreshed 2026-07-22):
...
- **9733** total Python features tracked across **484** modules.
- **6357 PORTED**, **0 PARTIAL**, **3376 REAL_GAP**, 0 STUB, 0 NA.
- **484 modules** have ≥1 REAL_GAP. NEVER-STARTED count is now folded into the
  per-module gaps (the original 76 "never started" modules are all REAL_GAP
  work — see doctrine below).

> The live totals are regenerated by `make parity-walkway` (single source of
> truth). Re-run `python3 tests/slermes_parity_battleground.py --json` before
> trusting any count; do NOT hand-maintain these numbers.
